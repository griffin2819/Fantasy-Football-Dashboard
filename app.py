...
def _sim_opponent_pick(avail, opp_names, board, rnd, overall, randomness, rng):
    """Fast opponent pick with realistic one-QB/TE/K/DL/DB needs."""
    if avail.empty:
        return None
    a=avail.copy()
    fallback=pd.to_numeric(a.get("model_rank",999),errors="coerce").fillna(999)
    market=pd.to_numeric(a.get("consensus_rank",np.nan),errors="coerce").fillna(fallback)
    a["_opp_score"]=market+rng.normal(0,max(float(randomness),1.0),len(a))

    opp_roster=board[board.player.isin(opp_names)] if opp_names else board.iloc[0:0]
    oc=opp_roster.position.value_counts().to_dict() if len(opp_roster) else {}

    if rnd<=7:
        a.loc[a.position.isin(["DL","DB"]),"_opp_score"]+=25.0
    if rnd>=9 and oc.get("DL",0)<1:
        a.loc[a.position.eq("DL"),"_opp_score"]-=7.0
    if rnd>=10 and oc.get("DB",0)<1:
        a.loc[a.position.eq("DB"),"_opp_score"]-=7.0
    if rnd>=14 and oc.get("K",0)<1:
        a.loc[a.position.eq("K"),"_opp_score"]-=8.0

    if oc.get("QB",0)>=1: a.loc[a.position.eq("QB"),"_opp_score"]+=12.0
    if oc.get("TE",0)>=1: a.loc[a.position.eq("TE"),"_opp_score"]+=8.0
    if oc.get("K",0)>=1: a.loc[a.position.eq("K"),"_opp_score"]+=100.0
    if oc.get("DL",0)>=1: a.loc[a.position.eq("DL"),"_opp_score"]+=5.0
    if oc.get("DB",0)>=1: a.loc[a.position.eq("DB"),"_opp_score"]+=5.0

    return a.sort_values("_opp_score").iloc[0]


def simulate_current_fantasy_edge_once(board, teams, slot, rounds, slots, randomness, seed):
    """Automated draft using the SAME current Fantasy Edge user-pick pipeline as Mock/Live."""
    rng=np.random.default_rng(int(seed))
    drafted=set()
    user_rows=[]
    opp_rosters={i:[] for i in range(1,int(teams)+1)}

    for overall in range(1,int(teams)*int(rounds)+1):
        rnd=max(1,(overall-1)//int(teams)+1)
        owner=_snake_owner(overall,int(teams))
        avail=board[~board.player.astype(str).isin(drafted)].copy()
        if avail.empty:
            break

        if int(owner)==int(slot):
            roster=pd.DataFrame(user_rows) if user_rows else board.iloc[0:0].copy()
            ranked,nxt=prepare_user_draft_candidates(
                avail,roster,rnd,overall,int(slot),int(teams),slots,int(randomness)
            )
            if ranked.empty:
                continue
            ready=ranked[ranked.get("timing_ready",False).fillna(False).astype(bool)].copy()
            pool=ready if len(ready) else ranked
            score_col="execution_score" if "execution_score" in pool.columns else "evaluation_score"
            choice=pool.sort_values(score_col,ascending=False).iloc[0].copy()
            row=choice.to_dict()
            row["mock_round"]=rnd
            row["mock_pick"]=overall
            user_rows.append(row)
            drafted.add(str(choice.player))
        else:
            choice=_sim_opponent_pick(
                avail,opp_rosters.get(int(owner),[]),board,rnd,overall,randomness,rng
            )
            if choice is not None:
                drafted.add(str(choice.player))
                opp_rosters.setdefault(int(owner),[]).append(str(choice.player))

    roster=pd.DataFrame(user_rows)
    if roster.empty:
        return None

    # Grade with the current Fantasy Edge grading framework.
...
with tabs[2]:
    ...
    mode=st.radio("Test mode",["Interactive mock","Automated 1,000-draft test"],horizontal=True)
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🧪 Automated 1,000-Draft Test")
        st.caption(
        ...
        batch_size=st.selectbox(
            ...
            index=2,
            help="Results persist in this session. Larger batches take longer on Streamlit Cloud."
        )
        ...
        a,b=st.columns(2)
        run=a.button(
            "▶️ Run / continue 1,000-draft test",
            type="primary",
        ...
        if run:
            ...
            for j in range(this_batch):
                ...
                if (j+1)==1 or (j+1)%5==0 or (j+1)==this_batch:
                ...
                if (time.perf_counter()-started)>25.0 and j+1<this_batch:
                    status.warning(
                        "Runtime safety cutoff reached. Completed results were saved — press "
                        "Run / continue to resume the 1,000-draft test."
                    )
                    break
        ...
        if len(sim_df):
            ...
            if len(sim_df)>=target_sims:
                st.success(
                    "1,000-draft test complete. Use these distributions to tune Fantasy Edge "
