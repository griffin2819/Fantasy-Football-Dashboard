...
def _market_pick_series_capped_idp(df, teams):
    """V7.57 challenger-only market transform.

    The legacy transform maps IDP ECR onto an approximate overall scale:
      teams*8 + (IDP_ECR-1)*2
    V7.56 showed this can inflate a valid consensus rank of 150 to market pick 394.

    Challenger integrity fix:
    - preserve the existing transform for normal IDP ranks;
    - cap transformed IDP market pick at raw consensus rank + 96 picks
      (eight 12-team rounds), preventing multi-hundred-pick inflation while
      still keeping IDP later than its page ECR.
    """
    cr=pd.to_numeric(df["consensus_rank"],errors="coerce")
    out=cr.copy()
    idp=df["position"].isin(["DL","DB"])
...
def _fast_benchmark_pool(board, teams=12, idp_market_integrity_fix=False):
    ...
    market_pick=(_market_pick_series_capped_idp(pool,teams) if bool(idp_market_integrity_fix) else _market_pick_series(pool,teams)).to_numpy(dtype=float)
...
st.title("🏈 Fantasy Edge V7.57 — IDP Market-Pick Integrity Fix Challenger")
st.caption("V7.57 • V7.50 champion frozen • IDP market-pick integrity challenger")
st.caption("Build ID: V7.57-IDPMARKETFIX-20260816")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v757_live_slot")
    ...
    if "v757_target_queue" not in st.session_state:
        st.session_state.v757_target_queue={}
    queue=st.session_state.v757_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v757_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.57 keeps V7.50 as the live/default champion with the 55.97 rule unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.57 robustness report")
        ...
        st.markdown("### 🧪 V7.57 champion baseline validation")
        st.info("V7.57 is challenger-only. The IDP market-pick integrity fix is off in the live/default champion and runs only in matched validation.")
        ...
        if "v757_independent_results" not in st.session_state:
            st.session_state.v757_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v757_independent_results"),pd.DataFrame) else "ready to run"))
        if "v757_independent_summary" not in st.session_state:
            st.session_state.v757_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v757_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v757_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v757_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v757_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v757_independent_results=iv
                st.session_state.v757_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v757_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v757_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.57 Champion Baseline Check")
            st.caption("V7.57 keeps V7.50 fully frozen and tests a narrow challenger that corrects pathological IDP market-pick inflation while leaving all player valuation and execution architecture untouched.")
            ...
            st.download_button(
                "⬇️ Download V7.57 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_57_champion_baseline.csv",
                "text/csv",
                key="v757_baseline_download"
            )
        ...
        st.markdown("### 🧪 V7.57 — IDP Market-Pick Integrity Fix Challenger")
        st.caption(
            "Challenger only. V7.50 remains frozen/live. V7.57 changes only the IDP market-pick transformation used by the benchmark engine. "
            "Valid DL/DB consensus ranks can no longer be inflated by more than eight 12-team rounds beyond raw consensus. "
            "Player valuation, VORP, roster utility, 55.97 Pick-12 rule, execution logic and all offensive market ranks remain unchanged."
        )

        if "v757_results" not in st.session_state: st.session_state.v757_results=None
        sims=st.number_input(
            "V7.57 matched simulations per slot × room",
            min_value=10,max_value=40,value=20,step=5,key="v757_sims",
            help="Default: 4 slots × 5 rooms × 20 simulations = 400 fresh matched champion-vs-fix sets."
        )
        a,b=st.columns(2)
        run=a.button("Run IDP market-integrity challenger",type="primary",key="v757_run")
        clear=b.button("Clear V7.57 challenger",key="v757_clear")

        if clear:
            st.session_state.v757_results=None
            st.rerun()
        ...
        if run:
            ...
            fast_old=_fast_benchmark_pool(board,int(teams),idp_market_integrity_fix=False)
            fast_fix=_fast_benchmark_pool(board,int(teams),idp_market_integrity_fix=True)
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=957000+int(ds)*10000+j*149+sum(ord(c) for c in room_name)

                        champ=simulate_mock_fast(
                            fast_old,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name
                        )
                        cand=simulate_mock_fast(
                            fast_fix,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name
                        )
                        ...
                        # Round-15 behavior and market-price comparison.
                        c15=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(15)]
                        n15=cand[pd.to_numeric(cand.mock_round,errors="coerce").eq(15)]
                        champ15=str(c15.iloc[0].player) if len(c15) else ""
                        cand15=str(n15.iloc[0].player) if len(n15) else ""

                        def _late_reach(d):
                            x=d[pd.to_numeric(d.mock_round,errors="coerce").fillna(0)>=11].copy()
                            if not len(x): return np.nan
                            mp=pd.to_numeric(x.get("market_pick",x.get("consensus_rank")),errors="coerce")
                            pk=pd.to_numeric(x.mock_pick,errors="coerce")
                            return np.maximum(mp-pk,0).mean()
                        ...
                        rows.append({
                            ...
                            "R1_3_change":float(np_.loc["R1-3","Avg_market_delta"])-float(cp.loc["R1-3","Avg_market_delta"]),
                            "R4_6_change":float(np_.loc["R4-6","Avg_market_delta"])-float(cp.loc["R4-6","Avg_market_delta"]),
                            "R7_10_change":float(np_.loc["R7-10","Avg_market_delta"])-float(cp.loc["R7-10","Avg_market_delta"]),
                            "R11_plus_change":r11,
                            "Champion_late_reach":_late_reach(champ),
                            "Candidate_late_reach":_late_reach(cand),
                            "Late_reach_change":_late_reach(cand)-_late_reach(champ),
                            "Behavior_changed":changed>0,
                            "Changed_picks":changed,
                            "Champion_round15_player":champ15,
                            "Candidate_round15_player":cand15,
                            "Round15_changed":champ15!=cand15,
                            "Conditional_overall_win":(overall>0) if changed else np.nan,
                            "Conditional_nonloss":(overall>=0) if changed else np.nan,
                            "Conditional_R11_win":(r11>0) if changed else np.nan,
                        })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%80==0 or done==total:
                            status.caption(f"V7.57 challenger: {done}/{total} fresh matched sets")

            df=pd.DataFrame(rows)
            st.session_state.v757_results=df
            status.success(f"V7.57 complete: {len(df)} matched sets.")
            prog.empty()

        df=st.session_state.get("v757_results")
        if isinstance(df,pd.DataFrame) and len(df):
            changed=df[df.Behavior_changed].copy()
            ...
            reach=df.Late_reach_change.mean()
            activation=df.Behavior_changed.mean()
            r15=df.Round15_changed.mean()
            cond=changed.Conditional_overall_win.mean() if len(changed) else np.nan
            nonloss=changed.Conditional_nonloss.mean() if len(changed) else np.nan
            cond11=changed.Conditional_R11_win.mean() if len(changed) else np.nan

            c1,c2,c3,c4=st.columns(4)
            c1.metric("Overall vs V7.50",f"{overall:+.3f}")
            c2.metric("R11+ vs V7.50",f"{r11:+.3f}")
            c3.metric("Late reach change",f"{reach:+.2f}")
            c4.metric("Conditional overall win",f"{cond:.1%}" if pd.notna(cond) else "n/a")

            summary=pd.DataFrame([{
                "Matched sets":len(df),
                "Changed sets":len(changed),
                "Behavior activation":activation,
                "Round-15 changed":r15,
                "Overall change":overall,
                "R1-3 change":r13,
                "R4-6 change":r46,
                "R7-10 change":r710,
                "R11+ change":r11,
                "Late reach change":reach,
                "Conditional overall win":cond,
                "Conditional overall non-loss":nonloss,
                "Conditional R11+ win":cond11,
            }])
            ...
            slot=df.groupby("Draft_slot",as_index=False).agg(
                ...
                Late_reach_change=("Late_reach_change","mean"),
                Activation=("Behavior_changed","mean"),
                Round15_change=("Round15_changed","mean")
            )
            st.markdown("#### Integrity fix by draft slot")
            ...
            st.markdown("#### Integrity fix by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            # Show directly how the market transformation changes the problematic high-IDP tail.
            p_old=fast_old["pool"].copy()
            p_old["Old_market_pick"]=fast_old["market_pick"]
            p_old["Fixed_market_pick"]=fast_fix["market_pick"]
            p_old["Market_reduction"]=p_old["Old_market_pick"]-p_old["Fixed_market_pick"]
            affected=p_old[
                p_old.position.isin(["DL","DB"])
                & np.isfinite(p_old.Old_market_pick)
                & (p_old.Market_reduction>0)
            ][["player","position","consensus_rank","Old_market_pick","Fixed_market_pick","Market_reduction","model_rank","projection","vorp","draft_score"]]
            st.markdown("#### IDP market ranks changed by the integrity fix")
            st.dataframe(
                affected.sort_values("Market_reduction",ascending=False).head(30).round(3),
                use_container_width=True,hide_index=True
            )

            pos_slots=int((slot.Overall_change>0).sum())
            pos_rooms=int((room.Overall_change>0).sum())
            worst_slot=float(slot.Overall_change.min())
            worst_room=float(room.Overall_change.min())

            screen=(
                overall>0
                and r11>0
                and min(r13,r46,r710)>-.05
                and reach<0
                and activation>=.05
                and pd.notna(cond) and cond>=.55
                and pd.notna(nonloss) and nonloss>=.70
                and pos_slots>=3
                and pos_rooms>=4
                and worst_slot>-.10
                and worst_room>-.10
            ...
            gate=pd.DataFrame([{
                "Screen passed":screen,
                "Overall change":overall,
                "R11+ change":r11,
                "R1-10 protected":min(r13,r46,r710)>-.05,
                "Late reach reduced":reach<0,
                "Conditional overall win":cond,
                "Conditional non-loss":nonloss,
                "Positive slots":pos_slots,
                "Positive rooms":pos_rooms,
                "Worst slot":worst_slot,
                "Worst room":worst_room,
            }])
            st.markdown("#### V7.57 screening gate")
            st.dataframe(gate.round(4),use_container_width=True,hide_index=True)

            if screen:
                st.success(
                    "V7.57 SCREEN PASSED. Correcting pathological IDP market-pick inflation improves whole-draft and R11+ performance while protecting R1-10. "
                    "Move this exact transformation into a larger V7.58 validation before promotion."
                )
            else:
                st.warning(
                    "V7.57 does not clear screening. Do not promote the market-rank fix yet; use the affected-player table to identify whether the cap is too broad or the problem is actually terminal roster utility."
                )

            st.download_button(
                "⬇️ Download V7.57 IDP market-integrity challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_57_idp_market_pick_integrity_challenger.csv",
                "text/csv",key="v757_download"
            )
