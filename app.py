        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                local_choice,wr_scarcity,wr_diag=early_wr_scarcity_choice(
                    eval_score,cr,pos[sidx],overall,nxt,local_choice,teams=int(teams),randomness=randomness
                )
...
st.title("🏈 Fantasy Edge V7.22 — Early-WR Scarcity Correction")
st.caption("V7.22 • V7.16 stable base • conservative R1-3 WR scarcity correction • no WR quota")
st.caption("Build ID: V7.22-EWRSC-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v722_live_slot")
    ...
    if "v722_target_queue" not in st.session_state:
        st.session_state.v722_target_queue={}
    queue=st.session_state.v722_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v722_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.22 keeps V7.16 projections/evaluation frozen. R1-3 only: a WR can intercept when within 8 evaluation points, market-priority, and ≤52% likely to survive.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.22 robustness report")
        ...
        st.markdown("### 🧪 V7.22 Early-WR validation")
        ...
        if "v722_independent_results" not in st.session_state:
            st.session_state.v722_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v722_independent_results"),pd.DataFrame) else "ready to run"))
        if "v722_independent_summary" not in st.session_state:
            st.session_state.v722_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v722_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v722_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v722_run")
        clear_iv=clear_col.button("Clear independent results",key="v722_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v722_independent_results=iv
                st.session_state.v722_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v722_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v722_independent_summary") or {}
            st.success(
            ...
            st.download_button(
                "⬇️ Download V7.22 Early-WR validation CSV",
                iv_csv,
                "fantasy_edge_v7_22_early_wr_validation.csv",
                "text/csv",
                key="v722_download"
            )
