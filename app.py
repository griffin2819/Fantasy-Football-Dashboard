...
def _position_depth_after_pick(roster, pos):
    counts=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    counts=dict(counts)
    counts[str(pos)]=int(counts.get(str(pos),0))+1
    return counts


def _tier_cliff_value(x, candidate_idx):
    """Estimate how much quality is lost if this positional tier is skipped."""
    try:
        pos=str(x.at[candidate_idx,"position"])
        pool=x[x["position"].astype(str).eq(pos)].copy()
        if len(pool)<2:
            return 0.0
        pool=pool.sort_values("evaluation_score",ascending=False)
        here=np.where(pool.index.to_numpy()==candidate_idx)[0]
        if len(here)==0:
            return 0.0
        i=int(here[0])
        if i>=len(pool)-1:
            return 0.0
        cur=float(pd.to_numeric(pd.Series([pool.iloc[i]["evaluation_score"]]),errors="coerce").fillna(0).iloc[0])
        nxt=float(pd.to_numeric(pd.Series([pool.iloc[i+1]["evaluation_score"]]),errors="coerce").fillna(0).iloc[0])
        return float(np.clip(cur-nxt,0.0,10.0))
    except Exception:
        return 0.0


def _next_turn_replacement_loss(x, candidate_idx, current_pick, next_pick):
    """
    Estimate take-now vs likely next-turn replacement value at the same position.
    Uses market survival and the next best likely survivor.
    """
    try:
        pos=str(x.at[candidate_idx,"position"])
        current_eval=float(pd.to_numeric(pd.Series([x.at[candidate_idx,"evaluation_score"]]),errors="coerce").fillna(0).iloc[0])
        pool=x[x["position"].astype(str).eq(pos)].copy()
        if pool.empty:
            return 0.0

        market=pd.to_numeric(
            pool["market_pick"] if "market_pick" in pool.columns else pool.get("consensus_rank",np.nan),
            errors="coerce"
        ).fillna(float(next_pick)+20)

        # Candidates whose market slot is at/after our next turn are plausible survivors.
        survivors=pool[market>=float(next_pick)].copy()
        survivors=survivors.drop(index=candidate_idx,errors="ignore")
        if len(survivors):
            repl=float(pd.to_numeric(survivors["evaluation_score"],errors="coerce").max())
        else:
            # If nobody likely survives, compare to the next-best positional option.
            others=pool.drop(index=candidate_idx,errors="ignore")
            repl=float(pd.to_numeric(others["evaluation_score"],errors="coerce").max()) if len(others) else current_eval-8.0

        return float(np.clip(current_eval-repl,0.0,12.0))
    except Exception:
        return 0.0


def _starter_flex_improvement(pos, counts, slots):
    """Continuous lineup value instead of fixed roster templates."""
    p=str(pos)
    have=int(counts.get(p,0))
    required=int(slots.get(p,0))
    if have < required:
        return 7.0
    if p in ("RB","WR","TE"):
        ...
        if skill_have < skill_required:
            return 4.5
    # bench depth decays continuously
    if p=="RB":
        return [2.8,2.0,1.2,0.4,-1.8,-4.0][min(max(have-required,0),5)]
    if p=="WR":
        return [3.0,2.2,1.5,0.7,-1.2,-3.0][min(max(have-required,0),5)]
    if p=="TE":
        return 1.3 if have==1 else -2.5
    if p=="QB":
        return 1.0 if have==1 else -3.0
    if p in ("DL","DB"):
        return 1.0 if have==1 else -2.0 if have>=2 else 0.0
    if p=="K":
        return -100.0 if have>=1 else 0.0
    return 0.0


def _lookahead_rollout_score(x, roster, slots, current_pick, next_pick, top_n=18):
    """
    Lightweight live look-ahead. Evaluate the best candidates by:
      current value + lineup improvement + positional tier cliff
      + next-turn replacement loss - saturation/opportunity cost.
    This is deterministic and fast enough for live use.
    """
    y=x.copy()
    if y.empty:
        return y
    ...
    base=pd.to_numeric(y.get("unified_pick_score",y.get("execution_score",y.get("evaluation_score",0))),errors="coerce").fillna(0.0)
    y["lookahead_score"]=base.astype(float)
    y["tier_cliff_component"]=0.0
    y["replacement_loss_component"]=0.0
    y["lineup_improvement_component"]=0.0
    y["future_roster_component"]=0.0

    # Limit expensive per-row work to the serious contender set.
    serious=y.nlargest(min(int(top_n),len(y)),"lookahead_score").index
    for idx in serious:
        p=str(y.at[idx,"position"])
        cliff=_tier_cliff_value(y,idx)
        repl=_next_turn_replacement_loss(y,idx,current_pick,next_pick)
        lineup=_starter_flex_improvement(p,counts,slots)

        after=_position_depth_after_pick(roster,p)
        # portfolio future value: reward flexibility, penalize saturation
        rb,wr=int(after.get("RB",0)),int(after.get("WR",0))
        idp=int(after.get("DL",0))+int(after.get("DB",0))
        future=0.0
        if rb>5: future-=4.0
        if wr>5: future-=3.0
        if idp>3: future-=8.0
        if idp==3: future+=0.8  # optional third IDP is possible, never forced

        y.at[idx,"tier_cliff_component"]=cliff
        y.at[idx,"replacement_loss_component"]=repl
        y.at[idx,"lineup_improvement_component"]=lineup
        y.at[idx,"future_roster_component"]=future
        y.at[idx,"lookahead_score"] += (
            0.9*cliff + 1.15*repl + lineup + future
        )
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    if _idp_total<2:
        ...
        # No automatic third-IDP bonus. Only a small merit signal; look-ahead decides.
        _idp_marginal += np.where(_impact_idp,0.5+np.minimum(_impact,15.0)*0.04,0.0)
    elif _idp_total>=3:
    ...
    # Lightweight look-ahead optimizer: compare take-now value with next-turn replacement loss.
    x=_lookahead_rollout_score(x,roster,slots,int(current_pick),int(next_pick),top_n=18)

    # Confidence = gap between the top two live candidates after look-ahead.
    _sorted_la=pd.to_numeric(x["lookahead_score"],errors="coerce").sort_values(ascending=False)
    _gap=float(_sorted_la.iloc[0]-_sorted_la.iloc[1]) if len(_sorted_la)>=2 else 8.0
    x["recommendation_confidence"]=np.clip(50.0+_gap*7.0,50.0,99.0)

    # FINAL PICK authority. Hard completion/eligibility penalties applied earlier remain
    # embedded in the mature evaluation and cannot be bypassed.
    x["execution_score"]=x["lookahead_score"]
...
def grade_mock(roster, teams, slots):
    ...
    # Construction coupling: Model Edge should describe the quality of the completed draft,
    # not just isolated player-value wins. Construction below 92 increasingly pulls Edge down.
    _construction_gap=max(0.0,90.0-construction)
    _construction_edge_penalty=min(10.0,_construction_gap*0.40)
    model_edge=float(np.clip(model_edge-_construction_edge_penalty,35,100))

    opp+=_portfolio_penalty+_reach_penalty+_diversification_penalty

    score=float(np.clip(.27*draft_value+.32*construction+.23*positional+.18*model_edge-opp,0,100))
    ...
    return {"score":score,"grade":grade,"starter":starter,"value":avg_market,"penalty":max(0,100-construction),
            ...
            "big_reach_rate":_big_reach_rate,"portfolio_penalty":_portfolio_penalty,"diversification_penalty":_diversification_penalty,"construction_edge_penalty":_construction_edge_penalty}
...
def _sim_opponent_pick_fast(avail, opp_counts, rnd, randomness, rng):
    ...
    fallback=pd.to_numeric(avail.get("model_rank",999),errors="coerce").fillna(999)
    market=pd.to_numeric(avail.get("consensus_rank",np.nan),errors="coerce").fillna(fallback)
    ...
    base=pd.to_numeric(a.get("consensus_rank",np.nan),errors="coerce").fillna(
        pd.to_numeric(a.get("model_rank",999),errors="coerce").fillna(999)
    )
...
def _fast_sim_user_pick(avail, roster_rows, rnd, overall, slot, teams, slots, randomness, rng):
    ...
    if rb>=4: score += np.where(pos=="RB",-7.0,0.0)
    if rb>=5: score += np.where(pos=="RB",-22.0,0.0)

    if wr>=5: score += np.where(pos=="WR",-10.0,0.0)
    ...
    if dl>=1 and db>=1:
        if total_idp>=3:
            ...
            # Third IDP only if it clearly wins; otherwise keep offense open.
            if best_idp <= best_off+4.0:
                feasible &= (~idp).to_numpy()
            else:
                score += np.where(impact_idp,0.8,0.0)
...
def simulate_current_fantasy_edge_once(board, teams, slot, rounds, slots, randomness, seed):
    ...
    for overall in range(1,int(teams)*int(rounds)+1):
        ...
        if int(owner)==int(slot):
            ...
            choice=_fast_sim_user_pick(
                avail.reset_index(drop=True),user_rows,rnd,overall,
                int(slot),int(teams),slots,int(randomness),rng
            )
            if choice is None:
                continue
            choice=choice.copy()
...
with tabs[1]:
    ...
    st.dataframe(
        ...
        .rename(columns={
            ...
            "idp_external_rank":"IDP rank","idp_impact_score":"IDP impact","usable_vorp":"Usable VORP","marginal_roster_value":"Marginal roster value","bench_startability":"Bench startability","tier_cliff_component":"Tier cliff","replacement_loss_component":"Next-turn loss","lineup_improvement_component":"Lineup gain","recommendation_confidence":"Confidence %","value_over_next_roster_slot":"Value over next slot","idp_available_rank":"Available IDP rank","idp_available_score":"Available IDP score","roster_opportunity_adj":"Roster opp."
        }).head(120),
        use_container_width=True,hide_index=True
    )
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
            ...
            if owner==int(slot):
                ...
                st.dataframe(
                    top[["player","position","team","evaluation_rank","Action","Market pick",
                         "Survives to next pick","projection","vorp","usable_vorp","marginal_roster_value","bench_startability","value_over_next_roster_slot","tier_cliff_component","replacement_loss_component","lineup_improvement_component","recommendation_confidence",
        ...
        batch_size=st.selectbox(
            ...
            index=3,
            help="Ultra-fast mode is designed to handle all 100 drafts in one batch. Use smaller batches if your host is constrained."
        )
        ...
        if run:
            ...
            for j in range(this_batch):
                seed=int(st.session_state.fe1000_next_seed)
                if j==0:
                    _sim_board=_prepare_fast_sim_board(board)
                result=simulate_current_fantasy_edge_once(
                    _sim_board,int(teams),int(slot),int(rounds),
                    state["roster_slots"],int(randomness),seed
                )
                st.session_state.fe1000_next_seed=seed+1
                ...
                if (j+1)==1 or (j+1)%25==0 or (j+1)==this_batch:
                    prog.progress((j+1)/max(this_batch,1))
                    status.caption(
                        f"Completed {j+1}/{this_batch} this batch • "
                        f"{len(st.session_state.fe1000_results):,}/{target_sims:,} total"
                    )

                # Protect the app from host timeouts; results persist and Continue resumes.
                if (time.perf_counter()-started)>30.0 and j+1<this_batch:
                    status.warning(
                        "Runtime safety cutoff reached. Completed results were saved — press "
                        "Run / continue to resume the 100-draft test."
                    )
                    break
        ...
        if len(sim_df):
            ...
            st.dataframe(
                sim_df[["simulation","grade","model_edge","draft_value","construction",
                        "positional_advantage","opportunity_penalty","RB","WR","DL","DB","IDP_total","legal_roster"]]
                .tail(100),
                use_container_width=True,hide_index=True
            )
            ...
            if len(sim_df)>=target_sims:
                st.success(
                    "100-draft test complete. Use these distributions to tune Fantasy Edge "
                    "instead of relying on a single interactive mock."
                )
