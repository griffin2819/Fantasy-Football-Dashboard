...
st.title("🏈 Fantasy Edge V7.73 — Round-5 Production-Path Promotion Certification")
st.caption("V7.73 • V7.67 frozen champion • Round-5 promotion certification")
st.caption("Build ID: V7.73-R5-PROMOTION-CERT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.73 keeps V7.67 frozen/live while performing the final fresh-seed production-path promotion certification of the exact validated Round-5 dominance rule.")
        ...
        st.markdown("### 👑 V7.73 — Round-5 Production-Path Promotion Certification")
        st.caption(
            "Final fresh-seed promotion test of the exact frozen V7.71/V7.72 Round-5 dominance rule. "
            "V7.67 remains the live champion during this certification. No thresholds, players, slots, rooms, or downstream logic are retuned. "
            "Trigger-first execution reruns the challenger only when the exact true-board Round-5 rule activates."
        )

        if "v773_results" not in st.session_state:
            st.session_state.v773_results=None

        sims=st.number_input(
            "V7.73 certification simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v773_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh matched sets. At ~4.85% activation this should require about 1,050 full simulations rather than 2,000."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.73 production-path certification",type="primary",key="v773_run")
        clear=b.button("Clear V7.73 certification",key="v773_clear")
        if clear:
            st.session_state.v773_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=973000+int(ds)*100000+j*233+sum(ord(c) for c in room_name)

                        # Frozen live champion: V7.67.
                        ...
                        r5=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(5)]
                        trigger=False; champ5=""; champ5_pos=""; detected_alt=""
                        if len(r5):
                            rr=r5.iloc[0]
                            trigger=bool(int(pd.to_numeric(
                                pd.Series([rr.get("R771_Would_trigger",0)]),
                                errors="coerce"
                            ).fillna(0).iloc[0]))
                            champ5=str(rr.player)
                            champ5_pos=str(rr.position)
                            detected_alt=str(rr.get("R771_Best_alt_player",""))

                        if trigger:
                            # Exact frozen candidate path; no retuning from V7.71/V7.72.
                            cand=simulate_mock_fast(
                                pool,int(teams),int(ds),15,state["roster_slots"],
                                6.0,True,seed,room_name,
                                r5_dominance_override_enabled=True
                            )
                            reruns+=1
                            ...
                            n5=cand[pd.to_numeric(cand.mock_round,errors="coerce").eq(5)]
                            cand5=str(n5.iloc[0].player) if len(n5) else ""
                            cand5_pos=str(n5.iloc[0].position) if len(n5) else ""

                            def _gc(comp,key):
                                try: return float(comp[key])
                            ...
                            rb=_gc(nc,"roster_balance")-_gc(cc,"roster_balance")
                            sc=_gc(nc,"starter_completion")-_gc(cc,"starter_completion")
                            av=_gc(nc,"availability")-_gc(cc,"availability")
                            mv=_gc(nc,"market_value")-_gc(cc,"market_value")
                        else:
                            changed=0; cand5=champ5; cand5_pos=champ5_pos
                            overall=r13=r46=r710=r11=rb=sc=av=mv=0.0

                        rows.append({
                            "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Trigger_detected":trigger,
                            "Candidate_rerun":trigger,
                            "Behavior_changed":changed>0,
                            "Changed_picks":changed,
                            "Champion_round5_player":champ5,
                            "Champion_round5_position":champ5_pos,
                            "Detected_best_alt":detected_alt,
                            "Candidate_round5_player":cand5,
                            "Candidate_round5_position":cand5_pos,
                            "Overall_change":overall,
                            "R1_3_change":r13,
                            "R4_6_change":r46,
                            "R7_10_change":r710,
                            "R11_plus_change":r11,
                            "Roster_balance_change":rb,
                            "Starter_completion_change":sc,
                            "Availability_change":av,
                            "Market_value_change":mv,
                            "Conditional_overall_win":(overall>0) if trigger else np.nan,
                            "Conditional_nonloss":(overall>=0) if trigger else np.nan,
                            "Conditional_R4_6_win":(r46>0) if trigger else np.nan,
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.73 certification: {done}/{total} matched sets • candidate reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v773_results=df
            status.success(
                f"V7.73 complete: {len(df)} fresh matched sets using "
                f"{len(df)+int(df.Candidate_rerun.sum())} full simulations instead of {len(df)*2}."
            )
            prog.empty()

        df=st.session_state.get("v773_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            cond46=trig.Conditional_R4_6_win.mean() if len(trig) else np.nan
            cond710=trig.Conditional_R7_10_nonloss.mean() if len(trig) else np.nan
            cond11=trig.Conditional_R11_nonloss.mean() if len(trig) else np.nan

            full_sims=len(df)+int(df.Candidate_rerun.sum())
            saved=1-(full_sims/(2*len(df)))

            summary=pd.DataFrame([{
                "Matched sets":len(df),
                "Triggered sets":len(trig),
                "Full simulations executed":full_sims,
                "Simulation-work reduction":saved,
                "Overall change":overall,
                "R1-3 change":r13,
                "R4-6 change":r46,
                "R7-10 change":r710,
                "R11+ change":r11,
                "Roster balance change":df.Roster_balance_change.mean(),
                "Starter completion change":df.Starter_completion_change.mean(),
                "Availability change":df.Availability_change.mean(),
                "Market value change":df.Market_value_change.mean(),
                "Activation":activation,
                "Conditional overall win":cond,
                "Conditional non-loss":nonloss,
                "Conditional R4-6 win":cond46,
                "Conditional R7-10 non-loss":cond710,
                "Conditional R11+ non-loss":cond11,
            }])
            ...
            st.markdown("#### Final certification by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Final certification by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            paths=trig.groupby(
                ["Champion_round5_player","Champion_round5_position",
                 "Candidate_round5_player","Candidate_round5_position"],
                as_index=False
            ).agg(
                ...
                Nonloss=("Conditional_nonloss","mean")
            ).sort_values("Decisions",ascending=False)
            st.markdown("#### Final Round-5 candidate paths")
            st.dataframe(paths.head(30).round(4),use_container_width=True,hide_index=True)

            positive_rooms=int((room.Overall_change>0).sum())
            active_slots=slot[slot.Activation>0]
            positive_active_slots=int((active_slots.Overall_change>0).sum())
            ...
            certified=(
                ...
                and r11>-0.05
                and activation>=.02
                and pd.notna(cond) and cond>=.80
                and pd.notna(nonloss) and nonloss>=.80
                and pd.notna(cond46) and cond46>=.90
                and positive_rooms>=4
                and positive_active_slots==len(active_slots)
            )

            gate=pd.DataFrame([{
                "Promotion eligible":certified,
                "Overall positive":overall>0,
                "R4-6 positive":r46>0,
                "R1-3 protected":r13>-.03,
                "R7-10 protected":r710>-.05,
                "R11+ protected":r11>-.05,
                "Activation >=2%":activation>=.02,
                "Conditional win >=80%":pd.notna(cond) and cond>=.80,
                "Conditional non-loss >=80%":pd.notna(nonloss) and nonloss>=.80,
                "Conditional R4-6 win >=90%":pd.notna(cond46) and cond46>=.90,
                "Positive rooms":positive_rooms,
                "Positive active slots":positive_active_slots,
                "Active slots":len(active_slots),
            }])
            st.markdown("#### V7.73 promotion gate")
            st.dataframe(gate,use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.73 PROMOTION CERTIFICATION PASSED. The exact frozen Round-5 dominance rule is eligible to be promoted into the production/default path in the next champion build."
                )
            else:
                st.warning(
                    "V7.73 promotion certification did not pass. V7.67 remains champion; do not retune this frozen validation sample."
                )

            st.download_button(
                "⬇️ Download V7.73 production-path promotion certification CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_73_round5_production_path_promotion_certification.csv",
                "text/csv",key="v773_download"
            )
