...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Injury Source Reconciliation", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v755_quality_flags(board_df):
    """Detect contradictory records before they are allowed to drive destructive moves."""
    if board_df is None or board_df.empty: return pd.DataFrame()
    rows=[]
    key_counts=board_df.get("fe_key",board_df["player"].map(_owner_key)).value_counts()
    for _,r in board_df.iterrows():
        ...
        status=str(r.get("fe_status",_v677_normalize_injury_status(r.get("injury",""))))
        team=str(r.get("fe_team",r.get("team","")) or "").upper()
        if key_counts.get(key,0)>1: flags.append("duplicate identity")
        if market<=75 and ros<2.0 and wk<3.0: flags.append("market/value contradiction")
        if market<=40 and _v755_num(r,"fe_vorp",0)<-4: flags.append("market/VORP contradiction")
        if status=="ACTIVE" and str(r.get("injury","")).upper() in ("IR","OUT","PUP","NFI"): flags.append("status contradiction")
        if str(r.get("position","")).upper() in ("QB","RB","WR","TE","K","DL","DB") and not team: flags.append("missing live team")
        if flags: rows.append({"Player":r.get("player",""),"Flags":" • ".join(flags)})
    return pd.DataFrame(rows)
...
def _v756_stale_flags(board_df,now=None):
    """V7.59 source-reconciliation watchdog.

    Only unresolved/quarantined current-status evidence is warned here. A verified status
    is already freshness-checked by the reconciliation layer, so missing legacy timestamps
    no longer create dozens of false stale warnings.
    """
    rows=[]
    for _,r in board_df.iterrows():
        ...
        if quarantined or conflict or not verified:
            rows.append({
                "Player":r.get("player",""),
                "Source":"status reconciliation",
                "Status":"CONFLICT" if conflict else (f"QUARANTINED {quarantined}" if quarantined else "UNVERIFIED"),
...
def _v756_certify(board_df,state_snapshot):
    ...
    add("Value contradictions","PASS" if q.empty else "WARN",f"{len(q)} contradiction(s) flagged" if len(q) else "No major market/value contradictions")
    stale=_v756_stale_flags(board_df)
    add("Source freshness","PASS" if stale.empty else "WARN",f"{len(stale)} player status record(s) quarantined/unverified" if len(stale) else "All actionable injury statuses reconciled from fresh evidence")
...
try:
    ...
    _v756_stale=_v756_stale_flags(board)
...
with tabs[14]:
    ...
    if my.empty:
        ...
        _warning_detail=""
        if len(_fail_rows):
            _warning_detail=" • ".join([f"{x['Check']}: {x['Detail']}" for _,x in _fail_rows.head(2).iterrows()])
        elif len(_warn_rows):
            _warning_detail=" • ".join([f"{x['Check']}: {x['Detail']}" for _,x in _warn_rows.head(2).iterrows()])
        ...
        with st.expander("Data quality + freshness",expanded=False):
            if _v755_quality.empty and (not isinstance(_v756_stale,pd.DataFrame) or _v756_stale.empty):
                st.success("No player-state contradictions or critical stale injury records detected.")
            if not _v755_quality.empty: st.dataframe(_v755_quality,use_container_width=True,hide_index=True)
            if isinstance(_v756_stale,pd.DataFrame) and len(_v756_stale): st.dataframe(_v756_stale,use_container_width=True,hide_index=True)
