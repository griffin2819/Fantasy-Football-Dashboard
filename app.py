...
st.title("🏈 Fantasy Edge V7.93 — Production Round-2 Adaptation Champion Candidate")
st.caption("V7.93 • Round-2 adaptation ON • production certification")
st.caption("Build ID: V7.93-R2-PRODUCTION-CERT-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 👑 V7.93 — Production-Path Round-2 Promotion Certification")
        st.caption(
            "Final fresh-seed production-path certification of the exact validated V7.91/V7.92.1 Round-2 rule, now ON by default. "
            "The certified Round-7 rule remains ON, the late-round champion sequence remains protected, and all retired experimental branches stay OFF. "
            "No player, position, room, slot, or threshold tuning is introduced."
        )

        if "v793_results" not in st.session_state:
            st.session_state.v793_results=None

        sims=st.number_input(
            "V7.93 certification simulations per slot × room",
            min_value=10,max_value=40,value=15,step=5,key="v793_sims",
            help="Default: all 12 slots × all 5 rooms × 15 = 900 fresh matched production-path sets. "
                 "This compares V7.67 baseline (R2 rule OFF) vs V7.93 production path (R2 rule ON)."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.93 production certification",type="primary",key="v793_run")
        clear=b.button("Clear V7.93 certification",key="v793_clear")
        if clear:
            st.session_state.v793_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=993000+int(ds)*100000+j*359+sum(ord(c) for c in room_name)
                        ...
                        r2b=baseline[pd.to_numeric(baseline.mock_round,errors="coerce").eq(2)]
                        r2p=production[pd.to_numeric(production.mock_round,errors="coerce").eq(2)]
                        b2=str(r2b.iloc[0].player) if len(r2b) else ""
                        p2=str(r2p.iloc[0].player) if len(r2p) else ""
                        p2pos=str(r2p.iloc[0].position) if len(r2p) else ""
                        ...
                        rows.append({
                            ...
                            "Baseline_R2_player":b2,"Production_R2_player":p2,"Production_R2_position":p2pos,
                        ...
                        if done==1 or done%150==0 or done==total:
                            status.caption(f"V7.93 certification: {done}/{total} matched production-path sets")

            df=pd.DataFrame(rows)
            st.session_state.v793_results=df
            status.success(f"V7.93 complete: {len(df)} fresh matched production-path sets.")
            prog.empty()

        df=st.session_state.get("v793_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            paths=changed.groupby(
                ["Baseline_R2_player","Production_R2_player","Production_R2_position"],as_index=False
            ).agg(
            ...
            st.markdown("#### Production Round-2 paths")
            ...
            st.markdown("#### V7.93 promotion gate")
            st.dataframe(gate,use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.93 PRODUCTION CERTIFICATION PASSED. The exact validated Round-2 adaptation rule is eligible to remain ON by default alongside the certified Round-7 rule."
                )
            else:
                st.warning(
                    "V7.93 production certification did not pass. Revert the Round-2 rule to OFF and retain V7.67 as champion. Do not retune this certification sample."
                )

            st.download_button(
                "⬇️ Download V7.93 production certification CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_93_round2_production_path_certification.csv",
                "text/csv",key="v793_download"
            )
