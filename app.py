...
st.set_page_config(page_title="Fantasy Edge v9.38 — Exact League Construction", page_icon="🏈", layout="wide")
...
_V936_IDP_DL_CONSENSUS = {
    # 2026 external consensus fallback. Used only when packaged IDP evidence is absent/weak.
    # Lower rank is better; these values create relative evidence, never a forced FINAL PICK.
    "Myles Garrett": 1,
    "Aidan Hutchinson": 2,
    "Maxx Crosby": 3,
    "Will Anderson Jr.": 4,
...
def _v938_unified_player_universe(board):
    ...
    for name,pos,rank in overlays:
        ...
        market_pick=148.0 + min(rank,40)*1.20
...
def _v935_idp_impact_score(row):
    ...
    strong_single = (
        ...
        any(pd.notna(v) and v > 0 for v in production) or
        (pd.notna(ext_rank) and ((pos=="DL" and ext_rank<=15) or (pos=="DB" and ext_rank<=35)))
    )
...
st.title("🏈 Fantasy Edge v9.38 — Exact League Construction + Unified Recommendation")
st.caption("v9.38 • v9.08 certified engine • unified Draft/Mock player universe • 12-team snake • DB/DL IDP ingestion")
st.caption("Build ID: FE-V9.38-UNIFIED-MOCK-12TEAM-SNAKE-20260820")
...
with tabs[1]:
    ...
    # v9.34 endgame kicker guard.
    _v934_board_has_k = bool((board.position.astype(str).str.upper()=="K").any()) if "position" in board.columns else False
    _v934_kicker_needed = int(live_round)>=16 and not _v934_board_has_k
    if _v934_kicker_needed:
        st.warning("🦵 KICKER SLOT: your league requires 1 K, but this board has no kicker rankings. Do not use a second DL/DB as a substitute; draft your kicker manually for this roster slot.")
    ...
    st.dataframe(
        show[["player","position","team","Model target #","Action","Market pick","Faller picks",
              "Survives next","projection","vorp","roster_opportunity_adj","Recommendation reason",
...
with tabs[2]:
    ...
    st.success("🔒 v9.38 unified Draft/Mock universe active • 🐍 12-team snake • 🧠 DB/DL fantasy-IDP + exact 2-FLEX/6-bench construction")
