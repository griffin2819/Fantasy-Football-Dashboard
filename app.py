...
st.title("🏈 Fantasy Edge V7.76 — Round-2 True-Board Counterfactual Audit")
st.caption("V7.76 • V7.67 frozen champion • Round-2 true-board diagnostic")
st.caption("Build ID: V7.76-R2-TRUEBOARD-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.76 keeps V7.67 frozen/live and audits Round-2 >10-pick reaches against genuinely available true-board alternatives.")
        ...
        st.markdown("### 🔬 V7.76 — Round-2 True-Board Counterfactual Audit")
        st.caption(
            "Measurement only. V7.67 remains the frozen production champion. "
            "For every Round-2 champion selection that reaches >10 picks versus effective market, V7.76 captures the actual alive+eligible board state "
            "after opponent picks and records the top five genuinely available market-respecting alternatives. No player is hard-coded."
        )

        if "v776_results" not in st.session_state:
            st.session_state.v776_results=None

        sims=st.number_input(
            "V7.76 simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v776_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-2 true-board audit",type="primary",key="v776_run")
        clear=b.button("Clear V7.76 audit",key="v776_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=976000+int(ds)*100000+j*251+sum(ord(c) for c in room_name)
                        ...
                        if len(r2):
                            ...
                            reach=market-pick if pd.notna(market) and pd.notna(pick) else np.nan
                            audit=int(pd.to_numeric(pd.Series([r.get("R776_Audit",0)]),errors="coerce").fillna(0).iloc[0])

                            if pd.notna(reach) and reach>10 and audit==1:
                                row={
                                    "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                    "Round2_pick":pick,
                                    "Champion_player":str(r.player),"Champion_position":str(r.position),
                                    "Champion_market_pick":market,"Champion_reach":reach,
                                    ...
                                    "Actual_available_count":r.get("R776_Actual_available_count",np.nan),
                                    "Market_respecting_count":r.get("R776_Market_respecting_count",np.nan),
                                }
                                for k in range(1,6):
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.76 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v776_results=df
            status.success(f"V7.76 complete: {len(df)} true-board Round-2 >10-pick reaches audited.")
            prog.empty()

        df=st.session_state.get("v776_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            df["Alt1_market_improvement"]=df.Champion_market_pick-df.Alt1_market_pick
            ...
            summary=pd.DataFrame([{
                "Triggered R2 reaches":len(df),
                "Average champion reach":pd.to_numeric(df.Champion_reach,errors="coerce").mean(),
                "Actual available players":pd.to_numeric(df.Actual_available_count,errors="coerce").mean(),
                "Market-respecting alternatives":pd.to_numeric(df.Market_respecting_count,errors="coerce").mean(),
                "Alt1 market improvement":df.Alt1_market_improvement.mean(),
            ...
            champs=df.groupby(["Champion_player","Champion_position"],as_index=False).agg(
                Triggers=("Champion_player","count"),
                Avg_reach=("Champion_reach","mean"),
                Alt1_score_change=("Alt1_draft_score_change","mean"),
                Alt1_vorp_change=("Alt1_vorp_change","mean"),
                Alt1_dominance=("Alt1_dominates_score_vorp","mean")
            ).sort_values("Triggers",ascending=False)
            st.markdown("#### Round-2 reach drivers")
            st.dataframe(champs.head(25).round(4),use_container_width=True,hide_index=True)
            ...
            st.dataframe(alts.head(25).round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Sample top-5 true-board alternatives")
            showcols=["Draft_slot","Room","Round2_pick","Champion_player","Champion_position","Champion_reach"]
            for k in range(1,6):
                showcols += [f"Alt{k}_player",f"Alt{k}_position",f"Alt{k}_market_pick",f"Alt{k}_draft_score"]
            st.dataframe(df[showcols].head(50).round(3),use_container_width=True,hide_index=True)

            st.info(
                "V7.76 changes no production behavior. If the top true-board alternative repeatedly beats the Round-2 champion selection on both frozen draft score and VORP "
                "while improving market price, the next build can test a narrow full-draft challenger. Otherwise the Round-2 premium is a justified model conviction."
            )

            st.download_button(
                "⬇️ Download V7.76 Round-2 true-board audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_76_round2_true_board_counterfactual_audit.csv",
                "text/csv",key="v776_download"
            )
        elif isinstance(df,pd.DataFrame):
            st.success("No Round-2 >10-pick reaches occurred in this run.")
