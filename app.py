...
def _fast_candidate_deltas(fast, selected, candidates, slots, exact_cap=56):
    ...
    local=np.unique(np.concatenate([ai,bi]))
    if local.size>n:
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                score=draft_score[sidx] + 2.35*delta - market_pen
                if round_no<=7:
...
st.title("🏈 Fantasy Edge V7.3.1 — Dynamic Roster Performance")
st.caption("V7.3.1 • optimized dynamic roster utility • bounded exact evaluation • frozen model + grading • Yahoo + IDP")
st.caption("Build ID: V7.3.1-DRV-PERF-20260813")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.3.1 validation report")
            ...
            st.download_button("⬇️ Download V7.3.1 diagnostic CSV",csv,"fantasy_edge_v7_3_1_diagnostics.csv","text/csv")
