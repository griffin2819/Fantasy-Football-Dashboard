...
st.title("🏈 Fantasy Edge V7.43 — Pick 12 TE Valuation Architecture Audit")
st.caption("V7.43 • V7.24 9× champion frozen • Pick 12 TE valuation architecture audit")
st.caption("Build ID: V7.43-P12TEVA-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v743_live_slot")
    ...
    if "v743_target_queue" not in st.session_state:
        st.session_state.v743_target_queue={}
    queue=st.session_state.v743_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v743_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.43 keeps the V7.24 9× champion completely frozen. No TE adjustment or candidate rule is active.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.43 robustness report")
        ...
        st.markdown("### 🧪 V7.43 champion baseline validation")
        st.info("V7.43 is diagnostic only. It traces TE replacement PPG, VORP, starter/FLEX/bench role, roster-slot completion state, and marginal utility versus the passed WR faller.")
        ...
        if "v743_independent_results" not in st.session_state:
            st.session_state.v743_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v743_independent_results"),pd.DataFrame) else "ready to run"))
        if "v743_independent_summary" not in st.session_state:
            st.session_state.v743_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v743_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v743_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v743_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v743_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v743_independent_results=iv
                st.session_state.v743_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v743_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v743_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.43 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.43 audits the Pick-12 R4-6 TE valuation architecture against passed WR fallers.")
            ...
            st.download_button(
                "⬇️ Download V7.43 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_43_champion_baseline.csv",
                "text/csv",
                key="v743_baseline_download"
            )
        ...
        st.markdown("### 🧩 V7.43 — Pick 12 TE Valuation Architecture Audit")
        st.caption(
            "Diagnostic only. V7.24 9× remains frozen. This isolates Pick-12 R4-6 TE → WR-faller decisions and explains "
            "how TE replacement value and marginal roster utility are interacting."
        )

        if "v743_results" not in st.session_state: st.session_state.v743_results=None
        if "v743_meta" not in st.session_state: st.session_state.v743_meta=None

        sims=st.number_input("TE-architecture simulations per room",min_value=5,max_value=25,value=10,step=5,key="v743_sims")
        aa,ab=st.columns(2)
        run=aa.button("Run Pick 12 TE architecture audit",type="primary",key="v743_run")
        clear=ab.button("Clear TE architecture audit",key="v743_clear")

        if clear:
            st.session_state.v743_results=None
            st.session_state.v743_meta=None
            st.rerun()
        ...
        if run:
            ...
            for room_name in rooms:
                for j in range(int(sims)):
                    seed=943000+j*79+sum(ord(c) for c in room_name)
                    fe=simulate_mock_fast(
                        fast_champ,int(teams),12,15,state["roster_slots"],
                        6.0,True,seed,room_name
                    )
                    audited=fe[
                        pd.to_numeric(fe.get("MR43_TE_architecture_audit",0),errors="coerce").fillna(0).eq(1)
                    ].copy()
                    ...
                    for _,r in audited.iterrows():
                        rows.append({
                            ...
                            "TE_player":r.get("MR_Selected_player",""),
                            "WR_faller":r.get("MR_Passed_faller_player",""),
                            "TE_market_pick":r.get("MR_Selected_market_pick",np.nan),
                            "WR_faller_amount":r.get("MR_Passed_faller_amount",np.nan),
                            "Total_FE_gap":r.get("MR_Passed_faller_eval_gap",np.nan),
                            "Draftscore_gap":r.get("MR42_Faller_draftscore_gap",np.nan),
                            "Rosterutility_FE_gap":r.get("MR42_Faller_rosterutility_gap",np.nan),
                            "VORP_FE_gap":r.get("MR42_Faller_vorp_gap",np.nan),
                            "Projection_FE_gap":r.get("MR42_Faller_projection_gap",np.nan),
                            "TE_role":r.get("MR43_Selected_role",""),
                            "WR_role":r.get("MR43_Faller_role",""),
                            "TE_utility_delta":r.get("MR43_Selected_utility_delta",np.nan),
                            "WR_utility_delta":r.get("MR43_Faller_utility_delta",np.nan),
                            "Raw_utility_delta_gap":r.get("MR43_Utility_delta_gap",np.nan),
                            "TE_role_projection_component":r.get("MR43_Selected_role_proj",np.nan),
                            "TE_role_vorp_component":r.get("MR43_Selected_role_vorp",np.nan),
                            "WR_role_projection_component":r.get("MR43_Faller_role_proj",np.nan),
                            "WR_role_vorp_component":r.get("MR43_Faller_role_vorp",np.nan),
                            "TE_replacement_ppg":r.get("MR43_Selected_replacement_ppg",np.nan),
                            "WR_replacement_ppg":r.get("MR43_Faller_replacement_ppg",np.nan),
                            "Replacement_ppg_gap_TE_minus_WR":r.get("MR43_Replacement_gap",np.nan),
                            "TE_direct_slot_missing_before":r.get("MR43_Selected_direct_missing",np.nan),
                            "WR_direct_slot_missing_before":r.get("MR43_Faller_direct_missing",np.nan),
                            "QB_before":r.get("MR43_QB_before",np.nan),
                            "RB_before":r.get("MR43_RB_before",np.nan),
                            "WR_before":r.get("MR43_WR_before",np.nan),
                            "TE_before":r.get("MR43_TE_before",np.nan),
                            "Faller_survived_actual":r.get("MR_Passed_faller_survived_actual",np.nan),
                        })

                    done+=1
                    prog.progress(done/total)
                    if done==1 or done%10==0 or done==total:
                        status.caption(f"TE architecture audit: {done}/{total} drafts")

            df=pd.DataFrame(rows)
            st.session_state.v743_results=df
            st.session_state.v743_meta={
                "seconds":time.perf_counter()-started,
                "drafts":total,"decisions":len(df)
            }
            status.success(f"TE architecture audit complete: {len(df)} TE → WR-faller decisions.")
            prog.empty()

        df=st.session_state.get("v743_results")
        meta=st.session_state.get("v743_meta") or {}

        if isinstance(df,pd.DataFrame) and len(df):
            c1,c2,c3,c4=st.columns(4)
            c1.metric("TE→WR decisions",str(len(df)))
            c2.metric("Avg FE gap",f"{pd.to_numeric(df.Total_FE_gap,errors='coerce').mean():+.2f}")
            c3.metric("Avg roster-utility FE gap",f"{pd.to_numeric(df.Rosterutility_FE_gap,errors='coerce').mean():+.2f}")
            c4.metric("Avg VORP FE gap",f"{pd.to_numeric(df.VORP_FE_gap,errors='coerce').mean():+.2f}")

            st.markdown("#### Marginal roster-utility architecture")
            util=pd.DataFrame([
                {"Measure":"TE raw utility delta","Average":pd.to_numeric(df.TE_utility_delta,errors="coerce").mean()},
                {"Measure":"WR raw utility delta","Average":pd.to_numeric(df.WR_utility_delta,errors="coerce").mean()},
                {"Measure":"TE − WR raw utility gap","Average":pd.to_numeric(df.Raw_utility_delta_gap,errors="coerce").mean()},
                {"Measure":"TE role projection component","Average":pd.to_numeric(df.TE_role_projection_component,errors="coerce").mean()},
                {"Measure":"TE role VORP component","Average":pd.to_numeric(df.TE_role_vorp_component,errors="coerce").mean()},
                {"Measure":"WR role projection component","Average":pd.to_numeric(df.WR_role_projection_component,errors="coerce").mean()},
                {"Measure":"WR role VORP component","Average":pd.to_numeric(df.WR_role_vorp_component,errors="coerce").mean()},
            ])
            st.dataframe(util.round(3),use_container_width=True,hide_index=True)

            st.markdown("#### Replacement-level architecture")
            repl=pd.DataFrame([{
                "TE replacement PPG":pd.to_numeric(df.TE_replacement_ppg,errors="coerce").mean(),
                "WR replacement PPG":pd.to_numeric(df.WR_replacement_ppg,errors="coerce").mean(),
                "TE − WR replacement gap":pd.to_numeric(df.Replacement_ppg_gap_TE_minus_WR,errors="coerce").mean(),
                "TE direct slot missing rate":(pd.to_numeric(df.TE_direct_slot_missing_before,errors="coerce")>0).mean(),
                "WR direct slot missing rate":(pd.to_numeric(df.WR_direct_slot_missing_before,errors="coerce")>0).mean(),
            }])
            st.dataframe(repl.round(3),use_container_width=True,hide_index=True)

            role=df.groupby(["TE_role","WR_role"],as_index=False).agg(
                Decisions=("Total_FE_gap","count"),
                Total_FE_gap=("Total_FE_gap","mean"),
                Rosterutility_gap=("Rosterutility_FE_gap","mean"),
                Raw_utility_gap=("Raw_utility_delta_gap","mean"),
                VORP_gap=("VORP_FE_gap","mean"),
                Faller_size=("WR_faller_amount","mean")
            )
            st.markdown("#### TE role → WR role path")
            st.dataframe(role.sort_values("Decisions",ascending=False).round(3),use_container_width=True,hide_index=True)

            state=df.groupby(["TE_before","WR_before"],as_index=False).agg(
                Decisions=("Total_FE_gap","count"),
                Rosterutility_gap=("Rosterutility_FE_gap","mean"),
                Raw_utility_gap=("Raw_utility_delta_gap","mean"),
                VORP_gap=("VORP_FE_gap","mean"),
            ...
            st.markdown("#### Roster state before the decision")
            st.dataframe(state.sort_values("Decisions",ascending=False).round(3),use_container_width=True,hide_index=True)
            ...
            room=df.groupby("Room",as_index=False).agg(
                ...
                Raw_utility_gap=("Raw_utility_delta_gap","mean"),
                VORP_gap=("VORP_FE_gap","mean"),
                Replacement_gap=("Replacement_ppg_gap_TE_minus_WR","mean"),
                WR_faller_size=("WR_faller_amount","mean")
            )
            st.markdown("#### TE architecture by room")
            st.dataframe(room.round(3),use_container_width=True,hide_index=True)

            te_missing=(pd.to_numeric(df.TE_direct_slot_missing_before,errors="coerce")>0).mean()
            wr_missing=(pd.to_numeric(df.WR_direct_slot_missing_before,errors="coerce")>0).mean()
            raw_util=pd.to_numeric(df.Raw_utility_delta_gap,errors="coerce").mean()
            repl_gap=pd.to_numeric(df.Replacement_ppg_gap_TE_minus_WR,errors="coerce").mean()
            vorp_gap=pd.to_numeric(df.VORP_FE_gap,errors="coerce").mean()

            if te_missing>=.70 and raw_util>0 and vorp_gap>0:
                st.warning(
                    f"TE is receiving two simultaneous starter-completion advantages: its direct starter slot is still open in {te_missing:.0%} of these decisions, "
                    f"while WR's direct slot is open in only {wr_missing:.0%}. The same TE also carries a positive VORP edge ({vorp_gap:+.2f} FE points). "
                    "V7.44 should test whether TE starter-completion utility and VORP are double-counting the same scarcity signal."
                )
            elif raw_util>0 and repl_gap<0 and vorp_gap>0:
                st.warning(
                    f"TE's marginal utility advantage ({raw_util:+.2f} raw utility) and lower replacement baseline ({repl_gap:+.2f} PPG TE−WR) are reinforcing each other. "
                    "V7.44 should run a diagnostic double-counting calibration rather than another execution intercept."
                )
            else:
                st.info(
                    "The TE premium is not explained by one obvious starter-slot or replacement-level interaction. Use the role and roster-state tables to isolate the next test."
                )

            st.download_button(
                "⬇️ Download V7.43 Pick 12 TE architecture audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_43_pick12_te_valuation_architecture.csv",
                "text/csv",key="v743_download"
            )
        else:
            st.info("Run the audit to populate Pick-12 TE → WR-faller architecture diagnostics.")
