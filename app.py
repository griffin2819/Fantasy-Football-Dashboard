...
st.title("🏈 Fantasy Edge V7.80 — Round-3 True-Board Dominance Challenger")
st.caption("V7.80 • V7.67 frozen champion • Round-3 full-draft challenger")
st.caption("Build ID: V7.80-R3-DOMINANCE-CHALLENGER-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.80 keeps V7.67 frozen/live and tests the generic Round-3 true-board score+VORP dominance rule across the full remainder of each draft.")
        ...
        st.markdown("### 🧪 V7.80 — Round-3 True-Board Dominance Challenger")
        st.caption(
            "Full-draft matched challenger. V7.67 remains frozen/live. Round 3 changes only when the normal champion choice reaches >10 picks "
        ...
        if "v780_results" not in st.session_state:
            st.session_state.v780_results=None

        sims=st.number_input(
            "V7.80 matched simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v780_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh matched sets. Trigger-first: challenger reruns only on actual Round-3 dominance triggers."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-3 dominance challenger",type="primary",key="v780_run")
        clear=b.button("Clear V7.80 challenger",key="v780_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=980000+int(ds)*100000+j*271+sum(ord(c) for c in room_name)
                        ...
                        r3=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(3)]
                        trigger=False; champ3=""; champ3pos=""; alt=""
                        if len(r3):
                            rr=r3.iloc[0]
                            trigger=bool(int(pd.to_numeric(
                                pd.Series([rr.get("R780_Would_trigger",0)]),
                                errors="coerce"
                            ).fillna(0).iloc[0]))
                            champ3=str(rr.player)
                            champ3pos=str(rr.position)
                            alt=str(rr.get("R780_Best_alt_player",""))
                        ...
                        if trigger:
                            cand=simulate_mock_fast(
                                ...
                                r5_dominance_override_enabled=False,
                                r2_dominance_override_enabled=False,
                                r3_dominance_override_enabled=True
                            )
                            reruns+=1
                            ...
                            n3=cand[pd.to_numeric(cand.mock_round,errors="coerce").eq(3)]
                            cand3=str(n3.iloc[0].player) if len(n3) else ""
                            cand3pos=str(n3.iloc[0].position) if len(n3) else ""
                        ...
                        rows.append({
                            ...
                            "Champion_round3_player":champ3,
                            "Champion_round3_position":champ3pos,
                            "Detected_best_alt":alt,
                            "Candidate_round3_player":cand3,
                            "Candidate_round3_position":cand3pos,
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.80: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v780_results=df
            status.success(
                f"V7.80 complete: {len(df)} matched sets using "
                f"{len(df)+int(df.Challenger_rerun.sum())} full simulations instead of {len(df)*2}."
            )
            prog.empty()

        df=st.session_state.get("v780_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            c2.metric("R1–3 vs V7.67",f"{r13:+.3f}")
            c3.metric("Activation",f"{activation:.1%}")
            ...
            paths=trig.groupby(
                ["Champion_round3_player","Champion_round3_position",
                 "Candidate_round3_player","Candidate_round3_position"],
                as_index=False
            ).agg(
            ...
            st.markdown("#### Round-3 override paths")
            ...
            if screen:
                st.success(
                    "V7.80 SCREEN PASSED. Advance the exact frozen Round-3 rule to larger independent validation; V7.67 remains champion."
                )
            else:
                st.warning(
                    "V7.80 does not clear screening. Keep V7.67 champion and do not tune the Round-3 rule from this sample."
                )

            st.download_button(
                "⬇️ Download V7.80 Round-3 dominance challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_80_round3_dominance_challenger.csv",
                "text/csv",key="v780_download"
            )
