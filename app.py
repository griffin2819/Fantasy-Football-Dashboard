...
st.title("🏈 Fantasy Edge V8.07 — Round-5 Generic Mechanism Audit")
st.caption("V8.07 • V8.05 frozen champion • R5 generic mechanism measurement")
st.caption("Build ID: V8.07-R5-GENERIC-MECHANISM-AUDIT-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V8.07 — Round-5 Generic Mechanism Audit")
        st.caption(
            "Measurement only. V8.05 remains frozen champion. V8.07 studies the exact-board Round-5 signal found by V8.06.4 "
            "without changing any production rule. It explicitly separates Cam Skattebo cases from all non-Skattebo cases "
            "and measures roster state, position path, market-distance bands, room/slot breadth, and R6–R8 downstream construction."
        )

        if "v807_results" not in st.session_state:
            st.session_state.v807_results=None

        sims=st.number_input(
            "V8.07 simulations per slot × room", min_value=10, max_value=40, value=20, step=5, key="v807_sims",
            help="Default: 1,200 fresh V8.05 champion drafts. Only exact-board Round-5 decisions are retained."
        )
        a,b=st.columns(2)
        run=a.button("Run V8.07 R5 mechanism audit",type="primary",key="v807_run")
        clear=b.button("Clear V8.07 audit",key="v807_clear")
        if clear:
            st.session_state.v807_results=None
            st.rerun()
            ...
            for ds in range(1,int(teams)+1):
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8070000+ds*100000+j*439+sum(ord(c) for c in room_name)
                        ...
                        r5=d[pd.to_numeric(d.mock_round,errors="coerce").eq(5)]
                        if len(r5):
                            r=r5.iloc[0]
                            ...
                            future=d[
                                (pd.to_numeric(d.mock_round,errors="coerce")>=6) &
                                (pd.to_numeric(d.mock_round,errors="coerce")<=8)
                            ...
                            rows.append({
                                ...
                                "Alt_is_Skattebo":alt.strip().lower()=="cam skattebo",
                                "VORP_gap":vg,"Draft_score_gap":sg,"Alt_market_distance":amd,
                                "Market_distance_band":band,
                                ...
                                "R6_8_avg_VORP":fvorp.mean(),
                                "R6_8_avg_score":fscore.mean(),
                                "R6_8_position_sequence":" > ".join(future["position"].astype(str).tolist()),
                                "R6_player":str(future.iloc[0].player) if len(future)>0 else "",
                                "R7_player":str(future.iloc[1].player) if len(future)>1 else "",
                                "R8_player":str(future.iloc[2].player) if len(future)>2 else "",
                            })

                        done+=1
                        prog.progress(done/total)
                        if done%200==0 or done==total:
                            status.caption(f"V8.07: {done}/{total} fresh champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v807_results=df
            status.success(f"V8.07 complete: {len(df):,} exact-board Round-5 decisions audited.")
            prog.empty()

        df=st.session_state.get("v807_results")
        if isinstance(df,pd.DataFrame) and len(df):
            sig=df[df.Alt_dominates_le10].copy()
            non_skat=sig[~sig.Alt_is_Skattebo].copy()
            skat=sig[sig.Alt_is_Skattebo].copy()

            summary=pd.DataFrame([
                {"Population":"All R5 decisions","N":len(df),"Signal_N":len(sig),"Signal_rate":len(sig)/len(df) if len(df) else np.nan,
                 "Avg_VORP_gap":sig.VORP_gap.mean() if len(sig) else np.nan,"Avg_score_gap":sig.Draft_score_gap.mean() if len(sig) else np.nan,
                 "Avg_R6_8_VORP":sig.R6_8_avg_VORP.mean() if len(sig) else np.nan},
                {"Population":"Skattebo signal","N":int(df.Alt_is_Skattebo.sum()),"Signal_N":len(skat),
                 "Signal_rate":len(skat)/max(1,int(df.Alt_is_Skattebo.sum())),
                 "Avg_VORP_gap":skat.VORP_gap.mean() if len(skat) else np.nan,"Avg_score_gap":skat.Draft_score_gap.mean() if len(skat) else np.nan,
                 "Avg_R6_8_VORP":skat.R6_8_avg_VORP.mean() if len(skat) else np.nan},
                {"Population":"Non-Skattebo signal","N":int((~df.Alt_is_Skattebo).sum()),"Signal_N":len(non_skat),
                 "Signal_rate":len(non_skat)/max(1,int((~df.Alt_is_Skattebo).sum())),
                 "Avg_VORP_gap":non_skat.VORP_gap.mean() if len(non_skat) else np.nan,"Avg_score_gap":non_skat.Draft_score_gap.mean() if len(non_skat) else np.nan,
                 "Avg_R6_8_VORP":non_skat.R6_8_avg_VORP.mean() if len(non_skat) else np.nan},
            ])
            st.markdown("#### Skattebo vs generic signal")
            ...
            paths=sig.groupby(["Champion_position","Best_alt_position"],as_index=False).agg(
                ...
                Skattebo_share=("Alt_is_Skattebo","mean"),
                Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),
                Avg_market_distance=("Alt_market_distance","mean"),
                Avg_R6_8_VORP=("R6_8_avg_VORP","mean"),
                Avg_R6_8_score=("R6_8_avg_score","mean")
            ).sort_values("Decisions",ascending=False)
            st.markdown("#### Position mechanism")
            st.dataframe(paths.round(4),use_container_width=True,hide_index=True)

            nonpaths=non_skat.groupby(["Champion_position","Best_alt_position"],as_index=False).agg(
                Decisions=("Champion_player","count"),
                Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),
                Avg_market_distance=("Alt_market_distance","mean"),
                Avg_R6_8_VORP=("R6_8_avg_VORP","mean")
            ).sort_values("Decisions",ascending=False) if len(non_skat) else pd.DataFrame()
            st.markdown("#### Non-Skattebo mechanism only")
            st.dataframe(nonpaths.round(4),use_container_width=True,hide_index=True)

            room=sig.groupby("Room",as_index=False).agg(
                Signals=("Champion_player","count"),Skattebo_share=("Alt_is_Skattebo","mean"),
                Avg_VORP_gap=("VORP_gap","mean"),Avg_score_gap=("Draft_score_gap","mean"))
            slot=sig.groupby("Draft_slot",as_index=False).agg(
                Signals=("Champion_player","count"),Skattebo_share=("Alt_is_Skattebo","mean"),
                Avg_VORP_gap=("VORP_gap","mean"),Avg_score_gap=("Draft_score_gap","mean"))
            st.markdown("#### Signal by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Signal by slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            bands=sig.groupby("Market_distance_band",as_index=False).agg(
                Signals=("Champion_player","count"),Skattebo_share=("Alt_is_Skattebo","mean"),
                Avg_VORP_gap=("VORP_gap","mean"),Avg_score_gap=("Draft_score_gap","mean"),
                Avg_R6_8_VORP=("R6_8_avg_VORP","mean"))
            st.markdown("#### Market-distance bands")
            st.dataframe(bands.round(4),use_container_width=True,hide_index=True)

            roster=non_skat.groupby(
                ["QB_count_pre","RB_count_pre","WR_count_pre","TE_count_pre","Champion_position","Best_alt_position"],
                as_index=False
            ).agg(
                Decisions=("Champion_player","count"),Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),Avg_market_distance=("Alt_market_distance","mean"),
                Avg_R6_8_VORP=("R6_8_avg_VORP","mean")
            ).sort_values("Decisions",ascending=False) if len(non_skat) else pd.DataFrame()
            st.markdown("#### Non-Skattebo roster-state signatures")
            st.dataframe(roster.head(50).round(4),use_container_width=True,hide_index=True)

            room_breadth=int(non_skat.Room.nunique()) if len(non_skat) else 0
            slot_breadth=int(non_skat.Draft_slot.nunique()) if len(non_skat) else 0
            generic_share=len(non_skat)/len(sig) if len(sig) else 0
            st.info(
                f"Genericity check: non-Skattebo cases are {generic_share:.1%} of the exact-board R5 signal, "
                f"covering {room_breadth}/5 rooms and {slot_breadth}/12 slots. "
                "V8.07 is measurement-only; do not create or tune an R5 production rule from this sample."
            )

            st.download_button(
                "⬇️ Download V8.07 R5 mechanism audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_07_r5_generic_mechanism_audit.csv",
                "text/csv",key="v807_download"
            )
