...
st.set_page_config(page_title="Fantasy Edge v9.32 — Exact League Construction", page_icon="🏈", layout="wide")
...
def roster_opportunity_adjustment(df, roster, round_no, current_pick, slots):
    ...
    for _,r in x.iterrows():
        ...
        # v9.28 impact-IDP quality gate.
        # Prefer established, fantasy-relevant NFL defenders using fields already present
        # in the board. This is mechanism-based, not a hard-coded player-name list.
        if pos in ["DL","DB"]:
            _idp_market = pd.to_numeric(pd.Series([r.get("adp", np.nan)]), errors="coerce").iloc[0]
            _idp_vorp = pd.to_numeric(pd.Series([r.get("vorp", np.nan)]), errors="coerce").iloc[0]
            _idp_proj = pd.to_numeric(pd.Series([r.get("projection", r.get("proj", np.nan))]), errors="coerce").iloc[0]
            _idp_team = str(r.get("team","")).strip().upper()
            _idp_name = str(r.get("player","")).strip()

            # Missing/invalid team or missing market + weak model evidence should not
            # crowd the recommendation table simply because the position is needed.
            _idp_team_bad = (_idp_team in ["", "FA", "N/A", "NA", "NONE", "NAN"])
            _idp_market_missing = pd.isna(_idp_market)
            _idp_model_weak = (pd.isna(_idp_vorp) or _idp_vorp <= 0) and (pd.isna(_idp_proj) or _idp_proj <= 0)

            _idp_cons = pd.to_numeric(pd.Series([r.get("consensus_rank", r.get("rank", np.nan))]), errors="coerce").iloc[0]
            _idp_has_market = (pd.notna(_idp_market) or pd.notna(_idp_cons))
            _idp_has_model = ((pd.notna(_idp_vorp) and _idp_vorp > 0) or (pd.notna(_idp_proj) and _idp_proj > 0))

            # v9.30: defenders need credible evidence to become aggressive recommendations.
            # Missing both market/rank evidence and positive model evidence is a near-disqualifier.
            if (not _idp_has_market) and (not _idp_has_model):
                adj -= 40.0
                why.append("IDP fails eligibility evidence gate")
            elif (not _idp_has_market) and (pd.isna(_idp_vorp) or _idp_vorp <= 0):
                adj -= 24.0
                why.append("IDP lacks market/rank support")

            if _idp_team_bad:
                adj -= 22.0
                why.append("IDP uncertain NFL role/team")
            if _idp_market_missing and _idp_model_weak:
                adj -= 28.0
                why.append("IDP missing market + weak projection")
            elif _idp_market_missing:
                adj -= 7.0
                why.append("IDP missing market")
            # v9.31 replacement-level discipline: a defender with no marginal value
            # cannot win merely because a DL/DB starter is still empty.
            if pd.isna(_idp_vorp) or _idp_vorp <= 0:
                adj -= 30.0
                why.append("IDP zero/negative VORP blocked")
            elif _idp_vorp < 0.75:
                adj -= 12.0
                why.append("IDP below impact threshold")
            elif _idp_vorp >= 1.5:
                adj += 11.0
                why.append("impact IDP VORP")
            if pd.notna(_idp_proj) and _idp_proj > 0:
                adj += 4.0
                why.append("projected IDP role")
...
st.title("🏈 Fantasy Edge v9.32 — Exact League Construction + Unified Recommendation")
st.caption("v9.33 • v9.08 certified engine • useful-challenger context gate + authoritative FINAL PICK")
st.caption("Build ID: FE-V9.33-CONTEXT-QUALITY-GATE-20260820")
...
with tabs[1]:
    ...
    x=apply_v931_idp_opportunity_gate(x,myb,live_round)
    x=apply_v932_exact_league_construction(x,myb,live_round,16,slots)
...
with tabs[2]:
    ...
    st.success("🔒 v9.23.1 availability integrity active • 🧠 v9.33 context quality + exact 2-FLEX/6-bench construction + authoritative FINAL PICK active")
    ...
    st.caption("V9.08 remains the frozen certified core; V9.32 applies exact 1QB/2RB/2WR/2FLEX/1DL/1DB/6-bench construction, availability, injury, and impact-IDP overlays without changing the frozen core.")
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                avail=apply_v931_idp_opportunity_gate(avail,roster,rnd)
                avail=apply_v932_exact_league_construction(avail,roster,rnd,int(rounds),state["roster_slots"])
