...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — True Fragment Navigation", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
st.caption("⚡ True fragment navigation • page switches rerun only the selected page • refresh is manual")
...
def _v676_optimize_lineup(roster_df):
    ...
    def _canonical_week_status(row):
        # fe_status is the reconciled authority. injury_effective is the legacy fallback.
        return _v677_normalize_injury_status(row.get("fe_status",row.get("injury_effective",row.get("injury","ACTIVE"))))
    ...
    def add_row(label,row):
        ...
        slots.append({
            ...
            "Status":_v677_status_icon(row.get("fe_status",row.get("injury_effective",row.get("injury","")))),
...
def _v775_best_replacement(row, my_df):
    ...
    pool=pool[pool.apply(lambda r:_v677_normalize_injury_status(r.get("fe_status",r.get("injury_effective",r.get("injury","ACTIVE")))) not in unavailable,axis=1)]
    if pos in ("RB","WR","TE"):
...
def _v775_unified_decisions(board_df,state_snapshot,limit=12,include_trade_scan=False):
    ...
    for _,r in my.iterrows():
        p=str(r.get("player","")); status=_v677_normalize_injury_status(r.get("fe_status",r.get("injury_effective",r.get("injury","ACTIVE"))))
    ...
    if isinstance(starters,pd.DataFrame) and len(starters):
        for _,sr in starters.iterrows():
            ...
            if src.empty: continue
            r=src.iloc[0]; status=_v677_normalize_injury_status(r.get("fe_status",r.get("injury_effective",r.get("injury","ACTIVE"))))
...
def _v781_render_page(_v777_page_idx):
    ...
    if _v777_page_idx==6:
        ...
        if _my.empty:
            ...
            _status_series=_my.apply(
                lambda r:_v677_normalize_injury_status(r.get("fe_status",r.get("injury_effective",r.get("injury","ACTIVE")))),
                axis=1,
            )
            ...
            if len(_starters):
                for _,sr in _starters.iterrows():
                    ...
                    src=src.iloc[0] if len(src) else pd.Series(dtype=object)
                    _src_status=_v677_normalize_injury_status(src.get("fe_status",src.get("injury_effective",src.get("injury","ACTIVE"))))
            ...
            if len(_unavailable_rows):
                ...
                for _,r in _unavailable_rows.iterrows():
                    _st=_v677_normalize_injury_status(r.get("fe_status",r.get("injury_effective",r.get("injury","ACTIVE"))))
            ...
            if _monitor_total:
                with st.expander(f"🚨 Injury / Practice Monitor ({_monitor_total})",expanded=False):
                    ...
                    for _,r in _watch_df.iterrows():
                        ...
                        _ir.append({
                            "Player":str(r.get("player","") or ""),
                            "Status":_v677_status_icon(r.get("fe_status",r.get("injury_effective",r.get("injury","")))),
...
def _v781_navigation_fragment():
    ...
    _choice=st.radio(
        "Fantasy Edge navigation",
        _V777_PAGES,
        index=_current,
        horizontal=True,
        key="_v781_page_radio",
