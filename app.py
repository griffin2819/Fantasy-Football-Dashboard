...
def constrained_turn_sequence(eval_score, market_pick, current_pick, next_pick, randomness=12,
                              approved_cap=8):
    """Sequence adjacent turn picks without expanding the V7.10 approved candidate set.

    Step 1: build the approved pool using V7.10 execution readiness.
    Step 2: allow a V7.10 Faller Intercept into that approved pool if triggered.
    Step 3: pick two approved players and only optimize their ORDER.

    This function never promotes a WAIT-only player into the pair.
    """
    ev=np.asarray(eval_score,dtype=float)
    mp=np.asarray(market_pick,dtype=float)
    if len(ev)<2 or next_pick is None or int(next_pick)!=int(current_pick)+1:
        return None

    normal,ready,surv=execution_choice(ev,mp,current_pick,next_pick,randomness)
    normal2,intercepted,_=faller_intercept_choice(
        ev,mp,current_pick,normal,min_fall=10,model_band=16
    )

    # Approved means V7.10 timing window is already open.
    approved=np.flatnonzero(ready)
    if normal2 is not None and int(normal2) not in approved:
        approved=np.append(approved,int(normal2))

    if approved.size<2:
        return None

    # Keep only the strongest V7.10-approved candidates.
    order=approved[np.argsort(ev[approved])[::-1]]
    order=order[:min(int(approved_cap),len(order))]
    if len(order)<2:
        return None

    # Select the best two approved players strictly by V7.10 evaluation.
    pair=order[:2].copy()

    # Sequence them using market urgency/faller value only.
    fall=np.maximum(float(current_pick)-mp[pair],0)
    reach=np.maximum(mp[pair]-float(current_pick),0)

    # More valuable faller first; otherwise earlier market player first.
    a,b=int(pair[0]),int(pair[1])
    score_a=.45*fall[0]-.08*reach[0]
    score_b=.45*fall[1]-.08*reach[1]
    if score_b>score_a:
        a,b=b,a
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None, room_profile="Consensus"):
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                # Use a previously-approved turn sequence second pick only if it remains legal.
                if pending_turn_choice is not None and int(pending_turn_choice) in set(eidx.tolist()):
                    ...
                    # V7.10 stable PLAYER EVALUATION.
                    ...
                    # V7.12: turn sequencing may only use V7.10-approved players.
                    turn_plan=constrained_turn_sequence(
                        eval_score,cr,overall,nxt,randomness,approved_cap=8
                    )
                    if turn_plan is not None:
                        chosen=int(sidx[int(turn_plan["first"])])
                        pending_turn_choice=int(sidx[int(turn_plan["second"])])
                    else:
                        # V7.10 execution + faller intercept unchanged.
...
st.title("🏈 Fantasy Edge V7.12 — Constrained Turn Sequencing")
st.caption("V7.12 • V7.10 rollback + constrained turn sequencing • no WAIT-only pair promotions")
st.caption("Build ID: V7.12-CTS-20260813")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v712_live_slot")
    ...
    constrained_pair=None
    if live_next is not None and int(live_next)==int(current_overall)+1:
        tp=constrained_turn_sequence(
            x.evaluation_score.to_numpy(float),
            cr.to_numpy(float),
            current_overall,live_next,
            state.get("mock",{}).get("randomness",6),
            approved_cap=8
        )
        if tp is not None:
            constrained_pair=(x.iloc[int(tp["first"])],x.iloc[int(tp["second"])],tp)
            p1,p2,_=constrained_pair
            ...
            st.markdown("### 🔁 Next Two Picks — Constrained Turn Sequence")
            st.success(f"1. {p1.player} ({p1.position})  →  2. {p2.player} ({p2.position})")
            st.caption(f"Both players already passed V7.10 timing rules. Turn logic only sequences the order. After #{live_next}, about {wait_picks} other picks occur before your next turn.")
    ...
    if constrained_pair is not None:
        p1,p2,tp=constrained_pair
        p1_action,_=market_timing_state(p1.market_pick,current_overall,live_next,state.get("mock",{}).get("randomness",6))
        prefix="💎" if p1_action=="VALUE FALLER" else "✅"
        st.success(f"{prefix} DRAFT NOW — TURN SEQUENCE 1: {p1.player} ({p1.position}, {p1.team})")
        st.caption(f"Immediately follow with {p2.player} ({p2.position}) at pick #{live_next}. No WAIT-only player can enter this pair.")
    elif intercepted:
    ...
    if "v712_target_queue" not in st.session_state:
        st.session_state.v712_target_queue={}
    queue=st.session_state.v712_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v712_target_queue=queue
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                constrained_pair=None
                if nxt is not None and int(nxt)==int(overall)+1:
                    tp=constrained_turn_sequence(
                        avail.evaluation_score.to_numpy(float),
                        cr.to_numpy(float),
                        overall,nxt,randomness,approved_cap=8
                    )
                    if tp is not None:
                        constrained_pair=(avail.iloc[int(tp["first"])],avail.iloc[int(tp["second"])],tp)
                        cp1,cp2,_=constrained_pair
                        st.markdown("### 🔁 Next Two Picks — Constrained Turn Sequence")
                        st.success(f"1. {cp1.player} ({cp1.position})  →  2. {cp2.player} ({cp2.position})")
                        st.caption("Both players already pass V7.10 execution rules; turn logic only changes order.")
                ...
                if constrained_pair is not None:
                    dn=constrained_pair[0]
                    st.success(f"✅ DRAFT THIS PICK — TURN SEQUENCE: {dn.player}")
                    st.caption(f"Then take {constrained_pair[1].player} with the immediately adjacent pick #{nxt}.")
                elif intercepted:
                ...
                if constrained_pair is not None:
                    pair_name=constrained_pair[0].player
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.12 robustness report")
            ...
            st.download_button("⬇️ Download V7.12 diagnostic CSV",csv,"fantasy_edge_v7_12_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.12 independent validation")
        ...
        if "v712_independent_results" not in st.session_state:
            st.session_state.v712_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v712_independent_results"),pd.DataFrame) else "ready to run"))
        if "v712_independent_summary" not in st.session_state:
            st.session_state.v712_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v712_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v712_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v712_run")
        clear_iv=clear_col.button("Clear independent results",key="v712_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v712_independent_results=iv
                st.session_state.v712_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v712_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v712_independent_summary") or {}
            st.success(
            ...
            turn_mask=iv_saved.Draft_slot.isin([1,int(teams)])
            if turn_mask.any() and (~turn_mask).any():
                q1,q2,q3,q4=st.columns(4)
                q1.metric("Turn-slot gap",f"{iv_saved.loc[turn_mask,'Difference'].mean():+.2f}")
                q2.metric("Non-turn gap",f"{iv_saved.loc[~turn_mask,'Difference'].mean():+.2f}")
                q3.metric("Turn avg reach",f"{iv_saved.loc[turn_mask,'FE_avg_reach'].mean():.1f}")
                q4.metric("Turn avg faller",f"{iv_saved.loc[turn_mask,'FE_avg_faller'].mean():.1f}")
            ...
            st.download_button(
                "⬇️ Download V7.12 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_12_independent_validation.csv",
                "text/csv",
                key="v712_download"
            )
