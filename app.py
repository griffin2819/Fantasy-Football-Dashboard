...
st.set_page_config(page_title="Fantasy Edge v9.46 — Exact League + Kicker Requirement", page_icon="🏈", layout="wide")
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    for _p in ("DL","DB"):
        _pm=x["position"].eq(_p) & x["idp_eligible"].fillna(False)
        if _pm.any():
...
st.caption("Build ID: FE-V9.46-DOMINANCE-ROSTEROPP-IDP-20260821")
