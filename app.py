    ...
    if _my_roster.empty:
        ...
        if _moves_df.empty:
            ...
            if not _allow_protected and len(_moves_df):
                _moves_df=_moves_df[pd.to_numeric(_moves_df["Drop Cost"],errors="coerce").fillna(999).lt(20.0)].copy()
                _moves_df=_moves_df[~_moves_df["Drop"].astype(str).isin(_starter_names)].copy()
