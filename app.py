...
st.title("🏈 Fantasy Edge V7.41 — Pick 12 Execution Activation Audit")
st.caption("V7.41 • V7.24 9× champion frozen • Pick 12 execution activation audit")
st.caption("Build ID: V7.41-P12EAA-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v741_live_slot")
    ...
    if "v741_target_queue" not in st.session_state:
        st.session_state.v741_target_queue={}
    queue=st.session_state.v741_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v741_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.41 keeps the V7.24 9× champion completely frozen. No candidate execution rule is active.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.41 robustness report")
        ...
        st.markdown("### 🧪 V7.41 champion baseline validation")
        st.info("V7.41 is diagnostic only. It measures sequential activation gates and counterfactual trigger rates with survival or reach removed.")
        ...
        if "v741_independent_results" not in st.session_state:
            st.session_state.v741_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v741_independent_results"),pd.DataFrame) else "ready to run"))
        if "v741_independent_summary" not in st.session_state:
            st.session_state.v741_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v741_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v741_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v741_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v741_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v741_independent_results=iv
                st.session_state.v741_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v741_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v741_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.41 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.41 diagnoses why the Pick-12 V7.40 execution trigger did not change behavior.")
            ...
            st.download_button(
                "⬇️ Download V7.41 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_41_champion_baseline.csv",
                "text/csv",
                key="v741_baseline_download"
            )
        ...
        st.markdown("### 🔎 V7.41 — Pick 12 Execution Activation Audit")
        st.caption(
            "Diagnostic only. V7.24 9× remains frozen. This measures exactly where the V7.40 faller-vs-need trigger dies. "
            "No execution rule or candidate model is active."
        )
        if "v741_results" not in st.session_state: st.session_state.v741_results=None
        if "v741_meta" not in st.session_state: st.session_state.v741_meta=None

        sims=st.number_input("Activation-audit simulations per room",min_value=5,max_value=25,value=10,step=5,key="v741_sims")
        aa,ab=st.columns(2)
        run=aa.button("Run Pick 12 activation audit",type="primary",key="v741_run")
        clear=ab.button("Clear activation audit",key="v741_clear")
        if clear:
            st.session_state.v741_results=None; st.session_state.v741_meta=None; st.rerun()

        if run:
            rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]
            fast_champ=_fast_benchmark_pool(board,int(teams)); rows=[]
            total=len(rooms)*int(sims); done=0
            prog=st.progress(0); status=st.empty(); started=time.perf_counter()

            # Reconstruct each champion R4-6 decision from its MR audit fields.
            # Thresholds mirror V7.40 B (7 fall / 8 eval / 5 reach / 55% survival),
            # with explicit counterfactual no-survival and no-reach gates.
            for room_name in rooms:
                for j in range(int(sims)):
                    seed=941000+j*71+sum(ord(c) for c in room_name)
                    fe=simulate_mock_fast(fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name)
                    audited=fe[pd.to_numeric(fe.get("MR_Audit",0),errors="coerce").fillna(0).eq(1)].copy()
                    for _,r in audited.iterrows():
                        selpos=str(r.get("MR_Selected_position",""))
                        rbte=selpos in ["RB","TE"]
                        reach=float(pd.to_numeric(pd.Series([r.get("MR_Selected_reach",np.nan)]),errors="coerce").iloc[0])
                        surv=float(pd.to_numeric(pd.Series([r.get("MR_Selected_survival_est",np.nan)]),errors="coerce").iloc[0])
                        fall=float(pd.to_numeric(pd.Series([r.get("MR_Passed_faller_amount",np.nan)]),errors="coerce").iloc[0])
                        deficit=float(pd.to_numeric(pd.Series([r.get("MR_Passed_faller_eval_gap",np.nan)]),errors="coerce").iloc[0])
                        fallpos=str(r.get("MR_Passed_faller_position",""))
                        reach_pass=bool(rbte and pd.notna(reach) and reach>=5.0)
                        faller_exists=bool(rbte and pd.notna(fall) and fall>=7.0)
                        eval_pass=bool(faller_exists and pd.notna(deficit) and deficit<=8.0)
                        survival_pass=bool(rbte and pd.notna(surv) and surv<=.55)
                        full=bool(rbte and reach_pass and faller_exists and eval_pass and survival_pass)
                        no_surv=bool(rbte and reach_pass and faller_exists and eval_pass)
                        no_reach=bool(rbte and faller_exists and eval_pass and survival_pass)
                        rows.append({
                            "Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Round":int(r.get("mock_round",0)),"Pick":int(r.get("mock_pick",0)),
                            "Selected_player":r.get("MR_Selected_player",""),"Selected_position":selpos,
                            "RBTE_gate":rbte,"Normal_reach":reach,"Reach_gate":reach_pass,
                            "Passed_faller_player":r.get("MR_Passed_faller_player",""),
                            "Passed_faller_position":fallpos,"Best_faller_amount":fall,
                            "Faller_gate":faller_exists,"Faller_eval_deficit":deficit,"Eval_gate":eval_pass,
                            "Normal_survival_est":surv,"Survival_gate":survival_pass,
                            "Full_trigger":full,"Trigger_without_survival":no_surv,"Trigger_without_reach":no_reach,
                            "Passed_faller_survived_actual":r.get("MR_Passed_faller_survived_actual",np.nan)
                        })
                    done+=1; prog.progress(done/total)
                    if done==1 or done%10==0 or done==total: status.caption(f"Activation audit: {done}/{total} drafts")
            df=pd.DataFrame(rows); st.session_state.v741_results=df
            st.session_state.v741_meta={"seconds":time.perf_counter()-started,"drafts":total,"decisions":len(df)}
            status.success(f"Activation audit complete: {len(df)} R4-6 decisions."); prog.empty()

        df=st.session_state.get("v741_results"); meta=st.session_state.get("v741_meta") or {}
        if isinstance(df,pd.DataFrame) and len(df):
            n=len(df)
            gates=pd.DataFrame([
                {"Stage":"1 • RB/TE normal choice","Passes":int(df.RBTE_gate.sum()),"Pass rate":df.RBTE_gate.mean()},
                {"Stage":"2 • Reach ≥5","Passes":int((df.RBTE_gate & df.Reach_gate).sum()),"Pass rate":(df.RBTE_gate & df.Reach_gate).mean()},
                {"Stage":"3 • Faller ≥7 exists","Passes":int((df.RBTE_gate & df.Reach_gate & df.Faller_gate).sum()),"Pass rate":(df.RBTE_gate & df.Reach_gate & df.Faller_gate).mean()},
                {"Stage":"4 • Faller within 8 FE points","Passes":int((df.RBTE_gate & df.Reach_gate & df.Faller_gate & df.Eval_gate).sum()),"Pass rate":(df.RBTE_gate & df.Reach_gate & df.Faller_gate & df.Eval_gate).mean()},
                {"Stage":"5 • Need survival ≤55%","Passes":int(df.Full_trigger.sum()),"Pass rate":df.Full_trigger.mean()},
            ])
            st.markdown("#### Sequential activation funnel")
            st.dataframe(gates,use_container_width=True,hide_index=True)

            c1,c2,c3=st.columns(3)
            c1.metric("Full trigger",f"{df.Full_trigger.mean():.1%}")
            c2.metric("Without survival gate",f"{df.Trigger_without_survival.mean():.1%}")
            c3.metric("Without reach gate",f"{df.Trigger_without_reach.mean():.1%}")

            rb=df[df.RBTE_gate].copy()
            if len(rb):
                dist=pd.DataFrame([{
                    "Avg normal reach":rb.Normal_reach.mean(),
                    "Median normal reach":rb.Normal_reach.median(),
                    "Avg survival estimate":rb.Normal_survival_est.mean(),
                    "Median survival estimate":rb.Normal_survival_est.median(),
                    "Avg passed-faller amount":rb.Best_faller_amount.mean(),
                    "Median faller amount":rb.Best_faller_amount.median(),
                    "Avg faller eval deficit":rb.Faller_eval_deficit.mean(),
                    "Actual faller survival":pd.to_numeric(rb.Passed_faller_survived_actual,errors="coerce").mean()
                }])
                st.markdown("#### Pick-12 RB/TE decision distributions")
                st.dataframe(dist.round(3),use_container_width=True,hide_index=True)

                pos=rb.groupby("Passed_faller_position",dropna=False,as_index=False).agg(
                    Decisions=("RBTE_gate","count"),Avg_fall=("Best_faller_amount","mean"),
                    Avg_eval_deficit=("Faller_eval_deficit","mean"),Full_triggers=("Full_trigger","sum"))
                st.markdown("#### Passed-faller position profile")
                st.dataframe(pos.sort_values("Decisions",ascending=False).round(3),use_container_width=True,hide_index=True)

                room=rb.groupby("Room",as_index=False).agg(
                    Decisions=("RBTE_gate","count"),Reach_pass=("Reach_gate","mean"),Faller_pass=("Faller_gate","mean"),
                    Eval_pass=("Eval_gate","mean"),Survival_pass=("Survival_gate","mean"),
                    Full_trigger=("Full_trigger","mean"),No_survival=("Trigger_without_survival","mean"),
                    No_reach=("Trigger_without_reach","mean"))
                st.markdown("#### Activation by room")
                st.dataframe(room.round(3),use_container_width=True,hide_index=True)

            full=int(df.Full_trigger.sum()); ns=int(df.Trigger_without_survival.sum()); nr=int(df.Trigger_without_reach.sum())
            if full==0 and ns>0:
                st.warning("The survival gate is the blocking condition: viable faller/reach cases exist before survival is applied. V7.42 should test execution without the survival estimator.")
            elif full==0 and nr>0:
                st.warning("The reach gate is the blocking condition. V7.42 should test a faller-urgency rule without requiring the normal RB/TE to be a reach.")
            elif full==0:
                st.warning("The trigger dies before both counterfactual gates. Inspect the funnel: the faller or FE-evaluation requirement is the real blocker.")
            else:
                st.info("The V7.40 logic can activate in this audit. If V7.40 still produced no behavior change, the next audit should inspect post-trigger override ordering.")

            st.download_button("⬇️ Download V7.41 Pick 12 activation audit CSV",df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_41_pick12_execution_activation_audit.csv","text/csv",key="v741_download")
