...
st.title("🏈 Fantasy Edge V7.97 — Round-4 Generic Adaptation Signature Audit")
st.caption("V7.97 • V7.93 frozen champion • Round-4 generic signature audit")
st.caption("Build ID: V7.97-R4-GENERIC-SIGNATURE-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V7.97 — Round-4 Generic Adaptation Signature Audit")
        st.caption(
            "Measurement only. V7.93 remains frozen champion. "
            "V7.97 isolates the Round-4 dominance events from V7.96 and searches for a room-agnostic, player-agnostic, slot-agnostic signature "
            "using market distance, score/VORP dominance, roster state after R1–3, and positional context."
        )

        if "v797_results" not in st.session_state:
            st.session_state.v797_results=None

        sims=st.number_input(
            "V7.97 simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v797_sims",
            help="Default: 12 slots × 5 rooms × 10 = 600 fresh frozen-champion drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.97 R4 signature audit",type="primary",key="v797_run")
        clear=b.button("Clear V7.97 audit",key="v797_clear")
        if clear:
            st.session_state.v797_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=997000+int(ds)*100000+j*379+sum(ord(c) for c in room_name)
                        ...
                        if len(r4):
                            ...
                            # Use V7.96 true-board alternatives.
                            for k in range(1,6):
                                alt_player=str(r.get(f"R796_Alt{k}_player",""))
                                if not alt_player:
                                    continue
                                alt_vorp=pd.to_numeric(pd.Series([r.get(f"R796_Alt{k}_vorp",np.nan)]),errors="coerce").iloc[0]
                                alt_score=pd.to_numeric(pd.Series([r.get(f"R796_Alt{k}_draft_score",np.nan)]),errors="coerce").iloc[0]
                                alt_market=pd.to_numeric(pd.Series([r.get(f"R796_Alt{k}_market_pick",np.nan)]),errors="coerce").iloc[0]
                                alt_distance=pd.to_numeric(pd.Series([r.get(f"R796_Alt{k}_market_distance",np.nan)]),errors="coerce").iloc[0]

                                score_change=alt_score-champ_score if pd.notna(alt_score) and pd.notna(champ_score) else np.nan
                                vorp_change=alt_vorp-champ_vorp if pd.notna(alt_vorp) and pd.notna(champ_vorp) else np.nan
                                dominates=bool(pd.notna(score_change) and pd.notna(vorp_change) and score_change>0 and vorp_change>0)

                                rows.append({
                                    "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                    "Round4_pick":champ_pick,
                                    ...
                                    "QB_count_pre":r.get("R796_QB_count_pre",0),
                                    "RB_count_pre":r.get("R796_RB_count_pre",0),
                                    "WR_count_pre":r.get("R796_WR_count_pre",0),
                                    "TE_count_pre":r.get("R796_TE_count_pre",0),
                                    "Alt_rank":k,
                                    "Alt_player":alt_player,
                                    "Alt_position":str(r.get(f"R796_Alt{k}_position","")),
                                    "Alt_market_pick":alt_market,
                                    "Alt_market_distance":alt_distance,
                                    "Alt_VORP":alt_vorp,
                                    "Alt_draft_score":alt_score,
                                    "VORP_change":vorp_change,
                                    "Draft_score_change":score_change,
                                    "Dominates":dominates,
                                    "Sig_le_5":dominates and pd.notna(alt_distance) and alt_distance<=5,
                                    "Sig_le_10":dominates and pd.notna(alt_distance) and alt_distance<=10,
                                    "Sig_le_15":dominates and pd.notna(alt_distance) and alt_distance<=15,
                                    "Sig_le_20":dominates and pd.notna(alt_distance) and alt_distance<=20,
                                })

                        done+=1
                        prog.progress(done/total)
                        if done%100==0 or done==total:
                            status.caption(f"V7.97: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v797_results=df
            status.success(f"V7.97 complete: {len(df):,} Round-4 alternative comparisons.")
            prog.empty()

        df=st.session_state.get("v797_results")
        if isinstance(df,pd.DataFrame) and len(df):
            # Keep only the top-ranked available alternative for the primary generic signature tables.
            top=df[df.Alt_rank==1].copy()

            room=top.groupby("Room",as_index=False).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Dominates","mean"),
                Sig_5=("Sig_le_5","mean"),
                Sig_10=("Sig_le_10","mean"),
                Sig_15=("Sig_le_15","mean"),
                Sig_20=("Sig_le_20","mean"),
                Avg_VORP_change=("VORP_change","mean"),
                Avg_score_change=("Draft_score_change","mean"),
            )
            st.markdown("#### Generic R4 signature by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            slot=top.groupby("Draft_slot",as_index=False).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Dominates","mean"),
                Sig_10=("Sig_le_10","mean"),
                Avg_VORP_change=("VORP_change","mean"),
            ...
            st.markdown("#### Generic R4 signature by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            roster=top.groupby(
                ["QB_count_pre","RB_count_pre","WR_count_pre","TE_count_pre",
                 "Champion_position","Alt_position"],as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Dominates","mean"),
                Sig_10=("Sig_le_10","mean"),
            ...
            st.markdown("#### Post-R1–3 roster-state signature")
            st.dataframe(roster.head(50).round(4),use_container_width=True,hide_index=True)

            # Depth: how many of top 5 dominate?
            depth=df.groupby(["Draft_slot","Room","Simulation","Seed"],as_index=False).agg(
                Dominating_alts_top5=("Dominates","sum"),
                Any_le10=("Sig_le_10","max")
            )
            depth_room=depth.groupby("Room",as_index=False).agg(
                Avg_dominating_alts_top5=("Dominating_alts_top5","mean"),
                Any_le10_rate=("Any_le10","mean"),
                Two_plus_dominating=("Dominating_alts_top5",lambda x:(x>=2).mean())
            )
            st.markdown("#### Dominance depth across top five")
            st.dataframe(depth_room.round(4),use_container_width=True,hide_index=True)

            paths=top[top.Sig_le_10].groupby(
                ["Champion_position","Alt_position"],as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Avg_VORP_change=("VORP_change","mean"),
            ...
            st.markdown("#### Generic position-transition paths at ≤10")
            st.dataframe(paths.round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.97 is measurement-only. The goal is a generic Round-4 trigger that does not encode player names, room labels, or draft-slot cutoffs. "
                "If a clean signature emerges, the next build can test it as a full-draft challenger."
            )

            st.download_button(
                "⬇️ Download V7.97 Round-4 generic adaptation signature audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_97_round4_generic_adaptation_signature_audit.csv",
                "text/csv",key="v797_download"
            )
