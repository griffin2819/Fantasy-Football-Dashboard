...
st.title("🏈 Fantasy Edge V7.71 — Round-5 Dominance Override Challenger")
st.caption("V7.71 • V7.67 frozen champion • Round-5 dominance challenger")
st.caption("Build ID: V7.71-R5-DOMINANCE-CHALLENGER-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.71 keeps certified V7.67 frozen/live and tests a Round-5 true-board dominance challenger with no player or slot hard-coding.")
        ...
        st.markdown("### 🧪 V7.71 — Round-5 Dominance Override Challenger")
        st.caption(
            "Matched challenger only. V7.67 remains frozen/live. The challenger changes only Round 5, and only when the normal champion selection "
            "reaches >10 picks while a genuinely available market-respecting alternative has BOTH higher frozen draft score and higher VORP. "
            "No player names, slots, or room types are hard-coded."
        )

        if "v771_results" not in st.session_state:
            st.session_state.v771_results=None

        sims=st.number_input(
            "V7.71 matched simulations per slot × room",
            min_value=10,max_value=50,value=25,step=5,key="v771_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 fresh matched V7.67-vs-challenger drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-5 dominance challenger",type="primary",key="v771_run")
        clear=b.button("Clear V7.71 challenger",key="v771_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=971000+int(ds)*100000+j*227+sum(ord(c) for c in room_name)
                        ...
                        c5=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(5)]
                        n5=cand[pd.to_numeric(cand.mock_round,errors="coerce").eq(5)]
                        champ5=str(c5.iloc[0].player) if len(c5) else ""
                        cand5=str(n5.iloc[0].player) if len(n5) else ""
                        champ5_pos=str(c5.iloc[0].position) if len(c5) else ""
                        ...
                        rows.append({
                            ...
                            "Conditional_R7_10_nonloss":(r710>=0) if changed else np.nan,
                        ...
                        if done==1 or done%100==0 or done==total:
                            status.caption(f"V7.71 challenger: {done}/{total} fresh matched drafts")

            df=pd.DataFrame(rows)
            st.session_state.v771_results=df
            status.success(f"V7.71 complete: {len(df)} matched drafts.")
            prog.empty()

        df=st.session_state.get("v771_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            cond710=changed.Conditional_R7_10_nonloss.mean() if len(changed) else np.nan
            ...
            st.markdown("#### Round-5 override paths")
            ...
            if screen:
                st.success(
                    "V7.71 SCREEN PASSED. The Round-5 dominance override improves whole-draft and R4-6 performance without material downstream damage. "
                    "Move this exact frozen rule into larger validation before promotion."
                )
            else:
                st.warning(
                    "V7.71 does not clear screening. Keep V7.67 frozen and do not create a blanket Round-5 anti-reach rule."
                )

            st.download_button(
                "⬇️ Download V7.71 Round-5 dominance challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_71_round5_dominance_challenger.csv",
                "text/csv",key="v771_download"
            )
