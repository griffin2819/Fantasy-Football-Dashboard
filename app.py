...
st.set_page_config(page_title="Fantasy Edge v9.2 — Live Draft", page_icon="🏈", layout="wide")
...
def _injury_overlay_penalty(status):
    ...
    if x in ["pup","reserve/pup"]: return 30.0
    if x in ["out","o","suspended","susp"]: return 24.0
    if x in ["doubtful","d"]: return 12.0
    if x in ["questionable","q"]: return 4.0
    return 0.0
...
st.title("🏈 Fantasy Edge v9.2 — Live Draft + Injury Intelligence")
...
with tabs[1]:
    st.subheader("Live roster-aware draft assistant — v9.2")
    st.success("✅ v9.08 certified engine active • v9.2 live injury overlay active • Mine/Opponent tracking enabled")
    ...
    with st.expander("🚑 Live Injury Center", expanded=False):
        ...
        # Keep the default Injury Center focused on players who can materially affect this draft.
        # Draft relevance is based on the certified production board and current market/model ranks.
        draft_relevant=pd.Series(False,index=flagged.index)
        if "v9_model_rank" in flagged.columns:
            draft_relevant |= pd.to_numeric(flagged["v9_model_rank"],errors="coerce").le(200)
        if "v9_market_pick" in flagged.columns:
            draft_relevant |= pd.to_numeric(flagged["v9_market_pick"],errors="coerce").le(200)
        if "market_pick" in flagged.columns:
            draft_relevant |= pd.to_numeric(flagged["market_pick"],errors="coerce").le(200)
        ...
        show_all_injuries=st.toggle(
            ...
            help="Off = draft-relevant players only. Turn on to inspect the full injury/status feed."
        )
        ...
        if len(display_flagged):
            ...
            st.caption(
                ...
                + ("injured/status players." if show_all_injuries else "draft-relevant injured/status players.")
            )
            st.dataframe(
            ...
            if len(flagged) and not show_all_injuries:
                st.caption("No draft-relevant injury designations right now. Turn on “Show all” to inspect the full feed.")
            else:
    ...
    # v9.2 injury overlay is temporary and does not alter the frozen certified board.
    x["evaluation_score"]-=pd.to_numeric(x.get("injury_penalty",0),errors="coerce").fillna(0)

    # PICK EXECUTION — market timing remains a presentation/execution layer around v9.1 ranking.
    ...
    st.dataframe(show[["player","position","team","Model target #","Action","Market pick","Faller picks","Survives next","projection","vorp","Profile","Confidence","injury","injury_source"]].rename(columns={"injury":"Injury","injury_source":"Injury source"}).head(120),use_container_width=True,hide_index=True)
