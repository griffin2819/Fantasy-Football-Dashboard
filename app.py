...
def _small_roster_utility_df(roster, slots):
    ...
    bench_mult={"QB":[.18,.05],"RB":[.34,.25,.15,.07],"WR":[.36,.28,.18,.10,.05],
                "TE":[.20,.07],"DL":[.08],"DB":[.08]}
...
def _candidate_roster_delta_df(roster, candidates, slots):
    """Marginal expected roster utility for each candidate."""
    ...
    for _,row in candidates.iterrows():
        ...
        vals.append(_small_roster_utility_df(temp,slots)-current)
...
def _fast_benchmark_pool(board):
    ...
    return {"pool":pool,"pos_names":pos_names,"pos_code":pos_code,"consensus":consensus,
            "model_rank":model_rank,"draft_score":draft_score,"projection":projection,"vorp":vorp}
...
def _fast_roster_utility(fast, selected, slots):
    ...
    mults={0:[.18,.05],1:[.34,.25,.15,.07],2:[.36,.28,.18,.10,.05],3:[.20,.07],4:[.08],5:[.08]}
    for code in range(6):
...
def _fast_candidate_deltas(fast, selected, candidates, slots):
    current=_fast_roster_utility(fast,selected,slots)
    return np.asarray([_fast_roster_utility(fast,selected+[int(c)],slots)-current for c in candidates],dtype=float)
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                delta=_fast_candidate_deltas(fast,user_idx,eidx,slots)
                cr=np.where(np.isnan(consensus[eidx]),float(overall),consensus[eidx])
                market_pen=np.maximum(overall-cr,0)*.08
                # Player model remains the anchor; roster utility changes which bench shape is worth buying.
                score=draft_score[eidx] + 2.35*delta - market_pen
                if round_no<=7:
                ...
                cr=np.where(np.isnan(consensus[eidx]),model_rank[eidx],consensus[eidx])
                chosen=int(eidx[int(np.argmin(cr+noise))])
...
st.title("🏈 Fantasy Edge V7.3 — Dynamic Roster Value")
st.caption("V7.3 • Dynamic Roster Value • expected final-roster utility • frozen projection/regression + grading • Yahoo + IDP")
st.caption("Build ID: V7.3-DRV-20260813")
...
with tabs[1]:
    ...
    live_delta=_candidate_roster_delta_df(myb,x,slots)
    x["roster_delta"]=live_delta
    x["live_score"]=x.draft_score+2.35*live_delta
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                delta=_candidate_roster_delta_df(roster,avail,state["roster_slots"])
                avail["live"]=avail.draft_score+2.35*delta
                if rnd<=7:
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.3 validation report")
            st.caption("V7.3 evaluates every available player's marginal contribution to the expected final roster instead of targeting a predetermined bench shape. Identity fixes, projections, regression/progression and grading remain frozen.")
            ...
            st.download_button("⬇️ Download V7.3 diagnostic CSV",csv,"fantasy_edge_v7_3_diagnostics.csv","text/csv")
