...
st.title("🏈 Fantasy Edge V7.77 — Round-2 True-Board Dominance Challenger")
st.caption("V7.77 • V7.67 frozen champion • Round-2 full-draft challenger")
st.caption("Build ID: V7.77-R2-DOMINANCE-CHALLENGER-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.77 keeps V7.67 frozen/live and tests the generic Round-2 true-board score+VORP dominance rule across the full remainder of each draft.")
        ...
        st.markdown("### 🧪 V7.77 — Round-2 True-Board Dominance Challenger")
        st.caption(
            "Full-draft matched challenger. V7.67 remains frozen/live. Round 2 changes only when the normal champion choice reaches >10 picks "
            "and a genuinely alive+eligible market-respecting alternative has BOTH higher frozen draft score and higher VORP. "
            "No player, position, slot, or room is hard-coded."
        )
        if "v777_results" not in st.session_state: st.session_state.v777_results=None

        sims=st.number_input(
            "V7.77 matched simulations per slot × room",10,50,25,5,key="v777_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 fresh matched sets. Trigger-first: challenger reruns only on actual Round-2 triggers."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-2 dominance challenger",type="primary",key="v777_run")
        clear=b.button("Clear V7.77 challenger",key="v777_clear")
        if clear:
            st.session_state.v777_results=None; st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=977000+int(ds)*100000+j*257+sum(ord(c) for c in room_name)
                        champ=simulate_mock_fast(
                            pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name,
                            r5_dominance_override_enabled=False,r2_dominance_override_enabled=False
                        )

                        r2=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(2)]
                        trigger=False; champ2=""; champ2pos=""; alt=""
                        if len(r2):
                            rr=r2.iloc[0]
                            trigger=bool(int(pd.to_numeric(pd.Series([rr.get("R777_Would_trigger",0)]),errors="coerce").fillna(0).iloc[0]))
                            champ2=str(rr.player); champ2pos=str(rr.position); alt=str(rr.get("R777_Best_alt_player",""))

                        if trigger:
                            cand=simulate_mock_fast(
                                pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name,
                                r5_dominance_override_enabled=False,r2_dominance_override_enabled=True
                            )
                            reruns+=1
                            ...
                            n2=cand[pd.to_numeric(cand.mock_round,errors="coerce").eq(2)]
                            cand2=str(n2.iloc[0].player) if len(n2) else ""
                            cand2pos=str(n2.iloc[0].position) if len(n2) else ""
                            ...
                            changed=0; cand2=champ2; cand2pos=champ2pos
                        ...
                        rows.append({
                            ...
                            "Champion_round2_player":champ2,"Champion_round2_position":champ2pos,
                            "Detected_best_alt":alt,
                            "Candidate_round2_player":cand2,"Candidate_round2_position":cand2pos,
                            ...
                            "Conditional_R1_3_win":(r13>0) if trigger else np.nan,
                            "Conditional_R4_6_nonloss":(r46>=0) if trigger else np.nan,
                            "Conditional_R7_10_nonloss":(r710>=0) if trigger else np.nan,
                            "Conditional_R11_nonloss":(r11>=0) if trigger else np.nan,
                        })
                        done+=1; prog.progress(done/total)
                        if done==1 or done%100==0 or done==total:
                            status.caption(f"V7.77: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows); st.session_state.v777_results=df
            status.success(f"V7.77 complete: {len(df)} matched sets; {int(df.Challenger_rerun.sum())} challenger reruns.")
            prog.empty()

        df=st.session_state.get("v777_results")
        if isinstance(df,pd.DataFrame) and len(df):
            trig=df[df.Trigger_detected].copy()
            overall=df.Overall_change.mean(); r13=df.R1_3_change.mean()
            r46=df.R4_6_change.mean(); r710=df.R7_10_change.mean(); r11=df.R11_plus_change.mean()
            ...
            cond13=trig.Conditional_R1_3_win.mean() if len(trig) else np.nan

            summary=pd.DataFrame([{
                "Matched sets":len(df),"Triggered sets":len(trig),"Activation":activation,
                "Overall change":overall,"R1-3 change":r13,"R4-6 change":r46,
                "R7-10 change":r710,"R11+ change":r11,
                "Conditional overall win":cond,"Conditional non-loss":nonloss,
                "Conditional R1-3 win":cond13,
            }])
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            slot=df.groupby("Draft_slot",as_index=False).agg(
                Overall_change=("Overall_change","mean"),R1_3_change=("R1_3_change","mean"),
                R4_6_change=("R4_6_change","mean"),R7_10_change=("R7_10_change","mean"),
                R11_plus_change=("R11_plus_change","mean"),Activation=("Trigger_detected","mean")
            )
            ...
            st.markdown("#### Challenger by draft slot"); st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Challenger by room"); st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            paths=trig.groupby(
                ["Champion_round2_player","Champion_round2_position","Candidate_round2_player","Candidate_round2_position"],
                as_index=False
            ).agg(
                Decisions=("Trigger_detected","count"),Overall_change=("Overall_change","mean"),
                R1_3_change=("R1_3_change","mean"),R4_6_change=("R4_6_change","mean"),
                R7_10_change=("R7_10_change","mean"),R11_plus_change=("R11_plus_change","mean"),
                Win_rate=("Conditional_overall_win","mean")
            ).sort_values("Decisions",ascending=False)
            st.markdown("#### Round-2 override paths"); st.dataframe(paths.head(30).round(4),use_container_width=True,hide_index=True)

            active_rooms=room[room.Activation>0]
            active_slots=slot[slot.Activation>0]
            screen=(
                overall>0 and r13>0
                and r46>-0.05 and r710>-0.05 and r11>-0.05
                and activation>=.02
                and pd.notna(cond) and cond>=.70
                and pd.notna(nonloss) and nonloss>=.75
                and pd.notna(cond13) and cond13>=.90
            ...
            if screen:
                st.success("V7.77 SCREEN PASSED. Advance the exact frozen Round-2 rule to larger independent validation; V7.67 remains champion.")
            else:
                st.warning("V7.77 does not clear screening. Keep V7.67 champion and do not tune the Round-2 rule from this sample.")

            st.download_button(
                "⬇️ Download V7.77 Round-2 dominance challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_77_round2_dominance_challenger.csv","text/csv",key="v777_download"
            )
