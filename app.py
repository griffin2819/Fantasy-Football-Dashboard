...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Fast Finalize", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v781_render_page(_v777_page_idx):
    ...
    if _v777_page_idx==3:
        ...
        _fa_raw=board[board["player"].map(lambda n:_is_free_agent(state,n))].copy()
        ...
        if _my_roster.empty:
            ...
            _fa_eval=_fa_eval.sort_values("_v804_prefilter",ascending=False).groupby("position",sort=False).head(18).copy()
            ...
            _fa_eval=_fa_eval.sort_values(["_candidate_value","_v739_opportunity_boost","vorp"],ascending=False).head(18)
            ...
            if _v737_selected_emergency and len(_eligible_drops):
                ...
                _weak_overall=_eligible_drops.sort_values(["_emergency_drop_rank","_drop_cost","_drop_value","_ros_value"]).head(6)
            else:
                _eligible_drops["_emergency_drop_rank"]=_eligible_drops["_drop_cost"]
                _weak_overall=_eligible_drops.sort_values(["_drop_cost","_drop_value","_ros_value"]).head(5)
            ...
            for _,add_raw in _fa_eval.iterrows():
                ...
                if str(add.get("position","")).upper() in _v737_active_emergencies:
                    ...
                    _drop_pool=_drop_pool[_drop_pool["_emergency_drop_rank"].lt(900)].sort_values(["_emergency_drop_rank","_drop_cost","_drop_value","_ros_value"]).head(6)
                else:
                    _same_pos=_eligible_drops[_eligible_drops["position"].eq(add["position"])].sort_values(["_drop_cost","_drop_value","_ros_value"]).head(3)
                    ...
                    if _add_pos_u in ("QB","TE","K") and _req_u>0 and _playable_u>=_req_u+1:
                        ...
                        _drop_pool=_drop_pool.drop_duplicates(subset=["player"]).head(5)
