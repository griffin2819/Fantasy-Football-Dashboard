...
def _fast_sim_user_pick(avail, roster_rows, rnd, overall, slot, teams, slots, randomness, rng):
    """
    Simulation-only scorer that mirrors the current Fantasy Edge decision principles
    without calling the expensive production ranking stack on every simulated user pick.
    Live Draft and Interactive Mock remain unchanged and still use the full engine.
    """
    if avail.empty:
        return None
    ...
    # Numeric base fields.
    proj=pd.to_numeric(a.get("projection",0),errors="coerce").fillna(0.0)
    vorp=pd.to_numeric(a.get("vorp",0),errors="coerce").fillna(0.0)
    market=pd.to_numeric(
        a.get("consensus_rank",a.get("model_rank",999)),
        errors="coerce"
    ).fillna(pd.to_numeric(a.get("model_rank",999),errors="coerce").fillna(999))

    # Base player value: projection + VORP + market quality.
    score=(proj*0.85)+(vorp*3.2)-np.minimum(market,300)*0.025
    pos=a["position"].astype(str)

    # Exact required-roster feasibility.
    ...
    def min_required(after):
        fixed={p:max(int(slots.get(p,0))-int(after.get(p,0)),0)
               for p in ["QB","RB","WR","TE","K","DL","DB"]}
        required_skill=int(slots.get("RB",0))+int(slots.get("WR",0))+int(slots.get("TE",0))+int(slots.get("FLEX",0))
    ...
    for p in ["QB","RB","WR","TE","K","DL","DB"]:
        ...
        if len(idx):
            after=dict(counts); after[p]=after.get(p,0)+1
            if min_required(after)>remaining_after:
                feasible[idx]=False

    # Roster construction / saturation.
    ...
    # Best-available IDP quality / impact bench logic.
    idp=(pos.isin(["DL","DB"]))
    tier=pd.to_numeric(a.get("idp_quality_tier",5),errors="coerce").fillna(5.0)
    impact=pd.to_numeric(a.get("idp_impact_score",0),errors="coerce").fillna(0.0)
    talent=pd.to_numeric(a.get("idp_talent_score",0),errors="coerce").fillna(0.0)
    idp_ropp=pd.to_numeric(a.get("roster_opportunity_adj",0),errors="coerce").fillna(0.0)

    impact_idp=idp & (tier<=2) & (idp_ropp>=-5.0)
    score += np.where(impact_idp,0.20*impact+0.16*talent+5.0,0.0)

    total_idp=dl+db
    if (rb>=5 or wr>=5) and total_idp<4:
        score += np.where(impact_idp,12.0,0.0)

    # Prevent poor-fit backup defenders.
    if dl>=1 and db>=1:
        feasible &= (~(idp & ((tier>2) | (idp_ropp<-5.0)))).to_numpy()

    # Timing/reach protection.
    ...
    # Small room-randomness only; never enough to erase hard roster logic.
    score += rng.normal(0,max(float(randomness),1.0)*0.10,len(a))

    # Hard infeasible candidates to bottom.
        ...
        if run:
            ...
            for j in range(this_batch):
                ...
                result=simulate_current_fantasy_edge_once(
                    board,int(teams),int(slot),int(rounds),
