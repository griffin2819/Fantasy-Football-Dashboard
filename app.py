...
st.title("🏈 Fantasy Edge V8.06.1 — Frozen Champion Whole-Draft Opportunity Audit")
st.caption("V8.06.1 • V8.05 frozen champion • opportunity audit hotfix")
st.caption("Build ID: V8.06.1-WHOLE-DRAFT-AUDIT-HOTFIX-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V8.06 — Frozen Champion Whole-Draft Opportunity Audit")
        st.caption(
            "Measurement only. V8.05 remains the frozen production champion. "
            "This audit maps remaining opportunity across all 15 rounds using local value gaps, market distance, roster state, "
            "cross-room/slot breadth, and short-horizon downstream construction. No certified production rule is changed."
        )

        if "v806_results" not in st.session_state:
            st.session_state.v806_results=None

        sims=st.number_input(
            "V8.06.1 simulations per slot × room",
            min_value=5,max_value=30,value=20,step=5,key="v806_sims",
            help="Default: 12 slots × 5 rooms × 20 = 1,200 fresh V8.05 champion drafts. Measurement-only, one simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run V8.06.1 opportunity audit",type="primary",key="v806_run")
        clear=b.button("Clear V8.06.1 audit",key="v806_clear")
        ...
        if run:
            ...
            # V8.06.1 robust benchmark-column resolution.
            _pool_cols=set(pool.columns)
            _vorp_col=next((c for c in ["vorp","VORP","value_over_replacement"] if c in _pool_cols),None)
            _score_col=next((c for c in ["draft_score","Draft_score","score","model_score"] if c in _pool_cols),None)
            _market_col=next((c for c in ["market_pick","Market_pick","adp","ADP"] if c in _pool_cols),None)
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8060000+int(ds)*100000+j*431+sum(ord(c) for c in room_name)
                        ...
                        # Cumulative roster state before each pick.
                        counts={"QB":0,"RB":0,"WR":0,"TE":0,"DL":0,"DB":0}

                        for idx,(_,r) in enumerate(d.sort_values("mock_round").iterrows()):
                            ...
                            champ_market=pd.to_numeric(pd.Series([r.get("market_pick",np.nan)]),errors="coerce").iloc[0]

                            pre_counts=counts.copy()

                            # Generic true-board proxy from benchmark pool, excluding players already drafted earlier in this champion draft.
                            drafted_names=set(d.sort_values("mock_round").iloc[:idx]["player"].astype(str).tolist())
                            alive=pool[~pool["player"].astype(str).isin(drafted_names)].copy()
                            alt=alive[alive["player"].astype(str)!=str(r.get("player",""))].copy()

                            best_player=""; best_pos=""
                            best_vorp=best_score=best_market=np.nan
                            if len(alt):
                                alt["_vorp"]=pd.to_numeric(alt[_vorp_col],errors="coerce") if _vorp_col else np.nan
                                alt["_score"]=pd.to_numeric(alt[_score_col],errors="coerce") if _score_col else np.nan
                                alt["_market"]=pd.to_numeric(alt[_market_col],errors="coerce") if _market_col else np.nan
                                alt=alt.sort_values(
                                    ["_score","_vorp","_market"],
                                    ascending=[False,False,True],na_position="last"
                                )
                                b=alt.iloc[0]
                                best_player=str(b.get("player","")); best_pos=str(b.get("position",""))
                                best_vorp=float(b["_vorp"]) if pd.notna(b["_vorp"]) else np.nan
                                best_score=float(b["_score"]) if pd.notna(b["_score"]) else np.nan
                                best_market=float(b["_market"]) if pd.notna(b["_market"]) else np.nan

                            score_gap=best_score-champ_score if pd.notna(best_score) and pd.notna(champ_score) else np.nan
                            vorp_gap=best_vorp-champ_vorp if pd.notna(best_vorp) and pd.notna(champ_vorp) else np.nan
                            market_distance=best_market-pick if pd.notna(best_market) and pd.notna(pick) else np.nan
                            dominates=bool(pd.notna(score_gap) and pd.notna(vorp_gap) and score_gap>0 and vorp_gap>0)
                            within10=bool(dominates and pd.notna(market_distance) and market_distance<=10)

                            # Short-horizon downstream profile: next 1-3 champion rounds, when present.
                            future=d[
                                (pd.to_numeric(d.mock_round,errors="coerce")>rnd) &
                                (pd.to_numeric(d.mock_round,errors="coerce")<=rnd+3)
                            ]
                            downstream_vorp=pd.to_numeric(future.get("vorp",pd.Series(dtype=float)),errors="coerce").mean()
                            downstream_score=pd.to_numeric(future.get("draft_score",pd.Series(dtype=float)),errors="coerce").mean()
                            downstream_seq=" > ".join(future.sort_values("mock_round")["position"].astype(str).tolist())

                            protected=(rnd in [2,3,7]) or (rnd>=11)

                            rows.append({
                                "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                "Round":rnd,"Pick":pick,
                                "Champion_player":str(r.get("player","")),"Champion_position":pos,
                                "Champion_VORP":champ_vorp,"Champion_draft_score":champ_score,
                                "Champion_market_pick":champ_market,
                                "Best_alt_player":best_player,"Best_alt_position":best_pos,
                                "Best_alt_VORP":best_vorp,"Best_alt_draft_score":best_score,
                                "Best_alt_market_pick":best_market,
                                "VORP_gap":vorp_gap,"Draft_score_gap":score_gap,
                                "Alt_market_distance":market_distance,
                                "Alt_dominates":dominates,"Alt_dominates_le10":within10,
                                "QB_count_pre":pre_counts.get("QB",0),
                                "RB_count_pre":pre_counts.get("RB",0),
                                "WR_count_pre":pre_counts.get("WR",0),
                                "TE_count_pre":pre_counts.get("TE",0),
                                "DL_count_pre":pre_counts.get("DL",0),
                                "DB_count_pre":pre_counts.get("DB",0),
                                "Downstream_1_3_avg_VORP":downstream_vorp,
                                "Downstream_1_3_avg_score":downstream_score,
                                "Downstream_1_3_sequence":downstream_seq,
                                "Protected_round":protected,
                            })
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V8.06.1: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v806_results=df
            status.success(f"V8.06.1 complete: {len(df):,} champion decisions audited.")
            prog.empty()

        df=st.session_state.get("v806_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in ["VORP_gap","Draft_score_gap","Alt_market_distance","Downstream_1_3_avg_VORP","Downstream_1_3_avg_score"]:
                df[c]=pd.to_numeric(df[c],errors="coerce")

            by_round=df.groupby("Round",as_index=False).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Alt_dominates","mean"),
                Le10_rate=("Alt_dominates_le10","mean"),
                Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),
                Avg_downstream_VORP=("Downstream_1_3_avg_VORP","mean"),
                Avg_downstream_score=("Downstream_1_3_avg_score","mean"),
                Protected=("Protected_round","max")
            )

            # Breadth across rooms and slots for the <=10 signal.
            room_sig=df.groupby(["Round","Room"],as_index=False).agg(
                Le10_rate=("Alt_dominates_le10","mean")
            )
            slot_sig=df.groupby(["Round","Draft_slot"],as_index=False).agg(
                Le10_rate=("Alt_dominates_le10","mean")
            )
            room_breadth=room_sig.groupby("Round",as_index=False).agg(
                Rooms_with_signal=("Le10_rate",lambda x:int((x>0).sum())),
                Room_signal_avg=("Le10_rate","mean")
            )
            slot_breadth=slot_sig.groupby("Round",as_index=False).agg(
                Slots_with_signal=("Le10_rate",lambda x:int((x>0).sum())),
                Slot_signal_avg=("Le10_rate","mean")
            )
            by_round=by_round.merge(room_breadth,on="Round",how="left").merge(slot_breadth,on="Round",how="left")

            by_round["Opportunity_score"]=(
                by_round.Le10_rate
                * by_round.Avg_VORP_gap.clip(lower=0)
                * (by_round.Avg_score_gap.clip(lower=0)/10.0)
                * (by_round.Rooms_with_signal/5.0)
                * (by_round.Slots_with_signal/12.0)
            )
            ranked=by_round.sort_values("Opportunity_score",ascending=False)

            st.markdown("#### Ranked whole-draft opportunity map")
            st.dataframe(ranked.round(4),use_container_width=True,hide_index=True)

            candidates=ranked[~ranked.Protected]
            if len(candidates):
                top=candidates.iloc[0]
                st.warning(
                    f"Highest unprotected discovery target: Round {int(top.Round)} "
                    f"(opportunity score {float(top.Opportunity_score):.4f}, "
                    f"≤10 dominance {float(top.Le10_rate):.1%}, "
                    f"rooms with signal {int(top.Rooms_with_signal)}/5, "
                    f"slots with signal {int(top.Slots_with_signal)}/12). "
                    "This is discovery-only; measure the mechanism before building a challenger."
                )

            st.markdown("#### Room × round signal")
            st.dataframe(room_sig.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Slot × round signal")
            st.dataframe(slot_sig.round(4),use_container_width=True,hide_index=True)

            paths=df[
                (~df.Protected_round) & df.Alt_dominates_le10
            ].groupby(
                ["Round","Champion_position","Best_alt_position"],as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),
                Avg_market_distance=("Alt_market_distance","mean"),
                Avg_downstream_VORP=("Downstream_1_3_avg_VORP","mean"),
                Avg_downstream_score=("Downstream_1_3_avg_score","mean"),
            ).sort_values(["Decisions","Avg_score_gap"],ascending=[False,False])

            st.markdown("#### Repeated unprotected opportunity mechanisms")
            st.dataframe(paths.head(60).round(4),use_container_width=True,hide_index=True)

            st.info(
                "V8.06 is measurement-only. Certified R2, R3 and R7 plus the protected R11–15 architecture are excluded from automatic challenger targeting. "
                "Do not retune any certified threshold from this audit."
            )

            st.download_button(
                "⬇️ Download V8.06 whole-draft opportunity audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_06_1_frozen_champion_whole_draft_opportunity_audit.csv",
                "text/csv",key="v806_download"
            )
