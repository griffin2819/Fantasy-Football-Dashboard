...
st.set_page_config(page_title="Fantasy Edge V6.59 — Player Pool + Ownership Fix", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V6.75-SUFFIX-IDENTITY-GUARD"
...
with tabs[1]:
    ...
    with st.expander("🚑 Live Injury Center", expanded=False):
        st.caption("Automated status: Sleeper NFL player feed, cached up to 30 minutes. Manual overrides always take priority.")
...
with tabs[3]:
    ...
    x=_waiver_board[_waiver_board["player"].map(lambda n:_is_free_agent(state,n))].sort_values("waiver_score",ascending=False).copy()
...
with tabs[5]:
    ...
    st.caption("V6.73 keeps scoring unchanged and strengthens historical coverage with a dual-source 2025 baseline: FantasyPros first, nflverse PPR fallback for offense. 2025 PPG and source are real historical data; PPG Δ = 2026 projected PPG minus 2025 PPG. Historical data refreshes daily; injury/status checks remain every 30 minutes.")

with tabs[6]:
    x=board[board["player"].map(lambda n:_player_owner(state,n)=="1")].copy()
