...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Actionability Cleanup", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
with tabs[14]:
    ...
    if my.empty:
        ...
        if not _actions:
            _mon_txt=(f" {_inj} injury watch(es) and {len([m for m in _monitors if m[1]=='DATA'])} data verification item(s) remain under monitoring." if _monitors else "")
            st.markdown(f"<div class='fecc-ok'>✅ <b>Your lineup is optimized. No action needed.</b><br><span style='color:#abc0b6'>No Start/Sit or Waiver move currently clears Fantasy Edge's action threshold.{_mon_txt}</span></div>",unsafe_allow_html=True)
        else:
