...
st.title("🏈 Fantasy Edge V7.83 — R11–13 Offense vs IDP Boundary Audit")
st.caption("V7.83 • V7.67 frozen champion • offense-vs-IDP boundary diagnostic")
st.caption("Build ID: V7.83-R11-13-OFF-IDP-BOUNDARY-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.83 keeps V7.67 frozen/live and isolates offensive versus IDP alternatives for R11–13 deep-faller decisions.")
        ...
        st.markdown("### 🔬 V7.83 — R11–13 Offense vs IDP Boundary Audit")
        st.caption(
            "Measurement only. V7.67 remains frozen champion. V7.83 isolates the late-round scoring boundary by comparing each R11–13 >30-pick faller against "
            "the best genuinely available OFFENSIVE alternative and the best genuinely available IDP alternative separately."
        )

        if "v783_results" not in st.session_state:
            st.session_state.v783_results=None

        sims=st.number_input(
            "V7.83 simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v783_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run offense-vs-IDP boundary audit",type="primary",key="v783_run")
        clear=b.button("Clear V7.83 audit",key="v783_clear")
        if clear:
            st.session_state.v783_results=None
            st.rerun()
        ...
        if run:
            ...
            slots_to_test=list(dict.fromkeys([x for x in [1,3,7,min(12,int(teams))] if 1<=x<=int(teams)]))
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=983000+int(ds)*100000+j*283+sum(ord(c) for c in room_name)
                        ...
                        late=d[pd.to_numeric(d.mock_round,errors="coerce").between(11,13,inclusive="both")]
                        for _,r in late.iterrows():
                            ...
                            audit=int(pd.to_numeric(pd.Series([r.get("R782_Audit",0)]),errors="coerce").fillna(0).iloc[0])

                            if pd.notna(delta) and delta < -30 and audit==1:
                                row={
                                    "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                    "Round":int(r.mock_round),"Pick":pick,
                                    "Champion_player":str(r.player),"Champion_position":str(r.position),
                                    "Champion_market_pick":market,"Champion_market_delta":delta,
                                    "Champion_projection":r.get("projection",np.nan),
                                    "Champion_vorp":r.get("vorp",np.nan),
                                    "Champion_draft_score":r.get("draft_score",np.nan),

                                    "Best_off_player":r.get("R783_Best_off_player",""),
                                    "Best_off_position":r.get("R783_Best_off_position",""),
                                    "Best_off_market_pick":r.get("R783_Best_off_market_pick",np.nan),
                                    "Best_off_projection":r.get("R783_Best_off_projection",np.nan),
                                    "Best_off_vorp":r.get("R783_Best_off_vorp",np.nan),
                                    "Best_off_draft_score":r.get("R783_Best_off_draft_score",np.nan),

                                    "Best_idp_player":r.get("R783_Best_idp_player",""),
                                    "Best_idp_position":r.get("R783_Best_idp_position",""),
                                    "Best_idp_market_pick":r.get("R783_Best_idp_market_pick",np.nan),
                                    "Best_idp_projection":r.get("R783_Best_idp_projection",np.nan),
                                    "Best_idp_vorp":r.get("R783_Best_idp_vorp",np.nan),
                                    "Best_idp_draft_score":r.get("R783_Best_idp_draft_score",np.nan),
                                }
                                rows.append(row)
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.83 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v783_results=df
            status.success(f"V7.83 complete: {len(df)} deep-faller decisions audited.")
            prog.empty()

        df=st.session_state.get("v783_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in [
                "Champion_projection","Champion_vorp","Champion_draft_score",
                "Best_off_projection","Best_off_vorp","Best_off_draft_score",
                "Best_idp_projection","Best_idp_vorp","Best_idp_draft_score"
            ]:
                df[c]=pd.to_numeric(df[c],errors="coerce")

            df["Off_projection_change"]=df.Best_off_projection-df.Champion_projection
            df["Off_vorp_change"]=df.Best_off_vorp-df.Champion_vorp
            df["Off_draft_score_change"]=df.Best_off_draft_score-df.Champion_draft_score
            df["Off_dominates_score_vorp"]=(df.Off_vorp_change>0)&(df.Off_draft_score_change>0)

            df["IDP_projection_change"]=df.Best_idp_projection-df.Champion_projection
            df["IDP_vorp_change"]=df.Best_idp_vorp-df.Champion_vorp
            df["IDP_draft_score_change"]=df.Best_idp_draft_score-df.Champion_draft_score
            df["IDP_dominates_score_vorp"]=(df.IDP_vorp_change>0)&(df.IDP_draft_score_change>0)

            summary=pd.DataFrame([{
                "Deep-faller decisions":len(df),
                "Best OFF VORP change":df.Off_vorp_change.mean(),
                "Best OFF draft-score change":df.Off_draft_score_change.mean(),
                "Best OFF dominance rate":df.Off_dominates_score_vorp.mean(),
                "Best IDP VORP change":df.IDP_vorp_change.mean(),
                "Best IDP draft-score change":df.IDP_draft_score_change.mean(),
                "Best IDP dominance rate":df.IDP_dominates_score_vorp.mean(),
            }])
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            by_round=df.groupby("Round",as_index=False).agg(
                Decisions=("Champion_player","count"),
                OFF_vorp_change=("Off_vorp_change","mean"),
                OFF_score_change=("Off_draft_score_change","mean"),
                OFF_dominance=("Off_dominates_score_vorp","mean"),
                IDP_vorp_change=("IDP_vorp_change","mean"),
                IDP_score_change=("IDP_draft_score_change","mean"),
                IDP_dominance=("IDP_dominates_score_vorp","mean"),
            )
            st.markdown("#### Offense vs IDP by round")
            st.dataframe(by_round.round(4),use_container_width=True,hide_index=True)

            off=df.groupby(["Best_off_player","Best_off_position"],as_index=False).agg(
                Times_best_off=("Best_off_player","count"),
                VORP_change=("Off_vorp_change","mean"),
                Draft_score_change=("Off_draft_score_change","mean"),
                Dominance_rate=("Off_dominates_score_vorp","mean"),
            ).sort_values("Times_best_off",ascending=False)
            st.markdown("#### Best offensive alternatives")
            st.dataframe(off.head(30).round(4),use_container_width=True,hide_index=True)

            idp=df.groupby(["Best_idp_player","Best_idp_position"],as_index=False).agg(
                Times_best_idp=("Best_idp_player","count"),
                VORP_change=("IDP_vorp_change","mean"),
                Draft_score_change=("IDP_draft_score_change","mean"),
                Dominance_rate=("IDP_dominates_score_vorp","mean"),
            ).sort_values("Times_best_idp",ascending=False)
            st.markdown("#### Best IDP alternatives")
            st.dataframe(idp.head(30).round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.83 is diagnostic only. If offensive alternatives also dominate at a high rate, the late-round issue is not merely an IDP scoring-scale artifact. "
                "If only IDPs dominate, the next investigation should focus on cross-position score normalization rather than a draft override."
            )

            st.download_button(
                "⬇️ Download V7.83 offense-vs-IDP boundary audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_83_r11_13_offense_vs_idp_boundary_audit.csv",
                "text/csv",key="v783_download"
            )
