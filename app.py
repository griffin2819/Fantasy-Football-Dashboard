...
st.title("🏈 Fantasy Edge V7.95 — Frozen Champion Weakness Discovery")
st.caption("V7.95 • V7.93 frozen champion • weakness discovery")
st.caption("Build ID: V7.95-V793-WEAKNESS-DISCOVERY-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V7.95 — Frozen Champion Weakness Discovery")
        st.caption(
            "Measurement only. V7.93 remains frozen champion. R2, R7 and the protected late-round architecture are unchanged."
        )
        if "v795_results" not in st.session_state:
            st.session_state.v795_results=None

        sims=st.number_input("V7.95 simulations per slot × room",5,20,10,5,key="v795_sims",
            help="Default: 600 fresh champion drafts across all 12 slots and all five rooms.")
        a,b=st.columns(2)
        run=a.button("Run V7.95 weakness audit",type="primary",key="v795_run")
        clear=b.button("Clear V7.95",key="v795_clear")
        if clear:
            st.session_state.v795_results=None
            st.rerun()
            ...
            for ds in range(1,int(teams)+1):
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=995000+ds*100000+j*367+sum(ord(c) for c in room_name)
                        ...
                        for _,r in d.iterrows():
                            ...
                            rows.append({
                                ...
                                "Round":rnd,"Pick":r.mock_pick,"Player":r.player,"Position":r.position,
                                "VORP":r.get("vorp",np.nan),"Draft_score":r.get("draft_score",np.nan),
                                "Market_pick":r.get("market_pick",np.nan),
                                "Protected":rnd==2 or rnd==7 or rnd>=11
                            })
                        done+=1
                        prog.progress(done/total)
                        if done%100==0 or done==total:
                            status.caption(f"V7.95: {done}/{total} frozen-champion drafts")
            df=pd.DataFrame(rows)
            st.session_state.v795_results=df
            status.success(f"V7.95 complete: {len(df):,} champion selections mapped.")
            prog.empty()

        df=st.session_state.get("v795_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in ["VORP","Draft_score","Market_pick","Pick"]:
                df[c]=pd.to_numeric(df[c],errors="coerce")
            df["Market_delta"]=df.Market_pick-df.Pick

            rr=df.groupby("Round",as_index=False).agg(
                Decisions=("Player","count"),Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),Avg_market_delta=("Market_delta","mean"))
            st.markdown("#### Full 15-round champion map")
            st.dataframe(rr.round(4),use_container_width=True,hide_index=True)

            room=df.groupby(["Room","Round"],as_index=False).agg(
                Avg_VORP=("VORP","mean"),Avg_draft_score=("Draft_score","mean"),
            ...
            slot=df.groupby(["Draft_slot","Round"],as_index=False).agg(
                Avg_VORP=("VORP","mean"),Avg_draft_score=("Draft_score","mean"),
                Avg_market_delta=("Market_delta","mean"))
            st.markdown("#### Slot × round")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            # Cross-room dispersion highlights rounds where champion performance is least robust.
            disp=room.groupby("Round",as_index=False).agg(
                VORP_room_std=("Avg_VORP","std"),
                Score_room_std=("Avg_draft_score","std"),
                Market_room_std=("Avg_market_delta","std"))
            disp["Protected"]=disp.Round.eq(2)|disp.Round.eq(7)|disp.Round.ge(11)
            disp["Weakness_index"]=disp.VORP_room_std.fillna(0)+disp.Score_room_std.fillna(0)/10+disp.Market_room_std.fillna(0)/10
            disp=disp.sort_values("Weakness_index",ascending=False)
            st.markdown("#### Ranked cross-environment instability")
            st.dataframe(disp.round(4),use_container_width=True,hide_index=True)

            cand=disp[~disp.Protected]
            if len(cand):
                t=cand.iloc[0]
                st.warning(f"Highest unprotected discovery target: Round {int(t.Round)}. Measure this round directly before creating any challenger.")

            st.info("Discovery only: do not retune R2 ≤10, R7, or R11–15 from this audit.")
            st.download_button("⬇️ Download V7.95 weakness audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_95_frozen_champion_weakness_discovery_audit.csv",
                "text/csv",key="v795_download")
