...
def early_qb_gate_snapshot(fast, candidate_idx, eval_score, normal_choice,
                           current_pick, next_pick, teams=12, randomness=12):
    """Diagnostic snapshot for an R1-3 QB recommendation. Never changes the pick."""
    cand=np.asarray(candidate_idx,dtype=int)
    ev=np.asarray(eval_score,dtype=float)
    pos=fast["pos_code"]; market=fast["market_pick"]; pool=fast["pool"]
    normal=int(normal_choice)
    snap={
        "EQB_trigger":0,"EQB_qb_player":"","EQB_qb_survival":np.nan,
        "EQB_qb_market_reach":np.nan,"EQB_qb_eval":np.nan,
        "EQB_alt_player":"","EQB_alt_position":"","EQB_alt_survival":np.nan,
        "EQB_alt_eval":np.nan,"EQB_eval_edge":np.nan,"EQB_alt_idx":None,
        "EQB_alt_survived_actual":np.nan
    }
    round_no=max(1,int((int(current_pick)-1)//max(int(teams),1))+1)
    if round_no>3 or len(cand)==0 or normal<0 or normal>=len(cand) or pos[cand[normal]]!=0:
        return snap

    qg=int(cand[normal]); qmp=float(market[qg]) if np.isfinite(market[qg]) else np.nan
    qsurv=survival_probability(qmp,next_pick,randomness) if np.isfinite(qmp) else np.nan
    qreach=max(qmp-float(current_pick),0) if np.isfinite(qmp) else np.nan

    # Best RB/WR alternative by model evaluation, regardless of survival.
    alt_loc=np.flatnonzero(np.isin(pos[cand],[1,2]))
    if len(alt_loc):
        ai=int(alt_loc[np.argmax(ev[alt_loc])]); ag=int(cand[ai])
        ...
        ap=str(pool.iloc[ag].player); apos=str(pool.iloc[ag].position)
        aeval=float(ev[ai]); edge=float(ev[normal]-aeval)
    else:
        ag=None; amp=np.nan; asurv=np.nan; ap=""; apos=""; aeval=np.nan; edge=np.nan

    snap.update({
        "EQB_trigger":1,
        "EQB_qb_player":str(pool.iloc[qg].player),
        "EQB_qb_survival":qsurv,
        "EQB_qb_market_reach":qreach,
        "EQB_qb_eval":float(ev[normal]),
        "EQB_alt_player":ap,
        "EQB_alt_position":apos,
        "EQB_alt_survival":asurv,
        "EQB_alt_eval":aeval,
        "EQB_eval_edge":edge,
        "EQB_alt_idx":ag,
        "EQB_alt_survived_actual":np.nan
...
def early_qb_threshold_matrix_row(qb_surv, alt_surv, eval_edge):
    """Return hypothetical gate triggers for several calibration thresholds."""
    out={}
    combos=[
        ("58_45_18",.58,.45,18.0),
        ("55_45_18",.55,.45,18.0),
    ...
    for name,qmin,amax,emax in combos:
        ok=(pd.notna(qb_surv) and pd.notna(alt_surv) and pd.notna(eval_edge)
            and float(qb_surv)>=qmin and float(alt_surv)<=amax and float(eval_edge)<=emax)
        out[f"Gate_{name}"]=1 if ok else 0
    return out
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            if pending_alt_idx is not None and pending_alt_diag is not None:
                user_diag[pending_alt_diag]["EQB_alt_survived_actual"]=1.0 if alive[int(pending_alt_idx)] else 0.0
                pending_alt_idx=None; pending_alt_diag=None
            ...
            if model_user:
                ...
                diag=early_qb_gate_snapshot(
                    fast,sidx,eval_score,local_choice,overall,nxt,
                    teams=int(teams),randomness=randomness
                )
                if diag.get("EQB_trigger",0):
                    diag.update(early_qb_threshold_matrix_row(
                        diag.get("EQB_qb_survival"),diag.get("EQB_alt_survival"),diag.get("EQB_eval_edge")
                    ))
            ...
            if model_user and diag.get("EQB_trigger",0) and diag.get("EQB_alt_idx") is not None:
                pending_alt_idx=int(diag["EQB_alt_idx"]); pending_alt_diag=len(user_diag)-1
            counts[pos[chosen]]+=1
        else:
...
st.title("🏈 Fantasy Edge V7.19 — Early-QB Gate Calibration")
st.caption("V7.19 • V7.16 draft engine frozen • early-QB gate calibration matrix • diagnostic only")
st.caption("Build ID: V7.19-EQGC-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v719_live_slot")
    ...
    if "v719_target_queue" not in st.session_state:
        st.session_state.v719_target_queue={}
    queue=st.session_state.v719_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v719_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.19 is diagnostic only. V7.16 draft logic is frozen while several hypothetical R1-3 QB wait gates are measured side-by-side.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.19 robustness report")
            ...
            st.download_button("⬇️ Download V7.19 diagnostic CSV",csv,"fantasy_edge_v7_19_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.19 gate calibration diagnostics")
        ...
        if "v719_independent_results" not in st.session_state:
            st.session_state.v719_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v719_independent_results"),pd.DataFrame) else "ready to run"))
        if "v719_independent_summary" not in st.session_state:
            st.session_state.v719_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v719_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v719_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v719_run")
        clear_iv=clear_col.button("Clear independent results",key="v719_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                for ds in iv_slots:
                    for room_name in iv_rooms:
                        for j in range(int(iv_sims)):
                            ...
                            eqb=fe[pd.to_numeric(fe.get("EQB_trigger",0),errors="coerce").fillna(0).eq(1)].copy() if "EQB_trigger" in fe.columns else fe.iloc[0:0].copy()
                            row["EQB_decisions"]=int(len(eqb))
                            row["EQB_avg_qb_survival"]=float(pd.to_numeric(eqb.get("EQB_qb_survival"),errors="coerce").mean()) if len(eqb) else np.nan
                            row["EQB_avg_alt_survival_est"]=float(pd.to_numeric(eqb.get("EQB_alt_survival"),errors="coerce").mean()) if len(eqb) else np.nan
                            row["EQB_alt_survival_actual"]=float(pd.to_numeric(eqb.get("EQB_alt_survived_actual"),errors="coerce").mean()) if len(eqb) else np.nan
                            row["EQB_avg_eval_edge"]=float(pd.to_numeric(eqb.get("EQB_eval_edge"),errors="coerce").mean()) if len(eqb) else np.nan
                            row["EQB_avg_qb_reach"]=float(pd.to_numeric(eqb.get("EQB_qb_market_reach"),errors="coerce").mean()) if len(eqb) else np.nan
                            row["EQB_alt_WR_share"]=float((eqb.get("EQB_alt_position",pd.Series(dtype=str)).astype(str)=="WR").mean()) if len(eqb) else np.nan
                            row["EQB_alt_RB_share"]=float((eqb.get("EQB_alt_position",pd.Series(dtype=str)).astype(str)=="RB").mean()) if len(eqb) else np.nan
                            for gate_col in ["Gate_58_45_18","Gate_55_45_18","Gate_55_50_18","Gate_50_45_18","Gate_50_50_18","Gate_55_50_20","Gate_50_55_20"]:
                                row[gate_col]=float(pd.to_numeric(eqb.get(gate_col),errors="coerce").mean()) if len(eqb) and gate_col in eqb.columns else 0.0
                ...
                st.session_state.v719_independent_results=iv
                st.session_state.v719_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v719_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v719_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.19 Early-QB Gate Calibration")
            st.caption("Diagnostic only. V7.16 draft choices are unchanged. This shows why hypothetical QB wait gates would or would not fire.")

            g1,g2,g3,g4=st.columns(4)
            g1.metric("Early-QB decisions/draft",f"{iv_saved.EQB_decisions.mean():.2f}")
            g2.metric("QB survival estimate",f"{iv_saved.EQB_avg_qb_survival.mean():.0%}")
            g3.metric("Best RB/WR survival estimate",f"{iv_saved.EQB_avg_alt_survival_est.mean():.0%}")
            g4.metric("Alternative actually survives",f"{iv_saved.EQB_alt_survival_actual.mean():.0%}")
            g5,g6,g7=st.columns(3)
            g5.metric("QB eval edge vs RB/WR",f"{iv_saved.EQB_avg_eval_edge.mean():+.2f}")
            g6.metric("QB market reach",f"{iv_saved.EQB_avg_qb_reach.mean():.2f} picks")
            g7.metric("Best alternative mix",f"{iv_saved.EQB_alt_WR_share.mean():.0%} WR / {iv_saved.EQB_alt_RB_share.mean():.0%} RB")

            matrix=pd.DataFrame([
                {"Gate":"58% QB / 45% alt / edge≤18","Would fire":iv_saved.Gate_58_45_18.mean()},
                {"Gate":"55% QB / 45% alt / edge≤18","Would fire":iv_saved.Gate_55_45_18.mean()},
                {"Gate":"55% QB / 50% alt / edge≤18","Would fire":iv_saved.Gate_55_50_18.mean()},
                {"Gate":"50% QB / 45% alt / edge≤18","Would fire":iv_saved.Gate_50_45_18.mean()},
                {"Gate":"50% QB / 50% alt / edge≤18","Would fire":iv_saved.Gate_50_50_18.mean()},
                {"Gate":"55% QB / 50% alt / edge≤20","Would fire":iv_saved.Gate_55_50_20.mean()},
                {"Gate":"50% QB / 55% alt / edge≤20","Would fire":iv_saved.Gate_50_55_20.mean()},
            ])
            matrix["Would fire"]=matrix["Would fire"].map(lambda x:f"{x:.0%}")
            st.markdown("#### Hypothetical gate activation matrix")
            ...
            for ds,g in iv_saved.groupby("Draft_slot"):
                slot_gate.append({
                    "Draft slot":int(ds),
                    "QB decisions":g.EQB_decisions.mean(),
                    "QB survival":g.EQB_avg_qb_survival.mean(),
                    "Alt survival est":g.EQB_avg_alt_survival_est.mean(),
                    "Alt survives actual":g.EQB_alt_survival_actual.mean(),
                    "Eval edge":g.EQB_avg_eval_edge.mean(),
                    "QB reach":g.EQB_avg_qb_reach.mean(),
                    "55/50/18 fires":g.Gate_55_50_18.mean(),
                    "50/50/18 fires":g.Gate_50_50_18.mean(),
                })
            st.markdown("#### Gate calibration by draft slot")
            sg=pd.DataFrame(slot_gate)
            st.dataframe(sg.round(2),use_container_width=True,hide_index=True)

            # Identify a candidate threshold without changing the model.
            rates={
                "58/45/18":iv_saved.Gate_58_45_18.mean(),
                "55/45/18":iv_saved.Gate_55_45_18.mean(),
                "55/50/18":iv_saved.Gate_55_50_18.mean(),
                "50/45/18":iv_saved.Gate_50_45_18.mean(),
                "50/50/18":iv_saved.Gate_50_50_18.mean(),
                "55/50/20":iv_saved.Gate_55_50_20.mean(),
                "50/55/20":iv_saved.Gate_50_55_20.mean(),
            }
            viable=[(k,v) for k,v in rates.items() if .08<=v<=.35]
            if viable:
                pick=min(viable,key=lambda kv:abs(kv[1]-.20))
                st.info(f"Calibration candidate: {pick[0]} would affect about {pick[1]:.0%} of early-QB decisions — enough to test without rewriting Fantasy Edge's early-draft identity.")
            else:
                st.warning("None of the tested gates lands in the conservative 8–35% activation range. That suggests the early-QB issue may not be solvable with a simple survival threshold.")
            ...
            st.download_button(
                "⬇️ Download V7.19 gate calibration diagnostics CSV",
                iv_csv,
                "fantasy_edge_v7_19_qb_gate_calibration.csv",
                "text/csv",
                key="v719_download"
            )
