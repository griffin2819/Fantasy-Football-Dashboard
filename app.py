...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    _next_gap=max(int(next_pick)-int(overall_pick),1)
    _survival_margin=_market-float(overall_pick)
    _next_turn_risk=np.clip((_next_gap-_survival_margin)/max(_next_gap,1),0,1)*6.0

    # Continuous reach cost.
    _reach=np.maximum(_market-float(overall_pick),0.0)
    _reach_cost=np.minimum(_reach,48.0)*0.14

    # Controlled IDP portfolio: 1 DL + 1 DB required; one optional third impact IDP.
    ...
    if _idp_total<2:
        _idp_marginal += np.where(_impact_idp,4.0+np.minimum(_impact,15.0)*0.12,0.0)
    elif _idp_total==2 and int(round_no)>=11 and (_rb>=4 or _wr>=4):
        _idp_marginal += np.where(_impact_idp,1.5+np.minimum(_impact,15.0)*0.08,0.0)
    elif _idp_total>=3:
        _idp_marginal += np.where(_idp,-60.0,0.0)
    ...
    # embedded in evaluation_score and therefore cannot be bypassed here.
    x["execution_score"]=x["unified_pick_score"]
...
def _fast_sim_user_pick(avail, roster_rows, rnd, overall, slot, teams, slots, randomness, rng):
    ...
    if dl>=1 and db>=1:
        if total_idp>=3:
        ...
        elif rnd>=11 and (rb>=4 or wr>=4) and impact_idp.any():
            ...
            if best_idp < best_off+2.5:
                feasible &= (~idp).to_numpy()
            else:
                score += np.where(impact_idp,2.0,0.0)
