...
def dynamic_faller_threshold(current_pick, teams=12, rounds=15):
    ...
    if round_no<=3:
        return 14.0, 12, 8.0   # min fall, model band, improvement over normal choice
    if round_no<=6:
        return 11.0, 14, 6.0
    if round_no<=10:
        return 8.0, 18, 4.0
    # V7.15 surgical late-round harvest: the core roster is largely built, so
    # model-compatible market fallers deserve a wider candidate band and lower
    # interception threshold. R1-10 values above are intentionally frozen.
    return 4.0, 28, 2.0
...
def faller_intercept_choice(eval_score, market_pick, current_pick, normal_choice,
    ...
    if round_no>=11:
        # V7.15: late-round boards contain many large fallers. Reward the size
        # of the fall more aggressively, while still requiring model compatibility.
        fall_value=.46*fall[cand] + .055*(excess**1.40)
        intercept_score=fall_value + 1.45*compatibility
    else:
        # Frozen V7.13 behavior for Rounds 1-10.
    ...
    if round_no>=11:
        ...
        # If the normal late-round choice is itself a 4+ pick reach, do not demand
        # extra market separation beyond the late-round minimum. This directly
        # addresses V7.14's R11+ pattern: large available fallers while FE still reached.
        if normal_reach>=4.0:
            required=min_fall
        if float(fall[best_cand]) >= required:
            return best_cand,True,float(fall[best_cand])
    else:
        # Frozen V7.13 rule for Rounds 1-10.
...
st.title("🏈 Fantasy Edge V7.15 — Late-Round Value Harvest")
st.caption("V7.15 • R1-10 frozen • surgical R11+ Late-Round Value Harvest • phase diagnostics retained")
st.caption("Build ID: V7.15-LRVH-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v715_live_slot")
    ...
    if "v715_target_queue" not in st.session_state:
        st.session_state.v715_target_queue={}
    queue=st.session_state.v715_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v715_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.15 keeps R1-10 frozen: R1-3 14+ • R4-6 11+ • R7-10 8+. Only R11+ changes to a 4+ pick Late-Round Value Harvest with a wider model-compatible band.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.15 robustness report")
            ...
            st.download_button("⬇️ Download V7.15 diagnostic CSV",csv,"fantasy_edge_v7_15_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.15 independent validation")
        ...
        if "v715_independent_results" not in st.session_state:
            st.session_state.v715_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v715_independent_results"),pd.DataFrame) else "ready to run"))
        if "v715_independent_summary" not in st.session_state:
            st.session_state.v715_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v715_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v715_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v715_run")
        clear_iv=clear_col.button("Clear independent results",key="v715_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v715_independent_results=iv
                st.session_state.v715_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v715_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v715_independent_summary") or {}
            st.success(
            ...
            # Frozen V7.14 reference for the phase this release targets.
            current_r11=phase_df[phase_df["Phase"].eq("R11+")].iloc[0]
            lr1,lr2,lr3=st.columns(3)
            lr1.metric("R11+ market gap",f"{current_r11['Market gap']:+.2f}","V7.14 reference: -18.80")
            lr2.metric("R11+ FE reach",f"{current_r11['FE reach']:.2f}","V7.14 reference: 5.98")
            lr3.metric("R11+ FE faller",f"{current_r11['FE faller']:.2f}","V7.14 reference: 29.27")
            ...
            r11_row=phase_df[phase_df["Phase"].eq("R11+")].iloc[0]
            st.info(
                f"V7.15 surgical target — R11+: market gap {r11_row['Market gap']:+.2f}, "
                f"FE reach {r11_row['FE reach']:.2f}, FE faller {r11_row['FE faller']:.2f}. "
                "Rounds 1-10 are frozen; improvement should come almost entirely from this phase."
            )
            ...
            st.download_button(
                "⬇️ Download V7.15 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_15_late_round_validation.csv",
                "text/csv",
                key="v715_download"
            )
