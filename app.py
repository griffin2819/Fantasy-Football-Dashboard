...
st.set_page_config(page_title="Fantasy Edge V7.29 — Live Schedule Matchup Resolver", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V7.26-PROVISIONAL-RAMP-MARKET-SIGNAL"
...
def _v694_trade_partner_suggestions(board_df,state,max_rows=20):
    """V7.31 Trade Intelligence 2.0.

    Automatic recommendations must improve Griffin's optimized roster. Fair-but-worse
    trades are intentionally suppressed. Searches 1-for-1 and 2-for-1 consolidation deals.
    """
    ...
    # Keep the search bounded and favor movable/depth assets. Core holds can still be
    # considered, but only the hard improvement gates below can let them through.
    my_pool=my_pool.sort_values("_tv",ascending=False).head(12)
    ...
    for owner in [str(i) for i in range(2,13)]:
        ...
        # Prefer players who address Griffin's need, but search a few elite upgrades too.
        preferred=opp_pool[opp_pool["position"].eq(my_need_pos)].sort_values("_tv",ascending=False).head(6)
        extras=opp_pool.sort_values("_tv",ascending=False).head(6)
        targets=pd.concat([preferred,extras]).drop_duplicates("player").head(9)

        # Offers that address the partner's primary weakness are searched first.
        preferred_offers=my_pool[my_pool["position"].eq(opp_need_pos)].sort_values("_tv",ascending=False)
        offer_pool=pd.concat([preferred_offers,my_pool]).drop_duplicates("player").head(10)

        candidates=[]
        # 1-for-1
        for oi in range(len(offer_pool)):
            candidates.append((offer_pool.iloc[[oi]],None))
        # 2-for-1 consolidation: particularly useful for turning depth into a weekly starter.
        from itertools import combinations
        top_pkg=offer_pool.head(8)
        for a,b in combinations(range(len(top_pkg)),2):
            candidates.append((top_pkg.iloc[[a,b]],"2-for-1"))

        for _,target in targets.iterrows():
            get_df=pd.DataFrame([target])
            for give_df,package_type in candidates:
                if target["player"] in give_df["player"].tolist():
                    continue
                ...
                # ---- V7.31 HARD RECOMMENDATION GATES ----
                # Never recommend a trade that makes Griffin's optimized roster worse.
                if my_gain < 0.35:
                    continue
                # A trade that only shuffles depth must still create a material overall gain.
                if lineup_gain < 0.15 and my_gain < 0.90:
                    continue
                # Do not call a large overpay a recommendation unless it buys a major lineup upgrade.
                if asset_gap < -2.5 and lineup_gain < 1.75:
                    continue
                # It needs a plausible reason for the other manager to say yes.
                if partner_gain < -0.50:
                    continue

                # Protected/core assets require a clearly meaningful upgrade.
    ...
    # Improvement drives ranking. Realism matters only after a trade already helps Griffin.
    out["_rank"]=out["Lineup Gain"]*3.0+out["Your Gain"]*2.0+out["Trade Fit"]/20.0+out["Partner Gain"]*0.6
    out=out.sort_values(["_rank","Trade Fit","Lineup Gain"],ascending=False).drop(columns=["_rank"])
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        st.caption("V7.18 frozen waiver guard: reserve protection is retained, and waiver moves now account for the opportunity cost of the player being cut. Starters/high-market assets cannot be sacrificed for stash or covered-position luxury adds without a clear lineup or major ROS gain.")
        ...
        if _moves_df.empty:
            ...
            _moves_df=_moves_df[_moves_df.apply(_v714_final_transaction_ok,axis=1)].copy()
...
with tabs[13]:
    st.subheader("🔁 V7.31 Trade Intelligence 2.0")
    st.caption(
        "Only recommends trades that improve your optimized roster, then checks whether the other manager has a plausible reason to accept. Searches 1-for-1 and 2-for-1 consolidation paths."
    )
    ...
    if my.empty or other.empty:
        ...
        if _ideas.empty:
            st.success("HOLD — No trade currently improves your roster enough to recommend.")
        else:
            _show_cols=[c for c in ["Trade Fit","Grade","Partner","Type","You Give","You Get","Lineup Gain","Your Gain","ROS Change","Partner Gain"] if c in _ideas.columns]
            st.dataframe(_ideas[_show_cols].head(10),use_container_width=True,hide_index=True)
            _best_trade=_ideas.iloc[0]
            st.markdown(
                f"**Best path:** {_best_trade['You Give']} → {_best_trade['You Get']} "
            ...
            with st.expander("Why these trades rank here", expanded=False):
                st.dataframe(
                    _ideas[[c for c in ["Partner","You Give","You Get","Your Need","Their Need","Asset Gap","Depth Change","Realism","Why"] if c in _ideas.columns]].head(10),
                    use_container_width=True,hide_index=True
                )
            st.caption(
                "A trade cannot appear here with negative Your Gain. Starting-lineup improvement and total roster gain drive the ranking; fairness and partner fit are secondary gates."
            )
