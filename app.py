...
st.title("🏈 Fantasy Edge V7.39 — Pick 12 Dual-Driver Damping Matrix")
st.caption("V7.39 • V7.24 9× champion frozen • Pick 12 dual-driver damping calibration")
st.caption("Build ID: V7.39-P12DDM-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v739_live_slot")
    ...
    if "v739_target_queue" not in st.session_state:
        st.session_state.v739_target_queue={}
    queue=st.session_state.v739_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v739_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.39 keeps the V7.24 9× champion completely frozen. Damping candidates run only inside the Pick-12 calibration matrix.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.39 robustness report")
        ...
        st.markdown("### 🧪 V7.39 champion baseline validation")
        st.info("V7.39 is calibration only. It tests modest player-score and roster-utility damping only in identified Pick-12 R4-6 dual-driver RB/TE-vs-WR conflicts.")
        ...
        if "v739_independent_results" not in st.session_state:
            st.session_state.v739_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v739_independent_results"),pd.DataFrame) else "ready to run"))
        if "v739_independent_summary" not in st.session_state:
            st.session_state.v739_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v739_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v739_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v739_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v739_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v739_independent_results=iv
                st.session_state.v739_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v739_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v739_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.39 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.39 calibrates Pick-12 R4-6 dual-driver damping only in matched simulations.")
            ...
            st.download_button(
                "⬇️ Download V7.39 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_39_champion_baseline.csv",
                "text/csv",
                key="v739_baseline_download"
            )
        ...
        st.markdown("### 🧪 V7.39 — Pick 12 R4-6 Dual-Driver Damping Matrix")
        st.caption("Calibration only. V7.24 9× remains frozen. Damping activates only at Pick 12, Rounds 4-6, when both player score and roster utility favor the normal RB/TE over the best WR.")

        if "v739_matrix_results" not in st.session_state: st.session_state.v739_matrix_results=None
        if "v739_matrix_meta" not in st.session_state: st.session_state.v739_matrix_meta=None
        sims=st.number_input("Matrix simulations per room",min_value=5,max_value=20,value=10,step=5,key="v739_matrix_sims")
        ca,cb=st.columns(2)
        run=ca.button("Run Pick 12 damping matrix",type="primary",key="v739_matrix_run")
        clear=cb.button("Clear damping matrix",key="v739_matrix_clear")
        if clear:
            st.session_state.v739_matrix_results=None; st.session_state.v739_matrix_meta=None; st.rerun()

        if run:
            configs=[
                ("CHAMPION 100/100",False,1.00,1.00),
                ("A • Player 85 / Roster 100",True,.85,1.00),
                ("B • Player 100 / Roster 85",True,1.00,.85),
                ("C • Player 90 / Roster 90",True,.90,.90),
                ("D • Player 85 / Roster 85",True,.85,.85),
                ("E • Player 80 / Roster 90",True,.80,.90),
            ]
            ...
            for room_name in rooms:
                for j in range(int(sims)):
                    seed=939000+j*61+sum(ord(c) for c in room_name)
                    ...
                    for name,enabled,pw,rw in configs:
                        fe=simulate_mock_fast(fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,
                            p12_dual_damp_enabled=enabled,p12_player_weight=pw,p12_roster_weight=rw)
                        ...
                        rows.append({"Config":name,"Player_weight":pw,"Roster_weight":rw,"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "FE_grade":fc["overall"],"Consensus_grade":cc["overall"],"Difference":fc["overall"]-cc["overall"],
                            "FE_win":fc["overall"]>cc["overall"],"Market_gap":fc["market_value"]-cc["market_value"],
                        ...
                        if done==1 or done%50==0 or done==total: status.caption(f"Damping matrix: {done}/{total}")
            df=pd.DataFrame(rows); st.session_state.v739_matrix_results=df
            st.session_state.v739_matrix_meta={"seconds":time.perf_counter()-started,"drafts":len(df)}
            status.success(f"Damping matrix complete: {len(df)} model drafts."); prog.empty()

        df=st.session_state.get("v739_matrix_results"); meta=st.session_state.get("v739_matrix_meta") or {}
        if isinstance(df,pd.DataFrame) and len(df):
            summ=df.groupby(["Config","Player_weight","Roster_weight"],as_index=False).agg(
                Overall_gap=("Difference","mean"),FE_grade=("FE_grade","mean"),Win_rate=("FE_win","mean"),Market_gap=("Market_gap","mean"),
                Roster_balance=("Roster_balance_gap","mean"),R1_3=("R1_3_gap","mean"),R4_6=("R4_6_gap","mean"),
                R7_10=("R7_10_gap","mean"),R11_plus=("R11_plus_gap","mean"),R4_6_RB=("R4_6_RB","mean"),
                R4_6_WR=("R4_6_WR","mean"),R4_6_TE=("R4_6_TE","mean"),Tests=("Simulation","count"))
            ctrl=summ[summ.Config.eq("CHAMPION 100/100")].iloc[0]
            for metric in ["Overall_gap","R4_6","R7_10","R11_plus"]:
                summ[metric+"_change"]=summ[metric]-float(ctrl[metric])
            summ["Calibration_score"]=summ.Overall_gap_change+.35*summ.R4_6_change-.20*np.maximum(-summ.R7_10_change,0)-.10*np.maximum(-summ.R11_plus_change,0)
            summ=summ.sort_values(["Calibration_score","Overall_gap_change","R4_6_change"],ascending=False).reset_index(drop=True)
            summ["Rank"]=np.arange(1,len(summ)+1)
            st.dataframe(summ[["Rank","Config","Calibration_score","Overall_gap","Overall_gap_change","R4_6","R4_6_change",
                "R7_10","R7_10_change","R11_plus","R11_plus_change","Roster_balance","R4_6_RB","R4_6_WR","R4_6_TE"]].round(3),
                use_container_width=True,hide_index=True)

            leader=summ.iloc[0]
            room=df.groupby(["Config","Room"],as_index=False).agg(Overall=("Difference","mean"),R4_6=("R4_6_gap","mean"),R7_10=("R7_10_gap","mean"))
            croom=room[room.Config.eq("CHAMPION 100/100")][["Room","Overall","R4_6","R7_10"]].rename(columns={"Overall":"C_Overall","R4_6":"C_R4_6","R7_10":"C_R7_10"})
            lroom=room[room.Config.eq(leader.Config)].merge(croom,on="Room")
            lroom["Overall_change"]=lroom.Overall-lroom.C_Overall; lroom["R4_6_change"]=lroom.R4_6-lroom.C_R4_6; lroom["R7_10_change"]=lroom.R7_10-lroom.C_R7_10
            st.markdown("#### Calibration leader by room")
            st.dataframe(lroom[["Room","Overall_change","R4_6_change","R7_10_change"]].round(3),use_container_width=True,hide_index=True)
            if leader.Config=="CHAMPION 100/100":
                st.warning("The untouched champion leads. Do not damp either Pick-12 driver.")
            elif leader.Overall_gap_change>0 and leader.R4_6_change>0 and leader.R7_10_change>-0.40 and int((lroom.Overall_change>0).sum())>=3:
                st.success(f"Screening leader: {leader.Config}. Move only this configuration into V7.40 matched validation.")
            else:
                st.warning(f"{leader.Config} leads the matrix but does not clear the screening gate. Keep V7.24 frozen.")
            st.download_button("⬇️ Download V7.39 Pick 12 damping matrix CSV",df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_39_pick12_dual_driver_damping_matrix.csv","text/csv",key="v739_download")
