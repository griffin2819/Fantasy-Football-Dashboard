...
def _v743_role_confidence(row):
    ...
    if _v682_num(row,"_live_team_verified",0)>0:
        score+=28; signals.append("live team")
    ext=row.get("_external_week_projection",np.nan)
    try:
        if ext is not None and not pd.isna(ext) and float(ext)>0:
            score+=34; signals.append("weekly projection")
    except Exception:
        pass
    market=_v712_row_num(row,["market_pick","Market_pick","market_rank","Market Rank"],999.0)
    if market<=240:
        score+=18 if market<=120 else 12; signals.append("market")
    model=_v712_row_num(row,["model_rank","Model_rank","Model Rank"],999.0)
    if model<=240:
        score+=14 if model<=120 else 9; signals.append("model")
    proj=max(0.0,_v712_row_num(row,["projection","Projection"],0.0))
    if proj>0:
        score+=6; signals.append("FE projection")
    if rec=="PRIOR_SEASON" and games>0:
        score+=5; signals.append("historical context")
    score=max(0.0,min(92.0,score))
    label=("INFERRED — "+", ".join(signals[:3])) if score>=50 else "LOW CONFIDENCE"
...
def _v737_required_position_emergencies(roster_df, news_df=None):
    ...
    for pos,floor in req.items():
        ...
        for _,r in sub.iterrows():
            stx=_v677_normalize_injury_status(r.get("injury_effective",r.get("injury","")))
            high_news=str(news_risk.get(_owner_key(r.get("player","")),"")).upper()=="HIGH"
            # High-risk news on the only required-position option triggers a provisional
            # contingency. The official status is left untouched everywhere.
            if stx not in bad and not high_news:
                playable+=1
        if playable<floor:
            out.add(pos)
    return out
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        st.caption("V7.43 role-confidence engine: observed usage stays distinct from inferred role confidence; waiver HOLDs now expose the best rejected transaction and exact veto reason.")
        ...
        _v737_emergency_positions=_v737_required_position_emergencies(_my_roster,_v742_waiver_news)
        ...
        if _add_pos!="All":
            ...
            # Default All view must prioritize the position(s) with no safe starter.
        ...
        if _moves_df.empty:
            ...
            if "Emergency Coverage" in _moves_df.columns and _moves_df["Emergency Coverage"].fillna(False).any():
                _moves_df=_moves_df.sort_values(["Transaction Score","Decision Score","Emergency Drop Rank","Priority Score"],ascending=[False,False,True,False])
            else:
            ...
            if _best.empty:
                ...
                st.dataframe(_best[["Add","Pos","Drop","Drop Pos","Transaction Score","Decision Score","Context Fit","Decision Why","Drop Cost","Emergency Drop Rank","Drop Context","Priority Score","Waiver Call","FAAB","Claim Advice","Move Type","Need Fit","Evidence","Evidence Detail","Opportunity Boost","Effective Add PPG","Opportunity","Opportunity Confidence","Why Not Priority","Net Week","Net ROS","Add Role","Add Recent PPG","Add Status","Add Practice","Add Injury","Add Matchup","Drop Protection","Drop Status","Drop Practice","Drop Role","Drop Recent PPG","Add PPG","Drop PPG","Add ROS","Drop ROS","Add VORP","Drop VORP","Why"]].head(40),
                             use_container_width=True,hide_index=True)
