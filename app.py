...
st.set_page_config(page_title="Fantasy Edge v9.44 — IDP Positional Authority + Kicker Requirement", page_icon="🏈", layout="wide")
...
def load_state():
    default={
        ...
        "roster_slots":{"QB":1,"RB":2,"WR":2,"TE":0,"FLEX":2,"K":1,"DL":1,"DB":1,"BENCH":6},
        "mock":{"draft_slot":1,"rounds":16,"randomness":12},
...
def v9_live_rank(avail, round_no, current_pick, roster_counts):
    ...
    need=[]
    caps={"QB":(1,2),"RB":(2,99),"WR":(2,99),"TE":(0,2),"K":(1,1),"DL":(1,1),"DB":(1,1)}
    for p in x["position"]:
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    impacts=[]; ext_ranks=[]; eligibles=[]
    for _,r in x.iterrows():
        if str(r.get("position","")).upper() in ("DL","DB"):
            sc,ok,_=_v935_idp_impact_score(r)
            impacts.append(sc)
            ext_ranks.append(_v936_external_idp_rank(r))
            eligibles.append(ok)
        else:
            impacts.append(np.nan); ext_ranks.append(np.nan); eligibles.append(np.nan)
    x["idp_impact_score"]=impacts
    x["idp_external_rank"]=ext_ranks
    x["idp_eligible"]=eligibles
    qtiers=[]; qev=[]
    for _,r in x.iterrows():
        if str(r.get("position","")).upper() in ("DL","DB"):
            qt,qe=_v941_idp_quality(r); qtiers.append(qt); qev.append(qe)
        else:
            qtiers.append(np.nan); qev.append(np.nan)
    x["idp_quality_tier"]=qtiers
    x["idp_quality_evidence"]=qev
    ...
    x=apply_v932_exact_league_construction(x,roster,int(round_no),16,slots)

    # Evidence authority after all roster construction: ineligible IDPs cannot be FINAL PICK.
    bad=x["position"].isin(["DL","DB"]) & (~x["idp_eligible"].fillna(False))
    x.loc[bad,"evaluation_score"]-=100.0
    # v9.44 IDP positional authority.
    # DL/DB use the same shared candidate pipeline as every other position, but their
    # *within-position* ordering is driven by IDP-specific evidence/rank rather than
    # a meaningless global offensive model rank. This prevents low-quality fallback
    # defenders from jumping elite IDPs merely because DL/DB is the last open slot.
    for _p in ("DL","DB"):
        pm=x["position"].eq(_p) & x["idp_eligible"].fillna(False)
        if pm.any():
            ext=pd.to_numeric(x["idp_external_rank"],errors="coerce")
            imp=pd.to_numeric(x["idp_impact_score"],errors="coerce").fillna(0.0)
            tier=pd.to_numeric(x["idp_quality_tier"],errors="coerce")
            best_tier=tier[pm].min()
            if pd.notna(best_tier):
                worse=pm & (tier >= best_tier+2)
                x.loc[worse,"evaluation_score"]-=45.0
            # Explicit positional-rank authority: top IDPs receive a bounded bonus,
            # while deep positional fallbacks are penalized. This is not a forced pick;
            # roster timing/availability still run afterward through execution_choice.
            known=pm & ext.notna()
            if known.any():
                if _p=="DL":
                    rank_bonus=(18.0-ext).clip(lower=-18.0,upper=17.0)*1.35
                else:
                    rank_bonus=(30.0-ext).clip(lower=-20.0,upper=29.0)*0.80
                x.loc[known,"evaluation_score"]+=rank_bonus[known]
            # Impact remains independent player evidence and breaks close rank ties.
            x.loc[pm,"evaluation_score"]+=imp[pm].clip(lower=0,upper=30)*0.30
...
def grade_mock(roster, teams, slots):
    ...
    v9.42 changes:
    - Honors the exact league: TE is NOT a required starter when TE=0; FLEX may use RB/WR/TE.
    ...
    # Exact required starters. Do NOT force a TE in a 0-TE / 2-FLEX league.
    req={p:max(int(slots.get(p,d)),0) for p,d in
         {"QB":1,"RB":2,"WR":2,"TE":0,"K":1,"DL":1,"DB":1}.items()}
    flex_n=max(int(slots.get("FLEX",0)),0)
    ...
    # 60 = neutral competent draft; genuine repeated advantages move the score upward.
    model_edge=float(np.clip(60.0+2.15*avg_pick_edge,35,96))
    ...
    draft_value=float(np.clip(62+0.62*avg_market+2.0*avg_vorp,35,96))
    ...
    # Healthy depth targets for this exact six-bench build, without forcing a TE.
    ...
    positional=float(np.clip(50+starter*.78,35,96))
    ...
    score=float(np.clip(.27*draft_value+.30*construction+.23*positional+.20*model_edge-opp,0,97))
...
def _v938_unified_player_universe(board):
    """Ensure Draft Mode and Mock Draft Lab use the same v9 production player universe.
    Missing consensus-overlay IDPs are appended to the live board with conservative
    late-draft timing fields so they can be ranked by the same roster/IDP mechanism."""
    if board is None or len(board)==0:
        return board
    x=board.copy()
    existing=set(x["player"].astype(str).map(norm)) if "player" in x.columns else set()
    ...
    for name,pos,rank in overlays:
        ...
        # Conservative late-round timing for a 12-team, 16-round snake.
        # External fantasy-IDP rank determines relative order; it does not force a pick.
        market_pick=132.0 + min(rank,40)*1.40
        consensus_rank=market_pick
        model_rank=95.0 + min(rank,40)*0.85
        base_score=max(0.0, 18.0-rank)*0.65
        d.update({
            "id":"v938:"+norm(name),
            "player":name,
            "key":norm(name),
            ...
            "regression":0.0,
            "scarcity":0.0,
            "consensus_strength":45.0,
            "consensus_rank":consensus_rank,
            "market_pick":market_pick,
            "model_rank":model_rank,
            "pure_model_score":base_score,
            "draft_score":base_score,
            "confidence":0.70,
            "identity_key":"v938:"+norm(name)+"|"+pos,
            "v9_market_pick":market_pick,
            "v9_vorp":np.nan,
            "v9_draft_score":base_score,
            "v9_model_rank":model_rank,
            "v9_base_live_score":base_score,
        })
        rows.append(d)
    ...
    # One canonical universe for every mode.
    x=x.drop_duplicates(subset=["key"],keep="first").reset_index(drop=True)
    return x
...
def apply_v932_exact_league_construction(df, roster, round_no, rounds, slots):
    ...
    K is intentionally reserved because the certified board currently has no kicker valuation rows.
    """
    x=df.copy()
...
st.title("🏈 Fantasy Edge v9.42 — Realized Model Edge + Exact League Construction")
st.caption("v9.38 • v9.08 certified engine • unified Draft/Mock player universe • 12-team snake • DB/DL IDP ingestion")
st.caption("Build ID: FE-V9.41-IDP-QUALITY-DOMINANCE-20260821")
...
with st.sidebar:
    ...
    st.caption("League profile: 1 QB • 2 RB • 2 WR • 2 FLEX • 1 K • 1 DL • 1 DB • 6 bench. Kicker is reserved for the final roster slot; the current certified player-value board does not rank kickers.")
...
with tabs[1]:
    ...
    st.success("✅ v9.08 certified engine • v9.22 injury overlay • v9.27.1 coordinated roster opportunity cost • Mine/Opponent tracking")
    ...
    # v9.39 shared Draft/Mock ranking pipeline.
    x,_v939_local=v939_shared_candidate_rank(
        x,myb,live_round,current_overall,live_next,slots,
        randomness=state.get("mock",{}).get("randomness",6)
    )
    ...
    intercept_local,intercepted,intercept_fall=faller_intercept_choice(
        ...
        teams=int(teams),rounds=16
    )
    dyn_min_fall,dyn_band,dyn_improve=dynamic_faller_threshold(current_overall,int(teams),16)
    ...
    st.dataframe(
        ...
        .rename(columns={
            ...
            "idp_external_rank":"IDP rank","idp_impact_score":"IDP impact","roster_opportunity_adj":"Roster opp."
        }).head(120),
        use_container_width=True,hide_index=True
    )
...
with tabs[2]:
    ...
    st.success("🔒 v9.44 SAME Draft/Mock • 🦵 required K • 🛡️ IDP positional authority • 🐍 12-team snake")

    m=state["mock"]
    c1,c2,c3=st.columns(3)
    slot=c1.number_input("Your draft slot",1,12,min(int(m.get("draft_slot",1)),12))
    rounds=c2.number_input("Rounds",8,25,int(m.get("rounds",16)))
    ...
    st.caption("V9.08 remains the frozen certified core; V9.44 applies exact 1QB/2RB/2WR/2FLEX/1K/1DL/1DB/6-bench construction, availability, injury, and a shared positional IDP authority layer without changing the frozen core.")
    if st.button("Set validation preset: Pick 7 • 16 rounds • randomness 6",key="v731_validation_preset"):
        state["mock"]={"draft_slot":7,"rounds":16,"randomness":6}
        save_state(state)
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                nxt=next_user_pick(overall,int(slot),int(teams),int(rounds))
                avail,_v939_local=v939_shared_candidate_rank(
                    avail,roster,rnd,overall,nxt,state["roster_slots"],randomness
                )
