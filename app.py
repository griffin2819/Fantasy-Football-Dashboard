...
def _v775_unified_decisions(board_df,state_snapshot,limit=12):
    ...
    if isinstance(starters,pd.DataFrame) and len(starters):
        for _,sr in starters.iterrows():
            ...
            edge=start_ppg-alt_ppg
            conf=_v716_start_confidence(r,edge,state_snapshot.get("recommendation_history",[]),state_snapshot)
            alt_win=_v774_alt_win_probability(r,alt)
            matters=_v774_is_real_decision(r,alt,edge,conf,status,r.get("practice_status",""),has_alt)
            if matters:
                # opponent posture only nudges close calls; it never reverses a clear projection edge.
                reason=f"edge {edge:+.2f} PPG • alt win {alt_win:.0f}% • {conf}% confidence • {strategy['posture'].lower()} posture"
                rows.append({"Tier":"DO NOW" if conf<62 or abs(edge)<0.6 else "WATCH","Area":"START/SIT","Player":p,"Action":f"Start {p} over {str(alt.get('player',''))}","Week Impact":round(max(0.0,edge),2),"ROS Impact":0.0,"Confidence":round(conf,0),"Why":reason,"Posture":strategy['posture']})
...
with tabs[5]:
    st.caption("V7.75 pipeline: breakout/regression signals now feed the Unified Decision Engine; available breakout candidates remain WATCH until the certified Waiver engine clears the transaction.")
...
with tabs[6]:
    ...
    if _my.empty:
        ...
        st.caption(f"Fantasy Edge V7.75 • {_v677_season} Week {_v677_week} • Decision-quality Start/Sit with authoritative availability, uncertainty-aware close calls, and separate unavailable/monitor groups.")
...
with tabs[11]:
    st.caption("V7.75 pipeline: opportunity signals are cross-checked against ownership, scarcity, and the certified waiver guard before they can become an action.")
...
with tabs[13]:
    st.caption("V7.75 arbitration: Trade Finder feeds the Unified Decision Engine only when both teams benefit and the modeled lineup gain clears the action threshold.")
