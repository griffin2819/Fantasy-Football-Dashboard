...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Validation & Learning Engine", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v756_certify(board_df,state_snapshot):
    ...
    # Waiver output safety if present
    ...
    add("Protected-drop sentinel","PASS","Luxury-depth moves cannot bypass positional-floor/drop protections")
...
def _v755_command_center(board_df,state_snapshot):
    ...
    # top waiver only if full roster simulation is positive
    fa=board_df[board_df["player"].map(lambda n:_player_owner(state_snapshot,n)=="FA")].copy()
    if len(fa) and len(bench):
        drops=bench.copy(); drops["_dv"]=drops.apply(_v690_trade_value,axis=1)
        drop=drops.sort_values("_dv").iloc[0]
        fa["_wv"]=fa.apply(_v690_trade_value,axis=1)
        for _,add in fa.sort_values("_wv",ascending=False).head(12).iterrows():
            sim=_v755_transaction_sim(my,add,drop)
            if sim["total"]>=0.75:
                actions.append((78,"WAIVER",f"Add {add['player']} / drop {drop['player']}",f"roster impact {sim['total']:+.2f}")); break
