...
def prepare_user_draft_candidates(avail, roster, round_no, current_pick, slot, teams, slots, randomness):
    ...
    if _qb>=2 and _te>=1:
        x.loc[x.position.eq("TE"),"_construction_bonus"]-=24.0
        x.loc[x.position.eq("TE"),"second_backup_penalty"]=-24.0
    if _te>=2 and _qb>=1:
        x.loc[x.position.eq("QB"),"_construction_bonus"]-=24.0
        x.loc[x.position.eq("QB"),"second_backup_penalty"]=-24.0
...
def _single_authority_strategy(y,roster,slots,current_pick,next_pick):
    ...
    for idx in keep:
        ...
        z.at[idx,"strategic_pick_value"]=(
            ...
            1.35*wait+
            float(room.loc[idx])+
            1.45*float(_cross_slot.loc[idx])-
    ...
    for _p in ["RB","WR"]:
        ...
        if _have>=5:
            ...
            if _pm.any() and _alt.any():
                ...
                _premium=(
                    (_mrv >= _best_alt_mrv+2.5) &
                    ((_vorp >= _best_alt_vorp+1.0) | (_proj >= _best_alt_proj+1.5)) &
                    (_ropp >= -2.0)
                )
    ...
    if _minimum_now==0:
        ...
        for _idx in _indices:
            ...
            if not _others.any():
                continue
            _dom=(
                (_mrv[_others] >= float(_mrv.loc[_idx])+2.0) &
                (_vorp[_others] >= float(_vorp.loc[_idx])+0.8) &
                (_proj[_others] >= float(_proj.loc[_idx])+1.0) &
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        target_sims=100
        _benchmark_build="FE-V6.5-VALUE-OPTIMIZER-20260826"
        ...
        if len(sim_df):
            ...
            if len(sim_df)>=target_sims:
                _cert={
                    ...
                    "Average RB <= 5.25": bool(pd.to_numeric(sim_df["RB"],errors="coerce").mean()<=5.25),
                    "Average WR <= 5.50": bool(pd.to_numeric(sim_df["WR"],errors="coerce").mean()<=5.50),
                }
