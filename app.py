...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Cached Usage Refresh", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v676_player_value(row):
    # V7.65: when the canonical V7.55 projection ensemble exists, it is the weekly
    # scoring anchor for lineup optimization.  Older/raw model projection remains a
    # safe fallback for code paths that run before canonical player state is attached.
    ...
    v=float(pd.to_numeric(pd.Series([row.get("vorp",row.get("fe_vorp",0))]),errors="coerce").fillna(0).iloc[0])
    ...
    if _v777_page_idx==3:
        ...
        if _my_roster.empty:
            ...
            <div class="fe-stat-grid">
              ...
              <div class="fe-stat-card"><div class="fe-stat-label">Roster Value</div><div class="fe-stat-value">{_base_roster_score:.1f}</div></div>
            ...
            _waiver_data_ready=bool(_v797_sources_ready and _v778_snapshot_loaded and _team_cov>0 and _ext_matches>0 and (_mat_cov>0 or _inj_cov>0 or _role_conf_cov>0))
