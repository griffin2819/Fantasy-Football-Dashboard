...
st.title("🏈 Fantasy Edge V7.85 — R11–15 Roster-State & Sequence Audit")
st.caption("V7.85 • V7.67 frozen champion • late-round sequence diagnostic")
st.caption("Build ID: V7.85-R11-15-SEQUENCE-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.85 keeps V7.67 frozen/live and audits the R11–15 roster-state and sequencing logic without changing production behavior.")
        ...
        st.markdown("### 🔬 V7.85 — R11–15 Roster-State & Sequence Audit")
        st.caption(
            "Measurement only. V7.67 remains the frozen production champion. "
            "V7.85 investigates why the champion's late-round sequence outperforms isolated score/VORP overrides. "
            "It records roster state, positional counts, remaining needs, next-round availability proxies, and the actual sequence of R11–15 selections."
        )

        if "v785_results" not in st.session_state:
            st.session_state.v785_results=None

        sims=st.number_input(
            "V7.85 simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v785_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run R11–15 sequence audit",type="primary",key="v785_run")
        clear=b.button("Clear V7.85 audit",key="v785_clear")
        if clear:
            st.session_state.v785_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=985000+int(ds)*100000+j*307+sum(ord(c) for c in room_name)
                        ...
                        # Build cumulative roster-state snapshots at each late pick.
                        pos_counts={}
                        for _,r in d.iterrows():
                            ...
                            # Lightweight remaining-need proxies based on declared roster slots.
                            rs=state["roster_slots"]
                            def need(slot_key,pos_key):
                                try:
                                    target=int(rs.get(slot_key,0))
                                except Exception:
                                    target=0
                                return max(target-pos_counts.get(pos_key,0),0)
                            ...
                            rows.append({
                                ...
                                "Round":rnd,"Pick":pick,
                                "Player":str(r.get("player","")),"Position":pos,
                                "Market_pick":market,"Market_delta":delta,
                                "Projection":r.get("projection",np.nan),
                                "VORP":r.get("vorp",np.nan),
                                "Draft_score":r.get("draft_score",np.nan),
                                "Model_rank":r.get("model_rank",np.nan),

                                "QB_count":pos_counts.get("QB",0),
                                "RB_count":pos_counts.get("RB",0),
                                "WR_count":pos_counts.get("WR",0),
                                "TE_count":pos_counts.get("TE",0),
                                "DL_count":pos_counts.get("DL",0),
                                "DB_count":pos_counts.get("DB",0),
                                ...
                                "DL_need":dl_need,
                                "DB_need":db_need,
                                "Total_core_need":qb_need+rb_need+wr_need+te_need,
                                "Total_idp_need":dl_need+db_need,
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.85 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v785_results=df
            status.success(f"V7.85 complete: {len(df)} R11–15 sequence decisions audited.")
            prog.empty()

        df=st.session_state.get("v785_results")
        if isinstance(df,pd.DataFrame) and len(df):
            d=df.copy()
            for c in ["Market_delta","Projection","VORP","Draft_score","Model_rank"]:
                d[c]=pd.to_numeric(d[c],errors="coerce")

            round_summary=d.groupby("Round",as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Avg_projection=("Projection","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
                Avg_core_need=("Total_core_need","mean"),
                Avg_idp_need=("Total_idp_need","mean"),
            )
            st.markdown("#### R11–15 sequence by round")
            st.dataframe(round_summary.round(4),use_container_width=True,hide_index=True)

            seq=d.groupby(["Round","Position"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
                Avg_core_need=("Total_core_need","mean"),
                Avg_idp_need=("Total_idp_need","mean"),
            ).sort_values(["Round","Decisions"],ascending=[True,False])
            st.markdown("#### Round × selected position")
            st.dataframe(seq.round(4),use_container_width=True,hide_index=True)

            player_seq=d.groupby(["Round","Player","Position"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
                Avg_core_need=("Total_core_need","mean"),
                Avg_idp_need=("Total_idp_need","mean"),
            ).sort_values(["Decisions","Round"],ascending=[False,True])
            st.markdown("#### Most repeated late-round sequence selections")
            st.dataframe(player_seq.head(50).round(4),use_container_width=True,hide_index=True)

            needs=d.groupby(["Round"],as_index=False).agg(
                QB_need=("QB_need","mean"),
                RB_need=("RB_need","mean"),
                WR_need=("WR_need","mean"),
                TE_need=("TE_need","mean"),
                DL_need=("DL_need","mean"),
                DB_need=("DB_need","mean"),
            )
            st.markdown("#### Average remaining positional needs")
            st.dataframe(needs.round(4),use_container_width=True,hide_index=True)

            slot=d.groupby(["Draft_slot","Round"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
            ...
            st.markdown("#### Draft slot × late round")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            room=d.groupby(["Room","Round"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
            ...
            st.markdown("#### Room × late round")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.85 is measurement-only. The goal is to identify whether V7.67's R11–15 success comes from filling specific positional needs, "
                "preserving a particular sequence, or deferring certain positions until later. No challenger should be built until that sequencing pattern is clear."
            )

            st.download_button(
                "⬇️ Download V7.85 R11–15 sequence audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_85_r11_15_roster_state_sequence_audit.csv",
                "text/csv",key="v785_download"
            )
