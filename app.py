...
st.title("🏈 Fantasy Edge V8.01 — Round-3 QB Timing & Opportunity-Cost Audit")
st.caption("V8.01 • V7.93 frozen champion • R3 QB timing audit")
st.caption("Build ID: V8.01-R3-QB-TIMING-AUDIT-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V8.01 — Round-3 QB Timing & Opportunity-Cost Audit")
        st.caption(
            "Measurement only. V7.93 remains frozen champion. "
            "V8.01 studies the opportunity cost of selecting QB in Round 3 versus waiting, using actual downstream roster construction and later-QB availability. "
            "No player, room, or draft-slot rule is introduced."
        )

        if "v801_results" not in st.session_state:
            st.session_state.v801_results=None

        sims=st.number_input(
            "V8.01 simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v801_sims",
            help="Default: all 12 slots × all 5 rooms × 10 = 600 fresh V7.93 champion drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run V8.01 QB timing audit",type="primary",key="v801_run")
        clear=b.button("Clear V8.01 audit",key="v801_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8010000+int(ds)*100000+j*401+sum(ord(c) for c in room_name)
                        ...
                        if len(r3):
                            r=r3.iloc[0]
                            r3_pos=str(r.get("position",""))
                            if r3_pos=="QB":
                                # Identify first later QB actually selected by the champion, if any.
                                later_qb=d[
                                    (pd.to_numeric(d.mock_round,errors="coerce")>3) &
                                    (d.position.astype(str)=="QB")
                                ].sort_values("mock_round")

                                later_qb_round=int(later_qb.iloc[0].mock_round) if len(later_qb) else np.nan
                                later_qb_player=str(later_qb.iloc[0].player) if len(later_qb) else ""
                                later_qb_vorp=pd.to_numeric(pd.Series([later_qb.iloc[0].get("vorp",np.nan)]),errors="coerce").iloc[0] if len(later_qb) else np.nan
                                later_qb_score=pd.to_numeric(pd.Series([later_qb.iloc[0].get("draft_score",np.nan)]),errors="coerce").iloc[0] if len(later_qb) else np.nan

                                # Capture immediate opportunity-cost candidate using V7.99 R3 alternatives.
                                best_non_qb=None
                                for k in range(1,6):
                                    p=str(r.get(f"R799_Alt{k}_position",""))
                                    if p and p!="QB":
                                        best_non_qb={
                                            "player":str(r.get(f"R799_Alt{k}_player","")),
                                            "position":p,
                                            "market_pick":r.get(f"R799_Alt{k}_market_pick",np.nan),
                                            "vorp":r.get(f"R799_Alt{k}_vorp",np.nan),
                                            "draft_score":r.get(f"R799_Alt{k}_draft_score",np.nan),
                                        }
                                        break

                                # Downstream R4-6 construction.
                                r46=d[pd.to_numeric(d.mock_round,errors="coerce").between(4,6,inclusive="both")]
                                r46_seq=" > ".join(r46.sort_values("mock_round")["position"].astype(str).tolist())
                                r46_vorp=pd.to_numeric(r46.get("vorp",pd.Series(dtype=float)),errors="coerce").mean()
                                r46_score=pd.to_numeric(r46.get("draft_score",pd.Series(dtype=float)),errors="coerce").mean()
                                ...
                                rows.append({
                                    ...
                                    "R3_QB_player":str(r.get("player","")),
                                    "R3_QB_market_pick":r.get("market_pick",np.nan),
                                    "R3_QB_VORP":champ_vorp,
                                    "R3_QB_draft_score":champ_score,
                                    "Best_nonQB_player":best_non_qb["player"] if best_non_qb else "",
                                    "Best_nonQB_position":best_non_qb["position"] if best_non_qb else "",
                                    "Best_nonQB_market_pick":best_non_qb["market_pick"] if best_non_qb else np.nan,
                                    "Best_nonQB_VORP":best_non_qb["vorp"] if best_non_qb else np.nan,
                                    "Best_nonQB_draft_score":best_non_qb["draft_score"] if best_non_qb else np.nan,
                                    "NonQB_VORP_change":(
                                        pd.to_numeric(pd.Series([best_non_qb["vorp"]]),errors="coerce").iloc[0]-champ_vorp
                                        if best_non_qb else np.nan
                                    ),
                                    "NonQB_score_change":(
                                        pd.to_numeric(pd.Series([best_non_qb["draft_score"]]),errors="coerce").iloc[0]-champ_score
                                        if best_non_qb else np.nan
                                    ),
                                    "Later_QB_player":later_qb_player,
                                    "Later_QB_round":later_qb_round,
                                    "Later_QB_VORP":later_qb_vorp,
                                    "Later_QB_draft_score":later_qb_score,
                                    "R4_6_position_sequence":r46_seq,
                                    "R4_6_avg_VORP":r46_vorp,
                                    "R4_6_avg_draft_score":r46_score,
                                    "QB_count_pre":r.get("R799_QB_count_pre",0),
                                    "RB_count_pre":r.get("R799_RB_count_pre",0),
                                    "WR_count_pre":r.get("R799_WR_count_pre",0),
                                    "TE_count_pre":r.get("R799_TE_count_pre",0),
                                })

                        done+=1
                        prog.progress(done/total)
                        if done%100==0 or done==total:
                            status.caption(f"V8.01: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v801_results=df
            status.success(f"V8.01 complete: {len(df)} Round-3 QB selections audited.")
            prog.empty()

        df=st.session_state.get("v801_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in [
                "R3_QB_VORP","R3_QB_draft_score","Best_nonQB_VORP","Best_nonQB_draft_score",
                "NonQB_VORP_change","NonQB_score_change","Later_QB_round","Later_QB_VORP",
                "Later_QB_draft_score","R4_6_avg_VORP","R4_6_avg_draft_score"
            ]:
                df[c]=pd.to_numeric(df[c],errors="coerce")

            df["NonQB_dominates"]=(df.NonQB_VORP_change>0)&(df.NonQB_score_change>0)
            df["Later_QB_exists"]=df.Later_QB_player.astype(str).str.len()>0

            summary=pd.DataFrame([{
                "R3 QB decisions":len(df),
                "Best non-QB dominates":df.NonQB_dominates.mean(),
                "Avg non-QB VORP change":df.NonQB_VORP_change.mean(),
                "Avg non-QB score change":df.NonQB_score_change.mean(),
                "Later QB exists":df.Later_QB_exists.mean(),
                "Avg later QB round":df.Later_QB_round.mean(),
                "Avg later QB VORP":df.Later_QB_VORP.mean(),
                "Avg R4-6 VORP":df.R4_6_avg_VORP.mean(),
                "Avg R4-6 score":df.R4_6_avg_draft_score.mean(),
            }])
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            room=df.groupby("Room",as_index=False).agg(
                Decisions=("R3_QB_player","count"),
                NonQB_dominance=("NonQB_dominates","mean"),
                Avg_nonQB_VORP_change=("NonQB_VORP_change","mean"),
                Avg_nonQB_score_change=("NonQB_score_change","mean"),
                Later_QB_exists=("Later_QB_exists","mean"),
                Avg_later_QB_round=("Later_QB_round","mean"),
                Avg_R4_6_VORP=("R4_6_avg_VORP","mean"),
                Avg_R4_6_score=("R4_6_avg_draft_score","mean"),
            )
            st.markdown("#### R3 QB timing by room")
            ...
            st.markdown("#### R3 QB timing by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            paths=df.groupby(
                ["R3_QB_player","Best_nonQB_player","Best_nonQB_position","Later_QB_player"],
                as_index=False
            ).agg(
                Decisions=("R3_QB_player","count"),
                NonQB_dominance=("NonQB_dominates","mean"),
            ...
            st.markdown("#### R3 QB → non-QB opportunity → later QB paths")
            st.dataframe(paths.head(50).round(4),use_container_width=True,hide_index=True)

            seq=df.groupby(["R4_6_position_sequence"],as_index=False).agg(
                Decisions=("R3_QB_player","count"),
                NonQB_dominance=("NonQB_dominates","mean"),
                Avg_nonQB_VORP_change=("NonQB_VORP_change","mean"),
                Avg_nonQB_score_change=("NonQB_score_change","mean"),
            ...
            st.markdown("#### R4–6 construction after an R3 QB")
            st.dataframe(seq.head(40).round(4),use_container_width=True,hide_index=True)

            st.info(
                "V8.01 is measurement-only. The goal is to determine whether R3 QB selections systematically sacrifice stronger non-QB value while viable later-QB options remain. "
                "Do not hard-code Drake Maye or any specific later QB from this audit."
            )

            st.download_button(
                "⬇️ Download V8.01 R3 QB timing audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_01_round3_qb_timing_opportunity_cost_audit.csv",
                "text/csv",key="v801_download"
            )
