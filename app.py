...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Runtime-Safe Availability", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v680_injury_watch_feed():
    ...
    for key in keys:
        ...
        if _reserve_override:
            ...
            if status in reserve and _obs_same:
                verified=True
                verify_reason="current live roster/reserve designation"
            elif status=="OUT" and (obs_corroboration >= 2 or _same_has_dnp):
                verified=True
                verify_reason="current OUT designation corroborated by live status/practice evidence"
        ...
        _has_authoritative_current = bool(fresh) or bool(verified and (status in ("QUESTIONABLE","DOUBTFUL") or (status in severe and obs_corroboration>=2)))
...
with tabs[6]:
    ...
    if _my.empty:
        ...
        st.caption("Only players eligible to play this week can appear in the optimized lineup. OUT/IR/PUP/NFI/SUSPENDED players are hard-excluded before projections are compared.")
        if _decision_rows:
