...
st.title("🏈 Fantasy Edge V7.6.2 — Standalone Independent Validation")
st.caption("V7.6.2 • standalone independent validation • no regular benchmark dependency • V7.5 draft engine frozen")
st.caption("Build ID: V7.6.2-SIV-20260813")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.6.2 robustness report")
            ...
            st.download_button("⬇️ Download V7.6.2 diagnostic CSV",csv,"fantasy_edge_v7_6_2_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.6.2 independent stress test")
        st.info("This section is now completely separate from the regular benchmark. You do NOT need to run the regular benchmark first.")
        ...
        if "v762_independent_results" not in st.session_state:
            st.session_state.v762_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v762_independent_results"),pd.DataFrame) else "ready to run"))
        if "v762_independent_summary" not in st.session_state:
            st.session_state.v762_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v762_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v762_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v762_run")
        clear_iv=clear_col.button("Clear independent results",key="v762_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                for ds in iv_slots:
                    for room_name in iv_rooms:
                        for j in range(int(iv_sims)):
                            ...
                            fg=independent_draft_grade(fe,int(teams))
                            cg=independent_draft_grade(co,int(teams))
                            rows.append({
                                "Draft_slot":ds,
                                "Room":room_name,
                                "Simulation":j+1,
                                "Fantasy_Edge_grade":fg,
                                "Consensus_grade":cg,
                                "Difference":fg-cg,
                                "Fantasy_Edge_win":fg>cg
                            })
                            done+=1
                ...
                st.session_state.v762_independent_results=iv
                st.session_state.v762_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v762_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v762_independent_summary") or {}
            st.success(
            ...
            st.download_button(
                "⬇️ Download V7.6.2 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_6_2_independent_validation.csv",
                "text/csv",
                key="v762_download"
            )
