...
def late_slot_wr_conflict_choice(eval_score, market_pick, position_code, draft_score, vorp,
    ...
    if (not enabled) or int(draft_slot) not in [7,12] or rnd<4 or rnd>6:
...
st.title("🏈 Fantasy Edge V7.34 — Late-Slot R4-6 WR Conflict Calibration")
st.caption("V7.34 • V7.24 9× champion retained • late-slot R4-6 WR intercept calibration")
st.caption("Build ID: V7.34-LSWRIC-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v734_live_slot")
    ...
    if "v734_target_queue" not in st.session_state:
        st.session_state.v734_target_queue={}
    queue=st.session_state.v734_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v734_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.34 keeps the V7.24 9× champion live/default. Alternative late-slot WR conflict intercepts run only inside the matched calibration.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.34 robustness report")
        ...
        st.markdown("### 🧪 V7.34 champion baseline validation")
        st.info("V7.34 does not change live draft behavior. The calibration tests targeted Picks 7/12 R4-6 WR intercept thresholds against the untouched champion.")
        ...
        if "v734_independent_results" not in st.session_state:
            st.session_state.v734_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v734_independent_results"),pd.DataFrame) else "ready to run"))
        if "v734_independent_summary" not in st.session_state:
            st.session_state.v734_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v734_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v734_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v734_run")
        clear_iv=clear_col.button("Clear independent results",key="v734_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v734_independent_results=iv
                st.session_state.v734_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v734_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v734_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.34 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.34 adds only a matched calibration of late-slot R4-6 WR conflict intercepts.")
            ...
            st.download_button(
                "⬇️ Download V7.34 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_34_champion_baseline.csv",
                "text/csv",
                key="v734_baseline_download"
            )
        ...
        st.markdown("### 🎯 V7.34 Late-Slot R4-6 WR Conflict Intercept Calibration")
        st.caption(
            "The V7.24 9× champion remains live/default. Calibration variants activate only at Picks 7/12 in Rounds 4-6 "
            "when the normal choice is RB/TE and the strongest WR is close enough, unlikely to survive, and the RB/TE edge is player/VORP-driven."
        )

        if "v734_calibration_results" not in st.session_state:
            st.session_state.v734_calibration_results=None
        if "v734_calibration_summary" not in st.session_state:
            st.session_state.v734_calibration_summary=None

        cal_sims=st.number_input(
            "Calibration simulations per slot × room",
            min_value=2,max_value=10,value=3,step=1,key="v734_cal_sims",
            help="Use 3 for screening. Rerun at 10 before promoting any intercept."
        )
        ca,cb=st.columns(2)
        run_cal=ca.button("Run late-slot WR calibration",type="primary",key="v734_cal_run")
        clear_cal=cb.button("Clear late-slot calibration",key="v734_cal_clear")
        ...
        if run_cal:
            configs=[
                ("CONTROL • no late WR intercept",False,0.0,0.0),
                ("WR 6 / 35",True,6.0,.35),
                ("WR 8 / 35",True,8.0,.35),
                ("WR 8 / 45",True,8.0,.45),
                ("WR 10 / 45",True,10.0,.45),
                ("WR 12 / 45",True,12.0,.45),
            ]
            ...
            rows=[]
            total=len(cal_slots)*len(cal_rooms)*int(cal_sims)*len(configs)
            done=0
            prog=st.progress(0)
            status=st.empty()
            ...
            for ds in cal_slots:
                for room_name in cal_rooms:
                    for j in range(int(cal_sims)):
                        seed=934000+int(ds)*10000+j*41+sum(ord(c) for c in room_name)
                        ...
                        for name,enabled,edef,surv in configs:
                            fe=simulate_mock_fast(
                                fast_champ,int(teams),int(ds),15,state["roster_slots"],
                                6.0,True,seed,room_name,
                                late_wr_enabled=enabled,
                                late_wr_eval_deficit=edef,
                                late_wr_survival=surv
                            )
                            fc=independent_draft_grade_components(fe,int(teams))
                            fep=independent_phase_metrics(fe).set_index("Phase")
                            ...
                            row={
                                "Config":name,
                                "Late_WR_enabled":enabled,
                                "Eval_deficit_cap":edef,
                                "WR_survival_cap":surv,
                                "Draft_slot":int(ds),
                                "Room":room_name,
                                "Simulation":j+1,
                                "FE_grade":fc["overall"],
                            ...
                            if done==1 or done%30==0 or done==total:
                                status.caption(f"Late-slot WR calibration: {done}/{total}")

            cal=pd.DataFrame(rows)
            st.session_state.v734_calibration_results=cal
            st.session_state.v734_calibration_summary={
                "seconds":time.perf_counter()-started,
                "sims":int(cal_sims),
                "variant_sims":len(cal)
            }
            status.success(f"Late-slot WR calibration complete: {len(cal)} variant simulations.")
            prog.empty()

        cal_saved=st.session_state.get("v734_calibration_results")
        if isinstance(cal_saved,pd.DataFrame) and len(cal_saved):
            meta=st.session_state.get("v734_calibration_summary") or {}

            summary=cal_saved.groupby(
                ["Config","Late_WR_enabled","Eval_deficit_cap","WR_survival_cap"],as_index=False
            ).agg(
            ...
            control=summary[summary.Config.eq("CONTROL • no late WR intercept")].iloc[0]

            # Since variants can only fire at slots 7/12, score late-slot gain explicitly
            # while protecting the rest of the champion.
            late=cal_saved[cal_saved.Draft_slot.isin([7,12])].groupby("Config",as_index=False).agg(
                Late_overall_gap=("Difference","mean"),
                Late_R4_6_gap=("R4_6_market_gap","mean")
            )
            summary=summary.merge(late,on="Config",how="left")
            late_control=summary[summary.Config.eq("CONTROL • no late WR intercept")].iloc[0]
            ...
            summary["Calibration_score"]=(
                summary.Overall_gap
                +.35*(summary.Late_R4_6_gap-late_control.Late_R4_6_gap)
                +.15*(summary.Late_overall_gap-late_control.Late_overall_gap)
                -.20*summary.Non_mid_leakage
            )
            summary=summary.sort_values(
                ["Calibration_score","Late_R4_6_gap","Overall_gap"],ascending=False
            ).reset_index(drop=True)
            summary["Rank"]=np.arange(1,len(summary)+1)

            disp=summary[[
                "Rank","Config","Calibration_score","Overall_gap","Late_overall_gap",
                "R4_6_gap","Late_R4_6_gap","R1_3_gap","R7_10_gap","R11_plus_gap",
            ...
            c2.metric("Late-slot R4-6 gap",f"{winner.Late_R4_6_gap:+.3f}",
                      f"{winner.Late_R4_6_gap-late_control.Late_R4_6_gap:+.3f}")
            c3.metric("Overall gap",f"{winner.Overall_gap:+.3f}",
                      f"{winner.Overall_gap-control.Overall_gap:+.3f}")
            c4.metric("R4-6 WRs",f"{winner.R4_6_WR:.2f}/draft",
            ...
            if int(meta.get("sims",0))<10:
                st.info(
                    f"Screening leader: {winner.Config}. If it improves Picks 7/12 without damaging Picks 1/3 or other phases, "
                    "rerun the same matrix at 10 simulations per slot × room."
                )
            else:
                if winner.Config=="CONTROL • no late WR intercept":
                    st.success("Full calibration keeps the untouched V7.24 champion. Do not add a late-slot WR intercept.")
                else:
                    st.success(
                        f"Full calibration leader: {winner.Config}. Move only this intercept to a clean champion-validation build."
                    )

            st.download_button(
                "⬇️ Download V7.34 late-slot WR calibration CSV",
                cal_saved.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_34_late_slot_wr_calibration.csv",
                "text/csv",key="v734_cal_download"
            )
