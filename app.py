...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Decision Quality Engine", page_icon="🏈", layout="wide")
st.markdown("""
<style>
    ...
    if _my.empty:
        ...
        if len(_starters):
            for _,sr in _starters.iterrows():
                ...
                matters=(has_alt and (pd.isna(edge) or abs(float(edge))<=close_threshold or conf<70)) or risk!="🟢 LOW"
        ...
        st.caption("Eligible players are ranked by expected PPG first, then a conservative decision-quality layer accounts for projection uncertainty, current role evidence, matchup context, and external agreement. OUT/IR/PUP/NFI/SUSPENDED players remain hard-excluded.")
        if _decision_rows:
        ...
        with st.expander("ℹ️ Why? Decision details",expanded=False):
            if _detail_rows:
                ...
                st.dataframe(_dd[["Slot","Player","Alternative","Edge","Raw PPG Edge","Decision Threshold","Role","Return Ramp","Ramp Basis","Practice Trend","Recent PPG","External PPG","Projection Uncertainty","Floor","Ceiling"]],use_container_width=True,hide_index=True)
            else:
