    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                if round_no==4:
                    mr_diag.update({
                        "R798_Would_trigger":1 if _r798_would_trigger else 0,
                        "R798_Normal_player":str(pool.iloc[int(_r798_normal_global)].player) if _r798_normal_global is not None else "",
                        "R798_Alt_player":str(pool.iloc[int(_r798_best_global)].player) if _r798_best_global is not None else "",
                        "R798_Alt_position":str(pool.iloc[int(_r798_best_global)].position) if _r798_best_global is not None else "",
                    })
...
st.title("🏈 Fantasy Edge V7.98 — Round-4 Generic Adaptation Challenger")
st.caption("V7.98 • V7.93 frozen champion • Round-4 generic challenger")
st.caption("Build ID: V7.98-R4-GENERIC-CHALLENGER-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🧪 V7.98 — Round-4 Generic Adaptation Challenger")
        st.caption(
            "Full-draft matched challenger against the frozen V7.93 champion. "
            "The exact V7.97 signature is tested unchanged: Round 4 only, genuinely alive+eligible alternative, "
            "BOTH higher frozen draft score and higher VORP, effective market distance ≤10 picks. "
            "No room, player, position, roster-state, or draft-slot hard-coding."
        )

        if "v798_results" not in st.session_state:
            st.session_state.v798_results=None

        sims=st.number_input(
            "V7.98 matched simulations per slot × room",
            min_value=10,max_value=40,value=20,step=5,key="v798_sims",
            help="Default: all 12 slots × all 5 rooms × 20 = 1,200 fresh matched sets. "
                 "Trigger-first reruns only drafts where the Round-4 rule activates."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.98 Round-4 challenger",type="primary",key="v798_run")
        clear=b.button("Clear V7.98 challenger",key="v798_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=998000+int(ds)*100000+j*383+sum(ord(c) for c in room_name)
                        ...
                        rr=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(4)]
                        trigger=False; normal=""; alt=""; altpos=""
                        if len(rr):
                            r=rr.iloc[0]
                            trigger=bool(int(pd.to_numeric(
                                pd.Series([r.get("R798_Would_trigger",0)]),
                                errors="coerce"
                            ).fillna(0).iloc[0]))
                            normal=str(r.get("R798_Normal_player",""))
                            alt=str(r.get("R798_Alt_player",""))
                            altpos=str(r.get("R798_Alt_position",""))
                        ...
                        if trigger:
                            cand=simulate_mock_fast(
                                ...
                                late_sequence_variant="champion",
                                r2_generic_adaptation_enabled=True,
                                r4_generic_adaptation_enabled=True
                            )
                            reruns+=1
                        ...
                        rows.append({
                            ...
                            "Normal_R4_player":normal,"Alternative_R4_player":alt,"Alternative_position":altpos,
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.98: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v798_results=df
            status.success(f"V7.98 complete: {len(df)} matched sets; {int(df.Challenger_rerun.sum())} challenger reruns.")
            prog.empty()

        df=st.session_state.get("v798_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.markdown("#### Challenger by all five rooms")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Challenger by all 12 slots")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            paths=trig.groupby(
                ["Normal_R4_player","Alternative_R4_player","Alternative_position"],
                as_index=False
            ).agg(
            ...
            st.markdown("#### Most common activated R4 paths")
            ...
            if screen:
                st.success(
                    "V7.98 SCREEN PASSED. Freeze this exact Round-4 ≤10 rule and advance it to independent fresh-seed validation. V7.93 remains champion until validation."
                )
            else:
                st.warning(
                    "V7.98 does not clear screening. Keep V7.93 champion and do not tune the ≤10 Round-4 signature from this sample."
                )

            st.download_button(
                "⬇️ Download V7.98 Round-4 generic adaptation challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_98_round4_generic_adaptation_challenger.csv",
                "text/csv",key="v798_download"
            )
