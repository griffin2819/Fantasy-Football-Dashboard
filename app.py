...
st.set_page_config(page_title="Fantasy Edge V6.62 — Draft Identity + Playing Time Authority", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V6.62-DRAFT-IDENTITY-PLAYING-TIME-403"
...
def _apply_2026_playing_time_overlay(df):
    """
    Transparent late-round playing-time layer.
    Reads packaged role overrides verified around final 53-man cutdown.
    This does not manufacture projections; it only adjusts recommendation eligibility
    and adds a bounded role bonus/penalty so starters/rotation players beat buried depth.
    """
    y=df.copy()
    if y.empty:
        return y
    if "role_score" not in y.columns:
        y["role_score"]=0.0
    if "playing_time_tier" not in y.columns:
        y["playing_time_tier"]="UNKNOWN"
    if "playing_time_note" not in y.columns:
        y["playing_time_note"]=""
    if "playing_time_block" not in y.columns:
        y["playing_time_block"]=False
    try:
        ov=pd.read_csv(APP_DIR / "playing_time_overrides_2026.csv")
    except Exception:
        return y
    if ov.empty or "player" not in ov.columns:
        return y
    ov=ov.copy()
    ov["key"]=ov["player"].astype(str).map(norm)
    role_map=dict(zip(ov["key"],pd.to_numeric(ov.get("role_score",0),errors="coerce").fillna(0.0)))
    tier_map=dict(zip(ov["key"],ov.get("tier",pd.Series("UNKNOWN",index=ov.index)).astype(str)))
    note_map=dict(zip(ov["key"],ov.get("note",pd.Series("",index=ov.index)).fillna("").astype(str)))
    block_map=dict(zip(ov["key"],ov.get("block_recommendation",pd.Series(False,index=ov.index)).fillna(False).astype(bool)))
    keys=y["player"].astype(str).map(norm) if "player" in y.columns else y["Player"].astype(str).map(norm)
    mapped=keys.map(role_map)
    existing=pd.to_numeric(y["role_score"],errors="coerce").fillna(0.0)
    y["role_score"]=np.where(mapped.notna(),mapped,existing)
    y["playing_time_tier"]=keys.map(tier_map).fillna(y["playing_time_tier"])
    y["playing_time_note"]=keys.map(note_map).fillna(y["playing_time_note"])
    y["playing_time_block"]=keys.map(block_map).fillna(False).astype(bool) | y["playing_time_block"].fillna(False).astype(bool)
    return y
...
def _objective_player_shortlist(x, top_n=15):
    ...
    """
    y=_apply_2026_playing_time_overlay(x.copy())
    if y.empty:
        return y
    ...
    y["player_shortlist_score"]=100.0*(
        0.31*proj_pct.fillna(0.5) +
        0.29*vorp_pct.fillna(0.5) +
        0.12*market_pct.fillna(0.5) +
        0.10*model_pct.fillna(0.5) +
        0.18*role_pct.fillna(0.5)
    ) - np.clip(injury,0,30)*1.2
    # Explicit recommendation blocks (IR/season-ending or no viable role) never shortlist.
    if "playing_time_block" in y.columns:
        y.loc[y["playing_time_block"].fillna(False).astype(bool),"player_shortlist_score"]=-1e9
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    x=_apply_2026_playing_time_overlay(avail.copy())
    counts=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    ...
    # projection, VORP and verified playing-time role explicit.
    _role=_uc_num("role_score",0.0)
    # Bounded playing-time authority: separate starters/rotation players from buried depth
    # without allowing role alone to overpower a true projection edge.
    _role_component=np.clip(_role,-10.0,10.0)*1.35
    _player_value=0.72*_eval + 0.15*_vorp + 0.09*_proj + 0.04*_role_component
    ...
    x["playing_time_component"]=_role_component
    ...
    if int(round_no)>=11:
        # Back-of-draft tie breaker: actual path to snaps matters more than tiny model gaps.
        x["unified_pick_score"] += np.clip(_role,-8.0,8.0)*1.15
    ...
    if "playing_time_block" in x.columns:
        _hard_block |= x["playing_time_block"].fillna(False).astype(bool)
...
def _fast_local_draft_board():
    ...
    x=_apply_2026_playing_time_overlay(x)
...
# V6.59 startup-order fix: this helper must exist before V6.58 merges packaged
# drafted names into state during module initialization.
def _dedupe_names_canonical(values):
    out=[]; seen=set()
    for value in (values or []):
        name=str(value).strip(); key=norm(name)
        if name and key and key not in seen:
            seen.add(key); out.append(name)
    return out

# V6.58: one authoritative unavailable-player identity set. The packaged live
# draft log is merged into saved state on every startup so a stale Streamlit
# state file cannot resurrect already-drafted players. All UI/database paths
# must filter through the same canonical keys.
def _v658_packaged_drafted_names():
    names=[]
    for _fname in ["fantasy_edge_live_draft_log_cleaned.csv","fantasy_edge_live_draft_log(3).csv"]:
        _p=APP_DIR / _fname
        if not _p.exists():
            continue
        try:
            _d=pd.read_csv(_p)
            _pc=next((c for c in ["player","Player","name","Name"] if c in _d.columns),None)
            if _pc:
                names += [str(v).strip() for v in _d[_pc].dropna().tolist() if str(v).strip()]
        except Exception:
            pass
    return names

# V6.61: hard identity guard for live-draft picks that have already been confirmed
# as unavailable. This supplements (never replaces) the state + packaged draft-log
# sources, protecting every UI path from stale Streamlit/session state or a delayed
# state-file write. Values are canonical keys so display spelling/suffixes cannot leak.
_V661_CONFIRMED_DRAFTED_KEYS={
    "deebosamuel", "stefondiggs", "jacorycroskeymerritt",
    "marshawnlloyd", "travishunter"
}

def _v658_unavailable_keys(extra=None):
    vals=list(state.get("taken",[]) or []) + list(state.get("my_team",[]) or [])
    vals += _v658_packaged_drafted_names()
    if extra:
        vals += list(extra)
    keys={norm(v) for v in vals if norm(v)}
    keys.update(_V661_CONFIRMED_DRAFTED_KEYS)
    return keys

_v658_log_names=_v658_packaged_drafted_names()
state["taken"]=_dedupe_names_canonical(list(state.get("taken",[]) or []) + _v658_log_names + list(state.get("my_team",[]) or []))
state["teams"]=12
...
# V6.58 hard availability guard: retain the master board for analysis, but tag
# canonical availability once so every downstream view can consistently exclude
# drafted identities, including dual-role players such as Travis Hunter.
board["_v658_player_key"]=board["player"].astype(str).map(norm)
board["_v658_unavailable"]=board["_v658_player_key"].isin(_v658_unavailable_keys())
...
state["my_team"]=_dedupe_names_canonical(state.get("my_team",[]))
state["taken"]=_dedupe_names_canonical(state.get("taken",[]))
...
with tabs[0]:
    ...
    _all_name_set=set(all_names)
    _my_default=[n for n in state["my_team"] if n in _all_name_set]
    my=st.multiselect("Players on MY Yahoo team",all_names,default=_my_default)
    _taken_default=[n for n in list(dict.fromkeys(state["taken"]+my)) if n in _all_name_set]
    taken=st.multiselect("All players already rostered/taken in Yahoo",all_names,default=_taken_default)
    if st.button("💾 Save roster & taken players"):
        state["my_team"]=_dedupe_names_canonical(my); state["taken"]=_dedupe_names_canonical(list(taken)+list(my)+_v658_packaged_drafted_names()); save_state(state); st.success("Saved")
...
with tabs[1]:
    ...
    _taken_live_keys=_v658_unavailable_keys()
    x=board[~board["player"].astype(str).map(norm).isin(_taken_live_keys)].copy()

    _my_live_keys={norm(n) for n in state.get("my_team",[]) if norm(n)}
    myb=board[board["player"].astype(str).map(norm).isin(_my_live_keys)].copy()
    ...
    with st.expander("📚 Full available player database", expanded=False):
        _taken_db_keys=_v658_unavailable_keys()
        _db=board[~board["player"].astype(str).map(norm).isin(_taken_db_keys)].copy()
        _db["_canon"]= _db["player"].astype(str).map(norm)
        _db=_db.drop_duplicates("_canon",keep="first").drop(columns="_canon")
    ...
    _taken_display_keys=_v658_unavailable_keys()
    _display_master["_canon_taken"]=_display_master["player"].astype(str).map(norm)
    _display_pool=_display_master[~_display_master["_canon_taken"].isin(_taken_display_keys)].copy()
    _display_pool=_display_pool.drop_duplicates("_canon_taken",keep="first").drop(columns="_canon_taken")
    _rec_keys={norm(n) for n in x["player"].astype(str)} if len(x) else set()
    _display_pool["recommendation_pool"]=_display_pool["player"].astype(str).map(
        lambda n: "Yes" if norm(n) in _rec_keys else "No — model filtered"
    )
    ...
    _taken_keys=_v658_unavailable_keys(_taken_names)
    _record_by_key={}
    ...
    if _record_clicked:
        ...
        if not _commit_pick:
            ...
            state["taken"]=_dedupe_names_canonical(list(state.get("taken",[]))+[_commit_pick])
    ...
    if undo_col.button("↩️ Undo Last Pick", use_container_width=True):
        ...
        if not log:
            ...
            _last_key=norm(player)
            taken=[n for n in taken if norm(n)!=_last_key]
            state["taken"]=_dedupe_names_canonical(taken)
...
with tabs[3]:
    ...
    _taken_waiver_keys=_v658_unavailable_keys()
    x=_waiver_board[~_waiver_board["player"].astype(str).map(norm).isin(_taken_waiver_keys)].sort_values("waiver_score",ascending=False).copy()
...
with tabs[4]:
    ...
    _taken_idp_keys=_v658_unavailable_keys()
    x=board[board["position"].isin(["DL","DB"]) & ~board["player"].astype(str).map(norm).isin(_taken_idp_keys)].copy()
...
with tabs[7]:
    ...
    st.write({"Team":p.team,"Position":p.position,"Listed position":p.raw_position,
              ...
              "Yahoo status":"My team" if norm(name) in {norm(n) for n in state.get("my_team",[]) if norm(n)} else "Taken" if norm(name) in _v658_unavailable_keys() else "Available"})
