...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    if _rb_depth or _wr_depth:
        _idp_pool=(
            x["position"].isin(["DL","DB"]) &
            x["idp_eligible"].fillna(False).astype(bool) &
            (_safe_num_series(x,"idp_quality_tier",5.0)<=2)
        )
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        target_sims=100
        _benchmark_build="FE-PREFLIGHT-V3-20260825"
        ...
        if st.session_state.get("fe_benchmark_failures"):
            ...
            st.error(f"Benchmark engine failures: {len(_fails)}. Certification cannot pass until reset and rerun cleanly.")
            with st.expander("Show benchmark failures"):
                st.dataframe(pd.DataFrame(_fails),use_container_width=True,hide_index=True)
