...
st.title("🏈 Fantasy Edge V7.87 — Full-System Champion Stress Test")
st.caption("V7.87 • V7.67 frozen champion • full-system stress baseline")
st.caption("Build ID: V7.87-FULL-SYSTEM-STRESS-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.87 keeps V7.67 frozen/live and establishes a comprehensive all-slot, all-room champion stability baseline with no production changes.")
        ...
        st.markdown("### 🧱 V7.87 — Full-System Champion Stress Test")
        st.caption(
            "V7.67 remains the frozen production champion. V7.87 changes no production behavior. "
            "This stress test runs fresh champion drafts across ALL 12 draft slots and all five room environments to establish a comprehensive stability baseline "
            "before any new independent improvement branch is opened."
        )

        if "v787_results" not in st.session_state:
            st.session_state.v787_results=None

        sims=st.number_input(
            "V7.87 simulations per slot × room",
            min_value=10,max_value=50,value=20,step=5,key="v787_sims",
            help="Default: 12 slots × 5 rooms × 20 = 1,200 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.87 full-system stress test",type="primary",key="v787_run")
        clear=b.button("Clear V7.87 stress test",key="v787_clear")
        if clear:
            st.session_state.v787_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=987000+int(ds)*100000+j*313+sum(ord(c) for c in room_name)
                        ...
                        for ph in ["R1-3","R4-6","R7-10","R11+"]:
                            rows.append({
                                ...
                                "Phase_market_delta":float(phase.loc[ph,"Avg_market_delta"]) if ph in phase.index else np.nan,
                                "Roster_balance":comp.get("roster_balance",np.nan),
                                "Starter_completion":comp.get("starter_completion",np.nan),
                                "Availability":comp.get("availability",np.nan),
                                "Market_value":comp.get("market_value",np.nan),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.87 stress test: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v787_results=df
            status.success(f"V7.87 complete: {total} fresh champion drafts across all slots and rooms.")
            prog.empty()

        df=st.session_state.get("v787_results")
        if isinstance(df,pd.DataFrame) and len(df):
            # One row per phase, so collapse overall-grade and component fields carefully.
            draft_level=df.groupby(
                ["Draft_slot","Room","Simulation","Seed"],as_index=False
            ).agg(
                Overall_grade=("Overall_grade","first"),
                Roster_balance=("Roster_balance","first"),
                Starter_completion=("Starter_completion","first"),
                Availability=("Availability","first"),
                Market_value=("Market_value","first"),
            )

            phase_summary=df.groupby("Phase",as_index=False).agg(
                Decisions=("Phase","count"),
                Avg_market_delta=("Phase_market_delta","mean"),
                Std_market_delta=("Phase_market_delta","std"),
                Min_market_delta=("Phase_market_delta","min"),
                Max_market_delta=("Phase_market_delta","max"),
            )

            slot_summary=draft_level.groupby("Draft_slot",as_index=False).agg(
                Drafts=("Seed","count"),
                Avg_overall=("Overall_grade","mean"),
                Std_overall=("Overall_grade","std"),
                Min_overall=("Overall_grade","min"),
                Max_overall=("Overall_grade","max"),
                Avg_roster_balance=("Roster_balance","mean"),
                Avg_starter_completion=("Starter_completion","mean"),
                Avg_availability=("Availability","mean"),
                Avg_market_value=("Market_value","mean"),
            )

            room_summary=draft_level.groupby("Room",as_index=False).agg(
                Drafts=("Seed","count"),
                Avg_overall=("Overall_grade","mean"),
                Std_overall=("Overall_grade","std"),
                Min_overall=("Overall_grade","min"),
            ...
            phase_slot=df.groupby(["Draft_slot","Phase"],as_index=False).agg(
                Avg_market_delta=("Phase_market_delta","mean"),
                Std_market_delta=("Phase_market_delta","std"),
            )

            phase_room=df.groupby(["Room","Phase"],as_index=False).agg(
                Avg_market_delta=("Phase_market_delta","mean"),
                Std_market_delta=("Phase_market_delta","std"),
            )

            overall_mean=draft_level.Overall_grade.mean()
            overall_std=draft_level.Overall_grade.std()
            worst_slot=float(slot_summary.Avg_overall.min())
            worst_room=float(room_summary.Avg_overall.min())
            spread_slots=float(slot_summary.Avg_overall.max()-slot_summary.Avg_overall.min())
            spread_rooms=float(room_summary.Avg_overall.max()-room_summary.Avg_overall.min())

            top=pd.DataFrame([{
                "Champion drafts":len(draft_level),
                "Overall mean":overall_mean,
                "Overall std":overall_std,
                "Worst slot avg":worst_slot,
                "Worst room avg":worst_room,
                "Slot avg spread":spread_slots,
                "Room avg spread":spread_rooms,
            }])
            st.dataframe(top.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Phase stability")
            st.dataframe(phase_summary.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### All 12 draft slots")
            st.dataframe(slot_summary.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### All five room environments")
            st.dataframe(room_summary.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Slot × room stress matrix")
            st.dataframe(slot_room.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Phase × draft slot")
            st.dataframe(phase_slot.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Phase × room")
            st.dataframe(phase_room.round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.87 is a baseline stress test, not a challenger. Use this file to establish V7.67's full slot/room stability envelope. "
                "Any future challenger should be judged against this baseline rather than only a four-slot subset."
            )

            st.download_button(
                "⬇️ Download V7.87 full-system stress test CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_87_full_system_champion_stress_test.csv",
                "text/csv",key="v787_download"
            )
