...
st.title("🏈 Fantasy Edge V7.10 — Faller Intercept")
st.caption("V7.10 • Evaluation + Execution + Faller Intercept • Target Queue • independent validator retained")
st.caption("Build ID: V7.10-FI-20260813")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v710_live_slot")
    ...
    if "v710_target_queue" not in st.session_state:
        st.session_state.v710_target_queue={}
    queue=st.session_state.v710_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v710_target_queue=queue
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.10 robustness report")
            ...
            st.download_button("⬇️ Download V7.10 diagnostic CSV",csv,"fantasy_edge_v7_10_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.10 independent validation")
        ...
        if "v710_independent_results" not in st.session_state:
            st.session_state.v710_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v710_independent_results"),pd.DataFrame) else "ready to run"))
        if "v710_independent_summary" not in st.session_state:
            st.session_state.v710_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v710_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v710_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v710_run")
        clear_iv=clear_col.button("Clear independent results",key="v710_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v710_independent_results=iv
                st.session_state.v710_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v710_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v710_independent_summary") or {}
            st.success(
            ...
            st.download_button(
                "⬇️ Download V7.10 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_10_independent_validation.csv",
                "text/csv",
                key="v710_download"
            )
