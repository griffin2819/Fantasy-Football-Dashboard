...
st.title("🏈 Fantasy Edge V8.00 — Round-3 Causal Downstream Probe")
st.caption("V8.00 • V7.93 frozen champion • Round-3 causal probe")
st.caption("Build ID: V8.00-R3-CAUSAL-PROBE-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🧬 V8.00 — Round-3 Causal Downstream Probe")
        st.caption(
            "Causal mechanism experiment, not a promotion challenger. V7.93 remains frozen champion. "
            "When the generic Round-3 signature appears (alive+eligible alternative, higher frozen score + higher VORP, market distance ≤10), "
            "V8.00 forks the draft: untouched champion vs forced Round-3 alternative, then fully redrafts R4–15."
        )

        if "v800_results" not in st.session_state:
            st.session_state.v800_results=None

        sims=st.number_input(
            "V8.00 simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v800_sims",
            help="Default: all 12 slots × all 5 rooms × 10 = 600 fresh champion seeds. "
                 "Trigger-first only reruns the causal fork when the R3 signature appears."
        )
        a,b=st.columns(2)
        run=a.button("Run V8.00 R3 causal probe",type="primary",key="v800_run")
        clear=b.button("Clear V8.00 probe",key="v800_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8000000+int(ds)*100000+j*397+sum(ord(c) for c in room_name)
                        ...
                        rr=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(3)]
                        trigger=False; normal=""; alt=""; altpos=""
                        if len(rr):
                            r=rr.iloc[0]
                            trigger=bool(int(pd.to_numeric(
                                pd.Series([r.get("R800_Would_trigger",0)]),
                                errors="coerce"
                            ).fillna(0).iloc[0]))
                            normal=str(r.get("R800_Normal_player",""))
                            alt=str(r.get("R800_Alt_player",""))
                            altpos=str(r.get("R800_Alt_position",""))

                        if trigger:
                            fork=simulate_mock_fast(
                                pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name,
                                r5_dominance_override_enabled=False,
                                r2_dominance_override_enabled=False,
                                r3_dominance_override_enabled=False,
                                ...
                                r2_generic_adaptation_enabled=True,
                                r4_generic_adaptation_enabled=False,
                                r3_causal_probe_enabled=True
                            )
                            reruns+=1

                            cc=independent_draft_grade_components(champ,int(teams))
                            fc=independent_draft_grade_components(fork,int(teams))
                            cp=independent_phase_metrics(champ).set_index("Phase")
                            fp=independent_phase_metrics(fork).set_index("Phase")

                            champ_players=champ.set_index("mock_pick")["player"].to_dict()
                            changed=sum(champ_players.get(x.mock_pick,None)!=x.player for _,x in fork.iterrows())

                            overall=fc["overall"]-cc["overall"]
                            r13=float(fp.loc["R1-3","Avg_market_delta"])-float(cp.loc["R1-3","Avg_market_delta"])
                            r46=float(fp.loc["R4-6","Avg_market_delta"])-float(cp.loc["R4-6","Avg_market_delta"])
                            r710=float(fp.loc["R7-10","Avg_market_delta"])-float(cp.loc["R7-10","Avg_market_delta"])
                            r11=float(fp.loc["R11+","Avg_market_delta"])-float(cp.loc["R11+","Avg_market_delta"])

                            # R4-6 construction deltas for mechanism interpretation.
                            c46=champ[pd.to_numeric(champ.mock_round,errors="coerce").between(4,6,inclusive="both")]
                            f46=fork[pd.to_numeric(fork.mock_round,errors="coerce").between(4,6,inclusive="both")]
                            c46_v=pd.to_numeric(c46.get("vorp",pd.Series(dtype=float)),errors="coerce").mean()
                            f46_v=pd.to_numeric(f46.get("vorp",pd.Series(dtype=float)),errors="coerce").mean()
                            c46_s=pd.to_numeric(c46.get("draft_score",pd.Series(dtype=float)),errors="coerce").mean()
                            f46_s=pd.to_numeric(f46.get("draft_score",pd.Series(dtype=float)),errors="coerce").mean()
                            cseq=" > ".join(c46.sort_values("mock_round")["position"].astype(str).tolist())
                            fseq=" > ".join(f46.sort_values("mock_round")["position"].astype(str).tolist())
                        else:
                            changed=0
                            overall=r13=r46=r710=r11=0.0
                            c46_v=f46_v=c46_s=f46_s=np.nan
                            cseq=fseq=""

                        rows.append({
                            "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Trigger_detected":trigger,"Fork_rerun":trigger,
                            "Champion_R3_player":normal,"Fork_R3_player":alt,"Fork_R3_position":altpos,
                            "Changed_picks":changed,
                            "Overall_change":overall,"R1_3_change":r13,"R4_6_change":r46,
                            "R7_10_change":r710,"R11_plus_change":r11,
                            "R4_6_VORP_change":f46_v-c46_v if trigger else np.nan,
                            "R4_6_score_change":f46_s-c46_s if trigger else np.nan,
                            "Champion_R4_6_sequence":cseq,"Fork_R4_6_sequence":fseq,
                        ...
                        if done==1 or done%100==0 or done==total:
                            status.caption(f"V8.00: {done}/{total} seeds • causal forks {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v800_results=df
            status.success(f"V8.00 complete: {len(df)} seeds; {int(df.Fork_rerun.sum())} causal forks.")
            prog.empty()

        df=st.session_state.get("v800_results")
        if isinstance(df,pd.DataFrame) and len(df):
            trig=df[df.Trigger_detected].copy()
            summary=pd.DataFrame([{
                "Seeds":len(df),
                "Triggered forks":len(trig),
                "Activation":df.Trigger_detected.mean(),
                "Overall change":df.Overall_change.mean(),
                "R1-3 change":df.R1_3_change.mean(),
                "R4-6 change":df.R4_6_change.mean(),
                "R7-10 change":df.R7_10_change.mean(),
                "R11+ change":df.R11_plus_change.mean(),
                "Conditional win":trig.Conditional_overall_win.mean() if len(trig) else np.nan,
                "Conditional non-loss":trig.Conditional_nonloss.mean() if len(trig) else np.nan,
                "Avg changed picks":trig.Changed_picks.mean() if len(trig) else np.nan,
                "R4-6 VORP change when triggered":trig.R4_6_VORP_change.mean() if len(trig) else np.nan,
                "R4-6 score change when triggered":trig.R4_6_score_change.mean() if len(trig) else np.nan,
            }])
            ...
            st.markdown("#### Causal probe by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Causal probe by all 12 slots")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            paths=trig.groupby(
                ["Champion_R3_player","Fork_R3_player","Fork_R3_position"],
                as_index=False
            ).agg(
                Decisions=("Trigger_detected","count"),
                Overall_change=("Overall_change","mean"),
                R4_6_change=("R4_6_change","mean"),
                R4_6_VORP_change=("R4_6_VORP_change","mean"),
                R4_6_score_change=("R4_6_score_change","mean"),
                Win_rate=("Conditional_overall_win","mean"),
            ).sort_values("Decisions",ascending=False) if len(trig) else pd.DataFrame()
            st.markdown("#### R3 causal paths")
            st.dataframe(paths.head(40).round(4),use_container_width=True,hide_index=True)

            seq=trig.groupby(
                ["Champion_R4_6_sequence","Fork_R4_6_sequence"],as_index=False
            ).agg(
                Decisions=("Trigger_detected","count"),
                Overall_change=("Overall_change","mean"),
                Win_rate=("Conditional_overall_win","mean"),
            ).sort_values("Decisions",ascending=False) if len(trig) else pd.DataFrame()
            st.markdown("#### Downstream sequence changes")
            st.dataframe(seq.head(40).round(4),use_container_width=True,hide_index=True)

            st.info(
                "V8.00 is a causal mechanism probe, not a promotion challenger. "
                "If the forced R3 alternative loses broadly, retire the R3 branch. "
                "If it wins and the effect generalizes beyond one player path, the next build can define a proper challenger."
            )

            st.download_button(
                "⬇️ Download V8.00 R3 causal downstream probe CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_00_round3_causal_downstream_probe.csv",
                "text/csv",key="v800_download"
            )
