...
with tabs[1]:
    ...
    with st.form("v640_record_pick_form", clear_on_submit=False):
        pick=st.selectbox(
            "Player selected",
            [""]+_record_options,
    ...
    if _record_clicked:
        ...
        if not _commit_pick:
            ...
            state["draft_log"]=log
            save_state(state)
            # Reset only the new form fields after a successful commit.
            st.session_state["v640_record_pick"]=""
            st.session_state["v640_opponent_team"]=""
            st.rerun()
