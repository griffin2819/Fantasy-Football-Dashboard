...
st.title("🏈 Fantasy Edge V7.89 — R1–4 Sharp/Value Board-Depletion Audit")
st.caption("V7.89 • V7.67 frozen champion • early-board depletion diagnostic")
st.caption("Build ID: V7.89-R1-4-SHARP-DEPLETION-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.89 keeps V7.67 frozen/live and determines whether the early Sharp/value deficit is unavoidable board depletion or an actionable fallback-selection failure.")
        ...
        st.markdown("### 🔬 V7.89 — R1–4 Sharp/Value Board-Depletion Audit")
        st.caption(
            "Measurement only. V7.67 remains the frozen production champion. "
            "V7.89 compares Sharp/value versus Consensus/ADP-heavy in Rounds 1–4 using the exact board state at each user pick. "
            "The goal is to distinguish unavoidable board depletion from a fallback/adaptation failure."
        )

        if "v789_results" not in st.session_state:
            st.session_state.v789_results=None

        sims=st.number_input(
            "V7.89 simulations per slot × room",
            min_value=10,max_value=50,value=20,step=5,key="v789_sims",
            help="Default: all 12 slots × 3 focus rooms × 20 = 720 fresh champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run R1–4 board-depletion audit",type="primary",key="v789_run")
        clear=b.button("Clear V7.89 audit",key="v789_clear")
        if clear:
            st.session_state.v789_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=989000+int(ds)*100000+j*331+sum(ord(c) for c in room_name)
                        ...
                        early=d[pd.to_numeric(d.mock_round,errors="coerce").between(1,4,inclusive="both")]
                        for _,r in early.iterrows():
                            audit=int(pd.to_numeric(pd.Series([r.get("R789_Audit",0)]),errors="coerce").fillna(0).iloc[0])
                            if audit!=1:
                                continue

                            champ_score=pd.to_numeric(pd.Series([r.get("draft_score",np.nan)]),errors="coerce").iloc[0]
                            champ_vorp=pd.to_numeric(pd.Series([r.get("vorp",np.nan)]),errors="coerce").iloc[0]
                            best_score=pd.to_numeric(pd.Series([r.get("R789_Best_score_draft_score",np.nan)]),errors="coerce").iloc[0]
                            best_score_vorp=pd.to_numeric(pd.Series([r.get("R789_Best_score_vorp",np.nan)]),errors="coerce").iloc[0]
                            best_vorp=pd.to_numeric(pd.Series([r.get("R789_Best_vorp_vorp",np.nan)]),errors="coerce").iloc[0]
                            best_vorp_score=pd.to_numeric(pd.Series([r.get("R789_Best_vorp_draft_score",np.nan)]),errors="coerce").iloc[0]

                            rows.append({
                                "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                "Round":int(r.mock_round),"Pick":r.get("mock_pick",np.nan),
                                "Champion_player":str(r.get("player","")),"Champion_position":str(r.get("position","")),
                                "Champion_market_pick":r.get("market_pick",np.nan),
                                "Champion_VORP":champ_vorp,"Champion_draft_score":champ_score,
                                "Actual_available_count":r.get("R789_Actual_available_count",np.nan),

                                "Best_score_player":r.get("R789_Best_score_player",""),
                                "Best_score_position":r.get("R789_Best_score_position",""),
                                "Best_score_market_pick":r.get("R789_Best_score_market_pick",np.nan),
                                "Best_score_VORP":best_score_vorp,
                                "Best_score_draft_score":best_score,

                                "Best_vorp_player":r.get("R789_Best_vorp_player",""),
                                "Best_vorp_position":r.get("R789_Best_vorp_position",""),
                                "Best_vorp_market_pick":r.get("R789_Best_vorp_market_pick",np.nan),
                                "Best_vorp_VORP":best_vorp,
                                "Best_vorp_draft_score":best_vorp_score,

                                "Score_gap_best_minus_champ":best_score-champ_score if pd.notna(best_score) and pd.notna(champ_score) else np.nan,
                                "VORP_gap_bestscore_minus_champ":best_score_vorp-champ_vorp if pd.notna(best_score_vorp) and pd.notna(champ_vorp) else np.nan,
                                "VORP_gap_bestvorp_minus_champ":best_vorp-champ_vorp if pd.notna(best_vorp) and pd.notna(champ_vorp) else np.nan,
                                "Score_gap_bestvorp_minus_champ":best_vorp_score-champ_score if pd.notna(best_vorp_score) and pd.notna(champ_score) else np.nan,
                                "Best_score_dominates":bool(pd.notna(best_score) and pd.notna(best_score_vorp) and pd.notna(champ_score) and pd.notna(champ_vorp) and best_score>champ_score and best_score_vorp>champ_vorp),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%120==0 or done==total:
                            status.caption(f"V7.89 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v789_results=df
            status.success(f"V7.89 complete: {len(df)} R1–4 true-board decisions audited.")
            prog.empty()

        df=st.session_state.get("v789_results")
        if isinstance(df,pd.DataFrame) and len(df):
            d=df.copy()
            for c in ["Champion_VORP","Champion_draft_score","Score_gap_best_minus_champ",
                      "VORP_gap_bestscore_minus_champ","VORP_gap_bestvorp_minus_champ",
                      "Score_gap_bestvorp_minus_champ"]:
                d[c]=pd.to_numeric(d[c],errors="coerce")

            by_room_round=d.groupby(["Room","Round"],as_index=False).agg(
                Decisions=("Champion_player","count"),
                Avg_champion_VORP=("Champion_VORP","mean"),
                Avg_champion_score=("Champion_draft_score","mean"),
                Avg_best_score_gap=("Score_gap_best_minus_champ","mean"),
                Avg_bestscore_VORP_gap=("VORP_gap_bestscore_minus_champ","mean"),
                Avg_bestvorp_VORP_gap=("VORP_gap_bestvorp_minus_champ","mean"),
                Best_score_dominance=("Best_score_dominates","mean"),
            )
            st.markdown("#### True-board quality gap by room and round")
            st.dataframe(by_room_round.round(4),use_container_width=True,hide_index=True)

            baseline=by_room_round[by_room_round.Room.isin(["Consensus","ADP-heavy"])].groupby("Round",as_index=False).agg(
                Baseline_score_gap=("Avg_best_score_gap","mean"),
                Baseline_vorp_gap=("Avg_bestscore_VORP_gap","mean"),
                Baseline_dominance=("Best_score_dominance","mean"),
            )
            sharp=by_room_round[by_room_round.Room=="Sharp/value"].rename(columns={
                "Avg_best_score_gap":"Sharp_score_gap",
                "Avg_bestscore_VORP_gap":"Sharp_vorp_gap",
                "Best_score_dominance":"Sharp_dominance",
            })
            diff=sharp.merge(baseline,on="Round",how="left")
            diff["Excess_score_gap"]=diff.Sharp_score_gap-diff.Baseline_score_gap
            diff["Excess_vorp_gap"]=diff.Sharp_vorp_gap-diff.Baseline_vorp_gap
            diff["Excess_dominance"]=diff.Sharp_dominance-diff.Baseline_dominance

            st.markdown("#### Sharp/value excess fallback gap vs baseline rooms")
            st.dataframe(diff[[
                "Round","Sharp_score_gap","Baseline_score_gap","Excess_score_gap",
                "Sharp_vorp_gap","Baseline_vorp_gap","Excess_vorp_gap",
                "Sharp_dominance","Baseline_dominance","Excess_dominance"
            ]].round(4),use_container_width=True,hide_index=True)

            players=d[d.Room=="Sharp/value"].groupby(
                ["Round","Champion_player","Champion_position","Best_score_player","Best_score_position"],as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Avg_score_gap=("Score_gap_best_minus_champ","mean"),
                Avg_vorp_gap=("VORP_gap_bestscore_minus_champ","mean"),
                Dominance_rate=("Best_score_dominates","mean"),
            ).sort_values(["Decisions","Avg_score_gap"],ascending=[False,False])

            st.markdown("#### Most repeated Sharp/value fallback paths")
            st.dataframe(players.head(40).round(4),use_container_width=True,hide_index=True)

            slot=d[d.Room=="Sharp/value"].groupby(["Draft_slot","Round"],as_index=False).agg(
                Decisions=("Champion_player","count"),
                Avg_score_gap=("Score_gap_best_minus_champ","mean"),
                Avg_vorp_gap=("VORP_gap_bestscore_minus_champ","mean"),
                Dominance_rate=("Best_score_dominates","mean"),
            )
            st.markdown("#### Sharp/value draft slot × round")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            # Interpretation screen:
            # adaptation failure = Sharp has materially larger quality gaps than baseline rooms.
            material=diff[
                (diff.Excess_score_gap>1.0) &
                (diff.Excess_vorp_gap>0.10)
            ]
            if len(material):
                first_round=int(material.sort_values("Round").iloc[0].Round)
                st.warning(
                    f"Potential adaptation failure detected beginning in Round {first_round}: "
                    "better true-board alternatives remain available in Sharp/value rooms at a larger rate/magnitude than in baseline rooms. "
                    "Use that exact round as the next counterfactual target."
                )
            else:
                st.success(
                    "No material excess fallback gap detected. The Sharp/value deficit is more consistent with unavoidable opponent board depletion than with a simple fallback-selection defect."
                )

            st.info(
                "V7.89 changes no production behavior. A challenger is justified only if Sharp/value leaves clearly superior true-board alternatives available more often than baseline rooms. "
                "If not, the performance deficit is likely opponent-driven rather than fixable by a local override."
            )

            st.download_button(
                "⬇️ Download V7.89 R1–4 board-depletion audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_89_r1_4_sharp_value_board_depletion_audit.csv",
                "text/csv",key="v789_download"
            )
