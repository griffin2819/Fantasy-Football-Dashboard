    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                elif bool(p12_te_gap_conditional_enabled) and int(slot)==12 and 4<=round_no<=6:
                    # Frozen V7.48 discriminator: evaluate the normal .90 decision first.
                    _base_sidx,_base_delta=_fast_candidate_deltas(fast,user_idx,eidx,slots,exact_cap=56,te_starter_vorp_mult=.90)
                    _base_edge=(market[_base_sidx]-pno)
                    _base_eval=(0.38*draft_score[_base_sidx]+3.60*_base_delta+0.18*_base_edge)
                    _base_best=int(_base_sidx[int(np.argmax(_base_eval))])
                    _base_pos=str(pos[_base_best])
                    _other_mask=np.asarray([str(pos[int(x)])!=_base_pos for x in _base_sidx],dtype=bool)
                    _faller_gap=-np.inf
                    if np.any(_other_mask):
                        _oi=np.where(_other_mask)[0]
                        _faller_i=int(_oi[int(np.argmax(_base_eval[_oi]))])
                        _faller_gap=float(_base_eval[_faller_i]-_base_eval[int(np.argmax(_base_eval))])
                        _faller_gap=abs(_faller_gap)
                    if _base_pos=="TE" and _faller_gap>=float(p12_te_gap_threshold):
                        _te_util_mult=.30
...
st.title("🏈 Fantasy Edge V7.48 — Frozen TE .30 Conditional Rule Validation")
st.caption("V7.48 • V7.24/.90 champion frozen • 55.97 conditional rule out-of-sample validation")
st.caption("Build ID: V7.48-P12TE30-GAP5597-OOS-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v748_live_slot")
    ...
    if "v748_target_queue" not in st.session_state:
        st.session_state.v748_target_queue={}
    queue=st.session_state.v748_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v748_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.48 uses .90 as the live/default champion. The .30 setting activates only inside the validation candidate when the frozen 55.97 condition is met.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.48 robustness report")
        ...
        st.markdown("### 🧪 V7.48 champion baseline validation")
        st.info("V7.48 introduces no live tuning and does not retune the 55.97 threshold. It is a pure untouched-seed validation of the single V7.47 discriminator.")
        ...
        if "v748_independent_results" not in st.session_state:
            st.session_state.v748_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v748_independent_results"),pd.DataFrame) else "ready to run"))
        if "v748_independent_summary" not in st.session_state:
            st.session_state.v748_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v748_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v748_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v748_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v748_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v748_independent_results=iv
                st.session_state.v748_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v748_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v748_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.48 Champion Baseline Check")
            st.caption("V7.48 keeps V7.24/.90 live and validates the frozen 55.97 Pick-12 R4-6 TE conditional discriminator on a new untouched seed family.")
            ...
            st.download_button(
                "⬇️ Download V7.48 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_48_champion_baseline.csv",
                "text/csv",
                key="v748_baseline_download"
            )
        ...
        st.markdown("### 🧪 V7.48 — Frozen TE .30 Conditional Rule Validation")
        st.caption(
            "Out-of-sample validation only. V7.24/.90 remains live/default. "
            "Frozen rule: Pick 12 + Rounds 4-6 + baseline .90 selects TE + passed-faller evaluation gap ≥ 55.97 → use .30 TE-starter VORP utility. "
            "The 55.97 threshold is locked and is not retuned here."
        )

        if "v748_results" not in st.session_state: st.session_state.v748_results=None
        sims=st.number_input("V7.48 validation simulations per room",min_value=40,max_value=125,value=100,step=5,key="v748_sims",
            help="Default 100 × 5 rooms = 500 fresh matched champion-vs-rule sets.")
        a,b=st.columns(2)
        run=a.button("Run frozen-rule validation",type="primary",key="v748_run")
        clear=b.button("Clear V7.48 validation",key="v748_clear")
        if clear:
            st.session_state.v748_results=None
            st.rerun()
        ...
        if run:
            ...
            for room_name in rooms:
                for j in range(int(sims)):
                    # New seed family: untouched by V7.47 threshold discovery.
                    seed=948000+j*103+sum(ord(c) for c in room_name)
                    ...
                    if done==1 or done%100==0 or done==total:
                        status.caption(f"V7.48 validation: {done}/{total} fresh matched sets")

            df=pd.DataFrame(rows)
            st.session_state.v748_results=df
            status.success(f"V7.48 complete: {len(df)} untouched-seed matched sets.")
            prog.empty()

        df=st.session_state.get("v748_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            behavior=df.Behavior_changed.mean()
            ...
            c3.metric("Rule activation",f"{behavior:.1%}")
            c4.metric("Conditional win",f"{cond:.1%}" if pd.notna(cond) else "n/a")
            ...
            st.markdown("#### Untouched-seed room robustness")
            ...
            st.markdown("#### Frozen-rule certification gate")
            st.dataframe(gate.round(4),use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.48 CERTIFIED. The frozen 55.97 conditional discriminator survived untouched seeds. "
                    "A later promotion build can add this exact rule without changing the threshold."
                )
            else:
                st.warning(
                    "V7.48 FAILED out-of-sample certification. Keep V7.24/.90 frozen and retire the 55.97 conditional rule rather than retuning it."
                )

            st.download_button(
                "⬇️ Download V7.48 frozen-rule validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_48_frozen_te30_conditional_validation.csv",
                "text/csv",key="v748_download"
            )
