...
st.title("🏈 Fantasy Edge V8.04 — R3 QB Opportunity-Cost Production Candidate")
st.caption("V8.04 • R3 QB opportunity-cost ON • production certification")
st.caption("Build ID: V8.04-R3-QB-PRODUCTION-CERT-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 👑 V8.04 — R3 QB Opportunity-Cost Production Certification")
        st.caption(
            "Final production-path certification of the exact validated V8.02/V8.03 Round-3 QB opportunity-cost rule, now ON by default in this candidate build. "
            "The certified Round-2 and Round-7 rules remain ON, the failed Round-4 branch remains OFF, and the protected late-round architecture is unchanged. "
            "No threshold, player, room, slot, or position tuning is introduced."
        )

        if "v804_results" not in st.session_state:
            st.session_state.v804_results=None

        sims=st.number_input(
            "V8.04 certification simulations per slot × room",
            min_value=10,max_value=40,value=15,step=5,key="v804_sims",
            help="Default: 12 slots × 5 rooms × 15 = 900 fresh matched production-path sets. "
                 "Baseline = frozen V7.93 production path with R3 QB rule OFF. Candidate = same engine with exact validated R3 QB rule ON."
        )
        a,b=st.columns(2)
        run=a.button("Run V8.04 production certification",type="primary",key="v804_run")
        clear=b.button("Clear V8.04 certification",key="v804_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8040000+int(ds)*100000+j*421+sum(ord(c) for c in room_name)

                        baseline=simulate_mock_fast(
                        ...
                        bc=independent_draft_grade_components(baseline,int(teams))
                        pc=independent_draft_grade_components(production,int(teams))
                        bp=independent_phase_metrics(baseline).set_index("Phase")
                        pp=independent_phase_metrics(production).set_index("Phase")

                        base_players=baseline.set_index("mock_pick")["player"].to_dict()
                        changed=sum(base_players.get(x.mock_pick,None)!=x.player for _,x in production.iterrows())

                        r3b=baseline[pd.to_numeric(baseline.mock_round,errors="coerce").eq(3)]
                        r3p=production[pd.to_numeric(production.mock_round,errors="coerce").eq(3)]
                        b3=str(r3b.iloc[0].player) if len(r3b) else ""
                        p3=str(r3p.iloc[0].player) if len(r3p) else ""
                        p3pos=str(r3p.iloc[0].position) if len(r3p) else ""

                        overall=pc["overall"]-bc["overall"]
                        r13=float(pp.loc["R1-3","Avg_market_delta"])-float(bp.loc["R1-3","Avg_market_delta"])
                        r46=float(pp.loc["R4-6","Avg_market_delta"])-float(bp.loc["R4-6","Avg_market_delta"])
                        r710=float(pp.loc["R7-10","Avg_market_delta"])-float(bp.loc["R7-10","Avg_market_delta"])
                        r11=float(pp.loc["R11+","Avg_market_delta"])-float(bp.loc["R11+","Avg_market_delta"])

                        rows.append({
                            "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Behavior_changed":changed>0,"Changed_picks":changed,
                            "Baseline_R3_player":b3,"Production_R3_player":p3,"Production_R3_position":p3pos,
                        ...
                        prog.progress(done/total)
                        if done==1 or done%150==0 or done==total:
                            status.caption(f"V8.04 certification: {done}/{total} matched production-path sets")

            df=pd.DataFrame(rows)
            st.session_state.v804_results=df
            status.success(f"V8.04 complete: {len(df)} fresh matched production-path sets.")
            prog.empty()

        df=st.session_state.get("v804_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.markdown("#### Production certification by all 12 slots")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            paths=changed.groupby(
                ["Baseline_R3_player","Production_R3_player","Production_R3_position"],as_index=False
            ).agg(
                Decisions=("Behavior_changed","count"),
                Overall_change=("Overall_change","mean"),
                Win_rate=("Conditional_overall_win","mean"),
            ).sort_values("Decisions",ascending=False) if len(changed) else pd.DataFrame()

            st.markdown("#### Production Round-3 paths")
            st.dataframe(paths.head(40).round(4),use_container_width=True,hide_index=True)

            positive_rooms=int((room.Overall_change>0).sum())
            nonnegative_slots=int((slot.Overall_change>=0).sum())
            ...
            certified=(
                ...
                and nonnegative_slots>=10
            )

            gate=pd.DataFrame([{
                "Production certification passed":certified,
                "Overall > 0":overall>0,
                "R1-3 > 0":r13>0,
                "R4-6 > 0":r46>0,
                "R7-10 > -0.15":r710>-0.15,
                "R11+ > -0.15":r11>-0.15,
                "Activation >=5%":activation>=.05,
                "Conditional win >=60%":pd.notna(cond) and cond>=.60,
                "Conditional non-loss >=65%":pd.notna(nonloss) and nonloss>=.65,
                "Positive rooms":positive_rooms,
                "Non-negative slots":nonnegative_slots,
            }])
            st.markdown("#### V8.04 promotion gate")
            st.dataframe(gate,use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V8.04 PRODUCTION CERTIFICATION PASSED. The exact validated R3 QB opportunity-cost rule is eligible to remain ON by default alongside certified R2 and R7."
                )
            else:
                st.warning(
                    "V8.04 production certification did not pass. Revert the R3 QB opportunity-cost rule to OFF and retain V7.93 as champion. Do not retune this certification sample."
                )

            st.download_button(
                "⬇️ Download V8.04 production certification CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_04_r3_qb_production_path_certification.csv",
                "text/csv",key="v804_download"
            )
