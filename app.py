...
def independent_draft_grade(draft, teams=12):
    """Independent validation: no Fantasy Edge projection, VORP, regression, draft_score, or utility weights."""
    if draft is None or len(draft)==0: return 0.0
    d=draft.copy()
    market=pd.to_numeric(d.get("market_pick",d.get("consensus_rank")),errors="coerce")
    pick=pd.to_numeric(d["mock_pick"],errors="coerce")
    value=(pick-market).clip(-18,18).fillna(0)
    ...
    availability=-8*inj.isin(["ir","pup","nfi","reserve/ir","reserve/pup"]).sum()-4*inj.isin(["out","suspended","susp"]).sum()
    return float(np.clip(70+.70*value.mean()-8*missing+balance+availability,0,100))
...
st.title("🏈 Fantasy Edge V7.6 — Independent Validation Engine")
st.caption("V7.6 • independent validation • multi-room stress testing • V7.5 draft engine frozen")
st.caption("Build ID: V7.6-IVE-20260813")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.6 robustness report")
            ...
            st.download_button("⬇️ Download V7.6 diagnostic CSV",csv,"fantasy_edge_v7_6_diagnostics.csv","text/csv")
            st.markdown("### 🧪 V7.6 independent stress test")
            st.caption("The V7.5 draft engine stays frozen. This grades it with market value, roster legality/balance and availability only.")
            iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v76_slots")
            iv_rooms=st.multiselect("Room profiles",["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"],
                                    default=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"],key="v76_rooms")
            iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v76_sims")
            if st.button("Run independent stress test",type="primary",key="v76_run"):
                rows=[]; total=max(1,len(iv_slots)*len(iv_rooms)*int(iv_sims)); done=0
                prog=st.progress(0); fast_iv=_fast_benchmark_pool(board,int(teams))
                for ds in iv_slots:
                    for room_name in iv_rooms:
                        for j in range(int(iv_sims)):
                            seed=910000+int(ds)*10000+j*31+sum(ord(c) for c in room_name)
                            fe=simulate_mock_fast(fast_iv,int(teams),int(ds),int(rounds),state["roster_slots"],float(randomness),True,seed,room_name)
                            co=simulate_mock_fast(fast_iv,int(teams),int(ds),int(rounds),state["roster_slots"],float(randomness),False,seed,room_name)
                            fg=independent_draft_grade(fe,int(teams)); cg=independent_draft_grade(co,int(teams))
                            rows.append({"Draft_slot":ds,"Room":room_name,"Simulation":j+1,
                                         "Fantasy_Edge_grade":fg,"Consensus_grade":cg,
                                         "Difference":fg-cg,"Fantasy_Edge_win":fg>cg})
                            done+=1; prog.progress(done/total)
                iv=pd.DataFrame(rows)
                if len(iv):
                    a,b,c,d=st.columns(4)
                    a.metric("Independent FE win rate",f"{iv.Fantasy_Edge_win.mean():.0%}")
                    b.metric("Independent FE grade",f"{iv.Fantasy_Edge_grade.mean():.2f}")
                    c.metric("Independent consensus",f"{iv.Consensus_grade.mean():.2f}")
                    d.metric("Independent advantage",f"{iv.Difference.mean():+.2f}")
                    stress=iv.groupby(["Draft_slot","Room"]).agg(FE_grade=("Fantasy_Edge_grade","mean"),
                        Consensus_grade=("Consensus_grade","mean"),Advantage=("Difference","mean"),
                        FE_win_rate=("Fantasy_Edge_win","mean")).reset_index()
                    ...
                    st.download_button("⬇️ Download V7.6 independent validation CSV",
                        iv.to_csv(index=False).encode(),"fantasy_edge_v7_6_independent_validation.csv","text/csv")
