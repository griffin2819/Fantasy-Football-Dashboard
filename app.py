...
st.set_page_config(page_title="Fantasy Edge v9.46.1 — Exact League + Kicker Requirement", page_icon="🏈", layout="wide")
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    x["idp_impact_score"]=pd.to_numeric(pd.Series(impacts,index=x.index),errors="coerce")
    x["idp_external_rank"]=pd.to_numeric(pd.Series(ext_ranks,index=x.index),errors="coerce")
    # v9.46.1: nullable boolean prevents pandas Int/Float casting errors on fallback assignment.
    x["idp_eligible"]=pd.Series(
        [pd.NA if pd.isna(v) else bool(v) for v in eligibles],
        index=x.index,
        dtype="boolean"
    )
    _overlay=x.get("id",pd.Series("",index=x.index)).astype(str).str.startswith("v938:")
    ...
    # v9.46 same-position quality dominance.
    ...
    for _p in ("QB","RB","WR","TE"):
        _inds=x.index[x["position"].eq(_p)]
        if len(_inds)<2: continue
        for _i in _inds:
            _pi,_vi=_proj_all.loc[_i],_vorp_all.loc[_i]
            if pd.isna(_pi) or pd.isna(_vi): continue
            _better=[]
            for _j in _inds:
                if _j==_i: continue
                _pj,_vj=_proj_all.loc[_j],_vorp_all.loc[_j]
                if pd.notna(_pj) and pd.notna(_vj) and _pj>=_pi and _vj>=_vi and ((_pj-_pi)>=0.35 or (_vj-_vi)>=0.35):
                    _better.append(_j)
            if _better:
                _gap=max((_proj_all.loc[_j]-_pi)*1.25+(_vorp_all.loc[_j]-_vi)*2.0 for _j in _better)
                x.at[_i,"same_pos_dominated"]=True
                x.at[_i,"same_pos_quality_gap"]=_gap
                x.at[_i,"evaluation_score"]-=min(30.0,12.0+_gap)
    ...
    if len(_good_fit):
        ...
        _badfit=(_ropp_final<=-5) & (pd.to_numeric(x["evaluation_score"],errors="coerce")<=_best_good+8.0)
        x.loc[_badfit,"evaluation_score"]-=18.0
...
st.caption("Build ID: FE-V9.46.1-IDP-BOOLEAN-DTYPE-HOTFIX-20260821")
...
with tabs[2]:
    ...
    st.success("🔒 v9.44.2 SAME Draft/Mock • TE + K required • 🧠 honest IDP-relative metrics • 🐍 12-team, 17-round snake")
    ...
    st.caption("V9.08 remains the frozen certified core; V9.44.2 applies exact 1QB/2RB/2WR/1TE/2FLEX/1K/1DL/1DB/6-bench construction (17 rounds), availability, injury, and impact-IDP overlays without changing the frozen core.")
            ...
            if owner==int(slot):
                ...
                if not pick_options:
                    ...
                    if st.button("Draft this player"):
                        ...
                        _commit,_=v939_shared_candidate_rank(
                            _commit,roster,rnd,overall,next_user_pick(overall,int(slot),int(teams),int(rounds)),
                            state["roster_slots"],randomness
                        )
