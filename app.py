...
def _single_authority_strategy(y,roster,slots,round_no,current_pick,next_pick):
    ...
    _market_value_component=np.where(
        _market_delta>=0.0,
        np.minimum(_market_delta,30.0)*0.20,
        np.maximum(_market_delta,-28.0)*0.32
    )
    ...
    for _,r in x.iterrows():
        ...
        if pos in ["DL","DB"] and idp_count>=2:
            ...
            if offense_depth_thin:
                adj -= 14.0
                why.append("IDP3 vs offensive depth")
            elif rnd<=12:
                adj -= 10.0
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        target_sims=100
        _benchmark_build="FE-V6.7-ADAPTIVE-VALUE-20260826"
