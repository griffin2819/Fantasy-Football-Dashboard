    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                _te_util_mult=(float(p12_te_starter_vorp_mult) if bool(p12_te_utility_dedup_enabled) and int(slot)==12 and 4<=round_no<=6 else .90)
...
st.title("🏈 Fantasy Edge V7.47 — TE .30 Win/Loss Discriminator Audit")
st.caption("V7.47 • V7.24/.90 champion restored • TE .30 win/loss discriminator audit")
st.caption("Build ID: V7.47-P12TE30-WLDA-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v747_live_slot")
    ...
    if "v747_target_queue" not in st.session_state:
        st.session_state.v747_target_queue={}
    queue=st.session_state.v747_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v747_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.47 uses .90 as the live/default champion. The .30 setting appears only inside the diagnostic win/loss audit.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.47 robustness report")
        ...
        st.markdown("### 🧪 V7.47 champion baseline validation")
        st.info("V7.47 introduces no live tuning. It compares winning and losing .30 interventions and screens pre-draft features for a clean conditional discriminator.")
        ...
        if "v747_independent_results" not in st.session_state:
            st.session_state.v747_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v747_independent_results"),pd.DataFrame) else "ready to run"))
        if "v747_independent_summary" not in st.session_state:
            st.session_state.v747_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v747_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v747_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v747_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v747_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v747_independent_results=iv
                st.session_state.v747_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v747_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v747_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.47 Champion Baseline Check")
            st.caption("V7.47 restores the V7.24/.90 live champion and diagnoses only the fresh matched drafts where the rejected .30 Pick-12 R4-6 TE utility setting changes behavior.")
            ...
            st.download_button(
                "⬇️ Download V7.47 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_47_champion_baseline.csv",
                "text/csv",
                key="v747_baseline_download"
            )
        ...
        st.markdown("### 🔬 V7.47 — Pick 12 TE .30 Win/Loss Discriminator Audit")
        st.caption(
            "Diagnostic only. The live/default champion is reverted to V7.24/.90. "
            "This audit reruns .90 vs .30 on fresh matched seeds, isolates only drafts where behavior changes, "
            "and compares winning versus losing .30 interventions to look for a clean pre-draft discriminator."
        )

        if "v747_results" not in st.session_state: st.session_state.v747_results=None

        sims=st.number_input("Win/loss audit simulations per room",min_value=30,max_value=100,value=75,step=5,key="v747_sims")
        a,b=st.columns(2)
        run=a.button("Run V7.47 win/loss discriminator audit",type="primary",key="v747_run")
        clear=b.button("Clear V7.47 audit",key="v747_clear")
        if clear:
            st.session_state.v747_results=None
            st.rerun()

        if run:
            rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]
            fast_champ=_fast_benchmark_pool(board,int(teams))
            rows=[]; total=len(rooms)*int(sims); done=0
            prog=st.progress(0); status=st.empty()

            for room_name in rooms:
                for j in range(int(sims)):
                    seed=947000+j*101+sum(ord(c) for c in room_name)

                    oldc=simulate_mock_fast(
                        fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,
                        p12_te_utility_dedup_enabled=True,p12_te_starter_vorp_mult=.90
                    )
                    newc=simulate_mock_fast(
                        fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,
                        p12_te_utility_dedup_enabled=True,p12_te_starter_vorp_mult=.30
                    )

                    oc=independent_draft_grade_components(oldc,int(teams))
                    nc=independent_draft_grade_components(newc,int(teams))
                    op=independent_phase_metrics(oldc).set_index("Phase")
                    np_=independent_phase_metrics(newc).set_index("Phase")

                    old_by_pick=oldc.set_index("mock_pick")
                    new_by_pick=newc.set_index("mock_pick")
                    changed_picks=[
                        int(p) for p in new_by_pick.index
                        if p in old_by_pick.index and str(new_by_pick.loc[p,"player"])!=str(old_by_pick.loc[p,"player"])
                    ]
                    if changed_picks:
                        first_pick=min(changed_picks)
                        nr=new_by_pick.loc[first_pick]
                        orow=old_by_pick.loc[first_pick]

                        # Pull pre-existing R4-6 diagnostic values from the old-champion decision.
                        rows.append({
                            "Room":room_name,"Simulation":j+1,"Seed":seed,
                            "First_changed_pick":first_pick,
                            "First_changed_round":int(nr.get("mock_round",0)),
                            "Old_player":orow.get("player",""),
                            "Old_position":orow.get("position",""),
                            "New_player":nr.get("player",""),
                            "New_position":nr.get("position",""),
                            "Changed_picks_total":len(changed_picks),
                            "Overall_change":nc["overall"]-oc["overall"],
                            "Outcome":"WIN" if nc["overall"]>oc["overall"] else ("LOSS" if nc["overall"]<oc["overall"] else "TIE"),
                            "R4_6_change":float(np_.loc["R4-6","Avg_market_delta"])-float(op.loc["R4-6","Avg_market_delta"]),
                            "R7_10_change":float(np_.loc["R7-10","Avg_market_delta"])-float(op.loc["R7-10","Avg_market_delta"]),
                            "R11_plus_change":float(np_.loc["R11+","Avg_market_delta"])-float(op.loc["R11+","Avg_market_delta"]),
                            "Old_selected_reach":orow.get("MR_Selected_reach",np.nan),
                            "Old_selected_market_pick":orow.get("MR_Selected_market_pick",np.nan),
                            "Passed_faller_player":orow.get("MR_Passed_faller_player",""),
                            "Passed_faller_position":orow.get("MR_Passed_faller_position",""),
                            "Passed_faller_amount":orow.get("MR_Passed_faller_amount",np.nan),
                            "Passed_faller_eval_gap":orow.get("MR_Passed_faller_eval_gap",np.nan),
                            "Passed_faller_survival_est":orow.get("MR_Passed_faller_survival_est",np.nan),
                            "Passed_faller_survived_actual":orow.get("MR_Passed_faller_survived_actual",np.nan),
                            "Draftscore_gap":orow.get("MR42_Faller_draftscore_gap",np.nan),
                            "Rosterutility_FE_gap":orow.get("MR42_Faller_rosterutility_gap",np.nan),
                            "Marketedge_gap":orow.get("MR42_Faller_marketedge_gap",np.nan),
                            "VORP_FE_gap":orow.get("MR42_Faller_vorp_gap",np.nan),
                            "Projection_FE_gap":orow.get("MR42_Faller_projection_gap",np.nan),
                            "TE_role":orow.get("MR43_Selected_role",""),
                            "Faller_role":orow.get("MR43_Faller_role",""),
                            "TE_utility_delta":orow.get("MR43_Selected_utility_delta",np.nan),
                            "Faller_utility_delta":orow.get("MR43_Faller_utility_delta",np.nan),
                            "Raw_utility_gap":orow.get("MR43_Utility_delta_gap",np.nan),
                            "TE_replacement_ppg":orow.get("MR43_Selected_replacement_ppg",np.nan),
                            "Faller_replacement_ppg":orow.get("MR43_Faller_replacement_ppg",np.nan),
                            "Replacement_gap_TE_minus_faller":orow.get("MR43_Replacement_gap",np.nan),
                            "TE_direct_slot_missing":orow.get("MR43_Selected_direct_missing",np.nan),
                            "Faller_direct_slot_missing":orow.get("MR43_Faller_direct_missing",np.nan),
                            "QB_before":orow.get("MR43_QB_before",np.nan),
                            "RB_before":orow.get("MR43_RB_before",np.nan),
                            "WR_before":orow.get("MR43_WR_before",np.nan),
                            "TE_before":orow.get("MR43_TE_before",np.nan),
                        })

                    done+=1; prog.progress(done/total)
                    if done==1 or done%75==0 or done==total:
                        status.caption(f"V7.47 audit: {done}/{total} matched sets")

            df=pd.DataFrame(rows)
            st.session_state.v747_results=df
            status.success(f"V7.47 complete: {len(df)} behavior-changing matched drafts.")
            prog.empty()

        df=st.session_state.get("v747_results")
        if isinstance(df,pd.DataFrame) and len(df):
            wins=df[df.Outcome.eq("WIN")].copy()
            losses=df[df.Outcome.eq("LOSS")].copy()
            ties=df[df.Outcome.eq("TIE")].copy()

            c1,c2,c3,c4=st.columns(4)
            c1.metric("Behavior-changing drafts",str(len(df)))
            c2.metric("Wins",f"{len(wins)} ({len(wins)/len(df):.1%})")
            c3.metric("Losses",f"{len(losses)} ({len(losses)/len(df):.1%})")
            c4.metric("Mean overall change",f"{df.Overall_change.mean():+.3f}")

            numeric=[
                "Passed_faller_amount","Passed_faller_eval_gap","Passed_faller_survival_est",
                "Draftscore_gap","Rosterutility_FE_gap","Marketedge_gap","VORP_FE_gap",
                "Projection_FE_gap","TE_utility_delta","Faller_utility_delta","Raw_utility_gap",
                "TE_replacement_ppg","Faller_replacement_ppg","Replacement_gap_TE_minus_faller",
                "Old_selected_reach","Changed_picks_total","R4_6_change","R7_10_change","R11_plus_change"
            ]
            compare=[]
            for col in numeric:
                if col in df.columns:
                    w=pd.to_numeric(wins[col],errors="coerce").mean() if len(wins) else np.nan
                    l=pd.to_numeric(losses[col],errors="coerce").mean() if len(losses) else np.nan
                    compare.append({"Feature":col,"WIN mean":w,"LOSS mean":l,"WIN−LOSS":w-l})
            comp=pd.DataFrame(compare)
            st.markdown("#### Winning vs losing .30 interventions")
            st.dataframe(comp.reindex(comp["WIN−LOSS"].abs().sort_values(ascending=False).index).round(3),use_container_width=True,hide_index=True)

            st.markdown("#### Outcomes by room")
            room=pd.crosstab(df.Room,df.Outcome,normalize="index").reset_index()
            st.dataframe(room.round(3),use_container_width=True,hide_index=True)

            # Simple one-dimensional threshold screen using only pre-draft features.
            feature_cols=[
                "Passed_faller_amount","Passed_faller_eval_gap","Passed_faller_survival_est",
                "Draftscore_gap","Rosterutility_FE_gap","Marketedge_gap","VORP_FE_gap",
                "Projection_FE_gap","Raw_utility_gap","Replacement_gap_TE_minus_faller","Old_selected_reach"
            ]
            screen=[]
            labeled=df[df.Outcome.isin(["WIN","LOSS"])].copy()
            labeled["WinFlag"]=labeled.Outcome.eq("WIN")
            for col in feature_cols:
                x=pd.to_numeric(labeled[col],errors="coerce")
                valid=x.notna()
                if valid.sum()<20: continue
                vals=np.unique(np.nanquantile(x[valid],[.20,.30,.40,.50,.60,.70,.80]))
                for t in vals:
                    for direction in [">=","<="]:
                        mask=(x>=t) if direction==">=" else (x<=t)
                        n=int(mask.sum())
                        if n<10: continue
                        wr=float(labeled.loc[mask,"WinFlag"].mean())
                        coverage=n/len(labeled)
                        mean_gain=float(labeled.loc[mask,"Overall_change"].mean())
                        screen.append({"Feature":col,"Rule":f"{direction} {t:.3f}","N":n,"Coverage":coverage,"Win_rate":wr,"Mean_overall_change":mean_gain})
            scr=pd.DataFrame(screen)
            if len(scr):
                scr["Score"]=(scr.Win_rate-.5)*scr.Coverage + .25*np.maximum(scr.Mean_overall_change,0)
                scr=scr.sort_values(["Score","Win_rate","Coverage"],ascending=False)
                st.markdown("#### Pre-draft one-feature discriminator screen")
                st.dataframe(scr.head(20).round(3),use_container_width=True,hide_index=True)

                best=scr.iloc[0]
                if best.Win_rate>=.65 and best.Coverage>=.30 and best.Mean_overall_change>0:
                    st.success(
                        f"Candidate discriminator found: {best.Feature} {best.Rule}. "
                        f"Among behavior-changing WIN/LOSS cases it covers {best.Coverage:.0%}, wins {best.Win_rate:.0%}, "
                        f"and averages {best.Mean_overall_change:+.3f} overall. V7.48 should validate this exact condition on untouched seeds."
                    )
                else:
                    st.warning(
                        "No strong one-feature pre-draft discriminator cleared the screening bar. "
                        "Do not tune a conditional .30 rule from weak separation; keep V7.24/.90 frozen."
                    )

            st.download_button(
                "⬇️ Download V7.47 behavior-change win/loss audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_47_te30_behavior_change_win_loss_audit.csv",
                "text/csv",key="v747_download"
            )
        else:
            st.info("Run V7.47 to generate fresh behavior-changing .90 vs .30 matched cases.")
