...
def faller_intercept_choice(eval_score, market_pick, current_pick, normal_choice,
                            min_fall=10, model_band=16):
    """Optional value override for a model-compatible market faller.

    This does NOT blindly draft ADP. A faller must:
    - be at least `min_fall` picks past market expectation,
    - rank inside the model's top `model_band` candidates at this pick, and
    - beat the normal execution choice on a conservative evaluation+fall score.

    Returns (choice_index, intercepted_bool, fall_picks).
    """
    ev=np.asarray(eval_score,dtype=float)
    mp=np.asarray(market_pick,dtype=float)
    ...
    if not np.any(valid):
        return int(normal_choice),False,float(fall[int(normal_choice)]) if np.isfinite(fall[int(normal_choice)]) else 0.0

    n=min(int(model_band),len(ev))
    top=np.argpartition(-ev,n-1)[:n] if n<len(ev) else np.arange(len(ev))
    cand=top[valid[top]]
    if len(cand)==0:
        return int(normal_choice),False,float(fall[int(normal_choice)]) if np.isfinite(fall[int(normal_choice)]) else 0.0

    # Normalize evaluation inside the model-compatible band so fall value can matter
    # without turning the system into an ADP-only drafter.
    ev_top=ev[top]
    spread=max(float(np.nanstd(ev_top)),1.0)
    best_eval=float(np.nanmax(ev_top))
    compatibility=(ev[cand]-best_eval)/spread  # <= 0; closer to 0 is more model-compatible
    intercept_score=.38*fall[cand] + 2.0*compatibility
    ...
    # Require a meaningful market-value upgrade; otherwise preserve normal execution.
    if float(fall[best_cand]) >= max(float(min_fall),normal_fall+6):
        return best_cand,True,float(fall[best_cand])
    return normal,False,normal_fall


def is_turn_pair(current_pick, slot, teams, rounds):
    """True when the user's next snake pick is immediately after this one."""
    nxt=next_user_pick(current_pick,int(slot),int(teams),int(rounds))
    return nxt is not None and int(nxt)==int(current_pick)+1


def turn_pair_choice_fast(fast, selected, candidates, eval_score, current_pick, slots,
                          max_candidates=14):
    """Plan two adjacent turn picks together.

    The V7.10 player evaluations remain intact. Pair planning evaluates the
    combined final-roster utility of two model-compatible candidates and adds a
    modest bonus for real market fallers. It does not force positions.
    """
    cand=np.asarray(candidates,dtype=int)
    ev=np.asarray(eval_score,dtype=float)
    if cand.size<2:
        return (0, None) if cand.size else (None,None)

    n=min(int(max_candidates),cand.size)
    top=np.argpartition(-ev,n-1)[:n] if n<cand.size else np.arange(cand.size)
    market=fast["market_pick"][cand[top]]
    fall=np.maximum(float(current_pick)-market,0)
    reach=np.maximum(market-float(current_pick),0)

    base_utility=_fast_roster_utility(fast,selected,slots)
    best_score=-1e18
    best_pair=(int(top[0]),int(top[1] if len(top)>1 else top[0]))

    # Pair search is tiny (<= 91 combinations).
    for a in range(len(top)):
        for b in range(a+1,len(top)):
            ia=int(top[a]); ib=int(top[b])
            ga=int(cand[ia]); gb=int(cand[ib])
            pair_utility=_fast_roster_utility(fast,selected+[ga,gb],slots)-base_utility
            # Model evaluation dominates; combined roster utility handles positional synergy.
            # Faller value matters, while modest reaches are tolerated at the turn because
            # there is no opponent pick between the two selections.
            pair_score=(ev[ia]+ev[ib] + 1.65*pair_utility
                        +.20*(fall[a]+fall[b]) -.06*(reach[a]+reach[b]))
            if pair_score>best_score:
                best_score=pair_score
                best_pair=(ia,ib)

    # Order the pair: take the larger faller/earlier market player first.
    ia,ib=best_pair
    ma=fast["market_pick"][cand[ia]]; mb=fast["market_pick"][cand[ib]]
    fa=max(float(current_pick)-float(ma),0) if np.isfinite(ma) else 0
    fb=max(float(current_pick)-float(mb),0) if np.isfinite(mb) else 0
    if fb>fa+1 or (abs(fb-fa)<=1 and np.isfinite(mb) and np.isfinite(ma) and mb<ma):
        ia,ib=ib,ia
    return int(ia),int(ib)


def turn_pair_plan_df(roster, candidates, current_pick, slots, max_candidates=12):
    """DataFrame version used by Live Draft Mode and Interactive Mock."""
    if candidates is None or len(candidates)<2:
        return None
    c=candidates.copy().reset_index(drop=False).rename(columns={"index":"_orig_index"})
    if "evaluation_score" not in c.columns:
        return None
    c=c.sort_values("evaluation_score",ascending=False).head(int(max_candidates)).reset_index(drop=True)
    base=_small_roster_utility_df(roster,slots)
    best=None; best_score=-1e18

    for a in range(len(c)):
        for b in range(a+1,len(c)):
            ra=c.iloc[a]; rb=c.iloc[b]
            pair=pd.DataFrame([ra,rb])
            temp=pd.concat([roster,pair],ignore_index=True) if len(roster) else pair.copy()
            utility=_small_roster_utility_df(temp,slots)-base
            ma=ra.get("market_pick",np.nan); mb=rb.get("market_pick",np.nan)
            fa=max(float(current_pick)-float(ma),0) if pd.notna(ma) else 0
            fb=max(float(current_pick)-float(mb),0) if pd.notna(mb) else 0
            rca=max(float(ma)-float(current_pick),0) if pd.notna(ma) else 0
            rcb=max(float(mb)-float(current_pick),0) if pd.notna(mb) else 0
            score=(float(ra.evaluation_score)+float(rb.evaluation_score)+1.65*utility
                   +.20*(fa+fb)-.06*(rca+rcb))
            if score>best_score:
                best_score=score
                best=(ra,rb,fa,fb)
    ...
    if best is None:
        return None
    ra,rb,fa,fb=best
    # More valuable faller first; otherwise earlier market pick first.
    if fb>fa+1 or (abs(fb-fa)<=1 and pd.notna(rb.market_pick) and pd.notna(ra.market_pick)
                   and float(rb.market_pick)<float(ra.market_pick)):
        ra,rb=rb,ra
    return {
        "first":ra,
        "second":rb,
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None, room_profile="Consensus"):
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                # If the previous turn pick planned this exact second selection, use it
                # unless roster legality or availability changed unexpectedly.
                if pending_turn_choice is not None and int(pending_turn_choice) in set(eidx.tolist()):
                    chosen=int(pending_turn_choice)
                    pending_turn_choice=None
                else:
                    pending_turn_choice=None
                    ...
                    # V7.9 PLAYER EVALUATION remains unchanged.
                    ...
                    # V7.11 TURN-AWARE PAIR PLANNING: when we own the immediately
                    # following pick, optimize the two-player combination together.
                    if nxt is not None and int(nxt)==int(overall)+1 and len(sidx)>=2:
                        first_local,second_local=turn_pair_choice_fast(
                            fast,user_idx,sidx,eval_score,overall,slots,max_candidates=14
                        )
                        chosen=int(sidx[int(first_local)])
                        pending_turn_choice=int(sidx[int(second_local)]) if second_local is not None else None
                    else:
                        # V7.10 normal execution + faller intercept stays frozen.
                        local_choice,_,_=execution_choice(eval_score,cr,overall,nxt,randomness)
                        local_choice,_,_=faller_intercept_choice(
                            eval_score,cr,overall,local_choice,min_fall=10,model_band=16
                        )
...
st.title("🏈 Fantasy Edge V7.11 — Turn-Aware Pair Planning")
st.caption("V7.11 • turn-aware pair planning • Evaluation + Execution + Faller Intercept retained")
st.caption("Build ID: V7.11-TAPP-20260813")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v711_live_slot")
    ...
    pair_plan=None
    if live_next is not None and int(live_next)==int(current_overall)+1:
        pair_plan=turn_pair_plan_df(myb,x,current_overall,slots,max_candidates=12)
        if pair_plan is not None:
            p1=pair_plan["first"]; p2=pair_plan["second"]
            after_pair=next_user_pick(int(live_next),int(live_slot),int(teams),18)
            wait_picks=(after_pair-int(live_next)-1) if after_pair is not None else 0
            st.markdown("### 🔁 Next Two Picks — Turn Plan")
            st.success(f"1. {p1.player} ({p1.position})  →  2. {p2.player} ({p2.position})")
            st.caption(f"Plan both adjacent picks together. After pick #{live_next}, approximately {wait_picks} other selections occur before your next turn.")
    ...
    if pair_plan is not None:
        p1=pair_plan["first"]; p2=pair_plan["second"]
        p1_fall=max(current_overall-float(p1.market_pick),0) if pd.notna(p1.market_pick) else 0
        prefix="💎" if p1_fall>=10 else "✅"
        st.success(f"{prefix} DRAFT NOW — TURN PICK 1: {p1.player} ({p1.position}, {p1.team})")
        st.caption(f"Pick #{live_next} plan: {p2.player} ({p2.position}). Pair planning overrides isolated single-pick execution only at adjacent snake turns.")
    elif intercepted:
        ...
        st.caption(f"Override is allowed because the player remains inside Fantasy Edge's top model-compatible target band. Model target #{int(fi.evaluation_rank)} • market #{int(round(fi.market_pick)) if pd.notna(fi.market_pick) else '—'}")
    elif len(draft_now):
        dn=draft_now.iloc[0]
    ...
    if "v711_target_queue" not in st.session_state:
        st.session_state.v711_target_queue={}
    queue=st.session_state.v711_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v711_target_queue=queue
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                pair_plan=turn_pair_plan_df(roster,avail,overall,state["roster_slots"],max_candidates=12) if (nxt is not None and int(nxt)==int(overall)+1) else None
                if pair_plan is not None:
                    pp1=pair_plan["first"]; pp2=pair_plan["second"]
                    st.markdown("### 🔁 Next Two Picks — Turn Plan")
                    st.success(f"1. {pp1.player} ({pp1.position})  →  2. {pp2.player} ({pp2.position})")
                ...
                if pair_plan is not None:
                    dn=pair_plan["first"]
                    st.success(f"✅ DRAFT THIS PICK — TURN PLAN: {dn.player}")
                    st.caption(f"Then target {pair_plan['second'].player} with your immediately adjacent pick #{nxt}.")
                elif intercepted:
                ...
                if pair_plan is not None:
                    pair_name=pair_plan["first"].player
                    pick_options=[pair_name]+[p for p in pick_options if p!=pair_name]
                elif intercepted:
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.11 robustness report")
            ...
            st.download_button("⬇️ Download V7.11 diagnostic CSV",csv,"fantasy_edge_v7_11_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.11 independent validation")
        ...
        if "v711_independent_results" not in st.session_state:
            st.session_state.v711_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v711_independent_results"),pd.DataFrame) else "ready to run"))
        if "v711_independent_summary" not in st.session_state:
            st.session_state.v711_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v711_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v711_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v711_run")
        clear_iv=clear_col.button("Clear independent results",key="v711_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v711_independent_results=iv
                st.session_state.v711_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v711_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v711_independent_summary") or {}
            st.success(
            ...
            by_slot=iv_saved.groupby("Draft_slot").agg(
                ...
                Starter_gap=("Starter_completion_difference","mean"),Balance_gap=("Roster_balance_difference","mean"),
                Availability_gap=("Availability_difference","mean"),FE_avg_reach=("FE_avg_reach","mean"),
                FE_avg_faller=("FE_avg_faller","mean"),FE_big_reaches=("FE_big_reaches","mean")).reset_index()
            st.markdown("#### Failure components by draft slot")
            st.dataframe(by_slot.round(2),use_container_width=True,hide_index=True)

            edge_slots={1,int(teams)}
            edge_mask=iv_saved.Draft_slot.isin(edge_slots)
            if edge_mask.any() and (~edge_mask).any():
                ta,tb=st.columns(2)
                ta.metric("Turn-slot independent gap",f"{iv_saved.loc[edge_mask,'Difference'].mean():+.2f}")
                tb.metric("Non-turn-slot independent gap",f"{iv_saved.loc[~edge_mask,'Difference'].mean():+.2f}")
            ...
            st.download_button(
                "⬇️ Download V7.11 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_11_independent_validation.csv",
                "text/csv",
                key="v711_download"
            )
