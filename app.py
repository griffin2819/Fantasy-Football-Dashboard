...
def market_timing_state(market_pick,current_pick,next_pick,randomness=12):
    """User-facing action label: draft now, wait, or capture a market faller."""
    ...
    reach=mp-cur
    if next_pick is not None and reach>=8 and pd.notna(surv) and surv>=.58:
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                # V7.8 market-aware timing: liking a player does not mean drafting him too early.
                nxt=next_user_pick(overall,int(slot),int(teams),int(rounds))
                survive=survival_probability_array(cr,nxt,randomness)
                wait_cost=survive*np.minimum(reach,36.0)*0.34
                urgency=(1.0-survive)*np.minimum(reach,24.0)*0.06

                score=(0.38*draft_score[sidx] + 3.60*delta + 0.18*edge
                       -0.11*reach - wait_cost + urgency + 0.045*fall)
                score-=np.where(injury[sidx]==2,12.0,np.where(injury[sidx]==1,2.5,0.0))
                # Confidence-scaled near-tie variation remains only in simulated draft rooms.
...
st.title("🏈 Fantasy Edge V7.8 — Market-Aware Pick Timing")
st.caption("V7.8 • market-aware pick timing • DRAFT NOW / WAIT / VALUE FALLER • independent validator retained")
st.caption("Build ID: V7.8-MAPT-20260813")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v78_live_slot")
    ...
    survive=survival_probability_array(cr.to_numpy(float),live_next,state.get("mock",{}).get("randomness",6))
    wait_cost=survive*np.minimum(reach,36.0)*.34
    urgency=(1-survive)*np.minimum(reach,24.0)*.06
    x["live_score"]=(.38*x.draft_score+3.60*live_delta+.18*edge-.11*reach-wait_cost+urgency+.045*fall)
    ...
    top=x.head(5).copy()
    if len(top):
        actions=[market_timing_state(mp,current_overall,live_next,state.get("mock",{}).get("randomness",6)) for mp in top.market_pick]
        top["Action"]=[a[0] for a in actions]
        top["Survival"]=[a[1] for a in actions]
        best=top.iloc[0]
        if best.Action=="VALUE FALLER":
            st.success(f"💎 VALUE FALLER: {best.player} ({best.position}, {best.team})")
        elif best.Action=="WAIT / TARGET NEXT PICK":
            st.warning(f"⏳ WAIT / TARGET NEXT PICK: {best.player} ({best.position}, {best.team})")
        else:
            st.success(f"✅ DRAFT NOW: {best.player} ({best.position}, {best.team})")
        market_txt="—" if pd.isna(best.market_pick) else f"#{int(round(best.market_pick))}"
        surv_txt="—" if pd.isna(best.Survival) else f"{best.Survival:.0%}"
        st.caption(f"Current pick #{current_overall} • next pick {('#'+str(live_next)) if live_next else '—'} • market {market_txt} • chance available next pick {surv_txt} • {best.profile}")
        cols=st.columns(min(3,len(top)))
        for i,(_,r) in enumerate(top.head(3).iterrows()):
            with cols[i]:
                st.metric(f"#{i+1} {r.player}",r.Action,f"{r.projection:.1f} PPG")
                mp="—" if pd.isna(r.market_pick) else f"Market #{int(round(r.market_pick))}"
                sp="—" if pd.isna(r.Survival) else f"{r.Survival:.0%} survives"
                st.caption(f"{mp} • {sp} • {r.profile}")
    ...
    st.dataframe(show[["player","position","team","Action","Market pick","Survives next","projection","vorp","model_rank","Profile","Confidence","injury"]].head(120),use_container_width=True,hide_index=True)
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                nxt=next_user_pick(overall,int(slot),int(teams),int(rounds))
                survive=survival_probability_array(cr.to_numpy(float),nxt,randomness)
                wait_cost=survive*np.minimum(reach,36.0)*.34
                urgency=(1-survive)*np.minimum(reach,24.0)*.06
                avail["live"]=(.38*avail.draft_score+3.60*delta+.18*edge-.11*reach-wait_cost+urgency+.045*fall)
                ...
                best=top.iloc[0]
                if best.Action=="VALUE FALLER":
                    st.success(f"💎 VALUE FALLER: {best.player}")
                elif best.Action=="WAIT / TARGET NEXT PICK":
                    st.warning(f"⏳ WAIT / TARGET NEXT PICK: {best.player}")
                else:
                    st.success(f"✅ DRAFT NOW: {best.player}")
                if nxt:
                    st.caption(f"Current pick #{overall} • next pick #{nxt}. Timing uses normalized market pick and estimated survival probability.")
                st.dataframe(top[["player","position","team","Action","Market pick","Survives to next pick","projection","vorp","model_rank","profile"]],use_container_width=True,hide_index=True)
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.8 robustness report")
            ...
            st.download_button("⬇️ Download V7.8 diagnostic CSV",csv,"fantasy_edge_v7_8_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.8 independent validation")
        ...
        if "v78_independent_results" not in st.session_state:
            st.session_state.v78_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v78_independent_results"),pd.DataFrame) else "ready to run"))
        if "v78_independent_summary" not in st.session_state:
            st.session_state.v78_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v78_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v78_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v78_run")
        clear_iv=clear_col.button("Clear independent results",key="v78_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v78_independent_results=iv
                st.session_state.v78_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v78_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v78_independent_summary") or {}
            st.success(
            ...
            st.download_button(
                "⬇️ Download V7.8 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_8_independent_validation.csv",
                "text/csv",
                key="v78_download"
            )
