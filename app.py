...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Scoring Integrity", page_icon="🏈", layout="wide")
st.markdown("""
<style>
        ...
        if _my_roster.empty:
            ...
            if _moves_df.empty:
                ...
                with st.expander("Advanced diagnostics",expanded=False):
                    ...
                    # V7.93: expose the actual refresh handoff so a failed provider can
                    # be identified immediately instead of masquerading as generic 0% coverage.
                    ...
                    st.dataframe(pd.DataFrame(_v817_score_rows),use_container_width=True,hide_index=True)
                    if isinstance(_base_starters,pd.DataFrame) and len(_base_starters):
                        _v817_cols=[c for c in ["Slot","Player","Pos","Projected PPG","Canonical PPG","Adjusted PPG","Status"] if c in _base_starters.columns]
                        st.dataframe(_base_starters[_v817_cols],use_container_width=True,hide_index=True)
                    if _v737_active_emergencies:
