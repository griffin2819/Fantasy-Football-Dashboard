...
st.title("🏈 Fantasy Edge V7.51 — R11+ Architecture Audit")
st.caption("V7.51 • V7.50 champion frozen • R11+ architecture diagnostic")
st.caption("Build ID: V7.51-R11ARCH-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v751_live_slot")
    ...
    if "v751_target_queue" not in st.session_state:
        st.session_state.v751_target_queue={}
    queue=st.session_state.v751_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v751_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.51 keeps V7.50 as the live/default champion with the 55.97 rule unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.51 robustness report")
        ...
        st.markdown("### 🧪 V7.51 champion baseline validation")
        st.info("V7.51 is diagnostic only. No live weight, execution, position, or threshold changes are introduced.")
        ...
        if "v751_independent_results" not in st.session_state:
            st.session_state.v751_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v751_independent_results"),pd.DataFrame) else "ready to run"))
        if "v751_independent_summary" not in st.session_state:
            st.session_state.v751_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v751_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v751_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v751_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v751_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v751_independent_results=iv
                st.session_state.v751_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v751_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v751_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.51 Champion Baseline Check")
            st.caption("V7.51 keeps the V7.50 champion fully frozen and audits the unresolved R11+ late-draft architecture.")
            ...
            st.download_button(
                "⬇️ Download V7.51 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_51_champion_baseline.csv",
                "text/csv",
                key="v751_baseline_download"
            )
        ...
        st.markdown("### 🔬 V7.51 — R11+ Late-Draft Architecture Audit")
        st.caption(
            "Diagnostic only. V7.50 remains the frozen live champion. This audit isolates why Fantasy Edge loses late-draft market value in Rounds 11+ "
            "by comparing position mix, reach/faller behavior, model-rank divergence, roster state, IDP timing, and FE-vs-market evaluation."
        )

        if "v751_results" not in st.session_state: st.session_state.v751_results=None
        sims=st.number_input(
            "Late-draft audit simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v751_sims",
            help="Default: slots 1/3/7/12 × 5 rooms × 10 simulations = 200 champion drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run R11+ architecture audit",type="primary",key="v751_run")
        clear=b.button("Clear R11+ audit",key="v751_clear")
        if clear:
            st.session_state.v751_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=951000+int(ds)*10000+j*109+sum(ord(c) for c in room_name)
                        ...
                        late=fe[pd.to_numeric(fe.mock_round,errors="coerce").fillna(0)>=11].copy()
                        late_c=co[pd.to_numeric(co.mock_round,errors="coerce").fillna(0)>=11].copy()

                        if len(late):
                            late["market_pick_num"]=pd.to_numeric(late.get("market_pick",late.get("consensus_rank")),errors="coerce")
                            late["mock_pick_num"]=pd.to_numeric(late.mock_pick,errors="coerce")
                            late["market_delta"]=late["mock_pick_num"]-late["market_pick_num"]
                            late["reach_amt"]=np.maximum(late["market_pick_num"]-late["mock_pick_num"],0)
                            late["faller_amt"]=np.maximum(late["mock_pick_num"]-late["market_pick_num"],0)
                            late["model_rank_num"]=pd.to_numeric(late.get("model_rank",np.nan),errors="coerce")
                            late["model_market_divergence"]=late["market_pick_num"]-late["model_rank_num"]
                        if len(late_c):
                            late_c["market_pick_num"]=pd.to_numeric(late_c.get("market_pick",late_c.get("consensus_rank")),errors="coerce")
                            late_c["mock_pick_num"]=pd.to_numeric(late_c.mock_pick,errors="coerce")
                            late_c["market_delta"]=late_c["mock_pick_num"]-late_c["market_pick_num"]

                        rows.append({
                            "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Overall_gap":fc["overall"]-cc["overall"],
                            "Late_market_gap":late.market_delta.mean()-late_c.market_delta.mean() if len(late) and len(late_c) else np.nan,
                            "Late_FE_market_delta":late.market_delta.mean() if len(late) else np.nan,
                            "Late_consensus_market_delta":late_c.market_delta.mean() if len(late_c) else np.nan,
                            "Late_avg_reach":late.reach_amt.mean() if len(late) else np.nan,
                            "Late_avg_faller":late.faller_amt.mean() if len(late) else np.nan,
                            "Late_big_reaches":int((late.reach_amt>=10).sum()) if len(late) else 0,
                            "Late_model_market_divergence":late.model_market_divergence.mean() if len(late) else np.nan,
                            "Late_QB":int((late.position=="QB").sum()) if len(late) else 0,
                            "Late_RB":int((late.position=="RB").sum()) if len(late) else 0,
                            "Late_WR":int((late.position=="WR").sum()) if len(late) else 0,
                            "Late_TE":int((late.position=="TE").sum()) if len(late) else 0,
                            "Late_DL":int((late.position=="DL").sum()) if len(late) else 0,
                            "Late_DB":int((late.position=="DB").sum()) if len(late) else 0,
                            "Late_IDP_share":float(late.position.isin(["DL","DB"]).mean()) if len(late) else np.nan,
                            "Late_offense_share":float(late.position.isin(["QB","RB","WR","TE"]).mean()) if len(late) else np.nan,
                            "Late_WR_share":float((late.position=="WR").mean()) if len(late) else np.nan,
                            "Late_RB_share":float((late.position=="RB").mean()) if len(late) else np.nan,
                            "Late_TE_share":float((late.position=="TE").mean()) if len(late) else np.nan,
                            "Late_QB_share":float((late.position=="QB").mean()) if len(late) else np.nan,
                        })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%40==0 or done==total:
                            status.caption(f"R11+ audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v751_results=df
            status.success(f"R11+ audit complete: {len(df)} champion drafts.")
            prog.empty()

        df=st.session_state.get("v751_results")
        if isinstance(df,pd.DataFrame) and len(df):
            summary=pd.DataFrame([{
                "Overall gap":df.Overall_gap.mean(),
                "R11+ market gap":df.Late_market_gap.mean(),
                "FE late market delta":df.Late_FE_market_delta.mean(),
                "Consensus late market delta":df.Late_consensus_market_delta.mean(),
                "Avg late reach":df.Late_avg_reach.mean(),
                "Avg late faller":df.Late_avg_faller.mean(),
                "Avg model-vs-market divergence":df.Late_model_market_divergence.mean(),
                "IDP share":df.Late_IDP_share.mean(),
                "WR share":df.Late_WR_share.mean(),
                "RB share":df.Late_RB_share.mean(),
                "TE share":df.Late_TE_share.mean(),
                "QB share":df.Late_QB_share.mean(),
            }])
            st.markdown("#### R11+ architecture summary")
            st.dataframe(summary.round(3),use_container_width=True,hide_index=True)

            st.markdown("#### R11+ weakness by draft slot")
            slot=df.groupby("Draft_slot",as_index=False).agg(
                Overall_gap=("Overall_gap","mean"),
                Late_market_gap=("Late_market_gap","mean"),
                Avg_reach=("Late_avg_reach","mean"),
                Avg_faller=("Late_avg_faller","mean"),
                Model_market_divergence=("Late_model_market_divergence","mean"),
                IDP_share=("Late_IDP_share","mean"),
                WR_share=("Late_WR_share","mean"),
                RB_share=("Late_RB_share","mean"),
                TE_share=("Late_TE_share","mean")
            )
            st.dataframe(slot.round(3),use_container_width=True,hide_index=True)

            st.markdown("#### R11+ weakness by room profile")
            ...
            # Infer the dominant late-draft failure signature.
            reach=float(df.Late_avg_reach.mean())
            faller=float(df.Late_avg_faller.mean())
            div=float(df.Late_model_market_divergence.mean())
            idp=float(df.Late_IDP_share.mean())
            late_gap=float(df.Late_market_gap.mean())

            if reach>=8 and div>=10:
                st.warning(
                    f"Primary R11+ signature: aggressive model-vs-market divergence. FE is reaching {reach:.1f} picks on average late, "
                    f"with model-vs-market divergence around {div:.1f}. V7.52 should audit late-round player-score vs market-edge weighting before any positional rule."
                )
            elif idp>=.35 and late_gap<0:
                st.warning(
                    f"Primary R11+ signature: IDP timing. IDP represents {idp:.0%} of late picks while R11+ market gap is {late_gap:+.2f}. "
                    "V7.52 should compare offense-vs-IDP timing and whether IDP candidates are entering the draft window too early."
                )
            elif faller<2 and late_gap<0:
                st.warning(
                    f"Primary R11+ signature: FE is not harvesting fallers late (avg faller {faller:.1f}) despite a {late_gap:+.2f} market gap. "
                    "V7.52 should audit execution-choice late-round readiness rather than player valuation."
                )
            else:
                st.info(
                    "R11+ weakness is mixed. Use the slot/room tables to identify whether reaches, IDP timing, or position mix is the most stable failure mode."
                )

            st.download_button(
                "⬇️ Download V7.51 R11+ architecture audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_51_r11plus_architecture_audit.csv",
                "text/csv",key="v751_download"
            )
