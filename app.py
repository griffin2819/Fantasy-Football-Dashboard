    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                # V7.64 challenger-only Round-7 dominance override.
                # Never hard-codes a player. It fires only when:
                # 1) normal V7.60 choice is >10 picks ahead of effective market,
                # 2) an actually alive+eligible alternative is market-respecting,
                # 3) that alternative has both higher frozen draft score and higher VORP.
                if bool(r7_dominance_override_enabled) and round_no==7:
                    _normal=int(local_choice)
                    ...
                    if np.isfinite(_normal_reach) and _normal_reach>10.0:
                        ...
                        if len(_r7_ids):
                            ...
                            if len(_dom):
                                # Among true-board dominance candidates, take the strongest frozen draft score,
                                # then use better market price as a deterministic tie-break.
                                _ord=np.lexsort((
                                    np.where(np.isnan(market_pick[_dom]),1e9,market_pick[_dom]),
                                    -draft_score[_dom]
                                ))
                                _best_global=int(_dom[_ord[0]])
                                _match=np.where(sidx==_best_global)[0]
                                if len(_match):
...
st.title("🏈 Fantasy Edge V7.64 — Round-7 Dominance Override Challenger")
st.caption("V7.64 • V7.60 frozen champion • Round-7 dominance challenger")
st.caption("Build ID: V7.64-R7-DOMINANCE-CHALLENGER-20260816")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.64 keeps V7.60 frozen/live and tests a challenger that overrides only Round-7 >10-pick reaches when a true-board market-respecting alternative has both higher draft score and higher VORP.")
        ...
        st.markdown("### 🧪 V7.64 — Round-7 Dominance Override Challenger")
        st.caption(
            "Matched challenger only. V7.60 remains frozen/live. The challenger changes only Round 7 and only when the normal V7.60 choice "
            "reaches >10 picks while a genuinely available market-respecting player has BOTH higher frozen draft score and higher VORP. "
            "No player names are hard-coded."
        )

        if "v764_results" not in st.session_state:
            st.session_state.v764_results=None

        sims=st.number_input(
            "V7.64 matched simulations per slot × room",
            min_value=10,max_value=50,value=25,step=5,key="v764_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 fresh matched V7.60-vs-challenger drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-7 dominance challenger",type="primary",key="v764_run")
        clear=b.button("Clear V7.64 challenger",key="v764_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=964000+int(ds)*100000+j*181+sum(ord(c) for c in room_name)
                        ...
                        c7=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(7)]
                        n7=cand[pd.to_numeric(cand.mock_round,errors="coerce").eq(7)]
                        champ7=str(c7.iloc[0].player) if len(c7) else ""
                        cand7=str(n7.iloc[0].player) if len(n7) else ""
                        champ7_pos=str(c7.iloc[0].position) if len(c7) else ""
                        cand7_pos=str(n7.iloc[0].position) if len(n7) else ""

                        def _grade_component(comp,key):
                            try:
                        ...
                        rows.append({
                            ...
                            "R7_10_change":r710,
                            "R11_plus_change":r11,
                            "Roster_balance_change":_grade_component(nc,"roster_balance")-_grade_component(cc,"roster_balance"),
                            "Starter_completion_change":_grade_component(nc,"starter_completion")-_grade_component(cc,"starter_completion"),
                            "Availability_change":_grade_component(nc,"availability")-_grade_component(cc,"availability"),
                            "Market_value_change":_grade_component(nc,"market_value")-_grade_component(cc,"market_value"),
                            "Conditional_overall_win":(overall>0) if changed else np.nan,
                            "Conditional_nonloss":(overall>=0) if changed else np.nan,
                            "Conditional_R7_10_win":(r710>0) if changed else np.nan,
                            "Conditional_R11_nonloss":(r11>=0) if changed else np.nan,
                        })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%100==0 or done==total:
                            status.caption(f"V7.64 challenger: {done}/{total} fresh matched drafts")

            df=pd.DataFrame(rows)
            st.session_state.v764_results=df
            status.success(f"V7.64 complete: {len(df)} matched drafts.")
            prog.empty()

        df=st.session_state.get("v764_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            cond710=changed.Conditional_R7_10_win.mean() if len(changed) else np.nan
            cond11=changed.Conditional_R11_nonloss.mean() if len(changed) else np.nan
            ...
            summary=pd.DataFrame([{
                "Matched drafts":len(df),
                "Changed drafts":len(changed),
            ...
            st.markdown("#### Round-7 override paths")
            ...
            screen=(
                ...
                and positive_slots>=2
                and positive_rooms>=4
            )

            if screen:
                st.success(
                    "V7.64 SCREEN PASSED. The true-board Round-7 dominance override improves whole-draft results without materially damaging earlier or late phases. "
                    "Move this exact rule into a larger frozen validation before promotion."
                )
            else:
                st.warning(
                    "V7.64 does not clear screening. Keep V7.60 frozen and do not hard-code a Round-7 rule from the V7.63 diagnostic."
                )

            st.download_button(
                "⬇️ Download V7.64 Round-7 dominance challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_64_round7_dominance_challenger.csv",
                "text/csv",key="v764_download"
            )
