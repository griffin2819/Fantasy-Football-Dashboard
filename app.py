...
st.set_page_config(page_title="Fantasy Edge v9.28 — Live Draft", page_icon="🏈", layout="wide")
...
def v9_live_rank(avail, round_no, current_pick, roster_counts):
    ...
    need=[]
    caps={"QB":(1,2),"RB":(2,99),"WR":(2,99),"TE":(1,2),"DL":(1,1),"DB":(1,1)}
    for p in x["position"]:
...
def grade_mock(roster, teams, slots):
    ...
    market_rank=pd.to_numeric(r.consensus_rank,errors="coerce")
    verified=market_rank.notna()
    edge=(market_rank-r.model_rank).clip(-20,20)
    avg=float(edge[verified].mean()) if verified.any() else 0.0
    verified_rate=float(verified.mean())
    draft_value=float(np.clip(68+avg+verified_rate*4,35,96))
    construction=100.0
    ...
    defenders=r[r.position.isin(["DL","DB"])].copy()
    if len(defenders):
        dcr=pd.to_numeric(defenders.consensus_rank,errors="coerce")
        idp_quality=float((100-(dcr-1)*2.5).clip(0,100).fillna(0).mean())
    else:
        idp_quality=50.0
    # v9.28: Model Edge rewards *verified* disagreement, confidence and IDP quality.
    # Missing-market picks no longer receive free edge credit.
    model_edge=float(np.clip(58+avg*.85+(conf-.70)*22+verified_rate*8+(idp_quality-50)*.08,35,96))
...
def roster_opportunity_adjustment(df, roster, round_no, current_pick, slots):
    ...
    for _,r in x.iterrows():
        ...
        picks_left=max(0, int(slots.get("QB",0)+slots.get("RB",0)+slots.get("WR",0)+slots.get("TE",0)+
                              slots.get("DL",0)+slots.get("DB",0)+slots.get("FLEX",0)) - len(roster))
        missing_required=0
        for p in ["QB","RB","WR","TE","DL","DB"]:
...
def recommendation_integrity_overlay(df, current_pick):
    """v9.28 recommendation-only board truth + IDP quality gate.

    The raw player board remains visible, but aggressive MODEL TARGET / DRAFT NOW
    recommendations require current market evidence. This prevents missing-market
    defenders and implausibly stale offensive fallers from being treated as steals.
    """
    x=df.copy()
    cr=pd.to_numeric(x.get("consensus_rank",np.nan),errors="coerce")
    mp=pd.to_numeric(x.get("market_pick",cr),errors="coerce")
    idp=x["position"].isin(["DL","DB"])
    offense=~idp
    cur=float(current_pick)

    x["recommendation_eligible"]=True
    x["board_truth_reason"]="verified market listing"

    # No current consensus listing = no aggressive recommendation. The player can
    # still appear on the raw IDP/player boards and can be manually drafted.
    missing=cr.isna()
    x.loc[missing,"recommendation_eligible"]=False
    x.loc[missing,"board_truth_reason"]="no current consensus listing"

    # Offensive players more than 42 picks beyond a current market rank are often
    # stale/absent on the host board. Require manual confirmation instead of calling
    # them a value faller. (IDP market ranks are transformed separately, so this
    # specific stale-faller rule does not apply to defenders.)
    stale_offense=offense & mp.notna() & ((cur-mp)>42)
    x.loc[stale_offense,"recommendation_eligible"]=False
    x.loc[stale_offense,"board_truth_reason"]="extreme fall — verify host board"

    # IDP quality: combine current positional ECR, projected fantasy scoring and
    # data confidence. This moves proven, market-supported defenders ahead of
    # obscure names that only pop because of a noisy projection.
    x["idp_quality_score"]=np.nan
    x["idp_quality_bonus"]=0.0
    conf=pd.to_numeric(x.get("confidence",.5),errors="coerce").fillna(.5)
    proj=pd.to_numeric(x.get("projection",0),errors="coerce").fillna(0)
    for pos in ["DL","DB"]:
        m=x["position"].eq(pos)
        if not m.any():
            continue
        pct=proj.loc[m].rank(pct=True,method="average")*100
        mr=cr.loc[m]
        market_strength=(100-(mr-1)*2.5).clip(0,100).fillna(0)
        quality=.48*market_strength + .32*pct + .20*(conf.loc[m]*100)
        x.loc[m,"idp_quality_score"]=quality
        x.loc[m,"idp_quality_bonus"]=(quality-50).mul(.16).clip(-7,10)

    # For a one-starter DL/DB format there is no reason to recommend a defender
    # outside the top 40 positional consensus or with a sub-replacement quality
    # profile. This is intentionally a recommendation gate, not a deletion.
    weak_idp=idp & (cr.gt(40) | x["idp_quality_score"].fillna(0).lt(38))
    x.loc[weak_idp,"recommendation_eligible"]=False
    x.loc[weak_idp,"board_truth_reason"]="below IDP quality gate"
    return x
...
st.title("🏈 Fantasy Edge v9.28 — Board Truth + IDP Quality")
st.caption("v9.28 • board-truth recommendation gate • IDP quality upgrade • calibrated model edge")
st.caption("Build ID: FE-V9.28-BOARD-TRUTH-IDP-EDGE-20260820")
...
with tabs[1]:
    st.subheader("Live roster-aware draft assistant — v9.22")
    st.success("✅ v9.08 certified engine • v9.22 injury overlay • v9.28 board-truth + IDP quality overlay • Mine/Opponent tracking")
    ...
    x=recommendation_integrity_overlay(x,max(1,len(state["taken"])+1))
    rejected_live=x[~x.recommendation_eligible].copy()
    verified_live=x[x.recommendation_eligible].copy()
    if not verified_live.empty:
        x=verified_live
    else:
        st.warning("Board-truth gate found no verified recommendation candidates; showing the unfiltered pool as a fallback.")
    ...
    # v9.1 CERTIFIED LIVE ENGINE — frozen v9.08 production translation.
    x=v9_live_rank(x,live_round,current_overall,counts)
    x["evaluation_score"]=x["v9_live_score"]
    # v9.22 injury overlay is temporary and does not alter the frozen certified board.
    x["evaluation_score"]-=pd.to_numeric(x.get("injury_penalty",0),errors="coerce").fillna(0)

    # v9.24 roster opportunity cost: value the pick in the context of MY current roster.
    x=roster_opportunity_adjustment(x,myb,live_round,current_overall,slots)
    x["evaluation_score"]+=pd.to_numeric(x["roster_opportunity_adj"],errors="coerce").fillna(0)
    x["evaluation_score"]+=pd.to_numeric(x.get("idp_quality_bonus",0),errors="coerce").fillna(0)

    # PICK EXECUTION — market timing remains separate from player/roster evaluation.
    ...
    # v9.28 identity integrity: preserve the exact intercept player before sorting.
    live_intercept_player=None
    if intercepted and intercept_local is not None and 0 <= int(intercept_local) < len(x):
        live_intercept_player=str(x.iloc[int(intercept_local)].player)
    ...
    live_intercept_row=None
    if intercepted and live_intercept_player:
        rr=x[x.player.astype(str).eq(live_intercept_player)]
        if len(rr): live_intercept_row=rr.iloc[0]
    if live_intercept_row is not None:
        fi=live_intercept_row
        st.success(f"💎 FALLER INTERCEPT: {fi.player} ({fi.position}, {fi.team}) — {intercept_fall:.0f} picks past market")
        st.caption(f"Dynamic override triggered because the fall is large enough for this draft stage and the player remains model-compatible. Model target #{int(fi.evaluation_rank) if pd.notna(fi.evaluation_rank) else 999} • market #{int(round(fi.market_pick)) if pd.notna(fi.market_pick) else '—'}")
    elif len(draft_now):
        dn=draft_now.iloc[0]
        ...
        st.caption(f"Model target #{int(dn.evaluation_rank) if pd.notna(dn.evaluation_rank) else 999} • market {('—' if pd.isna(dn.market_pick) else '#'+str(int(round(dn.market_pick))))} • timing window is open")
    ...
    st.dataframe(
        show[["player","position","team","Model target #","Action","Market pick","Faller picks",
              "Survives next","projection","vorp","roster_opportunity_adj","roster_opportunity_note",
              "Profile","Confidence","injury","injury_source"]]
        .rename(columns={
            "injury":"Injury","injury_source":"Injury source",
            "roster_opportunity_adj":"Roster opp.","roster_opportunity_note":"Roster reason"
        }).head(120),
        use_container_width=True,hide_index=True
    )
...
with tabs[2]:
    ...
    st.success("🔒 v9.23.1 availability integrity active • 🧠 v9.27.1 QB2 preserved + TE2/IDP2 opportunity active")
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                if avail.empty:
                    avail=board[(~board.player.isin(st.session_state.mock_drafted)) & (board.injury_severity<3)].copy()
                avail=recommendation_integrity_overlay(avail,overall)
                rejected_mock=avail[~avail.recommendation_eligible].copy()
                verified_mock=avail[avail.recommendation_eligible].copy()
                if not verified_mock.empty:
                    avail=verified_mock
                else:
                    st.warning("No verified board-truth candidates remain; using the unfiltered eligible pool as a fallback.")
                ...
                # Use calibrated v9.22 injury penalties in mock mode too.
                avail["evaluation_score"]-=pd.to_numeric(avail.get("injury_penalty",0),errors="coerce").fillna(0)

                # v9.24 dynamic roster opportunity cost.
                avail=roster_opportunity_adjustment(avail,roster,rnd,overall,state["roster_slots"])
                avail["evaluation_score"]+=pd.to_numeric(avail["roster_opportunity_adj"],errors="coerce").fillna(0)
                avail["evaluation_score"]+=pd.to_numeric(avail.get("idp_quality_bonus",0),errors="coerce").fillna(0)
                ...
                rc=roster.position.value_counts().to_dict() if len(roster) else {}
                # QB2/TE2, RB saturation, WR urgency and IDP opportunity cost are now
                # handled consistently by v9.24 in evaluation_score.
                ...
                if intercept_row is not None:
                    ...
                    # Never surface an intercept unless the exact player is still in the
                    # authoritative available-player pool.
                    intercepted=False
                    dn=now_top.iloc[0]
                ...
                if len(rejected_mock):
                    st.caption(f"🛡️ Board Truth blocked {len(rejected_mock)} stale/missing-market or low-quality IDP candidates from aggressive recommendations this pick.")
                top=now_top.copy()
                ...
                st.dataframe(
                    top[["player","position","team","evaluation_rank","Action","Market pick",
                         "Survives to next pick","projection","vorp",
                         "roster_opportunity_adj","roster_opportunity_note","idp_quality_score","board_truth_reason","profile"]]
                    .rename(columns={
                        "roster_opportunity_adj":"Roster opp.",
                        "roster_opportunity_note":"Roster reason",
                        "idp_quality_score":"IDP quality",
                        "board_truth_reason":"Board truth"
                    }),
                    use_container_width=True,hide_index=True
                )
                ...
                if intercepted and intercept_player and intercept_player in available_names:
                    pick_options=[intercept_player]+[p for p in pick_options if p!=intercept_player]
                ...
                if not pick_options:
                    ...
                    if st.button("Draft this player"):
                        ...
                        if choice not in set(current_available.player.astype(str)):
                            ...
                            st.rerun()
                        row=current_available[current_available.player.astype(str).eq(str(choice))].iloc[0].to_dict()
