...
def _single_authority_strategy(y,roster,slots,current_pick,next_pick):
    ...
    _market=_safe_num_series(z,"market_pick",float(current_pick)+12.0)
    _reach_ahead=np.maximum(_market-float(current_pick)-8.0,0.0)
    _reach_cost=np.minimum(_reach_ahead,45.0)*0.20
    ...
    for idx in keep:
        ...
        z.at[idx,"strategic_pick_value"]=(
            ...
            1.35*wait+
            float(room.loc[idx])+
            1.20*float(_cross_slot.loc[idx])-
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    for _flag in ["roster_completion_blocked","same_pos_dominated","idp_quality_blocked","extra_idp_blocked"]:
    ...
    if (_feasible_now & (_ropp_final>=0)).any():
        _bad_luxury=_feasible_now & (_ropp_final<-6.0)
        x.loc[_bad_luxury,"execution_score"]-=250.0
...
def grade_mock(roster, teams, slots):
    ...
    # Interpretable quality composite. 90+ requires high Draft Value, Construction,
    # Positional Advantage, strong clean-pick consistency, and low opportunity loss.
...
def _sim_user_pick_same_engine(avail,roster,round_no,current_pick,slot,teams,slots,randomness=6):
    ...
    sim_avail=_benchmark_candidate_prefilter(avail,int(current_pick))
    ...
    if ranked is None or len(ranked)==0:
        ...
        raise RuntimeError(
            f"No eligible production candidate at round={int(round_no)}, overall={int(current_pick)}, "
            f"roster={counts}, remaining_pool={remaining}"
        )
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        target_sims=100
        _benchmark_build="FE-V6.1-BENCHMARK-STABILITY-20260826"
