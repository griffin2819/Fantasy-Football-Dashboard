...
st.title("🏈 Fantasy Edge V7.52 — R11+ Decision-Level Execution Audit")
st.caption("V7.52 • V7.50 champion frozen • R11+ decision-level diagnostic")
st.caption("Build ID: V7.52-R11DLA-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v752_live_slot")
    ...
    if "v752_target_queue" not in st.session_state:
        st.session_state.v752_target_queue={}
    queue=st.session_state.v752_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v752_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.52 keeps V7.50 as the live/default champion with the 55.97 rule unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.52 robustness report")
        ...
        st.markdown("### 🧪 V7.52 champion baseline validation")
        st.info("V7.52 is diagnostic only. No late-round weight, execution, position, or threshold changes are introduced.")
        ...
        if "v752_independent_results" not in st.session_state:
            st.session_state.v752_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v752_independent_results"),pd.DataFrame) else "ready to run"))
        if "v752_independent_summary" not in st.session_state:
            st.session_state.v752_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v752_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v752_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v752_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v752_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v752_independent_results=iv
                st.session_state.v752_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v752_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v752_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.52 Champion Baseline Check")
            st.caption("V7.52 keeps the V7.50 champion fully frozen and captures each R11+ selection against the actual passed market faller.")
            ...
            st.download_button(
                "⬇️ Download V7.52 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_52_champion_baseline.csv",
                "text/csv",
                key="v752_baseline_download"
            )
        ...
        st.markdown("### 🔎 V7.52 — R11+ Decision-Level Execution Audit")
        st.caption(
            "Diagnostic only. V7.50 remains the frozen champion. This captures each Fantasy Edge decision in Rounds 11-15 and compares the selected player "
            "with the best passed market faller, including FE score gap, roster utility, projection, VORP, market edge, survival, and actual next-turn availability."
        )

        if "v752_results" not in st.session_state: st.session_state.v752_results=None
        sims=st.number_input(
            "Decision-audit simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v752_sims",
            help="Default: slots 1/3/7/12 × 5 rooms × 10 simulations = 200 champion drafts / ~1,000 late-round decisions."
        )
        a,b=st.columns(2)
        run=a.button("Run R11+ decision audit",type="primary",key="v752_run")
        clear=b.button("Clear R11+ decision audit",key="v752_clear")
        if clear:
            st.session_state.v752_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=952000+int(ds)*10000+j*113+sum(ord(c) for c in room_name)
                        ...
                        late=fe[
                            (pd.to_numeric(fe.mock_round,errors="coerce").fillna(0)>=11)
                            & (pd.to_numeric(fe.get("MR_Audit",0),errors="coerce").fillna(0).eq(1))
                        ].copy()

                        for _,r in late.iterrows():
                            selected_market=pd.to_numeric(pd.Series([r.get("MR_Selected_market_pick",np.nan)]),errors="coerce").iloc[0]
                            selected_pick=pd.to_numeric(pd.Series([r.get("mock_pick",np.nan)]),errors="coerce").iloc[0]
                            selected_reach=max(float(selected_market)-float(selected_pick),0) if pd.notna(selected_market) and pd.notna(selected_pick) else np.nan

                            rows.append({
                                "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                "Round":int(r.get("mock_round",0)),"Pick":int(r.get("mock_pick",0)),
                                "Selected_player":r.get("MR_Selected_player",""),
                                "Selected_position":r.get("MR_Selected_position",""),
                                "Selected_market_pick":selected_market,
                                "Selected_reach":selected_reach,
                                "Selected_eval":r.get("MR_Selected_eval",np.nan),
                                "Selected_projection":r.get("MR_Selected_projection",np.nan),
                                "Selected_vorp":r.get("MR_Selected_vorp",np.nan),
                                "Selected_survival_est":r.get("MR_Selected_survival_est",np.nan),

                                "Market_best_player":r.get("MR_Market_best_player",""),
                                "Market_best_position":r.get("MR_Market_best_position",""),
                                "Market_best_pick":r.get("MR_Market_best_pick",np.nan),
                                "Market_best_eval":r.get("MR_Market_best_eval",np.nan),
                                "Market_best_survival_est":r.get("MR_Market_best_survival_est",np.nan),
                                "Market_best_survived_actual":r.get("MR_Market_best_survived_actual",np.nan),
                                ...
                                "Draftscore_gap":r.get("MR42_Faller_draftscore_gap",np.nan),
                                "Rosterutility_gap":r.get("MR42_Faller_rosterutility_gap",np.nan),
                                "Marketedge_gap":r.get("MR42_Faller_marketedge_gap",np.nan),
                                "VORP_gap":r.get("MR42_Faller_vorp_gap",np.nan),
                                "Projection_gap":r.get("MR42_Faller_projection_gap",np.nan),
                                "Progression_gap":r.get("MR42_Faller_progression_gap",np.nan),
                                "Regression_gap":r.get("MR42_Faller_regression_gap",np.nan),
                                "Scarcity_gap":r.get("MR42_Faller_scarcity_gap",np.nan),
                                "Consensus_gap":r.get("MR42_Faller_consensus_gap",np.nan),
                                "Residual_noise_gap":r.get("MR42_Faller_residual_noise_gap",np.nan),

                                "Eval_gap_selected_vs_market_best":r.get("MR_Eval_gap_selected_vs_market_best",np.nan),
                                "Market_gap_selected_vs_market_best":r.get("MR_Market_gap_selected_vs_market_best",np.nan),
                                "Selected_is_IDP":str(r.get("MR_Selected_position","")) in ["DL","DB"],
                                "Faller_is_IDP":str(r.get("MR_Passed_faller_position","")) in ["DL","DB"],
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%40==0 or done==total:
                            status.caption(f"R11+ decision audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v752_results=df
            status.success(f"R11+ decision audit complete: {len(df)} late-round decisions.")
            prog.empty()

        df=st.session_state.get("v752_results")
        if isinstance(df,pd.DataFrame) and len(df):
            numeric_cols=[
                "Selected_reach","Passed_faller_amount","Passed_faller_eval_gap",
                "Passed_faller_survival_est","Passed_faller_survived_actual",
                "Draftscore_gap","Rosterutility_gap","Marketedge_gap","VORP_gap",
                "Projection_gap","Residual_noise_gap","Eval_gap_selected_vs_market_best",
                "Market_gap_selected_vs_market_best"
            ]
            for c in numeric_cols:
                df[c]=pd.to_numeric(df[c],errors="coerce")

            summary=pd.DataFrame([{
                "Late decisions":len(df),
                "Avg selected reach":df.Selected_reach.mean(),
                "Avg passed faller":df.Passed_faller_amount.mean(),
                "Avg selected-vs-faller FE gap":df.Passed_faller_eval_gap.mean(),
                "Avg roster-utility gap":df.Rosterutility_gap.mean(),
                "Avg draft-score gap":df.Draftscore_gap.mean(),
                "Avg VORP gap":df.VORP_gap.mean(),
                "Avg projection gap":df.Projection_gap.mean(),
                "Avg market-edge gap":df.Marketedge_gap.mean(),
                "Faller estimated survival":df.Passed_faller_survival_est.mean(),
                "Faller actual survival":df.Passed_faller_survived_actual.mean(),
                "Selected IDP share":df.Selected_is_IDP.mean(),
                "Faller IDP share":df.Faller_is_IDP.mean(),
            }])
            st.markdown("#### Late-decision architecture summary")
            st.dataframe(summary.round(3),use_container_width=True,hide_index=True)

            st.markdown("#### Why FE passes the faller — by draft slot")
            slot=df.groupby("Draft_slot",as_index=False).agg(
                Decisions=("Pick","count"),
                Selected_reach=("Selected_reach","mean"),
                Faller_amount=("Passed_faller_amount","mean"),
                FE_gap=("Passed_faller_eval_gap","mean"),
                Draftscore_gap=("Draftscore_gap","mean"),
                Rosterutility_gap=("Rosterutility_gap","mean"),
                VORP_gap=("VORP_gap","mean"),
                Projection_gap=("Projection_gap","mean"),
                Marketedge_gap=("Marketedge_gap","mean"),
                Faller_survival_est=("Passed_faller_survival_est","mean"),
                Faller_survived_actual=("Passed_faller_survived_actual","mean"),
                Selected_IDP_share=("Selected_is_IDP","mean")
            )
            st.dataframe(slot.round(3),use_container_width=True,hide_index=True)

            st.markdown("#### Why FE passes the faller — by room")
            ...
            path=df.groupby(["Selected_position","Passed_faller_position"],as_index=False).agg(
                Decisions=("Pick","count"),
                Selected_reach=("Selected_reach","mean"),
                Faller_amount=("Passed_faller_amount","mean"),
                FE_gap=("Passed_faller_eval_gap","mean"),
                ...
                Draftscore_gap=("Draftscore_gap","mean"),
                VORP_gap=("VORP_gap","mean"),
                Faller_survived=("Passed_faller_survived_actual","mean")
            )
            st.markdown("#### Selected position → passed-faller position")
            st.dataframe(path.sort_values("Decisions",ascending=False).round(3),use_container_width=True,hide_index=True)

            # Focus on major reaches where a real market faller is present.
            bad=df[(df.Selected_reach>=15) & (df.Passed_faller_amount>=10)].copy()
            if len(bad):
                st.markdown("#### High-cost late decisions (reach ≥15 + faller ≥10)")
                high=pd.DataFrame([{
                    "Decisions":len(bad),
                    "Share of all late decisions":len(bad)/len(df),
                    "Avg reach":bad.Selected_reach.mean(),
                    "Avg faller":bad.Passed_faller_amount.mean(),
                    "Avg FE gap":bad.Passed_faller_eval_gap.mean(),
                    "Draftscore gap":bad.Draftscore_gap.mean(),
                    "Rosterutility gap":bad.Rosterutility_gap.mean(),
                    "VORP gap":bad.VORP_gap.mean(),
                    "Projection gap":bad.Projection_gap.mean(),
                    "Market-edge gap":bad.Marketedge_gap.mean(),
                    "Faller actual survival":bad.Passed_faller_survived_actual.mean()
                }])
                st.dataframe(high.round(3),use_container_width=True,hide_index=True)

            # Identify the largest positive FE component driving the selected player over the faller.
            means={
                "draft score":float(df.Draftscore_gap.mean()),
                "roster utility":float(df.Rosterutility_gap.mean()),
                "market edge":float(df.Marketedge_gap.mean()),
                "VORP":float(df.VORP_gap.mean()),
                "projection":float(df.Projection_gap.mean()),
            }
            dominant=max(means,key=means.get)
            fall_survive=float(df.Passed_faller_survived_actual.mean())
            reach=float(df.Selected_reach.mean())

            if dominant=="roster utility" and means[dominant]>5:
                st.warning(
                    f"Primary R11+ driver: roster utility ({means[dominant]:+.2f} FE points) is overpowering market timing while FE reaches {reach:.1f} picks on average. "
                    "V7.53 should audit late-round roster-utility construction before changing execution."
                )
            elif dominant in ["draft score","VORP"] and means[dominant]>5:
                st.warning(
                    f"Primary R11+ driver: player valuation ({dominant}, {means[dominant]:+.2f}) is overpowering the passed faller. "
                    "V7.53 should decompose late-round player score and replacement/VORP before adding a market intercept."
                )
            elif fall_survive<.35 and reach>=15:
                st.warning(
                    f"Primary R11+ driver appears to be execution timing: FE reaches {reach:.1f} picks while passed fallers survive only {fall_survive:.0%} of the time. "
                    "V7.53 should calibrate a late-round faller-vs-reach execution rule with V7.50 frozen."
                )
            else:
                st.info(
                    "The late-draft failure is mixed. Use the slot and position-path tables to identify the most stable driver before building V7.53."
                )

            st.download_button(
                "⬇️ Download V7.52 R11+ decision-level audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_52_r11plus_decision_level_audit.csv",
                "text/csv",key="v752_download"
            )
