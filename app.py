...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Evidence Time Authority", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def apply_v92_injury_overlay(board,state):
    ...
    for i,row in b.iterrows():
        ...
        _status_verified.append(True if current_source.lower().startswith("manual") else bool(watch.get("status_verified", final=="ACTIVE")))
        _status_quarantined.append(str(watch.get("quarantined_status","") or ""))
        _status_freshness.append(str(watch.get("freshness_reason","") or ""))
        _status_conflict.append(bool(watch.get("status_conflict",False)))
        _status_evidence_expired.append(bool(watch.get("evidence_expired",False)))
...
def _v755_quality_flags(board_df):
    ...
    for _,r in board_df.iterrows():
        ...
        if conflict or quarantined or not verified:
...
def _v756_stale_flags(board_df,now=None,state_snapshot=None,actionable_only=False):
    ...
    for _,r in board_df.iterrows():
        ...
        if not (quarantined or conflict or not verified):
            continue
...
def _v761_monitor_queue(board_df,state_snapshot):
    ...
    for _,r in my.iterrows():
        ...
        if expired:
            continue
        if (not verified) or conflict or quar:
            key=(p,"DATA")
            if key not in seen:
