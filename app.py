...
st.set_page_config(page_title="Fantasy Edge V6.33 — Active Roster Sync", page_icon="🏈", layout="wide")
...
# V6.33 active NFL roster sync. This runs even in fast-local mode so the player
# selector does not depend on the smaller certified ranking board. A failed
# network request never resets or alters the saved live draft state.
...
try:
    ...
    board=_v633_merge_active_roster_directory(board,_active_roster_export)
    st.sidebar.success(f"Active roster synced: {len(_active_roster_export):,} NFL players")
...
with tabs[1]:
    ...
    if len(_active_roster_export):
        _export_cols=[c for c in ["player_id","player","position","raw_position","team","active","injury_status","years_exp"] if c in _active_roster_export.columns]
        st.download_button(
    ...
    # V6.34: restore the original simple Live Draft picker. Active-roster sync
    # ONLY expands the names available here; it does not add a second search/filter
    # step that can hide or invalidate the current selection.
    _record_pool=board[~board["player"].isin(state.get("taken",[]))].copy()
    _record_options=sorted(
        _record_pool["player"].astype(str).drop_duplicates().tolist(),
        key=str.lower
    )
    pick=st.selectbox(
        "Player selected",
