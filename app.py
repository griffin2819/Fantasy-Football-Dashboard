...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Waiver Runtime Fix", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v781_render_page(_v777_page_idx):
    ...
    if _v777_page_idx==3:
        ...
        if _my_roster.empty:
            ...
            _team_cov=int(round(100.0*float(pd.to_numeric(board.get("_live_team_verified",pd.Series(0,index=board.index,dtype=float)),errors="coerce").fillna(0).mean()))) if len(board) else 0
            # V7.87: navigation must be snapshot-only. Count projections already
            # stored on the certified board instead of calling a provider from Waivers.
            _ext_matches=0
            try:
                _ext_col=pd.to_numeric(board.get("_external_week_projection",pd.Series(index=board.index,dtype=float)),errors="coerce")
                _ext_matches=int(_ext_col.notna().sum())
            except Exception:
                _ext_matches=0
            ...
            # V7.87: do not touch a live usage provider on a page click. The
            # snapshot's recency fields are the authority until manual refresh.
            _current_usage_rows=pd.DataFrame()
            _pregame_role_state=(
                _live_week<=1
                and _role_current_cov==0
