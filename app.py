...
def v9_live_rank(avail, round_no, current_pick, roster_counts):
    ...
    if pb.empty or "v9_base_live_score" not in x:
        x["v9_live_score"]=pd.to_numeric(x.get("draft_score",0),errors="coerce").fillna(0)
        return x
    ...
    # Use the old certified player prior as a stabilizer, not as an identity lock.
    # v9.08's default player weight is 16, which was large enough to overwhelm
    # current VORP/market/roster context and repeatedly surface the same names.
    mr=pd.to_numeric(x["v9_model_rank"],errors="coerce")
    player_prior=(1-(mr-1)/max(len(pb)-1,1)).clip(0,1).fillna(0)
    dynamic_rank=pd.to_numeric(x.get("draft_score",0),errors="coerce").rank(pct=True).fillna(.5)
    dynamic_vorp=pd.to_numeric(x.get("vorp",0),errors="coerce").rank(pct=True).fillna(.5)
    current_quality=(0.62*dynamic_rank+0.38*dynamic_vorp)
    ...
    # Cap frozen identity influence and replace the removed weight with live quality.
    # This keeps certification heritage while allowing the board to adapt each draft.
    frozen_pw=min(float(pw),5.0)
    live_quality_weight=max(3.0,min(8.0,float(pw)-frozen_pw))
    x["v9_live_score"]=(base
                         +frozen_pw*player_prior
                         +live_quality_weight*current_quality
                         +posw*position_prior
                         +tw*timing
                         +nw*np.asarray(need))
    x["frozen_player_prior_weight"]=frozen_pw
    x["live_quality_weight"]=live_quality_weight
    ...
    if mode=="Interactive mock":
        ...
        if b.button("Auto-draft opponents until my next pick"):
            ...
            while overall<=int(teams)*int(rounds):
                ...
                overdue_pressure=np.minimum(overdue,42.0)*0.30
