...
def mid_round_pipeline_audit_snapshot(fast, candidate_idx, delta, edge, eval_score, chosen_global,
    ...
    if falls[fj] > 0:
        ...
        snap.update({
            ...
            "MR_Passed_faller_eval_gap":chosen_eval-float(ev[fj]),
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None,
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user and 4<=round_no<=6:
...
st.title("🏈 Fantasy Edge V7.38 — Pick 12 R4-6 Decision-Path Audit")
st.caption("V7.38 • V7.24 9× champion frozen • Pick 12 R4-6 dual-driver audit")
st.caption("Build ID: V7.38-P12DPA-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v738_live_slot")
    ...
    if "v738_target_queue" not in st.session_state:
        st.session_state.v738_target_queue={}
    queue=st.session_state.v738_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v738_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.38 keeps the V7.24 9× champion completely frozen. No Pick-7 or Pick-12 intercept is active.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.38 robustness report")
        ...
        st.markdown("### 🧪 V7.38 champion baseline validation")
        st.info("V7.38 is diagnostic only. It isolates Pick-12 R4-6 cases where RB/TE choices are simultaneously supported by player score and roster marginal utility.")
        ...
        if "v738_independent_results" not in st.session_state:
            st.session_state.v738_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v738_independent_results"),pd.DataFrame) else "ready to run"))
        if "v738_independent_summary" not in st.session_state:
            st.session_state.v738_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v738_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v738_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v738_run")
        clear_iv=clear_col.button("Clear independent results",key="v738_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v738_independent_results=iv
                st.session_state.v738_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v738_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v738_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.38 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.38 adds a dedicated Pick-12 R4-6 decision-path audit.")
            ...
            st.download_button(
                "⬇️ Download V7.38 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_38_champion_baseline.csv",
                "text/csv",
                key="v738_baseline_download"
            )
        ...
        st.markdown("### 🔬 V7.38 — Pick 12 R4-6 Dual-Driver Decision Audit")
        st.caption(
            "Diagnostic only. The V7.24 9× champion remains frozen. This isolates Pick 12 R4-6 cases where Fantasy Edge "
            "selects RB/TE over a WR alternative and BOTH player/draft score and roster marginal utility support the RB/TE."
        )

        if "v738_audit_results" not in st.session_state:
            st.session_state.v738_audit_results=None
        if "v738_audit_meta" not in st.session_state:
            st.session_state.v738_audit_meta=None

        audit_sims=st.number_input(
            "Pick 12 audit simulations per room",
            min_value=5,max_value=25,value=10,step=5,key="v738_audit_sims",
            help="Default 10 × five room profiles = 50 Pick-12 champion drafts / 150 audited R4-6 decisions."
        )
        aa,ab=st.columns(2)
        run_audit=aa.button("Run Pick 12 dual-driver audit",type="primary",key="v738_audit_run")
        clear_audit=ab.button("Clear Pick 12 audit",key="v738_audit_clear")
        ...
        if run_audit:
            ...
            rows=[]
            total=len(rooms)*int(audit_sims)
            done=0
            prog=st.progress(0)
            status=st.empty()
            ...
            for room_name in rooms:
                for j in range(int(audit_sims)):
                    seed=938000+j*59+sum(ord(c) for c in room_name)

                    fe=simulate_mock_fast(
                        fast_champ,int(teams),12,15,state["roster_slots"],
                        6.0,True,seed,room_name
                    )
                    fc=independent_draft_grade_components(fe,int(teams))
                    ph=independent_phase_metrics(fe).set_index("Phase")

                    audited=fe[
                        pd.to_numeric(fe.get("MR_Audit",0),errors="coerce").fillna(0).eq(1)
                    ].copy()

                    for _,r in audited.iterrows():
                        ds_gap=float(pd.to_numeric(pd.Series([r.get("MR_Conflict_draftscore_gap",np.nan)]),errors="coerce").iloc[0])
                        ru_gap=float(pd.to_numeric(pd.Series([r.get("MR_Conflict_rosterdelta_gap",np.nan)]),errors="coerce").iloc[0])
                        edge_gap=float(pd.to_numeric(pd.Series([r.get("MR_Conflict_edge_gap",np.nan)]),errors="coerce").iloc[0])
                        conflict=int(pd.to_numeric(pd.Series([r.get("MR_Positional_conflict",0)]),errors="coerce").fillna(0).iloc[0])
                        dual=int(conflict==1 and pd.notna(ds_gap) and pd.notna(ru_gap) and ds_gap>0 and ru_gap>0)

                        total_pos=0.0
                        for v in [ds_gap,ru_gap,edge_gap]:
                            if pd.notna(v) and v>0: total_pos+=v
                        ds_share=(max(ds_gap,0)/total_pos) if total_pos>0 and pd.notna(ds_gap) else np.nan
                        ru_share=(max(ru_gap,0)/total_pos) if total_pos>0 and pd.notna(ru_gap) else np.nan

                        rows.append({
                            "Record_type":"pick12_r4_6_decision",
                            "Room":room_name,
                            "Simulation":j+1,
                            "Seed":seed,
                            "Overall_grade":fc["overall"],
                            "Overall_gap_vs_consensus":fc["overall"]-independent_draft_grade_components(
                                simulate_mock_fast(
                                    fast_champ,int(teams),12,15,state["roster_slots"],
                                    6.0,False,seed,room_name
                                ),int(teams)
                            )["overall"],
                            "R4_6_market_delta":float(ph.loc["R4-6","Avg_market_delta"]),
                            "Round":int(r.get("mock_round",0)),
                            "Pick":int(r.get("mock_pick",0)),
                            "Selected_player":r.get("MR_Selected_player",""),
                            "Selected_position":r.get("MR_Selected_position",""),
                            "Selected_market_pick":r.get("MR_Selected_market_pick",np.nan),
                            "Selected_reach":r.get("MR_Selected_reach",np.nan),
                            "Selected_faller":r.get("MR_Selected_faller",np.nan),
                            "Selected_survival":r.get("MR_Selected_survival_est",np.nan),
                            "Positional_conflict":conflict,
                            "Dual_driver_conflict":dual,
                            "Alternative_player":r.get("MR_Conflict_alt_player",""),
                            "Alternative_position":r.get("MR_Conflict_alt_position",""),
                            "Conflict_eval_gap":r.get("MR_Conflict_eval_gap",np.nan),
                            "Draftscore_contribution":ds_gap,
                            "Roster_utility_contribution":ru_gap,
                            "Market_edge_contribution":edge_gap,
                            "Draftscore_positive_share":ds_share,
                            "Roster_utility_positive_share":ru_share,
                            "Conflict_market_gap":r.get("MR_Conflict_market_gap",np.nan),
                            "Projection_raw_gap":r.get("MR_Conflict_projection_raw_gap",np.nan),
                            "Replacement_raw_gap":r.get("MR_Conflict_replacement_raw_gap",np.nan),
                            "VORP_raw_gap":r.get("MR_Conflict_vorp_raw_gap",np.nan),
                            "VORP_contribution":r.get("MR_Conflict_vorp_component_gap",np.nan),
                            "Projection_contribution":r.get("MR_Conflict_projection_component_gap",np.nan),
                            "Progression_contribution":r.get("MR_Conflict_progression_component_gap",np.nan),
                            "Regression_contribution":r.get("MR_Conflict_regression_component_gap",np.nan),
                            "Scarcity_contribution":r.get("MR_Conflict_scarcity_component_gap",np.nan),
                            "Consensus_contribution":r.get("MR_Conflict_consensus_component_gap",np.nan),
                            "Draftscore_primary_driver":r.get("MR_DraftScore_primary_driver",""),
                            "Passed_faller_player":r.get("MR_Passed_faller_player",""),
                            "Passed_faller_position":r.get("MR_Passed_faller_position",""),
                            "Passed_faller_amount":r.get("MR_Passed_faller_amount",np.nan),
                            "Passed_faller_eval_gap":r.get("MR_Passed_faller_eval_gap",np.nan),
                            "Passed_faller_survival_est":r.get("MR_Passed_faller_survival_est",np.nan),
                            "Passed_faller_survived_actual":r.get("MR_Passed_faller_survived_actual",np.nan),
                        })

                    done+=1
                    prog.progress(done/total)
                    if done==1 or done%10==0 or done==total:
                        status.caption(f"Pick 12 audit: {done}/{total} drafts")

            audit=pd.DataFrame(rows)
            st.session_state.v738_audit_results=audit
            st.session_state.v738_audit_meta={
                "seconds":time.perf_counter()-started,
                "drafts":total,
                "audited_decisions":len(audit)
            }
            status.success(f"Pick 12 audit complete: {len(audit)} R4-6 decisions.")
            prog.empty()

        audit=st.session_state.get("v738_audit_results")
        meta=st.session_state.get("v738_audit_meta") or {}

        if isinstance(audit,pd.DataFrame) and len(audit):
            conflict=audit[audit.Positional_conflict.eq(1)].copy()
            dual=audit[audit.Dual_driver_conflict.eq(1)].copy()

            a1,a2,a3,a4=st.columns(4)
            a1.metric("R4-6 decisions audited",str(len(audit)))
            a2.metric("RB/TE vs WR/QB conflicts",f"{len(conflict)/len(audit):.1%}")
            a3.metric("Dual-driver conflicts",f"{len(dual)/len(audit):.1%}")
            a4.metric("Dual-driver share of conflicts",f"{len(dual)/max(len(conflict),1):.1%}")

            if len(dual):
                comp=pd.DataFrame([
                    {"Component":"Player / draft score","Avg contribution":dual.Draftscore_contribution.mean(),
                     "Avg share of positive advantage":dual.Draftscore_positive_share.mean()},
                    {"Component":"Roster marginal utility","Avg contribution":dual.Roster_utility_contribution.mean(),
                     "Avg share of positive advantage":dual.Roster_utility_positive_share.mean()},
                    {"Component":"Model-vs-market edge","Avg contribution":dual.Market_edge_contribution.mean(),
                     "Avg share of positive advantage":np.nan},
                ])
                st.markdown("#### Why Pick 12 RB/TE choices survive both gates")
                st.dataframe(comp.round(3),use_container_width=True,hide_index=True)

                score=pd.DataFrame([
                    {"Player-score input":"VORP","Avg FE contribution":dual.VORP_contribution.mean()},
                    {"Player-score input":"Projection","Avg FE contribution":dual.Projection_contribution.mean()},
                    {"Player-score input":"Progression","Avg FE contribution":dual.Progression_contribution.mean()},
                    {"Player-score input":"Regression","Avg FE contribution":dual.Regression_contribution.mean()},
                    {"Player-score input":"Scarcity","Avg FE contribution":dual.Scarcity_contribution.mean()},
                    {"Player-score input":"Consensus","Avg FE contribution":dual.Consensus_contribution.mean()},
                ])
                st.markdown("#### Player-score decomposition inside dual-driver conflicts")
                st.dataframe(score.round(3),use_container_width=True,hide_index=True)

                urgency=pd.DataFrame([{
                    "Selected reach":dual.Selected_reach.mean(),
                    "Conflict market gap":dual.Conflict_market_gap.mean(),
                    "Passed faller size":dual.Passed_faller_amount.mean(),
                    "Passed faller survives":dual.Passed_faller_survived_actual.mean(),
                    "Passed faller survival est":dual.Passed_faller_survival_est.mean(),
                }])
                st.markdown("#### Draft-window urgency")
                st.dataframe(urgency.round(3),use_container_width=True,hide_index=True)

                room=dual.groupby("Room",as_index=False).agg(
                    Dual_conflicts=("Dual_driver_conflict","count"),
                    Avg_eval_gap=("Conflict_eval_gap","mean"),
                    Draftscore=("Draftscore_contribution","mean"),
                    Roster_utility=("Roster_utility_contribution","mean"),
                    VORP=("VORP_contribution","mean"),
                    Selected_reach=("Selected_reach","mean"),
                    Faller_size=("Passed_faller_amount","mean"),
                    Faller_survival=("Passed_faller_survived_actual","mean"),
                )
                st.markdown("#### Dual-driver conflicts by room")
                st.dataframe(room.round(3),use_container_width=True,hide_index=True)

                posmix=dual.groupby(
                    ["Selected_position","Alternative_position"],as_index=False
                ).agg(
                    Decisions=("Dual_driver_conflict","count"),
                    Eval_gap=("Conflict_eval_gap","mean"),
                    Draftscore=("Draftscore_contribution","mean"),
                    Roster_utility=("Roster_utility_contribution","mean"),
                    VORP=("VORP_contribution","mean"),
                    Market_gap=("Conflict_market_gap","mean")
                )
                st.markdown("#### Selected-position → alternative-position paths")
                st.dataframe(posmix.sort_values("Decisions",ascending=False).round(3),use_container_width=True,hide_index=True)

                ds=float(dual.Draftscore_contribution.mean())
                ru=float(dual.Roster_utility_contribution.mean())
                vorp=float(dual.VORP_contribution.mean())
                fall=float(dual.Passed_faller_amount.mean())
                survive=float(dual.Passed_faller_survived_actual.mean())

                if ru>=ds*.60 and vorp>0:
                    st.warning(
                        f"Pick 12 is a genuine dual-driver problem: roster utility ({ru:+.2f}) reinforces the player-score edge "
                        f"({ds:+.2f}), while VORP contributes {vorp:+.2f}. A future fix should not be another WR intercept; "
                        "it should test Pick-12 R4-6 roster-utility damping versus player-score damping in a matched matrix."
                    )
                elif ds>ru and vorp>0:
                    st.warning(
                        f"Pick 12 remains primarily player-score driven ({ds:+.2f} vs roster utility {ru:+.2f}), but roster utility is also positive. "
                        "The next test should compare modest player-score damping against combined player-score + roster-utility damping."
                    )
                elif fall>=7 and survive<.40:
                    st.warning(
                        "The strongest repeatable signal is draft-window loss: large passed fallers rarely survive. "
                        "The next test should focus on execution urgency rather than positional preference."
                    )
                else:
                    st.info(
                        "No single dual-driver mechanism dominates strongly enough for a model change. Use the room and position-path tables before designing V7.39."
                    )
            else:
                st.info("No dual-driver Pick-12 conflicts appeared in this sample.")

            st.success(
                f"Saved V7.38 audit: {meta.get('audited_decisions',len(audit))} R4-6 decisions"
                + (f" • {meta.get('seconds',0):.1f}s" if meta else "")
            )
            st.download_button(
                "⬇️ Download V7.38 Pick 12 decision-path audit CSV",
                audit.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_38_pick12_r4_6_decision_path_audit.csv",
                "text/csv",key="v738_download"
            )
