...
st.set_page_config(page_title="Fantasy Edge V6.39 — Roster Pick Commit Fix", page_icon="🏈", layout="wide")
...
try:
    ...
    st.sidebar.success(f"Active roster synced: {len(_active_roster_export):,} NFL players • {_ranked_dir} with live consensus rank")
...
with tabs[1]:
    ...
    _record_clicked=st.button("➕ Record draft pick",type="primary")
    if _record_clicked:
        # Read directly from session_state on submit instead of trusting the
        # transient local selectbox return value created during this rerun.
        _commit_pick=str(st.session_state.get("v634_record_pick",pick) or pick or "").strip()
        if not _commit_pick:
            ...
            # Resolve position from the ranked board first, then the synced
            # roster directory. Missing model data must never block recording.
            ...
            # Draft-log sequence is authoritative for the next recorded overall
            # pick. This also avoids roster-only names being rejected because
            # they do not exist in the certified projection board.
            _existing_overalls=[]
            for _entry in log:
                try:
