...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Snapshot Navigation", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
st.caption("⚡ Snapshot navigation • page clicks use cached data only • live network work runs only when you choose Refresh weekly data now")
...
def _v775_unified_decisions(board_df,state_snapshot,limit=12):
    ...
    # 5) Trade engine: only mutually viable ideas make the unified feed.
    try:
...
# Attach the canonical V7.55 state once; every tab can consume the same fields.
try:
    ...
    # V7.56 position-aware confidence calibration and news-change impact.
    ...
    _v755_quality=pd.DataFrame(); _v755_scarcity=pd.DataFrame()
...
# V7.77 LAZY PAGE NAVIGATION. st.tabs executes the body of every tab on every rerun,
# which made a single click recompute the whole application. A horizontal radio keeps
# the tab-like UX but executes only the selected page block below.
...
if _v777_page_idx==14:
    ...
    if my.empty:
        ...
        starters,bench,lineup=_v676_optimize_lineup(my)
        _v775_feed=_v775_unified_decisions(board,state,12)
        _actions=[]
