...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    # Authoritative live opportunity optimizer.
    x=_live_opportunity_optimizer(
        x,roster,slots,int(current_pick),int(next_pick),top_n=20
    )
    ...
    x["execution_score"]=x["opportunity_score"]
...
def grade_mock(roster, teams, slots):
    ...
    # Model Edge calibration: 90+ should require strong Draft Value AND Construction.
    _draft_value_gap=max(0.0,85.0-draft_value)
    _draft_value_edge_penalty=min(10.0,_draft_value_gap*0.45)

    model_edge=float(np.clip(
        model_edge-_construction_edge_penalty-_draft_value_edge_penalty,
        35,100
    ))

    opp+=_portfolio_penalty+_reach_penalty+_diversification_penalty

    score=float(np.clip(.30*draft_value+.31*construction+.23*positional+.16*model_edge-opp,0,100))
    ...
    return {"score":score,"grade":grade,"starter":starter,"value":avg_market,"penalty":max(0,100-construction),
            ...
            "big_reach_rate":_big_reach_rate,"portfolio_penalty":_portfolio_penalty,"diversification_penalty":_diversification_penalty,"construction_edge_penalty":_construction_edge_penalty,"draft_value_edge_penalty":_draft_value_edge_penalty}
...
def _fast_sim_user_pick(avail, roster_rows, rnd, overall, slot, teams, slots, randomness, rng):
    ...
    if dl>=1 and db>=1:
        if total_idp>=3:
            ...
            # Third IDP needs a REAL cross-position edge and some randomness from room flow.
            # No structural third-IDP preference.
            required_edge = 5.5 + rng.normal(0,1.0)
            if best_idp <= best_off + required_edge:
                feasible &= (~idp).to_numpy()
            else:
                score += np.where(impact_idp,0.3,0.0)
