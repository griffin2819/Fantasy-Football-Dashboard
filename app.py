...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Availability-Gated Start/Sit", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v680_injury_watch_feed():
    ...
    for key in keys:
        ...
        fresh=[r for r in candidates if r["_fresh"]]
    ...
    if _my.empty:
        ...
        if len(_starters):
            for _,sr in _starters.iterrows():
                ...
                decision=_v720_decision(conf,conf_edge,src.get("injury_effective",src.get("injury","")),has_alt,src.get("practice_status",""))
