...
st.title("🏈 Fantasy Edge V7.81 — R8–15 Faller Architecture Audit")
st.caption("V7.81 • V7.67 frozen champion • late-round architecture diagnostic")
st.caption("Build ID: V7.81-R8-15-FALLER-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.81 keeps V7.67 frozen/live and audits the structural R8–15 faller behavior without changing any production rule.")
        ...
        st.markdown("### 🔬 V7.81 — R8–15 Faller Architecture Audit")
        st.caption(
            "Measurement only. V7.67 remains the frozen production champion. "
            "V7.81 studies the structural late-round behavior identified in V7.75: increasingly deep market fallers from Rounds 8–15. "
            "No recommendation logic, weights, market transform, VORP, roster utility, or certified Round-7 rule is changed."
        )

        if "v781_results" not in st.session_state:
            st.session_state.v781_results=None

        sims=st.number_input(
            "V7.81 simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v781_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run R8–15 faller architecture audit",type="primary",key="v781_run")
        clear=b.button("Clear V7.81 audit",key="v781_clear")
        if clear:
            st.session_state.v781_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=981000+int(ds)*100000+j*277+sum(ord(c) for c in room_name)
                        ...
                        for _,r in d.iterrows():
                            ...
                            rows.append({
                                ...
                                "Faller10":bool(pd.notna(delta) and delta < -10),
                                "Faller20":bool(pd.notna(delta) and delta < -20),
                                "Faller30":bool(pd.notna(delta) and delta < -30),
                                "Reach10":bool(pd.notna(delta) and delta > 10),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.81 audit: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v781_results=df
            status.success(f"V7.81 complete: {len(df)} R8–15 decisions audited.")
            prog.empty()

        df=st.session_state.get("v781_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            round_summary=d.groupby("Round",as_index=False).agg(
                ...
                Median_market_delta=("Market_delta","median"),
                Faller10_rate=("Faller10","mean"),
                Faller20_rate=("Faller20","mean"),
                Faller30_rate=("Faller30","mean"),
                Reach10_rate=("Reach10","mean"),
                Avg_projection=("Projection","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
            )
            st.markdown("#### R8–15 by round")
            st.dataframe(round_summary.round(4),use_container_width=True,hide_index=True)

            pos=d.groupby(["Round","Position"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Faller10_rate=("Faller10","mean"),
                Faller20_rate=("Faller20","mean"),
            ...
            st.markdown("#### Round × position")
            st.dataframe(pos.round(4),use_container_width=True,hide_index=True)

            faller_players=d[d.Faller20].groupby(
                ["Round","Player","Position"],as_index=False
            ).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Avg_projection=("Projection","mean"),
            ...
            ).sort_values(["Decisions","Avg_market_delta"],ascending=[False,True])

            st.markdown("#### Most repeated >20-pick late fallers")
            st.dataframe(faller_players.head(50).round(3),use_container_width=True,hide_index=True)

            # Compare strong versus weak fallers on frozen value metrics.
            d["Faller_band"]=pd.cut(
                d.Market_delta,
                bins=[-1e9,-30,-20,-10,1e9],
                labels=["30+ faller","20–30 faller","10–20 faller","Not >10 faller"]
            )
            bands=d.groupby("Faller_band",observed=True,as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Avg_projection=("Projection","mean"),
                Avg_VORP=("VORP","mean"),
            ...
            st.markdown("#### Faller-depth value profile")
            st.dataframe(bands.round(4),use_container_width=True,hide_index=True)

            slot=d.groupby("Draft_slot",as_index=False).agg(
                Avg_market_delta=("Market_delta","mean"),
                Faller20_rate=("Faller20","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
            )
            room=d.groupby("Room",as_index=False).agg(
                Avg_market_delta=("Market_delta","mean"),
                Faller20_rate=("Faller20","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
            ...
            st.markdown("#### Late-round architecture by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Late-round architecture by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.81 is diagnostic only. The purpose is to determine whether deep late-round fallers are associated with stronger frozen VORP/draft-score profiles "
                "or whether the champion is systematically over-rewarding market drops. No challenger should be created until this architecture-level evidence is clear."
            )

            st.download_button(
                "⬇️ Download V7.81 R8–15 faller architecture audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_81_r8_15_faller_architecture_audit.csv",
                "text/csv",key="v781_download"
            )
