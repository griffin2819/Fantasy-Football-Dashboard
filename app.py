...
# ---------------- V7.55 Fantasy Edge Command Center ----------------
with tabs[14]:
    st.subheader("🏠 Fantasy Edge Command Center")
    st.caption("Your team, distilled to the actions that matter today.")
    my=board[board["player"].map(lambda n:_player_owner(state,n)=="1")].copy()
    if my.empty:
        ...
        _lowconf=int((pd.to_numeric(my.get("fe_role_confidence",0),errors="coerce").fillna(0)<50).sum())
        c1,c2,c3,c4=st.columns(4)
        c1.metric("Lineup",f"{lineup:.1f} PPG")
        c2.metric("Actions",len(_actions))
        c3.metric("Injury watch",_inj)
        c4.metric("Low confidence",_lowconf)
        if not _actions:
        ...
        s1.metric("Projected starters",f"{lineup:.1f}")
        s2.metric("Bench options",len(bench))
        _topscar="—" if _v755_scarcity.empty else str(_v755_scarcity.iloc[0]["Position"])
        s3.metric("Tightest league position",_topscar)
        with st.expander("Why / model details"):
            st.caption("Fantasy Edge uses one canonical player state across Draft, Waivers, Start/Sit, Trade Finder, Player Lab and IDP surfaces.")
            _show=my[[c for c in ["player","position","team","fe_week","fe_week_low","fe_week_high","fe_ros","fe_vorp","fe_role","fe_role_confidence","fe_opportunity_trend","fe_status"] if c in my.columns]].copy()
            _show=_show.rename(columns={"player":"Player","position":"Pos","team":"Team","fe_week":"Week","fe_week_low":"Low","fe_week_high":"High","fe_ros":"ROS","fe_vorp":"VORP","fe_role":"Role","fe_role_confidence":"Confidence","fe_opportunity_trend":"Opportunity","fe_status":"Status"})
            st.dataframe(_show,use_container_width=True,hide_index=True)
        with st.expander("Data quality watchdog"):
            if _v755_quality.empty: st.success("No player-state contradictions detected.")
            else: st.dataframe(_v755_quality,use_container_width=True,hide_index=True)
        with st.expander("League scarcity"):
            st.dataframe(_v755_scarcity,use_container_width=True,hide_index=True)
