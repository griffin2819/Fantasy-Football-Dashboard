...
def _v748_required_position_need_state(roster_df, news_df=None, starter_names=None):
    """Split required-position risk into HARD holes and correctly-attributed PREP watches.

    V7.50 attribution rule: a contingency belongs to the *at-risk starter*, not the
    healthy coverage player.  If the starter is risky but a second playable option is
    rostered, the position is covered and only an informational PREP relationship is
    returned.  Risk attached only to a backup does not create a lineup contingency.
    """
    req={"QB":1,"RB":2,"WR":2,"TE":1,"K":1,"DL":1,"DB":1}
    bad={"OUT","DOUBTFUL","IR","PUP","NFI","SUSPENDED","DO NOT DRAFT"}
    ...
    for pos,floor in req.items():
        ...
        playable=[]; risky=[]; risky_starters=[]
        for _,r in sub.iterrows():
            name=str(r.get("player","") or "")
            key=_owner_key(name)
            stx=_v677_normalize_injury_status(r.get("injury_effective",r.get("injury","")))
            risk=str(news_risk.get(key,"")).upper()
            is_playable=stx not in bad
            if is_playable:
                playable.append(name)
            if is_playable and (stx=="QUESTIONABLE" or risk in ("HIGH","ELEVATED")):
                item=(name,stx,risk)
                risky.append(item)
                if key in starter_keys:
                    risky_starters.append(item)
        if len(playable)<floor:
            hard.add(pos)
            reasons[pos]=f"{len(playable)}/{floor} playable"
            continue

        # Prefer actual optimized starters when available.  This prevents a healthy
        # starter from being mislabeled because the backup has an unrelated news hit.
        candidate_risks=risky_starters if starter_keys else risky
        if not candidate_risks:
            continue
        who,stx,risk=candidate_risks[0]
        flags=[]
        ...
        if len(playable)==floor:
            prep.add(pos)
            reasons[pos]=f"{who}: {flag_text} • no second playable option"
        elif coverage:
            # Covered risk is informational only; do not boost waiver candidates.
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        st.caption("V7.50 contingency attribution: risk is assigned to the at-risk starter, healthy backups are shown as coverage, backup-only news does not create a lineup contingency, and covered positions remain upgrade-only.")
        ...
        if _v748_active_prep:
            ...
            st.info("👀 CONTINGENCY PREP — no second playable option is rostered. " + " • ".join(_prep_bits))
        elif _covered_risk_bits:
            st.info("✅ CONTINGENCY COVERED — no waiver move is required for this risk. " + " • ".join(_covered_risk_bits))
