...
st.title("🏈 Fantasy Edge V7.69 — Round-4 True-Board Counterfactual Audit")
st.caption("V7.69 • V7.67 frozen champion • Round-4 true-board diagnostic")
st.caption("Build ID: V7.69-R4-TRUEBOARD-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.69 keeps certified V7.67 frozen and audits Round-4 >10-pick reaches against genuinely available true-board alternatives.")
        ...
        st.markdown("### 🔬 V7.69 — Round-4 True-Board Counterfactual Audit")
        st.caption(
            "Measurement only. V7.67 remains the frozen production champion. "
            "For every Round-4 champion reach >10 picks, V7.69 captures the actual alive+eligible board state after opponent picks "
            "and records the top five genuinely available market-respecting alternatives. No player is hard-coded."
        )

        if "v769_results" not in st.session_state:
            st.session_state.v769_results=None

        sims=st.number_input(
            "V7.69 simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v769_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-4 true-board audit",type="primary",key="v769_run")
        clear=b.button("Clear V7.69 audit",key="v769_clear")
        if clear:
            st.session_state.v769_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=969000+int(ds)*100000+j*211+sum(ord(c) for c in room_name)
                        d=simulate_mock_fast(
                            pool,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name
                        )

                        r4=d[pd.to_numeric(d.mock_round,errors="coerce").eq(4)]
                        if len(r4):
                            r=r4.iloc[0]
                            ...
                            audit=int(pd.to_numeric(pd.Series([r.get("R769_Audit",0)]),errors="coerce").fillna(0).iloc[0])
                            ...
                            if pd.notna(reach) and reach>10 and audit==1:
                                row={
                                    ...
                                    "Actual_available_count":r.get("R769_Actual_available_count",np.nan),
                                    "Market_respecting_count":r.get("R769_Market_respecting_count",np.nan),
                                }
                                for k in range(1,6):
                                    for fld in ["player","position","market_pick","model_rank","projection","vorp","draft_score"]:
                                        row[f"Alt{k}_{fld}"]=r.get(f"R769_Alt{k}_{fld}","" if fld in ["player","position"] else np.nan)
                                rows.append(row)

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.69 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v769_results=df
            status.success(f"V7.69 complete: {len(df)} true-board Round-4 >10-pick reaches audited.")
            prog.empty()

        df=st.session_state.get("v769_results")
        if isinstance(df,pd.DataFrame) and len(df):
            numcols=["Champion_projection","Champion_vorp","Champion_draft_score","Champion_market_pick",
            ...
            st.markdown("#### Round-4 reach drivers")
            ...
            showcols=["Draft_slot","Room","Round4_pick","Champion_player","Champion_position","Champion_reach"]
            for k in range(1,6):
            ...
            st.info(
                "V7.69 does not alter the champion. If a true-board alternative repeatedly has both higher draft score and higher VORP "
                "while improving market price, the next build can test a narrow Round-4 full-draft challenger. "
                "Do not retune the certified V7.67 Round-7 rule."
            )

            st.download_button(
                "⬇️ Download V7.69 Round-4 true-board counterfactual CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_69_round4_true_board_counterfactual_audit.csv",
                "text/csv",key="v769_download"
            )
        elif isinstance(df,pd.DataFrame):
            st.success("No Round-4 >10-pick reaches occurred in this run.")
