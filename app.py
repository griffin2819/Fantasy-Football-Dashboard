...
st.title("🏈 Fantasy Edge V7.55 — R11+ Reach-vs-Faller Counterfactual")
st.caption("V7.55 • V7.50 champion frozen • R11+ causal reach-vs-faller audit")
st.caption("Build ID: V7.55-R11COUNTERFACTUAL-20260816")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v755_live_slot")
    ...
    if "v755_target_queue" not in st.session_state:
        st.session_state.v755_target_queue={}
    queue=st.session_state.v755_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v755_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.55 keeps V7.50 as the live/default champion with the 55.97 rule unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.55 robustness report")
        ...
        st.markdown("### 🧪 V7.55 champion baseline validation")
        st.info("V7.55 is diagnostic only. No live weights, survival curves, valuation logic, or thresholds are changed.")
        ...
        if "v755_independent_results" not in st.session_state:
            st.session_state.v755_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v755_independent_results"),pd.DataFrame) else "ready to run"))
        if "v755_independent_summary" not in st.session_state:
            st.session_state.v755_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v755_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v755_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v755_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v755_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v755_independent_results=iv
                st.session_state.v755_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v755_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v755_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.55 Champion Baseline Check")
            st.caption("V7.55 keeps the V7.50 champion fully frozen and runs single-decision R11+ counterfactuals to measure whether paying a large reach is actually worth more than taking the available faller.")
            ...
            st.download_button(
                "⬇️ Download V7.55 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_55_champion_baseline.csv",
                "text/csv",
                key="v755_baseline_download"
            )
        ...
        st.markdown("### 🧬 V7.55 — R11+ Reach-vs-Faller Counterfactual")
        st.caption(
            "Diagnostic only. V7.50 remains frozen champion. For each late round independently, this reruns the exact same draft seed and forces the passed market faller "
            "only when V7.50 would reach ≥15 picks while a ≥10-pick faller is available. After that single forced decision, drafting returns immediately to normal V7.50."
        )

        if "v755_results" not in st.session_state: st.session_state.v755_results=None
        sims=st.number_input(
            "Counterfactual simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v755_sims",
            help="Default: 4 slots × 5 rooms × 10 base drafts, with independent R11/R12/R13/R14/R15 counterfactual reruns."
        )
        a,b=st.columns(2)
        run=a.button("Run reach-vs-faller counterfactual",type="primary",key="v755_run")
        clear=b.button("Clear V7.55 counterfactual",key="v755_clear")
        if clear:
            st.session_state.v755_results=None
            st.rerun()
        ...
        if run:
            ...
            rows=[]; total=len(slots_to_test)*len(rooms)*int(sims)*5; done=0
            prog=st.progress(0); status=st.empty()

            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=955000+int(ds)*10000+j*137+sum(ord(c) for c in room_name)
                        ...
                        base_by_round=base_draft.set_index("mock_round")

                        for force_round in [11,12,13,14,15]:
                            cf=simulate_mock_fast(
                                fast_champ,int(teams),int(ds),15,state["roster_slots"],
                                6.0,True,seed,room_name,
                                late_empirical_survival_enabled=False,
                                late_force_faller_round=force_round
                            )
                            cc=independent_draft_grade_components(cf,int(teams))
                            cp=independent_phase_metrics(cf).set_index("Phase")

                            changed=False
                            normal_player=""; forced_player=""
                            normal_market=np.nan; forced_market=np.nan
                            if force_round in base_by_round.index and force_round in cf.set_index("mock_round").index:
                                br=base_by_round.loc[force_round]
                                cr_=cf.set_index("mock_round").loc[force_round]
                                if isinstance(br,pd.DataFrame): br=br.iloc[0]
                                if isinstance(cr_,pd.DataFrame): cr_=cr_.iloc[0]
                                normal_player=str(br.player); forced_player=str(cr_.player)
                                changed=normal_player!=forced_player
                                normal_market=pd.to_numeric(pd.Series([br.get("market_pick",np.nan)]),errors="coerce").iloc[0]
                                forced_market=pd.to_numeric(pd.Series([cr_.get("market_pick",np.nan)]),errors="coerce").iloc[0]

                            overall=cc["overall"]-bc["overall"]
                            r11=float(cp.loc["R11+","Avg_market_delta"])-float(bp.loc["R11+","Avg_market_delta"])

                            rows.append({
                                "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                "Forced_round":force_round,
                                "Behavior_changed":changed,
                                "Normal_player":normal_player,
                                "Forced_faller":forced_player,
                                "Normal_market_pick":normal_market,
                                "Forced_market_pick":forced_market,
                                "Market_price_improvement":(normal_market-forced_market) if pd.notna(normal_market) and pd.notna(forced_market) else np.nan,
                                "Overall_change":overall,
                                "R1_3_change":float(cp.loc["R1-3","Avg_market_delta"])-float(bp.loc["R1-3","Avg_market_delta"]),
                                "R4_6_change":float(cp.loc["R4-6","Avg_market_delta"])-float(bp.loc["R4-6","Avg_market_delta"]),
                                "R7_10_change":float(cp.loc["R7-10","Avg_market_delta"])-float(bp.loc["R7-10","Avg_market_delta"]),
                                "R11_plus_change":r11,
                                "Counterfactual_win":(overall>0) if changed else np.nan,
                                "Counterfactual_nonloss":(overall>=0) if changed else np.nan,
                                "Counterfactual_R11_win":(r11>0) if changed else np.nan,
                            })
                            done+=1
                            prog.progress(done/total)
                            if done==1 or done%200==0 or done==total:
                                status.caption(f"V7.55 counterfactual: {done}/{total} reruns")

            df=pd.DataFrame(rows)
            st.session_state.v755_results=df
            status.success(f"V7.55 complete: {len(df)} counterfactual reruns.")
            prog.empty()

        df=st.session_state.get("v755_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ch=df[df.Behavior_changed].copy()

            st.markdown("#### Causal effect when the faller was actually forced")
            if len(ch):
                summary=pd.DataFrame([{
                    "Eligible forced decisions":len(ch),
                    "Eligibility rate":len(ch)/len(df),
                    "Overall change":ch.Overall_change.mean(),
                    "R11+ change":ch.R11_plus_change.mean(),
                    "Market-price improvement":ch.Market_price_improvement.mean(),
                    "Whole-draft win rate":ch.Counterfactual_win.mean(),
                    "Whole-draft non-loss":ch.Counterfactual_nonloss.mean(),
                    "R11+ win rate":ch.Counterfactual_R11_win.mean(),
                }])
                st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

                rnd=ch.groupby("Forced_round",as_index=False).agg(
                    Decisions=("Behavior_changed","count"),
                    Overall_change=("Overall_change","mean"),
                    R11_plus_change=("R11_plus_change","mean"),
                    Market_price_improvement=("Market_price_improvement","mean"),
                    Win_rate=("Counterfactual_win","mean"),
                    Nonloss=("Counterfactual_nonloss","mean")
                )
                st.markdown("#### Counterfactual value by round")
                st.dataframe(rnd.round(4),use_container_width=True,hide_index=True)

                slot=ch.groupby("Draft_slot",as_index=False).agg(
                    Decisions=("Behavior_changed","count"),
                    Overall_change=("Overall_change","mean"),
                    R11_plus_change=("R11_plus_change","mean"),
                    Win_rate=("Counterfactual_win","mean")
                )
                st.markdown("#### Counterfactual value by draft slot")
                st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

                room=ch.groupby("Room",as_index=False).agg(
                    Decisions=("Behavior_changed","count"),
                    Overall_change=("Overall_change","mean"),
                    R11_plus_change=("R11_plus_change","mean"),
                    Win_rate=("Counterfactual_win","mean")
                )
                st.markdown("#### Counterfactual value by room")
                st.dataframe(room.round(4),use_container_width=True,hide_index=True)

                win=float(ch.Counterfactual_win.mean())
                overall=float(ch.Overall_change.mean())
                r11=float(ch.R11_plus_change.mean())
                if overall>0 and r11>0 and win>=.55:
                    st.success(
                        "The forced-faller counterfactual is causally positive. V7.56 should derive a narrow live discriminator from the winning round/slot subsets rather than globally forcing fallers."
                    )
                elif r11>0 and overall<=0:
                    st.warning(
                        "Forcing the faller improves market value but hurts whole-draft quality. The internal FE valuation advantage is buying real roster value; do not add a crude market-price rule."
                    )
                else:
                    st.warning(
                        "The forced-faller counterfactual does not improve whole-draft outcomes. Keep V7.50 frozen and move the next diagnostic toward the internal late-round valuation/roster-utility components."
                    )
            else:
                st.info("No ≥15-reach / ≥10-faller decisions triggered in this sample.")

            st.download_button(
                "⬇️ Download V7.55 reach-vs-faller counterfactual CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_55_r11_reach_vs_faller_counterfactual.csv",
                "text/csv",key="v755_download"
            )
