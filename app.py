...
st.title("🏈 Fantasy Edge V7.44 — Pick 12 TE Utility De-duplication Calibration")
st.caption("V7.44 • V7.24 9× champion frozen • Pick 12 TE utility de-duplication calibration")
st.caption("Build ID: V7.44-P12TEUD-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v744_live_slot")
    ...
    if "v744_target_queue" not in st.session_state:
        st.session_state.v744_target_queue={}
    queue=st.session_state.v744_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v744_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.44 keeps the V7.24 9× champion completely frozen/live. Candidate TE utility multipliers run only inside the calibration matrix.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.44 robustness report")
        ...
        st.markdown("### 🧪 V7.44 champion baseline validation")
        st.info("V7.44 is calibration only. It leaves player score and upstream VORP untouched while reducing only the TE-starter VORP pathway inside roster utility.")
        ...
        if "v744_independent_results" not in st.session_state:
            st.session_state.v744_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v744_independent_results"),pd.DataFrame) else "ready to run"))
        if "v744_independent_summary" not in st.session_state:
            st.session_state.v744_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v744_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v744_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v744_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v744_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v744_independent_results=iv
                st.session_state.v744_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v744_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v744_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.44 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.44 calibrates only the TE-starter VORP contribution inside Pick-12 R4-6 marginal roster utility.")
            ...
            st.download_button(
                "⬇️ Download V7.44 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_44_champion_baseline.csv",
                "text/csv",
                key="v744_baseline_download"
            )
        ...
        st.markdown("### 🧪 V7.44 — Pick 12 TE Utility De-duplication Calibration")
        st.caption("Calibration only. V7.24 9× remains frozen/live. Only the TE STARTER VORP multiplier inside marginal roster utility changes at Pick 12 in Rounds 4-6; player score and upstream VORP remain untouched.")
        if "v744_results" not in st.session_state: st.session_state.v744_results=None
        if "v744_meta" not in st.session_state: st.session_state.v744_meta=None
        sims=st.number_input("TE de-duplication simulations per room",min_value=5,max_value=20,value=10,step=5,key="v744_sims")
        aa,ab=st.columns(2)
        run=aa.button("Run TE utility de-duplication matrix",type="primary",key="v744_run")
        clear=ab.button("Clear TE de-duplication matrix",key="v744_clear")
        if clear:
            st.session_state.v744_results=None; st.session_state.v744_meta=None; st.rerun()

        if run:
            configs=[
                ("CHAMPION • TE VORP .90",False,.90),
                ("A • TE VORP .75",True,.75),
                ("B • TE VORP .60",True,.60),
                ("C • TE VORP .45",True,.45),
                ("D • TE VORP .30",True,.30),
                ("E • TE VORP .00",True,.00),
            ]
            rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]
            fast_champ=_fast_benchmark_pool(board,int(teams)); rows=[]
            total=len(configs)*len(rooms)*int(sims); done=0
            prog=st.progress(0); status=st.empty(); started=time.perf_counter()

            for room_name in rooms:
                for j in range(int(sims)):
                    seed=944000+j*83+sum(ord(c) for c in room_name)
                    co=simulate_mock_fast(fast_champ,int(teams),12,15,state["roster_slots"],6.0,False,seed,room_name)
                    cc=independent_draft_grade_components(co,int(teams)); cop=independent_phase_metrics(co).set_index("Phase")
                    champ=simulate_mock_fast(fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,
                        p12_te_utility_dedup_enabled=False,p12_te_starter_vorp_mult=.90)
                    ...
                    for name,enabled,mult in configs:
                        fe=simulate_mock_fast(fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,
                            p12_te_utility_dedup_enabled=enabled,p12_te_starter_vorp_mult=mult)
                        fc=independent_draft_grade_components(fe,int(teams)); fep=independent_phase_metrics(fe).set_index("Phase")
                        r46=fe[pd.to_numeric(fe.mock_round,errors="coerce").fillna(99).between(4,6)]
                        changed=sum(champ_players.get(rr.mock_pick,None)!=rr.player for _,rr in fe.iterrows())
                        rows.append({
                            "Config":name,"TE_starter_vorp_multiplier":mult,"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "FE_grade":fc["overall"],"Consensus_grade":cc["overall"],"Difference":fc["overall"]-cc["overall"],
                            "Market_gap":fc["market_value"]-cc["market_value"],"Roster_balance_gap":fc["roster_balance"]-cc["roster_balance"],
                            "Changed_picks_vs_champion":changed,"Behavior_changed":changed>0,
                            ...
                            "R4_6_RB":int((r46.position=="RB").sum()),"R4_6_WR":int((r46.position=="WR").sum()),"R4_6_TE":int((r46.position=="TE").sum())
                        })
                        done+=1; prog.progress(done/total)
                        if done==1 or done%50==0 or done==total: status.caption(f"TE de-duplication matrix: {done}/{total}")
            df=pd.DataFrame(rows); st.session_state.v744_results=df
            st.session_state.v744_meta={"seconds":time.perf_counter()-started,"drafts":len(df)}
            status.success(f"TE de-duplication calibration complete: {len(df)} model drafts."); prog.empty()

        df=st.session_state.get("v744_results")
        if isinstance(df,pd.DataFrame) and len(df):
            summ=df.groupby(["Config","TE_starter_vorp_multiplier"],as_index=False).agg(
                Overall=("Difference","mean"),Market=("Market_gap","mean"),Roster=("Roster_balance_gap","mean"),
                R1_3=("R1_3_gap","mean"),R4_6=("R4_6_gap","mean"),R7_10=("R7_10_gap","mean"),R11_plus=("R11_plus_gap","mean"),
                Behavior_change_rate=("Behavior_changed","mean"),Changed_picks=("Changed_picks_vs_champion","mean"),
                RB=("R4_6_RB","mean"),WR=("R4_6_WR","mean"),TE=("R4_6_TE","mean"),Tests=("Simulation","count"))
            ctrl=summ[summ.Config.str.startswith("CHAMPION")].iloc[0]
            for m in ["Overall","R4_6","R7_10","R11_plus"]: summ[m+"_change"]=summ[m]-float(ctrl[m])
            summ["Score"]=summ.Overall_change+.40*summ.R4_6_change-.20*np.maximum(-summ.R7_10_change,0)-.10*np.maximum(-summ.R11_plus_change,0)
            summ=summ.sort_values(["Score","Overall_change","R4_6_change"],ascending=False).reset_index(drop=True)
            summ["Rank"]=np.arange(1,len(summ)+1)
            st.dataframe(summ[["Rank","Config","Score","Overall","Overall_change","R4_6","R4_6_change","R7_10","R7_10_change",
                "R11_plus","R11_plus_change","Behavior_change_rate","Changed_picks","Roster","RB","WR","TE"]].round(3),
                use_container_width=True,hide_index=True)

            leader=summ.iloc[0]
            room=df.groupby(["Config","Room"],as_index=False).agg(Overall=("Difference","mean"),R4_6=("R4_6_gap","mean"),
                R7_10=("R7_10_gap","mean"),Behavior_change=("Behavior_changed","mean"),Changed_picks=("Changed_picks_vs_champion","mean"))
            cr=room[room.Config.str.startswith("CHAMPION")][["Room","Overall","R4_6","R7_10"]].rename(columns={"Overall":"C_Overall","R4_6":"C_R4_6","R7_10":"C_R7_10"})
            lr=room[room.Config.eq(leader.Config)].merge(cr,on="Room")
            lr["Overall_change"]=lr.Overall-lr.C_Overall; lr["R4_6_change"]=lr.R4_6-lr.C_R4_6; lr["R7_10_change"]=lr.R7_10-lr.C_R7_10
            st.markdown("#### Calibration leader by room")
            st.dataframe(lr[["Room","Overall_change","R4_6_change","R7_10_change","Behavior_change","Changed_picks"]].round(3),use_container_width=True,hide_index=True)

            if str(leader.Config).startswith("CHAMPION"):
                st.warning("The frozen .90 champion leads. Do not reduce TE starter VORP inside roster utility.")
            else:
                positive_rooms=int((lr.Overall_change>0).sum()); behavior=float(leader.Behavior_change_rate)
                if leader.Overall_change>0 and leader.R4_6_change>0 and leader.R7_10_change>-0.35 and positive_rooms>=3 and behavior>=.10:
                    st.success(f"Screening leader: {leader.Config}. Move only this multiplier into V7.45 larger matched validation.")
                elif behavior<.10:
                    st.warning(f"{leader.Config} ranks highest among candidates but changes behavior in only {behavior:.0%} of drafts. Do not promote an inert calibration.")
                else:
                    st.warning(f"{leader.Config} leads the candidates but does not clear the screening gate. Keep V7.24 frozen.")

            st.download_button("⬇️ Download V7.44 Pick 12 TE de-duplication matrix CSV",df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_44_pick12_te_utility_dedup_matrix.csv","text/csv",key="v744_download")
