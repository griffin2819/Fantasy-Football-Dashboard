...
st.title("🏈 Fantasy Edge V7.28 — Replacement-Level / VORP Baseline Audit")
st.caption("V7.28 • V7.24 champion frozen • replacement/VORP baseline audit")
st.caption("Build ID: V7.28-RVBA-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v728_live_slot")
    ...
    if "v728_target_queue" not in st.session_state:
        st.session_state.v728_target_queue={}
    queue=st.session_state.v728_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v728_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.28 is diagnostic only. The V7.24 champion is frozen while the replacement-level and VORP baseline are stress-tested under nearby FLEX allocations and lower diagnostic VORP weights.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.28 robustness report")
        ...
        st.markdown("### 🧪 V7.28 replacement/VORP audit")
        st.info("V7.28 keeps the V7.24 champion frozen. The independent test now audits replacement-level/VORP sensitivity without changing selections.")
        ...
        if "v728_independent_results" not in st.session_state:
            st.session_state.v728_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v728_independent_results"),pd.DataFrame) else "ready to run"))
        if "v728_independent_summary" not in st.session_state:
            st.session_state.v728_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v728_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v728_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v728_run")
        clear_iv=clear_col.button("Clear independent results",key="v728_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v728_independent_results=iv
                st.session_state.v728_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v728_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v728_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.28 Replacement-Level / VORP Baseline Audit")
            st.caption("Diagnostic only. V7.24 champion logic is frozen. This tests whether the R4-6 RB/TE advantage is coming from the replacement baseline itself or simply from the 9× VORP multiplier.")

            r1,r2,r3,r4=st.columns(4)
            r1.metric("Chosen replacement rank",f"{iv_saved.MR28_chosen_replacement_rank.mean():.1f}")
            r2.metric("Alt replacement rank",f"{iv_saved.MR28_alt_replacement_rank.mean():.1f}")
            r3.metric("Replacement PPG gap",f"{iv_saved.MR28_base_replacement_gap.mean():+.2f}")
            r4.metric("Base VORP gap",f"{iv_saved.MR28_base_vorp_gap.mean():+.2f}")

            flex_df=pd.DataFrame([
                {"Diagnostic FLEX allocation":"38 RB / 55 WR / 7 TE (current)",
                 "RB/TE minus WR/QB VORP gap":iv_saved.MR28_base_vorp_gap.mean()},
                {"Diagnostic FLEX allocation":"45 RB / 45 WR / 10 TE",
                 "RB/TE minus WR/QB VORP gap":iv_saved.MR28_vorp_gap_45_45_10.mean()},
                {"Diagnostic FLEX allocation":"33 RB / 60 WR / 7 TE",
                 "RB/TE minus WR/QB VORP gap":iv_saved.MR28_vorp_gap_33_60_07.mean()},
                {"Diagnostic FLEX allocation":"30 RB / 60 WR / 10 TE",
                 "RB/TE minus WR/QB VORP gap":iv_saved.MR28_vorp_gap_30_60_10.mean()},
                {"Diagnostic FLEX allocation":"Equal thirds",
                 "RB/TE minus WR/QB VORP gap":iv_saved.MR28_vorp_gap_equal.mean()},
            ])
            st.markdown("#### Hypothetical replacement-baseline sensitivity")
            st.dataframe(flex_df.round(3),use_container_width=True,hide_index=True)

            weight_df=pd.DataFrame([
                {"VORP multiplier":"9× current","Equivalent gap":iv_saved.MR28_base_vorp_gap.mean()},
                {"VORP multiplier":"7× diagnostic","Equivalent gap":iv_saved.MR28_vorp_gap_weight7.mean()},
                {"VORP multiplier":"6× diagnostic","Equivalent gap":iv_saved.MR28_vorp_gap_weight6.mean()},
            ])
            st.markdown("#### VORP-weight sensitivity")
            st.dataframe(weight_df.round(3),use_container_width=True,hide_index=True)
            ...
            for ds,g in iv_saved.groupby("Draft_slot"):
                slot_rows.append({
                    ...
                    "Base replacement gap":g.MR28_base_replacement_gap.mean(),
                    "Base VORP gap":g.MR28_base_vorp_gap.mean(),
                    "45/45/10 VORP gap":g.MR28_vorp_gap_45_45_10.mean(),
                    "33/60/7 VORP gap":g.MR28_vorp_gap_33_60_07.mean(),
                    "Equal-flex VORP gap":g.MR28_vorp_gap_equal.mean(),
                })
            st.markdown("#### Replacement sensitivity by draft slot")
            st.dataframe(pd.DataFrame(slot_rows).round(3),use_container_width=True,hide_index=True)

            base_gap=float(iv_saved.MR28_base_vorp_gap.mean())
            candidates={
                "45/45/10 FLEX":float(iv_saved.MR28_vorp_gap_45_45_10.mean()),
                "33/60/7 FLEX":float(iv_saved.MR28_vorp_gap_33_60_07.mean()),
                "30/60/10 FLEX":float(iv_saved.MR28_vorp_gap_30_60_10.mean()),
                "equal FLEX":float(iv_saved.MR28_vorp_gap_equal.mean()),
            }
            best=min(candidates,key=lambda k:abs(candidates[k]))
            best_gap=candidates[best]
            flex_effect=abs(base_gap-best_gap)
            weight7=abs(base_gap-float(iv_saved.MR28_vorp_gap_weight7.mean()))
            weight6=abs(base_gap-float(iv_saved.MR28_vorp_gap_weight6.mean()))

            if flex_effect > max(weight7,weight6):
                st.warning(
                    f"Replacement-baseline sensitivity is dominant. {best} reduces the average R4-6 positional VORP gap "
                    f"from {base_gap:+.2f} to {best_gap:+.2f}. V7.29 should calibrate replacement/FLEX allocation before changing VORP weight."
                )
            else:
                st.warning(
                    f"The baseline allocation is not the only issue. Diagnostic VORP-weight changes have comparable or larger effect "
                    f"than nearby FLEX reallocations. V7.29 should run a matched calibration of replacement allocation versus VORP multiplier."
                )
            ...
            st.download_button(
                "⬇️ Download V7.28 replacement/VORP audit CSV",
                iv_csv,
                "fantasy_edge_v7_28_replacement_vorp_audit.csv",
                "text/csv",
                key="v728_download"
            )
