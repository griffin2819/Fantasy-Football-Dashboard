...
st.set_page_config(page_title="Fantasy Edge V7.36 — Early Injury Warning", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V7.35-EARLY-INJURY-WARNING"
...
_SYNC_STATUS_FILE = Path("fantasy_edge_sync_status.json")
...
def _v739_temporary_opportunity(add_row, roster_df):
...
def _v739_apply_temporary_opportunity(add_row, roster_df):
    """Copy a candidate row with only its *weekly* projection temporarily adjusted."""
    row=add_row.copy()
    boost,reason,source,confidence=_v739_temporary_opportunity(row,roster_df)
...
def _v737_required_position_emergencies(roster_df):
    """Return required starting positions that currently lack a playable option.

    This is deliberately availability-based, not value-based. A DOUBTFUL/OUT/reserve
    player does not count as safe coverage for the current week. QUESTIONABLE still
    counts as playable until stronger evidence says otherwise.
    """
...
def _v737_emergency_drop_ok(drop_row, starter_names, emergency_pos):
    ...
    We relax HOLD just enough to fill an otherwise empty required lineup slot, while
    retaining hard protection for verified reserve stashes, core assets, and the injured
    player whose temporary replacement is being added.
    """
    prot=str(_v682_drop_protection(drop_row) or "")
    pos=str(drop_row.get("position","") or "").upper()
    name=str(drop_row.get("player","") or "")
    cost=_v711_drop_asset_cost(drop_row,starter_names)
    # Never solve a TE contingency by cutting the only injured TE being covered.
    if pos==str(emergency_pos).upper():
        return False
    if "VERIFIED RESERVE" in prot or "CORE HOLD" in prot:
        return False
    # Extreme-value assets remain off limits even in an emergency.
    if cost>=35.0:
        return False
    return True
...
def _v738_emergency_drop_rank(drop_row, roster_df, starter_names, emergency_pos):
    ...
    Lower is better. Unlike raw Drop Cost, this adds roster construction: surplus
    bench IDP/depth is cheaper to cut, while starters and positions with no remaining
    playable cover are heavily protected. It never makes the emergency-position player
    itself the solution.
    """
    req={"QB":1,"RB":2,"WR":2,"TE":1,"K":1,"DL":1,"DB":1}
    bad={"OUT","DOUBTFUL","IR","PUP","NFI","SUSPENDED","DO NOT DRAFT"}
    ...
    if isinstance(roster_df,pd.DataFrame) and len(roster_df):
        ...
        if remaining < floor:
            ...
            # Surplus bench DL/DB/QB are intentionally easier to cut than RB/WR depth.
            if pos in ("DL","DB","QB"):
                score -= min(16.0,8.0+4.0*surplus)
            elif pos in ("RB","WR"):
                score -= min(7.0,2.0+2.0*surplus)
            elif pos=="K" and remaining>=1:
                score -= 5.0

    # Prefer-hold remains meaningful, but unlike CORE HOLD it can be broken for an
    # actual empty required slot if it is still the least damaging choice.
    if "PREFER HOLD" in prot:
        score += 6.0
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        _role_label=(
            ...
            f"Current role/usage: {_role_current_cov}% {'✓' if _role_current_cov>=30 else '⚠'}"
        )
        _feed_parts=[
        ...
        _role_bad=(not _pregame_role_state) and (_role_current_cov<30 and _role_hist_cov<30)
        ...
        st.caption("V7.39 waiver engine: V7.38 emergency drop ranking is retained, plus a temporary opportunity boost for direct same-team injury replacements. Weekly replacement value automatically falls away when the injured starter becomes playable again.")
        ...
        _fa_eval["_v739_opportunity_boost"]=_fa_eval.apply(lambda r:_v739_temporary_opportunity(r,_my_roster)[0],axis=1)
        _fa_eval["_v739_opportunity_reason"]=_fa_eval.apply(lambda r:_v739_temporary_opportunity(r,_my_roster)[1],axis=1)
        _fa_eval["_candidate_value"]=_fa_eval.apply(_v676_player_value,axis=1)+_fa_eval["_v739_opportunity_boost"]
        ...
        if not _allow_protected:
            # V7.08 hard guard: NEVER silently fall back to protected players.
            # If every roster asset is protected, there simply is no recommended drop
            # until the user explicitly enables Allow protected drops.
        ...
        if _eligible_drops.empty and not _allow_protected:
            st.info("All roster players are currently protected. No add/drop recommendation will use a protected player unless you enable Allow protected drops.")
        ...
        _v737_emergency_positions=_v737_required_position_emergencies(_my_roster)
        _v737_selected_emergency=(_add_pos in _v737_emergency_positions) if _add_pos!="All" else False
        ...
        if _v737_selected_emergency:
            ...
            _emergency_pool=_emergency_pool[_emergency_pool.apply(lambda r:_v737_emergency_drop_ok(r,_starter_names,_add_pos),axis=1)].copy()
            ...
            st.warning(f"🚨 REQUIRED POSITION COVERAGE — no safe playable {_add_pos} is currently available on your roster. Fantasy Edge is relaxing the normal HOLD gate only enough to evaluate the least-damaging contingency add/drop.")
        ...
        if not _allow_protected and not _v737_selected_emergency:
            _eligible_drops["_asset_hold"]=_eligible_drops["_drop_cost"].ge(20.0)
            _eligible_drops["_starter_hold"]=_eligible_drops["player"].astype(str).isin(_starter_names)
            _eligible_drops=_eligible_drops[~(_eligible_drops["_asset_hold"] | _eligible_drops["_starter_hold"])].copy()
        elif _v737_selected_emergency:
            _eligible_drops=_eligible_drops[_eligible_drops.apply(lambda r:_v737_emergency_drop_ok(r,_starter_names,_add_pos),axis=1)].copy()
        ...
        if _v737_selected_emergency and len(_eligible_drops):
            _eligible_drops["_emergency_drop_rank"]=_eligible_drops.apply(
                lambda r:_v738_emergency_drop_rank(r,_my_roster,_starter_names,_add_pos),axis=1)
        ...
        for _,add_raw in _fa_eval.iterrows():
            ...
            add=_v739_apply_temporary_opportunity(add_raw,_my_roster)
            ...
            if _opp_boost>0:
                _evidence=(str(_evidence)+f" • TEMP ROLE +{_opp_boost:.1f}").strip(" •")
            if _v737_selected_emergency:
            ...
            for _,drop in _drop_pool.iterrows():
                ...
                if _is_emergency_move and _v737_emergency_drop_ok(drop,_starter_names,add.get("position","")):
                    _drop_cost=_v711_drop_asset_cost(drop,_starter_names)
                    _viable=True; _hold_reason=""
                else:
                ...
                _moves.append({
                    ...
                    "Need Adj":_need_adj,"Need Fit":("REQUIRED POSITION COVERAGE" if _is_emergency_move else _need_label),"Evidence":_evidence,
        ...
        if _moves_df.empty:
            ...
            if not _allow_protected and len(_moves_df):
                ...
                _normal_ok=pd.to_numeric(_moves_df["Drop Cost"],errors="coerce").fillna(999).lt(20.0) & ~_moves_df["Drop"].astype(str).isin(_starter_names)
            ...
            if "Emergency Coverage" in _moves_df.columns and _moves_df["Emergency Coverage"].fillna(False).any():
                _moves_df=_moves_df.sort_values(["Emergency Drop Rank","Priority Score","Net Week","Net ROS"],ascending=[True,False,False,False])
            else:
            ...
            if _best.empty:
                ...
                if "Emergency Coverage" in _best.columns and _best["Emergency Coverage"].fillna(False).any():
                    _best=_best.sort_values(["Emergency Drop Rank","Priority Score","Net Week"],ascending=[True,False,False])
                else:
                ...
                st.dataframe(_best[["Add","Pos","Drop","Drop Pos","Drop Cost","Emergency Drop Rank","Priority Score","Waiver Call","FAAB","Claim Advice","Move Type","Need Fit","Evidence","Opportunity Boost","Effective Add PPG","Opportunity","Opportunity Confidence","Why Not Priority","Net Week","Net ROS","Add Role","Add Recent PPG","Add Status","Add Practice","Add Injury","Add Matchup","Drop Protection","Drop Status","Drop Practice","Drop Role","Drop Recent PPG","Add PPG","Drop PPG","Add ROS","Drop ROS","Add VORP","Drop VORP","Why"]].head(40),
                             use_container_width=True,hide_index=True)
...
def _v735_news_watch(board_df,state):
    my=board_df[board_df['player'].map(lambda n:_player_owner(state,n)=='1')].copy()
    if my.empty: return pd.DataFrame()
    # Poll single-depth required positions first. This keeps refreshes fast while catching
    # exactly the situations where breaking news requires an immediate contingency add.
    active=my[~my.apply(_v735_is_verified_reserve,axis=1)].copy()
    ...
    # Also include players with a real injury/practice concern. Healthy/LOW rows stay out.
    concern=my[my.apply(_v735_has_injury_concern,axis=1)]
    watch=pd.concat([watch,concern],ignore_index=True).drop_duplicates('player').head(8)
    rows=[]
    for _,r in watch.iterrows():
        player=str(r.get('player','')); pos=str(r.get('position',''))
        official=_v677_normalize_injury_status(r.get('injury_effective',r.get('injury','')))
        for n in _v735_google_news_player(player):
            text=n.pop('_text','')
            sev='HIGH' if any(k in text for k in _NEWS_HIGH_TERMS) else 'ELEVATED'
            m=re.search(r'(?:miss|out)[^.!]{0,35}?(\d+|one|two)\s+(?:game|week)',text)
            expected=(m.group(0).strip() if m else 'reported availability concern')
            rows.append({**n,"Pos":pos,"News Risk":sev,"Official Status":official,"Expected":expected,"Depth":int(counts.get(pos,0))})
    if not rows: return pd.DataFrame()
    out=pd.DataFrame(rows)
    out['_sev']=out['News Risk'].map({'HIGH':0,'ELEVATED':1}).fillna(9)
    return out.sort_values(['_sev','Player']).drop_duplicates('Player').drop(columns='_sev')
