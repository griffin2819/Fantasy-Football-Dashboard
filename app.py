...
st.set_page_config(page_title="Fantasy Edge V6.57 — Live State Sync + Canonical Availability", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V6.57-LIVE-STATE-CANONICAL-AVAILABILITY-396"
...
with tabs[1]:
    ...
    _taken_live_keys={norm(n) for n in state.get("taken",[]) if norm(n)}
    ...
    with st.expander("📚 Full available player database", expanded=False):
        _taken_db_keys={norm(n) for n in state.get("taken",[]) if norm(n)}
