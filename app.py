...
st.set_page_config(page_title="Fantasy Edge V6.40 — Atomic Pick Form Fix", page_icon="🏈", layout="wide")
...
def _v645_razzball_projection_table():
    ...
    Returns player/position/team plus STD, half-PPR and PPR season points.
    No rank-derived fantasy point estimates are created here.
    """
    import io
    ...
    target=None
    for t in tables:
        # Flatten multi-level headings when present.
        tt=t.copy()
...
def _v645_apply_published_projections(board, ppr, teams, slots):
    """Overlay published season projections and rebuild VORP/model values.

    Projection shown in Fantasy Edge is PPG (season projection divided by 17).
    Replacement levels are recalculated from the actual league size/starting slots.
    IDP stays on the league's custom IDP model rather than importing incompatible
    provider scoring.
    """
    if board is None or board.empty:
        return board,0,''
    try:
        proj=_v645_razzball_projection_table()
    except Exception as e:
        return board,0,f'{type(e).__name__}: {e}'
    if proj is None or proj.empty:
        return board,0,'projection table empty'

    x=board.copy()
    if 'key' not in x.columns:
        x['key']=x['player'].astype(str).map(norm)
    # Calculate season points for the app's reception scoring. Standard + PPR
    # reception premium gives a clean interpolation for 0.0-1.0 PPR settings.
    pr=proj.copy()
    _ppr=float(ppr)
    std=pd.to_numeric(pr['std_points'],errors='coerce')
    full=pd.to_numeric(pr['ppr_points'],errors='coerce')
    half=pd.to_numeric(pr['half_ppr_points'],errors='coerce')
    calc=std + _ppr*(full-std)
    # If a row/source omits standard, use half/full rather than discarding it.
    ...
    mp=pr.set_index('key')
    eligible=x['position'].isin(['QB','RB','WR','TE','K']) & x['key'].isin(mp.index)
    if not eligible.any():
        return x,0,'no player-name matches'
    idx=x.index[eligible]
    vals=x.loc[idx,'key'].map(mp['published_ppg'])
    x.loc[idx,'projection']=pd.to_numeric(vals,errors='coerce').values
    ...
    x.loc[idx,'projection_source']='Razzball 2026 published season projection'
    ...
    repl={}
    for pos,n in replacement_slots.items():
        vals=pd.to_numeric(x.loc[x['position'].eq(pos),'projection'],errors='coerce').dropna().sort_values(ascending=False).reset_index(drop=True)
        repl[pos]=float(vals.iloc[min(max(int(n)-1,0),len(vals)-1)]) if len(vals) else 0.0
    off=x['position'].isin(replacement_slots)
    x.loc[off,'replacement_ppg']=x.loc[off,'position'].map(repl).fillna(0.0)
    x.loc[off,'vorp']=pd.to_numeric(x.loc[off,'projection'],errors='coerce').fillna(0)-pd.to_numeric(x.loc[off,'replacement_ppg'],errors='coerce').fillna(0)
    x.loc[off,'display_vorp']=x.loc[off,'vorp']
    ...
    # Rebuild offensive model scores from actual projection/VORP evidence.
    ...
    x.loc[off,'pure_model_score']=pure.loc[off]
    x.loc[off,'draft_score']=pure.loc[off]+cs.loc[off]*.10
    # Rank the complete board after the new scoring evidence is installed.
    x['model_rank']=pd.to_numeric(x['draft_score'],errors='coerce').rank(method='min',ascending=False)
    x['display_model_rank']=x['model_rank']
    return x,int(eligible.sum()),''
...
def _fast_local_draft_board():
    ...
    try:
        _v644_added=pd.read_csv(BASE / "v644_added_players.csv")
...
if _v645_projection_matches:
    st.sidebar.success(f"Published projections loaded: {_v645_projection_matches} players • VORP recalculated")
else:
...
if _v646_projection_matches:
    st.sidebar.success(f"Final projection pass: {_v646_projection_matches} players • full pool VORP recalculated")
...
# V6.46: NEVER erase recorded live-draft state just because a temporary player
# feed/board rebuild does not contain a name. Roster-only picks are legitimate.
all_names=board["player"].astype(str).tolist()
state["my_team"]=list(dict.fromkeys(map(str,state.get("my_team",[]) or [])))
state["taken"]=list(dict.fromkeys(map(str,state.get("taken",[]) or [])))
...
with tabs[1]:
    ...
    # V6.47: DISPLAY POOL != RECOMMENDATION POOL.
    # The prior Live Draft table was rendered directly from `x`, which is the
    # recommendation-eligibility pool. That made legitimate production-board
    # players disappear from the visible board whenever roster/feasibility/value
    # gates suppressed them. Build the visible board from every undrafted
    # production player, then overlay recommendation-engine metrics where present.
    _taken_display=set(map(str,state.get("taken",[])))
    _display_pool=board[~board["player"].astype(str).isin(_taken_display)].copy()
    ...
    # Stable display ordering: recommendation candidates first, then the full
    # database by model rank / market pick. No row cap: users must be able to
    # find every undrafted player on the main board.
    _display_pool["_rec_sort"]=_display_pool["recommendation_pool"].eq("Yes").astype(int)
    ...
    _display_pool=_display_pool.sort_values(
        ["_rec_sort","_model_sort","_market_sort","projection"],
    ...
    _eval_rank_display=pd.to_numeric(show["evaluation_rank"],errors="coerce")
    show["Model target #"]=_eval_rank_display.map(
    ...
    for _c,_default in _draft_display_defaults.items():
        if _c not in show.columns:
            show[_c]=_default
    st.caption(f"Showing {len(show):,} undrafted players from the full production database. ‘Recommendation pool’ shows whether the decision engine currently considers that player eligible, but filtered players remain visible.")
    st.dataframe(
        show[["player","position","team","Model target #","Action","Market pick","Faller picks",
              "Survives next","projection","vorp","idp_external_rank","idp_impact_score","roster_opportunity_adj","recommendation_pool","Recommendation reason",
    ...
    _record_names=set(board.loc[~board["player"].astype(str).isin(_taken_names),"player"].astype(str).tolist())
    if isinstance(_active_roster_export,pd.DataFrame) and len(_active_roster_export):
        _record_names.update(
            _active_roster_export.loc[
                ~_active_roster_export["player"].astype(str).isin(_taken_names),"player"
            ].astype(str).tolist()
        )
    # V6.39: preserve the currently selected roster-directory player across the
    # Streamlit button rerun. A roster-only name must remain recordable even if
    # the external roster sync transiently fails on the exact submit rerun.
    _session_pick=str(st.session_state.get("v634_record_pick","") or "").strip()
    if _session_pick:
        _record_names.add(_session_pick)
    _record_options=sorted(_record_names,key=str.lower)
    ...
    if _record_clicked:
        ...
        elif _commit_pick in set(map(str,state.get("taken",[]))):
            st.warning(f"{_commit_pick} is already marked as drafted.")
        else:
            if pick_owner=="Mine":
