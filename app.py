...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Mobile Nav + Availability Authority", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v783_authoritative_week_status(row):
    ...
    Reserve/out designations from the reconciled injury layer are hard authority.
    A stale canonical ACTIVE value must never override an OUT/PUP/IR/NFI/SUSPENDED
    status that already exists in injury_effective. This closes the Branch-on-PUP
    leak that could still reach Start/Sit and the Unified Decision Engine.
    """
    hard_order=("SUSPENDED","NFI","PUP","IR","OUT","DO NOT DRAFT")
    candidates=[]
    for key in ("injury_effective","fe_status","injury"):
        try:
    ...
    # Any reconciled hard-unavailable designation wins over ACTIVE/QUESTIONABLE.
    for status in hard_order:
        if status in candidates:
            return status
    # Otherwise prefer canonical fe_status, then effective injury, then raw injury.
    for key in ("fe_status","injury_effective","injury"):
        try:
            val=_v677_normalize_injury_status(row.get(key,""))
        except Exception:
            val="ACTIVE"
...
def _v781_render_page(_v777_page_idx):
    ...
    if _v777_page_idx==6:
        ...
        if _my.empty:
            ...
            if _decision_rows:
                ...
                st.dataframe(_dec[["Slot","Player","Decision","Confidence","Alternative"]],use_container_width=True,hide_index=True)
            else:
            ...
            if len(_unavailable_rows):
                ...
                st.dataframe(pd.DataFrame(_u),use_container_width=True,hide_index=True)

            if _locked_rows:
                with st.expander(f"✅ Locked / clear starters ({len(_locked_rows)})",expanded=False):
                    _lk=pd.DataFrame(_locked_rows)
                    st.dataframe(_lk[["Slot","Player","Status","Practice","Proj PPG"]],use_container_width=True,hide_index=True)
            ...
            st.caption(f"Fantasy Edge V7.76 • {_v677_season} Week {_v677_week} • Decision-quality Start/Sit with authoritative availability, uncertainty-aware close calls, and separate unavailable/monitor groups.")
    ...
    if _v777_page_idx==14:
        ...
        if my.empty:
            ...
            @media(max-width:700px){{.fecc-brand{{font-size:1.28rem}}.fecc-grid{{grid-template-columns:repeat(2,minmax(0,1fr));gap:.48rem}}.fecc-metric{{min-height:76px;padding:.58rem .62rem}}.fecc-metric .v{{font-size:1.2rem}}.fecc-footer-grid{{grid-template-columns:1fr}}.fecc-status{{padding:.7rem .75rem}}}}
            </style>
            ...
            if _lineup_rows:
                ...
                for x in _lineup_rows:
                    ...
                    _trs.append(f"<tr><td>{x['Slot']}</td><td>{x['Player']}</td><td>{x['Pos']}</td><td><b>{x['PPG']:.2f}</b></td><td>{x['Base']:.2f}</td><td>{x['Range']}</td><td><span class='fecc-badge {_ccls}'>{int(x['Conf'])}%</span></td><td>{x['Status']}</td></tr>")
                st.markdown("<div class='fecc-scroll'><table class='fecc-table'><thead><tr><th>Slot</th><th>Player</th><th>Pos</th><th>Adj PPG</th><th>Base</th><th>Range</th><th>Confidence</th><th>Status</th></tr></thead><tbody>"+"".join(_trs)+"</tbody></table></div>",unsafe_allow_html=True)
            ...
            if _inj_rows:
                ...
                for x in _inj_rows:
                    ...
                    _trs.append(f"<tr><td>{x['Player']}</td><td>{x['Pos']}</td><td><b>{status}</b></td><td>{x['Est. Return']}</td><td><span class='fecc-badge {cls}'>{x['Impact']}</span></td><td><span class='fecc-badge'>{x['Recommendation']}</span></td></tr>")
                st.markdown("<div class='fecc-scroll'><table class='fecc-table'><thead><tr><th>Player</th><th>Pos</th><th>Status</th><th>Est. Return</th><th>Impact</th><th>Recommendation</th></tr></thead><tbody>"+"".join(_trs)+"</tbody></table></div>",unsafe_allow_html=True)
            else:
