...
st.title("🏈 Fantasy Edge V7.67 — Production Round-7 Dominance Champion")
st.caption("V7.67 • promoted Round-7 dominance champion • optimized certification")
st.caption("Build ID: V7.67-R7-DOMINANCE-CHAMPION-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.67 promotes the exact frozen Round-7 dominance rule into the live/default production path with no additional tuning.")
        ...
        st.markdown("### 👑 V7.67 — Production Champion Certification")
        st.caption(
            "The exact V7.64–V7.66 Round-7 dominance rule is now promoted into the live/default production path with zero retuning. "
            "This optimized certification compares historical V7.60 against production V7.67 and reruns V7.67 only when the frozen trigger fires."
        )

        if "v767_results" not in st.session_state:
            st.session_state.v767_results=None

        sims=st.number_input(
            "V7.67 certification simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v767_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 matched sets. Trigger-first execution avoids unnecessary second simulations."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.67 production certification",type="primary",key="v767_run")
        clear=b.button("Clear V7.67 certification",key="v767_clear")
        if clear:
            st.session_state.v767_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=967000+int(ds)*100000+j*197+sum(ord(c) for c in room_name)

                        # Historical V7.60 path: explicitly disable the promoted rule.
                        oldd=simulate_mock_fast(
                            pool,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name,
                            r7_dominance_override_enabled=False
                        )

                        r7=oldd[pd.to_numeric(oldd.mock_round,errors="coerce").eq(7)]
                        trigger=False; old7=""; old7pos=""
                        if len(r7):
                            rr=r7.iloc[0]
                            trigger=bool(int(pd.to_numeric(pd.Series([rr.get("R766_Would_trigger",0)]),errors="coerce").fillna(0).iloc[0]))
                            old7=str(rr.player); old7pos=str(rr.position)

                        if trigger:
                            # Production/default V7.67 path.
                            newd=simulate_mock_fast(
                                pool,int(teams),int(ds),15,state["roster_slots"],
                                6.0,True,seed,room_name
                            )
                            reruns+=1
                            ...
                            old_players=oldd.set_index("mock_pick")["player"].to_dict()
                            changed=sum(old_players.get(rr.mock_pick,None)!=rr.player for _,rr in newd.iterrows())

                            n7=newd[pd.to_numeric(newd.mock_round,errors="coerce").eq(7)]
                            new7=str(n7.iloc[0].player) if len(n7) else ""
                            new7pos=str(n7.iloc[0].position) if len(n7) else ""

                            overall=nc["overall"]-oc["overall"]
                            r13=float(np_.loc["R1-3","Avg_market_delta"])-float(op.loc["R1-3","Avg_market_delta"])
                            r46=float(np_.loc["R4-6","Avg_market_delta"])-float(op.loc["R4-6","Avg_market_delta"])
                            r710=float(np_.loc["R7-10","Avg_market_delta"])-float(op.loc["R7-10","Avg_market_delta"])
                            r11=float(np_.loc["R11+","Avg_market_delta"])-float(op.loc["R11+","Avg_market_delta"])
                        else:
                            changed=0; new7=old7; new7pos=old7pos
                            overall=r13=r46=r710=r11=0.0

                        rows.append({
                            "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Trigger_detected":trigger,
                            "Production_rerun":trigger,
                            "Behavior_changed":changed>0,
                            "Changed_picks":changed,
                            "Historical_V760_round7_player":old7,
                            "Historical_V760_round7_position":old7pos,
                            "Production_V767_round7_player":new7,
                            "Production_V767_round7_position":new7pos,
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.67 certification: {done}/{total} matched sets • production reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v767_results=df
            status.success(
                f"V7.67 certification complete: {len(df)} matched sets using "
                f"{len(df)+int(df.Production_rerun.sum())} full simulations instead of {len(df)*2}."
            )
            prog.empty()

        df=st.session_state.get("v767_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            cond710=trig.Conditional_R7_10_win.mean() if len(trig) else np.nan

            full_sims=len(df)+int(df.Production_rerun.sum())
            ...
            st.markdown("#### Production certification by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Production certification by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            paths=trig.groupby(
                ["Historical_V760_round7_player","Historical_V760_round7_position",
                 "Production_V767_round7_player","Production_V767_round7_position"],
                as_index=False
            ).agg(
            ...
            st.markdown("#### Production Round-7 paths")
            ...
            # Production certification focuses on reproducibility of the already frozen rule.
            certified=(
                overall>0
                and r710>0
                and r13>-0.03
                ...
                and r11>-0.08
                and activation>=.05
                and pd.notna(cond) and cond>=.95
                and pd.notna(nonloss) and nonloss>=.97
            ...
            if certified:
                st.success(
                    "V7.67 PRODUCTION CHAMPION CERTIFIED. The promoted/default path reproduces the frozen Round-7 dominance advantage. V7.67 is the new champion."
                )
            else:
                st.error(
                    "V7.67 production-path certification failed. Revert the default Round-7 override to OFF and retain V7.60 as champion."
                )

            st.download_button(
                "⬇️ Download V7.67 production champion certification CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_67_production_champion_certification.csv",
                "text/csv",key="v767_download"
            )
