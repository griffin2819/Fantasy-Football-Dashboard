...
st.title("🏈 Fantasy Edge V7.45 — Pick 12 TE Utility Finalist Validation")
st.caption("V7.45 • V7.24 9× champion retained • Pick 12 TE utility finalist validation")
st.caption("Build ID: V7.45-P12TEFV-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v745_live_slot")
    ...
    if "v745_target_queue" not in st.session_state:
        st.session_state.v745_target_queue={}
    queue=st.session_state.v745_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v745_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.45 keeps the V7.24 9× champion live/default. The .45 and .30 finalists run only inside the validation matrix.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.45 robustness report")
        ...
        st.markdown("### 🧪 V7.45 champion baseline validation")
        st.info("V7.45 is validation only. No additional tuning is introduced; it compares the frozen .90 champion directly against the .45 and .30 finalists.")
        ...
        if "v745_independent_results" not in st.session_state:
            st.session_state.v745_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v745_independent_results"),pd.DataFrame) else "ready to run"))
        if "v745_independent_summary" not in st.session_state:
            st.session_state.v745_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v745_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v745_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v745_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v745_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v745_independent_results=iv
                st.session_state.v745_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v745_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v745_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.45 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.45 performs large matched validation of the .45 and .30 Pick-12 R4-6 TE utility finalists.")
            ...
            st.download_button(
                "⬇️ Download V7.45 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_45_champion_baseline.csv",
                "text/csv",
                key="v745_baseline_download"
            )
        ...
        st.markdown("### 🏁 V7.45 — Pick 12 TE Utility Finalist Validation")
        st.caption(
            "Large matched validation of the two V7.44 finalists. V7.24 9× remains live/default. "
            "Only the Pick-12 R4-6 TE-starter VORP multiplier inside marginal roster utility differs."
        )

        if "v745_results" not in st.session_state:
            st.session_state.v745_results=None
        if "v745_meta" not in st.session_state:
            st.session_state.v745_meta=None

        sims=st.number_input(
            "Validation simulations per room",
            min_value=20,max_value=75,value=50,step=5,key="v745_sims",
            help="Default 50 × 5 rooms × 3 configs = 750 model drafts."
        )
        ca,cb=st.columns(2)
        run=ca.button("Run V7.45 finalist validation",type="primary",key="v745_run")
        clear=cb.button("Clear finalist validation",key="v745_clear")
        ...
        if run:
            configs=[
                ("V7.24 CHAMPION • TE VORP .90",False,.90),
                ("V7.45 BALANCED • TE VORP .45",True,.45),
                ("V7.45 AGGRESSIVE • TE VORP .30",True,.30),
            ]
            rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]
            fast_champ=_fast_benchmark_pool(board,int(teams))
            rows=[]
            total=len(configs)*len(rooms)*int(sims)
            done=0
            prog=st.progress(0)
            status=st.empty()
            ...
            for room_name in rooms:
                for j in range(int(sims)):
                    seed=945000+j*89+sum(ord(c) for c in room_name)
                    ...
                    champ=simulate_mock_fast(
                        ...
                        p12_te_utility_dedup_enabled=False,
                        p12_te_starter_vorp_mult=.90
                    )
                    champ_players=champ.set_index("mock_pick")["player"].to_dict()

                    for name,enabled,mult in configs:
                        fe=simulate_mock_fast(
                            fast_champ,int(teams),12,15,state["roster_slots"],
                            6.0,True,seed,room_name,
                            p12_te_utility_dedup_enabled=enabled,
                            p12_te_starter_vorp_mult=mult
                        )
                        fc=independent_draft_grade_components(fe,int(teams))
                        fep=independent_phase_metrics(fe).set_index("Phase")
                        ...
                        changed=0
                        for _,rr in fe.iterrows():
                            if champ_players.get(rr.mock_pick,None)!=rr.player:
                                changed+=1

                        rows.append({
                            "Config":name,
                            "TE_starter_vorp_multiplier":mult,
                            "Room":room_name,
                            "Simulation":j+1,
                            "Seed":seed,
                            "FE_grade":fc["overall"],
                            ...
                            "Market_gap":fc["market_value"]-cc["market_value"],
                            "Roster_balance_gap":fc["roster_balance"]-cc["roster_balance"],
                            "Starter_completion_gap":fc["starter_completion"]-cc["starter_completion"],
                            "Availability_gap":fc["availability"]-cc["availability"],
                            "Changed_picks_vs_champion":changed,
                            "Behavior_changed":changed>0,
                            "R1_3_gap":float(fep.loc["R1-3","Avg_market_delta"])-float(cop.loc["R1-3","Avg_market_delta"]),
                            "R4_6_gap":float(fep.loc["R4-6","Avg_market_delta"])-float(cop.loc["R4-6","Avg_market_delta"]),
                            "R7_10_gap":float(fep.loc["R7-10","Avg_market_delta"])-float(cop.loc["R7-10","Avg_market_delta"]),
                            "R11_plus_gap":float(fep.loc["R11+","Avg_market_delta"])-float(cop.loc["R11+","Avg_market_delta"]),
                            "R4_6_RB":int((r46.position=="RB").sum()),
                            "R4_6_WR":int((r46.position=="WR").sum()),
                            "R4_6_TE":int((r46.position=="TE").sum()),
                        })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%75==0 or done==total:
                            status.caption(f"Finalist validation: {done}/{total}")

            df=pd.DataFrame(rows)
            st.session_state.v745_results=df
            st.session_state.v745_meta={
                "seconds":time.perf_counter()-started,
                "drafts":len(df),
                "matched_sets":len(df)//3
            }
            status.success(f"V7.45 validation complete: {len(df)} model drafts.")
            prog.empty()

        df=st.session_state.get("v745_results")
        meta=st.session_state.get("v745_meta") or {}

        if isinstance(df,pd.DataFrame) and len(df):
            summ=df.groupby(["Config","TE_starter_vorp_multiplier"],as_index=False).agg(
                Overall=("Difference","mean"),
                FE_grade=("FE_grade","mean"),
                Win_rate=("FE_win","mean"),
                Market=("Market_gap","mean"),
                Roster=("Roster_balance_gap","mean"),
                Starter=("Starter_completion_gap","mean"),
                Availability=("Availability_gap","mean"),
                R1_3=("R1_3_gap","mean"),
                R4_6=("R4_6_gap","mean"),
                R7_10=("R7_10_gap","mean"),
                R11_plus=("R11_plus_gap","mean"),
                Behavior_change_rate=("Behavior_changed","mean"),
                Changed_picks=("Changed_picks_vs_champion","mean"),
                RB=("R4_6_RB","mean"),
                WR=("R4_6_WR","mean"),
                TE=("R4_6_TE","mean"),
                Tests=("Simulation","count")
            ...
            ctrl=summ[summ.Config.str.startswith("V7.24 CHAMPION")].iloc[0]
            for m in ["Overall","R4_6","R7_10","R11_plus"]:
                summ[m+"_change"]=summ[m]-float(ctrl[m])

            summ["Validation_score"]=(
                summ.Overall_change
                +.40*summ.R4_6_change
                -.25*np.maximum(-summ.R7_10_change,0)
                -.10*np.maximum(-summ.R11_plus_change,0)
            )
            summ=summ.sort_values(
                ["Validation_score","Overall_change","R4_6_change"],ascending=False
            ).reset_index(drop=True)
            summ["Rank"]=np.arange(1,len(summ)+1)

            st.dataframe(
                summ[[
                    "Rank","Config","Validation_score",
                    "Overall","Overall_change",
                    "R4_6","R4_6_change",
                    "R7_10","R7_10_change",
                    "R11_plus","R11_plus_change",
                    "Behavior_change_rate","Changed_picks",
                    "Roster","RB","WR","TE"
                ]].round(3),
                use_container_width=True,hide_index=True
            )

            champ_name="V7.24 CHAMPION • TE VORP .90"
            pair=df.pivot_table(
                index=["Room","Simulation","Seed"],
                columns="Config",
                values=["Difference","R4_6_gap","R7_10_gap","R11_plus_gap"],
                aggfunc="first"
            )

            metrics=[]
            for candidate in [
                "V7.45 BALANCED • TE VORP .45",
                "V7.45 AGGRESSIVE • TE VORP .30"
            ]:
                od=pair[("Difference",candidate)]-pair[("Difference",champ_name)]
                md=pair[("R4_6_gap",candidate)]-pair[("R4_6_gap",champ_name)]
                rd=pair[("R7_10_gap",candidate)]-pair[("R7_10_gap",champ_name)]
                metrics.append({
                    "Candidate":candidate,
                    "Paired overall wins":(od>0).mean(),
                    "Paired overall equal/better":(od>=0).mean(),
                    "Paired R4-6 improves":(md>0).mean(),
                    "Paired R7-10 improves":(rd>0).mean(),
                    "Mean paired overall change":od.mean(),
                    "Mean paired R4-6 change":md.mean(),
                    "Mean paired R7-10 change":rd.mean()
                })

            st.markdown("#### Paired finalist comparison")
            pair_df=pd.DataFrame(metrics)
            st.dataframe(pair_df.round(3),use_container_width=True,hide_index=True)

            room=df.groupby(["Config","Room"],as_index=False).agg(
                Overall=("Difference","mean"),
                R4_6=("R4_6_gap","mean"),
                R7_10=("R7_10_gap","mean"),
                Behavior_change=("Behavior_changed","mean"),
                Changed_picks=("Changed_picks_vs_champion","mean")
            )
            croom=room[room.Config.eq(champ_name)][["Room","Overall","R4_6","R7_10"]].rename(
                columns={"Overall":"C_Overall","R4_6":"C_R4_6","R7_10":"C_R7_10"}
            )
            ...
            for candidate in [
                ...
                rr=room[room.Config.eq(candidate)].merge(croom,on="Room")
                rr["Overall_change"]=rr.Overall-rr.C_Overall
                rr["R4_6_change"]=rr.R4_6-rr.C_R4_6
                rr["R7_10_change"]=rr.R7_10-rr.C_R7_10
                rr["Candidate"]=candidate
                room_rows.append(rr)

            room_eval=pd.concat(room_rows,ignore_index=True)
            st.markdown("#### Finalists by room profile")
            st.dataframe(
                room_eval[[
                    "Candidate","Room","Overall_change","R4_6_change",
                    "R7_10_change","Behavior_change","Changed_picks"
                ]].round(3),
                use_container_width=True,hide_index=True
            )

            gate_rows=[]
            for _,r in summ[~summ.Config.str.startswith("V7.24 CHAMPION")].iterrows():
                candidate=str(r.Config)
                pr=pair_df[pair_df.Candidate.eq(candidate)].iloc[0]
                rr=room_eval[room_eval.Candidate.eq(candidate)]
                positive_rooms=int((rr.Overall_change>0).sum())
                nonnegative_mid=int((rr.R4_6_change>=0).sum())
                worst_room=float(rr.Overall_change.min())
                passed=(
                    r.Overall_change>0
                    and r.R4_6_change>0
                    and r.R7_10_change>-0.20
                    and r.R11_plus_change>-0.25
                    and pr["Paired overall wins"]>=.50
                    and pr["Paired R4-6 improves"]>=.50
                    and positive_rooms>=4
                    and nonnegative_mid>=4
                    and worst_room>-0.15
                )
                gate_rows.append({
                    "Candidate":candidate,
                    "Gate passed":passed,
                    "Positive overall rooms":positive_rooms,
                    "Non-negative R4-6 rooms":nonnegative_mid,
                    "Worst room overall":worst_room,
                    "Paired overall wins":pr["Paired overall wins"],
                    "Paired R4-6 improves":pr["Paired R4-6 improves"],
                    "R7-10 change":r.R7_10_change
                })

            gates=pd.DataFrame(gate_rows)
            st.markdown("#### Promotion gate")
            st.dataframe(gates.round(3),use_container_width=True,hide_index=True)

            passed=gates[gates["Gate passed"]]
            if len(passed)==1:
                winner=passed.iloc[0]["Candidate"]
                st.success(
                    f"PROMOTION GATE PASSED: {winner}. V7.46 can promote only this Pick-12 R4-6 TE utility multiplier into the live champion."
                )
            elif len(passed)>1:
                # Prefer stronger validation score if both pass.
                eligible=summ[summ.Config.isin(passed.Candidate)]
                winner=eligible.sort_values("Validation_score",ascending=False).iloc[0].Config
                st.success(
                    f"Both finalists pass. Preferred promotion candidate: {winner} based on the stronger protected validation score."
                )
            else:
                st.warning(
                    "Neither finalist passes the full robustness gate. Keep the V7.24 champion and do not continue tuning this TE utility multiplier around noise."
                )

            st.success(
                f"Saved V7.45 validation: {meta.get('drafts',len(df))} model drafts"
                + (f" • {meta.get('seconds',0):.1f}s" if meta else "")
            )

            st.download_button(
                "⬇️ Download V7.45 TE finalist validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_45_pick12_te_utility_finalist_validation.csv",
                "text/csv",key="v745_download"
            )
