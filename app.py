...
st.title("🏈 Fantasy Edge V7.92.1 — Resumable Round-2 Validation")
st.caption("V7.92.1 • V7.67 frozen champion • resumable Round-2 validation")
st.caption("Build ID: V7.92.1-R2-RESUMABLE-VALIDATION-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### ⚡ V7.92.1 — Resumable Frozen Round-2 Validation")
        st.caption("Same exact frozen V7.91 rule, but split into small resumable batches so Streamlit does not time out. V7.67 remains frozen/live.")

        if "v7921_results" not in st.session_state:
            st.session_state.v7921_results=pd.DataFrame()
        if "v7921_next_index" not in st.session_state:
            st.session_state.v7921_next_index=0

        target_sets=st.selectbox("Validation target",[1200,1800,2400],index=2,key="v7921_target")
        batch_per_combo=st.number_input("Batch size per slot × room",2,10,5,1,key="v7921_batch",
            help="5 = 300 matched sets per click. Use 2-3 if your host still times out.")

        a,b=st.columns(2)
        run=a.button("Run next batch",type="primary",key="v7921_run")
        clear=b.button("Reset V7.92.1",key="v7921_clear")
        if clear:
            st.session_state.v7921_results=pd.DataFrame()
            st.session_state.v7921_next_index=0
            st.rerun()
        ...
        combos=[(ds,r) for ds in range(1,int(teams)+1) for r in rooms]
        batch_sets=len(combos)*int(batch_per_combo)

        if run:
            existing=st.session_state.v7921_results
            ...
            if this_batch>0:
                ...
                start_index=int(st.session_state.v7921_next_index)
                ...
                for n in range(this_batch):
                    ...
                    seed=992100000+int(ds)*100000+int(cycle)*353+sum(ord(ch) for ch in room_name)
                    ...
                    champ=simulate_mock_fast(
                        ...
                        late_sequence_variant="champion",r2_generic_adaptation_enabled=False)
                    ...
                    if len(rr):
                        ...
                        normal=str(r.get("R791_Normal_player","")); alt=str(r.get("R791_Alt_player","")); altpos=str(r.get("R791_Alt_position",""))
                    ...
                    rows.append({"Global_index":gi,"Draft_slot":int(ds),"Room":room_name,"Cycle":cycle+1,"Seed":seed,
                        "Trigger_detected":trigger,"Normal_R2_player":normal,"Alternative_R2_player":alt,"Alternative_position":altpos,
                        "Changed_picks":changed,"Overall_change":overall,"R1_3_change":r13,"R4_6_change":r46,
                ...
                combined=combined.drop_duplicates("Global_index").sort_values("Global_index").reset_index(drop=True)
                st.session_state.v7921_results=combined
                st.session_state.v7921_next_index=start_index+this_batch
                status.success(f"Batch complete. Saved {len(combined):,}/{target_sets:,}. Run the next batch when ready.")
                prog.empty()
            else:
                st.success(f"Target complete: {already:,} matched sets.")

        df=st.session_state.v7921_results
        if isinstance(df,pd.DataFrame) and len(df):
            trig=df[df.Trigger_detected].copy()
            summary=pd.DataFrame([{
                "Saved sets":len(df),"Target":target_sets,"Completion":len(df)/float(target_sets),
                "Activation":df.Trigger_detected.mean(),"Overall change":df.Overall_change.mean(),
                "R1-3 change":df.R1_3_change.mean(),"R4-6 change":df.R4_6_change.mean(),
                "R7-10 change":df.R7_10_change.mean(),"R11+ change":df.R11_plus_change.mean(),
            ...
            if len(df)>=1200:
                room=df.groupby("Room",as_index=False).agg(
                    Activation=("Trigger_detected","mean"),Overall_change=("Overall_change","mean"),
                    R1_3_change=("R1_3_change","mean"),R11_plus_change=("R11_plus_change","mean"))
                slot=df.groupby("Draft_slot",as_index=False).agg(
                    Activation=("Trigger_detected","mean"),Overall_change=("Overall_change","mean"),
                    R1_3_change=("R1_3_change","mean"),R11_plus_change=("R11_plus_change","mean"))
                st.markdown("#### By room"); st.dataframe(room.round(4),use_container_width=True,hide_index=True)
                st.markdown("#### By draft slot"); st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            st.download_button("⬇️ Download current V7.92.1 validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_92_1_resumable_round2_validation.csv","text/csv",key="v7921_download")
