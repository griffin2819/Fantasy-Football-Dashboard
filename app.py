...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Decision Data Quality Gate", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
@media(max-width:700px){.block-container{padding-left:.9rem!important;padding-right:.9rem!important;padding-top:.75rem!important;}button[data-baseweb="tab"]{padding:0 .62rem!important;font-size:.82rem!important;}h1{font-size:1.72rem!important;}h2{font-size:1.42rem!important;}h3{font-size:1.18rem!important;}div[data-testid="stMetric"]{padding:.48rem .58rem!important;border-radius:13px!important;}div[data-testid="stMetricLabel"]{font-size:.70rem!important;}div[data-testid="stMetricValue"]{font-size:1.28rem!important;}div[data-testid="stMetricDelta"]{font-size:.66rem!important;}div[data-testid="stVerticalBlock"]{gap:.72rem!important;}}
</style>
""",unsafe_allow_html=True)
...
def _v637_consensus_directory_ranks():
    ...
    try:
        ...
        response=requests.get(url,timeout=(4,10),headers={"User-Agent":"FantasyEdge/8.06"})
        response.raise_for_status()
...
def _v637_attach_directory_ranks(roster_dir):
    ...
    cells=[(norm(r.player_raw),float(r.external_ecr_rank)) for _,r in ranks.iterrows()]
    def lookup(name):
        nk=norm(name)
        hits=[]
        for ck,rk in cells:
        ...
        for pid,pl in players.items():
            ...
            if designation!="ACTIVE" or practice_norm or body or injury_start:
                out[_owner_key(name)]={
                    ...
                    "injury":body,
                    # injury_start_date is onset metadata, not a source update timestamp.
                    # Treating it as "updated" made an old status look newly authoritative.
                    "updated":"",
...
def _v805_espn_team_json(teams, endpoint):
    """Fetch independent team endpoints concurrently; keep parsing on the caller thread."""
    def fetch(tw):
        team=(tw.get("team") or {})
        team_id=str(team.get("id") or "").strip()
        slug=str(team.get("slug") or "").strip()
        identifiers=([team_id,slug] if endpoint=="injuries" else [slug])
        for ident in dict.fromkeys(x for x in identifiers if x):
            try:
                url=f"https://site.api.espn.com/apis/site/v2/sports/football/nfl/teams/{ident}/{endpoint}"
                response=requests.get(url,timeout=(3,5),headers={"User-Agent":"Mozilla/5.0"})
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=12) as pool:
        # map preserves league order, so duplicate player names resolve as before.
        return list(pool.map(fetch,teams))
...
def _v680_espn_injury_watch_feed():
    ...
    try:
        ...
        for team,data in _v805_espn_team_json(teams,"injuries"):
...
def _v723_nflverse_practice_feed(season=None):
    ...
    try:
        ...
        for _,r in df.iterrows():
            ...
            # V7.69 runtime safety: pandas missing scalars (pd.NA) cannot be used in
            # Python boolean expressions (``value or ""`` raises "boolean value of NA
            # is ambiguous"). Normalize the source scalar explicitly before testing it.
            _report_val=r.get("report_status","")
            try:
                _report_missing=bool(pd.isna(_report_val))
            except Exception:
                _report_missing=False
            _report_raw="" if _report_missing else str(_report_val).strip()
            _report_raw_present=bool(_report_raw and _report_raw.upper() not in {"NAN","NONE","<NA>"})
            report=_designation(_report_raw)
            ...
            rec={"status":report,"injury":injury,"practice":practice,"updated":modified,"team":team,
                 ...
                 "player":name,
                 "report_raw":_report_raw,
                 "report_raw_present":_report_raw_present,
            ...
            if prev is None:
                ...
                if nw>pw:
                    out[key]=rec
                elif nw==pw and newer:
                    # V7.68: practice rows and game-status rows can be published separately.
                    # A later DNP row with a blank report_status must NOT erase an OUT
                    # designation that was already published for the same week. Merge the
                    # newest practice/detail fields while carrying the latest explicit game
                    # designation forward until a newer explicit designation replaces it.
                    merged_rec=dict(prev)
                    for _fld in ["practice","injury","updated","team","player","practice_source"]:
                        if str(rec.get(_fld,"") or "").strip() or _fld in {"updated","practice"}:
                            merged_rec[_fld]=rec.get(_fld)
                    if bool(rec.get("report_raw_present",False)):
                        merged_rec["status"]=rec.get("status","ACTIVE")
                        merged_rec["report_raw"]=rec.get("report_raw","")
                        merged_rec["report_raw_present"]=True
                    merged_rec["week"]=nw
                    merged_rec["source"]="nflverse official injury report"
                    out[key]=merged_rec
...
def _v762_expire_unverified_status(status, has_fresh, conflict=False, severe_undated=False):
    """V7.63 evidence-time lifecycle.

    Old unsupported injury evidence must not live forever in Monitoring.  Only fresh
    contradictory evidence or a fresh severe designation that lacks enough proof stays
    unresolved.  Everything else expires back to ACTIVE/clear.
    """
    st=_v677_normalize_injury_status(status)
    if conflict:
        return False
    if has_fresh and severe_undated:
        return False
    if not has_fresh and st in {"QUESTIONABLE","DOUBTFUL","OUT","IR","PUP","NFI","SUSPENDED"}:
        return True
    return False
...
def _v680_injury_watch_feed():
    ...
    """
    sleeper=_v681_sleeper_injury_watch() or {}
    espn=_v680_espn_injury_watch_feed() or {}
    official=_v723_nflverse_practice_feed() or {}
...
def _v645_apply_published_projections(board, ppr, teams, slots):
    ...
    if isinstance(rz,pd.DataFrame) and len(rz):
        ...
        provider_parts.append(pr[['key','published_ppg','projection_provider','team']])
    if isinstance(fp,pd.DataFrame) and len(fp):
        ...
        provider_parts.append(pr2[['key','published_ppg','projection_provider','team']])
    ...
    # V7.94 fallback team identity from the same published feeds that already
    # matched these players. Sleeper remains preferred when available.
    if 'team' in mp.columns:
        _pt=x.loc[idx,'key'].map(mp['team']).fillna('').astype(str).str.upper().str.strip()
        _cur=(x.loc[idx,'team'] if 'team' in x.columns else pd.Series('',index=idx)).fillna('').astype(str).str.upper().str.strip()
        _use=(~_pt.isin(['','NFL','FA','NONE','N/A','NAN'])) & _cur.isin(['','NFL','FA','NONE','N/A','NAN'])
        if bool(_use.any()):
            _use_idx=_use.index[_use]
            x.loc[_use_idx,'team']=_pt.loc[_use_idx].values
            if '_team_identity_source' not in x.columns: x['_team_identity_source']=''
            x.loc[_use_idx,'_team_identity_source']=x.loc[_use_idx,'key'].map(mp['projection_provider']).map(lambda v:f'{v} projection feed').values
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                # V7.92 challenger-only Round-2 generic adaptation rule.
...
# V7.84 persistent last-known hard availability registry.  A ruled-out/reserve
# designation survives page/deploy snapshot fallbacks until a later authoritative
# refresh explicitly clears or replaces it.
...
# V7.94: visible page controls can request the same full refresh as the sidebar.
_v794_forced_refresh=bool(st.session_state.pop("_v794_force_weekly_refresh",False))
_refresh_live=_v794_forced_refresh or st.sidebar.button(
...
if _refresh_live:
    ...
    for _v813_fn in [
        sleeper_players,
        globals().get("_v681_sleeper_injury_watch"),
        globals().get("_v680_espn_injury_watch_feed"),
        globals().get("_v680_injury_watch_feed"),
        globals().get("_v723_nflverse_practice_feed"),
        globals().get("_v679_espn_roster_status"),
        _v815_nfl_state_preflight,
        globals().get("_v677_week_matchups"),
        globals().get("_v730_build_matchup_lookup"),
...
if _refresh_live:
    ...
    with st.spinner("Refreshing weekly data once…"):
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as _v813_pool:
                _f_players=_v813_pool.submit(sleeper_players)
                _f_injury=_v813_pool.submit(_v680_injury_watch_feed)
                _f_state=_v813_pool.submit(_v815_nfl_state_preflight)
                try:
                    players=_f_players.result(timeout=12)
                except Exception as _e:
                    players={}
                    _v793_sources["sleeper"]={"ok":False,"error":f"{type(_e).__name__}: {_e}"}
                try:
                    _v813_injury_prefetch=_f_injury.result(timeout=18)
                    _v793_sources["injury_prefetch"]={"ok":True,"rows":len(_v813_injury_prefetch) if hasattr(_v813_injury_prefetch,"__len__") else 0}
                except Exception as _e:
                    _v813_injury_prefetch={}
                    _v793_sources["injury_prefetch"]={"ok":False,"error":f"{type(_e).__name__}: {_e}"}
                try:
                    _v813_live_state=_f_state.result(timeout=8)
                except Exception:
                    _v813_live_state={"season":datetime.now().year,"week":1}
            if not isinstance(players,dict) or not players:
...
def _v783_authoritative_week_status(row):
    ...
    V7.84 adds a persistent last-known hard-status registry so a current OUT/PUP/IR/
    NFI/SUSPENDED state cannot disappear merely because a later app build falls back
    to a local board snapshot.  Fresh reconciled fields still take precedence and a
    later authoritative ACTIVE refresh can clear the registry.
    """
    hard_order=("SUSPENDED","NFI","PUP","IR","OUT","DO NOT DRAFT")
    # V7.85: a week-scoped confirmed status outranks cached ACTIVE data, but only
    # during its own season/week. This prevents Action Queue from resurrecting a
    # player who is known OUT/PUP while still letting the record expire next week.
    try:
        _name=str(row.get("player",row.get("Player","")) or "")
        _ov=(globals().get("_V785_WEEKLY_STATUS_OVERRIDES",{}) or {}).get(_owner_key(_name),{})
        _ctx=globals().get("_v678_live_week",{}) or globals().get("_v778_snapshot_meta",{}) or {}
        _season=int(_ctx.get("season",datetime.now().year) or datetime.now().year)
        _week=int(_ctx.get("week",1) or 1)
        if isinstance(_ov,dict) and int(_ov.get("season",-1))==_season and int(_ov.get("week",-1))==_week:
            _ost=_v677_normalize_injury_status(_ov.get("status",""))
            if _ost in hard_order:
                return _ost
    except Exception:
        pass
    candidates=[]
    # Scan all reconciled/live-ish status fields that can survive into a saved board.
    for key in ("injury_effective","fe_status","injury","raw_player_status","raw_injury_status","player_status","roster_status","status"):
        try:
            raw=row.get(key,"")
            val=_v677_normalize_injury_status(raw)
        except Exception:
            val="ACTIVE"
        if val:
            candidates.append(val)
    # Persistent hard-status memory from the last authoritative refresh.
    try:
        name=str(row.get("player",row.get("Player","")) or "")
        reg=(globals().get("_V784_AVAILABILITY_REGISTRY",{}) or {}).get(_owner_key(name),{})
        reg_status=_v677_normalize_injury_status(reg.get("status","")) if isinstance(reg,dict) else "ACTIVE"
        if reg_status in hard_order:
            candidates.append(reg_status)
    except Exception:
        pass
    for status in hard_order:
        if status in candidates:
            return status
    for key in ("fe_status","injury_effective","injury","raw_player_status","raw_injury_status"):
        try:
            val=_v677_normalize_injury_status(row.get(key,""))
        except Exception:
            val="ACTIVE"
...
if _refresh_live:
    ...
    if _v816_reuse_usage:
        ...
        _v802_progress.info("Refresh step 3/4: matching observed player usage")
        board=_v683_enrich_role(board,_v678_live_week.get("season",datetime.now().year))
        _v793_sources["usage"]={"ok":True,"mode":"live-weekly-usage-refresh"}
    ...
    try:
        ...
        _v707_wp=_v692_sleeper_weekly_projections(int(_v678_live_week.get("season",datetime.now().year)),int(_v678_live_week.get("week",1)))
        ...
        else: board["_external_week_projection"]=np.nan
    ...
    if all(c in board.columns for c in ["_scarcity_score","_replacement_ppg","_scarcity_premium"]):
        try:
            _v693_scarcity=_v693_scarcity_context(board,state) if _v777_page_idx in (12,13) else pd.DataFrame()
        except Exception:
...
def _v756_opponent_intelligence(board_df,state_snapshot):
    """League-specific opponent needs/strengths for waiver competition and trade fit."""
    rows=[]
    for owner in [str(i) for i in range(2,13)]:
        r=board_df[board_df["player"].map(lambda n:_player_owner(state_snapshot,n)==owner)].copy()
        if r.empty: continue
        pos_counts=r["position"].astype(str).value_counts().to_dict()
        pos_scores=r.groupby("position")["fe_week"].mean().to_dict() if "fe_week" in r.columns else r.groupby("position")["projection"].mean().to_dict()
        weak=min(("QB","RB","WR","TE","DL","DB"),key=lambda p:(pos_counts.get(p,0),pos_scores.get(p,0)))
        strongest=max(("QB","RB","WR","TE","DL","DB"),key=lambda p:pos_scores.get(p,0))
        rows.append({"Owner":owner,"Team":state_snapshot.get("team_names",{}).get(owner,f"Team {owner}"),"Need":weak,"Strength":strongest,"Rostered":len(r)})
    return pd.DataFrame(rows)
...
def _v775_unified_decisions(board_df,state_snapshot,limit=12,include_trade_scan=False):
    ...
    if isinstance(ideas,pd.DataFrame) and len(ideas):
        for _,tr in ideas.head(6).iterrows():
            ...
            if lg<0.25 or pg<0 or fit<50 or "HOLD" in rec.upper(): continue
...
# V7.85 persist hard availability without allowing a generic ACTIVE fallback to
# erase a previously confirmed OUT/PUP/IR/NFI/SUSPENDED designation.
if _refresh_live:
    try:
        _reg=dict(state.get("_availability_registry",{}) or {})
        _hard={"OUT","IR","PUP","NFI","SUSPENDED","DO NOT DRAFT"}
        _now=datetime.now().isoformat(timespec="seconds")
        for _,_rr in board.iterrows():
            _name=str(_rr.get("player","") or ""); _key=_owner_key(_name)
            _resolved=_v783_authoritative_week_status(_rr)
            _verified=bool(_rr.get("injury_status_verified",True))
            _source=str(_rr.get("injury_source","") or "").lower()
            _updated=str(_rr.get("injury_updated","") or "")
            if _resolved in _hard:
                _reg[_key]={"status":_resolved,"updated":_now,"player":_name,"source":str(_rr.get("injury_source","") or "weekly authority")}
                continue
            # Only explicit authoritative clearance can erase a hard status.
            # Generic Sleeper/Snapshot ACTIVE is absence of evidence, not proof of health.
            _explicit_active=bool(
                _resolved=="ACTIVE" and _verified and (
                    ...
                    ("official" in _source and bool(_updated))
                )
            )
            if _explicit_active and _key in _reg:
                _reg.pop(_key,None)
        # Week-scoped confirmed statuses are mirrored into the registry for all
        # snapshot consumers, then naturally stop applying when the week changes.
        ...
        for _key,_ov in (globals().get("_V785_WEEKLY_STATUS_OVERRIDES",{}) or {}).items():
            if isinstance(_ov,dict) and int(_ov.get("season",-1))==_season and int(_ov.get("week",-1))==_week:
                _ost=_v677_normalize_injury_status(_ov.get("status",""))
                if _ost in _hard:
                    _reg[_key]={"status":_ost,"updated":_now,"player":str(_ov.get("player",_key)),"source":str(_ov.get("source","confirmed weekly status"))}
...
if _refresh_live:
    try:
        ...
        if _v779_saved:
            ...
            _v809_persist_refresh_state_fast(state)
            _v802_progress.success("Weekly refresh finished; saved player board is ready")
...
def _v781_render_page(_v777_page_idx):
    ...
    if _v777_page_idx==3:
        ...
        if _my_roster.empty:
            ...
            _v789_cov=_v789_snapshot_coverage(_v778_snapshot_meta,board)
            ...
            _v797_current_build=bool(isinstance(_v797_build,dict) and _v797_build.get("version")==APP_VERSION)
            _v797_required_sources=("base_model","history","market")
            _v797_sources_ready=bool(_v797_current_build and all(bool((_v793_refresh_sources.get(k) or {}).get("ok")) for k in _v797_required_sources))
            ...
            if not _v797_current_build:
                st.warning("The saved refresh result is from an older app version. Refresh weekly data here to collect current source errors.")
            ...
            # V8.19 Decision Data Quality Gate.  Projection-only fringe players must not
            # become actionable waiver adds simply because one model number is high.
            # Score the *candidate's* identity/evidence quality before deep simulations.
            def _v819_candidate_data_quality(_r):
                ...
                _bits=[]
                if _team_good:
                    _score+=18; _bits.append("team")
                if _verified:
                    _score+=12; _bits.append("verified identity")
                if _wk>0:
                    _score+=20; _bits.append("weekly projection")
                elif _base>0:
                    _score+=10; _bits.append("base projection")
                if _opps>0 or _games>0:
                    _score+=18; _bits.append("observed usage")
                elif _pos=="K" and (_wk>0 or _base>0):
                    _score+=12; _bits.append("kicker projection")
                if _rc>=70:
                    _score+=17; _bits.append("strong role")
                elif _rc>=55:
                    _score+=11; _bits.append("usable role")
                elif _rc>=45:
                    _score+=5; _bits.append("thin role")
                if min(_mr,_model)<=500:
                    _score+=8; _bits.append("market/model rank")
                if _status_verified:
                    _score+=7; _bits.append("status verified")
                return round(min(100.0,_score),1), ", ".join(_bits) if _bits else "projection-only / weak identity"

            _v819_quality=_fa_eval.apply(_v819_candidate_data_quality,axis=1)
            _fa_eval["_v819_data_confidence"]=[x[0] for x in _v819_quality]
            ...
            _fa_eval=_fa_eval[_fa_eval["_v819_data_confidence"].ge(_v819_floor)].copy()
            if _fa_eval.empty:
                st.info("No waiver candidates currently clear Fantasy Edge's minimum data-confidence gate.")
                return
            _fa_eval["_v804_prefilter"]=_fa_eval["_v804_prefilter"] + (_fa_eval["_v819_data_confidence"]-50.0).clip(lower=-15,upper=25)*0.06
            ...
            _fa_eval=_fa_eval.sort_values(["_candidate_value","_v739_opportunity_boost","vorp"],ascending=False).head(10)
            ...
            # V8.10: a temporary injury/projection dip must never turn a meaningful RB/WR
            # into the default cross-position cut.  This is a conservative *admission* guard;
            # users can still opt into protected drops explicitly.
            if not _allow_protected and len(_eligible_drops):
                _dp_pos=_eligible_drops.get("position",pd.Series("",index=_eligible_drops.index)).fillna("").astype(str).str.upper()
                _dp_market=pd.to_numeric(_eligible_drops.get("market_pick",pd.Series(index=_eligible_drops.index,dtype=float)),errors="coerce").fillna(9999)
                if "fe_market_rank" in _eligible_drops.columns:
                    _dp_market=pd.concat([_dp_market,pd.to_numeric(_eligible_drops["fe_market_rank"],errors="coerce").fillna(9999)],axis=1).min(axis=1)
                _dp_model=pd.to_numeric(_eligible_drops.get("model_rank",pd.Series(index=_eligible_drops.index,dtype=float)),errors="coerce").fillna(9999)
                _dp_vorp=pd.to_numeric(_eligible_drops.get("vorp",pd.Series(index=_eligible_drops.index,dtype=float)),errors="coerce").fillna(0)
                _dp_ros=pd.to_numeric(_eligible_drops.get("_ros_value",pd.Series(index=_eligible_drops.index,dtype=float)),errors="coerce").fillna(0)
                _skill_core=_dp_pos.isin(["RB","WR"]) & ((_dp_market<=200) | (_dp_model<=160) | (_dp_vorp>=0.75) | (_dp_ros>=8.0))
                _eligible_drops=_eligible_drops[~_skill_core].copy()
            ...
            for _,add_raw in _fa_eval.iterrows():
                ...
                if str(add.get("position","")).upper() in _v737_active_emergencies:
                    ...
                    _drop_pool=_drop_pool[_drop_pool["_emergency_drop_rank"].lt(900)].sort_values(["_emergency_drop_rank","_drop_cost","_drop_value","_ros_value"]).head(4)
                else:
                    _same_pos=_eligible_drops[_eligible_drops["position"].eq(add["position"])].sort_values(["_drop_cost","_drop_value","_ros_value"]).head(2)
                    ...
                    if _add_pos_u in ("QB","TE","K") and _req_u>0 and _playable_u>=_req_u+1:
                        ...
                        _drop_pool=_drop_pool.drop_duplicates(subset=["player"]).head(3)
                ...
                for _,drop in _drop_pool.iterrows():
                    ...
                    _moves.append({
                        ...
                        "Data Confidence":round(float(add_raw.get("_v819_data_confidence",0) or 0),1),
            ...
            def _v818_render_waiver_diagnostics():
                with st.expander("Advanced diagnostics", expanded=False):
                    ...
                    st.dataframe(pd.DataFrame(_v818_score_rows),use_container_width=True,hide_index=True)
                    if isinstance(_base_starters,pd.DataFrame) and len(_base_starters):
                        _v818_cols=[c for c in ["Slot","Player","Pos","Projected PPG","Canonical PPG","Adjusted PPG","Status"] if c in _base_starters.columns]
                        st.dataframe(_base_starters[_v818_cols],use_container_width=True,hide_index=True)
                    if _v737_active_emergencies:
                    ...
                    st.markdown("**Waiver candidate data confidence**")
                    if isinstance(_fa_eval,pd.DataFrame) and len(_fa_eval):
                        _qcols=[c for c in ["player","position","team","_v819_data_confidence","_v819_data_evidence","_external_week_projection","_opps_last","_role_confidence"] if c in _fa_eval.columns]
                        _qshow=_fa_eval[_qcols].copy().sort_values("_v819_data_confidence",ascending=False).head(15)
                        _qshow=_qshow.rename(columns={"player":"Player","position":"Pos","team":"Team","_v819_data_confidence":"Data Confidence","_v819_data_evidence":"Evidence","_external_week_projection":"Week PPG","_opps_last":"Last Opps","_role_confidence":"Role Conf."})
                        st.dataframe(_qshow,use_container_width=True,hide_index=True)
                        st.caption("Action gate: normal ADD/PRIORITY requires Data Confidence ≥60; emergency coverage requires ≥45. Lower-confidence candidates may remain WATCH only.")
                    ...
                    if len(_protect_show):
                        if "_ros_value" in _protect_show.columns:
                            _protect_show["_ros_value"]=pd.to_numeric(_protect_show["_ros_value"],errors="coerce").round(2)
                        st.dataframe(_protect_show.rename(columns={"player":"Player","position":"Pos","team":"Team","injury":"Status","_drop_tier":"Tier","_drop_protection":"Protection","_ros_value":"ROS Value"}),use_container_width=True,hide_index=True)
            ...
            if _moves_df.empty:
                ...
                if len(_moves_df):
                    _dc=pd.to_numeric(_moves_df.get("Data Confidence",pd.Series(0,index=_moves_df.index)),errors="coerce").fillna(0)
                ...
                st.markdown('<div class="fe-rec-title">Waiver Recommendations</div>',unsafe_allow_html=True)
                ...
                if not _waiver_data_ready:
                    ...
                    if not _waiver_data_ready:
                        st.warning("Waiver decision unavailable: required model sources are missing or stale. Refresh weekly data and check the source errors below.")
                    else:
                        st.markdown('''<div class="fe-hold"><div class="fe-hold-icon">🛡️</div><div><div class="fe-hold-main">HOLD</div><div class="fe-hold-sub">No roster move recommended.</div></div></div>''',unsafe_allow_html=True)
                else:
                    ...
                    _simple_cols=[c for c in ["Add","Pos","Drop","Waiver Call","Data Confidence","FAAB"] if c in _best.columns]
                    _simple=_best[_simple_cols].copy().head(12)
                    _simple=_simple.rename(columns={"Waiver Call":"Model","Data Confidence":"Data Conf.","FAAB":"FAAB / Claim"})
                    st.dataframe(_simple,use_container_width=True,hide_index=True)

                _v818_render_waiver_diagnostics()

            _v812_waiver_elapsed=time.perf_counter()-_v810_waiver_t0
            st.caption(f"⚡ Waiver engine: {_v812_waiver_elapsed:.2f}s • {len(_fa):,} current candidates • {len(_fa_eval):,} finalists • max {len(_fa_eval)*3:,} full add/drop simulations")
    ...
    if _v777_page_idx==5:
        st.caption("V7.76 pipeline: breakout/regression signals now feed the Unified Decision Engine; available breakout candidates remain WATCH until the certified Waiver engine clears the transaction.")
    ...
    # ---------------- V6.86 IDP Intelligence ----------------
        ...
        if _pc.empty:
            ...
            st.caption("Multipliers activate gently after 5+ graded calls at a position; small samples remain neutral.")
        ...
        st.markdown("### 7) Release regression suite")
        _reg=_v756_regression_suite()
        _rp=int((_reg["Result"]=="PASS").sum()) if len(_reg) else 0
        st.metric("Regression sentinels",f"{_rp}/{len(_reg)} passing")
        with st.expander("Show release tests",expanded=False):
            st.dataframe(_reg,use_container_width=True,hide_index=True)

        st.markdown("### 6) Unified engine report card")
        _v775_card=_v775_accuracy_scorecard(_history,state)
        if len(_v775_card):
            st.dataframe(_v775_card,use_container_width=True,hide_index=True)
        st.caption("Tracks Start/Sit, waiver and role/breakout outcomes from graded weekly checkpoints; future recommendations keep using the existing position calibration rather than overfitting a small sample.")
    ...
    if _v777_page_idx==13:
        ...
        if my.empty or other.empty:
            ...
            with _trc2:
                st.caption("Page opens from the saved trade snapshot. The full league scan runs only when you press the button.")
            if _run_trade_scan:
            ...
            if isinstance(_v756_oppintel,pd.DataFrame) and len(_v756_oppintel):
                with st.expander("Opponent needs / likely trade fits",expanded=False):
                    st.dataframe(_v756_oppintel,use_container_width=True,hide_index=True)
    ...
    if _v777_page_idx==14:
        ...
        if my.empty:
            ...
            .fecc-badge{{display:inline-block;min-width:68px;text-align:center;padding:.15rem .42rem;border-radius:5px;border:1px solid #526071;background:#334154;font-size:.68rem}} .fecc-badge.high,.fecc-badge.tough{{background:#721d1c;border-color:#ae302d;color:#fff}} .fecc-badge.medium,.fecc-badge.below-avg{{background:#765500;border-color:#b18400;color:#fff}} .fecc-badge.low,.fecc-badge.great,.fecc-badge.good{{background:#006c3d;border-color:#00a15c;color:#fff}} .fecc-badge.unverified{{background:#3e4652;color:#d2d7de}}
            .fecc-footer-grid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.65rem;margin:.7rem 0}} .fecc-mini{{padding:.75rem;border:1px solid #344255;border-radius:14px;background:linear-gradient(145deg,#17212c,#101720)}} .fecc-mini .k{{font-size:.72rem;color:#aeb7c5}} .fecc-mini .v{{font-size:1.05rem;font-weight:800;margin:.2rem 0}} .fecc-mini .s{{font-size:.68rem;color:#9ba7b6}}
            @media(max-width:700px){{.fecc-brand{{font-size:1.28rem}}.fecc-grid{{grid-template-columns:repeat(2,minmax(0,1fr));gap:.48rem}}.fecc-metric{{min-height:76px;padding:.58rem .62rem}}.fecc-metric .v{{font-size:1.2rem}}.fecc-footer-grid{{grid-template-columns:1fr}}.fecc-status{{padding:.7rem .75rem}}.fecc-scroll{{overflow-x:hidden}}table.fecc-table.fecc-compact{{min-width:0!important;font-size:.69rem;table-layout:fixed}}.fecc-compact th,.fecc-compact td{{white-space:normal!important;overflow-wrap:anywhere;padding:.4rem .35rem}}}}
            </style>
            ...
            if isinstance(_v775_feed,pd.DataFrame) and len(_v775_feed):
                ...
                if len(_show):
                    _cols=[c for c in ["Tier","Area","Action","Week Impact","ROS Impact","Confidence","Why","Posture"] if c in _show.columns]
                    _v784_cards(_show,[("Tier","Tier"),("Area","Area"),("Week","Week Impact"),("Confidence","Confidence")],title_field="Action")
                else:
            ...
            with st.expander("League opponent intelligence",expanded=False):
                if isinstance(_v756_oppintel,pd.DataFrame) and len(_v756_oppintel): st.dataframe(_v756_oppintel,use_container_width=True,hide_index=True)
