...
st.set_page_config(page_title="Fantasy Edge V6.58 — Authoritative Draft Availability", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V6.58-AUTHORITATIVE-DRAFT-AVAILABILITY-397"
...
with tabs[1]:
    ...
    _taken_display_keys={norm(n) for n in state.get("taken",[]) if norm(n)}
...
with tabs[3]:
    ...
    _taken_waiver_keys={norm(n) for n in state.get("taken",[]) if norm(n)}
...
with tabs[4]:
    ...
    _taken_idp_keys={norm(n) for n in state.get("taken",[]) if norm(n)}
