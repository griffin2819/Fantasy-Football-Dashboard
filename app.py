...
st.title("🏈 Fantasy Edge V7.40 — Pick 12 Faller-vs-Need Execution Calibration")
st.caption("V7.40 • V7.24 9× champion frozen • Pick 12 faller-vs-need execution calibration")
st.caption("Build ID: V7.40-P12FVNEC-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v740_live_slot")
    ...
    if "v740_target_queue" not in st.session_state:
        st.session_state.v740_target_queue={}
    queue=st.session_state.v740_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v740_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.40 keeps the V7.24 9× champion completely frozen. Execution candidates run only inside the Pick-12 calibration matrix.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.40 robustness report")
        ...
        st.markdown("### 🧪 V7.40 champion baseline validation")
        st.info("V7.40 is calibration only. It tests draft-window urgency between a model-compatible faller and a reachable RB/TE need; it does not force any position.")
        ...
        if "v740_independent_results" not in st.session_state:
            st.session_state.v740_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v740_independent_results"),pd.DataFrame) else "ready to run"))
        if "v740_independent_summary" not in st.session_state:
            st.session_state.v740_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v740_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v740_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v740_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v740_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v740_independent_results=iv
                st.session_state.v740_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v740_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v740_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.40 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.40 calibrates Pick-12 R4-6 faller-vs-need execution timing only in matched simulations.")
            ...
            st.download_button(
                "⬇️ Download V7.40 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_40_champion_baseline.csv",
                "text/csv",
                key="v740_baseline_download"
            )
        ...
        st.markdown("### ⏱️ V7.40 — Pick 12 R4-6 Faller-vs-Need Execution Calibration")
        st.caption(
            "Calibration only. V7.24 9× stays frozen. This does NOT force WR. It tests whether Pick 12 should harvest a model-compatible "
            "market faller before an RB/TE reach when that RB/TE has a reasonable chance to survive to the next turn."
        )
        if "v740_results" not in st.session_state: st.session_state.v740_results=None
        if "v740_meta" not in st.session_state: st.session_state.v740_meta=None

        sims=st.number_input("Execution-calibration simulations per room",min_value=5,max_value=20,value=10,step=5,key="v740_sims")
        ca,cb=st.columns(2)
        run=ca.button("Run Pick 12 execution calibration",type="primary",key="v740_run")
        clear=cb.button("Clear execution calibration",key="v740_clear")
        if clear:
            st.session_state.v740_results=None; st.session_state.v740_meta=None; st.rerun()

        if run:
            configs=[
                ("CHAMPION",False,9.0,8.0,6.0,.50),
                ("A • Fall 8 / Eval 8 / Reach 5 / Surv 50",True,8.0,8.0,5.0,.50),
                ("B • Fall 7 / Eval 8 / Reach 5 / Surv 55",True,7.0,8.0,5.0,.55),
                ("C • Fall 8 / Eval 6 / Reach 4 / Surv 55",True,8.0,6.0,4.0,.55),
                ("D • Fall 7 / Eval 6 / Reach 4 / Surv 60",True,7.0,6.0,4.0,.60),
                ("E • Fall 6 / Eval 8 / Reach 4 / Surv 60",True,6.0,8.0,4.0,.60),
            ]
            ...
            for room_name in rooms:
                for j in range(int(sims)):
                    seed=940000+j*67+sum(ord(c) for c in room_name)
                    ...
                    for name,enabled,minfall,evalcap,reach,surv in configs:
                        fe=simulate_mock_fast(
                            fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,
                            p12_faller_need_enabled=enabled,p12_faller_min=minfall,
                            p12_faller_eval_deficit=evalcap,p12_need_reach=reach,p12_need_survival=surv
                        )
                        ...
                        rows.append({
                            "Config":name,"Enabled":enabled,"Min_fall":minfall,"Eval_cap":evalcap,
                            "Need_reach":reach,"Need_survival":surv,"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "FE_grade":fc["overall"],"Consensus_grade":cc["overall"],"Difference":fc["overall"]-cc["overall"],
                            "Market_gap":fc["market_value"]-cc["market_value"],"Roster_balance":fc["roster_balance"]-cc["roster_balance"],
                            ...
                            "R4_6_RB":int((r46.position=="RB").sum()),"R4_6_WR":int((r46.position=="WR").sum()),
                            "R4_6_TE":int((r46.position=="TE").sum())})
                        done+=1; prog.progress(done/total)
                        if done==1 or done%50==0 or done==total: status.caption(f"Execution matrix: {done}/{total}")
            df=pd.DataFrame(rows); st.session_state.v740_results=df
            st.session_state.v740_meta={"seconds":time.perf_counter()-started,"drafts":len(df)}
            status.success(f"Execution calibration complete: {len(df)} model drafts."); prog.empty()

        df=st.session_state.get("v740_results"); meta=st.session_state.get("v740_meta") or {}
        if isinstance(df,pd.DataFrame) and len(df):
            summ=df.groupby(["Config","Min_fall","Eval_cap","Need_reach","Need_survival"],as_index=False).agg(
                Overall=("Difference","mean"),Market=("Market_gap","mean"),Roster=("Roster_balance","mean"),
            ...
            ctrl=summ[summ.Config.eq("CHAMPION")].iloc[0]
            ...
            st.dataframe(summ[["Rank","Config","Score","Overall","Overall_change","R4_6","R4_6_change","R7_10","R7_10_change",
                               "R11_plus","R11_plus_change","Roster","RB","WR","TE"]].round(3),use_container_width=True,hide_index=True)

            leader=summ.iloc[0]
            room=df.groupby(["Config","Room"],as_index=False).agg(Overall=("Difference","mean"),R4_6=("R4_6_gap","mean"),R7_10=("R7_10_gap","mean"))
            cr=room[room.Config.eq("CHAMPION")][["Room","Overall","R4_6","R7_10"]].rename(columns={"Overall":"C_Overall","R4_6":"C_R4_6","R7_10":"C_R7_10"})
            ...
            st.dataframe(lr[["Room","Overall_change","R4_6_change","R7_10_change"]].round(3),use_container_width=True,hide_index=True)

            if leader.Config=="CHAMPION":
                st.warning("The champion leads. Do not add a Pick-12 faller-vs-need execution rule.")
            elif leader.Overall_change>0 and leader.R4_6_change>0 and leader.R7_10_change>-0.35 and int((lr.Overall_change>0).sum())>=3:
                st.success(f"Screening leader: {leader.Config}. Move only this execution threshold into V7.41 matched validation.")
            else:
                st.warning(f"{leader.Config} leads but does not clear the screening gate. Keep V7.24 frozen.")
            st.download_button("⬇️ Download V7.40 Pick 12 execution calibration CSV",df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_40_pick12_faller_vs_need_execution_calibration.csv","text/csv",key="v740_download")
