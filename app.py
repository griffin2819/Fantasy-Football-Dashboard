...
def wr_pipeline_audit_snapshot(fast, eligible_idx, shortlist_idx, delta, eval_score,
                               chosen_global, current_pick, next_pick, randomness=12):
    """Diagnostic-only WR scoring pipeline snapshot for Rounds 1-10.

    Captures whether WR was favored by market, model evaluation, roster utility,
    and execution timing. This NEVER changes the selected player.
    """
    pool=fast["pool"]; pos=fast["pos_code"]; market=fast["market_pick"]
    model_rank=fast["model_rank"]; draft_score=fast["draft_score"]
    projection=fast["projection"]; vorp=fast["vorp"]

    eidx=np.asarray(eligible_idx,dtype=int)
    sidx=np.asarray(shortlist_idx,dtype=int)
    ...
    snap={
        "WR_Audit":1,
        "WR_Selected_player":str(pool.iloc[chosen_global].player),
        "WR_Selected_position":str(pool.iloc[chosen_global].position),
        "WR_Selected_market_pick":float(market[chosen_global]) if np.isfinite(market[chosen_global]) else np.nan,
        "WR_Selected_model_rank":float(model_rank[chosen_global]) if np.isfinite(model_rank[chosen_global]) else np.nan,
        "WR_Selected_projection":float(projection[chosen_global]),
        "WR_Selected_vorp":float(vorp[chosen_global]),
        "WR_Market_best_is_WR":0,
        "WR_Top3_FE":0,
        "WR_Best_player":"",
        "WR_Best_market_pick":np.nan,
        "WR_Best_model_rank":np.nan,
        "WR_Best_projection":np.nan,
        "WR_Best_vorp":np.nan,
        "WR_Best_eval":np.nan,
        "WR_Selected_eval":np.nan,
        "WR_Eval_gap":np.nan,
        "WR_DraftScore_component_gap":np.nan,
        "WR_RosterDelta_component_gap":np.nan,
        "WR_Edge_component_gap":np.nan,
        "WR_Total_pre_execution_gap":np.nan,
        "WR_Market_value_passed":np.nan,
        "WR_Survival_est":np.nan,
        "WR_Survived_actual":np.nan,
        "WR_Best_idx":None,
        "WR_Primary_suppressor":""
    }

    # Is the best MARKET-available eligible player a WR?
    if len(eidx):
        em=np.where(np.isnan(market[eidx]),9999.0,market[eidx])
        market_best=int(eidx[int(np.argmin(em))])
        snap["WR_Market_best_is_WR"]=1 if pos[market_best]==2 else 0

    # Need a WR represented in the evaluated shortlist.
    wloc=np.flatnonzero(pos[sidx]==2)
    if not len(wloc):
        snap["WR_Primary_suppressor"]="not_in_evaluated_shortlist"
        return snap

    # Highest Fantasy Edge evaluated WR.
    wj=int(wloc[np.argmax(ev[wloc])])
    wg=int(sidx[wj])
    snap["WR_Best_idx"]=wg
    snap["WR_Best_player"]=str(pool.iloc[wg].player)
    snap["WR_Best_market_pick"]=float(market[wg]) if np.isfinite(market[wg]) else np.nan
    snap["WR_Best_model_rank"]=float(model_rank[wg]) if np.isfinite(model_rank[wg]) else np.nan
    snap["WR_Best_projection"]=float(projection[wg])
    snap["WR_Best_vorp"]=float(vorp[wg])
    snap["WR_Best_eval"]=float(ev[wj])

    # Where does the WR rank in FE evaluation at the pick?
    rank_order=np.argsort(ev)[::-1]
    top3=set(rank_order[:min(3,len(rank_order))].tolist())
    snap["WR_Top3_FE"]=1 if wj in top3 else 0

    cj=np.flatnonzero(sidx==chosen_global)
    if len(cj):
        cj=int(cj[0])
        snap["WR_Selected_eval"]=float(ev[cj])
        snap["WR_Eval_gap"]=float(ev[wj]-ev[cj])

        # The evaluation formula is frozen:
        # 0.38*draft_score + 3.60*roster_delta + 0.18*edge.
        cr=np.where(np.isnan(market[sidx]),float(current_pick),market[sidx])
        edge=np.clip(cr-model_rank[sidx],-24,24)
        edge=np.where(np.isin(pos[sidx],[4,5]),edge*.45,edge)

        ds_gap=.38*(draft_score[wg]-draft_score[chosen_global])
        roster_gap=3.60*(delta[wj]-delta[cj])
        edge_gap=.18*(edge[wj]-edge[cj])
        snap["WR_DraftScore_component_gap"]=float(ds_gap)
        snap["WR_RosterDelta_component_gap"]=float(roster_gap)
        snap["WR_Edge_component_gap"]=float(edge_gap)
        snap["WR_Total_pre_execution_gap"]=float(ds_gap+roster_gap+edge_gap)

        # Identify the component most responsible for WR trailing the chosen player.
        comps={
            "player_model/draft_score":float(ds_gap),
            "roster_marginal_utility":float(roster_gap),
            "model-vs-market_edge":float(edge_gap)
        }
        snap["WR_Primary_suppressor"]=min(comps,key=comps.get)

    # Market value being passed: positive = WR has earlier/better market expectation.
    sm=snap["WR_Selected_market_pick"]; wm=snap["WR_Best_market_pick"]
    if pd.notna(sm) and pd.notna(wm):
        snap["WR_Market_value_passed"]=float(sm-wm)

    if pd.notna(wm):
        snap["WR_Survival_est"]=survival_probability(wm,next_pick,randomness)

    return snap


def wr_pipeline_summary(draft):
    """Aggregate one FE draft's R1-10 WR audit rows."""
    if draft is None or len(draft)==0 or "WR_Audit" not in draft.columns:
        return {}
    d=draft[pd.to_numeric(draft.WR_Audit,errors="coerce").fillna(0).eq(1)].copy()
    if d.empty:
        return {}

    out={
        "WR_Audit_picks":int(len(d)),
        "WR_market_best_rate":float(pd.to_numeric(d.WR_Market_best_is_WR,errors="coerce").mean()),
        "WR_top3_FE_rate":float(pd.to_numeric(d.WR_Top3_FE,errors="coerce").mean()),
        "WR_selected_rate":float((d.WR_Selected_position=="WR").mean()),
        "WR_avg_eval_gap":float(pd.to_numeric(d.WR_Eval_gap,errors="coerce").mean()),
        "WR_avg_market_value_passed":float(pd.to_numeric(d.WR_Market_value_passed,errors="coerce").mean()),
        "WR_avg_survival_est":float(pd.to_numeric(d.WR_Survival_est,errors="coerce").mean()),
        "WR_actual_survival_rate":float(pd.to_numeric(d.WR_Survived_actual,errors="coerce").mean()),
        "WR_avg_draftscore_gap":float(pd.to_numeric(d.WR_DraftScore_component_gap,errors="coerce").mean()),
        "WR_avg_rosterdelta_gap":float(pd.to_numeric(d.WR_RosterDelta_component_gap,errors="coerce").mean()),
        "WR_avg_edge_gap":float(pd.to_numeric(d.WR_Edge_component_gap,errors="coerce").mean()),
        "WR_avg_projection_gap":float(
            (pd.to_numeric(d.WR_Best_projection,errors="coerce")-
             pd.to_numeric(d.WR_Selected_projection,errors="coerce")).mean()
        ),
        "WR_avg_vorp_gap":float(
            (pd.to_numeric(d.WR_Best_vorp,errors="coerce")-
             pd.to_numeric(d.WR_Selected_vorp,errors="coerce")).mean()
        )
    }

    suppress=d.WR_Primary_suppressor.value_counts(normalize=True)
    for key in ["player_model/draft_score","roster_marginal_utility","model-vs-market_edge","not_in_evaluated_shortlist"]:
        safe=key.replace("/","_").replace("-","_")
        out[f"WR_suppress_{safe}"]=float(suppress.get(key,0))

    # Phase splits: R1-3, R4-6, R7-10.
    phases=[("R1_3",1,3),("R4_6",4,6),("R7_10",7,10)]
    rnd=pd.to_numeric(d.mock_round,errors="coerce")
    for name,lo,hi in phases:
        g=d[(rnd>=lo)&(rnd<=hi)]
        out[f"WR_{name}_market_best_rate"]=float(pd.to_numeric(g.WR_Market_best_is_WR,errors="coerce").mean()) if len(g) else np.nan
        out[f"WR_{name}_top3_FE_rate"]=float(pd.to_numeric(g.WR_Top3_FE,errors="coerce").mean()) if len(g) else np.nan
        out[f"WR_{name}_selected_rate"]=float((g.WR_Selected_position=="WR").mean()) if len(g) else np.nan
        out[f"WR_{name}_eval_gap"]=float(pd.to_numeric(g.WR_Eval_gap,errors="coerce").mean()) if len(g) else np.nan
        out[f"WR_{name}_market_value_passed"]=float(pd.to_numeric(g.WR_Market_value_passed,errors="coerce").mean()) if len(g) else np.nan
        out[f"WR_{name}_survival_actual"]=float(pd.to_numeric(g.WR_Survived_actual,errors="coerce").mean()) if len(g) else np.nan
    return out
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None, room_profile="Consensus"):
    ...
    user_diag=[]
    pending_wr_idx=None; pending_wr_diag=None
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            if pending_wr_idx is not None and pending_wr_diag is not None:
                user_diag[pending_wr_diag]["WR_Survived_actual"]=1.0 if alive[int(pending_wr_idx)] else 0.0
                pending_wr_idx=None; pending_wr_diag=None
            ...
            if model_user:
                ...
                diag={}
                if round_no<=10:
                    diag=wr_pipeline_audit_snapshot(
                        fast,eidx,sidx,delta,eval_score,chosen,overall,nxt,randomness
                    )
            else:
            ...
            user_idx.append(chosen); user_pick.append(overall); user_round.append(round_no); user_diag.append(diag)
            if model_user and round_no<=10 and diag.get("WR_Best_idx") is not None and int(diag["WR_Best_idx"])!=int(chosen):
                pending_wr_idx=int(diag["WR_Best_idx"])
                pending_wr_diag=len(user_diag)-1
            counts[pos[chosen]]+=1
        else:
    ...
    if len(user_diag)==len(roster):
        dd=pd.DataFrame(user_diag,index=roster.index)
...
st.title("🏈 Fantasy Edge V7.21 — WR Valuation Pipeline Audit")
st.caption("V7.21 • V7.16 draft engine frozen • WR scoring-pipeline audit • diagnostic only")
st.caption("Build ID: V7.21-WRVPA-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v721_live_slot")
    ...
    if "v721_target_queue" not in st.session_state:
        st.session_state.v721_target_queue={}
    queue=st.session_state.v721_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v721_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.21 is diagnostic only. V7.16 draft logic is frozen while WR valuation is traced through player score, roster utility, market edge, and execution timing in Rounds 1-10.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.21 robustness report")
            ...
            st.download_button("⬇️ Download V7.21 diagnostic CSV",csv,"fantasy_edge_v7_21_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.21 WR pipeline diagnostics")
        ...
        if "v721_independent_results" not in st.session_state:
            st.session_state.v721_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v721_independent_results"),pd.DataFrame) else "ready to run"))
        if "v721_independent_summary" not in st.session_state:
            st.session_state.v721_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v721_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v721_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v721_run")
        clear_iv=clear_col.button("Clear independent results",key="v721_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                for ds in iv_slots:
                    for room_name in iv_rooms:
                        for j in range(int(iv_sims)):
                            ...
                            row.update(wr_pipeline_summary(fe))
                ...
                st.session_state.v721_independent_results=iv
                st.session_state.v721_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v721_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v721_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧬 V7.21 WR Valuation Pipeline Audit")
            st.caption("Diagnostic only. V7.16 draft choices are frozen. This isolates whether WR is being suppressed by player evaluation, roster marginal utility, market-edge logic, or execution timing.")

            w1,w2,w3,w4=st.columns(4)
            w1.metric("WR = market-best available",f"{iv_saved.WR_market_best_rate.mean():.0%}")
            w2.metric("WR in FE top 3",f"{iv_saved.WR_top3_FE_rate.mean():.0%}")
            w3.metric("WR actually selected",f"{iv_saved.WR_selected_rate.mean():.0%}")
            w4.metric("Passed WR survives",f"{iv_saved.WR_actual_survival_rate.mean():.0%}")

            w5,w6,w7,w8=st.columns(4)
            w5.metric("WR eval gap vs selected",f"{iv_saved.WR_avg_eval_gap.mean():+.2f}")
            w6.metric("WR market value passed",f"{iv_saved.WR_avg_market_value_passed.mean():+.2f} picks")
            w7.metric("WR projection gap",f"{iv_saved.WR_avg_projection_gap.mean():+.2f} PPG")
            w8.metric("WR VORP gap",f"{iv_saved.WR_avg_vorp_gap.mean():+.2f}")

            component_df=pd.DataFrame([
                {"Pipeline component":"Player model / draft score",
                 "WR advantage vs selected":iv_saved.WR_avg_draftscore_gap.mean()},
                {"Pipeline component":"Roster marginal utility",
                 "WR advantage vs selected":iv_saved.WR_avg_rosterdelta_gap.mean()},
                {"Pipeline component":"Model-vs-market edge",
                 "WR advantage vs selected":iv_saved.WR_avg_edge_gap.mean()},
            ])
            st.markdown("#### Where WR gains or loses evaluation points")
            st.dataframe(component_df.round(2),use_container_width=True,hide_index=True)

            suppress_df=pd.DataFrame([
                {"Primary suppressor":"Player model / draft score",
                 "Share":iv_saved.WR_suppress_player_model_draft_score.mean()},
                {"Primary suppressor":"Roster marginal utility",
                 "Share":iv_saved.WR_suppress_roster_marginal_utility.mean()},
                {"Primary suppressor":"Model-vs-market edge",
                 "Share":iv_saved.WR_suppress_model_vs_market_edge.mean()},
                {"Primary suppressor":"Not in evaluated shortlist",
                 "Share":iv_saved.WR_suppress_not_in_evaluated_shortlist.mean()},
            ])
            suppress_df["Share"]=suppress_df["Share"].map(lambda x:f"{x:.0%}")
            st.markdown("#### Primary WR suppressor")
            st.dataframe(suppress_df,use_container_width=True,hide_index=True)

            phase_wr=[]
            for label,key in [("R1-3","R1_3"),("R4-6","R4_6"),("R7-10","R7_10")]:
                phase_wr.append({
                    "Phase":label,
                    "WR market-best rate":iv_saved[f"WR_{key}_market_best_rate"].mean(),
                    "WR FE-top3 rate":iv_saved[f"WR_{key}_top3_FE_rate"].mean(),
                    "WR selected rate":iv_saved[f"WR_{key}_selected_rate"].mean(),
                    "WR eval gap":iv_saved[f"WR_{key}_eval_gap"].mean(),
                    "WR market value passed":iv_saved[f"WR_{key}_market_value_passed"].mean(),
                    "Passed WR survives":iv_saved[f"WR_{key}_survival_actual"].mean(),
                })
            st.markdown("#### WR pipeline by draft phase")
            st.dataframe(pd.DataFrame(phase_wr).round(2),use_container_width=True,hide_index=True)

            slot_wr=[]
            for ds,g in iv_saved.groupby("Draft_slot"):
                slot_wr.append({
                    "Draft slot":int(ds),
                    "WR market-best":g.WR_market_best_rate.mean(),
                    "WR FE top 3":g.WR_top3_FE_rate.mean(),
                    "WR selected":g.WR_selected_rate.mean(),
                    "WR eval gap":g.WR_avg_eval_gap.mean(),
                    "WR market value passed":g.WR_avg_market_value_passed.mean(),
                    "Passed WR survives":g.WR_actual_survival_rate.mean(),
                    "Overall independent gap":g.Difference.mean()
                })
            st.markdown("#### WR pipeline by draft slot")
            st.dataframe(pd.DataFrame(slot_wr).round(2),use_container_width=True,hide_index=True)

            # Automatic interpretation: identify the most negative evaluation component.
            comps={
                "player model / draft score":float(iv_saved.WR_avg_draftscore_gap.mean()),
                "roster marginal utility":float(iv_saved.WR_avg_rosterdelta_gap.mean()),
                "model-vs-market edge":float(iv_saved.WR_avg_edge_gap.mean())
            }
            worst_wr=min(comps,key=comps.get)
            worst_val=comps[worst_wr]
            market_best=float(iv_saved.WR_market_best_rate.mean())
            top3=float(iv_saved.WR_top3_FE_rate.mean())
            selected=float(iv_saved.WR_selected_rate.mean())
            survives=float(iv_saved.WR_actual_survival_rate.mean())

            if worst_val < -0.50:
                st.warning(
                    f"WR suppression signal: {worst_wr} is the largest negative component "
                    f"({worst_val:+.2f} evaluation points vs the selected player). "
                    f"WR is market-best {market_best:.0%} of audited picks, FE-top3 {top3:.0%}, "
                    f"but selected only {selected:.0%}."
                )
            elif top3 > selected + .15 and survives < .50:
                st.warning(
                    f"WR execution-timing signal: WR reaches FE's top 3 {top3:.0%} of the time but is selected "
                    f"only {selected:.0%}, and passed WRs survive just {survives:.0%}. "
                    "The scoring pipeline may be reasonable while execution waits too often."
                )
            else:
                st.info(
                    "No single strong WR suppression mechanism dominates. Use the phase/slot tables to determine "
                    "whether the earlier low-WR pattern is player-specific rather than a systemic scoring bias."
                )
            ...
            st.download_button(
                "⬇️ Download V7.21 WR pipeline diagnostics CSV",
                iv_csv,
                "fantasy_edge_v7_21_wr_pipeline_audit.csv",
                "text/csv",
                key="v721_download"
            )
