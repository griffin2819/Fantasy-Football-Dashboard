...
def _single_authority_strategy(y,roster,slots,round_no,current_pick,next_pick):
    ...
    _reach_cost=np.minimum(_reach_ahead,45.0)*0.30
    ...
    for idx in keep:
        ...
        z.at[idx,"strategic_pick_value"]=(
            ...
            1.35*wait+
            float(room.loc[idx])+
            1.55*float(_cross_slot.loc[idx])+
            1.00*float(_quality_anchor.loc[idx])+
            0.55*float(_construction_context.loc[idx])-
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    _near_market=_legal_now & (_reach_gap<=8.0)
    if _near_market.any():
        for _idx in list(x.index[_legal_now & (_reach_gap>=18.0)]):
            _similar=(
                _near_market &
                (_vorp >= float(_vorp.loc[_idx])-0.80) &
                (_mrv >= float(_mrv.loc[_idx])-1.50) &
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        target_sims=100
        _benchmark_build="FE-V6.6-PLAYER-VALUE-AUTHORITY-20260826"
