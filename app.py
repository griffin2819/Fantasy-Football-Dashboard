...
st.title("🏈 Fantasy Edge V7.33 — Pick 7/12 R4-6 Decision Audit")
st.caption("V7.33 • V7.24 9× champion frozen • Pick 7/12 R4-6 decision audit")
st.caption("Build ID: V7.33-P712R46A-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v733_live_slot")
    ...
    if "v733_target_queue" not in st.session_state:
        st.session_state.v733_target_queue={}
    queue=st.session_state.v733_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v733_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.33 keeps the V7.24 9× champion fully frozen while Picks 7 and 12 are audited for the remaining R4-6 failure mode.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.33 robustness report")
        ...
        st.markdown("### 🧪 V7.33 late-slot audit validation")
        st.info("V7.33 is diagnostic only. No VORP, replacement, WR, roster-utility, faller, or slot-specific rule is changed.")
        ...
        if "v733_independent_results" not in st.session_state:
            st.session_state.v733_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v733_independent_results"),pd.DataFrame) else "ready to run"))
        if "v733_independent_summary" not in st.session_state:
            st.session_state.v733_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v733_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v733_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v733_run")
        clear_iv=clear_col.button("Clear independent results",key="v733_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v733_independent_results=iv
                st.session_state.v733_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v733_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v733_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.33 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.33 focuses the R4-6 diagnostics on Picks 7 and 12.")
            ...
            st.download_button(
                "⬇️ Download V7.33 Pick 7/12 audit CSV",
                iv_csv,
                "fantasy_edge_v7_33_pick7_12_r4_6_audit.csv",
                "text/csv",
                key="v733_download"
            )
