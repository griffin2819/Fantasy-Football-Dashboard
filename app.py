...
st.title("🏈 Fantasy Edge V7.16 — Mid-Round Value Discipline")
st.caption("V7.16 • V7.15 stable base • surgical R4-6 value discipline • all other phases frozen")
st.caption("Build ID: V7.16-MRVD-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v716_live_slot")
    ...
    if "v716_target_queue" not in st.session_state:
        st.session_state.v716_target_queue={}
    queue=st.session_state.v716_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v716_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.16 keeps R1-3, R7-10 and R11+ frozen. Only R4-6 changes: 9+ pick fall • top-18 model band • 4-pick improvement, with a weak 6+ reach safeguard.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.16 robustness report")
        ...
        st.markdown("### 🧪 V7.16 independent validation")
        ...
        if "v716_independent_results" not in st.session_state:
            st.session_state.v716_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v716_independent_results"),pd.DataFrame) else "ready to run"))
        if "v716_independent_summary" not in st.session_state:
            st.session_state.v716_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v716_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v716_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v716_run")
        clear_iv=clear_col.button("Clear independent results",key="v716_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v716_independent_results=iv
                st.session_state.v716_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v716_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v716_independent_summary") or {}
            st.success(
            ...
            st.download_button(
                "⬇️ Download V7.16 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_16_mid_round_validation.csv",
                "text/csv",
                key="v716_download"
            )
