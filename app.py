...
st.title("🏈 Fantasy Edge V7.53 — R11+ Survival Calibration Audit")
st.caption("V7.53 • V7.50 champion frozen • R11+ survival calibration diagnostic")
st.caption("Build ID: V7.53-R11SURVCAL-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v753_live_slot")
    ...
    if "v753_target_queue" not in st.session_state:
        st.session_state.v753_target_queue={}
    queue=st.session_state.v753_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v753_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.53 keeps V7.50 as the live/default champion with the 55.97 rule unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.53 robustness report")
        ...
        st.markdown("### 🧪 V7.53 champion baseline validation")
        st.info("V7.53 is diagnostic/calibration only. No live survival curve, execution, valuation, position, or threshold changes are introduced.")
        ...
        if "v753_independent_results" not in st.session_state:
            st.session_state.v753_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v753_independent_results"),pd.DataFrame) else "ready to run"))
        if "v753_independent_summary" not in st.session_state:
            st.session_state.v753_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v753_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v753_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v753_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v753_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v753_independent_results=iv
                st.session_state.v753_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v753_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v753_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.53 Champion Baseline Check")
            st.caption("V7.53 keeps the V7.50 champion fully frozen and measures actual R11+ faller survival against the current logistic survival model.")
            ...
            st.download_button(
                "⬇️ Download V7.53 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_53_champion_baseline.csv",
                "text/csv",
                key="v753_baseline_download"
            )
        ...
        st.markdown("### 📐 V7.53 — R11+ Survival Calibration Audit")
        st.caption(
            "Diagnostic/calibration only. V7.50 remains the frozen champion. "
            "This measures actual R11-15 faller survival against the current survival model and derives an empirical late-round curve without changing live execution."
        )

        if "v753_results" not in st.session_state: st.session_state.v753_results=None

        sims=st.number_input(
            "Survival-calibration simulations per slot × room",
            min_value=10,max_value=40,value=20,step=5,key="v753_sims",
            help="Default: 4 slots × 5 rooms × 20 simulations = 400 champion drafts / roughly 2,000 R11+ decisions."
        )
        a,b=st.columns(2)
        run=a.button("Run R11+ survival calibration",type="primary",key="v753_run")
        clear=b.button("Clear survival calibration",key="v753_clear")
        if clear:
            st.session_state.v753_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=953000+int(ds)*10000+j*127+sum(ord(c) for c in room_name)
                        ...
                        for _,r in late.iterrows():
                            cur=int(r.get("mock_pick",0))
                            rnd=int(r.get("mock_round",0))
                            nxt=next_user_pick(cur,int(ds),int(teams),15)

                            faller_pick=pd.to_numeric(
                                pd.Series([r.get("MR_Passed_faller_idx",np.nan)]),errors="coerce"
                            ).iloc[0]
                            faller_market=pd.to_numeric(
                                pd.Series([r.get("MR_Market_best_pick",np.nan)]),errors="coerce"
                            ).iloc[0]
                            # Use passed-faller market rank when available; fallback to market-best rank.
                            pf_name=str(r.get("MR_Passed_faller_player",""))
                            if pf_name:
                                # recover by name from the fast pool for the exact market pick
                                match=fast_champ["pool"].index[
                                    fast_champ["pool"]["player"].astype(str).eq(pf_name)
                                ].tolist()
                                if match:
                                    gm=int(match[0])
                                    mp=float(fast_champ["market_pick"][gm]) if np.isfinite(fast_champ["market_pick"][gm]) else np.nan
                                else:
                                    mp=np.nan
                            else:
                                mp=np.nan
                            ...
                            pred=pd.to_numeric(
                                pd.Series([r.get("MR_Passed_faller_survival_est",np.nan)]),errors="coerce"
                            ).iloc[0]
                            actual=pd.to_numeric(
                                pd.Series([r.get("MR_Passed_faller_survived_actual",np.nan)]),errors="coerce"
                            ).iloc[0]
                            fall_amt=pd.to_numeric(
                                pd.Series([r.get("MR_Passed_faller_amount",np.nan)]),errors="coerce"
                            ).iloc[0]

                            rows.append({
                                "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                "Round":rnd,"Current_pick":cur,"Next_user_pick":nxt,
                                "Picks_until_next":(int(nxt)-cur) if nxt is not None else np.nan,
                                "Faller_player":pf_name,
                                "Faller_position":r.get("MR_Passed_faller_position",""),
                                "Faller_market_pick":mp,
                                "Faller_amount":fall_amt,
                                "Market_minus_next_pick":(mp-float(nxt)) if pd.notna(mp) and nxt is not None else np.nan,
                                "Current_survival_est":pred,
                                "Actual_survival":actual,
                                "Calibration_error":actual-pred if pd.notna(actual) and pd.notna(pred) else np.nan,
                                ...
                                "Selected_vs_faller_eval_gap":r.get("MR_Passed_faller_eval_gap",np.nan),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%80==0 or done==total:
                            status.caption(f"R11+ survival calibration: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v753_results=df
            status.success(f"Survival calibration complete: {len(df)} late-round faller observations.")
            prog.empty()

        df=st.session_state.get("v753_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in [
                "Faller_market_pick","Faller_amount","Market_minus_next_pick",
                "Current_survival_est","Actual_survival","Calibration_error",
                "Picks_until_next","Selected_reach","Selected_vs_faller_eval_gap"
            ]:
                df[c]=pd.to_numeric(df[c],errors="coerce")

            valid=df[df.Actual_survival.notna() & df.Current_survival_est.notna()].copy()

            summary=pd.DataFrame([{
                "Observations":len(valid),
                "Current predicted survival":valid.Current_survival_est.mean(),
                "Actual survival":valid.Actual_survival.mean(),
                "Mean calibration error":valid.Calibration_error.mean(),
                "Median calibration error":valid.Calibration_error.median(),
                "Mean picks until next turn":valid.Picks_until_next.mean(),
                "Mean faller amount":valid.Faller_amount.mean(),
                "Mean market-minus-next-pick":valid.Market_minus_next_pick.mean(),
            }])
            st.markdown("#### Current late-round survival calibration")
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            # Bin by the exact variable the current logistic model uses: market_pick - next_pick.
            cal=valid[valid.Market_minus_next_pick.notna()].copy()
            if len(cal):
                bins=[-120,-60,-40,-30,-20,-10,0,10,20,40,60,120]
                labels=["≤-60","-60:-40","-40:-30","-30:-20","-20:-10","-10:0","0:10","10:20","20:40","40:60","60+"]
                cal["Distance_bin"]=pd.cut(cal.Market_minus_next_pick,bins=bins,labels=labels,include_lowest=True,right=False)
                curve=cal.groupby("Distance_bin",observed=True,as_index=False).agg(
                    Observations=("Actual_survival","count"),
                    Avg_distance=("Market_minus_next_pick","mean"),
                    Predicted_survival=("Current_survival_est","mean"),
                    Actual_survival=("Actual_survival","mean"),
                    Avg_faller=("Faller_amount","mean"),
                    Avg_turn_distance=("Picks_until_next","mean")
                )
                curve["Calibration_error"]=curve.Actual_survival-curve.Predicted_survival
                st.markdown("#### Empirical survival curve: market rank − next user pick")
                st.dataframe(curve.round(4),use_container_width=True,hide_index=True)

                # Coarser recommended empirical lookup bands, requiring enough observations.
                robust_curve=curve[curve.Observations>=20].copy()
                if len(robust_curve):
                    robust_curve["Recommended_survival"]=robust_curve.Actual_survival.clip(.05,.95)
                    st.markdown("#### Candidate R11+ empirical lookup bands")
                    st.dataframe(
                        robust_curve[["Distance_bin","Observations","Avg_distance","Recommended_survival","Predicted_survival","Calibration_error"]].round(4),
                        use_container_width=True,hide_index=True
                    )

            st.markdown("#### Survival calibration by draft slot")
            slot=valid.groupby("Draft_slot",as_index=False).agg(
                Observations=("Actual_survival","count"),
                Predicted=("Current_survival_est","mean"),
                Actual=("Actual_survival","mean"),
                Calibration_error=("Calibration_error","mean"),
                Turn_distance=("Picks_until_next","mean"),
                Faller_amount=("Faller_amount","mean")
            )
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Survival calibration by round")
            rnd=valid.groupby("Round",as_index=False).agg(
                Observations=("Actual_survival","count"),
                Predicted=("Current_survival_est","mean"),
                Actual=("Actual_survival","mean"),
                Calibration_error=("Calibration_error","mean"),
            ...
            st.dataframe(rnd.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Survival calibration by room")
            room=valid.groupby("Room",as_index=False).agg(
                Observations=("Actual_survival","count"),
                Predicted=("Current_survival_est","mean"),
                Actual=("Actual_survival","mean"),
                Calibration_error=("Calibration_error","mean")
            ...
            # Simple calibration decision.
            actual=float(valid.Actual_survival.mean())
            pred=float(valid.Current_survival_est.mean())
            err=actual-pred
            slot_spread=float(slot.Actual.max()-slot.Actual.min()) if len(slot) else np.nan
            room_spread=float(room.Actual.max()-room.Actual.min()) if len(room) else np.nan

            if err>=.30 and (pd.isna(room_spread) or room_spread<=.20):
                st.warning(
                    f"The current late-round survival model is materially under-calibrated: predicted {pred:.1%} vs actual {actual:.1%} "
                    f"(+{err:.1%} error). The miss is broad across rooms. V7.54 should test a dedicated R11+ empirical survival curve while leaving V7.50 player valuation frozen."
                )
            elif err>=.20:
                st.warning(
                    f"Late-round survival is under-estimated by {err:.1%}, but calibration varies enough by slot/room that V7.54 should use a slot-aware or turn-distance-aware curve rather than one global late-round constant."
                )
            else:
                st.info(
                    "The larger sample does not confirm a severe global late-round survival miss. Do not change the execution model until the bin-level curve is inspected."
                )

            st.download_button(
                "⬇️ Download V7.53 R11+ survival calibration CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_53_r11plus_survival_calibration.csv",
                "text/csv",key="v753_download"
            )
