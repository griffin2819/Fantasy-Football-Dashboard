...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Instant Navigation", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
st.caption("⚡ Instant navigation active • only the selected page is processed • external refresh is optional")
...
@st.cache_data(ttl=1800,show_spinner=False)
def _v756_week_matchup_index(team,pos,season,week):
...
def _v756_multiweek_player(row,season,week,horizon=6):
    ...
    for off in range(max(1,int(horizon))):
        wk=int(week)+off
        if wk>18: break
        idx,opp=_v756_week_matchup_index(team,pos,int(season),wk)
...
def _v756_playoff_outlook(row,season):
    vals=[]
    for wk in (15,16,17):
        idx,opp=_v756_week_matchup_index(str(row.get("team","") or "").upper(),str(row.get("position","") or "").upper(),int(season),wk)
...
if _v777_page_idx==14:
    ...
    if my.empty:
        ...
        with _h2:
            if st.button("↻ Refresh",key="v758_refresh_cert",use_container_width=True):
        ...
        if st.session_state.pop("_v756_cert_after_refresh",False):
            st.success("Fresh feeds loaded and certification rerun.")
