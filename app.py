...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        batch_size=st.selectbox(
            ...
            options=[4,8,12,20],
            index=1,
            help="Drafts run in parallel. 8 is the recommended Streamlit Cloud batch size; larger batches may use more memory."
        )
        ...
        if run:
            ...
            _seeds=list(range(_seed0,_seed0+this_batch))
            ...
            _workers=max(1,min(4,this_batch))
            _finished=[]
            with ThreadPoolExecutor(max_workers=_workers) as _pool:
                ...
                for _n,_future in enumerate(as_completed(_future_map),start=1):
            ...
            st.session_state.fe1000_next_seed=_seed0+this_batch
            _elapsed=max(time.perf_counter()-started,0.001)
            st.session_state["fe_last_batch_seconds"]=_elapsed
            st.session_state["fe_last_batch_rate"]=this_batch/_elapsed
            st.rerun()
