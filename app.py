...
st.title("🏈 Fantasy Edge V7.91 — Round-2 Generic Adaptation Challenger")
st.caption("V7.91 • V7.67 frozen champion • Round-2 generic challenger")
st.caption("Build ID: V7.91-R2-GENERIC-CHALLENGER-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🧪 V7.91 — Round-2 Generic Adaptation Challenger")
        st.caption(
            "Full-draft matched challenger. V7.67 remains frozen/live. "
            "The pre-registered V7.90 signature is tested unchanged: in Round 2 only, switch only when a genuinely alive+eligible alternative has BOTH "
            "higher frozen draft score and higher VORP and its effective market distance is ≤10 picks. "
            "No room, player, position, or draft-slot hard-coding."
        )

        if "v791_results" not in st.session_state:
            st.session_state.v791_results=None

        sims=st.number_input(
            "V7.91 matched simulations per slot × room",
            min_value=10,max_value=50,value=20,step=5,key="v791_sims",
            help="Default: all 12 slots × all 5 rooms × 20 = 1,200 matched sets. Trigger-first reruns only activated challengers."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.91 Round-2 challenger",type="primary",key="v791_run")
        clear=b.button("Clear V7.91 challenger",key="v791_clear")
        if clear:
            st.session_state.v791_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=991000+int(ds)*100000+j*347+sum(ord(c) for c in room_name)
                        ...
                        if len(rr):
                            r=rr.iloc[0]
                            trigger=bool(int(pd.to_numeric(pd.Series([r.get("R791_Would_trigger",0)]),errors="coerce").fillna(0).iloc[0]))
                        ...
                        if trigger:
                            ...
                            changed=0; overall=r13=r46=r710=r11=0.0
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.91: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v791_results=df
            status.success(f"V7.91 complete: {len(df)} matched sets; {int(df.Challenger_rerun.sum())} challenger reruns.")
            prog.empty()

        df=st.session_state.get("v791_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            summary=pd.DataFrame([{
                ...
                "Conditional overall win":trig.Conditional_overall_win.mean() if len(trig) else np.nan,
            ...
            paths=trig.groupby(["Normal_R2_player","Alternative_R2_player","Alternative_position"],as_index=False).agg(
            ...
            st.markdown("#### Most common activated R2 paths")
            ...
            screen=(
                df.Overall_change.mean()>0
                and df.R1_3_change.mean()>=0
                and df.R4_6_change.mean()>=0
                and df.R7_10_change.mean()>=0
                and df.R11_plus_change.mean()>=0
                and len(trig)>0
                and cond>=.60
                and nonloss>=.70
            ...
            if screen:
                st.success("V7.91 SCREEN PASSED. Freeze this exact ≤10 signature and advance it to independent fresh-seed validation; V7.67 remains champion until validation.")
            else:
                st.warning("V7.91 does not clear screening. Keep V7.67 champion. Do not tune the ≤10 threshold from this sample.")

            st.download_button(
                "⬇️ Download V7.91 Round-2 generic adaptation challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_91_round2_generic_adaptation_challenger.csv",
                "text/csv",key="v791_download"
            )
