...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Unified Action Safety", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
# ---------------- V7.56 Fantasy Edge Command Center + Weekly Certification ----------------
with tabs[14]:
    st.subheader("🏠 Fantasy Edge Command Center")
    st.caption("One screen for readiness, actions, multi-week planning and league context.")
    my=board[board["player"].map(lambda n:_player_owner(state,n)=="1")].copy()
    if my.empty:
        ...
        _inj=int(my.get("fe_status",my.get("injury",pd.Series(index=my.index,dtype=str))).astype(str).ne("ACTIVE").sum())
        _season756=int(_v678_live_week.get("season",datetime.now().year)); _week756=int(_v678_live_week.get("week",1))

        # One-click weekly refresh: clear cached public feeds, rerun, then certify automatically.
        if st.button("🔄 Refresh + certify weekly data",type="primary",key="v756_refresh_cert"):
        ...
        badge="✅" if _overall=="READY" else ("⚠️" if _overall=="READY WITH WARNINGS" else "🛑")
        c1,c2,c3,c4=st.columns(4)
        c1.metric("Weekly status",f"{badge} {_overall}")
        c2.metric("Lineup",f"{lineup:.1f} PPG")
        c3.metric("Actions",len(_actions))
        c4.metric("Injury watch",_inj)

        if not _actions:
            st.success("✅ Your lineup is optimized. No action needed.")
        else:
            st.markdown("### Today")
        ...
        # 6-week plan for current starters; schedule + ROS + playoff context.
        st.markdown("### Next 6 weeks")
        _plan=[]
        for _,sr in starters.iterrows():
            ...
            _plan.append({"Player":r.get("player",""),"Pos":r.get("position",""),"6W Avg":mw["avg"],"6W Floor":mw["floor"],"Playoff":pg,"News Δ":_v756_news_impact(r)})
        if _plan:
            st.dataframe(pd.DataFrame(_plan).sort_values(["Pos","6W Avg"],ascending=[True,False]),use_container_width=True,hide_index=True)

        with st.expander("Weekly certification",expanded=False):
        ...
        with st.expander("Player-state details",expanded=False):
            _show=my[[c for c in ["player","position","team","fe_week","fe_week_low","fe_week_high","fe_ros","fe_vorp","fe_role","fe_calibrated_confidence","fe_opportunity_trend","fe_news_impact","fe_status"] if c in my.columns]].copy()
