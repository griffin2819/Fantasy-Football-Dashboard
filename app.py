...
def simulate_mock(board, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None):
    ...
    # Boolean availability mask avoids repeatedly scanning the entire player pool
    # with player.isin(drafted) on every single pick. This is a performance-only
    # change; the draft scoring/model logic below is unchanged.
    ...
    for overall in range(1,total+1):
        ...
        # pool has a stable RangeIndex, preserved through filtering/sorting.
        alive[int(choice.name)]=False
    return pd.DataFrame(user_rows)

state=load_state()

st.title("🏈 Fantasy Edge V6.3 — Grade Calibration")
st.caption("Validation release • calibrated grading • opportunity cost • roster balance • Yahoo + IDP")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            for i in range(int(sims)):
                ...
                eta=(elapsed/done)*(int(sims)-done) if done else 0
