...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    # Dynamic extra-impact-IDP opportunity comparison.
    # After required DL/DB are secured, a Tier 1-2 defender can beat redundant offensive
    # depth starting in Round 10. No defender is forced: deep offense survives if it has
    # a clear evaluation advantage.
    _extra_counts=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    _extra_total_idp=int(_extra_counts.get("DL",0))+int(_extra_counts.get("DB",0))
    _extra_ready=(
        int(round_no)>=10 and
        int(_extra_counts.get("DL",0))>=int(slots.get("DL",1)) and
        int(_extra_counts.get("DB",0))>=int(slots.get("DB",1)) and
        _extra_total_idp<4 and
        (int(_extra_counts.get("RB",0))>=4 or int(_extra_counts.get("WR",0))>=4)
    )
    if _extra_ready:
        _x_ropp=pd.to_numeric(x.get("roster_opportunity_adj",0),errors="coerce").fillna(0.0)
        _impact_extra=(
            x["position"].isin(["DL","DB"]) &
            x["idp_eligible"].fillna(False).astype(bool) &
            (pd.to_numeric(x["idp_quality_tier"],errors="coerce").fillna(5)<=2) &
        ...
        if _impact_extra.any():
            x.loc[_impact_extra,"evaluation_score"]+=10.0
            _best_extra=float(pd.to_numeric(x.loc[_impact_extra,"evaluation_score"],errors="coerce").max())

            _deep_extra=(
                (x["position"].eq("RB") & (int(_extra_counts.get("RB",0))>=4)) |
                (x["position"].eq("WR") & (int(_extra_counts.get("WR",0))>=4)) |
                (x["position"].eq("QB") & (int(_extra_counts.get("QB",0))>=1)) |
                (x["position"].eq("TE") & (int(_extra_counts.get("TE",0))>=1))
            )
            _not_clear=_deep_extra & (
                pd.to_numeric(x["evaluation_score"],errors="coerce") < (_best_extra+3.0)
            )
            x.loc[_not_clear,"evaluation_score"]-=24.0
            x.loc[_not_clear,"extra_idp_opportunity_loss"]=True
...
def grade_mock(roster, teams, slots):
    ...
    _construction_gap=max(0.0,92.0-construction)
    _construction_edge_penalty=min(12.0,_construction_gap*0.55)
...
def _fast_sim_user_pick(avail, roster_rows, rnd, overall, slot, teams, slots, randomness, rng):
    ...
    if rb>=4: score += np.where(pos=="RB",-10.0,0.0)
    ...
    # Dynamic extra-IDP competition. Once both required IDP starters are filled and
    # the offense has usable bench depth, an impact defender competes directly with
    # RB5/WR5/RB6/WR6 rather than waiting for a sixth offensive player.
    if dl>=1 and db>=1 and total_idp<4 and rnd>=10 and (rb>=4 or wr>=4):
        score += np.where(impact_idp,10.0,0.0)
        ...
        if impact_idp.any():
            ...
            # Deep offense must clearly outperform the impact defender to use the slot.
            score=np.where(deep_offense & (score < best_idp+3.0),score-24.0,score)
