    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                # V7.76 measurement-only: capture the TRUE Round-2 board state after every opponent pick.
                # No production behavior changes. Uses the actual alive+eligible eidx set.
                if round_no==2:
                    _r2_market=np.where(np.isnan(market_pick[eidx]),model_rank[eidx],market_pick[eidx])
                    _r2_legal=(injury[eidx] < 3) & (_r2_market <= float(overall)+10.0)
                    _r2_ids=eidx[_r2_legal]
                    _r2_ids=_r2_ids[_r2_ids!=int(chosen)]
                    ...
                    mr_diag.update({
                        "R776_Audit":1,
                        "R776_Actual_available_count":int(len(eidx)),
                        "R776_Market_respecting_count":int(len(_r2_ids)),
                    })
                    ...
                    for _k in range(5):
                        if _k < len(_r2_top):
                            _g=int(_r2_top[_k])
                            mr_diag.update({
                                f"R776_Alt{_k+1}_player":str(pool.iloc[_g].player),
                                f"R776_Alt{_k+1}_position":str(pool.iloc[_g].position),
                                f"R776_Alt{_k+1}_market_pick":float(market_pick[_g]) if np.isfinite(market_pick[_g]) else np.nan,
                                f"R776_Alt{_k+1}_model_rank":float(model_rank[_g]) if np.isfinite(model_rank[_g]) else np.nan,
                                f"R776_Alt{_k+1}_projection":float(fast["projection"][_g]),
                                f"R776_Alt{_k+1}_vorp":float(fast["vorp"][_g]),
                                f"R776_Alt{_k+1}_draft_score":float(draft_score[_g]),
                            })
                        else:
                            mr_diag.update({
                                f"R776_Alt{_k+1}_player":"",
                                f"R776_Alt{_k+1}_position":"",
                                f"R776_Alt{_k+1}_market_pick":np.nan,
                                f"R776_Alt{_k+1}_model_rank":np.nan,
                                f"R776_Alt{_k+1}_projection":np.nan,
                                f"R776_Alt{_k+1}_vorp":np.nan,
                                f"R776_Alt{_k+1}_draft_score":np.nan,
                            })

                # V7.83 measurement-only: isolate offense vs IDP at the R11-13 true-board boundary.
                if 11<=round_no<=13:
                    _r82_market=np.where(np.isnan(market_pick[eidx]),model_rank[eidx],market_pick[eidx])
                    _r82_ids=eidx[eidx!=int(chosen)]
                    if len(_r82_ids):
                        _r82_order=np.lexsort((
                            np.where(np.isnan(market_pick[_r82_ids]),1e9,market_pick[_r82_ids]),
                            -draft_score[_r82_ids]
                        ))
                        _r82_top=_r82_ids[_r82_order[:5]]
                    else:
                        _r82_top=np.asarray([],dtype=int)

                    mr_diag.update({
                        "R782_Audit":1,
                        "R782_Actual_available_count":int(len(eidx)),
                    })

                    _off_ids=_r82_ids[~pool.iloc[_r82_ids].position.isin(["DL","DB"]).to_numpy()] if len(_r82_ids) else np.asarray([],dtype=int)
                    _idp_ids=_r82_ids[pool.iloc[_r82_ids].position.isin(["DL","DB"]).to_numpy()] if len(_r82_ids) else np.asarray([],dtype=int)

                    def _r783_best(ids):
                        if not len(ids):
                            return None
                        _ord=np.lexsort((
                            np.where(np.isnan(market_pick[ids]),1e9,market_pick[ids]),
                            -draft_score[ids]
                        ))
                        return int(ids[_ord[0]])

                    _best_off=_r783_best(_off_ids)
                    _best_idp=_r783_best(_idp_ids)

                    mr_diag.update({
                        "R783_Best_off_player":str(pool.iloc[_best_off].player) if _best_off is not None else "",
                        "R783_Best_off_position":str(pool.iloc[_best_off].position) if _best_off is not None else "",
                        "R783_Best_off_market_pick":float(market_pick[_best_off]) if _best_off is not None and np.isfinite(market_pick[_best_off]) else np.nan,
                        "R783_Best_off_model_rank":float(model_rank[_best_off]) if _best_off is not None and np.isfinite(model_rank[_best_off]) else np.nan,
                        "R783_Best_off_projection":float(fast["projection"][_best_off]) if _best_off is not None else np.nan,
                        "R783_Best_off_vorp":float(fast["vorp"][_best_off]) if _best_off is not None else np.nan,
                        "R783_Best_off_draft_score":float(draft_score[_best_off]) if _best_off is not None else np.nan,

                        "R783_Best_idp_player":str(pool.iloc[_best_idp].player) if _best_idp is not None else "",
                        "R783_Best_idp_position":str(pool.iloc[_best_idp].position) if _best_idp is not None else "",
                        "R783_Best_idp_market_pick":float(market_pick[_best_idp]) if _best_idp is not None and np.isfinite(market_pick[_best_idp]) else np.nan,
                        "R783_Best_idp_model_rank":float(model_rank[_best_idp]) if _best_idp is not None and np.isfinite(model_rank[_best_idp]) else np.nan,
                        "R783_Best_idp_projection":float(fast["projection"][_best_idp]) if _best_idp is not None else np.nan,
                        "R783_Best_idp_vorp":float(fast["vorp"][_best_idp]) if _best_idp is not None else np.nan,
                        "R783_Best_idp_draft_score":float(draft_score[_best_idp]) if _best_idp is not None else np.nan,
                    })
                    ...
                    for _k in range(5):
                        if _k < len(_r82_top):
                            _g=int(_r82_top[_k])
                            mr_diag.update({
                                f"R782_Alt{_k+1}_player":str(pool.iloc[_g].player),
                                f"R782_Alt{_k+1}_position":str(pool.iloc[_g].position),
                                f"R782_Alt{_k+1}_market_pick":float(market_pick[_g]) if np.isfinite(market_pick[_g]) else np.nan,
                                f"R782_Alt{_k+1}_model_rank":float(model_rank[_g]) if np.isfinite(model_rank[_g]) else np.nan,
                                f"R782_Alt{_k+1}_projection":float(fast["projection"][_g]),
                                f"R782_Alt{_k+1}_vorp":float(fast["vorp"][_g]),
                                f"R782_Alt{_k+1}_draft_score":float(draft_score[_g]),
                            })
                        else:
                            mr_diag.update({
                                f"R782_Alt{_k+1}_player":"",
                                f"R782_Alt{_k+1}_position":"",
                                f"R782_Alt{_k+1}_market_pick":np.nan,
                                f"R782_Alt{_k+1}_model_rank":np.nan,
                                f"R782_Alt{_k+1}_projection":np.nan,
                                f"R782_Alt{_k+1}_vorp":np.nan,
                                f"R782_Alt{_k+1}_draft_score":np.nan,
                            })
...
st.title("🏈 Fantasy Edge V7.88 — Sharp/Value Failure-Point Audit")
st.caption("V7.88 • V7.67 frozen champion • Sharp/value diagnostic")
st.caption("Build ID: V7.88-SHARP-VALUE-FAILURE-AUDIT-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.88 keeps V7.67 frozen/live and audits the first reproducible failure point in Sharp/value rooms without changing production behavior.")
        ...
        st.markdown("### 🔬 V7.88 — Sharp/Value Failure-Point Audit")
        st.caption(
            "Measurement only. V7.67 remains the frozen production champion. "
            "V7.88 compares Sharp/value rooms directly against Consensus and ADP-heavy rooms to identify where the overall-grade deficit first develops. "
            "It tracks round, position, market delta, VORP, draft score, roster state, and cumulative grade-context proxies without changing any production behavior."
        )

        if "v788_results" not in st.session_state:
            st.session_state.v788_results=None

        sims=st.number_input(
            "V7.88 simulations per slot × room",
            min_value=10,max_value=50,value=20,step=5,key="v788_sims",
            help="Default: all 12 draft slots × 3 focus rooms × 20 = 720 fresh champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run Sharp/value failure-point audit",type="primary",key="v788_run")
        clear=b.button("Clear V7.88 audit",key="v788_clear")
        if clear:
            st.session_state.v788_results=None
            st.rerun()

        if run:
            rooms=["Consensus","ADP-heavy","Sharp/value"]
            slots_to_test=list(range(1,int(teams)+1))
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=988000+int(ds)*100000+j*317+sum(ord(c) for c in room_name)

                        d=simulate_mock_fast(
                            pool,int(teams),int(ds),15,state["roster_slots"],
                        ...
                        # Cumulative roster counts by pick to reveal when Sharp/value starts
                        # forcing a different construction path.
                        counts={}
                        cumulative_market=0.0
                        cumulative_vorp=0.0
                        cumulative_score=0.0
                        n_market=n_vorp=n_score=0

                        for _,r in d.sort_values("mock_round").iterrows():
                            rnd=int(pd.to_numeric(pd.Series([r.get("mock_round",0)]),errors="coerce").fillna(0).iloc[0])
                            pos=str(r.get("position",""))
                            counts[pos]=counts.get(pos,0)+1

                            pick=pd.to_numeric(pd.Series([r.get("mock_pick",np.nan)]),errors="coerce").iloc[0]
                            market=pd.to_numeric(pd.Series([r.get("market_pick",np.nan)]),errors="coerce").iloc[0]
                            delta=(market-pick) if pd.notna(market) and pd.notna(pick) else np.nan
                            vorp=pd.to_numeric(pd.Series([r.get("vorp",np.nan)]),errors="coerce").iloc[0]
                            score=pd.to_numeric(pd.Series([r.get("draft_score",np.nan)]),errors="coerce").iloc[0]

                            if pd.notna(delta):
                                cumulative_market+=float(delta); n_market+=1
                            if pd.notna(vorp):
                                cumulative_vorp+=float(vorp); n_vorp+=1
                            if pd.notna(score):
                                cumulative_score+=float(score); n_score+=1

                            rs=state["roster_slots"]
                            def _need(key,poskey):
                                try: tgt=int(rs.get(key,0))
                                except Exception: tgt=0
                                return max(tgt-counts.get(poskey,0),0)

                            rows.append({
                                "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                "Round":rnd,"Pick":pick,"Player":str(r.get("player","")),"Position":pos,
                                "Market_pick":market,"Market_delta":delta,
                                "Projection":r.get("projection",np.nan),
                                "VORP":vorp,"Draft_score":score,"Model_rank":r.get("model_rank",np.nan),
                                "Overall_grade":comp.get("overall",np.nan),
                                "Cum_avg_market_delta":cumulative_market/max(n_market,1),
                                "Cum_avg_VORP":cumulative_vorp/max(n_vorp,1),
                                "Cum_avg_draft_score":cumulative_score/max(n_score,1),
                                "QB_count":counts.get("QB",0),"RB_count":counts.get("RB",0),
                                "WR_count":counts.get("WR",0),"TE_count":counts.get("TE",0),
                                "DL_count":counts.get("DL",0),"DB_count":counts.get("DB",0),
                                "QB_need":_need("QB","QB"),"RB_need":_need("RB","RB"),
                                "WR_need":_need("WR","WR"),"TE_need":_need("TE","TE"),
                                "DL_need":_need("DL","DL"),"DB_need":_need("DB","DB"),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%120==0 or done==total:
                            status.caption(f"V7.88 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v788_results=df
            status.success(f"V7.88 complete: {total} champion drafts / {len(df)} pick-level decisions.")
            prog.empty()

        df=st.session_state.get("v788_results")
        if isinstance(df,pd.DataFrame) and len(df):
            d=df.copy()
            for c in ["Market_delta","VORP","Draft_score","Overall_grade","Cum_avg_market_delta","Cum_avg_VORP","Cum_avg_draft_score"]:
                d[c]=pd.to_numeric(d[c],errors="coerce")

            # Draft-level overall comparison.
            draft_level=d.groupby(["Draft_slot","Room","Simulation","Seed"],as_index=False).agg(
                Overall_grade=("Overall_grade","first")
            )
            room_overall=draft_level.groupby("Room",as_index=False).agg(
                Drafts=("Seed","count"),
                Avg_overall=("Overall_grade","mean"),
                Std_overall=("Overall_grade","std"),
                Min_overall=("Overall_grade","min"),
                Max_overall=("Overall_grade","max"),
            )

            # Round-level pick metrics.
            round_room=d.groupby(["Room","Round"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Avg_VORP=("VORP","mean"),
                Avg_draft_score=("Draft_score","mean"),
                Cum_market_delta=("Cum_avg_market_delta","mean"),
                Cum_VORP=("Cum_avg_VORP","mean"),
                Cum_draft_score=("Cum_avg_draft_score","mean"),
            )

            # Pivot to quantify Sharp/value minus baseline rooms.
            baseline=round_room[round_room.Room.isin(["Consensus","ADP-heavy"])].groupby("Round",as_index=False).agg(
                Baseline_market_delta=("Avg_market_delta","mean"),
                Baseline_VORP=("Avg_VORP","mean"),
                Baseline_draft_score=("Avg_draft_score","mean"),
                Baseline_cum_market=("Cum_market_delta","mean"),
                Baseline_cum_VORP=("Cum_VORP","mean"),
                Baseline_cum_score=("Cum_draft_score","mean"),
            )
            sharp=round_room[round_room.Room=="Sharp/value"].rename(columns={
                "Avg_market_delta":"Sharp_market_delta",
                "Avg_VORP":"Sharp_VORP",
                "Avg_draft_score":"Sharp_draft_score",
                "Cum_market_delta":"Sharp_cum_market",
                "Cum_VORP":"Sharp_cum_VORP",
                "Cum_draft_score":"Sharp_cum_score",
            })
            diff=sharp.merge(baseline,on="Round",how="left")
            diff["Market_delta_gap"]=diff.Sharp_market_delta-diff.Baseline_market_delta
            diff["VORP_gap"]=diff.Sharp_VORP-diff.Baseline_VORP
            diff["Draft_score_gap"]=diff.Sharp_draft_score-diff.Baseline_draft_score
            diff["Cum_market_gap"]=diff.Sharp_cum_market-diff.Baseline_cum_market
            diff["Cum_VORP_gap"]=diff.Sharp_cum_VORP-diff.Baseline_cum_VORP
            diff["Cum_score_gap"]=diff.Sharp_cum_score-diff.Baseline_cum_score

            # Position mix by room/round.
            pos_mix=d.groupby(["Room","Round","Position"],as_index=False).size().rename(columns={"size":"Selections"})
            totals=pos_mix.groupby(["Room","Round"])["Selections"].transform("sum")
            pos_mix["Share"]=pos_mix.Selections/totals

            # Roster need state by room/round.
            needs=d.groupby(["Room","Round"],as_index=False).agg(
                QB_need=("QB_need","mean"),RB_need=("RB_need","mean"),WR_need=("WR_need","mean"),
                TE_need=("TE_need","mean"),DL_need=("DL_need","mean"),DB_need=("DB_need","mean")
            )

            st.markdown("#### Overall grade by focus room")
            st.dataframe(room_overall.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Round-level Sharp/value vs baseline gap")
            st.dataframe(diff[[
                "Round","Market_delta_gap","VORP_gap","Draft_score_gap",
                "Cum_market_gap","Cum_VORP_gap","Cum_score_gap"
            ]].round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Raw round × room metrics")
            st.dataframe(round_room.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Position mix by room and round")
            st.dataframe(pos_mix.sort_values(["Round","Room","Selections"],ascending=[True,True,False]).round(4),
                         use_container_width=True,hide_index=True)

            st.markdown("#### Remaining roster needs by room and round")
            st.dataframe(needs.round(4),use_container_width=True,hide_index=True)

            slot_room=draft_level.groupby(["Draft_slot","Room"],as_index=False).agg(
                Avg_overall=("Overall_grade","mean"),
                Std_overall=("Overall_grade","std"),
            )
            st.markdown("#### Draft slot × focus room")
            st.dataframe(slot_room.round(4),use_container_width=True,hide_index=True)

            # Simple first-divergence screen: first round where Sharp is materially worse
            # than the two-room baseline in both cumulative draft score and cumulative VORP.
            material=diff[
                (diff.Cum_score_gap < -0.5) &
                (diff.Cum_VORP_gap < -0.10)
            ]
            if len(material):
                first_round=int(material.sort_values("Round").iloc[0].Round)
                st.warning(
                    f"First material Sharp/value divergence detected by the audit screen: Round {first_round}. "
                    "Use that round and the immediately preceding roster state as the next diagnostic target; do not create an adaptive rule yet."
                )
            else:
                st.success(
                    "No single round clears the material-divergence screen. The Sharp/value deficit appears distributed rather than concentrated at one decision point."
                )

            st.info(
                "V7.88 is measurement-only. The goal is to locate the first reproducible Sharp/value failure point and determine whether it is driven by player quality, "
                "market timing, position mix, or roster-state pressure. V7.67 remains untouched."
            )

            st.download_button(
                "⬇️ Download V7.88 Sharp/value failure-point audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_88_sharp_value_failure_point_audit.csv",
                "text/csv",key="v788_download"
            )
