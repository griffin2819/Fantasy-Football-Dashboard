...
st.title("🏈 Fantasy Edge V7.49 — Frozen 55.97 Rule Large Confirmation")
st.caption("V7.49 • V7.24/.90 champion frozen • exact 55.97 rule large confirmation")
st.caption("Build ID: V7.49-P12TE30-GAP5597-CONFIRM-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v749_live_slot")
    ...
    if "v749_target_queue" not in st.session_state:
        st.session_state.v749_target_queue={}
    queue=st.session_state.v749_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v749_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.49 uses .90 as the live/default champion. The candidate remains exactly the V7.48.1 frozen 55.97 rule with no threshold or logic changes.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.49 robustness report")
        ...
        st.markdown("### 🧪 V7.49 champion baseline validation")
        st.info("V7.49 introduces no tuning. It is a large-sample confirmation designed to determine whether the exact frozen rule is robust and whether the conditional R4-6 frequency gate reflects whole-draft quality.")
        ...
        if "v749_independent_results" not in st.session_state:
            st.session_state.v749_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v749_independent_results"),pd.DataFrame) else "ready to run"))
        if "v749_independent_summary" not in st.session_state:
            st.session_state.v749_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v749_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v749_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v749_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v749_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v749_independent_results=iv
                st.session_state.v749_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v749_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v749_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.49 Champion Baseline Check")
            st.caption("V7.49 keeps V7.24/.90 live and reruns the exact frozen 55.97 conditional discriminator on a much larger new seed family.")
            ...
            st.download_button(
                "⬇️ Download V7.49 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_49_champion_baseline.csv",
                "text/csv",
                key="v749_baseline_download"
            )
        ...
        st.markdown("### 🧱 V7.49 — Frozen 55.97 Rule Large Confirmation")
        st.caption(
            "Confirmation only — no tuning. V7.24/.90 remains live/default. "
            "The exact V7.48.1 rule and frozen 55.97 threshold are rerun on a new seed family at much larger scale."
        )

        if "v749_results" not in st.session_state: st.session_state.v749_results=None
        sims=st.number_input(
            "V7.49 confirmation simulations per room",
            min_value=100,max_value=400,value=250,step=25,key="v749_sims",
            help="Default 250 × 5 rooms = 1,250 fresh matched champion-vs-rule sets."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.49 large confirmation",type="primary",key="v749_run")
        clear=b.button("Clear V7.49 confirmation",key="v749_clear")
        if clear:
            st.session_state.v749_results=None
            st.rerun()
        ...
        if run:
            ...
            for room_name in rooms:
                for j in range(int(sims)):
                    # Entirely new seed family. Threshold/logic remain frozen.
                    seed=949000+j*107+sum(ord(c) for c in room_name)

                    champ=simulate_mock_fast(
                        fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,
                        p12_te_gap_conditional_enabled=False
                    )
                    rule=simulate_mock_fast(
                        fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,
                        p12_te_gap_conditional_enabled=True,p12_te_gap_threshold=55.97
                    )

                    cc=independent_draft_grade_components(champ,int(teams))
                    rc=independent_draft_grade_components(rule,int(teams))
                    cp=independent_phase_metrics(champ).set_index("Phase")
                    rp=independent_phase_metrics(rule).set_index("Phase")
                    champ_players=champ.set_index("mock_pick")["player"].to_dict()
                    changed=sum(champ_players.get(rr.mock_pick,None)!=rr.player for _,rr in rule.iterrows())

                    overall=rc["overall"]-cc["overall"]
                    r46=float(rp.loc["R4-6","Avg_market_delta"])-float(cp.loc["R4-6","Avg_market_delta"])
                    rows.append({
                        "Room":room_name,"Simulation":j+1,"Seed":seed,
                        "Overall_change":overall,
                        "R1_3_change":float(rp.loc["R1-3","Avg_market_delta"])-float(cp.loc["R1-3","Avg_market_delta"]),
                        "R4_6_change":r46,
                        "R7_10_change":float(rp.loc["R7-10","Avg_market_delta"])-float(cp.loc["R7-10","Avg_market_delta"]),
                        "R11_plus_change":float(rp.loc["R11+","Avg_market_delta"])-float(cp.loc["R11+","Avg_market_delta"]),
                        ...
                        "Conditional_R4_6_win":(r46>0) if changed else np.nan,
                    })
                    done+=1
                    prog.progress(done/total)
                    if done==1 or done%125==0 or done==total:
                        status.caption(f"V7.49 confirmation: {done}/{total} fresh matched sets")

            df=pd.DataFrame(rows)
            st.session_state.v749_results=df
            status.success(f"V7.49 complete: {len(df)} fresh matched sets.")
            prog.empty()

        df=st.session_state.get("v749_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            cond_nonloss=changed.Conditional_overall_nonloss.mean() if len(changed) else np.nan
            cond46=changed.Conditional_R4_6_win.mean() if len(changed) else np.nan

            c1,c2,c3,c4=st.columns(4)
            c1.metric("Overall vs champion",f"{overall:+.3f}")
            c2.metric("R4-6 vs champion",f"{r46:+.3f}")
            c3.metric("Rule activation",f"{activation:.1%}")
            ...
            st.markdown("#### Large-sample room confirmation")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            pos_overall=int((room.Overall_change>0).sum())
            pos_r46=int((room.R4_6_change>0).sum())
            worst=float(room.Overall_change.min())

            # Preserve the original strict gate for reference.
            strict_pass=(
                overall>0 and r46>0 and r710>-.15 and r11>-.20
                and activation>=.05
                and pd.notna(cond) and cond>=.60
                and pd.notna(cond46) and cond46>=.55
                and pos_overall>=4 and pos_r46>=4 and worst>-.10
            )

            # Confirmation profile does NOT change the old gate.
            # It separately reports whether whole-draft evidence is robust enough
            # to reconsider the conditional R4-6 frequency metric in a later architecture decision.
            robust_profile=(
                overall>0 and r46>0 and r710>=0 and r11>=0
                and activation>=.05
                and pd.notna(cond) and cond>=.60
                and pd.notna(cond_nonloss) and cond_nonloss>=.75
                and pos_overall>=4 and pos_r46>=4 and worst>-.05
            )

            gate=pd.DataFrame([{
                "Frozen threshold":55.97,
                "Matched sets":len(df),
                ...
                "Conditional overall non-loss":cond_nonloss,
                "Conditional R4-6 win":cond46,
                "Positive overall rooms":pos_overall,
                "Positive R4-6 rooms":pos_r46,
                "Worst room overall":worst,
                "Original strict gate passed":strict_pass,
                "Robust whole-draft profile":robust_profile
            }])
            st.markdown("#### V7.49 confirmation decision table")
            st.dataframe(gate.round(4),use_container_width=True,hide_index=True)

            if strict_pass:
                st.success(
                    "V7.49 PASSES the original strict gate at large sample. "
                    "The exact frozen 55.97 rule is eligible for a promotion build."
                )
            elif robust_profile:
                st.warning(
                    "The original strict gate still fails, but the frozen rule has a robust whole-draft confirmation profile. "
                    "Do not change the rule or threshold; the next decision should examine whether conditional R4-6 win frequency is an appropriate promotion criterion."
                )
            else:
                st.warning(
                    "V7.49 does not confirm a sufficiently robust profile. "
                    "Keep V7.24/.90 frozen and retire the 55.97 rule."
                )

            st.download_button(
                "⬇️ Download V7.49 large confirmation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_49_frozen_5597_large_confirmation.csv",
                "text/csv",key="v749_download"
            )
