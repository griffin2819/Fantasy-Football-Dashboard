...
st.title("🏈 Fantasy Edge V8.09 — Round-4 Generic Mechanism Re-Audit")
st.caption("V8.09 • V8.05 frozen champion • fresh R4 genericity re-audit")
st.caption("Build ID: V8.09-R4-GENERIC-MECHANISM-REAUDIT-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V8.09 — Round-4 Generic Mechanism Re-Audit")
        st.caption(
            "Measurement only. V8.05 remains frozen champion. R5 and R6 are closed as player-specific. "
            "This is a fresh exact-board Round-4 audit and does NOT revive or modify the failed historical R4 challenger. "
            "Dominant alternative identities are detected mechanically and removed before genericity is assessed."
        )

        if "v809_results" not in st.session_state:
            st.session_state.v809_results=None

        sims=st.number_input(
            "V8.09 simulations per slot × room", min_value=10, max_value=40, value=20, step=5, key="v809_sims",
            help="Default: 12 slots × 5 rooms × 20 = 1,200 fresh V8.05 champion drafts. Only exact-board Round-4 decisions are retained."
        )
        a,b=st.columns(2)
        run=a.button("Run V8.09 R4 re-audit",type="primary",key="v809_run")
        clear=b.button("Clear V8.09 audit",key="v809_clear")
        if clear:
            st.session_state.v809_results=None
            st.rerun()
            ...
            for ds in range(1,int(teams)+1):
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8090000+ds*100000+j*449+sum(ord(c) for c in room_name)
                        ...
                        rr=d[pd.to_numeric(d.mock_round,errors="coerce").eq(4)]
                        if len(rr):
                            r=rr.iloc[0]
                            ...
                            future=d[
                                (pd.to_numeric(d.mock_round,errors="coerce")>=5) &
                            ...
                            rows.append({
                                ...
                                "R5_7_avg_VORP":fvorp.mean(),
                                "R5_7_avg_score":fscore.mean(),
                                "R5_7_position_sequence":" > ".join(future["position"].astype(str).tolist()),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done%200==0 or done==total:
                            status.caption(f"V8.09: {done}/{total} fresh champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v809_results=df
            status.success(f"V8.09 complete: {len(df):,} exact-board Round-4 decisions audited.")
            prog.empty()

        df=st.session_state.get("v809_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            summary=pd.DataFrame([{
                "R4 decisions":len(df),
                "Exact-board signals":len(sig),
                "Signal rate":len(sig)/len(df) if len(df) else np.nan,
                "Top alternative":top_name,
                "Top alternative N":top_n,
                "Top alternative share":top_share,
                "Dominant identities >=20%":", ".join(sorted(dominant_names)) if dominant_names else "None",
                "Residual signals":len(generic),
                "Residual share":len(generic)/len(sig) if len(sig) else np.nan,
                "Residual rooms":generic.Room.nunique() if len(generic) else 0,
                "Residual slots":generic.Draft_slot.nunique() if len(generic) else 0,
            }])
            ...
            identities=sig.groupby(["Best_alt_player","Best_alt_position"],as_index=False).agg(
                ...
                Avg_R5_7_VORP=("R5_7_avg_VORP","mean")
            ).sort_values("Signals",ascending=False)
            if len(identities):
                identities["Signal_share"]=identities.Signals/len(sig)
            ...
            paths=sig.groupby(["Champion_position","Best_alt_position"],as_index=False).agg(
                ...
                Avg_market_distance=("Alt_market_distance","mean"),
                Avg_R5_7_VORP=("R5_7_avg_VORP","mean"),
                Avg_R5_7_score=("R5_7_avg_score","mean")
            ).sort_values("Signals",ascending=False)
            ...
            st.markdown("#### Residual mechanism after dominant identities removed")
            ...
            st.markdown("#### Residual signal by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Residual signal by slot")
            ...
            st.markdown("#### Residual market-distance bands")
            ...
            st.markdown("#### Residual roster-state signatures")
            st.dataframe(roster.head(50).round(4),use_container_width=True,hide_index=True)

            room_breadth=generic.Room.nunique() if len(generic) else 0
            slot_breadth=generic.Draft_slot.nunique() if len(generic) else 0
            residual_share=len(generic)/len(sig) if len(sig) else 0
            if len(sig) and residual_share>=0.25 and room_breadth>=4 and slot_breadth>=6:
                st.warning(
                    f"R4 retains a potentially generic residual after dominant identities are removed: "
                    f"{len(generic)}/{len(sig)} signals ({residual_share:.1%}), across {room_breadth}/5 rooms and {slot_breadth}/12 slots. "
                    "This earns a separate causal-probe design; it does NOT validate the old failed R4 rule."
                )
            else:
                st.success(
                    f"R4 does not show enough broad residual genericity to reopen development: "
                    f"{len(generic)}/{len(sig)} residual signals, {room_breadth}/5 rooms, {slot_breadth}/12 slots."
                )

            st.info(
                "V8.09 is a fresh measurement-only re-audit. The historical failed R4 challenger remains OFF and is not being retested."
            )

            st.download_button(
                "⬇️ Download V8.09 R4 generic re-audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_09_r4_generic_mechanism_reaudit.csv",
                "text/csv",key="v809_download"
            )
