...
st.title("🏈 Fantasy Edge V7.42 — Pick 12 FE-Gap Decomposition Audit")
st.caption("V7.42 • V7.24 9× champion frozen • Pick 12 FE-gap decomposition audit")
st.caption("Build ID: V7.42-P12FGD-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v742_live_slot")
    ...
    if "v742_target_queue" not in st.session_state:
        st.session_state.v742_target_queue={}
    queue=st.session_state.v742_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v742_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.42 keeps the V7.24 9× champion completely frozen. No candidate rule or altered threshold is active.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.42 robustness report")
        ...
        st.markdown("### 🧪 V7.42 champion baseline validation")
        st.info("V7.42 is diagnostic only. It attributes the large selected-player versus passed-faller FE gap to player score, roster utility, market edge, injury/IDP adjustments, and residual noise.")
        ...
        if "v742_independent_results" not in st.session_state:
            st.session_state.v742_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v742_independent_results"),pd.DataFrame) else "ready to run"))
        if "v742_independent_summary" not in st.session_state:
            st.session_state.v742_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v742_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v742_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v742_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v742_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v742_independent_results=iv
                st.session_state.v742_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v742_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v742_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.42 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.42 decomposes the full Pick-12 R4-6 FE evaluation gap against passed market fallers.")
            ...
            st.download_button(
                "⬇️ Download V7.42 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_42_champion_baseline.csv",
                "text/csv",
                key="v742_baseline_download"
            )
        ...
        st.markdown("### 🧬 V7.42 — Pick 12 FE-Gap Decomposition Audit")
        st.caption(
            "Diagnostic only. V7.24 9× remains frozen. This decomposes the full FE evaluation gap between the selected Pick-12 R4-6 RB/TE "
            "and the best passed market faller, including the player-score internals and the residual stochastic component."
        )

        if "v742_results" not in st.session_state: st.session_state.v742_results=None
        if "v742_meta" not in st.session_state: st.session_state.v742_meta=None

        sims=st.number_input("FE-gap audit simulations per room",min_value=5,max_value=25,value=10,step=5,key="v742_sims")
        aa,ab=st.columns(2)
        run=aa.button("Run Pick 12 FE-gap audit",type="primary",key="v742_run")
        clear=ab.button("Clear FE-gap audit",key="v742_clear")

        if clear:
            st.session_state.v742_results=None
            st.session_state.v742_meta=None
            st.rerun()
        ...
        if run:
            ...
            for room_name in rooms:
                for j in range(int(sims)):
                    seed=942000+j*73+sum(ord(c) for c in room_name)
                    fe=simulate_mock_fast(
                        fast_champ,int(teams),12,15,state["roster_slots"],
                        6.0,True,seed,room_name
                    )
                    audited=fe[pd.to_numeric(fe.get("MR_Audit",0),errors="coerce").fillna(0).eq(1)].copy()
                    for _,r in audited.iterrows():
                        fall=float(pd.to_numeric(pd.Series([r.get("MR_Passed_faller_amount",np.nan)]),errors="coerce").iloc[0])
                        gap=float(pd.to_numeric(pd.Series([r.get("MR_Passed_faller_eval_gap",np.nan)]),errors="coerce").iloc[0])
                        rows.append({
                            ...
                            "Faller_player":r.get("MR_Passed_faller_player",""),
                            "Faller_position":r.get("MR_Passed_faller_position",""),
                            "Faller_amount":fall,
                            "Faller_eval_gap":gap,
                            "Faller_survival_est":r.get("MR_Passed_faller_survival_est",np.nan),
                            ...
                            "Injury_gap":r.get("MR42_Faller_injury_gap",np.nan),
                            "IDP_penalty_gap":r.get("MR42_Faller_idp_penalty_gap",np.nan),
                            ...
                            "Draftscore_internal_sum":r.get("MR42_Faller_component_sum",np.nan),
                            "RBTE_vs_WR":str(r.get("MR_Selected_position","")) in ["RB","TE"] and str(r.get("MR_Passed_faller_position",""))=="WR",
                            "Faller_7plus":pd.notna(fall) and fall>=7.0
                        })
                    done+=1
                    prog.progress(done/total)
                    if done==1 or done%10==0 or done==total:
                        status.caption(f"FE-gap audit: {done}/{total} drafts")

            df=pd.DataFrame(rows)
            st.session_state.v742_results=df
            st.session_state.v742_meta={"seconds":time.perf_counter()-started,"drafts":total,"decisions":len(df)}
            status.success(f"FE-gap audit complete: {len(df)} R4-6 decisions.")
            prog.empty()

        df=st.session_state.get("v742_results")
        meta=st.session_state.get("v742_meta") or {}
        if isinstance(df,pd.DataFrame) and len(df):
            focus=df[df.RBTE_vs_WR & df.Faller_7plus].copy()
            if not len(focus):
                focus=df[df.Faller_7plus].copy()

            st.markdown("#### Full FE evaluation-gap decomposition")
            if len(focus):
                top=pd.DataFrame([
                    {"Component":"Player / draft score","Avg FE gap":focus.Draftscore_gap.mean()},
                    {"Component":"Roster marginal utility","Avg FE gap":focus.Rosterutility_gap.mean()},
                    {"Component":"Model-vs-market edge","Avg FE gap":focus.Marketedge_gap.mean()},
                    {"Component":"Injury adjustment","Avg FE gap":focus.Injury_gap.mean()},
                    {"Component":"Early-IDP penalty","Avg FE gap":focus.IDP_penalty_gap.mean()},
                    {"Component":"Residual / random-noise term","Avg FE gap":focus.Residual_noise_gap.mean()},
                    {"Component":"TOTAL FE evaluation gap","Avg FE gap":focus.Faller_eval_gap.mean()},
                ])
                st.dataframe(top.round(3),use_container_width=True,hide_index=True)

                st.markdown("#### Player / draft-score internals")
                inner=pd.DataFrame([
                    {"Input":"VORP × 9","Avg FE gap contribution":focus.VORP_gap.mean()},
                    {"Input":"Projection × 1.6","Avg FE gap contribution":focus.Projection_gap.mean()},
                    {"Input":"Progression × .12","Avg FE gap contribution":focus.Progression_gap.mean()},
                    {"Input":"Regression × -.10","Avg FE gap contribution":focus.Regression_gap.mean()},
                    {"Input":"Positional scarcity","Avg FE gap contribution":focus.Scarcity_gap.mean()},
                    {"Input":"Consensus strength × .10","Avg FE gap contribution":focus.Consensus_gap.mean()},
                    {"Input":"Draft-score internal sum","Avg FE gap contribution":focus.Draftscore_internal_sum.mean()},
                ])
                st.dataframe(inner.round(3),use_container_width=True,hide_index=True)

                c1,c2,c3,c4=st.columns(4)
                c1.metric("Focused decisions",str(len(focus)))
                c2.metric("Avg total FE gap",f"{focus.Faller_eval_gap.mean():+.2f}")
                c3.metric("Avg faller size",f"{focus.Faller_amount.mean():.2f} picks")
                c4.metric("Actual faller survival",f"{pd.to_numeric(focus.Faller_survived_actual,errors='coerce').mean():.1%}")

                room=focus.groupby("Room",as_index=False).agg(
                    Decisions=("Faller_eval_gap","count"),
                    Total_gap=("Faller_eval_gap","mean"),
                    Draftscore=("Draftscore_gap","mean"),
                    Rosterutility=("Rosterutility_gap","mean"),
                    Marketedge=("Marketedge_gap","mean"),
                    VORP=("VORP_gap","mean"),
                    Projection=("Projection_gap","mean"),
                    Residual=("Residual_noise_gap","mean"),
                    Faller_size=("Faller_amount","mean")
                )
                st.markdown("#### Gap decomposition by room")
                st.dataframe(room.round(3),use_container_width=True,hide_index=True)

                path=focus.groupby(["Selected_position","Faller_position"],as_index=False).agg(
                    Decisions=("Faller_eval_gap","count"),
                    Total_gap=("Faller_eval_gap","mean"),
                    Draftscore=("Draftscore_gap","mean"),
                    Rosterutility=("Rosterutility_gap","mean"),
                ...
                st.markdown("#### Selected position → faller position")
                st.dataframe(path.sort_values("Decisions",ascending=False).round(3),use_container_width=True,hide_index=True)

                vals={
                    "player/draft score":float(focus.Draftscore_gap.mean()),
                    "roster utility":float(focus.Rosterutility_gap.mean()),
                    "market edge":float(focus.Marketedge_gap.mean()),
                    "residual/noise":float(focus.Residual_noise_gap.mean())
                }
                dominant=max(vals,key=vals.get)

                if dominant=="player/draft score":
                    inner_vals={
                        "VORP":float(focus.VORP_gap.mean()),
                        "projection":float(focus.Projection_gap.mean()),
                        "progression":float(focus.Progression_gap.mean()),
                        "regression":float(focus.Regression_gap.mean()),
                        "scarcity":float(focus.Scarcity_gap.mean()),
                        "consensus":float(focus.Consensus_gap.mean())
                    }
                    inner_dom=max(inner_vals,key=inner_vals.get)
                    st.warning(
                        f"Primary FE-gap driver: player/draft score ({vals[dominant]:+.2f} points). "
                        f"Inside player score, {inner_dom} is largest at {inner_vals[inner_dom]:+.2f}. "
                        "V7.43 should test the identified upstream valuation component, not widen an execution threshold."
                    )
                elif dominant=="roster utility":
                    st.warning(
                        f"Primary FE-gap driver: roster marginal utility ({vals[dominant]:+.2f}). "
                        "V7.43 should audit the Pick-12 R4-6 marginal-utility calculation against the passed faller."
                    )
                elif dominant=="residual/noise":
                    st.warning(
                        f"Primary FE-gap driver is the residual stochastic term ({vals[dominant]:+.2f}). "
                        "Before changing valuation weights, V7.43 should audit confidence/randomness behavior at Pick 12."
                    )
                else:
                    st.info(
                        f"Largest FE-gap component is {dominant} ({vals[dominant]:+.2f}). "
                        "Use the room/path tables to determine the next isolated test."
                    )
            else:
                st.info("No 7+ pick fallers were available in the current audit sample.")

            st.download_button(
                "⬇️ Download V7.42 Pick 12 FE-gap decomposition CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_42_pick12_fe_gap_decomposition.csv",
                "text/csv",key="v742_download"
            )
