...
def _objective_player_shortlist(x, top_n=15):
    """
    Stage 1: identify legitimate players BEFORE roster strategy.
    Projection, VORP, positional quality and market support dominate.
    Roster need/saturation is intentionally excluded.
    """
    y=x.copy()
    if y.empty:
        return y
    ...
    # Position-relative percentiles prevent QB/IDP scales from overwhelming RB/WR.
    proj_pct=proj.groupby(y["position"].astype(str)).rank(pct=True,method="average")
    vorp_pct=vorp.groupby(y["position"].astype(str)).rank(pct=True,method="average")
    ...
    idp=y["position"].astype(str).isin(["DL","DB"])
    idp_quality=np.zeros(len(y),dtype=float)
    if idp.any():
        ...
        idp_quality=np.where(
            idp,
            np.clip(
                0.50*impact.rank(pct=True,method="average") +
                0.30*(1.0-(tier-1.0)/4.0).clip(0,1) +
                0.20*(1.0-ext.rank(pct=True,method="average")),
                0,1
            ),
            0.0
        )
    ...
    y["player_shortlist_score"]=100.0*(
        0.38*proj_pct.fillna(0.5) +
        0.34*vorp_pct.fillna(0.5) +
        0.16*market_pct.fillna(0.5) +
        0.12*model_pct.fillna(0.5)
    )
    y.loc[idp,"player_shortlist_score"]=(
        0.68*y.loc[idp,"player_shortlist_score"] +
        32.0*np.asarray(idp_quality)[idp.to_numpy()]
    )

    # Always preserve the best few at every position so a required position cannot
    # vanish merely because of cross-position scale differences.
    keep=set(y.nlargest(min(int(top_n),len(y)),"player_shortlist_score").index.tolist())
    for p in y["position"].dropna().astype(str).unique():
        pm=y["position"].astype(str).eq(p)
...
def _two_stage_live_optimizer(x, roster, slots, current_pick, next_pick):
    ...
    for idx in finalists:
        ...
        y.at[idx,"final_pick_value"]=float(y.at[idx,"opportunity_score"])+rv
    ...
    if len(ranked)>=2:
        ...
        gap=8.0
    conf=float(np.clip(50.0+gap*5.8,50.0,99.0))
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    # Authoritative two-stage live optimizer.
    x=_two_stage_live_optimizer(
        x,roster,slots,int(current_pick),int(next_pick)
    )

    # FINAL PICK authority. Hard legality/completion penalties remain embedded in
    # evaluation_score and therefore survive the shortlist/rollout process.
...
def grade_mock(roster, teams, slots):
    ...
    _positional_gap=max(0.0,88.0-positional)
    _positional_edge_penalty=min(6.0,_positional_gap*0.35)
    _opportunity_edge_penalty=min(7.0,max(float(opp)-4.0,0.0)*0.55)

    model_edge=float(np.clip(
        model_edge-_construction_edge_penalty-_draft_value_edge_penalty
        -_positional_edge_penalty-_opportunity_edge_penalty,
        35,100
    ))
    ...
    return {"score":score,"grade":grade,"starter":starter,"value":avg_market,"penalty":max(0,100-construction),
            ...
            "big_reach_rate":_big_reach_rate,"portfolio_penalty":_portfolio_penalty,"diversification_penalty":_diversification_penalty,"construction_edge_penalty":_construction_edge_penalty,"draft_value_edge_penalty":_draft_value_edge_penalty,"positional_edge_penalty":_positional_edge_penalty,"opportunity_edge_penalty":_opportunity_edge_penalty}
...
def _fast_sim_user_pick(avail, roster_rows, rnd, overall, slot, teams, slots, randomness, rng):
    ...
    if dl>=1 and db>=1:
        if total_idp>=3:
            ...
            required_edge=6.0+rng.normal(0,1.5)
