...
st.title("🏈 Fantasy Edge V7.70 — Round-5 True-Board Counterfactual Audit")
st.caption("V7.70 • V7.67 frozen champion • Round-5 true-board diagnostic")
st.caption("Build ID: V7.70-R5-TRUEBOARD-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.70 keeps certified V7.67 frozen and audits Round-5 >10-pick reaches against genuinely available true-board alternatives.")
        ...
        st.markdown("### 🔬 V7.70 — Round-5 True-Board Counterfactual Audit")
        st.caption(
            "Measurement only. V7.67 remains frozen champion. Every Round-5 >10-pick reach is compared with the top five "
            "genuinely available market-respecting alternatives from the actual room state. No player is hard-coded."
        )
        if "v770_results" not in st.session_state: st.session_state.v770_results=None
        sims=st.number_input("V7.70 simulations per slot × room",25,100,50,5,key="v770_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh champion drafts; one simulation per seed.")
        a,b=st.columns(2)
        run=a.button("Run Round-5 true-board audit",type="primary",key="v770_run")
        clear=b.button("Clear V7.70 audit",key="v770_clear")
        if clear:
            st.session_state.v770_results=None; st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=970000+int(ds)*100000+j*223+sum(ord(c) for c in room_name)
                        ...
                        if len(r5):
                            ...
                            audit=int(pd.to_numeric(pd.Series([r.get("R770_Audit",0)]),errors="coerce").fillna(0).iloc[0])
                            if pd.notna(reach) and reach>10 and audit==1:
                                row={
                                    "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                    "Round5_pick":pick,"Champion_player":str(r.player),"Champion_position":str(r.position),
                                    "Champion_market_pick":market,"Champion_reach":reach,
                                    "Champion_model_rank":r.get("model_rank",np.nan),"Champion_projection":r.get("projection",np.nan),
                                    "Champion_vorp":r.get("vorp",np.nan),"Champion_draft_score":r.get("draft_score",np.nan),
                                    "Actual_available_count":r.get("R770_Actual_available_count",np.nan),
                                    "Market_respecting_count":r.get("R770_Market_respecting_count",np.nan),
                                }
                                for k in range(1,6):
                                    for fld in ["player","position","market_pick","model_rank","projection","vorp","draft_score"]:
                                        row[f"Alt{k}_{fld}"]=r.get(f"R770_Alt{k}_{fld}","" if fld in ["player","position"] else np.nan)
                                rows.append(row)
                        done+=1; prog.progress(done/total)
                        if done==1 or done%200==0 or done==total: status.caption(f"V7.70 audit: {done}/{total} champion drafts")
            df=pd.DataFrame(rows); st.session_state.v770_results=df
            status.success(f"V7.70 complete: {len(df)} true-board Round-5 >10-pick reaches audited."); prog.empty()

        df=st.session_state.get("v770_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in ["Champion_projection","Champion_vorp","Champion_draft_score","Champion_market_pick",
            ...
            champs=df.groupby(["Champion_player","Champion_position"],as_index=False).agg(
                Triggers=("Champion_player","count"),Avg_reach=("Champion_reach","mean"),
                Alt1_score_change=("Alt1_draft_score_change","mean"),Alt1_vorp_change=("Alt1_vorp_change","mean"),
                Alt1_dominance=("Alt1_dominates_score_vorp","mean")
            ).sort_values("Triggers",ascending=False)
            st.markdown("#### Round-5 reach drivers"); st.dataframe(champs.head(25).round(4),use_container_width=True,hide_index=True)

            alts=df.groupby(["Alt1_player","Alt1_position"],as_index=False).agg(
                Times_top_alt=("Alt1_player","count"),Avg_market_pick=("Alt1_market_pick","mean"),
                Projection_change=("Alt1_projection_change","mean"),VORP_change=("Alt1_vorp_change","mean"),
                Draft_score_change=("Alt1_draft_score_change","mean"),Dominance_rate=("Alt1_dominates_score_vorp","mean")
            ).sort_values("Times_top_alt",ascending=False)
            st.markdown("#### Top genuinely available alternatives"); st.dataframe(alts.head(25).round(4),use_container_width=True,hide_index=True)

            st.download_button("⬇️ Download V7.70 Round-5 true-board counterfactual CSV",
                df.to_csv(index=False).encode("utf-8"),"fantasy_edge_v7_70_round5_true_board_counterfactual_audit.csv",
                "text/csv",key="v770_download")
        elif isinstance(df,pd.DataFrame):
            st.success("No Round-5 >10-pick reaches occurred in this run.")
