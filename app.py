...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Mobile Cards + Availability Registry", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
# V7.84 persist last-known hard availability from an authoritative refresh.
if _refresh_live:
    try:
        ...
        for _,_rr in board.iterrows():
            ...
            _st=_v677_normalize_injury_status(_rr.get("injury_effective",_rr.get("fe_status",_rr.get("injury",""))))
            _verified=bool(_rr.get("injury_status_verified",True))
            if _st in _hard and _verified:
                _reg[_key]={"status":_st,"updated":_now,"player":_name}
            elif _st=="ACTIVE" and _verified and _key in _reg:
                # A fresh authoritative ACTIVE record is the only thing allowed to clear
                # a prior hard status.
                _reg.pop(_key,None)
...
def _v781_render_page(_v777_page_idx):
    global state
    """Render only the selected Fantasy Edge page.

    V7.81 keeps this entire renderer inside a Streamlit fragment, so a navigation
    click reruns only this function instead of replaying startup, board repair,
    cloud checks, and every helper definition.
    """
    if _v777_page_idx==0:
    ...
    if _v777_page_idx==6:
        ...
        if _my.empty:
            ...
            st.caption(f"Fantasy Edge V7.84 • {_v677_season} Week {_v677_week} • Decision-quality Start/Sit with authoritative availability, uncertainty-aware close calls, and separate unavailable/monitor groups.")
    ...
    if _v777_page_idx==14:
        ...
        if my.empty:
            ...
            _v780_snap_key=f"7.84|{_v780_base_key}|{_v780_avail_sig}"
