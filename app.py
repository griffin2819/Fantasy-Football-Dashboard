...
st.set_page_config(page_title="Fantasy Edge V7.00 — Calibrated Waiver Authority", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V7.00-CALIBRATED-WAIVER-AUTHORITY"
...
def _v697_waiver_priority_score(add_row,drop_row,state,net_week=None,net_ros=None,net_roster=None):
    ...
    if score>=88 and priority_gain: label="🔥 PRIORITY CLAIM"
    ...
    if _my_roster.empty:
        ...
        if _moves_df.empty:
            ...
            for _,_wr in _moves_df.iterrows():
                ...
                if len(_ar) and len(_dr): _sc,_lab=_v697_waiver_priority_score(_ar.iloc[0],_dr.iloc[0],state,_wr["Net Week"],_wr["Net ROS"],_wr["Net Roster"])
        ...
        st.caption("V7.00 scores actual simulated lineup, roster and ROS gains; 90+ is reserved for rare, material upgrades. The engine does not execute roster moves.")
