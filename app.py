...
def mid_round_pipeline_audit_snapshot(fast, candidate_idx, delta, edge, eval_score, chosen_global,
    ...
    replacement_ppg=fast["replacement_ppg"]; progression=fast["progression"]
...
st.title("🏈 Fantasy Edge V7.27 — R4-6 Player/Draft-Score Decomposition")
st.caption("V7.27 • V7.24 champion frozen • R4-6 player-score decomposition")
st.caption("Build ID: V7.27-MRDSD-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v727_live_slot")
    ...
    if "v727_target_queue" not in st.session_state:
        st.session_state.v727_target_queue={}
    queue=st.session_state.v727_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v727_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.27 is diagnostic only. The V7.24 champion is frozen while the R4-6 player/draft-score advantage is decomposed into VORP, projection, progression, regression, scarcity and consensus-strength inputs.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.27 robustness report")
        ...
        st.markdown("### 🧪 V7.27 R4-6 player-score decomposition")
        st.info("V7.27 keeps the V7.24 champion frozen. The independent test now decomposes the underlying R4-6 player score without changing selections.")
        ...
        if "v727_independent_results" not in st.session_state:
            st.session_state.v727_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v727_independent_results"),pd.DataFrame) else "ready to run"))
        if "v727_independent_summary" not in st.session_state:
            st.session_state.v727_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v727_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v727_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v727_run")
        clear_iv=clear_col.button("Clear independent results",key="v727_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v727_independent_results=iv
                st.session_state.v727_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v727_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v727_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧬 V7.27 R4-6 Player/Draft-Score Decomposition")
            st.caption("Diagnostic only. V7.24 champion logic is frozen. This breaks the R4-6 RB/TE-over-WR/QB player-score advantage into VORP, projection, progression, regression, positional scarcity and consensus reality-check components.")

            d1,d2,d3,d4=st.columns(4)
            d1.metric("R4-6 positional conflict",f"{iv_saved.MR_positional_conflict_rate.mean():.0%}")
            d2.metric("Conflict draft-score contribution",f"{iv_saved.MR_conflict_draftscore_gap.mean():+.2f}")
            d3.metric("Conflict roster-utility contribution",f"{iv_saved.MR_conflict_rosterdelta_gap.mean():+.2f}")
            d4.metric("Conflict market-edge contribution",f"{iv_saved.MR_conflict_edge_gap.mean():+.2f}")

            raw_df=pd.DataFrame([
                {"Underlying input":"Projection PPG","Raw chosen-minus-alt":iv_saved.MR27_projection_raw_gap.mean()},
                {"Underlying input":"Replacement PPG","Raw chosen-minus-alt":iv_saved.MR27_replacement_raw_gap.mean()},
                {"Underlying input":"VORP PPG","Raw chosen-minus-alt":iv_saved.MR27_vorp_raw_gap.mean()},
                {"Underlying input":"Progression score","Raw chosen-minus-alt":iv_saved.MR27_progression_raw_gap.mean()},
                {"Underlying input":"Regression score","Raw chosen-minus-alt":iv_saved.MR27_regression_raw_gap.mean()},
                {"Underlying input":"Scarcity constant","Raw chosen-minus-alt":iv_saved.MR27_scarcity_raw_gap.mean()},
                {"Underlying input":"Consensus strength","Raw chosen-minus-alt":iv_saved.MR27_consensus_strength_raw_gap.mean()},
            ])
            st.markdown("#### Raw player-model differences: selected RB/TE minus strongest WR/QB")
            st.dataframe(raw_df.round(3),use_container_width=True,hide_index=True)

            score_df=pd.DataFrame([
                {"Draft-score component":"VORP × 9","FE evaluation contribution":iv_saved.MR27_vorp_component_gap.mean()},
                {"Draft-score component":"Projection × 1.6","FE evaluation contribution":iv_saved.MR27_projection_component_gap.mean()},
                {"Draft-score component":"Progression × .12","FE evaluation contribution":iv_saved.MR27_progression_component_gap.mean()},
                {"Draft-score component":"Regression × -.10","FE evaluation contribution":iv_saved.MR27_regression_component_gap.mean()},
                {"Draft-score component":"Positional scarcity","FE evaluation contribution":iv_saved.MR27_scarcity_component_gap.mean()},
                {"Draft-score component":"Consensus strength × .10","FE evaluation contribution":iv_saved.MR27_consensus_component_gap.mean()},
            ])
            st.markdown("#### Exact contribution to the frozen FE evaluation gap")
            st.dataframe(score_df.round(3),use_container_width=True,hide_index=True)

            driver_df=pd.DataFrame([
                {"Primary draft-score driver":"VORP","Share":iv_saved.MR27_driver_VORP.mean()},
                {"Primary draft-score driver":"Projection","Share":iv_saved.MR27_driver_projection.mean()},
                {"Primary draft-score driver":"Progression","Share":iv_saved.MR27_driver_progression.mean()},
                {"Primary draft-score driver":"Regression","Share":iv_saved.MR27_driver_regression.mean()},
                {"Primary draft-score driver":"Positional scarcity","Share":iv_saved.MR27_driver_positional_scarcity.mean()},
                {"Primary draft-score driver":"Consensus reality check","Share":iv_saved.MR27_driver_consensus_reality_check.mean()},
            ])
            driver_df["Share"]=driver_df.Share.map(lambda x:f"{x:.0%}")
            st.markdown("#### Which player-score component most often drives the RB/TE advantage?")
            ...
            for ds,g in iv_saved.groupby("Draft_slot"):
                slot_rows.append({
                    ...
                    "Conflict rate":g.MR_positional_conflict_rate.mean(),
                    "Draft-score gap":g.MR_conflict_draftscore_gap.mean(),
                    ...
                    "Scarcity contribution":g.MR27_scarcity_component_gap.mean(),
                    "Consensus contribution":g.MR27_consensus_component_gap.mean(),
                })
            st.markdown("#### Player-score decomposition by draft slot")
            st.dataframe(pd.DataFrame(slot_rows).round(3),use_container_width=True,hide_index=True)

            components={
                "VORP":float(iv_saved.MR27_vorp_component_gap.mean()),
                "projection":float(iv_saved.MR27_projection_component_gap.mean()),
                "progression":float(iv_saved.MR27_progression_component_gap.mean()),
                "regression":float(iv_saved.MR27_regression_component_gap.mean()),
                "positional scarcity":float(iv_saved.MR27_scarcity_component_gap.mean()),
                "consensus reality check":float(iv_saved.MR27_consensus_component_gap.mean()),
            }
            dominant=max(components,key=components.get)
            positive_total=sum(max(v,0) for v in components.values() if pd.notna(v))
            dominant_share=components[dominant]/positive_total if positive_total>0 else np.nan

            # VORP is itself projection - replacement, so call that out separately.
            if dominant=="VORP":
                proj=float(iv_saved.MR27_projection_raw_gap.mean())
                repl=float(iv_saved.MR27_replacement_raw_gap.mean())
                st.warning(
                    f"Primary draft-score driver: VORP contributes {components['VORP']:+.2f} FE evaluation points "
                    f"({dominant_share:.0%} of positive player-score contribution). "
                    f"Raw projection gap is {proj:+.2f} PPG and replacement-level gap is {repl:+.2f} PPG. "
                    "V7.28 should audit the replacement/VORP baseline before changing positional weights."
                )
            elif pd.notna(dominant_share) and dominant_share>=.50:
                st.warning(
                    f"Primary draft-score driver: {dominant} contributes {components[dominant]:+.2f} FE evaluation points "
                    f"({dominant_share:.0%} of positive player-score contribution). V7.28 should target only this input."
                )
            else:
                st.info(
                    "No single player-score input dominates the R4-6 RB/TE preference. "
                    "Avoid modifying the champion until the slot decomposition shows a stable correction target."
                )
            ...
            st.download_button(
                "⬇️ Download V7.27 R4-6 player-score decomposition CSV",
                iv_csv,
                "fantasy_edge_v7_27_mid_round_draft_score_decomposition.csv",
                "text/csv",
                key="v727_download"
            )
