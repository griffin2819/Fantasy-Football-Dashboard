...
def _base_player_value_engine(x):
    ...
    base=100.0*(0.39*proj_pct.fillna(.5)+0.37*vorp_pct.fillna(.5)+
                0.14*market_pct.fillna(.5)+0.10*model_pct.fillna(.5))
...
def _marginal_slot_value(pos, counts, slots):
    ...
    curve={
        "RB":[3.0,2.1,1.2,0.3,-1.0,-2.6,-4.5],
        "WR":[3.2,2.4,1.5,0.6,-0.7,-2.2,-4.0],
        "TE":[1.4,-1.2,-3.2],
        "QB":[1.0,-1.8,-3.8],
        "DL":[1.0,-1.0,-3.8],
        "DB":[1.0,-1.0,-3.8],
...
def _single_authority_strategy(y,roster,slots,current_pick,next_pick):
...
def _branch_future_value(y,counts,slots,from_pick,to_pick,excluded,rng):
    """One lightweight stochastic future-board branch."""
    q=y.drop(index=list(excluded),errors="ignore").copy()
    if q.empty:
        return 0.0,None

    market=_safe_num_series(q,"market_pick",float(to_pick)+20)
    if "market_pick" not in q.columns and "consensus_rank" in q.columns:
        market=_safe_num_series(q,"consensus_rank",float(to_pick)+20)
    base=_safe_num_series(q,"base_player_value",0.0)

    # Sample availability around market expectation.
    probs=market.map(lambda mp:_market_survival_probability(mp,from_pick,to_pick)).to_numpy(float)
    alive=rng.random(len(q)) < probs
    if not alive.any():
        alive[np.argmax(probs)]=True

    utility=np.full(len(q),-1e9,dtype=float)
    positions=q["position"].astype(str).to_numpy()
    for i,(ok,p) in enumerate(zip(alive,positions)):
        if ok:
            utility[i]=float(base.iloc[i])+2.0*_marginal_slot_value(p,counts,slots)
...
def _single_authority_final(y,roster,slots,current_pick,next_pick):
    """
    Single source of truth:
    Base Player Value -> Strategic Pick Value -> 10-branch rollout
    -> regret/challenger test -> FINAL PICK.
    """
    z=_single_authority_strategy(y,roster,slots,current_pick,next_pick)
    ...
    for idx in contenders:
        rv=_multi_branch_rollout(z,idx,roster,slots,current_pick,next_pick,branches=10)
        z.at[idx,"rollout_value"]=rv
        z.at[idx,"final_pick_value"]=float(z.at[idx,"strategic_pick_value"])+rv

    ranked=z.loc[z["single_authority_shortlist"],"final_pick_value"].sort_values(ascending=False)
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    # FINAL PICK has one authority. Hard legality/completion blocks remain upstream.
...
def grade_mock(roster, teams, slots):
    ...
    _rv=pd.to_numeric(roster.get("vorp",0),errors="coerce").fillna(0.0)
    _rp=pd.to_numeric(roster.get("projection",0),errors="coerce").fillna(0.0)
    _ro=pd.to_numeric(roster.get("roster_opportunity",roster.get("roster_opportunity_adj",0)),errors="coerce").fillna(0.0)
    ...
    _draft_value_edge_penalty=min(12.0,_draft_value_gap*0.55)
    ...
    score=float(np.clip(.34*draft_value+.30*construction+.22*positional+.14*model_edge-1.12*opp,0,100))
...
def _sim_user_pick_same_engine(board,roster,round_no,current_pick,next_pick,slots,teams=12):
    """Automated simulator calls the same candidate engine as Live/Interactive Mock."""
    try:
        ranked,_=v939_shared_candidate_rank(
            board.copy(),roster.copy(),int(round_no),int(current_pick),int(next_pick),
            slots,teams=int(teams)
        )
...
def simulate_current_fantasy_edge_once(board, teams, slot, rounds, slots, randomness, seed):
    ...
    for overall in range(1,int(teams)*int(rounds)+1):
        ...
        if int(owner)==int(slot):
            ...
            choice=None
            try:
                _shared_pick=_sim_user_pick_same_engine(board,roster,rnd,current_pick,next_pick,slots,teams)
                if _shared_pick is not None:
                    choice=_shared_pick
            except Exception:
                choice=None
            if choice is None:
