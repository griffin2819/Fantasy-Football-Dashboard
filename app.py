...
st.title("🏈 Fantasy Edge V7.32 — 7× VORP Champion Validation")
st.caption("V7.32 • V7.24 champion retained • 7× VORP champion validation")
st.caption("Build ID: V7.32-7XCV-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v732_live_slot")
    ...
    if "v732_target_queue" not in st.session_state:
        st.session_state.v732_target_queue={}
    queue=st.session_state.v732_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v732_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.32 keeps the V7.24 champion as the live/default model while the 7× VORP candidate undergoes final matched champion validation.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.32 robustness report")
        ...
        st.markdown("### 🧪 V7.32 champion baseline validation")
        st.info("V7.32 keeps the V7.24 champion unchanged in the standard test. The 7× candidate runs only in final matched champion validation.")
        ...
        if "v732_independent_results" not in st.session_state:
            st.session_state.v732_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v732_independent_results"),pd.DataFrame) else "ready to run"))
        if "v732_independent_summary" not in st.session_state:
            st.session_state.v732_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v732_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v732_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v732_run")
        clear_iv=clear_col.button("Clear independent results",key="v732_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v732_independent_results=iv
                st.session_state.v732_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v732_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v732_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.32 Champion Baseline Check")
            st.caption("The standard independent test still uses the unchanged V7.24 champion. The validation below directly compares the 9× champion against the 7× candidate.")
            ...
            st.download_button(
                "⬇️ Download V7.32 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_32_champion_baseline.csv",
                "text/csv",
                key="v732_baseline_download"
            )
        ...
        st.markdown("### 🏆 V7.32 — 7× VORP Champion Validation")
        st.caption(
            "Final matched validation: V7.24 champion (9× VORP) versus the identical model with only VORP changed to 7×. "
            "Replacement remains 38/55/7 and Early-WR remains 10/52."
        )

        if "v732_validation_results" not in st.session_state:
            st.session_state.v732_validation_results=None
        if "v732_validation_summary" not in st.session_state:
            st.session_state.v732_validation_summary=None

        val_sims=st.number_input(
            "Validation simulations per slot × room",
            min_value=5,max_value=25,value=10,step=5,key="v732_val_sims",
            help="10 gives 200 matched cases per model across 4 draft slots × 5 room types."
        )
        va,vb=st.columns(2)
        run_val=va.button("Run 7× champion validation",type="primary",key="v732_val_run")
        clear_val=vb.button("Clear champion validation",key="v732_val_clear")
        ...
        if run_val:
            configs=[
                ("V7.24 CHAMPION • VORP 9×",9.0),
                ("V7.32 CANDIDATE • VORP 7×",7.0),
            ]
            ...
            for name,weight in configs:
                vb_board=_board_with_vorp_variant(
                    board,int(teams),state["roster_slots"],
                    {"RB":.38,"WR":.55,"TE":.07},weight
                )
                fast_variants[name]=_fast_benchmark_pool(vb_board,int(teams))
            ...
            for ds in val_slots:
                for room_name in val_rooms:
                    for j in range(int(val_sims)):
                        seed=932000+int(ds)*10000+j*37+sum(ord(c) for c in room_name)

                        co=simulate_mock_fast(
                            fast_consensus,int(teams),int(ds),15,state["roster_slots"],
                        ...
                        for name,weight in configs:
                            fe=simulate_mock_fast(
                                fast_variants[name],int(teams),int(ds),15,state["roster_slots"],
                            ...
                            row={
                                "Config":name,"VORP_weight":weight,
                            ...
                            if done==1 or done%40==0 or done==total:
                                status.caption(f"Champion validation: {done}/{total}")

            val=pd.DataFrame(rows)
            st.session_state.v732_validation_results=val
            st.session_state.v732_validation_summary={
                "seconds":time.perf_counter()-started,
                "sims":int(val_sims),"variant_sims":len(val)
            }
            status.success(f"Champion validation complete: {len(val)} model simulations.")
            prog.empty()

        val_saved=st.session_state.get("v732_validation_results")
        if isinstance(val_saved,pd.DataFrame) and len(val_saved):
            meta=st.session_state.get("v732_validation_summary") or {}
            summary=val_saved.groupby(["Config","VORP_weight"],as_index=False).agg(
            ...
            champ=summary[summary.Config.eq("V7.24 CHAMPION • VORP 9×")].iloc[0]
            cand=summary[summary.Config.eq("V7.32 CANDIDATE • VORP 7×")].iloc[0]

            compare=pd.DataFrame([
                {"Metric":"Overall gap","9× champion":champ.Overall_gap,"7× candidate":cand.Overall_gap,
                 "7× change":cand.Overall_gap-champ.Overall_gap},
                {"Metric":"R1-3 gap","9× champion":champ.R1_3_gap,"7× candidate":cand.R1_3_gap,
                 "7× change":cand.R1_3_gap-champ.R1_3_gap},
                {"Metric":"R4-6 gap","9× champion":champ.R4_6_gap,"7× candidate":cand.R4_6_gap,
                 "7× change":cand.R4_6_gap-champ.R4_6_gap},
                {"Metric":"R7-10 gap","9× champion":champ.R7_10_gap,"7× candidate":cand.R7_10_gap,
                 "7× change":cand.R7_10_gap-champ.R7_10_gap},
                {"Metric":"R11+ gap","9× champion":champ.R11_plus_gap,"7× candidate":cand.R11_plus_gap,
                 "7× change":cand.R11_plus_gap-champ.R11_plus_gap},
                {"Metric":"Roster balance","9× champion":champ.Roster_balance,"7× candidate":cand.Roster_balance,
                 "7× change":cand.Roster_balance-champ.Roster_balance},
            ])
            ...
            st.markdown("#### Validation by draft slot — watch Picks 7 and 12")
            ...
            piv=val_saved.pivot_table(
                ...
                columns="Config",values=["Difference","R4_6_market_gap","R1_3_market_gap"],
                aggfunc="first"
            )
            paired_n=len(piv)
            if paired_n:
                overall_delta=(
                    piv[("Difference","V7.32 CANDIDATE • VORP 7×")]
                    -piv[("Difference","V7.24 CHAMPION • VORP 9×")]
                )
                mid_delta=(
                    piv[("R4_6_market_gap","V7.32 CANDIDATE • VORP 7×")]
                    -piv[("R4_6_market_gap","V7.24 CHAMPION • VORP 9×")]
                )
                early_delta=(
                    piv[("R1_3_market_gap","V7.32 CANDIDATE • VORP 7×")]
                    -piv[("R1_3_market_gap","V7.24 CHAMPION • VORP 9×")]
                )
                p1,p2,p3=st.columns(3)
                p1.metric("7× wins paired overall",f"{(overall_delta>0).mean():.1%}")
                p2.metric("7× improves paired R4-6",f"{(mid_delta>0).mean():.1%}")
                p3.metric("7× avoids R1-3 damage",f"{(early_delta>=0).mean():.1%}")

                # Promotion gate: overall better, R4-6 better, no severe early/downstream regression.
                promote=(
                    cand.Overall_gap>champ.Overall_gap
                    and cand.R4_6_gap>champ.R4_6_gap
                    and (cand.R1_3_gap-champ.R1_3_gap)>-0.40
                    and (cand.R7_10_gap-champ.R7_10_gap)>-0.40
                    and (overall_delta>0).mean()>=0.50
                )
                if promote:
                    st.success(
                        "PROMOTION GATE PASSED: 7× improves overall and R4-6 performance without crossing the protected "
                        "R1-3/R7-10 regression limits. V7.33 can promote 7× as the new champion."
                    )
                else:
                    st.warning(
                        "PROMOTION GATE NOT YET PASSED. Keep the V7.24 9× champion and inspect the slot/room breakdown before changing live logic."
                    )

            st.success(
                f"Saved V7.32 validation: {len(val_saved)} model simulations"
                + (f" • {meta.get('seconds',0):.1f}s" if meta else "")
            )
            st.download_button(
                "⬇️ Download V7.32 7× champion validation CSV",
                val_saved.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_32_7x_champion_validation.csv",
                "text/csv",key="v732_val_download"
            )
