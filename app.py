...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None,
                       ...
                       r3_qb_opportunity_cost_enabled=False):
...
st.title("🏈 Fantasy Edge V8.03 — Frozen R3 QB Opportunity-Cost Validation")
st.caption("V8.03 • V7.93 frozen champion • resumable R3 QB validation")
st.caption("Build ID: V8.03-R3-QB-VALIDATION-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### ⚡ V8.03 — Frozen R3 QB Opportunity-Cost Validation")
        st.caption(
            "Independent fresh-seed validation of the exact V8.02 rule with ZERO tuning. "
            "V7.93 remains frozen/live. The Round-3 QB-only condition, non-QB alternative requirement, "
            "higher frozen draft score + higher VORP, and ≤10 effective market-distance threshold are unchanged. "
            "Validation runs in resumable batches to avoid Streamlit timeouts."
        )

        if "v803_results" not in st.session_state:
            st.session_state.v803_results=pd.DataFrame()
        if "v803_next_index" not in st.session_state:
            st.session_state.v803_next_index=0

        target_sets=st.selectbox(
            "V8.03 validation target",
            options=[1200,1800,2400,3000],
            index=3,
            key="v803_target",
            help="Full target is 3,000 fresh matched sets. You can export partial progress at any point."
        )
        batch_per_combo=st.number_input(
            "Batch size per slot × room",
            min_value=2,max_value=10,value=5,step=1,key="v803_batch",
            help="5 = 300 matched sets per click (12 slots × 5 rooms × 5). Use 2–3 if your Streamlit host still has a short execution timeout."
        )

        a,b=st.columns(2)
        run=a.button("Run next V8.03 batch",type="primary",key="v803_run")
        clear=b.button("Reset V8.03 validation",key="v803_clear")
        ...
        if run:
            existing=st.session_state.v803_results
            already=len(existing)
            remaining=max(int(target_sets)-already,0)
            this_batch=min(batch_sets,remaining)

            if this_batch<=0:
                st.success(f"Target already complete: {already:,} matched sets.")
            else:
                pool=_fast_benchmark_pool(board,int(teams))
                rows=[]
                prog=st.progress(0)
                status=st.empty()
                start_index=int(st.session_state.v803_next_index)
                ...
                for n in range(this_batch):
                    ...
                    seed=803000000+int(ds)*100000+int(cycle)*419+sum(ord(ch) for ch in room_name)
                    ...
                    if len(rr):
                        ...
                        trigger=bool(int(pd.to_numeric(
                            pd.Series([r.get("R802_Would_trigger",0)]),
                            errors="coerce"
                        ).fillna(0).iloc[0]))
                        normal=str(r.get("R802_Normal_player",""))
                        alt=str(r.get("R802_Alt_player",""))
                        altpos=str(r.get("R802_Alt_position",""))
                    ...
                    if trigger:
                        ...
                        changed=sum(champ_players.get(x.mock_pick,None)!=x.player for _,x in cand.iterrows())
                    ...
                    if (n+1)==1 or (n+1)%50==0 or (n+1)==this_batch:
                        ...
                        status.caption(
                            f"Batch {n+1}/{this_batch} • total after batch {already+n+1:,}/{target_sets:,}"
                        )

                newdf=pd.DataFrame(rows)
                combined=pd.concat([existing,newdf],ignore_index=True) if len(existing) else newdf
                combined=combined.drop_duplicates(subset=["Global_index"],keep="last").sort_values("Global_index").reset_index(drop=True)
                st.session_state.v803_results=combined
                st.session_state.v803_next_index=start_index+this_batch

                status.success(
                    f"Batch complete. Saved {len(combined):,}/{target_sets:,} matched sets. Run the next batch when ready."
                )
                prog.empty()

        df=st.session_state.get("v803_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            summary=pd.DataFrame([{
                ...
                "Avg changed picks when triggered":trig.Changed_picks.mean() if len(trig) else np.nan,
            }])
            ...
            if len(df)>=1200:
                ...
                st.markdown("#### Validation by all five rooms")
                st.dataframe(room.round(4),use_container_width=True,hide_index=True)
                st.markdown("#### Validation by all 12 draft slots")
                st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            if len(df)>=int(target_sets):
                active_rooms=room[room.Activation>0] if 'room' in locals() else pd.DataFrame()
                active_slots=slot[slot.Activation>0] if 'slot' in locals() else pd.DataFrame()
                ...
                passed=(
                    ...
                    and pd.notna(nonloss) and nonloss>=.65
                    and (len(active_rooms)==0 or int((active_rooms.Overall_change>0).sum())>=4)
                    and (len(active_slots)==0 or int((active_slots.Overall_change>=0).sum())>=10)
                )

                if passed:
                    st.success(
                        "V8.03 VALIDATION PASSED. The exact frozen V8.02 R3 QB opportunity-cost rule reproduced on independent fresh seeds. "
                        "V7.93 remains champion until a separate production-path promotion certification."
                    )
                else:
                    st.warning(
                        "V8.03 validation failed. Keep V7.93 champion and do not tune the frozen R3 QB rule from this sample."
                    )
            else:
                st.info(
                    f"Progress is saved in this Streamlit session: {len(df):,}/{target_sets:,}. "
                    "Run the next batch instead of restarting."
                )

            st.download_button(
                "⬇️ Download current V8.03 validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_03_frozen_r3_qb_opportunity_cost_validation.csv",
                "text/csv",key="v803_download"
            )
