...
st.title("🏈 Fantasy Edge V7.75 — Frozen V7.67 Champion Next-Opportunity Audit II")
st.caption("V7.75 • V7.67 frozen champion • next independent opportunity audit")
st.caption("Build ID: V7.75-FROZEN-CHAMPION-NEXT-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.75 keeps V7.67 frozen/live, retires the Round-5 challenger, and audits fresh champion drafts for the next independent improvement opportunity.")
        ...
        st.markdown("### 🔒 V7.75 — Frozen Champion Next-Opportunity Audit II")
        st.caption(
            "V7.67 remains the frozen production champion. The Round-5 challenger is retired and remains OFF. "
            "V7.75 changes no draft behavior. It runs fresh champion drafts and measures the remaining independent weaknesses "
            "by round, phase, position, player concentration, market reach, and market faller behavior."
        )

        if "v775_results" not in st.session_state:
            st.session_state.v775_results=None

        sims=st.number_input(
            "V7.75 audit simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v775_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.75 next-opportunity audit",type="primary",key="v775_run")
        clear=b.button("Clear V7.75 audit",key="v775_clear")
        if clear:
            st.session_state.v775_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=975000+int(ds)*100000+j*241+sum(ord(c) for c in room_name)

                        # Frozen V7.67 production path:
                        # Round-7 certified rule ON, retired Round-5 challenger OFF.
                        ...
                        for _,r in d.iterrows():
                            ...
                            phase_name="R1-3" if rnd<=3 else ("R4-6" if rnd<=6 else ("R7-10" if rnd<=10 else "R11+"))
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.75 audit: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v775_results=df
            status.success(f"V7.75 complete: {total} champion drafts / {len(df)} individual decisions.")
            prog.empty()

        df=st.session_state.get("v775_results")
        if isinstance(df,pd.DataFrame) and len(df):
            d=df.copy()
            d["Market_delta"]=pd.to_numeric(d.Market_delta,errors="coerce")
            d["Reach10"]=d.Market_delta>10
            d["Reach15"]=d.Market_delta>15
            d["Reach20"]=d.Market_delta>20
            d["Faller10"]=d.Market_delta<-10
            d["Faller20"]=d.Market_delta<-20

            phase_summary=d.groupby("Phase",as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Reach10_rate=("Reach10","mean"),
                Reach15_rate=("Reach15","mean"),
                Reach20_rate=("Reach20","mean"),
            ...
            ).sort_values(["Decisions","Avg_market_delta"],ascending=[False,False])

            st.markdown("#### Most repeated >10-pick reaches")
            st.dataframe(reaches.head(50).round(3),use_container_width=True,hide_index=True)

            fallers=d[d.Faller10].groupby(
                ["Round","Player","Position"],as_index=False
            ).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
            ...
            st.markdown("#### Most repeated >10-pick fallers")
            st.dataframe(fallers.head(50).round(3),use_container_width=True,hide_index=True)

            pos=d.groupby(["Phase","Position"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Reach10_rate=("Reach10","mean"),
                Reach20_rate=("Reach20","mean"),
            ...
            slot_round=d.groupby(["Draft_slot","Round"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Reach10_rate=("Reach10","mean"),
                Faller10_rate=("Faller10","mean")
            )
            st.markdown("#### Draft slot × round")
            st.dataframe(slot_round.round(4),use_container_width=True,hide_index=True)

            room_round=d.groupby(["Room","Round"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
                Reach10_rate=("Reach10","mean"),
                Faller10_rate=("Faller10","mean")
            )
            st.markdown("#### Room × round")
            st.dataframe(room_round.round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.75 is measurement-only. Round 5 is retired from this branch and the certified Round-7 rule remains frozen. "
                "Use the strongest repeated independent signal from this fresh audit as the next diagnostic target."
            )

            st.download_button(
                "⬇️ Download V7.75 frozen champion next-opportunity audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_75_frozen_champion_next_opportunity_audit.csv",
                "text/csv",key="v775_download"
            )
