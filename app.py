...
st.title("🏈 Fantasy Edge V7.79 — Round-3 True-Board Counterfactual Audit")
st.caption("V7.79 • V7.67 frozen champion • Round-3 true-board diagnostic")
st.caption("Build ID: V7.79-R3-TRUEBOARD-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.79 keeps V7.67 frozen/live and audits Round-3 >10-pick reaches against genuinely available true-board alternatives.")
        ...
        st.markdown("### 🔬 V7.79 — Round-3 True-Board Counterfactual Audit")
        st.caption(
            "Measurement only. V7.67 remains the frozen production champion. "
            "For every Round-3 champion selection that reaches >10 picks versus effective market, V7.79 captures the actual alive+eligible board state "
            "after opponent picks and records the top five genuinely available market-respecting alternatives. No player is hard-coded."
        )

        if "v779_results" not in st.session_state:
            st.session_state.v779_results=None

        sims=st.number_input(
            "V7.79 simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v779_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-3 true-board audit",type="primary",key="v779_run")
        clear=b.button("Clear V7.79 audit",key="v779_clear")
        if clear:
            st.session_state.v779_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=979000+int(ds)*100000+j*269+sum(ord(c) for c in room_name)
                        ...
                        if len(r3):
                            ...
                            audit=int(pd.to_numeric(pd.Series([r.get("R779_Audit",0)]),errors="coerce").fillna(0).iloc[0])
                            ...
                            if pd.notna(reach) and reach>10 and audit==1:
                                row={
                                    ...
                                    "Actual_available_count":r.get("R779_Actual_available_count",np.nan),
                                    "Market_respecting_count":r.get("R779_Market_respecting_count",np.nan),
                                }
                                for k in range(1,6):
                                    for fld in ["player","position","market_pick","model_rank","projection","vorp","draft_score"]:
                                        row[f"Alt{k}_{fld}"]=r.get(f"R779_Alt{k}_{fld}","" if fld in ["player","position"] else np.nan)
                                rows.append(row)

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.79 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v779_results=df
            status.success(f"V7.79 complete: {len(df)} true-board Round-3 >10-pick reaches audited.")
            prog.empty()

        df=st.session_state.get("v779_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.markdown("#### Round-3 reach drivers")
            ...
            showcols=["Draft_slot","Room","Round3_pick","Champion_player","Champion_position","Champion_reach"]
            for k in range(1,6):
            ...
            st.info(
                "V7.79 changes no production behavior. If a Round-3 true-board alternative repeatedly has both higher frozen draft score and higher VORP "
                "while improving market price, the next build can test a narrow full-draft challenger. Otherwise the reach is a justified model conviction."
            )

            st.download_button(
                "⬇️ Download V7.79 Round-3 true-board audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_79_round3_true_board_counterfactual_audit.csv",
                "text/csv",key="v779_download"
            )
        elif isinstance(df,pd.DataFrame):
            st.success("No Round-3 >10-pick reaches occurred in this run.")
