...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Fast Refresh Save", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v809_decision_mask(df,state_snapshot):
    ...
    Deep intelligence is useful for fantasy-relevant rows, not thousands of
    directory-only players with no projection/rank.  Keep all rows in the board but
    bound row-wise model work to owned players plus ranked/projected candidates.
    """
    if df is None or df.empty:
        return pd.Series(False,index=getattr(df,'index',[]))
    idx=df.index
    proj=pd.to_numeric(df.get('projection',pd.Series(index=idx,dtype=float)),errors='coerce')
    mr=pd.to_numeric(df.get('market_pick',df.get('consensus_rank',pd.Series(index=idx,dtype=float))),errors='coerce')
    model=pd.to_numeric(df.get('model_rank',pd.Series(index=idx,dtype=float)),errors='coerce')
    pos=df.get('position',pd.Series('',index=idx)).astype(str)
    mask=(proj.notna() & (proj>0)) | (mr.notna() & (mr<=450)) | (model.notna() & (model<=450))
    try:
        own=(state_snapshot or {}).get('ownership',{}) or {}
        owned_keys={str(k) for k,v in own.items() if isinstance(v,dict) and str(v.get('owner','FA'))!='FA'}
        keys=df.get('player',pd.Series('',index=idx)).astype(str).map(_owner_key)
        mask=mask | keys.isin(owned_keys)
    except Exception:
        pass
    mask=mask & pos.isin(['QB','RB','WR','TE','K','DL','DB'])
    # Hard ceiling keeps refresh bounded even if a provider marks every row ranked.
    if int(mask.sum())>900:
        score=pd.DataFrame({'mask':mask,'mr':mr.fillna(9999),'model':model.fillna(9999),'proj':proj.fillna(0)},index=idx)
        ranked=score[score['mask']].sort_values(['mr','model','proj'],ascending=[True,True,False]).head(900).index
        mask=pd.Series(False,index=idx); mask.loc[ranked]=True
    return mask
...
try:
    ...
    if _refresh_live or not _v779_canonical_ready:
        ...
        if len(_v809_idx):
            ...
            board.loc[_v809_idx,"fe_calibrated_confidence"]=board.loc[_v809_idx].apply(lambda r:_v756_calibrated_confidence(r,_hist756,state),axis=1)
            board.loc[_v809_idx,"fe_news_impact"]=board.loc[_v809_idx].apply(_v756_news_impact,axis=1)
