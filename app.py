...
st.title("🏈 Fantasy Edge V7.84 — R11–13 Offense-Only Dominance Challenger")
st.caption("V7.84 • V7.67 frozen champion • R11–13 offense-only challenger")
st.caption("Build ID: V7.84-R11-13-OFFENSE-DOMINANCE-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.84 keeps V7.67 frozen/live and tests the generic offense-only R11–13 deep-faller dominance rule across the full remainder of each draft.")
        ...
        st.markdown("### 🧪 V7.84 — R11–13 Offense-Only Dominance Challenger")
        st.caption(
            "Full-draft matched challenger. V7.67 remains frozen/live. In R11–13 only, when the normal champion selection is >30 picks below market, "
            "V7.84 may switch to a genuinely alive+eligible OFFENSIVE alternative only if it has BOTH higher frozen draft score and higher VORP. "
            "IDPs are excluded from the experimental override."
        )

        if "v784_results" not in st.session_state:
            st.session_state.v784_results=None

        sims=st.number_input(
            "V7.84 matched simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v784_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh matched sets. Trigger-first reruns the challenger only when at least one R11–13 offense-only dominance trigger occurs."
        )
        a,b=st.columns(2)
        run=a.button("Run R11–13 offense-only challenger",type="primary",key="v784_run")
        clear=b.button("Clear V7.84 challenger",key="v784_clear")
        if clear:
            st.session_state.v784_results=None
            st.rerun()
        ...
        if run:
            ...
            rows=[]; total=len(slots_to_test)*len(rooms)*int(sims); done=0; reruns=0
            prog=st.progress(0); status=st.empty()

            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=984000+int(ds)*100000+j*293+sum(ord(c) for c in room_name)

                        champ=simulate_mock_fast(
                        ...
                        late=champ[pd.to_numeric(champ.mock_round,errors="coerce").between(11,13,inclusive="both")]
                        trigger=False
                        trigger_rounds=[]
                        paths=[]
                        for _,rr in late.iterrows():
                            wt=bool(int(pd.to_numeric(pd.Series([rr.get("R784_Would_trigger",0)]),errors="coerce").fillna(0).iloc[0]))
                            if wt:
                                trigger=True
                                trigger_rounds.append(int(rr.mock_round))
                                paths.append(f"{rr.get('R784_Normal_player','')} -> {rr.get('R784_Best_off_alt_player','')}")

                        if trigger:
                            cand=simulate_mock_fast(
                                pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name,
                                r5_dominance_override_enabled=False,
                                r2_dominance_override_enabled=False,
                                r3_dominance_override_enabled=False,
                                r11_13_offense_dominance_override_enabled=True
                            )
                            reruns+=1
                            cc=independent_draft_grade_components(champ,int(teams))
                            nc=independent_draft_grade_components(cand,int(teams))
                            cp=independent_phase_metrics(champ).set_index("Phase")
                            np_=independent_phase_metrics(cand).set_index("Phase")
                            champ_players=champ.set_index("mock_pick")["player"].to_dict()
                            changed=sum(champ_players.get(rr.mock_pick,None)!=rr.player for _,rr in cand.iterrows())
                            overall=nc["overall"]-cc["overall"]
                            r13=float(np_.loc["R1-3","Avg_market_delta"])-float(cp.loc["R1-3","Avg_market_delta"])
                            r46=float(np_.loc["R4-6","Avg_market_delta"])-float(cp.loc["R4-6","Avg_market_delta"])
                            r710=float(np_.loc["R7-10","Avg_market_delta"])-float(cp.loc["R7-10","Avg_market_delta"])
                            r11=float(np_.loc["R11+","Avg_market_delta"])-float(cp.loc["R11+","Avg_market_delta"])
                        else:
                            changed=0
                            overall=r13=r46=r710=r11=0.0

                        rows.append({
                            "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Trigger_detected":trigger,"Challenger_rerun":trigger,
                            "Trigger_count":len(trigger_rounds),
                            "Trigger_rounds":",".join(map(str,trigger_rounds)),
                            "Detected_paths":" | ".join(paths),
                            "Behavior_changed":changed>0,"Changed_picks":changed,
                            "Overall_change":overall,"R1_3_change":r13,"R4_6_change":r46,
                            "R7_10_change":r710,"R11_plus_change":r11,
                            "Conditional_overall_win":(overall>0) if trigger else np.nan,
                            "Conditional_nonloss":(overall>=0) if trigger else np.nan,
                            "Conditional_R11_win":(r11>0) if trigger else np.nan,
                            "Conditional_R11_nonloss":(r11>=0) if trigger else np.nan,
                        })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.84: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v784_results=df
            status.success(
                f"V7.84 complete: {len(df)} matched sets using "
                f"{len(df)+int(df.Challenger_rerun.sum())} full simulations instead of {len(df)*2}."
            )
            prog.empty()

        df=st.session_state.get("v784_results")
        if isinstance(df,pd.DataFrame) and len(df):
            trig=df[df.Trigger_detected].copy()
            overall=df.Overall_change.mean()
            r13=df.R1_3_change.mean()
            r46=df.R4_6_change.mean()
            r710=df.R7_10_change.mean()
            r11=df.R11_plus_change.mean()
            activation=df.Trigger_detected.mean()
            cond=trig.Conditional_overall_win.mean() if len(trig) else np.nan
            nonloss=trig.Conditional_nonloss.mean() if len(trig) else np.nan
            cond11=trig.Conditional_R11_win.mean() if len(trig) else np.nan

            summary=pd.DataFrame([{
                "Matched sets":len(df),"Triggered sets":len(trig),
                "Activation":activation,"Overall change":overall,
                "R1-3 change":r13,"R4-6 change":r46,"R7-10 change":r710,
                "R11+ change":r11,"Conditional overall win":cond,
                "Conditional non-loss":nonloss,"Conditional R11+ win":cond11,
            }])
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            slot=df.groupby("Draft_slot",as_index=False).agg(
                Overall_change=("Overall_change","mean"),R11_plus_change=("R11_plus_change","mean"),
                Activation=("Trigger_detected","mean")
            )
            room=df.groupby("Room",as_index=False).agg(
                Overall_change=("Overall_change","mean"),R11_plus_change=("R11_plus_change","mean"),
                Activation=("Trigger_detected","mean")
            )
            st.markdown("#### Challenger by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Challenger by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            path_counts=trig.groupby(["Detected_paths"],as_index=False).agg(
                Decisions=("Trigger_detected","count"),
                Overall_change=("Overall_change","mean"),
                R11_plus_change=("R11_plus_change","mean"),
                Win_rate=("Conditional_overall_win","mean")
            ).sort_values("Decisions",ascending=False)
            st.markdown("#### Most common detected late-round paths")
            st.dataframe(path_counts.head(30).round(4),use_container_width=True,hide_index=True)

            active_rooms=room[room.Activation>0]
            active_slots=slot[slot.Activation>0]
            screen=(
                overall>0 and r11>0
                and r13>=0 and r46>=0 and r710>=0
                and activation>=.10
                and pd.notna(cond) and cond>=.70
                and pd.notna(nonloss) and nonloss>=.75
                and int((active_rooms.Overall_change>0).sum())>=max(1,len(active_rooms)-1)
                and int((active_slots.Overall_change>0).sum())==len(active_slots)
            )
            if screen:
                st.success("V7.84 SCREEN PASSED. Advance the exact frozen offense-only R11–13 rule to independent validation; V7.67 remains champion.")
            else:
                st.warning("V7.84 does not clear screening. Keep V7.67 champion and do not tune this rule from the sample.")

            st.download_button(
                "⬇️ Download V7.84 offense-only R11–13 challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_84_r11_13_offense_only_dominance_challenger.csv",
                "text/csv",key="v784_download"
            )
