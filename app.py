...
st.title("🏈 Fantasy Edge V7.86 — Late-Round Sequence Counterfactual")
st.caption("V7.86 • V7.67 frozen champion • late-round sequence counterfactual")
st.caption("Build ID: V7.86-LATE-SEQUENCE-CF-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.86 keeps V7.67 frozen/live and tests complete R11–15 positional sequences rather than isolated late-round player overrides.")
        ...
        st.markdown("### 🧪 V7.86 — Late-Round Sequence Counterfactual")
        st.caption(
            "Matched sequence-level experiment. V7.67 remains frozen/live. "
            "The champion sequence is OFF→OFF→OFF→DB→DL in R11–15. "
            "V7.86 compares that complete structure against two alternatives: "
            "OFF→OFF→DB→OFF→DL and OFF→DB→OFF→OFF→DL. "
            "Only the broad positional class is constrained; the frozen engine still chooses the best player within that class."
        )

        if "v786_results" not in st.session_state:
            st.session_state.v786_results=None

        sims=st.number_input(
            "V7.86 matched simulations per slot × room",
            min_value=10,max_value=50,value=25,step=5,key="v786_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 matched seeds per sequence variant."
        )
        a,b=st.columns(2)
        run=a.button("Run late-sequence counterfactual",type="primary",key="v786_run")
        clear=b.button("Clear V7.86 audit",key="v786_clear")
        if clear:
            st.session_state.v786_results=None
            st.rerun()
        ...
        if run:
            ...
            rows=[]; total=len(slots_to_test)*len(rooms)*int(sims)*len(variants); done=0
            prog=st.progress(0); status=st.empty()

            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=986000+int(ds)*100000+j*311+sum(ord(c) for c in room_name)
                        ...
                        for variant in variants:
                            ...
                            late_seq=" > ".join(
                                cand[pd.to_numeric(cand.mock_round,errors="coerce").between(11,15,inclusive="both")]
                                .sort_values("mock_round")["position"].astype(str).tolist()
                            )
                            ...
                            rows.append({
                                ...
                                "Candidate_late_position_sequence":late_seq,
                                "Overall_change":overall,
                                "R1_3_change":r13,
                                "R4_6_change":r46,
                                "R7_10_change":r710,
                                "R11_plus_change":r11,
                                "Overall_win":overall>0,
                                "Overall_nonloss":overall>=0,
                                "R11_win":r11>0,
                                "R11_nonloss":r11>=0,
                            })
                            ...
                            if done==1 or done%200==0 or done==total:
                                status.caption(f"V7.86: {done}/{total} sequence comparisons")

            df=pd.DataFrame(rows)
            st.session_state.v786_results=df
            status.success(f"V7.86 complete: {len(df)} sequence-level matched comparisons.")
            prog.empty()

        df=st.session_state.get("v786_results")
        if isinstance(df,pd.DataFrame) and len(df):
            summary=df.groupby("Variant",as_index=False).agg(
                Matched_sets=("Variant","count"),
                ...
                Overall_win_rate=("Overall_win","mean"),
                Overall_nonloss=("Overall_nonloss","mean"),
                R11_win_rate=("R11_win","mean"),
                R11_nonloss=("R11_nonloss","mean"),
                Avg_changed_picks=("Changed_picks","mean"),
            )
            st.markdown("#### Sequence-level results")
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            slot=df.groupby(["Variant","Draft_slot"],as_index=False).agg(
                Overall_change=("Overall_change","mean"),
                R11_plus_change=("R11_plus_change","mean"),
                Win_rate=("Overall_win","mean")
            )
            room=df.groupby(["Variant","Room"],as_index=False).agg(
                Overall_change=("Overall_change","mean"),
                R11_plus_change=("R11_plus_change","mean"),
                Win_rate=("Overall_win","mean")
            )
            st.markdown("#### Variant × draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Variant × room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            seqs=df.groupby(["Variant","Candidate_late_position_sequence"],as_index=False).size().rename(columns={"size":"Decisions"})
            st.markdown("#### Realized R11–15 position sequences")
            st.dataframe(seqs.sort_values(["Variant","Decisions"],ascending=[True,False]),use_container_width=True,hide_index=True)

            st.info(
                "V7.86 is an architecture test, not a promotion build. If both earlier-IDP variants lose, the frozen OFF→OFF→OFF→DB→DL sequence is strongly supported. "
                "If one variant wins consistently across rooms/slots, the next build should validate that exact sequence with fresh seeds."
            )

            st.download_button(
                "⬇️ Download V7.86 late-sequence counterfactual CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_86_late_round_sequence_counterfactual.csv",
                "text/csv",key="v786_download"
            )
