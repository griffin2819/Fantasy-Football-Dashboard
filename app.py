...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Decision Data Resolution", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v756_certify(board_df,state_snapshot):
    ...
    if len(q):
        ...
        q_block=q_scope[q_scope.get("Severity",pd.Series("REVIEW",index=q_scope.index)).astype(str).eq("BLOCK")]
    else:
...
def _v755_command_center(board_df,state_snapshot):
    ...
    actions=[]
    # Start/sit and injury actions
    for _,sr in starters.iterrows():
        p=str(sr.get("Player","")); rr=my[my["player"].eq(p)]
        if rr.empty: continue
        r=rr.iloc[0]; status=str(r.get("fe_status","ACTIVE")); conf=float(r.get("fe_role_confidence",0) or 0)
        if status in ("OUT","IR","PUP","NFI","DOUBTFUL"):
            actions.append((100,"START/SIT",f"Replace {p}",status))
        elif status=="QUESTIONABLE": actions.append((86,"INJURY",f"Monitor {p}","Questionable starter"))
        elif conf<45: actions.append((62,"ROLE",f"Review {p}","Low role confidence"))
    # Data-integrity warning outranks optional roster moves
    q=_v755_quality_flags(board_df)
    if not q.empty:
        mine=q[q["Player"].isin(my["player"])]
        if not mine.empty: actions.append((96,"DATA",f"Verify {mine.iloc[0]['Player']}",mine.iloc[0]['Flags']))
    # V7.57: Command Center is a VIEW of the certified Waiver engine, never a second
    # waiver model.  This closes the path that could independently choose the lowest
    # trade-value bench player (for example Alvin Kamara) even after Waivers protected him.
    shared=pd.DataFrame(st.session_state.get("_v742_waiver_transactions",[]))
    if len(shared):
        # Only rows that survived the Waiver tab's final V7.14/V7.37 safety boundary
        # are eligible to appear here. Recheck the stored protection tier defensively.
        ...
        for _,wr in shared.iterrows():
            ...
            if len(dr):
                ...
                # Ordinary recommendations may never drop LOCKED/PROTECTED assets.
    ...
    if my.empty:
        ...
        if len(_detail_rows):
            ...
            for _,_wr in _detail_rows.head(3).iterrows():
                ...
                if _ck=="Source freshness": _friendly.append("A decision-relevant player status still needs verification")
                elif _ck=="Decision data integrity": _friendly.append("A decision-relevant player record needs review")
        ...
        if not _actions:
            st.markdown("<div class='fecc-ok'>✅ <b>Your lineup is optimized. No action needed.</b><br><span style='color:#abc0b6'>No Start/Sit or Waiver move currently clears Fantasy Edge's action threshold. Injury watches can still require monitoring.</span></div>",unsafe_allow_html=True)
        else:
