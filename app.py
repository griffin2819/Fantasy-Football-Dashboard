...
st.set_page_config(page_title="Fantasy Edge v9.39 — Exact League Construction", page_icon="🏈", layout="wide")
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    for _,r in x.iterrows():
        if str(r.get("position","")).upper() in ("DL","DB"):
            ...
            impacts.append(np.nan); ext_ranks.append(np.nan); eligible.append(np.nan)
    x["idp_impact_score"]=impacts
    x["idp_external_rank"]=ext_ranks
    x["idp_eligible"]=eligible
...
st.title("🏈 Fantasy Edge v9.39 — Exact League Construction + Unified Recommendation")
st.caption("v9.38 • v9.08 certified engine • unified Draft/Mock player universe • 12-team snake • DB/DL IDP ingestion")
st.caption("Build ID: FE-V9.39-SHARED-DRAFT-MOCK-IDP-20260820")
...
with tabs[2]:
    ...
    st.success("🔒 v9.39 SAME Draft/Mock scoring • 🐍 12-team snake • 🧠 unified DB/DL fantasy-IDP")
