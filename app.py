...
st.title("🏈 Fantasy Edge V7.65 — Frozen Round-7 Dominance Validation")
st.caption("V7.65 • V7.60 frozen champion • frozen Round-7 dominance validation")
st.caption("Build ID: V7.65-R7-DOMINANCE-VALIDATION-20260816")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.65 keeps V7.60 frozen/live and performs large fresh-seed validation of the exact V7.64 Round-7 dominance rule with no additional tuning.")
        ...
        st.markdown("### 🧊 V7.65 — Frozen Round-7 Dominance Validation")
        st.caption(
            "Large-sample validation only. V7.60 remains the frozen live champion. "
            "V7.65 reuses the exact V7.64 Round-7 dominance rule with no threshold, player, slot, room, or weight changes."
        )

        if "v765_results" not in st.session_state:
            st.session_state.v765_results=None

        sims=st.number_input(
            "V7.65 validation simulations per slot × room",
            min_value=50,max_value=150,value=100,step=10,key="v765_sims",
            help="Default: 4 slots × 5 rooms × 100 = 2,000 fresh matched V7.60-vs-V7.65 drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.65 frozen validation",type="primary",key="v765_run")
        clear=b.button("Clear V7.65 validation",key="v765_clear")
        if clear:
            st.session_state.v765_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=965000+int(ds)*100000+j*191+sum(ord(c) for c in room_name)
                        ...
                        rows.append({
                            ...
                            "R7_10_change":r710,
                            "R11_plus_change":r11,
                            "Roster_balance_change":_gc(nc,"roster_balance")-_gc(cc,"roster_balance"),
                            "Starter_completion_change":_gc(nc,"starter_completion")-_gc(cc,"starter_completion"),
                            "Availability_change":_gc(nc,"availability")-_gc(cc,"availability"),
                            "Market_value_change":_gc(nc,"market_value")-_gc(cc,"market_value"),
                        ...
                        if done==1 or done%250==0 or done==total:
                            status.caption(f"V7.65 validation: {done}/{total} fresh matched drafts")

            df=pd.DataFrame(rows)
            st.session_state.v765_results=df
            status.success(f"V7.65 complete: {len(df)} matched drafts.")
            prog.empty()

        df=st.session_state.get("v765_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.markdown("#### Frozen Round-7 override paths")
            ...
            st.markdown("#### V7.65 promotion-validation gate")
            st.dataframe(gate.round(4),use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.65 CERTIFIED. The exact V7.64 Round-7 dominance rule survives large fresh-seed validation and is eligible for promotion."
                )
            else:
                st.warning(
                    "V7.65 does not clear the frozen validation gate. Keep V7.60 champion and do not retune the rule from this sample."
                )

            st.download_button(
                "⬇️ Download V7.65 frozen Round-7 validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_65_frozen_round7_dominance_validation.csv",
                "text/csv",key="v765_download"
            )
