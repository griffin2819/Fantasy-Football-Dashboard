...
st.set_page_config(page_title="Fantasy Edge v9.30 — Live Draft", page_icon="🏈", layout="wide")
...
def roster_opportunity_adjustment(df, roster, round_no, current_pick, slots):
    ...
    for _,r in x.iterrows():
        ...
        if pos in ["DL","DB"]:
            ...
            if pd.notna(_idp_vorp) and _idp_vorp >= 1.5:
                adj += 9.0
...
st.title("🏈 Fantasy Edge v9.30 — Live Draft + Unified Recommendation")
st.caption("v9.30 • v9.08 certified engine • v9.29 WR/RB frozen + onesie gate + IDP eligibility + unified recommendation")
...
with tabs[1]:
    ...
    if intercepted:
        ...
        st.success(f"💎 FALLER CONTEXT: {fi.player} ({fi.position}, {fi.team}) — {intercept_fall:.0f} picks past market")
...
with tabs[2]:
    ...
    st.success("🔒 v9.23.1 availability integrity active • 🧠 v9.30 unified recommendation + onesie/IDP gates active")
    ...
    st.caption("V9.08 remains the frozen certified core; V9.29 applies roster-construction, availability, injury, and impact-IDP overlays without changing the frozen core.")
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                if intercept_row is not None:
                    st.success(f"💎 FALLER CONTEXT: {intercept_row.player} — {intercept_fall:.0f} picks past market")
                    dn=intercept_row
                else:
                ...
                if nxt:
                    st.caption(f"Current pick #{overall} • next pick #{nxt}. FINAL PICK uses roster-adjusted evaluation. Model target and faller intercept are context only.")
                top=now_top.copy()
