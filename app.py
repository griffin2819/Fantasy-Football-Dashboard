...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Certified Snapshot Metadata Fix", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
if _refresh_live:
    with st.spinner("Refreshing weekly data once…"):
        try:
            ...
            st.sidebar.success("Live base board refreshed")
        except Exception:
            st.sidebar.warning("Live refresh failed; using prior snapshot/local board.")
            players=pd.DataFrame(); hist=pd.DataFrame(); market=pd.DataFrame()
...
def _v683_data_health(board_df,season,week):
    ...
    b=board_df if isinstance(board_df,pd.DataFrame) else pd.DataFrame()
    total=max(1,len(b))
    live_team=pd.to_numeric(b.get("_live_team_verified",pd.Series(0,index=b.index)),errors="coerce").fillna(0)>0
...
if _refresh_live:
    try:
        ...
        if _v778_blob:
            ...
            _v789_team_cov=int(round(100.0*float(pd.to_numeric(board.get("_live_team_verified",pd.Series(0,index=board.index)),errors="coerce").fillna(0).mean()))) if len(board) else 0
...
if _refresh_live:
    try:
        ...
        if _v779_blob:
            ...
            try:
                _v791_cov["team_coverage"]=int(round(100.0*float(pd.to_numeric(board.get("_live_team_verified",pd.Series(0,index=board.index)),errors="coerce").fillna(0).mean()))) if len(board) else 0
            except Exception:
                pass
            try:
