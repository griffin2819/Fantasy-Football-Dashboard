...
st.title("🏈 Fantasy Edge V7.72 — Frozen Round-5 Dominance Validation")
st.caption("V7.72 • V7.67 frozen champion • optimized Round-5 validation")
st.caption("Build ID: V7.72-R5-DOMINANCE-VALIDATION-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.72 keeps certified V7.67 frozen/live and performs optimized large-sample validation of the exact V7.71 Round-5 dominance rule with zero tuning.")
        ...
        st.markdown("### ⚡ V7.72 — Frozen Round-5 Dominance Validation")
        st.caption(
            "Large-sample validation of the exact V7.71 rule with zero tuning. V7.67 remains frozen/live. "
            "For speed, V7.72 runs the V7.67 baseline once per seed, reads the exact Round-5 would-trigger flag from the true board state, "
            "and only reruns the challenger on triggered seeds. Non-triggered seeds are exact zero-delta matched results."
        )

        if "v772_results" not in st.session_state:
            st.session_state.v772_results=None

        sims=st.number_input(
            "V7.72 validation simulations per slot × room",
            min_value=50,max_value=150,value=100,step=10,key="v772_sims",
            help="Default: 4 slots × 5 rooms × 100 = 2,000 matched sets. At ~5% activation, only ~2,100 full simulations should be required instead of 4,000."
        )
        a,b=st.columns(2)
        run=a.button("Run optimized V7.72 validation",type="primary",key="v772_run")
        clear=b.button("Clear V7.72 validation",key="v772_clear")
        if clear:
            st.session_state.v772_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=972000+int(ds)*100000+j*229+sum(ord(c) for c in room_name)
                        ...
                        trigger=False
                        champ5=""; champ5_pos=""; alt5=""
                        if len(r5):
                            rr=r5.iloc[0]
                            trigger=bool(int(pd.to_numeric(pd.Series([rr.get("R771_Would_trigger",0)]),errors="coerce").fillna(0).iloc[0]))
                            champ5=str(rr.player)
                            champ5_pos=str(rr.position)
                            alt5=str(rr.get("R771_Best_alt_player",""))
                        ...
                        if done==1 or done%250==0 or done==total:
                            status.caption(f"V7.72: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v772_results=df
            status.success(
                f"V7.72 complete: {len(df)} matched sets using {len(df)+int(df.Challenger_rerun.sum())} full simulations "
                f"instead of {len(df)*2}."
            )
            prog.empty()

        df=st.session_state.get("v772_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            full_sims=len(df)+int(df.Challenger_rerun.sum())
            saved=1-(full_sims/(2*len(df)))

            c1,c2,c3,c4=st.columns(4)
            c1.metric("Overall vs V7.67",f"{overall:+.3f}")
            c2.metric("R4–6 vs V7.67",f"{r46:+.3f}")
            c3.metric("Conditional win",f"{cond:.1%}" if pd.notna(cond) else "n/a")
            c4.metric("Simulation work saved",f"{saved:.1%}")

            summary=pd.DataFrame([{
                "Matched sets":len(df),
                "Triggered sets":len(trig),
                "Full simulations executed":full_sims,
                "Full simulations avoided":2*len(df)-full_sims,
            ...
            st.markdown("#### Frozen validation by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Frozen validation by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            paths=trig.groupby(
                ["Champion_round5_player","Champion_round5_position","Candidate_round5_player","Candidate_round5_position"],
                as_index=False
            ).agg(
            ...
            st.markdown("#### Frozen Round-5 override paths")
            ...
            st.markdown("#### V7.72 frozen validation gate")
            st.dataframe(gate.round(4),use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.72 CERTIFIED. The exact V7.71 Round-5 dominance rule survives larger fresh-seed validation. "
                    "It is eligible for a production-path promotion test, with V7.67 still frozen until that test passes."
                )
            else:
                st.warning(
                    "V7.72 does not certify. Keep V7.67 champion and do not retune the Round-5 rule from this validation sample."
                )

            st.download_button(
                "⬇️ Download V7.72 frozen Round-5 validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_72_frozen_round5_dominance_validation.csv",
                "text/csv",key="v772_download"
            )
