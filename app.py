...
st.title("🏈 Fantasy Edge V8.08 — Round-6 Generic Mechanism Audit")
st.caption("V8.08 • V8.05 frozen champion • R6 generic mechanism measurement")
st.caption("Build ID: V8.08-R6-GENERIC-MECHANISM-AUDIT-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V8.08 — Round-6 Generic Mechanism Audit")
        st.caption(
            "Measurement only. V8.05 remains frozen champion. R5 is closed as player-specific/no generic challenger. "
            "V8.08 studies the next unprotected exact-board target, Round 6, and tests whether its signal survives removal "
            "of dominant player identities across position paths, roster states, rooms, slots, market-distance bands, and R7–R9 construction."
        )

        if "v808_results" not in st.session_state:
            st.session_state.v808_results=None

        sims=st.number_input(
            "V8.08 simulations per slot × room", min_value=10, max_value=40, value=20, step=5, key="v808_sims",
            help="Default: 12 slots × 5 rooms × 20 = 1,200 fresh V8.05 champion drafts. Only exact-board Round-6 decisions are retained."
        )
        a,b=st.columns(2)
        run=a.button("Run V8.08 R6 mechanism audit",type="primary",key="v808_run")
        clear=b.button("Clear V8.08 audit",key="v808_clear")
        if clear:
            st.session_state.v808_results=None
            st.rerun()
            ...
            for ds in range(1,int(teams)+1):
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8080000+ds*100000+j*443+sum(ord(c) for c in room_name)
                        ...
                        rr=d[pd.to_numeric(d.mock_round,errors="coerce").eq(6)]
                        if len(rr):
                            r=rr.iloc[0]
                            ...
                            alt=str(r.get("R8063_Alt1_player",""))
                            altpos=str(r.get("R8063_Alt1_position",""))
                            champ=str(r.get("player",""))
                            champpos=str(r.get("position",""))

                            future=d[
                                (pd.to_numeric(d.mock_round,errors="coerce")>=7) &
                                (pd.to_numeric(d.mock_round,errors="coerce")<=9)
                            ].sort_values("mock_round")
                            fvorp=pd.to_numeric(future.get("vorp",pd.Series(dtype=float)),errors="coerce")
                            fscore=pd.to_numeric(future.get("draft_score",pd.Series(dtype=float)),errors="coerce")

                            if pd.isna(amd): band="missing"
                            elif amd<=0: band="<=0"
                            elif amd<=3: band="1-3"
                            elif amd<=6: band="4-6"
                            elif amd<=10: band="7-10"
                            else: band=">10"

                            rows.append({
                                "Draft_slot":ds,"Room":room_name,"Simulation":j+1,"Seed":seed,
                                "Champion_player":champ,"Champion_position":champpos,
                                "Best_alt_player":alt,"Best_alt_position":altpos,
                                "VORP_gap":vg,"Draft_score_gap":sg,"Alt_market_distance":amd,
                                "Market_distance_band":band,"Alt_dominates":dom,"Alt_dominates_le10":le10,
                                ...
                                "R7_9_avg_VORP":fvorp.mean(),"R7_9_avg_score":fscore.mean(),
                                "R7_9_position_sequence":" > ".join(future["position"].astype(str).tolist()),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done%200==0 or done==total:
                            status.caption(f"V8.08: {done}/{total} fresh champion drafts")

            df=pd.DataFrame(rows)

            # Identity concentration is learned from this measurement sample, not hard-coded.
            sig=df[df.Alt_dominates_le10].copy()
            if len(sig):
                freq=sig.Best_alt_player.value_counts()
                top_name=str(freq.index[0]); top_n=int(freq.iloc[0]); top_share=top_n/len(sig)
                sig_names=set(freq[freq/len(sig)>=0.20].index.astype(str).tolist())
            else:
                top_name=""; top_n=0; top_share=np.nan; sig_names=set()

            df["Dominant_alt_identity"]=df.Best_alt_player.astype(str).isin(sig_names)
            st.session_state.v808_results=df
            status.success(f"V8.08 complete: {len(df):,} exact-board Round-6 decisions audited.")
            prog.empty()

        df=st.session_state.get("v808_results")
        if isinstance(df,pd.DataFrame) and len(df):
            sig=df[df.Alt_dominates_le10].copy()
            freq=sig.Best_alt_player.value_counts() if len(sig) else pd.Series(dtype=int)
            top_name=str(freq.index[0]) if len(freq) else ""
            top_n=int(freq.iloc[0]) if len(freq) else 0
            top_share=top_n/len(sig) if len(sig) else np.nan
            dominant_names=set(freq[freq/len(sig)>=0.20].index.astype(str).tolist()) if len(sig) else set()
            generic=sig[~sig.Best_alt_player.astype(str).isin(dominant_names)].copy()

            summary=pd.DataFrame([{
                "R6 decisions":len(df),"Exact-board signals":len(sig),
                "Signal rate":len(sig)/len(df) if len(df) else np.nan,
                "Top alternative":top_name,"Top alternative N":top_n,"Top alternative share":top_share,
                "Dominant identities >=20%":", ".join(sorted(dominant_names)) if dominant_names else "None",
                "Signals after removing dominant identities":len(generic),
                "Generic share":len(generic)/len(sig) if len(sig) else np.nan,
                "Generic rooms":generic.Room.nunique() if len(generic) else 0,
                "Generic slots":generic.Draft_slot.nunique() if len(generic) else 0,
            }])
            st.markdown("#### Identity-concentration test")
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            identities=sig.groupby(["Best_alt_player","Best_alt_position"],as_index=False).agg(
                Signals=("Champion_player","count"),Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),Avg_market_distance=("Alt_market_distance","mean"),
                Avg_R7_9_VORP=("R7_9_avg_VORP","mean")
            ).sort_values("Signals",ascending=False)
            identities["Signal_share"]=identities.Signals/len(sig) if len(sig) else np.nan
            st.markdown("#### Alternative-player concentration")
            st.dataframe(identities.head(30).round(4),use_container_width=True,hide_index=True)

            paths=sig.groupby(["Champion_position","Best_alt_position"],as_index=False).agg(
                Signals=("Champion_player","count"),Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),Avg_market_distance=("Alt_market_distance","mean"),
                Avg_R7_9_VORP=("R7_9_avg_VORP","mean"),Avg_R7_9_score=("R7_9_avg_score","mean")
            ).sort_values("Signals",ascending=False)
            st.markdown("#### All-signal position mechanism")
            st.dataframe(paths.round(4),use_container_width=True,hide_index=True)

            gpaths=generic.groupby(["Champion_position","Best_alt_position"],as_index=False).agg(
                Signals=("Champion_player","count"),Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),Avg_market_distance=("Alt_market_distance","mean"),
                Avg_R7_9_VORP=("R7_9_avg_VORP","mean")
            ).sort_values("Signals",ascending=False) if len(generic) else pd.DataFrame()
            st.markdown("#### Mechanism after removing dominant identities")
            st.dataframe(gpaths.round(4),use_container_width=True,hide_index=True)

            room=generic.groupby("Room",as_index=False).agg(
                Signals=("Champion_player","count"),Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean")) if len(generic) else pd.DataFrame()
            slot=generic.groupby("Draft_slot",as_index=False).agg(
                Signals=("Champion_player","count"),Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean")) if len(generic) else pd.DataFrame()
            st.markdown("#### Generic residual by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Generic residual by slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            bands=generic.groupby("Market_distance_band",as_index=False).agg(
                Signals=("Champion_player","count"),Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),Avg_R7_9_VORP=("R7_9_avg_VORP","mean")
            ) if len(generic) else pd.DataFrame()
            st.markdown("#### Generic residual market-distance bands")
            st.dataframe(bands.round(4),use_container_width=True,hide_index=True)

            roster=generic.groupby(
                ["QB_count_pre","RB_count_pre","WR_count_pre","TE_count_pre","Champion_position","Best_alt_position"],
                as_index=False
            ).agg(
                Signals=("Champion_player","count"),Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),Avg_market_distance=("Alt_market_distance","mean"),
            ...
            st.markdown("#### Generic residual roster-state signatures")
            st.dataframe(roster.head(50).round(4),use_container_width=True,hide_index=True)

            st.info(
                "V8.08 is measurement-only. Dominant player identities are identified mechanically at >=20% of the R6 signal, "
                "then removed to test genericity. Do not create or tune an R6 production rule from this sample."
            )

            st.download_button(
                "⬇️ Download V8.08 R6 mechanism audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_08_r6_generic_mechanism_audit.csv",
                "text/csv",key="v808_download"
            )
