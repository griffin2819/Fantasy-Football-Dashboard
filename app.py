...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None,
                       ...
                       r2_generic_adaptation_enabled=False):
...
st.title("🏈 Fantasy Edge V7.92 — Frozen Round-2 Generic Adaptation Validation")
st.caption("V7.92 • V7.67 frozen champion • Round-2 independent validation")
st.caption("Build ID: V7.92-R2-GENERIC-VALIDATION-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### ⚡ V7.92 — Frozen Round-2 Generic Adaptation Validation")
        st.caption(
            "Independent fresh-seed validation of the exact V7.91 rule with zero tuning. "
            "V7.67 remains frozen/live. The ≤10 market-distance threshold, score+VORP requirements, and all other logic are unchanged. "
            "Trigger-first execution reruns the challenger only on actual Round-2 triggers."
        )

        if "v792_results" not in st.session_state:
            st.session_state.v792_results=None

        sims=st.number_input(
            "V7.92 validation simulations per slot × room",
            min_value=20,max_value=80,value=40,step=10,key="v792_sims",
            help="Default: all 12 slots × all 5 rooms × 40 = 2,400 fresh matched sets. Trigger-first execution reduces simulation work."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.92 frozen validation",type="primary",key="v792_run")
        clear=b.button("Clear V7.92 validation",key="v792_clear")
        if clear:
            st.session_state.v792_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=992000+int(ds)*100000+j*353+sum(ord(c) for c in room_name)
                        ...
                        rr=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(2)]
                        trigger=False; normal=""; alt=""; altpos=""
                        if len(rr):
                            r=rr.iloc[0]
                            trigger=bool(int(pd.to_numeric(
                                pd.Series([r.get("R791_Would_trigger",0)]),
                                errors="coerce"
                            ).fillna(0).iloc[0]))
                            normal=str(r.get("R791_Normal_player",""))
                            alt=str(r.get("R791_Alt_player",""))
                            altpos=str(r.get("R791_Alt_position",""))
                        ...
                        rows.append({
                            ...
                            "Normal_R2_player":normal,"Alternative_R2_player":alt,"Alternative_position":altpos,
                        ...
                        prog.progress(done/total)
                        if done==1 or done%300==0 or done==total:
                            status.caption(f"V7.92: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v792_results=df
            status.success(
                f"V7.92 complete: {len(df)} matched sets using "
                f"{len(df)+int(df.Challenger_rerun.sum())} full simulations instead of {len(df)*2}."
            )
            prog.empty()

        df=st.session_state.get("v792_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.markdown("#### Validation by room")
            ...
            paths=trig.groupby(
                ["Normal_R2_player","Alternative_R2_player","Alternative_position"],
                as_index=False
            ).agg(
            ...
            st.markdown("#### Frozen Round-2 paths")
            ...
            certified=(
                ...
                and r11>-0.15
                and activation>=.20
                and pd.notna(cond) and cond>=.58
                and pd.notna(nonloss) and nonloss>=.68
                and int((active_rooms.Overall_change>0).sum())>=4
                and int((active_slots.Overall_change>=0).sum())>=11
            )

            gate=pd.DataFrame([{
                "Validation passed":certified,
                "Overall > 0":overall>0,
                "R1-3 >= 0":r13>=0,
                "R4-6 >= 0":r46>=0,
                "R7-10 >= 0":r710>=0,
                "R11+ > -0.15":r11>-0.15,
                "Activation >=20%":activation>=.20,
                "Conditional win >=58%":pd.notna(cond) and cond>=.58,
                "Conditional non-loss >=68%":pd.notna(nonloss) and nonloss>=.68,
                "Positive rooms":int((active_rooms.Overall_change>0).sum()),
                "Non-negative active slots":int((active_slots.Overall_change>=0).sum()),
                "Active slots":len(active_slots),
            }])
            st.markdown("#### V7.92 frozen validation gate")
            st.dataframe(gate,use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.92 VALIDATION PASSED. The exact frozen ≤10 Round-2 adaptation rule reproduces on independent fresh seeds. "
                    "V7.67 remains champion until a separate production-path promotion test."
                )
            else:
                st.warning(
                    "V7.92 validation failed. Keep V7.67 champion and do not tune the frozen ≤10 rule from this sample."
                )

            st.download_button(
                "⬇️ Download V7.92 frozen Round-2 adaptation validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_92_frozen_round2_generic_adaptation_validation.csv",
                "text/csv",key="v792_download"
            )
