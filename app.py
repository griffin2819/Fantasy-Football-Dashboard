...
st.title("🏈 Fantasy Edge V8.02 — Round-3 QB Opportunity-Cost Challenger")
st.caption("V8.02 • V7.93 frozen champion • R3 QB opportunity-cost challenger")
st.caption("Build ID: V8.02-R3-QB-OPPORTUNITY-CHALLENGER-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🧪 V8.02 — Round-3 QB Opportunity-Cost Challenger")
        st.caption(
            "Full-draft matched challenger against frozen V7.93. "
            "Pre-registered generic rule: Round 3 only, normal champion choice must be QB, and the best genuinely alive+eligible non-QB alternative "
            "must have BOTH higher frozen draft score and higher VORP while remaining within ≤10 effective market picks. "
            "No Drake Maye, Josh Allen, Cam Skattebo, RB, room, or draft-slot hard-coding."
        )

        if "v802_results" not in st.session_state:
            st.session_state.v802_results=None

        sims=st.number_input(
            "V8.02 matched simulations per slot × room",
            min_value=10,max_value=40,value=20,step=5,key="v802_sims",
            help="Default: 12 slots × 5 rooms × 20 = 1,200 fresh matched sets. Trigger-first reruns only actual R3 QB opportunity-cost events."
        )
        a,b=st.columns(2)
        run=a.button("Run V8.02 R3 QB challenger",type="primary",key="v802_run")
        clear=b.button("Clear V8.02 challenger",key="v802_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8020000+int(ds)*100000+j*409+sum(ord(c) for c in room_name)
                        ...
                        rows.append({
                            ...
                            "Normal_R3_QB":normal,"Alternative_R3_player":alt,"Alternative_position":altpos,
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V8.02: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v802_results=df
            status.success(f"V8.02 complete: {len(df)} matched sets; {int(df.Challenger_rerun.sum())} challenger reruns.")
            prog.empty()

        df=st.session_state.get("v802_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.markdown("#### Challenger by all 12 draft slots")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            paths=trig.groupby(
                ["Normal_R3_QB","Alternative_R3_player","Alternative_position"],
                as_index=False
            ).agg(
            ...
            st.markdown("#### Activated R3 QB opportunity-cost paths")
            ...
            screen=(
                ...
                and int((active_slots.Overall_change>=0).sum())>=10
            )

            if screen:
                st.success(
                    "V8.02 SCREEN PASSED. Freeze this exact generic R3 QB opportunity-cost rule and advance it to independent fresh-seed validation. V7.93 remains champion until validation."
                )
            else:
                st.warning(
                    "V8.02 does not clear screening. Keep V7.93 champion and do not tune the R3 QB rule from this sample."
                )

            st.download_button(
                "⬇️ Download V8.02 R3 QB opportunity-cost challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_02_round3_qb_opportunity_cost_challenger.csv",
                "text/csv",key="v802_download"
            )
