...
def _v738_emergency_drop_rank(drop_row, roster_df, starter_names, emergency_pos):
    ...
    if isinstance(roster_df,pd.DataFrame) and len(roster_df):
        ...
        if remaining < floor:
            ...
            if pos in ("DL","DB","QB"):
                score -= min(18.0,9.0+4.0*surplus)
            elif pos in ("RB","WR"):
                score -= min(9.0,3.0+2.0*surplus)
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        st.caption("V7.41 waiver engine: projections remain the anchor, but close candidates are now tie-broken by your roster objective, direct injury-replacement role, opportunity confidence and transaction impact. This contextual score does not alter frozen projection/VORP values.")
        ...
        if _moves_df.empty:
            ...
            if "Emergency Coverage" in _moves_df.columns and _moves_df["Emergency Coverage"].fillna(False).any():
                _moves_df=_moves_df.sort_values(["Emergency Drop Rank","Decision Score","Priority Score","Net Week","Net ROS"],ascending=[True,False,False,False,False])
            else:
            ...
            if _best.empty:
                ...
                if "Emergency Coverage" in _best.columns and _best["Emergency Coverage"].fillna(False).any():
                    _best=_best.sort_values(["Emergency Drop Rank","Decision Score","Priority Score","Net Week"],ascending=[True,False,False,False])
                else:
                ...
                st.dataframe(_best[["Add","Pos","Drop","Drop Pos","Decision Score","Context Fit","Decision Why","Drop Cost","Emergency Drop Rank","Drop Context","Priority Score","Waiver Call","FAAB","Claim Advice","Move Type","Need Fit","Evidence","Evidence Detail","Opportunity Boost","Effective Add PPG","Opportunity","Opportunity Confidence","Why Not Priority","Net Week","Net ROS","Add Role","Add Recent PPG","Add Status","Add Practice","Add Injury","Add Matchup","Drop Protection","Drop Status","Drop Practice","Drop Role","Drop Recent PPG","Add PPG","Drop PPG","Add ROS","Drop ROS","Add VORP","Drop VORP","Why"]].head(40),
                             use_container_width=True,hide_index=True)
