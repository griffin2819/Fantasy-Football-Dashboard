...
def _market_pick_series_legacy_v750(df, teams):
    """Historical V7.50 IDP market transform; comparison only."""
...
def _fast_benchmark_pool(board, teams=12, idp_market_integrity_fix=True):
    ...
    market_pick=(_market_pick_series(pool,teams) if bool(idp_market_integrity_fix) else _market_pick_series_legacy_v750(pool,teams)).to_numpy(dtype=float)
...
st.title("🏈 Fantasy Edge V7.59 — Promoted IDP Market-Integrity Champion")
st.caption("V7.59 • promoted IDP market-integrity champion")
st.caption("Build ID: V7.59-IDPMARKET-CHAMPION-20260816")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v759_live_slot")
    ...
    if "v759_target_queue" not in st.session_state:
        st.session_state.v759_target_queue={}
    queue=st.session_state.v759_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v759_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.59 is the live/default champion; the 55.97 Pick-12 rule remains unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.59 robustness report")
        ...
        st.markdown("### 🧪 V7.59 champion baseline validation")
        st.info("V7.59 promotes the certified IDP market-pick fix into the default engine; V7.50 is retained only for historical comparison.")
        ...
        if "v759_independent_results" not in st.session_state:
            st.session_state.v759_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v759_independent_results"),pd.DataFrame) else "ready to run"))
        if "v759_independent_summary" not in st.session_state:
            st.session_state.v759_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v759_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v759_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v759_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v759_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v759_independent_results=iv
                st.session_state.v759_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v759_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v759_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.59 Champion Baseline Check")
            st.caption("V7.59 promotes the V7.58-certified IDP market-pick integrity correction into the live/default champion path with no additional tuning.")
            ...
            st.download_button(
                "⬇️ Download V7.59 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_59_champion_baseline.csv",
                "text/csv",
                key="v759_baseline_download"
            )
        ...
        st.markdown("### 👑 V7.59 — Promoted Champion Certification")
        st.caption(
            "The V7.58-certified IDP market-integrity correction is now the live/default production path. "
            "Historical V7.50 is retained only for matched certification. No additional tuning was introduced."
        )
        if "v759_results" not in st.session_state: st.session_state.v759_results=None
        sims=st.number_input(
            "Champion certification simulations per slot × room",
            min_value=20,max_value=100,value=50,step=10,key="v759_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh matched V7.59-vs-V7.50 drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.59 champion certification",type="primary",key="v759_run")
        clear=b.button("Clear V7.59 certification",key="v759_clear")
        if clear:
            st.session_state.v759_results=None
            st.rerun()
        ...
        if run:
            ...
            old_pool=_fast_benchmark_pool(board,int(teams),idp_market_integrity_fix=False)
            new_pool=_fast_benchmark_pool(board,int(teams),idp_market_integrity_fix=True)
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=959000+int(ds)*100000+j*157+sum(ord(c) for c in room_name)
                        oldd=simulate_mock_fast(old_pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name)
                        newd=simulate_mock_fast(new_pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name)
                        oc=independent_draft_grade_components(oldd,int(teams))
                        nc=independent_draft_grade_components(newd,int(teams))
                        op=independent_phase_metrics(oldd).set_index("Phase")
                        np_=independent_phase_metrics(newd).set_index("Phase")
                        oldplayers=oldd.set_index("mock_pick")["player"].to_dict()
                        changed=sum(oldplayers.get(rr.mock_pick,None)!=rr.player for _,rr in newd.iterrows())
                        ...
                        overall=nc["overall"]-oc["overall"]
                        r11=float(np_.loc["R11+","Avg_market_delta"])-float(op.loc["R11+","Avg_market_delta"])
                        rows.append({
                            "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Overall_change":overall,
                            "R1_3_change":float(np_.loc["R1-3","Avg_market_delta"])-float(op.loc["R1-3","Avg_market_delta"]),
                            ...
                            "R11_plus_change":r11,
                            "Late_reach_change":_late_reach(newd)-_late_reach(oldd),
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.59 certification: {done}/{total}")

            df=pd.DataFrame(rows)
            st.session_state.v759_results=df
            status.success(f"V7.59 certification complete: {len(df)} matched drafts.")
            prog.empty()

        df=st.session_state.get("v759_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ch=df[df.Behavior_changed].copy()
            overall=df.Overall_change.mean(); r13=df.R1_3_change.mean()
            r46=df.R4_6_change.mean(); r710=df.R7_10_change.mean()
            r11=df.R11_plus_change.mean(); reach=df.Late_reach_change.mean()
            cond=ch.Conditional_overall_win.mean() if len(ch) else np.nan
            nonloss=ch.Conditional_nonloss.mean() if len(ch) else np.nan
            cond11=ch.Conditional_R11_win.mean() if len(ch) else np.nan

            summary=pd.DataFrame([{
                "Matched drafts":len(df),"Changed drafts":len(ch),
                "Overall vs V7.50":overall,"R1-3 change":r13,"R4-6 change":r46,
                "R7-10 change":r710,"R11+ change":r11,"Late reach change":reach,
                "Conditional overall win":cond,"Conditional non-loss":nonloss,
            ...
            room=df.groupby("Room",as_index=False).agg(
                Overall_change=("Overall_change","mean"),R11_plus_change=("R11_plus_change","mean"),
                Late_reach_change=("Late_reach_change","mean"))
            st.markdown("#### V7.59 by draft slot"); st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### V7.59 by room"); st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            certified=(
                overall>0 and r11>0 and reach<0 and min(r13,r46,r710)>-.05
                and pd.notna(cond) and cond>=.70 and pd.notna(nonloss) and nonloss>=.85
                and pd.notna(cond11) and cond11>=.75
                and bool((slot.Overall_change>0).all()) and bool((slot.R11_plus_change>0).all())
                and bool((room.Overall_change>0).all()) and bool((room.R11_plus_change>0).all())
            )
            if certified:
                st.success("V7.59 CHAMPION CERTIFIED. The promoted production path reproduces the finalist advantage.")
            else:
                st.error("V7.59 certification failed. Revert to the historical V7.50 market transform until diagnosed.")

            st.download_button(
                "⬇️ Download V7.59 champion certification CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_59_champion_certification.csv","text/csv",key="v759_download")
