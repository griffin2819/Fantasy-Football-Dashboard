...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Status Lifecycle Cleanup", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v680_injury_watch_feed():
    ...
    def _freshness(rec):
        ...
        # ESPN injury endpoint / Sleeper are live endpoints.  Retrieval time proves the
        # endpoint response is current, but severe designations still need corroboration
        # when the item itself has no usable update timestamp.
        d=_dt(rec.get("updated",""))
        f=_dt(rec.get("fetched_at",""))
        if d is not None:
            age=max(0.0,(now-d).total_seconds()/3600.0)
            if age <= 24*7:
                return True,f"updated {age:.1f}h ago",age
            return False,f"item update {age/24:.1f}d old",age
        if f is not None:
            age=max(0.0,(now-f).total_seconds()/3600.0)
            if age <= 6:
                return True,"live endpoint; item timestamp unavailable",age
        return False,"stale/unknown timestamp",None
    ...
    for key in keys:
        ...
        def _stamp(rec):
            d=_dt(rec.get("updated","")) or _dt(rec.get("fetched_at",""))
        ...
        if status in severe and not chosen_has_item_time and corroboration < 2:
            # nflverse current-week rows are allowed because date_modified was required
            # above.  This branch mainly protects undated Sleeper/ESPN severe labels.
            verified=False
            verify_reason="severe designation has no current item timestamp or second-source confirmation"
        if not fresh:
            verified=False
            verify_reason="no fresh source record"
        ...
        _severe_undated = bool(status in severe and fresh and not chosen_has_item_time and corroboration < 2)
        if _v762_expire_unverified_status(status, bool(fresh), conflict=bool(conflict), severe_undated=_severe_undated):
            status="ACTIVE"
            verified=True
            conflict=False
        ...
        base["evidence_sources"]=" + ".join(dict.fromkeys(str(r.get("source","") or "") for r in fresh if str(r.get("source","") or "")))
