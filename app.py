...
st.title("🏈 Fantasy Edge V7.36 — Pick 7 Downstream Recovery Audit")
st.caption("V7.36 • V7.24 champion frozen • Pick 7 downstream recovery audit")
st.caption("Build ID: V7.36-P7DRA-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v736_live_slot")
    ...
    if "v736_target_queue" not in st.session_state:
        st.session_state.v736_target_queue={}
    queue=st.session_state.v736_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v736_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.36 keeps the V7.24 9× champion frozen. The 8/35 intercept runs only inside the downstream recovery audit.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.36 robustness report")
        ...
        st.markdown("### 🧪 V7.36 champion baseline validation")
        st.info("V7.36 is diagnostic only. It traces the R7-10 selections changed after the Pick 7 R4-6 8/35 WR intercept.")
        ...
        if "v736_independent_results" not in st.session_state:
            st.session_state.v736_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v736_independent_results"),pd.DataFrame) else "ready to run"))
        if "v736_independent_summary" not in st.session_state:
            st.session_state.v736_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v736_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v736_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v736_run")
        clear_iv=clear_col.button("Clear independent results",key="v736_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v736_independent_results=iv
                st.session_state.v736_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v736_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v736_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.36 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.36 audits the R7-10 path changes caused by the Pick 7 8/35 intercept.")
            ...
            st.download_button(
                "⬇️ Download V7.36 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_36_champion_baseline.csv",
                "text/csv",
                key="v736_baseline_download"
            )
        ...
        st.markdown("### 🔬 V7.36 — Pick 7 Intercept + Downstream Recovery Audit")
        st.caption("Diagnostic only. V7.24 remains frozen. This traces the R7-10 selections that change after the successful Pick-7 R4-6 WR 8/35 intercept.")

        if "v736_recovery_results" not in st.session_state: st.session_state.v736_recovery_results=None
        if "v736_recovery_trace" not in st.session_state: st.session_state.v736_recovery_trace=None
        if "v736_recovery_meta" not in st.session_state: st.session_state.v736_recovery_meta=None

        audit_sims=st.number_input("Recovery-audit simulations per room",min_value=3,max_value=25,value=10,step=1,key="v736_audit_sims")
        aa,ab=st.columns(2)
        run_audit=aa.button("Run downstream recovery audit",type="primary",key="v736_audit_run")
        clear_audit=ab.button("Clear recovery audit",key="v736_audit_clear")

        if clear_audit:
            st.session_state.v736_recovery_results=None
            st.session_state.v736_recovery_trace=None
            st.session_state.v736_recovery_meta=None
            st.rerun()
        ...
        if run_audit:
            ...
            rows=[]; traces=[]
            total=len(rooms)*int(audit_sims)*2; done=0
            prog=st.progress(0); status=st.empty(); started=time.perf_counter()
            for room_name in rooms:
                for j in range(int(audit_sims)):
                    seed=936000+j*47+sum(ord(c) for c in room_name)
                    for config,enabled in [("CHAMPION",False),("PICK7_835",True)]:
                        fe,tr=simulate_mock_fast_pick7_recovery_trace(
                            fast_champ,int(teams),7,15,state["roster_slots"],6.0,seed,room_name,enabled
                        )
                        fc=independent_draft_grade_components(fe,int(teams))
                        ...
                        rows.append({
                            "Record_type":"draft_summary","Config":config,"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Overall_grade":fc["overall"],"Market_value":fc["market_value"],
                            "Roster_balance":fc["roster_balance"],"Starter_completion":fc["starter_completion"],
                            "Availability":fc["availability"],"R4_6_market":float(ph.loc["R4-6","Avg_market_delta"]),
                            "R7_10_market":float(ph.loc["R7-10","Avg_market_delta"]),
                            "R11_plus_market":float(ph.loc["R11+","Avg_market_delta"]),
                            "R4_6_QB":int((r46.position=="QB").sum()),"R4_6_RB":int((r46.position=="RB").sum()),
                            "R4_6_WR":int((r46.position=="WR").sum()),"R4_6_TE":int((r46.position=="TE").sum()),
                            "R7_10_QB":int((r710.position=="QB").sum()),"R7_10_RB":int((r710.position=="RB").sum()),
                            "R7_10_WR":int((r710.position=="WR").sum()),"R7_10_TE":int((r710.position=="TE").sum())
                        })
                        tr=tr.copy(); tr["Record_type"]="pick_trace"; tr["Config"]=config
                        tr["Room"]=room_name; tr["Simulation"]=j+1; tr["Seed"]=seed
                        traces.append(tr)
                        done+=1; prog.progress(done/total)
                        if done==1 or done%20==0 or done==total: status.caption(f"Recovery audit: {done}/{total}")
            res=pd.DataFrame(rows)
            trace=pd.concat(traces,ignore_index=True) if traces else pd.DataFrame()
            st.session_state.v736_recovery_results=res
            st.session_state.v736_recovery_trace=trace
            st.session_state.v736_recovery_meta={"seconds":time.perf_counter()-started,"sims":int(audit_sims)}
            status.success(f"Recovery audit complete: {len(res)} model drafts."); prog.empty()

        res=st.session_state.get("v736_recovery_results")
        trace=st.session_state.get("v736_recovery_trace")
        meta=st.session_state.get("v736_recovery_meta") or {}
        if isinstance(res,pd.DataFrame) and len(res):
            piv=res.pivot_table(index=["Room","Simulation","Seed"],columns="Config",
                values=["Overall_grade","Market_value","Roster_balance","R4_6_market","R7_10_market","R11_plus_market",
                        "R7_10_QB","R7_10_RB","R7_10_WR","R7_10_TE"],aggfunc="first")
            metric_rows=[]
            for metric in ["Overall_grade","Market_value","Roster_balance","R4_6_market","R7_10_market","R11_plus_market",
                           "R7_10_QB","R7_10_RB","R7_10_WR","R7_10_TE"]:
                d=piv[(metric,"PICK7_835")]-piv[(metric,"CHAMPION")]
                metric_rows.append({"Metric":metric,"Candidate change":d.mean(),"Better %":(d>0).mean(),"Equal %":(d==0).mean()})
            st.markdown("#### Matched downstream changes")
            st.dataframe(pd.DataFrame(metric_rows).round(3),use_container_width=True,hide_index=True)

            room_rows=[]
            for room_name,g in res.groupby("Room"):
                p=g.pivot(index=["Simulation","Seed"],columns="Config",values=["Overall_grade","R4_6_market","R7_10_market","Roster_balance"])
                room_rows.append({
                    "Room":room_name,
                    "Overall change":(p[("Overall_grade","PICK7_835")]-p[("Overall_grade","CHAMPION")]).mean(),
                    "R4-6 change":(p[("R4_6_market","PICK7_835")]-p[("R4_6_market","CHAMPION")]).mean(),
                    "R7-10 change":(p[("R7_10_market","PICK7_835")]-p[("R7_10_market","CHAMPION")]).mean(),
                    "Roster change":(p[("Roster_balance","PICK7_835")]-p[("Roster_balance","CHAMPION")]).mean()
                })
            st.markdown("#### Recovery cost by room")
            st.dataframe(pd.DataFrame(room_rows).round(3),use_container_width=True,hide_index=True)

            if isinstance(trace,pd.DataFrame) and len(trace):
                t=trace[pd.to_numeric(trace.mock_round,errors="coerce").fillna(99).between(7,10)].copy()
                keys=["Room","Simulation","Seed","mock_pick"]
                a=t[t.Config=="CHAMPION"].set_index(keys)
                b=t[t.Config=="PICK7_835"].set_index(keys)
                diffs=[]
                for idx in a.index.intersection(b.index):
                    x=a.loc[idx]; y=b.loc[idx]
                    if isinstance(x,pd.DataFrame): x=x.iloc[0]
                    if isinstance(y,pd.DataFrame): y=y.iloc[0]
                    if x.get("player","")!=y.get("player",""):
                        diffs.append({
                            "Room":idx[0],"Simulation":idx[1],"Seed":idx[2],"Pick":idx[3],
                            "Champion_player":x.get("player",""),"Champion_pos":x.get("position",""),
                            "Candidate_player":y.get("player",""),"Candidate_pos":y.get("position",""),
                            "Champion_market_delta":x.get("market_delta",np.nan),
                            "Candidate_market_delta":y.get("market_delta",np.nan)
                        })
                diff=pd.DataFrame(diffs)
                st.markdown("#### R7-10 selections changed by the intercept")
                if len(diff):
                    st.dataframe(diff.head(100).round(3),use_container_width=True,hide_index=True)
                    trans=diff.groupby(["Champion_pos","Candidate_pos"],as_index=False).agg(
                        Changed_picks=("Pick","count"),Champion_market=("Champion_market_delta","mean"),
                        Candidate_market=("Candidate_market_delta","mean"))
                    trans["Market_delta_change"]=trans.Candidate_market-trans.Champion_market
                    st.markdown("#### Position-path transitions")
                    st.dataframe(trans.sort_values("Changed_picks",ascending=False).round(3),use_container_width=True,hide_index=True)
                    first=diff.sort_values("Pick").groupby(["Room","Simulation","Seed"],as_index=False).first()
                    st.markdown("#### First downstream divergence")
                    st.dataframe(first.head(100).round(3),use_container_width=True,hide_index=True)
                else:
                    st.success("No R7-10 player selections changed.")

            export=pd.concat([res,trace],ignore_index=True,sort=False) if isinstance(trace,pd.DataFrame) and len(trace) else res
            st.download_button("⬇️ Download V7.36 downstream recovery audit CSV",
                export.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_36_pick7_downstream_recovery_audit.csv","text/csv",key="v736_download")
