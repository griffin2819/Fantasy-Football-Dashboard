...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Availability Alternative Gate", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v781_render_page(_v777_page_idx):
    ...
    if _v777_page_idx==6:
        ...
        if _my.empty:
            ...
            st.caption(f"Fantasy Edge V7.86 • {_v677_season} Week {_v677_week} • Decision-quality Start/Sit with authoritative availability, uncertainty-aware close calls, and separate unavailable/monitor groups.")
    ...
    if _v777_page_idx==13:
        ...
        if my.empty or other.empty:
            ...
            # Cache only the small rendered trade result. Action Queue consumes this
            # snapshot instead of re-running the combinatorial trade search.
            try:
    ...
    if _v777_page_idx==14:
        ...
        if my.empty:
            ...
            _v780_snap_key=f"7.86|{_v780_base_key}|{_v780_avail_sig}"
...
def _v781_navigation_fragment():
    ...
    st.caption(f"⚡ Page render: {time.perf_counter()-_t:.2f}s • fragment navigation")
