...
def _fast_benchmark_pool(board):
    ...
    return {"pool":pool,"pos_names":pos_names,"pos_code":pos_code,"consensus":consensus,
            "model_rank":model_rank,"draft_score":draft_score}


def _fast_need_by_position(counts, slots):
    """Exact roster_need_for_mock logic, evaluated once per user pick for six positions."""
    # QB,RB,WR,TE,DL,DB
    qbi,rbi,wri,tei,dli,dbi=0,1,2,3,4,5
    out=np.full(6,-2.0,dtype=float)
    flex_used=max(0,(counts[rbi]+counts[wri]+counts[tei])-
                    (int(slots.get("RB",0))+int(slots.get("WR",0))+int(slots.get("TE",0))))
    flex_need=max(int(slots.get("FLEX",0))-flex_used,0)
    for code,pos in enumerate(["QB","RB","WR","TE","DL","DB"]):
        have=int(counts[code]); direct=max(int(slots.get(pos,0))-have,0)
        if direct>0:
            bonus=9.0+min(direct,2)*1.8
            if pos=="WR": bonus+=2.5
            out[code]=bonus; continue
        if pos=="WR": out[code]=6.0 if flex_need>0 else 2.5 if have<4 else -1.5
        elif pos=="RB": out[code]=4.0 if flex_need>0 else 1.5 if have<4 else -2.0
        elif pos=="TE": out[code]=-7.0 if have>=1 else 0.0
        elif pos=="QB": out[code]=-9.0 if have>=1 else 0.0
        elif pos in ["DL","DB"]: out[code]=-10.0 if have>=int(slots.get(pos,1)) else -2.0
    return out
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None):
    ...
    n=len(pool)
    fallback=model_rank+rng.normal(0,8,n)
    sim_cons=np.where(np.isnan(consensus),fallback,consensus)
    alive=np.ones(n,dtype=bool)
    counts=np.zeros(6,dtype=np.int16)
    user_idx=[]; user_pick=[]; user_round=[]
    total=int(teams)*int(rounds)
    late_gate=max(10,int(rounds)-4)
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            # draft_eligibility hard guardrails.
            if int(slots.get("QB",1))<=1:
                if counts[qb]>=2 or (counts[qb]>=1 and round_no<late_gate): eligible &= pos[idx]!=qb
            if int(slots.get("TE",1))<=1:
                if counts[te]>=2 or (counts[te]>=1 and round_no<late_gate): eligible &= pos[idx]!=te
            ...
            need=_fast_need_by_position(counts,slots)
            construction=np.zeros(6,dtype=float)
            desired_wr=max(3,int(slots.get("WR",2))+int(slots.get("FLEX",1)))
            desired_rb=max(3,int(slots.get("RB",2))+1)
            if 5<=round_no<=10:
                if counts[wr]<desired_wr: construction[wr]+=12
                if counts[rb]<desired_rb: construction[rb]+=7

            if model_user:
                cr=np.where(np.isnan(consensus[eidx]),float(overall),consensus[eidx])
                score=draft_score[eidx]+need[pos[eidx]]+construction[pos[eidx]]-np.maximum(overall-cr,0)*.08
                if round_no<=7:
                    score-=np.where(np.isin(pos[eidx],[dl,db]),(8-round_no)*2.2,0)
                if counts[qb]>=1: score-=np.where(pos[eidx]==qb,10 if round_no<=10 else 5,0)
                if counts[te]>=1: score-=np.where(pos[eidx]==te,8 if round_no<=10 else 3,0)
                chosen=int(eidx[int(np.argmax(score))])
            else:
                noise=rng.normal(0,float(randomness)/3,eidx.size)
                score=sim_cons[eidx]+noise-need[pos[eidx]]*.25
                chosen=int(eidx[int(np.argmin(score))])
            ...
            if cidx.size==0:
                k=min(30,idx.size)
                part=np.argpartition(base,k-1)[:k] if k<idx.size else np.arange(idx.size)
                cidx=idx[part]
            sd=np.clip(4+sim_cons[cidx]/35,4,10)
...
st.title("🏈 Fantasy Edge V6.5 — Performance Engine")
st.caption("V7 Diagnostics • frozen V6.3 model & grading • pick-level benchmark diagnostics • Yahoo + IDP")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            fast=_fast_benchmark_pool(board)
            ...
            status.caption(f"Completed {int(sims)}/{int(sims)} simulation pairs in {time.perf_counter()-started:.1f}s")
            ...
            st.markdown("### 🔬 V7 diagnostic report")
            ...
            flag_report=pd.DataFrame({
                "Diagnostic":["3 or fewer WR","6+ RB","2+ QB","2+ TE","IDP in rounds 1-7"],
                "Fantasy Edge frequency":[flags.WR_le_3.mean(),flags.RB_ge_6.mean(),flags.QB2_plus.mean(),flags.TE2_plus.mean(),flags.Early_IDP.mean()]
            })
            ...
            st.download_button("⬇️ Download V7 diagnostic CSV",csv,"fantasy_edge_v7_diagnostics.csv","text/csv")
