...
def board_certification_summary(board, slots):
    """Pre-draft data certification. Fail loudly before bad rows reach the draft engine."""
    if board is None or len(board)==0:
        return pd.DataFrame([{"Check":"Player board loaded","Status":"FAIL","Detail":"Board is empty"}])
    b=board.copy()
    ...
    cr=pd.to_numeric(b.get("consensus_rank",np.nan),errors="coerce")
    kr=pd.to_numeric(b.get("kicker_rank",cr),errors="coerce")
    fa=team.isin(["","FA","FREE AGENT","NONE","NAN","UNK","UNKNOWN"])
    k=pos.eq("K")
    k_verified=k & ~fa & kr.between(1,40,inclusive="both")
    dup=int(b.duplicated(subset=["player","position"],keep=False).sum()) if "player" in b.columns else 0
    rows=[
        {"Check":"Free agents draftable","Status":"PASS" if int(fa.sum())==0 else "WARN","Detail":f"{int(fa.sum())} source rows have no active team; hard-gated from recommendations"},
        {"Check":"Verified kickers","Status":"PASS" if int(k_verified.sum())>=12 else "FAIL","Detail":f"{int(k_verified.sum())} team-attached kickers with current K rank"},
        {"Check":"Kicker free agents","Status":"PASS" if int((k & fa).sum())==0 else "WARN","Detail":f"{int((k & fa).sum())} FA/unsigned K rows found; all hard-gated"},
        {"Check":"Duplicate player-position rows","Status":"PASS" if dup==0 else "WARN","Detail":str(dup)},
        {"Check":"Market coverage offense","Status":"PASS" if float(cr[pos.isin(["QB","RB","WR","TE"])].notna().mean() if pos.isin(["QB","RB","WR","TE"]).any() else 0)>=.80 else "WARN","Detail":f"{100*float(cr[pos.isin(['QB','RB','WR','TE'])].notna().mean() if pos.isin(['QB','RB','WR','TE']).any() else 0):.0f}% ranked"},
        {"Check":"IDP pool","Status":"PASS" if int(pos.isin(["DL","DB"]).sum())>=24 else "WARN","Detail":f"{int(pos.isin(['DL','DB']).sum())} DL/DB source rows"},
    ]
...
# v9.53 pre-draft source certification. This surfaces bad source rows before a mock/live pick.
try:
    ...
    with st.expander("🧪 Pre-draft data certification",expanded=bool(_fail)):
        ...
        if _fail:
            st.error(f"{_fail} certification check(s) failed. Fantasy Edge hard-gates invalid rows from recommendations; refresh the source feed before the real draft.")
        elif _warn:
            st.warning(f"Certification passed with {_warn} warning(s); invalid source rows are excluded from recommendations.")
        else:
