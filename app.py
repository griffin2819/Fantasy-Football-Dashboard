...
def make_board(players, hist, ppr, pass_td, idp, teams):
    rows=[]
    ...
    # Value Over Replacement Player (VORP): prevents 1-DL/1-DB formats from
    # pushing IDPs too high merely because of raw projected points.
    ...
    # Draft recommendation blends VORP, forward projection, progression,
    # regression risk and a modest positional-scarcity component.
    scarcity=b.position.map({"QB":0.2,"RB":1.0,"WR":0.8,"TE":0.5,"DL":0.15,"DB":0.10}).fillna(0)
    b["draft_score"]=b["vorp"]*9+b.projection*1.6+b.progression*.12-b.regression*.10+scarcity
    return b.sort_values("draft_score",ascending=False).reset_index(drop=True)

state=load_state()

st.title("🏈 Fantasy Edge — Yahoo + IDP")
st.caption("Yahoo manual sync • exact league scoring • VORP draft value • offense + DL + DB • real NFL data")
...
hist=nfl_history()
board=make_board(players,hist,ppr,pass_td,idp,teams)
...
with tabs[1]:
    ...
    # Mobile-friendly top recommendations.
    top=x.sort_values("draft_score",ascending=False).head(5).copy()
    if len(top):
        best=top.iloc[0]
        st.success(f"🏆 BEST PICK RIGHT NOW: {best.player} ({best.position}, {best.team})")
        st.caption(f"Projected {best.projection:.1f} PPG • VORP +{best.vorp:.1f} • Breakout {best.breakout:.0%} • Regression risk {best.decline:.0%}")
        ...
        for i,(_,r) in enumerate(top.head(3).iterrows()):
            with cols[i]:
                ...
                why=[]
                if r.breakout>=.65: why.append("breakout upside")
                if r.vorp>=3: why.append("strong positional edge")
                if r.decline<=.35: why.append("low regression risk")
                st.caption(" • ".join(why) if why else "balanced model value")
    ...
    st.dataframe(x[["player","position","raw_position","team","projection","vorp","Breakout","Regression","injury","draft_score"]].head(120),
