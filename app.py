...
def simulate_current_fantasy_edge_once(board, teams, slot, rounds, slots, randomness, seed):
    ...
    for overall in range(1,int(teams)*int(rounds)+1):
        ...
        if int(owner)==int(slot):
            ...
            # Exact same current Fantasy Edge preparation/ranking as interactive/live.
            ranked,nxt=prepare_user_draft_candidates(
                avail.reset_index(drop=True),roster,rnd,overall,
            ...
            choice=pool.nlargest(1,score_col).iloc[0].copy()
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.info(
            ...
            "Opponent picks use a lightweight market/roster model so 1,000 drafts finish much faster."
        )
        target_sims=1000
        batch_size=st.selectbox(
            ...
            index=1,
            help="Results persist in this session. 25 is the recommended fast/stable batch size on Streamlit Cloud."
        )
        ...
        if run:
            ...
            for j in range(this_batch):
                ...
                if (j+1)==1 or (j+1)%10==0 or (j+1)==this_batch:
                ...
                if (time.perf_counter()-started)>18.0 and j+1<this_batch:
                    status.warning(
