...
def roster_need_for_mock(roster_df, pos, slots):
    ...
    direct=max(int(slots.get(pos,0))-int(counts.get(pos,0)),0)
    ...
    if direct>0: return 8.0 + min(direct,2)*1.5
    if pos in ["RB","WR","TE"] and flex_need>0: return 3.0
    if pos in ["DL","DB"]: return -6.0
    if pos in ["QB","TE"]: return -3.0
    return -1.0

def grade_mock(roster, teams, slots):
    if roster.empty:
        return {"grade":"F","score":0,"starter":0,"value":0,"balance":0,"upside":0,"risk":0}
    # Starter strength approximated by best required players plus flex.
    chosen=[]
    leftovers=[]
    ...
    value=float(roster.consensus_edge.fillna(0).clip(-30,30).mean())
    balance=100.0
    for pos in ["QB","RB","WR","TE","DL","DB"]:
        req=int(slots.get(pos,0))
        have=int((roster.position==pos).sum())
        if have<req: balance-=12*(req-have)
    ...
    # 0-100 heuristic draft grade.
    score=np.clip(45 + starter*2.0 + value*.45 + (balance-70)*.20 + upside*.08 - risk*.10,0,100)
    ...
    return {"grade":grade,"score":score,"starter":starter,"value":value,"balance":balance,"upside":upside,"risk":risk}
    ...
    for overall in range(1,total+1):
        ...
        if owner_slot==slot:
            ...
            if model_user:
                ...
                avail["sim_score"]=avail.draft_score+avail["need"]-np.maximum(overall-avail.consensus_rank.fillna(overall),0)*.05
...
st.title("🏈 Fantasy Edge V6 — Yahoo + IDP")
st.caption("Yahoo manual sync • roster-aware draft engine • Mock Draft Lab • consensus benchmark • DL + DB")
