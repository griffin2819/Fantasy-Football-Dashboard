...
st.title("🏈 Fantasy Edge V7.13 — Dynamic Faller Threshold")
st.caption("V7.13 • V7.10 stable base • round-aware Dynamic Faller Intercept • no turn optimizer")
st.caption("Build ID: V7.13-DFT-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v713_live_slot")
    ...
    if "v713_target_queue" not in st.session_state:
        st.session_state.v713_target_queue={}
    queue=st.session_state.v713_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v713_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.13 Faller Intercept thresholds: R1-3 = 14+ picks • R4-6 = 11+ • R7-10 = 8+ • R11+ = 6+, always subject to model compatibility.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.13 robustness report")
            ...
            st.download_button("⬇️ Download V7.13 diagnostic CSV",csv,"fantasy_edge_v7_13_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.13 independent validation")
        ...
        if "v713_independent_results" not in st.session_state:
            st.session_state.v713_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v713_independent_results"),pd.DataFrame) else "ready to run"))
        if "v713_independent_summary" not in st.session_state:
            st.session_state.v713_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v713_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v713_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v713_run")
        clear_iv=clear_col.button("Clear independent results",key="v713_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v713_independent_results=iv
                st.session_state.v713_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v713_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v713_independent_summary") or {}
            st.success(
            ...
            st.download_button(
                "⬇️ Download V7.13 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_13_independent_validation.csv",
                "text/csv",
                key="v713_download"
            )
