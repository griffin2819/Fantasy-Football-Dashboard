    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                # V7.82 measurement-only: capture true-board R11-13 alternatives.
                # Used only when the eventual champion selection is a >30-pick faller.
                if 11<=round_no<=13:
...
st.title("🏈 Fantasy Edge V7.82 — R11–13 Deep-Faller True-Board Audit")
st.caption("V7.82 • V7.67 frozen champion • R11–13 deep-faller diagnostic")
st.caption("Build ID: V7.82-R11-13-DEEP-FALLER-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.82 keeps V7.67 frozen/live and audits R11–13 >30-pick faller selections against the actual true-board alternatives.")
        ...
        st.markdown("### 🔬 V7.82 — R11–13 Deep-Faller True-Board Audit")
        st.caption(
            "Measurement only. V7.67 remains the frozen production champion. "
            "For champion selections in Rounds 11–13 that are >30 picks below effective market, V7.82 captures the actual alive+eligible board state "
            "and compares the selection with the top five genuinely available alternatives on market pick, model rank, projection, VORP, and frozen draft score."
        )

        if "v782_results" not in st.session_state:
            st.session_state.v782_results=None

        sims=st.number_input(
            "V7.82 simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v782_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run R11–13 deep-faller audit",type="primary",key="v782_run")
        clear=b.button("Clear V7.82 audit",key="v782_clear")
        if clear:
            st.session_state.v782_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=982000+int(ds)*100000+j*281+sum(ord(c) for c in room_name)
                        ...
                        for _,r in late.iterrows():
                            ...
                            if pd.notna(delta) and delta < -30 and audit==1:
                                row={
                                    ...
                                    "Champion_model_rank":r.get("model_rank",np.nan),
                                    "Champion_projection":r.get("projection",np.nan),
                                    "Champion_vorp":r.get("vorp",np.nan),
                                    "Champion_draft_score":r.get("draft_score",np.nan),
                                    "Actual_available_count":r.get("R782_Actual_available_count",np.nan),
                                }
                                for k in range(1,6):
                                    for fld in ["player","position","market_pick","model_rank","projection","vorp","draft_score"]:
                                        row[f"Alt{k}_{fld}"]=r.get(
                                            f"R782_Alt{k}_{fld}",
                                            "" if fld in ["player","position"] else np.nan
                                        )
                                rows.append(row)
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.82 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v782_results=df
            status.success(f"V7.82 complete: {len(df)} R11–13 deep-faller decisions audited.")
            prog.empty()

        df=st.session_state.get("v782_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in [
                "Champion_projection","Champion_vorp","Champion_draft_score","Champion_market_pick",
                "Alt1_projection","Alt1_vorp","Alt1_draft_score","Alt1_market_pick"
            ]:
                df[c]=pd.to_numeric(df[c],errors="coerce")

            df["Alt1_market_change"]=df.Alt1_market_pick-df.Champion_market_pick
            df["Alt1_projection_change"]=df.Alt1_projection-df.Champion_projection
            df["Alt1_vorp_change"]=df.Alt1_vorp-df.Champion_vorp
            df["Alt1_draft_score_change"]=df.Alt1_draft_score-df.Champion_draft_score
            df["Alt1_dominates_score_vorp"]=(df.Alt1_draft_score_change>0)&(df.Alt1_vorp_change>0)

            summary=pd.DataFrame([{
                "Deep-faller decisions":len(df),
                "Average champion market delta":pd.to_numeric(df.Champion_market_delta,errors="coerce").mean(),
                "Actual available players":pd.to_numeric(df.Actual_available_count,errors="coerce").mean(),
                "Alt1 projection change":df.Alt1_projection_change.mean(),
                "Alt1 VORP change":df.Alt1_vorp_change.mean(),
                "Alt1 draft-score change":df.Alt1_draft_score_change.mean(),
                "Alt1 score+VORP dominance rate":df.Alt1_dominates_score_vorp.mean(),
            }])
            ...
            by_round=df.groupby("Round",as_index=False).agg(
                ...
                Champion_market_delta=("Champion_market_delta","mean"),
                Champion_VORP=("Champion_vorp","mean"),
                Champion_draft_score=("Champion_draft_score","mean"),
                Alt1_VORP_change=("Alt1_vorp_change","mean"),
                Alt1_draft_score_change=("Alt1_draft_score_change","mean"),
                Alt1_dominance=("Alt1_dominates_score_vorp","mean")
            )
            st.markdown("#### Deep fallers by round")
            st.dataframe(by_round.round(4),use_container_width=True,hide_index=True)

            champs=df.groupby(["Round","Champion_player","Champion_position"],as_index=False).agg(
                Decisions=("Champion_player","count"),
                Avg_market_delta=("Champion_market_delta","mean"),
                Avg_VORP=("Champion_vorp","mean"),
                Avg_draft_score=("Champion_draft_score","mean"),
                Alt1_VORP_change=("Alt1_vorp_change","mean"),
                Alt1_draft_score_change=("Alt1_draft_score_change","mean"),
                Dominance_rate=("Alt1_dominates_score_vorp","mean")
            ).sort_values("Decisions",ascending=False)
            st.markdown("#### Most repeated R11–13 deep fallers")
            st.dataframe(champs.head(40).round(4),use_container_width=True,hide_index=True)

            alts=df.groupby(["Alt1_player","Alt1_position"],as_index=False).agg(
                Times_top_alt=("Alt1_player","count"),
                Avg_market_pick=("Alt1_market_pick","mean"),
                Projection_change=("Alt1_projection_change","mean"),
                VORP_change=("Alt1_vorp_change","mean"),
                Draft_score_change=("Alt1_draft_score_change","mean"),
                Dominance_rate=("Alt1_dominates_score_vorp","mean")
            ).sort_values("Times_top_alt",ascending=False)
            st.markdown("#### Top genuinely available alternatives")
            st.dataframe(alts.head(40).round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Sample top-5 true-board alternatives")
            showcols=["Draft_slot","Room","Round","Pick","Champion_player","Champion_position","Champion_market_delta"]
            for k in range(1,6):
                showcols += [f"Alt{k}_player",f"Alt{k}_position",f"Alt{k}_market_pick",f"Alt{k}_vorp",f"Alt{k}_draft_score"]
            st.dataframe(df[showcols].head(50).round(3),use_container_width=True,hide_index=True)

            st.info(
                "V7.82 changes no production behavior. If genuinely available alternatives repeatedly dominate deep-faller selections on both frozen draft score and VORP, "
                "the next build can test a narrowly targeted late-round challenger. If not, the deep-faller behavior is likely a consequence of board exhaustion rather than a valuation defect."
            )

            st.download_button(
                "⬇️ Download V7.82 R11–13 deep-faller audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_82_r11_13_deep_faller_true_board_audit.csv",
                "text/csv",key="v782_download"
            )
        elif isinstance(df,pd.DataFrame):
            st.success("No R11–13 >30-pick faller decisions occurred in this run.")
