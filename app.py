...
def roster_need_for_mock(roster_df, pos, slots):
    ...
    if pos=="WR":
        ...
        if have==2: return 7.0      # WR3
        if have==3: return 6.0      # WR4
        if have==4: return 2.5      # WR5
        return -3.0                 # WR6+
    if pos=="RB":
        if flex_need>0 and have<3: return 5.0
        if have==2: return 3.0      # RB3
        if have==3: return 1.0      # RB4
        if have==4: return -5.0     # RB5
        return -12.0                # RB6+
    if pos=="TE":
        return -8.0 if have>=1 else 0.0
    if pos=="QB":
        return -10.0 if have>=1 else 0.0
    if pos in ["DL","DB"]:
        return -12.0 if have>=int(slots.get(pos,1)) else -1.0
    return -3.0

def draft_eligibility(avail, roster, round_no, rounds, slots):
    """Roster-aware constraints: preserve flexibility early, guarantee a legal lineup late."""
    a=avail.copy()
    counts=roster.position.value_counts().to_dict() if len(roster) else {}
    qb=counts.get("QB",0); te=counts.get("TE",0); wr=counts.get("WR",0); rb=counts.get("RB",0)
    dl=counts.get("DL",0); db=counts.get("DB",0)

    # Standard one-QB / one-TE economics.
    if int(slots.get("QB",1))<=1:
        if qb>=2: a=a[~a.position.eq("QB")]
        elif qb>=1 and round_no<12: a=a[~a.position.eq("QB")]
    if int(slots.get("TE",1))<=1:
        if te>=2: a=a[~a.position.eq("TE")]
        elif te>=1 and round_no<12: a=a[~a.position.eq("TE")]

    # One IDP starter means no duplicate IDP bench picks.
    if dl>=max(int(slots.get("DL",1)),1): a=a[~a.position.eq("DL")]
    if db>=max(int(slots.get("DB",1)),1): a=a[~a.position.eq("DB")]

    # Do not let RB5/RB6 crowd out a thin PPR WR room.
    if rb>=5 and wr<4:
        a=a[~a.position.eq("RB")]
    if rb>=6:
        a=a[~a.position.eq("RB")]

    # By Round 11, establish at least a four-WR PPR room before more luxury depth.
    if round_no>=11 and wr<4:
        wr_pool=a[a.position.eq("WR")]
        if len(wr_pool):
            a=wr_pool

    # IDP remains a later-round position in this shallow format.
    if round_no<=7:
        non_idp=a[~a.position.isin(["DL","DB"])]
        if len(non_idp)>=5: a=non_idp

    # Guarantee every required non-flex position is filled before the draft ends.
    missing=[]
    ...
    if missing and user_picks_left<=len(missing):
        ...
        forced=a[a.position.isin(required)]
...
def _fast_need_by_position(counts, slots):
    """NumPy version of marginal roster value for QB,RB,WR,TE,DL,DB."""
    qbi,rbi,wri,tei,dli,dbi=0,1,2,3,4,5
    out=np.full(6,-3.0,dtype=float)
    ...
    for code,pos in enumerate(["QB","RB","WR","TE","DL","DB"]):
        ...
        if direct>0:
            base=11.0+min(direct,2)*2.0+(2.0 if pos=="WR" else 0.0)
            out[code]=base; continue
        if pos=="WR":
            out[code]=8.0 if flex_need>0 else 7.0 if have==2 else 6.0 if have==3 else 2.5 if have==4 else -3.0
        elif pos=="RB":
            out[code]=5.0 if (flex_need>0 and have<3) else 3.0 if have==2 else 1.0 if have==3 else -5.0 if have==4 else -12.0
        elif pos=="TE": out[code]=-8.0 if have>=1 else 0.0
        elif pos=="QB": out[code]=-10.0 if have>=1 else 0.0
        elif pos in ["DL","DB"]: out[code]=-12.0 if have>=int(slots.get(pos,1)) else -1.0
    return out
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None):
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if int(slots.get("QB",1))<=1:
                if counts[qb]>=2 or (counts[qb]>=1 and round_no<12): eligible &= pos[idx]!=qb
            if int(slots.get("TE",1))<=1:
                if counts[te]>=2 or (counts[te]>=1 and round_no<12): eligible &= pos[idx]!=te
            if counts[dl]>=max(int(slots.get("DL",1)),1): eligible &= pos[idx]!=dl
            if counts[db]>=max(int(slots.get("DB",1)),1): eligible &= pos[idx]!=db

            # RB depth loses marginal value when the WR room is still thin.
            if counts[rb]>=5 and counts[wr]<4: eligible &= pos[idx]!=rb
            if counts[rb]>=6: eligible &= pos[idx]!=rb

            # Full-PPR minimum viable depth by the late rounds.
            if round_no>=11 and counts[wr]<4 and np.any(pos[idx]==wr):
                eligible &= pos[idx]==wr

            if round_no<=7 and np.count_nonzero(~np.isin(pos[idx],[dl,db]))>=5:
                eligible &= ~np.isin(pos[idx],[dl,db])

            # Force missing required starters when remaining user picks equal missing slots.
            missing_codes=[]
            for code,pname in enumerate(["QB","RB","WR","TE","DL","DB"]):
                need_n=max(int(slots.get(pname,0))-int(counts[code]),0)
                missing_codes.extend([code]*need_n)
            user_picks_left=int(rounds)-int(round_no)+1
            if missing_codes and user_picks_left<=len(missing_codes):
                force=np.isin(pos[idx],np.array(list(set(missing_codes)),dtype=int))
            ...
            # Dynamic marginal value rather than a one-time positional boost.
            if counts[wr]<3: construction[wr]+=5.0
            elif counts[wr]==3: construction[wr]+=4.0
            elif counts[wr]==4: construction[wr]+=1.0
            if counts[rb]>=4: construction[rb]-=4.0
            if counts[rb]>=5: construction[rb]-=8.0
...
st.caption("V7.1 • Data Integrity + Marginal Roster Value • frozen projection/regression + grading • Yahoo + IDP")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.1 validation report")
            st.caption("V7.1 changes only the two diagnosed failures: identity matching and marginal roster value. The projection/regression model and grading weights remain frozen.")
            ...
            st.download_button("⬇️ Download V7.1 diagnostic CSV",csv,"fantasy_edge_v7_1_diagnostics.csv","text/csv")
