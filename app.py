...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Practice-Aware Start/Sit", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v680_injury_watch_feed():
    ...
    for key in keys:
        ...
        if fresh:
            ...
            # No timestamped evidence exists. Repeated retrieval does NOT renew evidence.
            # Low-severity weekly tags may remain as monitor-only observations because
            # the endpoint is currently reporting them; they do not trigger data-warning
            # verification loops. Severe tags require independent corroboration.
            observed_nonactive=[r for r in observations if r.get("status") not in ("ACTIVE","")]
            if status in ("QUESTIONABLE","DOUBTFUL") and any(r.get("status")==status for r in observations):
                verified=True
...
def _v682_decision(conf, edge, status, practice="", has_alternative=True):
    ...
    V7.66 prevents a current DNP from being surfaced as a green START/LOCK.
...
with tabs[6]:
    ...
    if _my.empty:
        ...
        st.caption("Close lineup calls and availability concerns. A current DNP can never appear as a green START/LOCK; keep a backup ready until participation improves.")
        if _decision_rows:
