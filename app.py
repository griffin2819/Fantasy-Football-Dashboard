...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Mobile Flow + Status Authority", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
st.markdown("""<style>
div[role="radiogroup"]{display:flex!important;flex-wrap:nowrap!important;overflow-x:auto!important;gap:.35rem!important;padding:.25rem 0 .55rem 0!important;scrollbar-width:none;}
div[role="radiogroup"]::-webkit-scrollbar{display:none;}
div[role="radiogroup"] label{white-space:nowrap!important;background:rgba(20,27,36,.86);border:1px solid rgba(148,163,184,.20);border-radius:12px;padding:.48rem .72rem!important;}
    ...
    if isinstance(starters,pd.DataFrame) and len(starters):
        for _,sr in starters.iterrows():
            ...
            if matters:
                ...
                rows.append({"Tier":"DO NOW" if conf<62 or abs(raw_edge)<0.6 else "WATCH","Area":"START/SIT","Player":p,"Action":f"Start {p} over {str(alt.get('player',''))}","Week Impact":round(max(0.0,raw_edge),2),"ROS Impact":0.0,"Confidence":round(conf,0),"Why":reason,"Posture":strategy['posture']})
...
def _v781_render_page(_v777_page_idx):
    ...
    if _v777_page_idx==6:
        ...
        if _my.empty:
            ...
            st.caption(f"Fantasy Edge V7.85 • {_v677_season} Week {_v677_week} • Decision-quality Start/Sit with authoritative availability, uncertainty-aware close calls, and separate unavailable/monitor groups.")
    ...
    if _v777_page_idx==14:
        ...
        if my.empty:
            ...
            _v780_snap_key=f"7.85|{_v780_base_key}|{_v780_avail_sig}"
