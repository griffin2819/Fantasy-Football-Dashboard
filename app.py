...
def _add_marginal_roster_value(x, roster, slots):
    ...
    raw_vorp=pd.to_numeric(y.get("vorp",0),errors="coerce").fillna(0.0)
    raw_proj=pd.to_numeric(y.get("projection",0),errors="coerce").fillna(0.0)
...
def prepare_user_draft_candidates(avail, roster, round_no, current_pick, slot, teams, slots, randomness):
    ...
    if _rb>=5:
        x.loc[x.position.eq("RB"),"_construction_bonus"]-=22.0
        x.loc[x.position.eq("RB"),"rb6_depth_penalty"]=-22.0
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    # Impact-IDP bench value is evaluated only now, after IDP quality is known.
    _total_idp=int(counts.get("DL",0))+int(counts.get("DB",0))
    _deep_offense=(int(counts.get("RB",0))>=5) or (int(counts.get("WR",0))>=5)
    if _deep_offense and _total_idp<4:
        _impact_idp=(
        ...
        if _impact_idp.any():
            x.loc[_impact_idp,"_construction_bonus"]=pd.to_numeric(
                x.loc[_impact_idp].get("_construction_bonus",0),errors="coerce"
            ).fillna(0.0)+16.0
            x.loc[_impact_idp,"impact_idp_depth_bonus"]=16.0
    ...
    # Reward value that can actually be used by this roster, not raw bench accumulation.
    _mrv=pd.to_numeric(x.get("marginal_roster_value",0),errors="coerce").fillna(0.0)
    x["evaluation_score"]+=np.clip(_mrv,-8.0,14.0)

    # Cross-position bench opportunity cost: the candidate must justify using this roster slot.
    _vos=pd.to_numeric(x.get("value_over_next_roster_slot",0),errors="coerce").fillna(0.0)
    _portfolio_pen=pd.to_numeric(x.get("portfolio_depth_penalty",0),errors="coerce").fillna(0.0)
    x["evaluation_score"]+=np.clip(_vos*1.6,-12.0,10.0)
    x["evaluation_score"]-=np.clip(_portfolio_pen,0.0,16.0)
    ...
    # If 5 RBs are already rostered, compare RB6 against the best remaining
    # truly impactful defender. A backup IDP must be Tier 1-2 and roster-opportunity >= -5.
    _rc=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    _total_idp=int(_rc.get("DL",0))+int(_rc.get("DB",0))
    if int(_rc.get("RB",0))>=5 and _total_idp<4:
        _idp_ropp=pd.to_numeric(x.get("roster_opportunity_adj",0),errors="coerce").fillna(0.0)
        _elite_idp=(
            x["position"].isin(["DL","DB"]) &
            x["idp_eligible"].fillna(False).astype(bool) &
            (pd.to_numeric(x["idp_quality_tier"],errors="coerce").fillna(5)<=2) &
        ...
        if _elite_idp.any() and _rbmask.any():
            ...
            if _best_idp>=_best_rb-12.0:
                x.loc[_rbmask,"evaluation_score"]-=5000.0
    ...
    _depth_counts=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    _rb_depth=int(_depth_counts.get("RB",0))>=5
    _wr_depth=int(_depth_counts.get("WR",0))>=5
    if _rb_depth or _wr_depth:
        _idp_pool=(
            x["position"].isin(["DL","DB"]) &
            x["idp_eligible"].fillna(False).astype(bool) &
            (pd.to_numeric(x.get("idp_quality_tier",np.nan),errors="coerce").fillna(5)<=2)
        )
        _idp_ropp=pd.to_numeric(x.get("roster_opportunity_adj",0),errors="coerce").fillna(0.0)
        _idp_pool &= (_idp_ropp>=-5.0)

        if _idp_pool.any():
            _best_idp_mrv=float(pd.to_numeric(
                x.loc[_idp_pool,"marginal_roster_value"],errors="coerce"
            ).max())

            if _rb_depth:
                _rbm=x["position"].eq("RB")
                _rb_mrv=pd.to_numeric(x.loc[_rbm,"marginal_roster_value"],errors="coerce")
                _block_idx=_rb_mrv.index[_rb_mrv < (_best_idp_mrv-0.75)]
                x.loc[_block_idx,"evaluation_score"]-=5000.0
                x.loc[_block_idx,"deep_offense_blocked_for_idp"]=True

            if _wr_depth:
                _wrm=x["position"].eq("WR")
                _wr_mrv=pd.to_numeric(x.loc[_wrm,"marginal_roster_value"],errors="coerce")
                _block_idx=_wr_mrv.index[_wr_mrv < (_best_idp_mrv-0.75)]
                x.loc[_block_idx,"evaluation_score"]-=5000.0
                x.loc[_block_idx,"deep_offense_blocked_for_idp"]=True

    # Fantasy Edge late-bench impact reserve.
    # If offense already has 5 RB and 5 WR, a sixth RB/WR must CLEARLY beat the
    # best qualifying impact IDP; ties and small edges go to roster diversification.
    _bc=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    _deep_both=int(_bc.get("RB",0))>=5 and int(_bc.get("WR",0))>=5
    _total_idp2=int(_bc.get("DL",0))+int(_bc.get("DB",0))
    if _deep_both and _total_idp2<4:
        _ropp2=pd.to_numeric(x.get("roster_opportunity_adj",0),errors="coerce").fillna(0.0)
        _impact2=(
            x["position"].isin(["DL","DB"]) &
            x["idp_eligible"].fillna(False).astype(bool) &
            (pd.to_numeric(x.get("idp_quality_tier",np.nan),errors="coerce").fillna(5)<=2) &
        ...
        if _impact2.any():
            _idp_exec=pd.to_numeric(x.loc[_impact2,"evaluation_score"],errors="coerce")
            _best_idp=float(_idp_exec.max())
            _sixth=x["position"].isin(["RB","WR"])
            # Sixth offense must beat impact IDP by 6 evaluation points to justify redundancy.
            _weak_sixth=_sixth & (pd.to_numeric(x["evaluation_score"],errors="coerce") < (_best_idp+6.0))
            x.loc[_weak_sixth,"evaluation_score"]-=5000.0
            x.loc[_weak_sixth,"bench_diversification_blocked"]=True
    ...
    if _required_idp_filled:
        ...
        _idp_ropp=pd.to_numeric(x.get("roster_opportunity_adj",0),errors="coerce").fillna(0.0)
        _idp_tier=pd.to_numeric(x.get("idp_quality_tier",np.nan),errors="coerce").fillna(5.0)

        # Backup defender must be Tier 1-2 AND have acceptable roster opportunity.
        _bad_extra=_idp_mask_extra & ((_idp_tier>2) | (_idp_ropp<-5.0))
        x.loc[_bad_extra,"evaluation_score"]-=5000.0
        x.loc[_bad_extra,"extra_idp_blocked"]=True

    # Quality-conditional RB6/WR6 authority.
    # We do not ban sixth offensive players universally. They are blocked only when
    # a Tier 1-2, non-negative-fit impact defender is still available and competitive.
    _qc=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    _qropp=pd.to_numeric(x.get("roster_opportunity_adj",0),errors="coerce").fillna(0.0)
    _qidp=(
        x["position"].isin(["DL","DB"]) &
        x["idp_eligible"].fillna(False).astype(bool) &
        (pd.to_numeric(x.get("idp_quality_tier",np.nan),errors="coerce").fillna(5)<=2) &
    ...
    if _qidp.any():
        _best_qidp=float(pd.to_numeric(x.loc[_qidp,"evaluation_score"],errors="coerce").max())
        if int(_qc.get("RB",0))>=5:
            _m=x["position"].eq("RB")
            x.loc[_m & (pd.to_numeric(x["evaluation_score"],errors="coerce")<_best_qidp+5.0),"evaluation_score"]-=5000.0
        if int(_qc.get("WR",0))>=5:
            _m=x["position"].eq("WR")
            x.loc[_m & (pd.to_numeric(x["evaluation_score"],errors="coerce")<_best_qidp+5.0),"evaluation_score"]-=5000.0
...
def draft_eligibility(avail, roster, round_no, rounds, slots):
    ...
    # Allow up to two extra impact defenders on the six-player bench.
    # Required starters are 1 DL + 1 DB; total IDP cap is now 4.
    if total_idp>=4:
...
def grade_mock(roster, teams, slots):
    ...
    # Opportunity cost: true reaches, redundant depth, and unnecessary early QB2/TE2.
    ...
    # Portfolio/reach quality must affect Model Edge itself, not only overall grade.
    _reach_series=(r.consensus_rank-r.mock_pick) if "mock_pick" in r.columns else pd.Series(0,index=r.index)
    _big_reach_rate=float((_reach_series>24).mean()) if len(r) else 0.0
    _redundant_depth=max(counts.get("RB",0)-5,0)+max(counts.get("WR",0)-5,0)+max(counts.get("QB",0)-2,0)+max(counts.get("TE",0)-2,0)
    _portfolio_penalty=min(8.0,2.5*_redundant_depth)
    _reach_penalty=min(8.0,12.0*_big_reach_rate)

    # Additional penalty for ending with both RB6 and WR6 while carrying only
    # the two mandatory IDPs.
    ...
    model_edge=float(np.clip(model_edge-_portfolio_penalty-_reach_penalty-_diversification_penalty,35,100))
    ...
    return {"score":score,"grade":grade,"starter":starter,"value":avg_market,"penalty":max(0,100-construction),
            ...
            "big_reach_rate":_big_reach_rate,"portfolio_penalty":_portfolio_penalty,"diversification_penalty":_diversification_penalty}
...
with tabs[1]:
    ...
    st.dataframe(
        ...
        .rename(columns={
            ...
            "idp_external_rank":"IDP rank","idp_impact_score":"IDP impact","usable_vorp":"Usable VORP","marginal_roster_value":"Marginal roster value","bench_startability":"Bench startability","value_over_next_roster_slot":"Value over next slot","idp_available_rank":"Available IDP rank","idp_available_score":"Available IDP score","roster_opportunity_adj":"Roster opp."
        }).head(120),
        use_container_width=True,hide_index=True
    )
            ...
            if owner==int(slot):
                ...
                st.dataframe(
                    top[["player","position","team","evaluation_rank","Action","Market pick",
                         "Survives to next pick","projection","vorp","usable_vorp","marginal_roster_value","bench_startability","value_over_next_roster_slot",
