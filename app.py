...
def early_wr_scarcity_choice(eval_score, market_pick, position_code, current_pick, next_pick, normal_choice, teams=12, randomness=12, max_eval_deficit=8.0, max_wr_survival=.52):
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None,
                       room_profile="Consensus", wr_eval_deficit=8.0, wr_survival=.52, wr_enabled=True):
...
st.title("🏈 Fantasy Edge V7.23 — WR Scarcity Calibration Matrix")
st.caption("V7.23 • V7.22 8/52 live rule retained • seven-threshold WR calibration matrix")
st.caption("Build ID: V7.23-WRCM-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v723_live_slot")
    ...
    if "v723_target_queue" not in st.session_state:
        st.session_state.v723_target_queue={}
    queue=st.session_state.v723_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v723_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.23 keeps the live/default V7.22 rule at 8/52. The new calibration matrix compares 6/48, 6/52, 8/48, 8/52, 8/56, 10/52 and 10/56 without changing the live rule.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.23 robustness report")
        ...
        st.markdown("### 🧪 V7.23 Current 8/52 validation")
        ...
        if "v723_independent_results" not in st.session_state:
            st.session_state.v723_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v723_independent_results"),pd.DataFrame) else "ready to run"))
        if "v723_independent_summary" not in st.session_state:
            st.session_state.v723_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v723_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v723_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v723_run")
        clear_iv=clear_col.button("Clear independent results",key="v723_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                for ds in iv_slots:
                    for room_name in iv_rooms:
                        for j in range(int(iv_sims)):
                            ...
                            row["V722_R1_3_WR_count"]=int((early_fe.position=="WR").sum())
                            row["V722_R1_3_QB_count"]=int((early_fe.position=="QB").sum())
                            row["V722_R1_3_RB_count"]=int((early_fe.position=="RB").sum())
                            wr_early=early_fe[early_fe.position=="WR"]
                            row["V722_R1_3_WR_market_delta"]=float((pd.to_numeric(wr_early.mock_pick,errors="coerce")-pd.to_numeric(wr_early.market_pick,errors="coerce")).mean()) if len(wr_early) else np.nan
                ...
                st.session_state.v723_independent_results=iv
                st.session_state.v723_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v723_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v723_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🎯 V7.22 Early-WR Scarcity Validation")
            c1,c2,c3,c4=st.columns(4)
            c1.metric("R1-3 WR picks/draft",f"{iv_saved.V722_R1_3_WR_count.mean():.2f}","V7.16: 0.12")
            c2.metric("R1-3 QB picks/draft",f"{iv_saved.V722_R1_3_QB_count.mean():.2f}")
            c3.metric("R1-3 RB picks/draft",f"{iv_saved.V722_R1_3_RB_count.mean():.2f}","V7.16: 1.83")
            c4.metric("Early WR market delta",f"{iv_saved.V722_R1_3_WR_market_delta.mean():+.2f}")
            st.caption("No WR quota/projection boost: intercept requires WR within 8 evaluation points, market priority, and ≤52% survival.")
            ...
            st.download_button(
                "⬇️ Download V7.23 current 8/52 validation CSV",
                iv_csv,
                "fantasy_edge_v7_23_current_8_52_validation.csv",
                "text/csv",
                key="v723_current_download"
            )
            st.caption("The download remains available through normal Streamlit reruns until you press Clear independent results.")

        # ---------------------------------------------------------------
        # V7.23 WR threshold calibration matrix
        # ---------------------------------------------------------------
        st.markdown("### 🧪 V7.23 WR Scarcity Calibration Matrix")
        st.caption(
            "Compare seven nearby R1-3 WR scarcity thresholds using identical seeds. "
            "The normal/live model remains V7.22's 8/52 rule until validation chooses a winner."
        )

        if "v723_calibration_results" not in st.session_state:
            st.session_state.v723_calibration_results=None
        if "v723_calibration_summary" not in st.session_state:
            st.session_state.v723_calibration_summary=None

        cal_sims=st.number_input(
            "Calibration simulations per slot × room",
            min_value=2,max_value=10,value=3,step=1,key="v723_cal_sims",
            help="3 = quick matrix (60 room/slot seeds × 7 variants). Use 10 only for a full finalist-quality comparison."
        )
        ca,cb=st.columns(2)
        run_cal=ca.button("Run WR calibration matrix",type="primary",key="v723_cal_run")
        clear_cal=cb.button("Clear WR calibration",key="v723_cal_clear")
        ...
        if run_cal:
            ...
            cal_slots=[1,3,7,min(12,int(teams))]
            cal_slots=list(dict.fromkeys([x for x in cal_slots if 1<=x<=int(teams)]))
            cal_rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]
            fast_cal=_fast_benchmark_pool(board,int(teams))
            rows_cal=[]
            base_cases=max(1,len(cal_slots)*len(cal_rooms)*int(cal_sims))
            total_cal=base_cases*len(configs)
            done_cal=0
            prog_cal=st.progress(0)
            status_cal=st.empty()
            started_cal=time.perf_counter()

            for ds in cal_slots:
                for room_name in cal_rooms:
                    for j in range(int(cal_sims)):
                        seed=923000+int(ds)*10000+j*31+sum(ord(c) for c in room_name)

                        # Consensus is identical for every WR threshold; compute once and reuse.
                        co=simulate_mock_fast(
                            fast_cal,int(teams),int(ds),15,state["roster_slots"],
                            6.0,False,seed,room_name
                        )
                        cc=independent_draft_grade_components(co,int(teams))
                        cop=independent_phase_metrics(co).set_index("Phase")

                        for config_name,eval_cap,surv_cap in configs:
                            fe=simulate_mock_fast(
                                fast_cal,int(teams),int(ds),15,state["roster_slots"],
                                6.0,True,seed,room_name,
                                wr_eval_deficit=eval_cap,wr_survival=surv_cap,wr_enabled=True
                            )
                            fc=independent_draft_grade_components(fe,int(teams))
                            fep=independent_phase_metrics(fe).set_index("Phase")
                            early=fe[pd.to_numeric(fe.mock_round,errors="coerce").fillna(99)<=3]
                            wr_early=early[early.position.eq("WR")]

                            row={
                                "Config":config_name,
                                "Eval_deficit_cap":eval_cap,
                                "WR_survival_cap":surv_cap,
                                "Draft_slot":int(ds),
                                "Room":room_name,
                                "Simulation":j+1,
                                "FE_grade":fc["overall"],
                                ...
                                "R1_3_WR_count":int(len(wr_early)),
                                "R1_3_WR_market_delta":float(
                                    (pd.to_numeric(wr_early.mock_pick,errors="coerce")-
                                     pd.to_numeric(wr_early.market_pick,errors="coerce")).mean()
                                ) if len(wr_early) else np.nan,
                            }
                            for phase,key in [("R1-3","R1_3"),("R4-6","R4_6"),("R7-10","R7_10"),("R11+","R11_plus")]:
                                fm=float(fep.loc[phase,"Avg_market_delta"])
                                cm=float(cop.loc[phase,"Avg_market_delta"])
                                row[f"{key}_market_gap"]=fm-cm
                            ...
                            if done_cal==1 or done_cal%35==0 or done_cal==total_cal:
                                status_cal.caption(f"WR calibration: {done_cal}/{total_cal} variant simulations")

            cal=pd.DataFrame(rows_cal)
            st.session_state.v723_calibration_results=cal
            st.session_state.v723_calibration_summary={
                "seconds":time.perf_counter()-started_cal,
                "variant_sims":len(cal),
                "base_cases":base_cases,
                "sims":int(cal_sims)
            }
            status_cal.success(f"WR calibration complete: {len(cal)} variant simulations.")
            prog_cal.empty()

        cal_saved=st.session_state.get("v723_calibration_results")
        if isinstance(cal_saved,pd.DataFrame) and len(cal_saved):
            meta_cal=st.session_state.get("v723_calibration_summary") or {}
            st.success(
                f"Saved WR calibration: {len(cal_saved)} variant simulations"
                + (f" • {meta_cal.get('seconds',0):.1f}s" if meta_cal else "")
            )

            summary_cal=cal_saved.groupby(
                ["Config","Eval_deficit_cap","WR_survival_cap"],as_index=False
            ).agg(
                ...
                R1_3_WRs=("R1_3_WR_count","mean"),
                R1_3_WR_market_delta=("R1_3_WR_market_delta","mean"),
            ...
            # Conservative ranking: overall result is primary, with a penalty when
            # downstream R4-10 becomes materially worse than the current 8/52 rule.
            current_row=summary_cal[summary_cal.Config.eq("8/52 CURRENT")]
            if len(current_row):
                cur=current_row.iloc[0]
                summary_cal["Downstream_leakage"]=(
                    np.maximum(cur.R4_6_gap-summary_cal.R4_6_gap,0)
                    +np.maximum(cur.R7_10_gap-summary_cal.R7_10_gap,0)
                )
                summary_cal["Calibration_score"]=summary_cal.Overall_gap-.20*summary_cal.Downstream_leakage
            else:
                summary_cal["Downstream_leakage"]=0.0
                summary_cal["Calibration_score"]=summary_cal.Overall_gap

            summary_cal=summary_cal.sort_values(
                ["Calibration_score","Overall_gap"],ascending=False
            ).reset_index(drop=True)
            summary_cal["Rank"]=np.arange(1,len(summary_cal)+1)

            display=summary_cal[[
                "Rank","Config","Overall_gap","FE_grade","Win_rate","Market_gap",
                "R1_3_gap","R1_3_WRs","R4_6_gap","R7_10_gap","R11_plus_gap",
                "Avg_reach","Avg_faller","Roster_balance","Downstream_leakage"
            ]].copy()
            ...
            winner=summary_cal.iloc[0]
            current=summary_cal[summary_cal.Config.eq("8/52 CURRENT")].iloc[0]
            d1,d2,d3,d4=st.columns(4)
            d1.metric("Calibration leader",winner.Config,f"score {winner.Calibration_score:+.3f}")
            d2.metric("Leader overall gap",f"{winner.Overall_gap:+.3f}",f"{winner.Overall_gap-current.Overall_gap:+.3f} vs 8/52")
            d3.metric("Leader R1-3 gap",f"{winner.R1_3_gap:+.3f}",f"{winner.R1_3_gap-current.R1_3_gap:+.3f}")
            d4.metric("Leader early WRs",f"{winner.R1_3_WRs:.2f}/draft",f"{winner.R1_3_WRs-current.R1_3_WRs:+.2f}")

            if int(meta_cal.get("sims",0))<10:
                st.info(
                    f"Quick calibration leader: {winner.Config}. Treat this as screening, not a final promotion. "
                    "If the leader clearly beats 8/52, rerun the matrix at 10 simulations per slot × room before changing the stable rule."
                )
            else:
                if winner.Config=="8/52 CURRENT":
                    st.success("Full calibration keeps 8/52 as the leader. V7.22's threshold is likely already near the local sweet spot.")
                else:
                    st.success(
                        f"Full calibration leader: {winner.Config}. Compare its overall/R1-3 gains with downstream leakage before promoting it."
                    )

            cal_csv=cal_saved.to_csv(index=False).encode("utf-8")
            st.download_button(
                "⬇️ Download V7.23 WR calibration matrix CSV",
                cal_csv,
                "fantasy_edge_v7_23_wr_calibration_matrix.csv",
                "text/csv",
                key="v723_cal_download"
            )
