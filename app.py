...
def _v781_render_page(_v777_page_idx):
    ...
    if _v777_page_idx==3:
        ...
        if _my_roster.empty:
            ...
            _waiver_data_ready=bool(_v778_snapshot_loaded and _team_cov>0 and _ext_matches>0 and (_mat_cov>0 or _inj_cov>0 or _role_conf_cov>0))
            ...
            if not _waiver_data_ready:
                if st.button("↻ Repair / refresh weekly data now",key="v794_waiver_refresh",use_container_width=True):
                    st.session_state["_v794_force_weekly_refresh"]=True
                    st.rerun(scope="app")
                if not _v793_refresh_sources:
                    st.caption("No successful weekly provider refresh is stored yet. Use the button above; reloading the page alone does not refresh NFL data.")
            ...
            if _moves_df.empty:
                ...
                if _best.empty:
                    if not _waiver_data_ready:
                        st.warning("Weekly data snapshot is incomplete. Run ‘Refresh weekly data now’ once before trusting a HOLD decision.")
                    else:
                    ...
                    if _v793_refresh_sources:
                        ...
                        for _src,_info in _v793_refresh_sources.items():
                            ...
                            _src_rows.append({
                                ...
                                "Rows":_info.get("rows","—"),
                                "Detail":str(_info.get("error","") or "")[:140],
                            })
