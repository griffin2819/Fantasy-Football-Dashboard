...
st.title("🏈 Fantasy Edge V7.31 — Fine VORP Calibration")
st.caption("V7.31 • V7.24 champion retained • fine VORP calibration")
st.caption("Build ID: V7.31-FVC-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v731_live_slot")
    ...
    if "v731_target_queue" not in st.session_state:
        st.session_state.v731_target_queue={}
    queue=st.session_state.v731_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v731_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.31 keeps the V7.24 champion as the live/default model. Replacement stays fixed while VORP is fine-tuned across 7×, 6.5×, 6×, 5.5× and 5× on identical seeds.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.31 robustness report")
        ...
        st.markdown("### 🧪 V7.31 champion baseline validation")
        st.info("V7.31 keeps the V7.24 champion unchanged in the standard test. Fine VORP weights run only in the matched calibration matrix.")
        ...
        if "v731_independent_results" not in st.session_state:
            st.session_state.v731_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v731_independent_results"),pd.DataFrame) else "ready to run"))
        if "v731_independent_summary" not in st.session_state:
            st.session_state.v731_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v731_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v731_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v731_run")
        clear_iv=clear_col.button("Clear independent results",key="v731_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v731_independent_results=iv
                st.session_state.v731_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v731_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v731_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.31 Champion Baseline Check")
            st.caption("The standard independent test still uses the unchanged V7.24 champion. The fine calibration below tests 7×, 6.5×, 6×, 5.5× and 5×.")
            ...
            st.download_button(
                "⬇️ Download V7.31 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_31_champion_baseline.csv",
                "text/csv",
                key="v731_baseline_download"
            )
        ...
        st.markdown("### 🎯 V7.31 Fine VORP Calibration")
        st.caption(
            "The V7.24 champion remains live/default. Replacement stays fixed at 38/55/7. "
            "This matched-seed test narrows VORP to the 5×–7× neighborhood identified by V7.30."
        )

        if "v731_calibration_results" not in st.session_state:
            st.session_state.v731_calibration_results=None
        if "v731_calibration_summary" not in st.session_state:
            st.session_state.v731_calibration_summary=None

        cal_sims=st.number_input(
            "Fine-calibration simulations per slot × room",
            min_value=2,max_value=10,value=3,step=1,key="v731_cal_sims",
            help="Use 3 for screening. Rerun at 10 before promoting a new VORP weight."
        )
        ca,cb=st.columns(2)
        run_cal=ca.button("Run fine VORP calibration",type="primary",key="v731_cal_run")
        clear_cal=cb.button("Clear fine calibration",key="v731_cal_clear")
            ...
            for ds in cal_slots:
                for room_name in cal_rooms:
                    for j in range(int(cal_sims)):
                        seed=931000+int(ds)*10000+j*31+sum(ord(c) for c in room_name)
                        co=simulate_mock_fast(
                        ...
                        for name,weight in configs:
                            ...
                            if done_cal==1 or done_cal%25==0 or done_cal==total_cal:
                                status_cal.caption(f"Fine VORP calibration: {done_cal}/{total_cal}")

            cal=pd.DataFrame(rows_cal)
            st.session_state.v731_calibration_results=cal
            st.session_state.v731_calibration_summary={
                "seconds":time.perf_counter()-started,
                "sims":int(cal_sims),"variant_sims":len(cal)
            }
            status_cal.success(f"Fine VORP calibration complete: {len(cal)} variant simulations.")
            prog_cal.empty()

        cal_saved=st.session_state.get("v731_calibration_results")
        if isinstance(cal_saved,pd.DataFrame) and len(cal_saved):
            meta_cal=st.session_state.get("v731_calibration_summary") or {}
            summary=cal_saved.groupby(["Config","VORP_weight"],as_index=False).agg(
                Overall_gap=("Difference","mean"),FE_grade=("FE_grade","mean"),
                Win_rate=("FE_win","mean"),Market_gap=("Market_value_difference","mean"),
            ...
            # Protect early rounds explicitly. 7x is the upper-edge reference from V7.30.
            ref=summary[summary.Config.eq("VORP 7.0×")].iloc[0]
            summary["R1_3_damage_vs_7x"]=np.maximum(ref.R1_3_gap-summary.R1_3_gap,0)
            summary["Downstream_damage_vs_7x"]=(
                np.maximum(ref.R7_10_gap-summary.R7_10_gap,0)
                +np.maximum(ref.R11_plus_gap-summary.R11_plus_gap,0)
            )
            summary["Protected_score"]=(
                summary.Overall_gap
                +.30*(summary.R4_6_gap-ref.R4_6_gap)
                -.40*summary.R1_3_damage_vs_7x
                -.15*summary.Downstream_damage_vs_7x
            )
            summary=summary.sort_values(
                ["Protected_score","Overall_gap","R4_6_gap"],ascending=False
            ).reset_index(drop=True)
            summary["Rank"]=np.arange(1,len(summary)+1)

            disp=summary[[
                "Rank","Config","Protected_score","Overall_gap","FE_grade","Win_rate",
                "R1_3_gap","R4_6_gap","R7_10_gap","R11_plus_gap",
                "R1_3_damage_vs_7x","Downstream_damage_vs_7x",
                "R4_6_QB","R4_6_RB","R4_6_WR","R4_6_TE",
                "Avg_reach","Avg_faller","Roster_balance"
            ]].copy()
            ...
            c1,c2,c3,c4=st.columns(4)
            c1.metric("Protected-score leader",winner.Config)
            c2.metric("Overall gap",f"{winner.Overall_gap:+.3f}",
                      f"{winner.Overall_gap-ref.Overall_gap:+.3f} vs 7×")
            c3.metric("R4-6 gap",f"{winner.R4_6_gap:+.3f}",
                      f"{winner.R4_6_gap-ref.R4_6_gap:+.3f}")
            c4.metric("R1-3 gap",f"{winner.R1_3_gap:+.3f}",
                      f"{winner.R1_3_gap-ref.R1_3_gap:+.3f}")
            ...
            st.markdown("#### Fine calibration by draft slot")
            st.dataframe(slot_cfg.round(3),use_container_width=True,hide_index=True)

            st.success(
                f"Saved V7.31 calibration: {len(cal_saved)} variant simulations"
                + (f" • {meta_cal.get('seconds',0):.1f}s" if meta_cal else "")
            )
            if int(meta_cal.get("sims",0))<10:
                st.info(
                    f"Screening leader: {winner.Config}. The protected score rewards R4-6 improvement "
                    "while explicitly penalizing R1-3 and downstream damage. Rerun at 10 before promotion."
                )
            else:
                st.success(
                    f"Full fine-calibration leader: {winner.Config}. "
                    "If the slot/room results are broad rather than concentrated, move it to champion validation."
                )

            st.download_button(
                "⬇️ Download V7.31 fine VORP calibration CSV",
                cal_saved.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_31_fine_vorp_calibration.csv",
                "text/csv",key="v731_cal_download"
            )
