...
def roster_opportunity_adjustment(df, roster, round_no, current_pick, slots):
    ...
    for _,r in x.iterrows():
        ...
        if pos in ["DL","DB"]:
            _idp_market = pd.to_numeric(pd.Series([row.get("adp", np.nan)]), errors="coerce").iloc[0]
            _idp_vorp = pd.to_numeric(pd.Series([row.get("vorp", np.nan)]), errors="coerce").iloc[0]
            _idp_proj = pd.to_numeric(pd.Series([row.get("projection", row.get("proj", np.nan))]), errors="coerce").iloc[0]
            _idp_team = str(row.get("team","")).strip().upper()
            _idp_name = str(row.get("player","")).strip()
...
st.title("🏈 Fantasy Edge v9.28 — Live Draft + Impact IDP Quality")
st.caption("v9.28 • v9.08 certified engine • board integrity + impact IDP + roster opportunity")
st.caption("Build ID: FE-V9.28-IMPACT-IDP-BOARD-CLEANUP-20260820")
...
with tabs[2]:
    ...
    st.success("🔒 v9.23.1 availability integrity active • 🧠 v9.28 impact-IDP + coordinated roster opportunity active")
