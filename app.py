...
def validate_position(player, pos):
    p=canonical_position(pos)
    # Identity guardrail for prominent fantasy players. This prevents fuzzy/stat merges
    # from silently assigning an IDP position to an offensive player.
    canonical = {
        "Lamar Jackson":"QB","Josh Allen":"QB","Patrick Mahomes":"QB","Jalen Hurts":"QB",
        "Joe Burrow":"QB","Justin Herbert":"QB","Drake Maye":"QB","Brock Purdy":"QB",
        "Caleb Williams":"QB","Jayden Daniels":"QB","C.J. Stroud":"QB","Trevor Lawrence":"QB",
        "Matthew Stafford":"QB","Jordan Love":"QB","Dak Prescott":"QB","Kyler Murray":"QB",
        "Puka Nacua":"WR","Davante Adams":"WR","Wan'Dale Robinson":"WR",
        "Jonathan Taylor":"RB","Josh Jacobs":"RB","Bucky Irving":"RB","Jaylen Warren":"RB",
        "Cam Skattebo":"RB","Trey McBride":"TE","Tucker Kraft":"TE","George Kittle":"TE",
        "Ed Oliver":"DL"
    }
    return canonical.get(str(player), p)
...
def roster_need_for_mock(roster_df, pos, slots):
    ...
    flex_used=max(0,sum(counts.get(p,0) for p in ["RB","WR","TE"])-sum(int(slots.get(p,0)) for p in ["RB","WR","TE"]))
    flex_need=max(int(slots.get("FLEX",0))-flex_used,0)
    if direct>0:
        bonus=9.0 + min(direct,2)*1.8
        if pos=="WR": bonus += 2.5
        return bonus
    if pos=="WR":
        # PPR WR3/WR4 depth is useful for FLEX, injuries, and weekly volatility.
        if flex_need>0: return 6.0
        if have<4: return 2.5
        return -1.5
    if pos=="RB":
        if flex_need>0: return 4.0
        if have<4: return 1.5
        return -2.0
    if pos=="TE":
        return -7.0 if have>=1 else 0.0
    if pos=="QB":
        return -9.0 if have>=1 else 0.0
    if pos in ["DL","DB"]:
        return -10.0 if have>=int(slots.get(pos,1)) else -2.0
    return -2.0

def draft_eligibility(avail, roster, round_no, rounds, slots):
    """Hard guardrails for realistic redraft roster construction."""
    a=avail.copy()
    counts=roster.position.value_counts().to_dict() if len(roster) else {}
    qb=counts.get("QB",0); te=counts.get("TE",0)
    wr=counts.get("WR",0); rb=counts.get("RB",0)
    dl=counts.get("DL",0); db=counts.get("DB",0)

    # QB2 is a late-round luxury in a 1-QB league; QB3 is never recommended.
    if int(slots.get("QB",1))<=1:
        if qb>=2:
            a=a[~a.position.eq("QB")]
        elif qb>=1 and round_no < max(10, rounds-4):
            a=a[~a.position.eq("QB")]

    # TE2 only after the core roster is built; TE3 is never recommended.
    if int(slots.get("TE",1))<=1:
        if te>=2:
            a=a[~a.position.eq("TE")]
        elif te>=1 and round_no < max(10, rounds-4):
            a=a[~a.position.eq("TE")]

    # In 1-DL/1-DB formats, don't recommend a second player at either IDP slot.
    if dl>=max(int(slots.get("DL",1)),1):
        a=a[~a.position.eq("DL")]
    if db>=max(int(slots.get("DB",1)),1):
        a=a[~a.position.eq("DB")]

    # Force sensible offensive depth through the middle rounds.
    desired_wr=max(3,int(slots.get("WR",2))+int(slots.get("FLEX",1)))
    desired_rb=max(3,int(slots.get("RB",2))+1)
    if round_no>=5 and round_no<=10:
        if wr<desired_wr:
            a.loc[a.position.eq("WR"),"_construction_bonus"]=a.get("_construction_bonus",0)+12
        if rb<desired_rb:
            a.loc[a.position.eq("RB"),"_construction_bonus"]=a.get("_construction_bonus",0)+7

    # Avoid IDP before round 8 unless offensive pool is exhausted.
    if round_no<=7:
        non_idp=a[~a.position.isin(["DL","DB"])]
        if len(non_idp)>=5:
...
def grade_mock(roster, teams, slots):
    if roster.empty:
        return {"grade":"F","score":0,"starter":0,"value":0,"balance":0,"upside":0,"risk":0,"penalty":0}
    chosen=[]; leftovers=[]
    for pos in ["QB","RB","WR","TE","DL","DB"]:
        px=roster[roster.position.eq(pos)].sort_values("projection",ascending=False)
        n=int(slots.get(pos,0))
        chosen.extend(px.head(n).index.tolist())
        leftovers.extend(px.iloc[n:].index.tolist())
    flex_pool=roster.loc[list(set(leftovers))] if leftovers else roster.iloc[0:0]
    flex_pool=flex_pool[flex_pool.position.isin(["RB","WR","TE"])].sort_values("projection",ascending=False)
    chosen.extend(flex_pool.head(int(slots.get("FLEX",0))).index.tolist())
    starters=roster.loc[list(dict.fromkeys(chosen))] if chosen else roster.iloc[0:0]

    starter=float(starters.vorp.clip(lower=0).sum())
    # Cap self-reported model edge so disagreement cannot manufacture an A+.
    value=float(roster.consensus_edge.fillna(0).clip(-15,15).mean())
    counts=roster.position.value_counts().to_dict()
    penalty=0.0
    missing=0
    for pos in ["QB","RB","WR","TE","DL","DB"]:
        req=int(slots.get(pos,0))
        have=int(counts.get(pos,0))
        if have<req:
            missing += req-have
            penalty += 12*(req-have)

    # Severe penalties for inefficient bench construction in a 1-QB / 1-TE / 1-IDP-slot style league.
    penalty += max(0,counts.get("QB",0)-2)*10
    penalty += max(0,counts.get("TE",0)-2)*7
    penalty += max(0,counts.get("DL",0)-int(slots.get("DL",1))-1)*6
    penalty += max(0,counts.get("DB",0)-int(slots.get("DB",1))-1)*6

    # WR room minimum: starters + FLEX-quality depth. This directly catches the V6 failure.
    desired_wr=max(3,int(slots.get("WR",2))+int(slots.get("FLEX",1)))
    if counts.get("WR",0)<desired_wr:
        penalty += (desired_wr-counts.get("WR",0))*8

    # Premium-pick IDP penalty in shallow IDP formats.
    early_idp=((roster.position.isin(["DL","DB"])) & (roster.mock_round<=6)).sum() if "mock_round" in roster else 0
    penalty += float(early_idp)*7

    upside=float(roster.profile.isin(["Breakout target","Ascending","Veteran upside"]).mean()*100)
    risk=float(roster.profile.isin(["Decline risk","High variance"]).mean()*100)
    balance=max(0,100-penalty)
    score=np.clip(42 + starter*1.35 + value*.25 + upside*.05 - risk*.08 - penalty,0,100)
    grade="A+" if score>=94 else "A" if score>=90 else "A-" if score>=87 else "B+" if score>=83 else "B" if score>=80 else "B-" if score>=77 else "C+" if score>=73 else "C" if score>=70 else "C-" if score>=67 else "D" if score>=60 else "F"
    return {"grade":grade,"score":score,"starter":starter,"value":value,"balance":balance,"upside":upside,"risk":risk,"penalty":penalty}

def simulate_mock(board, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None):
    rng=np.random.default_rng(seed)
    pool=board.copy()
    pool=pool[pool.position.isin(["QB","RB","WR","TE","DL","DB"])].copy()
    ...
    for overall in range(1,total+1):
        ...
        avail=pool[~pool.player.isin(drafted)].copy()
        ...
        if owner_slot==slot:
            ...
            if model_user:
                # Model drafter balances roster need with model/market value.
                ...
                # Draft-capital guardrails: one-start IDPs are usually replaceable; don't burn premium picks.
                if round_no<=7:
                    avail.loc[avail.position.isin(["DL","DB"]),"sim_score"] -= (8-round_no)*2.2
                counts=roster.position.value_counts().to_dict() if len(roster) else {}
                if counts.get("QB",0)>=1:
                    avail.loc[avail.position.eq("QB"),"sim_score"] -= 10 if round_no<=10 else 5
                if counts.get("TE",0)>=1:
                    avail.loc[avail.position.eq("TE"),"sim_score"] -= 8 if round_no<=10 else 3
                choice=avail.sort_values("sim_score",ascending=False).iloc[0]
            else:
                # Baseline drafter mostly follows consensus while satisfying glaring roster needs.
                noise=rng.normal(0,randomness/3,len(avail))
                avail["baseline"]=avail.sim_consensus+noise-avail["need"]*.25
                choice=avail.sort_values("baseline").iloc[0]
            rec=choice.to_dict()
            rec["mock_pick"]=overall
            rec["mock_round"]=round_no
            user_rows.append(rec)
        else:
            # Opponents follow consensus with bounded variance: stars cannot randomly fall 100 picks.
            base=avail.sim_consensus.fillna(avail.model_rank)
            # Candidate window expands as draft progresses but remains realistic.
            window=max(18, min(45, int(16 + overall*.12)))
            cand=avail[base <= overall+window].copy()
            if cand.empty:
                cand=avail.nsmallest(min(30,len(avail)),"sim_consensus")
            # Smaller noise for elite players, larger later.
            sd=np.clip(4 + cand.sim_consensus.fillna(overall)/35,4,10)
            noise=rng.normal(0,sd)
            cand["opp_score"]=cand.sim_consensus.fillna(cand.model_rank)+noise
            if round_no<=7:
                cand.loc[cand.position.isin(["DL","DB"]),"opp_score"] += (8-round_no)*2.5
...
st.title("🏈 Fantasy Edge V6.2 — Draft Sanity Engine")
st.caption("Validation release • hard roster guardrails • realistic opponent mocks • WR/RB depth • Yahoo + IDP")
...
with tabs[1]:
    ...
    def need_bonus(pos):
        have=int(counts.get(pos,0))
        need=max(int(slots.get(pos,0))-have,0)
        if need>0:
            bonus=9.0+min(need,2)*1.8
            if pos=="WR": bonus+=2.5
            return bonus
        if pos=="WR":
            if flex_need>0: return 6.0
        ...
        if pos=="RB":
            ...
            if have<4: return 1.5
            return -2.0
        if pos=="QB": return -9.0 if have>=1 else 0.0
        if pos=="TE": return -7.0 if have>=1 else 0.0
        if pos in ["DL","DB"]: return -10.0 if have>=int(slots.get(pos,1)) else -2.0
        return -2.0
    ...
    x["_construction_bonus"]=x.get("_construction_bonus",0)
    x["roster_need"]=x.position.map(need_bonus)
    x["live_score"]=x.draft_score+x.roster_need+x["_construction_bonus"]
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                avail["live"]=avail.draft_score+avail["need"]+avail["_construction_bonus"]
                if rnd<=7:
        ...
        if st.button("▶️ Run benchmark"):
            ...
            for i in range(int(sims)):
                mr=simulate_mock(board,int(teams),int(slot),int(rounds),state["roster_slots"],int(randomness),True,1000+i)
                br=simulate_mock(board,int(teams),int(slot),int(rounds),state["roster_slots"],int(randomness),False,5000+i)
            ...
            best_i=int(np.argmax(model_scores))
            st.markdown("**Best Fantasy Edge mock roster**")
            st.dataframe(model_rosters[best_i][["mock_round","mock_pick","player","position","projection","vorp","model_rank","consensus_rank","profile"]],use_container_width=True,hide_index=True)
