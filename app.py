...
st.title("🏈 Fantasy Edge V7.29 — Replacement / VORP Matched Calibration")
st.caption("V7.29 • V7.24 champion retained • matched replacement/VORP calibration")
st.caption("Build ID: V7.29-RVMC-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v729_live_slot")
    ...
    if "v729_target_queue" not in st.session_state:
        st.session_state.v729_target_queue={}
    queue=st.session_state.v729_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v729_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.29 keeps the V7.24 champion as the live/default model. A matched calibration tests corrected replacement and 9×/8×/7× VORP weighting on identical seeds.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.29 robustness report")
        ...
        st.markdown("### 🧪 V7.29 champion baseline validation")
        st.info("V7.29 keeps the V7.24 champion unchanged in the standard test. Alternative replacement/VORP models run only in the matched calibration matrix.")
        ...
        if "v729_independent_results" not in st.session_state:
            st.session_state.v729_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v729_independent_results"),pd.DataFrame) else "ready to run"))
        if "v729_independent_summary" not in st.session_state:
            st.session_state.v729_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v729_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v729_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v729_run")
        clear_iv=clear_col.button("Clear independent results",key="v729_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v729_independent_results=iv
                st.session_state.v729_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v729_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v729_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.29 Champion Baseline Check")
            st.caption("The standard independent test still uses the unchanged V7.24 champion. Run the matched calibration below for alternative replacement/VORP models.")
            ...
            st.download_button(
                "⬇️ Download V7.29 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_29_champion_baseline.csv",
                "text/csv",
                key="v729_baseline_download"
            )
            st.caption("The download remains available through normal Streamlit reruns until you press Clear independent results.")



        st.markdown("### ⚖️ V7.29 Replacement / VORP Matched Calibration")
        st.caption("Live/default drafting stays on the V7.24 champion. Five valuation variants are tested on identical seeds.")
        st.info("Corrected replacement uses 33% RB / 60% WR / 7% TE FLEX allocation. V7.28 showed nearby corrected allocations collapsed to the same thresholds.")

        if "v729_calibration_results" not in st.session_state:
            st.session_state.v729_calibration_results=None
        if "v729_calibration_summary" not in st.session_state:
            st.session_state.v729_calibration_summary=None

        cal_sims=st.number_input("Calibration simulations per slot × room",2,10,3,1,key="v729_cal_sims",
                                 help="Use 3 for screening; rerun at 10 before promotion.")
        ca,cb=st.columns(2)
        run_cal=ca.button("Run replacement/VORP calibration",type="primary",key="v729_cal_run")
        clear_cal=cb.button("Clear replacement/VORP calibration",key="v729_cal_clear")
        ...
        if run_cal:
            configs=[
                ("CURRENT 38/55/7 • VORP 9×",{"RB":.38,"WR":.55,"TE":.07},9.0,"control"),
                ("CORRECTED 33/60/7 • VORP 9×",{"RB":.33,"WR":.60,"TE":.07},9.0,"replacement"),
                ("CORRECTED 33/60/7 • VORP 8×",{"RB":.33,"WR":.60,"TE":.07},8.0,"both"),
                ("CORRECTED 33/60/7 • VORP 7×",{"RB":.33,"WR":.60,"TE":.07},7.0,"both"),
                ("CURRENT 38/55/7 • VORP 7×",{"RB":.38,"WR":.55,"TE":.07},7.0,"weight"),
            ]
            cal_slots=list(dict.fromkeys([x for x in [1,3,7,min(12,int(teams))] if 1<=x<=int(teams)]))
            cal_rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]

            fast_variants={}
            replacement_meta={}
            for name,alloc,weight,kind in configs:
                vb=_board_with_vorp_variant(board,int(teams),state["roster_slots"],alloc,weight)
                fast_variants[name]=_fast_benchmark_pool(vb,int(teams))
                replacement_meta[name]=vb.attrs.get("replacement_slots",{})

            fast_consensus=_fast_benchmark_pool(board,int(teams))
            rows_cal=[]
            total_cal=len(cal_slots)*len(cal_rooms)*int(cal_sims)*len(configs)
            done_cal=0
            prog_cal=st.progress(0)
            status_cal=st.empty()
            started=time.perf_counter()

            for ds in cal_slots:
                for room_name in cal_rooms:
                    for j in range(int(cal_sims)):
                        seed=929000+int(ds)*10000+j*31+sum(ord(c) for c in room_name)
                        co=simulate_mock_fast(fast_consensus,int(teams),int(ds),15,state["roster_slots"],6.0,False,seed,room_name)
                        cc=independent_draft_grade_components(co,int(teams))
                        cop=independent_phase_metrics(co).set_index("Phase")

                        for name,alloc,weight,kind in configs:
                            fe=simulate_mock_fast(fast_variants[name],int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name)
                            fc=independent_draft_grade_components(fe,int(teams))
                            fep=independent_phase_metrics(fe).set_index("Phase")
                            r46=fe[pd.to_numeric(fe.mock_round,errors="coerce").fillna(99).between(4,6)]
                            rs=replacement_meta[name]
                            row={
                                "Config":name,"Variant_type":kind,"VORP_weight":weight,
                                "Flex_RB":alloc["RB"],"Flex_WR":alloc["WR"],"Flex_TE":alloc["TE"],
                                "Replacement_QB_rank":rs.get("QB",np.nan),"Replacement_RB_rank":rs.get("RB",np.nan),
                                "Replacement_WR_rank":rs.get("WR",np.nan),"Replacement_TE_rank":rs.get("TE",np.nan),
                                "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,
                                "FE_grade":fc["overall"],"Consensus_grade":cc["overall"],
                                "Difference":fc["overall"]-cc["overall"],"FE_win":fc["overall"]>cc["overall"],
                                "Market_value_difference":fc["market_value"]-cc["market_value"],
                                "Roster_balance_difference":fc["roster_balance"]-cc["roster_balance"],
                                "Starter_completion_difference":fc["starter_completion"]-cc["starter_completion"],
                                "Availability_difference":fc["availability"]-cc["availability"],
                                "FE_avg_reach":fc["avg_reach"],"FE_avg_faller":fc["avg_faller"],
                                "R4_6_QB_count":int((r46.position=="QB").sum()),"R4_6_RB_count":int((r46.position=="RB").sum()),
                                "R4_6_WR_count":int((r46.position=="WR").sum()),"R4_6_TE_count":int((r46.position=="TE").sum()),
                            }
                            for phase,key in [("R1-3","R1_3"),("R4-6","R4_6"),("R7-10","R7_10"),("R11+","R11_plus")]:
                                row[f"{key}_market_gap"]=float(fep.loc[phase,"Avg_market_delta"])-float(cop.loc[phase,"Avg_market_delta"])
                                row[f"FE_{key}_reach"]=float(fep.loc[phase,"Avg_reach"])
                                row[f"FE_{key}_faller"]=float(fep.loc[phase,"Avg_faller"])
                            rows_cal.append(row)
                            done_cal+=1
                            prog_cal.progress(done_cal/total_cal)
                            if done_cal==1 or done_cal%25==0 or done_cal==total_cal:
                                status_cal.caption(f"Replacement/VORP calibration: {done_cal}/{total_cal}")

            cal=pd.DataFrame(rows_cal)
            st.session_state.v729_calibration_results=cal
            st.session_state.v729_calibration_summary={"seconds":time.perf_counter()-started,"sims":int(cal_sims),"variant_sims":len(cal)}
            status_cal.success(f"Calibration complete: {len(cal)} variant simulations.")
            prog_cal.empty()

        cal_saved=st.session_state.get("v729_calibration_results")
        if isinstance(cal_saved,pd.DataFrame) and len(cal_saved):
            meta_cal=st.session_state.get("v729_calibration_summary") or {}
            st.success(f"Saved calibration: {len(cal_saved)} variant simulations • {meta_cal.get('seconds',0):.1f}s")

            summary=cal_saved.groupby(["Config","Variant_type","VORP_weight","Flex_RB","Flex_WR","Flex_TE"],as_index=False).agg(
                Overall_gap=("Difference","mean"),FE_grade=("FE_grade","mean"),Win_rate=("FE_win","mean"),
                Market_gap=("Market_value_difference","mean"),Roster_balance=("Roster_balance_difference","mean"),
                Starter_gap=("Starter_completion_difference","mean"),Availability_gap=("Availability_difference","mean"),
                Avg_reach=("FE_avg_reach","mean"),Avg_faller=("FE_avg_faller","mean"),
                R1_3_gap=("R1_3_market_gap","mean"),R4_6_gap=("R4_6_market_gap","mean"),
                R7_10_gap=("R7_10_market_gap","mean"),R11_plus_gap=("R11_plus_market_gap","mean"),
                R4_6_QB=("R4_6_QB_count","mean"),R4_6_RB=("R4_6_RB_count","mean"),
                R4_6_WR=("R4_6_WR_count","mean"),R4_6_TE=("R4_6_TE_count","mean"),
                Tests=("Simulation","count")
            )
            control=summary[summary.Config.eq("CURRENT 38/55/7 • VORP 9×")].iloc[0]
            summary["Non_mid_leakage"]=(
                np.maximum(control.R1_3_gap-summary.R1_3_gap,0)
                +np.maximum(control.R7_10_gap-summary.R7_10_gap,0)
                +np.maximum(control.R11_plus_gap-summary.R11_plus_gap,0)
            )
            summary["Calibration_score"]=summary.Overall_gap+.20*(summary.R4_6_gap-control.R4_6_gap)-.20*summary.Non_mid_leakage
            summary=summary.sort_values(["Calibration_score","Overall_gap"],ascending=False).reset_index(drop=True)
            summary["Rank"]=np.arange(1,len(summary)+1)

            disp=summary[["Rank","Config","Overall_gap","FE_grade","Win_rate","Market_gap","R4_6_gap",
                          "R4_6_QB","R4_6_RB","R4_6_WR","R4_6_TE","R1_3_gap","R7_10_gap","R11_plus_gap",
                          "Avg_reach","Avg_faller","Roster_balance","Non_mid_leakage"]].copy()
            disp["Win_rate"]=disp.Win_rate.map(lambda x:f"{x:.0%}")
            st.dataframe(disp.round(3),use_container_width=True,hide_index=True)

            winner=summary.iloc[0]
            c1,c2,c3,c4=st.columns(4)
            c1.metric("Calibration leader",winner.Config)
            c2.metric("Leader overall gap",f"{winner.Overall_gap:+.3f}",f"{winner.Overall_gap-control.Overall_gap:+.3f}")
            c3.metric("Leader R4-6 gap",f"{winner.R4_6_gap:+.3f}",f"{winner.R4_6_gap-control.R4_6_gap:+.3f}")
            c4.metric("Leader R4-6 WRs",f"{winner.R4_6_WR:.2f}/draft",f"{winner.R4_6_WR-control.R4_6_WR:+.2f}")

            corrected9=summary[summary.Config.eq("CORRECTED 33/60/7 • VORP 9×")].iloc[0]
            current7=summary[summary.Config.eq("CURRENT 38/55/7 • VORP 7×")].iloc[0]
            corrected7=summary[summary.Config.eq("CORRECTED 33/60/7 • VORP 7×")].iloc[0]
            replacement_gain=corrected9.Overall_gap-control.Overall_gap
            weight_gain=current7.Overall_gap-control.Overall_gap
            combined_gain=corrected7.Overall_gap-control.Overall_gap
            interaction=combined_gain-replacement_gain-weight_gain
            attr=pd.DataFrame([
                {"Effect":"Replacement correction only","Overall-gap change":replacement_gain,"R4-6-gap change":corrected9.R4_6_gap-control.R4_6_gap},
                {"Effect":"VORP 9× → 7× only","Overall-gap change":weight_gain,"R4-6-gap change":current7.R4_6_gap-control.R4_6_gap},
                {"Effect":"Corrected + 7×","Overall-gap change":combined_gain,"R4-6-gap change":corrected7.R4_6_gap-control.R4_6_gap},
                {"Effect":"Estimated interaction","Overall-gap change":interaction,
                 "R4-6-gap change":(corrected7.R4_6_gap-control.R4_6_gap)-(corrected9.R4_6_gap-control.R4_6_gap)-(current7.R4_6_gap-control.R4_6_gap)},
            ])
            st.markdown("#### Attribution of improvement")
            st.dataframe(attr.round(3),use_container_width=True,hide_index=True)

            if int(meta_cal.get("sims",0))<10:
                st.info(f"Screening leader: {winner.Config}. If it clearly beats control, rerun at 10 simulations per slot × room.")
            elif winner.Config=="CURRENT 38/55/7 • VORP 9×":
                st.success("Full calibration keeps the current V7.24 replacement/VORP construction.")
            else:
                st.success(f"Full matched calibration leader: {winner.Config}. Promote only after a clean champion-validation build.")

            slot_cfg=cal_saved.groupby(["Config","Draft_slot"]).agg(
                Overall_gap=("Difference","mean"),R4_6_gap=("R4_6_market_gap","mean"),
                R4_6_QB=("R4_6_QB_count","mean"),R4_6_RB=("R4_6_RB_count","mean"),
                R4_6_WR=("R4_6_WR_count","mean"),R4_6_TE=("R4_6_TE_count","mean")
            ).reset_index()
            st.markdown("#### Calibration by draft slot")
            st.dataframe(slot_cfg.round(3),use_container_width=True,hide_index=True)

            st.download_button(
                "⬇️ Download V7.29 replacement/VORP calibration CSV",
                cal_saved.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_29_replacement_vorp_calibration.csv",
                "text/csv",key="v729_cal_download"
            )
