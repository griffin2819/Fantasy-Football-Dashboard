...
st.set_page_config(page_title="Fantasy Edge V6.98 — Automated Weekly Action Engine", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V6.98-AUTOMATED-WEEKLY-ACTION-ENGINE"
...
def _v697_waiver_priority_score(add_row,drop_row,state):
    net_week=_v676_player_value(add_row)-_v676_player_value(drop_row)
    net_ros=_v682_ros_value(add_row)-_v682_ros_value(drop_row)
    ...
    score=50+max(-12,min(18,net_week*5))+max(-12,min(18,net_ros*3))+role*8+scarcity*8+injury_bonus*6
    score=max(0.0,min(100.0,score))
    if score>=86: label="🔥 PRIORITY CLAIM"
    elif score>=74: label="⬆️ ADD NOW"
    elif score>=64 and net_week<0.75 and net_ros>0: label="🧳 STASH"
    elif score>=58 and net_week>0.25: label="🎯 STREAMER"
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        if _moves_df.empty:
            ...
            _ps=[]; _pl=[]
            for _,_wr in _best.iterrows():
                _ar=board[board["player"].eq(_wr["Add"])]
                _dr=board[board["player"].eq(_wr["Drop"])]
                if len(_ar) and len(_dr): _sc,_lab=_v697_waiver_priority_score(_ar.iloc[0],_dr.iloc[0],state)
                else: _sc,_lab=0.0,"⛔ PASS"
                _ps.append(_sc); _pl.append(_lab)
            _best["Priority Score"]=_ps; _best["Waiver Call"]=_pl
            _best=_best.sort_values(["Priority Score","Net Week","Net ROS"],ascending=False)
            st.dataframe(_best[["Add","Pos","Priority Score","Waiver Call","Net Week","Net ROS","Add Role","Add Recent PPG","Add Status","Add Practice","Add Injury","Add Matchup","Drop","Drop Pos","Drop Role","Drop Recent PPG","Drop Protection","Drop Status","Drop Practice","Add PPG","Drop PPG","Add ROS","Drop ROS","Add VORP","Drop VORP","Why"]].head(40),
                         use_container_width=True,hide_index=True)
