...
def early_qb_wait_gate(eval_score, market_pick, position_code, current_pick, next_pick,
                       normal_choice, teams=12, randomness=12,
                       qb_survival_min=.58, alt_survival_max=.45, max_eval_edge=18.0):
    """Conservative R1-3 QB timing gate; evaluation scores remain untouched."""
    ev=np.asarray(eval_score,dtype=float)
    mp=np.asarray(market_pick,dtype=float)
    ...
    teams=max(int(teams),1)
    round_no=max(1,int((int(current_pick)-1)//teams)+1)
    diag={"qb_wait_gate":False,"qb_survival":np.nan,"alt_survival":np.nan,
          "eval_edge":np.nan,"alt_index":None}
    if round_no>3 or next_pick is None or len(ev)==0 or pc[normal]!=0:
        return normal,False,diag

    qb_surv=survival_probability(mp[normal],next_pick,randomness) if np.isfinite(mp[normal]) else np.nan
    diag["qb_survival"]=qb_surv
    if pd.isna(qb_surv) or qb_surv<qb_survival_min:
        return normal,False,diag

    alt=np.flatnonzero(np.isin(pc,[1,2]))  # RB/WR only
    if len(alt)==0:
        return normal,False,diag

    alt_surv=survival_probability_array(mp[alt],next_pick,randomness)
    at_risk=alt[np.isfinite(alt_surv) & (alt_surv<=alt_survival_max)]
    if len(at_risk)==0:
        return normal,False,diag

    best_alt=int(at_risk[np.argmax(ev[at_risk])])
    best_alt_surv=survival_probability(mp[best_alt],next_pick,randomness) if np.isfinite(mp[best_alt]) else np.nan
    eval_edge=float(ev[normal]-ev[best_alt])
    diag.update({"alt_survival":best_alt_surv,"eval_edge":eval_edge,"alt_index":best_alt})

    if eval_edge>float(max_eval_edge):
        return normal,False,diag

    diag["qb_wait_gate"]=True
    return best_alt,True,diag
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                local_choice,qb_gated,qb_gate_diag=early_qb_wait_gate(
...
st.title("🏈 Fantasy Edge V7.18 — Early-QB Timing Discipline")
st.caption("V7.18 • V7.16 stable base • conservative R1-3 QB Wait Gate • QB evaluation frozen")
st.caption("Build ID: V7.18-EQBTD-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v718_live_slot")
    ...
    local,qb_gated,qb_gate_diag=early_qb_wait_gate(
        x.evaluation_score.to_numpy(float),cr.to_numpy(float),
        x.position.map({"QB":0,"RB":1,"WR":2,"TE":3,"DL":4,"DB":5}).to_numpy(int),
        current_overall,live_next,local,teams=int(teams),
    ...
    if qb_gated:
        alt=x.iloc[int(qb_gate_diag["alt_index"])]
        st.warning(
            f"⏳ EARLY-QB WAIT GATE: take {alt.player} ({alt.position}) now and target the QB next. "
            f"QB survival ≈ {qb_gate_diag['qb_survival']:.0%}; "
            f"{alt.position} survival ≈ {qb_gate_diag['alt_survival']:.0%}; "
            f"QB evaluation edge {qb_gate_diag['eval_edge']:+.1f}."
        )
    ...
    if intercepted:
        ...
        gate_pick=x.iloc[int(local)]
        st.success(f"✅ DRAFT NOW — QB WAIT GATE: {gate_pick.player} ({gate_pick.position}, {gate_pick.team})")
        st.caption("The QB remains a top model target, but Fantasy Edge expects a better chance to get him at the next pick than this scarce RB/WR alternative.")
    elif len(draft_now):
        dn=draft_now.iloc[0]
    ...
    if "v718_target_queue" not in st.session_state:
        st.session_state.v718_target_queue={}
    queue=st.session_state.v718_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v718_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.18 keeps V7.16 evaluation frozen. Only R1-3 QB execution can wait when QB survival is ≥58%, a scarce RB/WR alternative survival is ≤45%, and the QB evaluation edge is ≤18.")
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                local,qb_gated,qb_gate_diag=early_qb_wait_gate(
                    ...
                    avail.position.map({"QB":0,"RB":1,"WR":2,"TE":3,"DL":4,"DB":5}).to_numpy(int),
                    overall,nxt,local,teams=int(teams),randomness=randomness
                )
                intercept_local,intercepted,intercept_fall=faller_intercept_choice(
                ...
                if intercepted:
                    ...
                    dn=avail.iloc[int(local)]
                    st.warning(
                        f"⏳ EARLY-QB WAIT GATE → DRAFT {dn.player} ({dn.position}) NOW. "
                        f"QB survival ≈ {qb_gate_diag['qb_survival']:.0%}; alternative survival ≈ {qb_gate_diag['alt_survival']:.0%}."
                    )
                else:
                    dn=now_top.iloc[0]
                ...
                if intercepted:
                    ...
                    gate_name=avail.iloc[int(local)].player
                    pick_options=[gate_name]+[p for p in pick_options if p!=gate_name]
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.18 robustness report")
            ...
            st.download_button("⬇️ Download V7.18 diagnostic CSV",csv,"fantasy_edge_v7_18_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.18 independent validation")
        ...
        if "v718_independent_results" not in st.session_state:
            st.session_state.v718_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v718_independent_results"),pd.DataFrame) else "ready to run"))
        if "v718_independent_summary" not in st.session_state:
            st.session_state.v718_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v718_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v718_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v718_run")
        clear_iv=clear_col.button("Clear independent results",key="v718_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                for ds in iv_slots:
                    for room_name in iv_rooms:
                        for j in range(int(iv_sims)):
                            ...
                            early_fe=fe[pd.to_numeric(fe.mock_round,errors="coerce").fillna(99)<=3].copy()
                            early_qb=early_fe[early_fe.position.eq("QB")]
                            row["FE_R1_3_QB_count"]=int(len(early_qb))
                            row["FE_R1_3_WR_count"]=int((early_fe.position=="WR").sum())
                            row["FE_R1_3_RB_count"]=int((early_fe.position=="RB").sum())
                            row["FE_R1_3_QB_market_delta"]=float(
                                (pd.to_numeric(early_qb.mock_pick,errors="coerce")-pd.to_numeric(early_qb.market_pick,errors="coerce")).mean()
                            ) if len(early_qb) else np.nan
                ...
                st.session_state.v718_independent_results=iv
                st.session_state.v718_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v718_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v718_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧠 V7.18 Early-QB Timing Check")
            q1,q2,q3,q4=st.columns(4)
            q1.metric("R1-3 QB picks/draft",f"{iv_saved.FE_R1_3_QB_count.mean():.2f}","V7.17 reference: 0.98")
            q2.metric("R1-3 WR picks/draft",f"{iv_saved.FE_R1_3_WR_count.mean():.2f}","V7.17 reference: 0.12")
            q3.metric("R1-3 RB picks/draft",f"{iv_saved.FE_R1_3_RB_count.mean():.2f}","V7.17 reference: 1.83")
            q4.metric("Early-QB market delta",f"{iv_saved.FE_R1_3_QB_market_delta.mean():+.2f}","V7.17 reference: -5.35")
            st.caption("Goal: improve early-QB timing without forcing QB out of Rounds 1-3 or changing QB evaluation.")
            ...
            st.download_button(
                "⬇️ Download V7.18 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_18_early_qb_validation.csv",
                "text/csv",
                key="v718_download"
            )
