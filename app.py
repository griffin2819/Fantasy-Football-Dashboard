...
def _early_opportunity_snapshot(fast, candidate_idx, eval_score, chosen_global,
                                overall, next_pick, randomness):
    """Describe what an early-round Fantasy Edge pick passed on.

    This is diagnostic only and never changes the chosen player.
    The primary alternative is the highest-evaluated candidate at a DIFFERENT
    position from the chosen player. We also record the best available WR/RB/QB/TE
    by model evaluation and their estimated survival to the next user pick.
    """
    cand=np.asarray(candidate_idx,dtype=int)
    ev=np.asarray(eval_score,dtype=float)
    pool=fast["pool"]
    pos=fast["pos_code"]
    market=fast["market_pick"]
    model_rank=fast["model_rank"]
    chosen_global=int(chosen_global)
    chosen_pos=int(pos[chosen_global])
    local_chosen=np.flatnonzero(cand==chosen_global)
    chosen_eval=float(ev[int(local_chosen[0])]) if len(local_chosen) else np.nan

    alt_mask=pos[cand]!=chosen_pos
    alt_local=np.flatnonzero(alt_mask)
    if len(alt_local):
        j=int(alt_local[np.argmax(ev[alt_local])])
        alt_global=int(cand[j])
        alt_eval=float(ev[j])
        alt_market=float(market[alt_global]) if np.isfinite(market[alt_global]) else np.nan
        alt_survival=survival_probability(alt_market,next_pick,randomness) if np.isfinite(alt_market) else np.nan
        alt_player=str(pool.iloc[alt_global].player)
        alt_pos=str(pool.iloc[alt_global].position)
        alt_model_rank=float(model_rank[alt_global]) if np.isfinite(model_rank[alt_global]) else np.nan
    else:
        alt_global=None; alt_eval=np.nan; alt_market=np.nan; alt_survival=np.nan
        alt_player=""; alt_pos=""; alt_model_rank=np.nan

    snap={
        "ER_Chosen_eval":chosen_eval,
        "ER_Alt_idx":alt_global,
        "ER_Alt_player":alt_player,
        "ER_Alt_position":alt_pos,
        "ER_Alt_eval":alt_eval,
        "ER_Eval_gap_vs_alt":chosen_eval-alt_eval if np.isfinite(chosen_eval) and np.isfinite(alt_eval) else np.nan,
        "ER_Alt_market_pick":alt_market,
        "ER_Alt_model_rank":alt_model_rank,
        "ER_Alt_survival_est":alt_survival,
        "ER_Alt_survived_actual":np.nan,
    }

    # Best model-evaluated candidate by major offensive position.
    ...
    for code,name in code_name.items():
        ...
        if len(loc):
            ...
            snap[f"ER_Best_{name}_survival_est"]=survival_probability(mp,next_pick,randomness) if np.isfinite(mp) else np.nan
        else:
...
def early_round_opportunity_summary(draft):
    """Aggregate R1-3 opportunity-cost diagnostics for one completed draft."""
    if draft is None or len(draft)==0:
        return {}
    d=draft.copy()
    er=d[pd.to_numeric(d.mock_round,errors="coerce").fillna(99)<=3]
    out={}
    counts=er.position.value_counts().to_dict() if len(er) else {}
    for p in ["QB","RB","WR","TE"]:
        out[f"ER_{p}_count"]=int(counts.get(p,0))
    if len(er):
        out["ER_avg_chosen_market_delta"]=float((pd.to_numeric(er.mock_pick,errors="coerce")-pd.to_numeric(er.market_pick,errors="coerce")).mean())
        out["ER_avg_alt_survival_est"]=float(pd.to_numeric(er.get("ER_Alt_survival_est"),errors="coerce").mean())
        out["ER_actual_alt_survival_rate"]=float(pd.to_numeric(er.get("ER_Alt_survived_actual"),errors="coerce").mean())
        out["ER_avg_eval_gap_vs_alt"]=float(pd.to_numeric(er.get("ER_Eval_gap_vs_alt"),errors="coerce").mean())
        for chosen_pos in ["QB","RB","WR","TE"]:
            g=er[er.position.eq(chosen_pos)]
            out[f"ER_{chosen_pos}_picks"]=int(len(g))
            out[f"ER_{chosen_pos}_avg_market_delta"]=float((pd.to_numeric(g.mock_pick,errors="coerce")-pd.to_numeric(g.market_pick,errors="coerce")).mean()) if len(g) else np.nan
            out[f"ER_{chosen_pos}_alt_survival_est"]=float(pd.to_numeric(g.get("ER_Alt_survival_est"),errors="coerce").mean()) if len(g) else np.nan
            out[f"ER_{chosen_pos}_alt_survival_actual"]=float(pd.to_numeric(g.get("ER_Alt_survived_actual"),errors="coerce").mean()) if len(g) else np.nan
            out[f"ER_{chosen_pos}_eval_gap_vs_alt"]=float(pd.to_numeric(g.get("ER_Eval_gap_vs_alt"),errors="coerce").mean()) if len(g) else np.nan
    return out
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None, room_profile="Consensus"):
    ...
    user_diag=[]
    pending_alt_idx=None
    pending_alt_diag=None
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            # Diagnostic only: did the primary alternative from our prior early pick
            # actually survive until this selection?
            if pending_alt_idx is not None and pending_alt_diag is not None:
                user_diag[pending_alt_diag]["ER_Alt_survived_actual"]=1.0 if alive[int(pending_alt_idx)] else 0.0
            ...
            if model_user:
                ...
                # V7.17 diagnostic-only early opportunity snapshot.
                diag={}
                if round_no<=3:
                    snap=_early_opportunity_snapshot(
            ...
            if model_user and round_no<=3 and diag.get("ER_Alt_idx") is not None:
                pending_alt_idx=int(diag["ER_Alt_idx"])
    ...
    # Attach diagnostic metadata to the selected rows only; it never feeds the draft engine.
    if len(user_diag)==len(roster):
        diag_df=pd.DataFrame(user_diag,index=roster.index)
        for col in diag_df.columns:
            roster[col]=diag_df[col]
    return roster
...
st.title("🏈 Fantasy Edge V7.17 — Early-Round Opportunity Cost")
st.caption("V7.17 • V7.16 draft engine frozen • R1-3 opportunity-cost diagnostics • no model changes")
st.caption("Build ID: V7.17-EROC-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v717_live_slot")
    ...
    if "v717_target_queue" not in st.session_state:
        st.session_state.v717_target_queue={}
    queue=st.session_state.v717_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v717_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.17 is diagnostic only. V7.16 drafting rules are frozen; the new report measures R1-3 QB/RB/WR/TE opportunity cost and whether passed alternatives actually survive.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.17 robustness report")
            ...
            st.download_button("⬇️ Download V7.17 diagnostic CSV",csv,"fantasy_edge_v7_17_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.17 independent diagnostics")
        ...
        if "v717_independent_results" not in st.session_state:
            st.session_state.v717_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v717_independent_results"),pd.DataFrame) else "ready to run"))
        if "v717_independent_summary" not in st.session_state:
            st.session_state.v717_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v717_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v717_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v717_run")
        clear_iv=clear_col.button("Clear independent results",key="v717_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                for ds in iv_slots:
                    for room_name in iv_rooms:
                        for j in range(int(iv_sims)):
                            ...
                            # V7.17 early-round opportunity-cost diagnostics (FE only).
                            ers=early_round_opportunity_summary(fe)
                            row.update(ers)
                ...
                st.session_state.v717_independent_results=iv
                st.session_state.v717_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v717_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v717_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🔎 V7.17 Early-Round Opportunity Cost")
            st.caption("Diagnostic only. V7.16 draft choices are unchanged; this measures what R1-3 selections pass on and whether those alternatives survive.")

            er_rows=[]
            for p in ["QB","RB","WR","TE"]:
                er_rows.append({
                    "Chosen position":p,
                    "Avg picks/draft":iv_saved[f"ER_{p}_picks"].mean(),
                    "Chosen market delta":iv_saved[f"ER_{p}_avg_market_delta"].mean(),
                    "Alt survival estimate":iv_saved[f"ER_{p}_alt_survival_est"].mean(),
                    "Alt actual survival":iv_saved[f"ER_{p}_alt_survival_actual"].mean(),
                    "Evaluation edge vs best other position":iv_saved[f"ER_{p}_eval_gap_vs_alt"].mean()
                })
            er_df=pd.DataFrame(er_rows)
            st.dataframe(er_df.round(2),use_container_width=True,hide_index=True)

            e1,e2,e3,e4=st.columns(4)
            e1.metric("R1-3 QB picks/draft",f"{iv_saved.ER_QB_picks.mean():.2f}")
            e2.metric("R1-3 WR picks/draft",f"{iv_saved.ER_WR_picks.mean():.2f}")
            e3.metric("Passed alt actually survives",f"{iv_saved.ER_actual_alt_survival_rate.mean():.0%}")
            e4.metric("Avg eval edge vs alt",f"{iv_saved.ER_avg_eval_gap_vs_alt.mean():+.2f}")

            # Diagnose early QB opportunity cost specifically.
            qb=er_df[er_df["Chosen position"].eq("QB")].iloc[0]
            wr=er_df[er_df["Chosen position"].eq("WR")].iloc[0]
            if qb["Avg picks/draft"]>0.5 and pd.notna(qb["Alt actual survival"]):
                if qb["Alt actual survival"]<0.45:
                    st.warning(
                        f"Early-QB opportunity-cost flag: after R1-3 QB picks, the best different-position alternative survives only "
                        f"{qb['Alt actual survival']:.0%} of the time. That suggests premium QB selections may be costing scarce RB/WR opportunities."
                    )
                else:
                    st.info(
                        f"Early-QB alternatives survive {qb['Alt actual survival']:.0%} of the time. "
                        "That weakens the case that early QB is the primary opportunity-cost problem."
                    )

            # Slot-specific early opportunity cost.
            slot_er=[]
            for ds,g in iv_saved.groupby("Draft_slot"):
                slot_er.append({
                    "Draft slot":int(ds),
                    "R1-3 QB picks":g.ER_QB_picks.mean(),
                    "R1-3 WR picks":g.ER_WR_picks.mean(),
                    "Alt survival actual":g.ER_actual_alt_survival_rate.mean(),
                    "Eval edge vs alt":g.ER_avg_eval_gap_vs_alt.mean(),
                    "R1-3 market gap":g.R1_3_Avg_market_delta_difference.mean()
                })
            st.markdown("#### Early opportunity cost by draft slot")
            st.dataframe(pd.DataFrame(slot_er).round(2),use_container_width=True,hide_index=True)
            ...
            st.download_button(
                "⬇️ Download V7.17 independent diagnostics CSV",
                iv_csv,
                "fantasy_edge_v7_17_early_round_opportunity_cost.csv",
                "text/csv",
                key="v717_download"
            )
