...
st.title("🏈 Fantasy Edge V7.54 — Empirical R11+ Survival Challenger")
st.caption("V7.54 • V7.50 champion frozen • empirical R11+ survival challenger")
st.caption("Build ID: V7.54-R11EMPSURV-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v754_live_slot")
    ...
    if "v754_target_queue" not in st.session_state:
        st.session_state.v754_target_queue={}
    queue=st.session_state.v754_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v754_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.54 keeps V7.50 as the live/default champion with the 55.97 rule unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.54 robustness report")
        ...
        st.markdown("### 🧪 V7.54 champion baseline validation")
        st.info("V7.54 is challenger validation only. The empirical survival curve is off in the live/default champion and runs only in matched tests.")
        ...
        if "v754_independent_results" not in st.session_state:
            st.session_state.v754_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v754_independent_results"),pd.DataFrame) else "ready to run"))
        if "v754_independent_summary" not in st.session_state:
            st.session_state.v754_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v754_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v754_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v754_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v754_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v754_independent_results=iv
                st.session_state.v754_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v754_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v754_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.54 Champion Baseline Check")
            st.caption("V7.54 keeps the V7.50 champion fully frozen and tests a challenger that changes only R11+ execution survival using the frozen V7.53 empirical curve.")
            ...
            st.download_button(
                "⬇️ Download V7.54 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_54_champion_baseline.csv",
                "text/csv",
                key="v754_baseline_download"
            )
        ...
        st.markdown("### 🧪 V7.54 — Empirical R11+ Survival Challenger")
        st.caption(
            "Challenger validation only. V7.50 remains the live/frozen champion. "
            "The challenger changes only R11+ execution survival, using the frozen V7.53 empirical slot+round+distance curve. "
            "Player score, 9× VORP, roster utility, 55.97 Pick-12 rule, position logic and all R1-10 execution remain unchanged."
        )

        if "v754_results" not in st.session_state: st.session_state.v754_results=None
        sims=st.number_input(
            "V7.54 matched simulations per slot × room",
            min_value=10,max_value=50,value=25,step=5,key="v754_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 fresh matched champion-vs-challenger sets."
        )
        a,b=st.columns(2)
        run=a.button("Run empirical survival challenger",type="primary",key="v754_run")
        clear=b.button("Clear V7.54 challenger",key="v754_clear")
        if clear:
            st.session_state.v754_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=954000+int(ds)*10000+j*131+sum(ord(c) for c in room_name)
                        ...
                        cand=simulate_mock_fast(
                            fast_champ,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name,
                            late_empirical_survival_enabled=True
                        )
                        ...
                        late_c=champ[pd.to_numeric(champ.mock_round,errors="coerce").fillna(0)>=11].copy()
                        late_n=cand[pd.to_numeric(cand.mock_round,errors="coerce").fillna(0)>=11].copy()
                        for x in [late_c,late_n]:
                            x["mp"]=pd.to_numeric(x.get("market_pick",x.get("consensus_rank")),errors="coerce")
                            x["pick"]=pd.to_numeric(x.mock_pick,errors="coerce")
                            x["reach"]=np.maximum(x["mp"]-x["pick"],0)
                        ...
                        rows.append({
                            ...
                            "R11_plus_change":r11,
                            "Champion_late_reach":late_c.reach.mean() if len(late_c) else np.nan,
                            "Candidate_late_reach":late_n.reach.mean() if len(late_n) else np.nan,
                            "Late_reach_change":(late_n.reach.mean()-late_c.reach.mean()) if len(late_n) and len(late_c) else np.nan,
                        ...
                        if done==1 or done%100==0 or done==total:
                            status.caption(f"V7.54 challenger: {done}/{total} fresh matched sets")

            df=pd.DataFrame(rows)
            st.session_state.v754_results=df
            status.success(f"V7.54 complete: {len(df)} fresh matched sets.")
            prog.empty()

        df=st.session_state.get("v754_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.markdown("#### V7.54 screening gate")
            st.dataframe(gate.round(4),use_container_width=True,hide_index=True)

            if screen:
                st.success(
                    "V7.54 SCREEN PASSED. The empirical late-survival curve improves whole-draft performance and R11+ while protecting R1-10. "
                    "Move this exact frozen curve into a larger V7.55 validation."
                )
            else:
                st.warning(
                    "V7.54 does not clear screening. Keep V7.50 frozen and do not promote the empirical curve without diagnosing the failed dimension."
                )

            st.download_button(
                "⬇️ Download V7.54 empirical-survival challenger CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_54_empirical_r11_survival_challenger.csv",
                "text/csv",key="v754_download"
            )
