...
st.title("🏈 Fantasy Edge V7.78 — Frozen Round-2 Dominance Validation")
st.caption("V7.78 • V7.67 frozen champion • optimized Round-2 validation")
st.caption("Build ID: V7.78-R2-DOMINANCE-VALIDATION-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.78 keeps V7.67 frozen/live and performs large fresh-seed validation of the exact V7.77 Round-2 dominance rule with zero tuning.")
        ...
        st.markdown("### ⚡ V7.78 — Frozen Round-2 Dominance Validation")
        st.caption(
            "Large-sample validation of the exact V7.77 rule with zero tuning. V7.67 remains frozen/live. "
            "Trigger-first execution runs V7.67 once per seed and reruns the challenger only when the true-board Round-2 dominance condition actually fires."
        )

        if "v778_results" not in st.session_state:
            st.session_state.v778_results=None

        sims=st.number_input(
            "V7.78 validation simulations per slot × room",
            min_value=50,max_value=150,value=100,step=10,key="v778_sims",
            help="Default: 4 slots × 5 rooms × 100 = 2,000 fresh matched sets. At ~9% activation, expect roughly 2,180 full simulations rather than 4,000."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.78 frozen validation",type="primary",key="v778_run")
        clear=b.button("Clear V7.78 validation",key="v778_clear")
        if clear:
            st.session_state.v778_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=978000+int(ds)*100000+j*263+sum(ord(c) for c in room_name)
                        ...
                        if len(r2):
                            ...
                            trigger=bool(int(pd.to_numeric(
                                pd.Series([rr.get("R777_Would_trigger",0)]),
                                errors="coerce"
                            ).fillna(0).iloc[0]))
                            champ2=str(rr.player)
                            champ2pos=str(rr.position)
                            alt=str(rr.get("R777_Best_alt_player",""))

                        if trigger:
                            cand=simulate_mock_fast(
                                pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name,
                                r5_dominance_override_enabled=False,
                                r2_dominance_override_enabled=True
                            )
                            reruns+=1
                        ...
                        rows.append({
                            ...
                            "Champion_round2_player":champ2,
                            "Champion_round2_position":champ2pos,
                            "Detected_best_alt":alt,
                            "Candidate_round2_player":cand2,
                            "Candidate_round2_position":cand2pos,
                        ...
                        if done==1 or done%250==0 or done==total:
                            status.caption(f"V7.78: {done}/{total} matched sets • challenger reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v778_results=df
            status.success(
                f"V7.78 complete: {len(df)} matched sets using "
                f"{len(df)+int(df.Challenger_rerun.sum())} full simulations instead of {len(df)*2}."
            )
            prog.empty()

        df=st.session_state.get("v778_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            paths=trig.groupby(
                ["Champion_round2_player","Champion_round2_position",
                 "Candidate_round2_player","Candidate_round2_position"],
                as_index=False
            ).agg(
            ...
            st.markdown("#### Frozen Round-2 override paths")
            ...
            positive_active_rooms=int((active_rooms.Overall_change>0).sum())
            ...
            certified=(
                ...
                and r11>-0.05
                and activation>=.02
                and pd.notna(cond) and cond>=.75
                and pd.notna(nonloss) and nonloss>=.80
                and positive_active_rooms>=max(1,len(active_rooms)-1)
            ...
            gate=pd.DataFrame([{
                ...
                "R1-3 positive":r13>0,
                "R4-6 protected":r46>-.05,
                "R7-10 protected":r710>-.05,
                "R11+ protected":r11>-.05,
                "Activation >=2%":activation>=.02,
                "Conditional win >=75%":pd.notna(cond) and cond>=.75,
                "Conditional non-loss >=80%":pd.notna(nonloss) and nonloss>=.80,
                "Positive active rooms":positive_active_rooms,
                "Active rooms":len(active_rooms),
                "Positive active slots":positive_active_slots,
                "Active slots":len(active_slots),
            }])
            st.markdown("#### V7.78 frozen validation gate")
            st.dataframe(gate,use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.78 CERTIFIED. The exact V7.77 Round-2 dominance rule survives large fresh-seed validation and is eligible for a final production-path promotion certification."
                )
            else:
                st.warning(
                    "V7.78 does not certify. Keep V7.67 champion and do not retune the Round-2 rule from this validation sample."
                )

            st.download_button(
                "⬇️ Download V7.78 frozen Round-2 validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_78_frozen_round2_dominance_validation.csv",
                "text/csv",key="v778_download"
            )
