...
st.title("🏈 Fantasy Edge V8.06 — Frozen Champion Whole-Draft Opportunity Audit")
st.caption("V8.06 • V8.05 frozen champion • whole-draft opportunity discovery")
st.caption("Build ID: V8.06-WHOLE-DRAFT-OPPORTUNITY-AUDIT-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        sims=st.number_input(
            "V8.06 simulations per slot × room",
        ...
        a,b=st.columns(2)
        run=a.button("Run V8.06 opportunity audit",type="primary",key="v806_run")
        clear=b.button("Clear V8.06 audit",key="v806_clear")
                ...
                for room_name in rooms:
                    for j in range(int(sims)):
                        ...
                        for idx,(_,r) in enumerate(d.sort_values("mock_round").iterrows()):
                            ...
                            if len(alt):
                                alt["_vorp"]=pd.to_numeric(alt.get("vorp",np.nan),errors="coerce")
                                alt["_score"]=pd.to_numeric(alt.get("draft_score",np.nan),errors="coerce")
                                alt["_market"]=pd.to_numeric(alt.get("market_pick",np.nan),errors="coerce")
                                alt=alt.sort_values(
                                    ["_score","_vorp","_market"],
                                    ascending=[False,False,True],na_position="last"
                                )
                                b=alt.iloc[0]
                                best_player=str(b.player); best_pos=str(b.position)
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V8.06: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v806_results=df
            status.success(f"V8.06 complete: {len(df):,} champion decisions audited.")
            prog.empty()
        ...
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.download_button(
                ...
                "fantasy_edge_v8_06_frozen_champion_whole_draft_opportunity_audit.csv",
