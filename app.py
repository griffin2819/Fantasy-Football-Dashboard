...
st.title("🏈 Fantasy Edge V7.63 — True-Board Round-7 Availability Audit")
st.caption("V7.63 • V7.60 frozen champion • true-board Round-7 diagnostic")
st.caption("Build ID: V7.63-R7-TRUEBOARD-20260816")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.63 leaves V7.60 completely frozen and fixes the Round-7 audit to use the actual alive+eligible room state after opponent picks.")
        ...
        st.markdown("### 🧪 V7.63 — True-Board Round-7 Availability Audit")
        st.caption(
            "Diagnostic only. V7.60 remains frozen champion. V7.63 fixes the V7.62 availability bug by capturing the actual alive+eligible room state "
            "immediately before the Round-7 selection, after every opponent pick. It records the top five genuinely available market-respecting alternatives."
        )

        if "v763_results" not in st.session_state:
            st.session_state.v763_results=None

        sims=st.number_input(
            "V7.63 simulations per slot × room",
            min_value=10,max_value=50,value=25,step=5,key="v763_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 fresh V7.60 champion drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run true-board Round-7 audit",type="primary",key="v763_run")
        clear=b.button("Clear V7.63 audit",key="v763_clear")
        if clear:
            st.session_state.v763_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=963000+int(ds)*100000+j*179+sum(ord(c) for c in room_name)
                        d=simulate_mock_fast(pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name)
                        ...
                        if len(r7):
                            ...
                            if pd.notna(reach) and reach>10 and int(pd.to_numeric(pd.Series([r.get("R763_Audit",0)]),errors="coerce").fillna(0).iloc[0])==1:
                                row={
                                    ...
                                    "Actual_available_count":r.get("R763_Actual_available_count",np.nan),
                                    "Market_respecting_count":r.get("R763_Market_respecting_count",np.nan),
                                }
                                for k in range(1,6):
                                    for fld in ["player","position","market_pick","model_rank","projection","vorp","draft_score"]:
                                        row[f"Alt{k}_{fld}"]=r.get(f"R763_Alt{k}_{fld}","" if fld in ["player","position"] else np.nan)
                                rows.append(row)

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%100==0 or done==total:
                            status.caption(f"V7.63 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v763_results=df
            status.success(f"V7.63 complete: {len(df)} true-board Round-7 reach triggers.")
            prog.empty()

        df=st.session_state.get("v763_results")
        if isinstance(df,pd.DataFrame) and len(df):
            # Calculate top-alternative deltas.
            for c in ["Champion_projection","Champion_vorp","Champion_draft_score","Alt1_projection","Alt1_vorp","Alt1_draft_score","Alt1_market_pick"]:
            ...
            champs=df.groupby(["Champion_player","Champion_position"],as_index=False).size().rename(columns={"size":"Triggers"}).sort_values("Triggers",ascending=False)
            st.markdown("#### Round-7 reach drivers")
            st.dataframe(champs.head(20),use_container_width=True,hide_index=True)

            alt1=df.groupby(["Alt1_player","Alt1_position"],as_index=False).agg(
            ...
            st.dataframe(alt1.head(25).round(3),use_container_width=True,hide_index=True)

            st.markdown("#### Sample top-5 true-board alternatives")
            showcols=["Draft_slot","Room","Round7_pick","Champion_player","Champion_reach"]
            for k in range(1,6):
            ...
            st.info(
                "V7.63 fixes only the audit. It does not force any alternative. If the true-board alternatives consistently preserve most of the champion's "
                "projection/VORP/draft-score while saving substantial market price, V7.64 can run an actual matched Round-7 challenger."
            )

            st.download_button(
                "⬇️ Download V7.63 true-board Round-7 audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_63_true_board_round7_availability_audit.csv",
                "text/csv",key="v763_download"
            )
        elif isinstance(df,pd.DataFrame):
            st.success("No Round-7 >10-pick reach triggers occurred in this run.")
