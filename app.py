    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                cr=np.where(np.isnan(consensus[sidx]),float(overall),consensus[sidx])
                # Correct market timing: reaching well AHEAD of consensus is a cost; a faller is a small bonus.
                reach=np.maximum(cr-float(overall),0)
                fall=np.maximum(float(overall)-cr,0)
                edge=np.clip(cr-model_rank[sidx],-30,30)
                # Positional marginal utility is the primary roster-construction signal.
                # Raw model score remains meaningful but cannot overwhelm bench opportunity cost.
...
st.title("🏈 Fantasy Edge V7.4 — Positional Marginal Utility")
st.caption("V7.4 • positional marginal utility • corrected market timing • balanced candidate coverage • frozen projections + grading")
st.caption("Build ID: V7.4-PMU-20260813")
...
with tabs[1]:
    ...
    cr=x.consensus_rank.fillna(current_overall)
    reach=np.maximum(cr-current_overall,0); fall=np.maximum(current_overall-cr,0)
    edge=np.clip(cr-x.model_rank,-30,30)
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                cr=avail.consensus_rank.fillna(overall)
                reach=np.maximum(cr-overall,0); fall=np.maximum(overall-cr,0)
                edge=np.clip(cr-avail.model_rank,-30,30)
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.4 validation report")
            st.caption("V7.4 keeps expected final-roster utility but recalibrates positional bench utility and fixes market timing so reaches are penalized and players falling past consensus receive a small value bonus. Projections and grading remain frozen.")
            ...
            st.download_button("⬇️ Download V7.4 diagnostic CSV",csv,"fantasy_edge_v7_4_diagnostics.csv","text/csv")
