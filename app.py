...
st.title("🏈 Fantasy Edge V7.68 — Frozen V7.67 Champion Audit")
st.caption("V7.68 • V7.67 frozen champion • next-opportunity diagnostic")
st.caption("Build ID: V7.68-FROZEN-CHAMPION-AUDIT-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.68 freezes the certified V7.67 production champion unchanged and audits fresh drafts for the next independent improvement opportunity.")
        ...
        st.markdown("### 🔒 V7.68 — Frozen Champion Next-Opportunity Audit")
        st.caption(
            "V7.67 is frozen as the production champion. This audit changes no draft behavior. "
            "It measures 1,000 fresh champion drafts across slots and room types to identify the next independent weakness "
            "without retuning the certified Round-7 dominance rule."
        )

        if "v768_results" not in st.session_state:
            st.session_state.v768_results=None

        sims=st.number_input(
            "V7.68 audit simulations per slot × room",
            min_value=25,max_value=100,value=50,step=5,key="v768_sims",
            help="Default: 4 slots × 5 rooms × 50 = 1,000 fresh V7.67 champion drafts. Only one simulation per seed is required."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.68 next-opportunity audit",type="primary",key="v768_run")
        clear=b.button("Clear V7.68 audit",key="v768_clear")
        if clear:
            st.session_state.v768_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=968000+int(ds)*100000+j*199+sum(ord(c) for c in room_name)
                        ...
                        for _,r in d.iterrows():
                            ...
                            rows.append({
                                ...
                                "Round":rnd,"Phase":phase_name,"Pick":pick,
                                "Player":str(r.get("player","")),"Position":str(r.get("position","")),
                                ...
                                "Phase_avg_market_delta":phase.loc[phase_name,"Avg_market_delta"] if phase_name in phase.index else np.nan,
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V7.68 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v768_results=df
            status.success(f"V7.68 complete: {total} champion drafts / {len(df)} individual decisions.")
            prog.empty()

        df=st.session_state.get("v768_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            reaches=d[d.Reach10].groupby(["Round","Player","Position"],as_index=False).agg(
                Decisions=("Player","count"),
                Avg_market_delta=("Market_delta","mean"),
            ...
            st.dataframe(reaches.head(40).round(3),use_container_width=True,hide_index=True)
            ...
            st.info(
                "V7.68 is measurement-only. Use the largest repeated weakness from this audit as the next diagnostic target. "
                "The V7.67 Round-7 dominance rule stays frozen and should not be retuned against this sample."
            )

            st.download_button(
                "⬇️ Download V7.68 frozen champion next-opportunity audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_68_frozen_champion_next_opportunity_audit.csv",
                "text/csv",key="v768_download"
            )
