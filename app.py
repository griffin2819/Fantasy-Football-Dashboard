...
def _v737_required_position_emergencies(roster_df, news_df=None):
    ...
    for pos,floor in req.items():
        ...
        for _,r in sub.iterrows():
            stx=_v677_normalize_injury_status(r.get("injury_effective",r.get("injury","")))
            risk=str(news_risk.get(_owner_key(r.get("player","")),"")).upper()
            high_news=(risk=="HIGH")
            # V7.47: availability and contingency are different questions. A sole
            # required-position option who is QUESTIONABLE, or has credible ELEVATED
            # breaking-news risk, should trigger preparation without rewriting the
            # official injury status. With real depth behind him, normal coverage stands.
            sole_required=(len(sub)<=floor)
            contingency_risk=sole_required and (stx=="QUESTIONABLE" or risk in ("HIGH","ELEVATED"))
            if stx not in bad and not high_news and not contingency_risk:
                playable+=1
        if playable<floor:
            out.add(pos)
    return out
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        st.caption("V7.47 valuation-integrity engine: sole required-position QUESTIONABLE/news-risk players trigger contingency preparation; drop ranking preserves positional depth and no longer treats the bare starter minimum as surplus.")
        ...
        for _,add_raw in _fa_eval.iterrows():
            ...
            for _,drop in _drop_pool.iterrows():
                ...
                _moves.append({
                    ...
                    "Need Adj":_need_adj,"Need Fit":("REQUIRED POSITION CONTINGENCY" if _is_emergency_move else _need_label),"Evidence":int(_evidence),
