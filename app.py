...
def validate_position(player, pos):
    # Guardrail for known franchise-level offensive players if an upstream fuzzy merge ever collides.
    # General solution still relies on canonical source positions first.
    p=canonical_position(pos)
    known_qbs={"Lamar Jackson","Josh Allen","Patrick Mahomes","Jalen Hurts","Joe Burrow","Justin Herbert",
               "Drake Maye","Brock Purdy","Caleb Williams","Jayden Daniels","C.J. Stroud"}
    if player in known_qbs:
        return "QB"
    return p
    ...
    for overall in range(1,total+1):
        ...
        if owner_slot==slot:
            ...
            if model_user:
                ...
                avail["sim_score"]=avail.draft_score+avail["need"]-np.maximum(overall-avail.consensus_rank.fillna(overall),0)*.08
            ...
            # Opponents mostly follow consensus/ADP, with realistic draft-room variance.
            noise=rng.normal(0,randomness,len(avail))
            avail["opp_score"]=avail.sim_consensus+noise
            # Prevent extreme early IDP runs in 1-DL/1-DB leagues.
            avail.loc[avail.position.isin(["DL","DB"]),"opp_score"] += max(0,55-overall)*.35
            choice=avail.sort_values("opp_score").iloc[0]
        drafted.append(choice.player)
    return pd.DataFrame(user_rows)

state=load_state()

st.title("🏈 Fantasy Edge V6.1 — Validation Fix")
st.caption("Validation release • WR-aware roster construction • stricter grading • position safeguards • Yahoo + IDP")
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                avail["live"]=avail.draft_score+avail["need"]
                if rnd<=7:
