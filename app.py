...
st.set_page_config(page_title="Fantasy Edge v9.50 — Exact League + Kicker Requirement", page_icon="🏈", layout="wide")
...
def v9_live_rank(avail, round_no, current_pick, roster_counts):
    """Adaptive live scorer for Fantasy Edge.

    The old certified v9 base score is intentionally NOT authoritative here.
    It is retained only as a tiny tie-breaker so Live/Mock decisions respond to
    the current board, current roster, current price and current player quality.
    """
    pb,cfg=load_v9_production()
    x=avail.copy()

    def _pct(s, ascending=False):
        v=pd.to_numeric(s,errors="coerce")
        # Higher is better unless ascending=True (market rank/pick).
        return v.rank(pct=True,ascending=ascending).fillna(.50)

    proj=pd.to_numeric(x.get("projection",0),errors="coerce").fillna(0.0)
    vorp=pd.to_numeric(x.get("vorp",0),errors="coerce").fillna(0.0)
    ds=pd.to_numeric(x.get("draft_score",0),errors="coerce").fillna(0.0)
    mp=pd.to_numeric(
        x.get("consensus_rank",x.get("market_pick",np.nan)),
        errors="coerce"
    ).fillna(pd.to_numeric(x.get("market_pick",current_pick),errors="coerce")).fillna(float(current_pick))

    proj_q=_pct(proj)
    vorp_q=_pct(vorp)
    draft_q=_pct(ds)

    # Current-board price context. Falling is useful only in a bounded way.
    fall=np.maximum(float(current_pick)-mp,0.0)
    reach=np.maximum(mp-float(current_pick),0.0)
    timing=np.clip(fall/24.0,0.0,1.25)-np.clip(reach/18.0,0.0,1.10)

    caps={"QB":(1,2),"RB":(2,99),"WR":(2,99),"TE":(1,2),"K":(1,1),"DL":(1,2),"DB":(1,2)}
    need=[]
    for p in x["position"].astype(str):
        lo,hi=caps.get(p,(0,99))
        cnt=int(roster_counts.get(p,0))
        need.append(1.0 if cnt<lo else 0.0)
    need=np.asarray(need,dtype=float)

    # Injury/status uncertainty must reduce quality rather than merely subtracting
    # a tiny final-score penalty. Manual overrides and Sleeper status feed both
    # flow through injury_effective.
    status=x.get("injury_effective",x.get("injury",pd.Series("",index=x.index))).fillna("").astype(str).str.lower()
    risk_mult=np.ones(len(x),dtype=float)
    risk_mult[np.asarray(status.isin(["questionable","q"]))]=0.90
    risk_mult[np.asarray(status.isin(["doubtful","d"]))]=0.76
    risk_mult[np.asarray(status.isin(["pup","reserve/pup"]))]=0.68
    risk_mult[np.asarray(status.isin(["out","o","suspended","susp"]))]=0.40
    risk_mult[np.asarray(status.isin(["ir","reserve/ir","nfi","reserve/nfi","do not draft","dnr","avoid"]))]=0.05
    x["risk_multiplier"]=risk_mult
    x["risk_adjusted_vorp"]=vorp*risk_mult

    risk_vorp_q=_pct(x["risk_adjusted_vorp"])
    current_quality=(
        0.28*draft_q +
        0.26*risk_vorp_q +
        0.22*proj_q +
    ...
    # Dynamic score is on roughly the same practical scale as the previous live
    # score, but no static identity prior can dominate it.
    score=5.0 + 13.0*current_quality + 1.60*timing + 1.25*need

    # Certified history is only a tie-breaker now.
    if "v9_base_live_score" in x.columns:
        cert=pd.to_numeric(x["v9_base_live_score"],errors="coerce").fillna(0.0)
        score += 0.12*np.clip(cert,-4.0,4.0)

    x["v9_live_score"]=score
    x["frozen_player_prior_weight"]=0.0
    x["live_quality_weight"]=13.0
    x["v9_market_pick_effective"]=mp
    x["adaptive_live_score"]=score
    return x
...
def _v944_add_idp_display_metrics(x):
    ...
    # Show true marginal roster utility for IDP.  roster_opportunity_adj is a timing/
    # construction overlay and could make an elite backup defender display -30 or worse
    # even when adding him actually improves the roster.
    if "roster_delta" in y.columns:
        y.loc[mask,"display_vorp"]=pd.to_numeric(y.loc[mask,"roster_delta"],errors="coerce")
    else:
        y.loc[mask,"display_vorp"]=pd.to_numeric(y.loc[mask,"vorp"],errors="coerce")
    y.loc[mask,"display_model_rank"]=pd.to_numeric(y.loc[mask,"idp_external_rank"],errors="coerce")
    return y

def _apply_player_pool_authority(avail):
    """Hard eligibility gate before any recommendation scoring.

    Fantasy Edge should never 'score its way out of' bad source data. Unsigned/free-agent
    players are removed before ranking; kickers additionally need a current team and a
    usable K-specific market rank. The returned audit columns feed pre-draft certification.
    """
    x=avail.copy()
    team=x.get("team",pd.Series("",index=x.index)).fillna("").astype(str).str.upper().str.strip()
    pos=x.get("position",pd.Series("",index=x.index)).fillna("").astype(str).str.upper().str.strip()
    status=x.get("injury_effective",x.get("injury",pd.Series("",index=x.index))).fillna("").astype(str).str.lower().str.strip()
    bad_team=team.isin(["","FA","FREE AGENT","NONE","NAN","UNK","UNKNOWN"])
    terminal=status.isin(["retired","waived","released","cut","reserve/retired","do not draft","dnr","avoid"])
    x["pool_eligible"]=~bad_team & ~terminal
    x["pool_exclusion_reason"]=""
    x.loc[bad_team,"pool_exclusion_reason"]="no active NFL team / free agent"
    x.loc[terminal,"pool_exclusion_reason"]="inactive roster status"

    # Kicker authority: a kicker must be attached to an NFL team and carry a current
    # K-specific consensus rank. Missing-rank camp bodies/free agents cannot become K1
    # simply because the roster is forced to fill K in the last round.
    kr=pd.to_numeric(x.get("kicker_rank",x.get("consensus_rank",np.nan)),errors="coerce")
    k=pos.eq("K")
    bad_k=k & (bad_team | kr.isna() | (kr<1) | (kr>40))
    x.loc[bad_k,"pool_eligible"]=False
    x.loc[bad_k,"pool_exclusion_reason"]="unverified kicker job / missing current K rank"
    x["kicker_job_verified"]=np.where(k,~bad_k,True)
    return x[x["pool_eligible"]].copy()




# --- Fantasy Edge v9.55 certified production universe -----------------------
# Kicker authority snapshot: FantasyPros 2026 redraft K consensus, refreshed
# 2026-08-22. This is intentionally explicit so a dirty raw roster feed cannot
# turn an unsigned camp kicker into a draft recommendation.
_V954_KICKER_AUTHORITY = [
    ("Brandon Aubrey","DAL",1),("Cameron Dicker","LAC",2),("Ka'imi Fairbairn","HOU",3),
    ("Cam Little","JAC",4),("Jason Myers","SEA",5),("Eddy Pineiro","SF",6),
    ("Tyler Loop","BAL",7),("Evan McPherson","CIN",8),("Cairo Santos","CHI",9),
    ("Jake Bates","DET",10),("Chase McLaughlin","TB",11),("Harrison Mevis","LAR",12),
    ("Andy Borregales","NE",13),("Chris Boswell","PIT",14),("Harrison Butker","KC",15),
    ("Will Reichard","MIN",16),("Wil Lutz","DEN",17),("Charlie Smyth","NO",18),
    ("Jake Elliott","PHI",19),("Blake Grupe","IND",20),("Tyler Bass","BUF",21),
    ("Chad Ryland","ARI",22),("Joey Slye","TEN",23),("Nick Folk","ATL",24),
]

def _v954_kicker_authority_overlay(board, teams=12):
    """Replace raw K ambiguity with a small, explicit 2026 draftable K universe.

    Existing Sleeper rows are updated when present. Missing authority kickers are
    injected with conservative projections so the draft never depends on whether
    a roster feed happened to attach a team to that kicker on this refresh.
    """
    ...
    auth={norm(n):(n,t,int(r)) for n,t,r in _V954_KICKER_AUTHORITY}

    # Remove every raw kicker that is not on the current authority list. This is
    # stronger than scoring/filtering and prevents FA/camp rows from resurfacing.
    is_k=x["position"].astype(str).str.upper().eq("K")
    keep_k=x["key"].astype(str).isin(auth)
    x=x[~is_k | keep_k].copy()

    existing=set(x["key"].astype(str))
    add=[]
    for key,(name,team,rank) in auth.items():
        # K1 ~9.8 PPG; K12 ~8.48; K24 ~7.04. K market cost is late-round overall,
        # not positional rank interpreted as pick 1-24.
        proj=float(max(6.80,9.80-0.12*(rank-1)))
        market=float(178.0+1.35*(rank-1))
        repl_rank=max(int(teams),1)
        repl=float(max(6.80,9.80-0.12*(repl_rank-1)))
        vorp=float(proj-repl)
        if key in existing:
            m=x["key"].astype(str).eq(key)
            vals={"player":name,"team":team,"position":"K","raw_position":"K",
                  "kicker_rank":float(rank),"projection":proj,"market_pick":market,
                  "consensus_rank":market,"consensus_page":"v9.54 kicker authority",
                  "replacement_ppg":repl,"vorp":vorp,"kicker_job_verified":True,
                  "pool_eligible":True,"market_authority_pick":market,
                  "market_authority_source":"2026 kicker ECR"}
            for c,v in vals.items():
                if c not in x.columns:
                    if isinstance(v,str): x[c]=pd.Series("",index=x.index,dtype=object)
                    elif isinstance(v,(bool,np.bool_)): x[c]=pd.Series(False,index=x.index,dtype=bool)
                    else: x[c]=np.nan
                x.loc[m,c]=v
        else:
            d={c:np.nan for c in x.columns}
            d.update({"id":"v954:k:"+key,"player":name,"key":key,"position":"K",
                      "raw_position":"K","team":team,"age":np.nan,"exp":np.nan,"injury":"",
                      "projection":proj,"replacement_ppg":repl,"vorp":vorp,
                      "progression":50.0,"regression":50.0,"pure_model_score":vorp*9+proj*1.6-1.25,
                      "draft_score":vorp*9+proj*1.6-1.25,"waiver_score":proj*4,
                      "model_rank":market,"consensus_rank":market,
                      "consensus_page":"v9.54 kicker authority","market_pick":market,
                      "kicker_rank":float(rank),"confidence":0.90,"kicker_job_verified":True,
                      "pool_eligible":True,"market_authority_pick":market,
                      "market_authority_source":"2026 kicker ECR",
                      "identity_key":"v954:k:"+key+"|K"})
            add.append(d)
    if add:
        x=pd.concat([x,pd.DataFrame(add)],ignore_index=True,sort=False)
    return x.drop_duplicates(subset=["key","position"],keep="first").reset_index(drop=True)

def _v954_market_authority(board):
    """Create one usable market-cost field with transparent source precedence.

    Current consensus wins. The certified production market is only a fallback for
    offensive players when the live ranking feed fails to match a player. IDP/K have
    their own positional authority layers.
    """
    x=board.copy()
    pos=x["position"].astype(str).str.upper()
    cur=pd.to_numeric(x.get("consensus_rank",np.nan),errors="coerce")
    old=pd.to_numeric(x.get("v9_market_pick",np.nan),errors="coerce")
    mp=pd.to_numeric(x.get("market_pick",np.nan),errors="coerce")
    authority=cur.copy()
    source=pd.Series(np.where(cur.notna(),"current consensus",""),index=x.index,dtype=object)
    offense=pos.isin(["QB","RB","WR","TE"])
    use_old=offense & authority.isna() & old.notna() & old.between(1,300,inclusive="both")
    authority.loc[use_old]=old.loc[use_old]
    source.loc[use_old]="certified market fallback"
    use_mp=authority.isna() & mp.notna() & mp.between(1,300,inclusive="both")
    authority.loc[use_mp]=mp.loc[use_mp]
    source.loc[use_mp]="position authority"
    x["market_authority_pick"]=authority
    x["market_authority_source"]=source
    # All execution/value code should see the authoritative cost. Keep the original
    # consensus column for diagnostics while normalizing market_pick for decisions.
    x["market_pick"]=authority.where(authority.notna(),mp)
    return x

def _v954_curated_draft_universe(board):
    """Build the actual draft universe first; score only after this gate.

    Raw Sleeper/nflverse rows are ingestion data, not a fantasy draft board. The
    curated universe has enough depth for a 12-team/17-round room while excluding
    thousands of unsigned, fringe, and unsupported rows that were poisoning audits.
    """
    if board is None or len(board)==0:
        return board
    x=board.copy()
    ...
    bad_team=team.isin(["","FA","FREE AGENT","NONE","NAN","UNK","UNKNOWN"])
    terminal=status.isin(["retired","waived","released","cut","reserve/retired","do not draft","dnr","avoid"])
    x=x[~bad_team & ~terminal & pos.isin(["QB","RB","WR","TE","K","DL","DB"])].copy()

    # K is already an explicit 24-player authority universe.
    k=x[x.position.eq("K")].copy()

    # Offensive board: preserve every player with credible current/certified market
    # support, then fill to safe position depth using the model only when necessary.
    caps={"QB":40,"RB":72,"WR":84,"TE":36}
    floors={"QB":28,"RB":48,"WR":60,"TE":24}
    off_parts=[]
    for p in ["QB","RB","WR","TE"]:
        g=x[x.position.eq(p)].copy()
        auth=pd.to_numeric(g.get("market_authority_pick",np.nan),errors="coerce")
        g["_auth"]=auth
        supported=g[g["_auth"].between(1,260,inclusive="both")].copy()
        supported=supported.sort_values(["_auth","draft_score"],ascending=[True,False]).head(caps[p])
        if len(supported)<floors[p]:
            need=floors[p]-len(supported)
            extra=g[~g.index.isin(supported.index)].sort_values("draft_score",ascending=False).head(need)
            supported=pd.concat([supported,extra],ignore_index=False)
        off_parts.append(supported.head(caps[p]))

    # IDP: the unified positional authority layer supplies the names we trust. Keep
    # enough depth for starters + one impact backup without thousands of fringe rows.
    idp_parts=[]
    for p,cap in [("DL",48),("DB",48)]:
        g=x[x.position.eq(p)].copy()
        ext=pd.to_numeric(g.get("idp_external_rank",pd.Series(np.nan,index=g.index)),errors="coerce")
        if ext.notna().any():
            g["_idpr"]=ext
            g=g.sort_values(["_idpr","draft_score"],ascending=[True,False])
        else:
            g=g.sort_values(["market_pick","draft_score"],ascending=[True,False])
        idp_parts.append(g.head(cap))

    out=pd.concat(off_parts+[k]+idp_parts,ignore_index=True,sort=False)
    out=out.drop_duplicates(subset=["key","position"],keep="first").reset_index(drop=True)
    out["curated_draftable"]=True
    out.drop(columns=["_auth","_idpr"],errors="ignore",inplace=True)
    return out


def _bench_saturation_penalty(row, counts, round_no):
    """Diminishing marginal utility for late bench depth."""
    p=str(row.get("position",""))
    have=int(counts.get(p,0))
    svd=float(pd.to_numeric(pd.Series([row.get("starter_vorp_delta",0)]),errors="coerce").fillna(0).iloc[0])
    rd=float(pd.to_numeric(pd.Series([row.get("roster_delta",0)]),errors="coerce").fillna(0).iloc[0])
    if p=="RB":
        base={0:0,1:0,2:0,3:1.5,4:6.0,5:15.0}.get(have,24.0)
    elif p=="WR":
        base={0:0,1:0,2:0,3:0.5,4:4.0,5:10.0}.get(have,18.0)
    elif p=="QB": base=16.0 if have>=1 else 0.0
    elif p=="TE": base=12.0 if have>=1 else 0.0
    else: base=0.0
    if int(round_no)>=11 and svd<=0.25 and rd<=1.0 and p in ["RB","WR"]:
        base+=6.0
    return float(base)


def board_certification_summary(board, slots):
    """Certify the *actual curated draft universe*, not raw ingestion rows."""
    if board is None or len(board)==0:
        return pd.DataFrame([{"Check":"Curated player board","Status":"FAIL","Detail":"Board is empty"}])
    b=board.copy()
    team=b.get("team",pd.Series("",index=b.index)).fillna("").astype(str).str.upper().str.strip()
    pos=b.get("position",pd.Series("",index=b.index)).fillna("").astype(str).str.upper()
    auth=pd.to_numeric(b.get("market_authority_pick",b.get("market_pick",np.nan)),errors="coerce")
    kr=pd.to_numeric(b.get("kicker_rank",np.nan),errors="coerce")
    fa=team.isin(["","FA","FREE AGENT","NONE","NAN","UNK","UNKNOWN"])
    k=pos.eq("K")
    k_verified=k & ~fa & kr.between(1,32,inclusive="both")
    dup=int(b.duplicated(subset=["player","position"],keep=False).sum()) if "player" in b.columns else 0
    offense=pos.isin(["QB","RB","WR","TE"])
    cov=float(auth[offense].notna().mean()) if offense.any() else 0.0
    counts=pos.value_counts().to_dict()
    rows=[
        {"Check":"Curated free agents","Status":"PASS" if int(fa.sum())==0 else "FAIL","Detail":f"{int(fa.sum())} draftable rows without an active team"},
        {"Check":"Verified kickers","Status":"PASS" if int(k_verified.sum())>=20 else "FAIL","Detail":f"{int(k_verified.sum())} authority-ranked, team-attached kickers"},
        {"Check":"Kicker contamination","Status":"PASS" if int((k & ~k_verified).sum())==0 else "FAIL","Detail":f"{int((k & ~k_verified).sum())} unverified kickers remain in curated pool"},
        {"Check":"Duplicate player-position rows","Status":"PASS" if dup==0 else "WARN","Detail":str(dup)},
        {"Check":"Market coverage offense","Status":"PASS" if cov>=.90 else ("WARN" if cov>=.80 else "FAIL"),"Detail":f"{100*cov:.0f}% of curated QB/RB/WR/TE have current or certified market cost"},
        {"Check":"Offensive depth","Status":"PASS" if counts.get("RB",0)>=40 and counts.get("WR",0)>=50 and counts.get("TE",0)>=18 and counts.get("QB",0)>=20 else "WARN","Detail":f"QB {counts.get('QB',0)} • RB {counts.get('RB',0)} • WR {counts.get('WR',0)} • TE {counts.get('TE',0)}"},
        {"Check":"IDP authority depth","Status":"PASS" if counts.get("DL",0)>=20 and counts.get("DB",0)>=20 else "WARN","Detail":f"DL {counts.get('DL',0)} • DB {counts.get('DB',0)}"},
        {"Check":"Total certified universe","Status":"PASS" if len(b)>=204 else "WARN","Detail":f"{len(b)} draftable players for a 204-pick room"},
    ]
    return pd.DataFrame(rows)



def _starter_slot_deficit_bonus_df(x, roster, slots, round_no):
    """Reward candidates that repair the weakest *starting* slots, not bench counts.

    The bonus is strongest while required RB/WR/TE starters or FLEX-quality skill slots
    are still weak. It deliberately penalizes RB3/RB4 from crowding out WR1/WR2 when
    the receiving foundation is incomplete.
    """
    if x is None or len(x)==0:
        return pd.Series(dtype=float)
    counts=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    pos=x['position'].astype(str)
    svd=pd.to_numeric(x.get('starter_vorp_delta',0),errors='coerce').fillna(0.0)
    bonus=pd.Series(0.0,index=x.index,dtype=float)
    rnd=int(round_no)
    req={p:int(slots.get(p,0)) for p in ['QB','RB','WR','TE','K','DL','DB']}

    # Required-starter deficits get explicit authority.
    for p in ['QB','RB','WR','TE']:
        missing=max(req.get(p,0)-int(counts.get(p,0)),0)
        if missing:
            base={'QB':2.0,'RB':4.0,'WR':6.5,'TE':3.5}.get(p,2.0)
            bonus.loc[pos.eq(p)] += base + 1.8*min(missing,2)

    # Two-WR foundation is especially important in a 2 WR + 2 FLEX PPR build.
    wr=int(counts.get('WR',0)); rb=int(counts.get('RB',0)); te=int(counts.get('TE',0))
    if rnd<=8 and wr<2:
        bonus.loc[pos.eq('WR')] += 4.5 + 1.5*(2-wr)
        if rb>=2:
            bonus.loc[pos.eq('RB')] -= 5.0 + 2.5*max(0,rb-2)
    if rnd<=7 and te<1:
        bonus.loc[pos.eq('TE')] += 2.0

    # FLEX-quality upgrades matter even after nominal starters are technically filled.
    skill=pos.isin(['RB','WR','TE'])
    bonus.loc[skill] += np.clip(svd.loc[skill],0,6)*0.85
    return bonus


def _idp_authority_reach_adjustment_df(x, current_pick):
    """IDP authority and reach control.

    A missing external positional rank is a data failure, not permission for roster need
    to elevate the defender. Elite defenders may be taken modestly early; ordinary IDPs
    pay a progressively larger reach cost.
    """
    if x is None or len(x)==0:
        return pd.Series(dtype=float), pd.Series(dtype=bool), pd.Series(dtype=float)
    pos=x['position'].astype(str).str.upper()
    is_idp=pos.isin(['DL','DB'])
    ext=pd.to_numeric(x.get('idp_external_rank',np.nan),errors='coerce')
    tier=pd.to_numeric(x.get('idp_quality_tier',5),errors='coerce').fillna(5)
    mp=pd.to_numeric(x.get('market_pick',x.get('consensus_rank',np.nan)),errors='coerce')
    reach=(mp-float(current_pick)).clip(lower=0.0)
    missing=is_idp & ext.isna()
    elite=is_idp & (tier<=2) & ext.notna() & (((pos=='DL') & (ext<=8)) | ((pos=='DB') & (ext<=10)))
    free=np.where(elite,14.0,8.0)
    penalty=pd.Series(0.0,index=x.index,dtype=float)
    penalty.loc[is_idp]=np.minimum(18.0,np.maximum(reach.loc[is_idp]-free[is_idp.to_numpy()],0.0)*0.48)
    # Truly elite IDP can justify some scarcity reach, but not 25+ picks automatically.
    penalty.loc[elite] *= 0.45
    return penalty,missing,reach


def _te2_expected_utility_adjustment_df(x, roster, current_pick, round_no):
    """TE2 must earn a roster spot through weekly utility, not name/ADP alone."""
    if x is None or len(x)==0:
        return pd.Series(dtype=float)
    counts=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    out=pd.Series(0.0,index=x.index,dtype=float)
    if int(counts.get('TE',0))<1:
        return out
    mask=x['position'].astype(str).eq('TE')
    if not mask.any():
        return out
    svd=pd.to_numeric(x.get('starter_vorp_delta',0),errors='coerce').fillna(0.0)
    rd=pd.to_numeric(x.get('roster_delta',0),errors='coerce').fillna(0.0)
    mp=pd.to_numeric(x.get('market_pick',np.nan),errors='coerce')
    fall=(float(current_pick)-mp).clip(lower=0.0)
    # Default TE2 is a luxury. Strong starter/FLEX gain or genuine late absurd value can earn back utility.
    out.loc[mask]=-8.0
    useful=mask & ((svd>=0.75) | (rd>=2.4))
    out.loc[useful]+=6.0
    late_value=mask & (int(round_no)>=13) & (fall>=28) & (rd>=1.3)
    out.loc[late_value]+=4.0
    return out


def _opponent_market_expiration_score(base, overall, noise):
    """Consensus-led opponent score with nonlinear ADP expiration.

    Lower score is drafted first. Once a player is 20+ picks overdue, passing probability
    collapses quickly; 40+ pick survivors become exceptional instead of routine.
    """
    b=np.asarray(base,dtype=float)
    n=np.asarray(noise,dtype=float)
    overdue=np.maximum(float(overall)-b,0.0)
    pressure=(0.55*np.minimum(overdue,20.0)
              +1.35*np.minimum(np.maximum(overdue-20.0,0.0),20.0)
              +2.75*np.maximum(overdue-40.0,0.0))
    return b+n-pressure


def mock_quality_certification(roster, previous_players=None):
    """Post-mock realism audit. Keeps impossible ADP gifts from masquerading as model skill."""
    if roster is None or len(roster)==0:
        return pd.DataFrame([{'Check':'Mock completed','Status':'FAIL','Detail':'No drafted roster'}])
    r=roster.copy()
    pick=pd.to_numeric(r.get('mock_pick',np.nan),errors='coerce')
    rnd=pd.to_numeric(r.get('mock_round',np.nan),errors='coerce')
    mp=pd.to_numeric(r.get('market_pick',r.get('consensus_rank',np.nan)),errors='coerce')
    delta=pick-mp
    over30=int((delta>30).sum()); over50=int((delta>50).sum())
    med=float(delta.abs().median()) if delta.notna().any() else np.nan
    early_reach=int((delta<-20).sum())
    pos=r.get('position',pd.Series('',index=r.index)).astype(str)
    idp_early=int((pos.isin(['DL','DB']) & (rnd<8)).sum())
    k_early=int((pos.eq('K') & (rnd<13)).sum())
    rows=[
        {'Check':'Players >30 picks past market','Status':'PASS' if over30<=2 else 'WARN','Detail':str(over30)},
        {'Check':'Players >50 picks past market','Status':'PASS' if over50==0 else 'FAIL','Detail':str(over50)},
        {'Check':'Median absolute market error','Status':'PASS' if pd.notna(med) and med<=18 else 'WARN','Detail':'—' if pd.isna(med) else f'{med:.1f} picks'},
        {'Check':'Major reaches (>20 early)','Status':'PASS' if early_reach<=2 else 'WARN','Detail':str(early_reach)},
        {'Check':'Premium-round IDP timing','Status':'PASS' if idp_early==0 else 'WARN','Detail':f'{idp_early} IDP before Round 8'},
        {'Check':'Kicker timing','Status':'PASS' if k_early==0 else 'WARN','Detail':f'{k_early} K before Round 13'},
    ]
    if previous_players:
        cur=set(r.get('player',pd.Series([],dtype=str)).astype(str))
        prev=set(str(v) for v in previous_players)
        denom=max(1,min(len(cur),len(prev)))
        repeat=len(cur & prev)/denom
        rows.append({'Check':'Repeat rate vs previous mock','Status':'PASS' if repeat<=0.50 else 'WARN','Detail':f'{repeat:.0%} ({len(cur & prev)}/{denom})'})
    return pd.DataFrame(rows)
...
def prepare_user_draft_candidates(avail, roster, round_no, current_pick, slot, teams, slots, randomness):
    ...
    x=avail.copy()
    x=_apply_player_pool_authority(x)
    ...
    if _rb>=4:
        x.loc[x.position.eq("RB"),"_construction_bonus"]-=10.0

    # Once RB5 is already rostered, prefer a real impact defender over RB6 depth.
    if _rb>=5:
        x.loc[x.position.eq("RB"),"_construction_bonus"]-=18.0
        _impact_idp=(
            x.position.isin(["DL","DB"]) &
            x.get("idp_eligible",pd.Series(False,index=x.index)).fillna(False).astype(bool)
        )
        if "idp_quality_tier" in x.columns:
            _impact_idp &= pd.to_numeric(x["idp_quality_tier"],errors="coerce").fillna(5)<=2
        if _impact_idp.any():
            x.loc[_impact_idp,"_construction_bonus"]+=14.0
            x.loc[_impact_idp,"impact_idp_depth_bonus"]=14.0
            x.loc[x.position.eq("RB"),"rb6_depth_penalty"]=-18.0
    ...
    x["roster_delta"]=_candidate_roster_delta_df(roster,x,slots,exact_cap=64)
    x["starter_vorp_delta"]=_candidate_starter_vorp_delta_df(roster,x,slots,exact_cap=96)
    x["current_starter_vorp"]=_starter_vorp_total_df(roster,slots)
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    x["evaluation_score"]-=pd.to_numeric(x.get("injury_penalty",0),errors="coerce").fillna(0)

    # Kicker-specific score. K rank + verified job control the position; generic
    # offensive VORP/model rank cannot make a camp/free-agent kicker the final pick.
    _k=x["position"].astype(str).str.upper().eq("K")
    _kr=pd.to_numeric(x.get("kicker_rank",x.get("consensus_rank",np.nan)),errors="coerce")
    x["kicker_authority_bonus"]=0.0
    _valid_k=_k & _kr.between(1,40,inclusive="both") & x.get("kicker_job_verified",pd.Series(True,index=x.index)).fillna(False).astype(bool)
    x.loc[_valid_k,"kicker_authority_bonus"]=np.clip(13.0-0.55*(_kr[_valid_k]-1.0),-4.0,13.0)
    x.loc[_valid_k,"evaluation_score"]+=x.loc[_valid_k,"kicker_authority_bonus"]
    x.loc[_k & ~_valid_k,"evaluation_score"]-=1000.0

    # Fantasy Edge roster-balance fix:
    # prepare_user_draft_candidates() already computes construction bonuses/penalties.
    # They must actually affect the authoritative score in BOTH Live and Mock.
    x["evaluation_score"]+=pd.to_numeric(
        x.get("_construction_bonus",pd.Series(0.0,index=x.index)),
        errors="coerce"
    ).fillna(0.0)

    # v9.51 marginal-value authority.
    # The previous build computed roster_delta but never used it. Make the actual
    # marginal roster gain authoritative, then directly reward candidates that
    # improve the best starting lineup/FLEX rather than merely adding bench depth.
    _rd=pd.to_numeric(x.get("roster_delta",0),errors="coerce").fillna(0.0)
    _svd=pd.to_numeric(x.get("starter_vorp_delta",0),errors="coerce").fillna(0.0)
    _csv=float(pd.to_numeric(x.get("current_starter_vorp",pd.Series([0.0])),errors="coerce").fillna(0.0).iloc[0])
    _starter_gap=max(0.0,50.0-_csv)
    _sv_mult=2.35 if int(round_no)<=10 else 1.65
    _sv_mult += min(1.10,(_starter_gap/50.0)*1.10)
    x["roster_delta_bonus"]=np.clip(_rd,-12.0,18.0)*0.70
    x["starter_vorp_bonus"]=np.clip(_svd,-8.0,12.0)*_sv_mult
    x["starter_vorp_target_gap"]=_starter_gap
    x["evaluation_score"]+=x["roster_delta_bonus"]+x["starter_vorp_bonus"]

    # v9.53 opportunity cost: compare every candidate with the best marginal lineup/
    # roster improvement still available at this pick. This prevents RB5/WR6 from
    # winning while a materially better starter/FLEX/required-position upgrade exists.
    _best_svd=float(_svd.max()) if len(_svd) else 0.0
    _best_rd=float(_rd.max()) if len(_rd) else 0.0
    _sv_gap=np.maximum(_best_svd-_svd,0.0)
    _rd_gap=np.maximum(_best_rd-_rd,0.0)
    _opp_mult=1.05 if int(round_no)>=11 else 0.72
    x["opportunity_cost_penalty"]=_opp_mult*(1.25*_sv_gap+0.45*_rd_gap)
    x["evaluation_score"]-=x["opportunity_cost_penalty"]

    # v9.55 Starter-VORP rescue. When the starting lineup is still below the 50
    # target, reward the candidates that actually close the gap rather than merely
    # carrying a strong generic player score.
    x["starter_upgrade_priority"]=0.0
    if _starter_gap>0 and int(round_no)<=10:
        _pos_svd=_svd.clip(lower=0.0)
        _sv_pct=_pos_svd.rank(pct=True,method="average").fillna(0.0)
        _rescue_scale=min(1.0,_starter_gap/8.0)
        x["starter_upgrade_priority"]=_rescue_scale*(1.15*_pos_svd + 3.0*_sv_pct*(_pos_svd>0))
        x["evaluation_score"]+=x["starter_upgrade_priority"]

    # v9.56 weakest-starter-slot authority. This explicitly prevents RB3/RB4 from
    # crowding out an unfinished WR foundation merely because generic player quality is high.
    x["starter_slot_deficit_bonus"]=_starter_slot_deficit_bonus_df(x,roster,slots,round_no)
    x["evaluation_score"]+=pd.to_numeric(x["starter_slot_deficit_bonus"],errors="coerce").fillna(0.0)

    # Replacement-aware wait-vs-draft-now value. This is a lightweight lookahead
    # to our next snake pick and favors real positional dropoffs, not stale fallers.
    _scarcity,_drop=_candidate_wait_scarcity_adjustment(x,current_pick,next_pick,counts,slots)
    x["wait_scarcity_bonus"]=_scarcity.reindex(x.index).fillna(0.0)
    x["next_pick_quality_drop"]=_drop.reindex(x.index).fillna(0.0)
    x["evaluation_score"]+=x["wait_scarcity_bonus"]

    # Explicit bench saturation. A late RB6/WR6 must have real marginal utility,
    # not merely a familiar name or an old ADP bargain.
    x["bench_saturation_penalty"]=[_bench_saturation_penalty(r,counts,round_no) for _,r in x.iterrows()]
    x["evaluation_score"]-=pd.to_numeric(x["bench_saturation_penalty"],errors="coerce").fillna(0.0)

    # Portfolio discipline: after Round 10, a RB/WR that does not improve a starter
    # or FLEX slot must clear a much higher bar as the room at that position fills.
    x["pure_depth_portfolio_penalty"]=0.0
    if int(round_no)>=11:
        _pp=x["position"].astype(str)
        _depth_candidate=_pp.isin(["RB","WR"]) & (_svd<=0.25)
        for _p in ["RB","WR"]:
            _have=int(counts.get(_p,0))
            _pm=_depth_candidate & _pp.eq(_p)
            if not _pm.any():
                continue
            _base=max(0.0,7.0+4.5*max(0,_have-3))
            _weak=_rd<1.6
            _pen=np.where(_weak, _base+4.0, _base)
            x.loc[_pm,"pure_depth_portfolio_penalty"]=_pen[_pm]
        x["evaluation_score"]-=pd.to_numeric(x["pure_depth_portfolio_penalty"],errors="coerce").fillna(0.0)

    # Realized draft value at the current price. Reaches are penalized faster than
    # fallers are rewarded, and the reward is capped so stale ADP cannot carry a
    # low-impact depth player by itself.
    _cr_now=pd.to_numeric(x.get("consensus_rank",np.nan),errors="coerce")
    _value_now=float(current_pick)-_cr_now
    x["draft_value_now"]=np.where(
        _value_now>=0,
        np.minimum(_value_now,28.0)*0.10,
        np.maximum(_value_now,-20.0)*0.16
    )
    x["evaluation_score"]+=pd.to_numeric(x["draft_value_now"],errors="coerce").fillna(0.0)

    # v9.56 TE2 expected-weekly-utility gate. A famous TE falling 80 picks is not
    # automatically the best use of a bench spot once TE1 is already strong.
    x["te2_utility_adjustment"]=_te2_expected_utility_adjustment_df(x,roster,current_pick,round_no)
    x["evaluation_score"]+=pd.to_numeric(x["te2_utility_adjustment"],errors="coerce").fillna(0.0)

    # v9.52 stale-value breaker.
    # A player who survives dramatically beyond consensus is not automatically a
    # bargain forever. When he no longer improves the starting lineup, large falls
    # become evidence that the market/board input may be stale or that the player
    # is merely replaceable depth.
    _fall_now=np.maximum(float(current_pick)-_cr_now,0.0)
    _pure_depth=_svd<0.75
    _stale=np.maximum(_fall_now-34.0,0.0)
    x["stale_market_penalty"]=np.where(
        _pure_depth,
        np.minimum(24.0,_stale*0.30),
        np.minimum(8.0,_stale*0.10)
    )
    x["evaluation_score"]-=pd.to_numeric(x["stale_market_penalty"],errors="coerce").fillna(0.0)

    # Deep-bench picks should chase differentiated ceiling, not accumulate the
    # same low-marginal veterans. This is position-agnostic and uses current
    # projection/model quality rather than a hard-coded player list.
    if int(round_no)>=11:
        _proj_pct=pd.to_numeric(x.get("projection",0),errors="coerce").rank(pct=True).fillna(.5)
        _ds_pct=pd.to_numeric(x.get("draft_score",0),errors="coerce").rank(pct=True).fillna(.5)
        _vr_pct=pd.to_numeric(x.get("risk_adjusted_vorp",x.get("vorp",0)),errors="coerce").rank(pct=True).fillna(.5)
        _ceiling=(0.42*_proj_pct+0.33*_ds_pct+0.25*_vr_pct)
        x["bench_ceiling_bonus"]=(_ceiling-.50)*7.0
        x.loc[_pure_depth,"evaluation_score"]+=x.loc[_pure_depth,"bench_ceiling_bonus"]
    else:
        x["bench_ceiling_bonus"]=0.0

    x=roster_opportunity_adjustment(x,roster,int(round_no),int(current_pick),slots)
    x["evaluation_score"]+=pd.to_numeric(x["roster_opportunity_adj"],errors="coerce").fillna(0)
    ...
    # If 5 RBs are already rostered, an eligible Tier 1-2 IDP within 10 points
    # of the best RB blocks RB6 from FINAL PICK.
    _rc=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
    if int(_rc.get("RB",0))>=5:
        _elite_idp=(
            x["position"].isin(["DL","DB"]) &
            x["idp_eligible"].fillna(False).astype(bool) &
            (pd.to_numeric(x["idp_quality_tier"],errors="coerce").fillna(5)<=2)
        )
        _rbmask=x["position"].eq("RB")
        if _elite_idp.any() and _rbmask.any():
            _best_idp=float(pd.to_numeric(x.loc[_elite_idp,"evaluation_score"],errors="coerce").max())
            _best_rb=float(pd.to_numeric(x.loc[_rbmask,"evaluation_score"],errors="coerce").max())
            if _best_idp>=_best_rb-10.0:
                x.loc[_rbmask,"evaluation_score"]-=1000.0
                x.loc[_rbmask,"rb6_blocked_for_impact_idp"]=True

    # v9.50 exact same-position Pareto dominance.
    # A candidate is blocked ONLY when an actual same-position player has BOTH
    # >= projection and >= VORP, with a meaningful edge in at least one.
    ...
    for _p in ("QB","RB","WR","TE"):
        _idxp=x.index[x["position"].eq(_p)]
        if len(_idxp)<2: continue
        _t=pd.DataFrame({
            "projection":_proj_all.loc[_idxp],
            "vorp":_vorp_all.loc[_idxp],
        },index=_idxp).dropna()
        if len(_t)<2: continue
        _t=_t.sort_values(["projection","vorp"],ascending=[False,False])
        _best_vorp=-np.inf
        _best_proj_for_vorp=np.nan
        for _i,_r in _t.iterrows():
            _pi=float(_r["projection"]); _vi=float(_r["vorp"])
            _is_dom=(
                np.isfinite(_best_vorp) and _best_vorp>=_vi and
                ((float(_best_proj_for_vorp)-_pi)>=0.20 or (_best_vorp-_vi)>=0.20)
            )
            if _is_dom:
                _gap=max(0.0,float(_best_proj_for_vorp)-_pi)*1.5 + max(0.0,_best_vorp-_vi)*2.5
                x.at[_i,"same_pos_dominated"]=True
                x.at[_i,"same_pos_quality_gap"]=_gap
                x.at[_i,"evaluation_score"]-=1000.0
            if _vi>_best_vorp:
                _best_vorp=_vi
                _best_proj_for_vorp=_pi
    ...
    # v9.55 meaningful-alternative gate. Do not spend a late pick on replaceable
    # RB/WR depth while a real lineup upgrade, required starter, or elite backup IDP
    # remains available. This is conditional, so useful depth can still win naturally.
    if int(round_no)>=11:
        _pp=x["position"].astype(str)
        _counts_now=roster.position.value_counts().to_dict() if roster is not None and len(roster) else {}
        _required_open_mask=pd.Series(False,index=x.index)
        for _p in ["QB","RB","WR","TE","K","DL","DB"]:
            if int(_counts_now.get(_p,0)) < int(slots.get(_p,0)):
                _required_open_mask |= _pp.eq(_p)
        _tier_all=pd.to_numeric(x.get("idp_quality_tier",5),errors="coerce").fillna(5)
        _elite_backup=_pp.isin(["DL","DB"]) & x.get("idp_eligible",pd.Series(False,index=x.index)).fillna(False).astype(bool) & (_tier_all<=2)
        _meaningful=(_svd>=0.55) | (_rd>=2.2) | _required_open_mask | _elite_backup
        if _meaningful.any():
            _replaceable=_pp.isin(["RB","WR"]) & (_svd<=0.20) & (_rd<1.6)
            # Only block when we already own at least three at that position.
            _replaceable &= _pp.map(lambda p: int(_counts_now.get(p,0))>=3)
            x.loc[_replaceable,"evaluation_score"]-=1000.0
            x.loc[_replaceable,"replaceable_depth_blocked"]=True

    # Negative roster opportunity is an authoritative cost.
    _ropp=pd.to_numeric(x.get("roster_opportunity_adj",0),errors="coerce").fillna(0)
    _neg_ropp=_ropp < -3.0
    ...
    for _p in ("DL","DB"):
        ...
        if pd.notna(_best_ext) and _best_ext<=_elite_cut:
            ...
            _all_talent=pd.to_numeric(x.get("idp_talent_score",np.nan),errors="coerce").fillna(-99.0)
    ...
    # v9.56 authoritative IDP rank + reach control. Missing positional rank is a
    # source-data failure; ordinary defenders cannot be pulled 20-30 picks early just
    # because the roster still needs a DB/DL.
    _idp_reach_pen,_idp_missing_rank,_idp_reach=_idp_authority_reach_adjustment_df(x,current_pick)
    x["idp_reach_penalty"]=_idp_reach_pen
    x["idp_market_reach"]=_idp_reach
    x["idp_missing_authority_rank"]=_idp_missing_rank
    x["evaluation_score"]-=pd.to_numeric(x["idp_reach_penalty"],errors="coerce").fillna(0.0)
    x.loc[_idp_missing_rank,"evaluation_score"]-=1000.0
    x.loc[_idp_missing_rank,"idp_quality_blocked"]=True

    # Evidence authority after all roster construction: ineligible IDPs cannot be FINAL PICK.
    bad=x["position"].isin(["DL","DB"]) & ((~x["idp_eligible"].fillna(False).astype(bool)) | _idp_missing_rank)
    ...
    _ropp_final=pd.to_numeric(x.get("roster_opportunity_adj",0),errors="coerce").fillna(0)
    ...
    cr=pd.to_numeric(x.get("market_pick",np.nan),errors="coerce").fillna(float(current_pick))
    ...
    reach=np.maximum(cr-float(current_pick),0)
    fall=np.maximum(float(current_pick)-cr,0)
    ...
    # Bounded execution value. Extreme late falls are often stale market signals,
    # not infinite value; do not re-inflate them after the evaluation staleness gate.
    x["execution_score"]+=.035*np.minimum(fall,34.0)
...
def make_board(players, hist, market, ppr, pass_td, idp, teams, slots):
    ...
    # v9.56 position-specific aging/risk profiles. A generic veteran curve was
    # incorrectly labeling prime-age QBs/edge rushers (e.g. elite age-30 players)
    # as decline risks. Decline labels now require position-appropriate age evidence.
    young_breakout=(b.age.fillna(99)<=26) & (b.exp<=4)
    age_floor=b.position.map({"QB":34,"RB":28,"WR":30,"TE":30,"K":35,"DL":31,"DB":30}).fillna(30)
    age_decline=b.age.fillna(0)>=age_floor
    b["profile"]="Stable / neutral"
    b.loc[age_decline & (b.decline>=.68),"profile"]="Decline risk"
    b.loc[age_decline & (b.decline>=.50)&(b.decline<.68),"profile"]="High variance"
    b.loc[(~age_decline) & (b.decline>=.68),"profile"]="Prime / stable"
    ...
    b.loc[(~young_breakout) & (~age_decline) & (b.breakout>=.65),"profile"]="Prime upside"
    b.loc[(~young_breakout) & age_decline & (b.breakout>=.65) & (b.decline<.50),"profile"]="Veteran upside"

    base=b.position.map({"QB":16,"RB":8.5,"WR":8,"TE":6,"K":8.5,"DL":6,"DB":5.5})
    b["projection"]=(b.last_ppg*.70+b.prior_ppg*.20+base*.10+b.growth_z*.50).clip(lower=1)

    # v9.53 kicker authority. K rankings are position-specific, not overall ADP.
    # Never invent an ordering for unranked kickers: missing K rank is an eligibility
    # problem handled by _apply_player_pool_authority(), not a scoring opportunity.
    _km=b.position.eq("K")
    b["kicker_rank"]=np.nan
    if _km.any():
        _krank=pd.to_numeric(b.loc[_km,"consensus_rank"],errors="coerce")
        b.loc[_km,"kicker_rank"]=_krank
        # Create meaningful, conservative weekly separation. K1 ~9.8; K12 ~8.48;
        # K24 ~7.04. Unknown kickers remain NaN-gated instead of receiving a fake floor.
        _kproj=(9.80 - 0.12*(_krank-1.0)).clip(lower=6.80,upper=9.80)
        b.loc[_km,"projection"]=_kproj
...
def _small_roster_utility_df(roster, slots):
    ...
    depth={"QB":0,"RB":0,"WR":0,"TE":0,"K":0,"DL":0,"DB":0}
    bench_mult={"QB":[.14,.03],"RB":[.25,.15,.08,.03],"WR":[.40,.30,.20,.11,.06],
                "TE":[.16,.04],"K":[.00],"DL":[.10],"DB":[.10]}
    for _,row in bench.iterrows():
        p=row.position; d=depth.get(p,0); arr=bench_mult.get(p,[.05])
        mult=arr[d] if d<len(arr) else .02
        # In this league a second elite defender can be more useful than RB6/WR6.
        # Give the first IDP backup real insurance/ceiling value when independent
        # positional authority says he is an impact player.
        if p in ["DL","DB"] and d==0:
            ext=_v936_external_idp_rank(row)
            impact,eligible,_=_v934_idp_impact_score(row)
            if eligible and ((pd.notna(ext) and ext <= (10 if p=="DL" else 12)) or impact>=10):
                mult=max(mult,.24)
...
def _starter_vorp_total_df(roster, slots):
    """Realized starter VORP for the exact league template.

    This mirrors grade_mock(): required positional starters are filled first, then
    FLEX is filled by the best remaining RB/WR/TE. It intentionally ignores bench
    VORP so depth cannot masquerade as starter improvement.
    """
    if roster is None or len(roster)==0:
        return 0.0
    r=roster.copy()
    r["_vorp_num"]=pd.to_numeric(r.get("vorp",0),errors="coerce").fillna(0.0)
    used=set()
    total=0.0
    for p in ["QB","RB","WR","TE","K","DL","DB"]:
        n=max(int(slots.get(p,0)),0)
        if n<=0:
            continue
        idx=r[r.position.eq(p)]["_vorp_num"].nlargest(n).index
        used.update(idx.tolist())
        total += float(r.loc[idx,"_vorp_num"].sum())
    flex_n=max(int(slots.get("FLEX",0)),0)
    if flex_n:
        flex_pool=r.loc[
            (~r.index.isin(used)) & r.position.isin(["RB","WR","TE"]),
            "_vorp_num"
        ]
        total += float(flex_pool.nlargest(flex_n).sum())
    return float(total)


def _candidate_starter_vorp_delta_df(roster, candidates, slots, exact_cap=96):
    """Marginal Starter-VORP gain from each candidate.

    The exact calculation is bounded to the candidates most likely to matter. A
    zero result means the player is currently pure depth and does not improve any
    starter/FLEX slot.
    """
    if candidates is None or len(candidates)==0:
        return np.array([],dtype=float)
    current=_starter_vorp_total_df(roster,slots)
    c=candidates
    vorp=pd.to_numeric(c.get("vorp",0),errors="coerce").fillna(0.0).to_numpy(float)
    ds=pd.to_numeric(c.get("draft_score",0),errors="coerce").fillna(-1e9).to_numpy(float)
    cr=pd.to_numeric(c.get("consensus_rank",999),errors="coerce").fillna(999).to_numpy(float)

    # Safe approximation for the long tail; exact-evaluate the strongest model,
    # market and VORP candidates plus positional leaders.
    out=np.maximum(vorp,0.0)*0.05
    n=min(int(exact_cap),len(c))
    if n<=0:
        return out
    take=max(1,n//3)
    idxs=[]
    idxs.extend(np.argpartition(-ds,min(take-1,len(ds)-1))[:take].tolist())
    idxs.extend(np.argpartition(cr,min(take-1,len(cr)-1))[:take].tolist())
    idxs.extend(np.argpartition(-vorp,min(take-1,len(vorp)-1))[:take].tolist())
    posarr=c["position"].astype(str).to_numpy()
    per=max(3,n//20)
    for pname in ["QB","RB","WR","TE","K","DL","DB"]:
        loc=np.flatnonzero(posarr==pname)
        if loc.size:
            k=min(per,loc.size)
            idxs.extend(loc[np.argpartition(-vorp[loc],min(k-1,loc.size-1))[:k]].tolist())
    short=np.unique(np.asarray(idxs,dtype=int))
    if len(short)>n:
        comp=(-ds[short]) + 0.04*cr[short] - 0.8*np.maximum(vorp[short],0.0)
        short=short[np.argsort(comp)[:n]]

    for j in short:
        row=c.iloc[int(j)]
        temp=pd.concat([roster,row.to_frame().T],ignore_index=True) if roster is not None and len(roster) else row.to_frame().T
        out[int(j)]=_starter_vorp_total_df(temp,slots)-current
    return out
...
def _candidate_roster_delta_df(roster, candidates, slots, exact_cap=72):
    ...
    return out
def _candidate_wait_scarcity_adjustment(x, current_pick, next_pick, counts, slots):
    """Replacement-aware one-turn lookahead.

    Reward a strong player now only when the same position is projected to lose
    meaningful quality before our next pick. Pure bench depth never receives a
    scarcity bonus merely because its ADP says it should already be gone.
    """
    if x is None or len(x)==0:
        return pd.Series(dtype=float), pd.Series(dtype=float)
    z=x.copy()
    if next_pick is None or pd.isna(next_pick):
        next_pick=float(current_pick)+24.0
    svd=pd.to_numeric(z.get("starter_vorp_delta",0),errors="coerce").fillna(0.0)
    rd=pd.to_numeric(z.get("roster_delta",0),errors="coerce").fillna(0.0)
    rv=pd.to_numeric(z.get("risk_adjusted_vorp",z.get("vorp",0)),errors="coerce").fillna(0.0)
    impact=pd.to_numeric(z.get("idp_impact_score",0),errors="coerce").fillna(0.0)
    mp=pd.to_numeric(z.get("market_pick",z.get("consensus_rank",np.nan)),errors="coerce")
    pos=z["position"].astype(str)
    quality=2.45*svd + 0.62*rd + 0.10*np.maximum(rv,0.0)
    quality += np.where(pos.isin(["DL","DB"]),0.16*np.maximum(impact,0.0),0.0)
    future_likely=mp.notna() & (mp >= float(next_pick)-2.0)
    bonus=pd.Series(0.0,index=z.index,dtype=float)
    dropcol=pd.Series(0.0,index=z.index,dtype=float)
    for p in ["QB","RB","WR","TE","K","DL","DB"]:
        pm=pos.eq(p)
        if not pm.any():
            continue
        q=quality[pm]
        best_now=float(q.max())
        fq=quality[pm & future_likely]
        best_wait=float(fq.max()) if len(fq) else 0.0
        drop=max(0.0,best_now-best_wait)
        dropcol.loc[pm]=drop
        required_open=int(counts.get(p,0)) < int(slots.get(p,0))
        meaningful=(svd.loc[pm]>=0.50) | (rd.loc[pm]>=2.0)
        if p in ["DL","DB"]:
            tier=pd.to_numeric(z.loc[pm].get("idp_quality_tier",5),errors="coerce").fillna(5)
            meaningful = meaningful | (tier<=2)
        # Only the genuinely strong options at the position capture the dropoff.
        rel=(q-best_wait).clip(lower=0.0)
        add=np.minimum(8.0,0.70*rel + (1.2 if required_open else 0.0))
        add=add.where(meaningful,0.0)
        bonus.loc[pm]=add
    return bonus,dropcol
...
def draft_eligibility(avail, roster, round_no, rounds, slots):
    ...
    # Allow one impact backup IDP. Previous code removed the entire position as soon
    # as the required starter was filled, making the RB6 -> impact defender rule impossible.
    if dl>=max(int(slots.get("DL",1))+1,2): a=a[~a.position.eq("DL")]
    if db>=max(int(slots.get("DB",1))+1,2): a=a[~a.position.eq("DB")]
    if counts.get("K",0)>=max(int(slots.get("K",1)),1): a=a[~a.position.eq("K")]
...
def grade_mock(roster, teams, slots):
    ...
    _market_cost=pd.to_numeric(r.get("market_pick",r.get("consensus_rank",np.nan)),errors="coerce")
    if "mock_pick" in r.columns:
        market_delta=r.mock_pick-_market_cost
    else:
        market_delta=_market_cost-r.model_rank
    market_delta=market_delta.where(_market_cost.notna())
    # v9.56 realism-adjusted market value. A +97 "value" is more likely a bad mock
    # room than model genius, so extreme fallers are progressively discounted.
    _md=market_delta.fillna(0.0)
    _realism_weight=np.where(_md>30.0,np.exp(-(_md-30.0)/24.0),1.0)
    market_points=np.where(market_delta.notna(),
        np.where(market_delta>=0, np.minimum(market_delta,36.0)*0.42*_realism_weight,
    ...
    valid_model=(~is_idp) & (~r.position.eq("K")) & r.model_rank.notna() & (r.model_rank<400)
    ...
    vorp_points=(vorp*0.72).to_numpy(dtype=float)
    ...
    _effective_market=np.where(_md>=0,np.minimum(_md,36.0)*_realism_weight,np.maximum(_md,-30.0))
    avg_market=float(np.nanmean(_effective_market)) if len(r) else 0.0
    avg_vorp=float(vorp.mean()) if len(r) else 0.0
    draft_value=float(np.clip(62+0.62*avg_market+2.0*avg_vorp,35,100))
    unrealistic_fallers=int((_md>50.0).sum())
    ...
    # Opportunity cost: real reaches and unnecessary early QB2/TE2 only.
    ...
    for _,row in ordered.iterrows():
        ...
        crv=row.get("market_pick",row.get("consensus_rank",np.nan)); pk=row.get("mock_pick",np.nan)
        if pd.notna(crv) and pd.notna(pk):
            reach=float(crv)-float(pk)  # positive = drafted ahead of realistic market timing
            if reach>18: opp+=min(8,(reach-18)*.12)

    score=float(np.clip(.27*draft_value+.30*construction+.23*positional+.20*model_edge-opp,0,100))
    ...
    return {"score":score,"grade":grade,"starter":starter,"value":avg_market,"penalty":max(0,100-construction),
            ...
            "unrealistic_faller_count":unrealistic_fallers}
...
def roster_opportunity_adjustment(df, roster, round_no, current_pick, slots):
    ...
    for _,r in x.iterrows():
        ...
        # v9.55 dynamic WR foundation. Build the required/FLEX shell, but do not
        # prescribe WR5 simply because the draft is late. WR4/WR5 must provide
        # actual starter/FLEX or marginal-roster value.
        if pos=="WR":
            _row_svd=pd.to_numeric(pd.Series([r.get("starter_vorp_delta",0)]),errors="coerce").fillna(0).iloc[0]
            _row_rd=pd.to_numeric(pd.Series([r.get("roster_delta",0)]),errors="coerce").fillna(0).iloc[0]
            if rnd>=7 and wr<3:
                adj += 10.0
                why.append("WR foundation priority")
            elif rnd>=7 and wr<4:
                adj += 5.0
                why.append("WR4 flexible depth")
            elif rnd>=10 and wr==4 and (_row_svd>=0.50 or _row_rd>=2.0):
                adj += 2.0
                why.append("WR5 earned marginal value")
        ...
        # Four useful RB/WR each is sufficient depth for this 2-FLEX build; do not
        # force five of both and crowd out impact IDP or other differentiated depth.
        offense_depth_thin = (wr < 4) or (rb < 4)
        if pos=="TE" and te>=1:
            if rnd<=10 and offense_depth_thin:
                ...
                adj -= 4.0
                why.append("TE2 opportunity cost")
        ...
        # DB timing is quality-aware. Elite authority DBs may be worth taking in the
        # value window; a non-authority DB should not win early merely from roster need.
        if pos=="DB" and rnd<=10:
            _db_tier,_=_v941_idp_quality(r)
            if _db_tier<=1:
                adj -= 1.0
                why.append("elite DB timing cost")
            elif _db_tier<=2:
                adj -= 5.0
                why.append("DB2-tier timing cost")
            else:
                adj -= 12.0
                why.append("replaceable DB early cost")

        # v9.55 IDP depth valuation. A second ordinary defender is still optional,
        # but an independently supported Tier 1/2 impact defender is legitimate bench
        # value and must not receive the old automatic -22 penalty.
        _idp_tier,_idp_qev=_v941_idp_quality(r) if pos in ["DL","DB"] else (5,0)
        _elite_backup_candidate=(pos in ["DL","DB"] and _idp_eligible and _idp_tier<=2)
        if pos=="DL" and dl>=1:
            if _elite_backup_candidate:
                adj += (4.0 if rnd>=12 else -4.0)
                why.append("elite DL2 impact depth")
            else:
                adj -= 22.0
                why.append("DL starter already filled")
        if pos=="DB" and db>=1:
            if _elite_backup_candidate:
                adj += (4.0 if rnd>=12 else -4.0)
                why.append("elite DB2 impact depth")
            else:
                adj -= 22.0
                why.append("DB starter already filled")
        ...
        idp_count=dl+db
        if pos in ["DL","DB"] and idp_count>=1:
            if _elite_backup_candidate and not offense_depth_thin and rnd>=12:
                adj += 3.0
                why.append("elite IDP2 over low-impact depth")
            elif offense_depth_thin:
                adj -= (5.0 if _elite_backup_candidate else 10.0)
                why.append("IDP2 vs offensive depth")
            elif rnd<=12:
                adj -= (2.0 if _elite_backup_candidate else 6.0)
                why.append("early IDP2 opportunity cost")
            else:
                adj -= (0.0 if _elite_backup_candidate else 1.0)
                why.append("late IDP2 optional depth")
        ...
        # Faller relief is earned only by meaningful roster value. The old version
        # forgave up to 65% of saturation penalties for any huge ADP fall, which
        # recreated the same stale RB/WR depth picks.
        mp=pd.to_numeric(pd.Series([r.get("market_pick",np.nan)]),errors="coerce").iloc[0]
        if pd.notna(mp):
            fall=max(cur-float(mp),0.0)
            _svd_row=pd.to_numeric(pd.Series([r.get("starter_vorp_delta",0)]),errors="coerce").fillna(0).iloc[0]
            _rd_row=pd.to_numeric(pd.Series([r.get("roster_delta",0)]),errors="coerce").fillna(0).iloc[0]
            _required_here=int(counts.get(pos,0)) < int(slots.get(pos,0))
            _meaningful_faller=(_svd_row>=0.75) or (_rd_row>=2.0) or _required_here or (pos in ["DL","DB"] and _elite_backup_candidate)
            if _meaningful_faller and fall>=24 and adj<0:
                adj *= 0.55
                why.append("earned extreme-faller relief")
            elif _meaningful_faller and fall>=16 and adj<0:
                adj *= 0.75
                why.append("earned faller relief")
            elif (not _meaningful_faller) and fall>=34:
                adj -= min(10.0,(fall-34.0)*0.12)
                why.append("stale depth faller")
...
def apply_v932_exact_league_construction(df, roster, round_no, rounds, slots):
    ...
    for idx,r in x.iterrows():
        ...
        # One DL/one DB only; extra IDP has essentially no bench utility in this format.
        if pos=="DL" and dl>=1: adj -= 30.0
        if pos=="DB" and db>=1: adj -= 30.0
        if adj:
...
def _market_pick_series(df, teams):
    """Translate position-specific expert ranks into realistic overall draft timing."""
    cr=pd.to_numeric(df["consensus_rank"],errors="coerce")
    out=cr.astype(float).copy()
    ...
    # Kicker ECR is a K1/K2/... positional rank, not overall pick 1/2/.... Convert
    # it to the final 2-3 rounds so Aubrey is K1 without being treated as 1.01.
    k=df["position"].astype(str).str.upper().eq("K")
    kr=pd.to_numeric(df.get("kicker_rank",cr),errors="coerce")
    # In a 12-team / 17-round room: K1 ~181, K12 ~195, K24 ~210.
    out.loc[k]=float(teams)*15.0 + 1.30*kr.loc[k]
    return out
...
def _fast_benchmark_pool(board, teams=12):
    """Convert the certified draft board to NumPy arrays once per benchmark run."""
    board=_apply_player_pool_authority(board)
    pool=board[board.position.isin(["QB","RB","WR","TE","DL","DB","K"])].copy().reset_index(drop=True)
    pos_names=np.array(["QB","RB","WR","TE","DL","DB","K"],dtype=object)
...
def _fast_roster_utility(fast, selected, slots, te_starter_vorp_mult=.90):
    ...
    for code,pname in enumerate(["QB","RB","WR","TE","DL","DB","K"]):
    ...
    mults={0:[.14,.03],1:[.25,.15,.08,.03],2:[.40,.30,.20,.11,.06],3:[.16,.04],4:[.06],5:[.06],6:[.00]}
    for code in range(7):
...
def _fast_roster_role_snapshot(fast, selected, candidate, slots):
    ...
    pcode=int(pos[j])
    pname=["QB","RB","WR","TE","DL","DB","K"][pcode]
    counts_before={name:int(np.sum(fast["pos_code"][before]==code)) if len(before) else 0
                   for code,name in enumerate(["QB","RB","WR","TE","DL","DB","K"])}
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None,
    ...
    qb,rb,wr,te,dl,db,k=0,1,2,3,4,5,6
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if counts[dl]>=max(int(slots.get("DL",1))+1,2): eligible &= pos[idx]!=dl
            if counts[db]>=max(int(slots.get("DB",1))+1,2): eligible &= pos[idx]!=db
            if counts[k]>=max(int(slots.get("K",1)),1): eligible &= pos[idx]!=k
            ...
            _noise=rng.normal(0,sd)
            opp=_opponent_market_expiration_score(sim_cons[cidx],overall,_noise)
            _od=np.maximum(float(overall)-sim_cons[cidx],0.0)
            if np.nanmax(_od)>=45.0:
                opp=opp+np.where(_od<35.0,1000.0,0.0)
            if room_value:
...
def simulate_mock(board, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None):
    ...
    rng=np.random.default_rng(seed)
    pool=board.copy()
    pool=pool[pool.position.isin(["QB","RB","WR","TE","K","DL","DB"])].copy().reset_index(drop=True)
    ...
    for overall in range(1,total+1):
        ...
        if owner_slot==slot:
            ...
            _cb=cand.sim_consensus.fillna(cand.model_rank)
            cand["opp_score"]=_opponent_market_expiration_score(_cb.to_numpy(float),overall,noise)
            _od=(float(overall)-_cb).clip(lower=0.0)
            if float(_od.max())>=45.0:
                cand.loc[_od<35.0,"opp_score"]+=1000.0
...
# v9.54: build authority layers, then shrink raw ingestion into the actual production draft universe.
board=_v954_kicker_authority_overlay(board,teams)
board=_v954_market_authority(board)
board=_v954_curated_draft_universe(board)

# v9.54 pre-draft certification measures the curated draft universe, not dirty raw feeds.
try:
    _cert=board_certification_summary(board,state.get("roster_slots",{}))
    _fail=int((_cert["Status"]=="FAIL").sum())
    _warn=int((_cert["Status"]=="WARN").sum())
    with st.expander("🧪 Pre-draft data certification",expanded=bool(_fail)):
        st.dataframe(_cert,use_container_width=True,hide_index=True)
        if _fail:
            st.error(f"{_fail} certification check(s) failed. The curated production universe has a blocking integrity problem; do not use Draft Mode until it passes.")
        elif _warn:
            st.warning(f"Certification passed with {_warn} warning(s); only the curated production universe is used for recommendations.")
        else:
            st.success("Player pool certified: no blocking source-data issues detected.")
except Exception as _cert_err:
    st.caption(f"Certification unavailable: {_cert_err}")
...
with tabs[1]:
    ...
    for i,pos in enumerate(["RB","WR","QB","TE","K","DL","DB"]):
    ...
    pos=st.multiselect("Position",["QB","RB","WR","TE","K","DL","DB"],default=["QB","RB","WR","TE","K","DL","DB"],key="draftpos")
    ...
    st.dataframe(
        ...
        .rename(columns={
            ...
            "idp_external_rank":"IDP rank","idp_impact_score":"IDP impact","idp_available_rank":"Available IDP rank","idp_available_score":"Available IDP score","roster_opportunity_adj":"Roster opp."
        }).head(120),
        use_container_width=True,hide_index=True
    )

    st.markdown("### Record the latest pick")
    pick=st.selectbox("Player selected",[""]+show.player.tolist())
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        if "mock_drafted" not in st.session_state:
            ...
            st.session_state.mock_room_seed=int(np.random.default_rng().integers(1,2_000_000_000))
        if "mock_room_seed" not in st.session_state:
            st.session_state.mock_room_seed=int(np.random.default_rng().integers(1,2_000_000_000))

        a,b=st.columns(2)
        if a.button("Start / reset mock"):
            # Preserve the completed prior room so v9.56 can measure whether the
            # engine is truly adapting instead of silently redrafting the same names.
            if len(st.session_state.get("mock_user",[]))>=10:
                st.session_state.mock_previous_players=[str(r.get("player","")) for r in st.session_state.get("mock_user",[]) if r.get("player")]
            st.session_state.mock_drafted=[]
            st.session_state.mock_user=[]
            st.session_state.mock_overall=1
            # A new mock should represent a genuinely new draft room. The previous
            # deterministic overall+slot seed recreated nearly identical boards.
            st.session_state.mock_room_seed=int(np.random.default_rng().integers(1,2_000_000_000))
            st.rerun()
        if b.button("Auto-draft opponents until my next pick"):
            ...
            rng=np.random.default_rng(int(st.session_state.mock_room_seed)+int(overall)*1009+int(slot)*9176)
            while overall<=int(teams)*int(rounds):
                ...
                avail=_apply_player_pool_authority(avail)
                if avail.empty: break
                fallback=avail.model_rank
                base=pd.to_numeric(avail.get("market_pick",np.nan),errors="coerce").fillna(fallback)
                noise=rng.normal(0,int(randomness),len(avail))
                # Opponents remain consensus-led, but overdue players become
                # progressively harder to keep passing. This prevents the same
                # consensus top-100 names from unrealistically surviving into
                # rounds 13-16 in every interactive room.
                overdue=np.maximum(float(overall)-pd.to_numeric(base,errors="coerce").fillna(float(overall)),0.0)
                avail["opp"]=_opponent_market_expiration_score(
                    pd.to_numeric(base,errors="coerce").fillna(float(overall)).to_numpy(float),overall,noise
                )
                # Hard realism backstop: once anyone reaches 45+ picks overdue, the
                # room must select from the materially overdue cluster. This prevents
                # Kittle-at-198 style boards without turning the whole mock into ADP order.
                if float(np.nanmax(overdue.to_numpy(float)))>=45.0:
                    urgent=overdue>=35.0
                    if urgent.any():
                        avail.loc[~urgent,"opp"]+=1000.0
            ...
            if owner==int(slot):
                ...
                st.dataframe(
                    top[["player","position","team","evaluation_rank","Action","Market pick",
                         "Survives to next pick","projection","vorp","idp_available_rank","idp_available_score",
                ...
                if not pick_options:
                    ...
                    if st.button("Draft this player"):
                        ...
                        if choice not in set(current_available.player.astype(str)):
                            ...
                            st.rerun()
                        _commit=current_available[current_available.player.astype(str).eq(str(choice))].copy()
                        _commit,_=prepare_user_draft_candidates(
                            _commit,roster,rnd,overall,int(slot),int(teams),state["roster_slots"],randomness
                        )
                        if _commit.empty:
        ...
        if len(roster):
            ...
            st.caption("Roster build: " + " • ".join(f"{p} {counts.get(p,0)}" for p in ["QB","RB","WR","TE","K","DL","DB"]))
            st.markdown("#### 🧪 Mock quality certification")
            _mq=mock_quality_certification(roster,st.session_state.get("mock_previous_players"))
            st.dataframe(_mq,use_container_width=True,hide_index=True)
            _mq_fail=int((_mq["Status"].astype(str)=="FAIL").sum())
            _mq_warn=int((_mq["Status"].astype(str)=="WARN").sum())
            if _mq_fail:
                st.error(f"{_mq_fail} mock-realism check(s) failed. Treat Draft Value as unreliable until the room passes.")
            elif _mq_warn:
                st.warning(f"Mock completed with {_mq_warn} realism warning(s).")
            else:
                st.success("Mock room realism certified.")
            _show=roster.copy()
            ...
            st.dataframe(
                _show[["mock_round","mock_pick","player","position","display_projection","display_vorp","display_model_rank","consensus_rank","market_pick","profile"]]
        ...
        st.markdown("### 👑 V8.05 — Frozen Production Champion")
        st.success(
            "V8.04 has passed production certification and is now frozen as the production champion. "
            "Certified Round-2 generic adaptation, certified Round-3 QB opportunity-cost adaptation, "
            "and certified Round-7 logic are ON by default. Protected late-round architecture remains unchanged."
        )
        st.caption(
            "Frozen R3 rule: Round 3 normal choice is QB → genuinely alive/eligible non-QB exists → "
            "higher frozen draft score AND higher VORP → effective market distance ≤10 picks → "
            "select the highest-scoring qualifying alternative. No player, room, slot, or alternative-position hard-coding."
        )
        ...
        st.markdown("### 🛡️ V8.11 — Frozen Champion Robustness Certification")
        st.caption(
            "Global out-of-sample stress test of the frozen V8.05 production champion. "
            "No drafting rule is changed or tuned. Certified R2, R3 and R7 remain ON, the failed R4 branch remains OFF, "
            "and protected late-round architecture remains unchanged."
        )

        if "v811_results" not in st.session_state:
            st.session_state.v811_results=pd.DataFrame()
        if "v811_next_index" not in st.session_state:
            st.session_state.v811_next_index=0

        target_sets=st.selectbox(
            "V8.11 robustness target",
            options=[1200,2400,3600,6000],
            index=2,
            key="v811_target",
            help="Recommended target: 3,600 fresh champion drafts. 6,000 is available for a deeper stress test."
        )
        batch_per_combo=st.number_input(
            "Batch size per slot × room",
            min_value=2,max_value=10,value=5,step=1,key="v811_batch",
            help="5 = 300 drafts per click. Results persist in this Streamlit session."
        )

        a,b=st.columns(2)
        run=a.button("Run next V8.11 robustness batch",type="primary",key="v811_run")
        clear=b.button("Reset V8.11 certification",key="v811_clear")
        ...
        rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]
        combos=[(ds,room_name) for ds in range(1,int(teams)+1) for room_name in rooms]
        batch_sets=len(combos)*int(batch_per_combo)

        if run:
            existing=st.session_state.v811_results
            already=len(existing)
            remaining=max(int(target_sets)-already,0)
            this_batch=min(batch_sets,remaining)

            if this_batch<=0:
                st.success(f"Target already complete: {already:,} drafts.")
            else:
                pool=_fast_benchmark_pool(board,int(teams))
                rows=[]; prog=st.progress(0); status=st.empty()
                start_index=int(st.session_state.v811_next_index)

                for n in range(this_batch):
                    gi=start_index+n
                    combo_idx=gi % len(combos)
                    cycle=gi // len(combos)
                    ds,room_name=combos[combo_idx]
                    seed=811000000+int(ds)*100000+int(cycle)*463+sum(ord(ch) for ch in room_name)

                    d=simulate_mock_fast(
                        pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name,
                        r5_dominance_override_enabled=False,
                        r2_dominance_override_enabled=False,
                        r3_dominance_override_enabled=False,
                        r11_13_offense_dominance_override_enabled=False,
                    ...
                    comp=independent_draft_grade_components(d,int(teams))
                    phase=independent_phase_metrics(d).set_index("Phase")
                    pos_counts=d.position.astype(str).value_counts().to_dict()

                    r2=d[pd.to_numeric(d.mock_round,errors="coerce").eq(2)]
                    r3=d[pd.to_numeric(d.mock_round,errors="coerce").eq(3)]
                    r7=d[pd.to_numeric(d.mock_round,errors="coerce").eq(7)]

                    r2_trigger=bool(int(pd.to_numeric(pd.Series([
                        r2.iloc[0].get("R791_Would_trigger",0) if len(r2) else 0
                    ]),errors="coerce").fillna(0).iloc[0]))
                    r3_trigger=bool(int(pd.to_numeric(pd.Series([
                        r3.iloc[0].get("R802_Would_trigger",0) if len(r3) else 0
                    ]),errors="coerce").fillna(0).iloc[0]))

                    rows.append({
                        "Global_index":gi,
                        "Draft_slot":int(ds),"Room":room_name,"Cycle":int(cycle)+1,"Seed":seed,
                        "Overall_grade":comp["overall"],
                        "R1_3_market":float(phase.loc["R1-3","Avg_market_delta"]),
                        "R4_6_market":float(phase.loc["R4-6","Avg_market_delta"]),
                        "R7_10_market":float(phase.loc["R7-10","Avg_market_delta"]),
                        "R11_plus_market":float(phase.loc["R11+","Avg_market_delta"]),
                        ...
                        "DL_count":int(pos_counts.get("DL",0)),
                        "DB_count":int(pos_counts.get("DB",0)),
                        "R2_trigger":r2_trigger,
                        "R3_trigger":r3_trigger,
                        "R7_player":str(r7.iloc[0].player) if len(r7) else "",
                        "R7_position":str(r7.iloc[0].position) if len(r7) else "",
                    })

                    if (n+1)==1 or (n+1)%50==0 or (n+1)==this_batch:
                        prog.progress((n+1)/this_batch)
                        status.caption(f"Batch {n+1}/{this_batch} • total {already+n+1:,}/{target_sets:,}")

                newdf=pd.DataFrame(rows)
                combined=pd.concat([existing,newdf],ignore_index=True) if len(existing) else newdf
                combined=combined.drop_duplicates("Global_index",keep="last").sort_values("Global_index").reset_index(drop=True)

                st.session_state.v811_results=combined
                st.session_state.v811_next_index=start_index+this_batch
                status.success(f"Batch complete. Saved {len(combined):,}/{target_sets:,} fresh champion drafts.")
                prog.empty()

        df=st.session_state.get("v811_results")
        if isinstance(df,pd.DataFrame) and len(df):
            summary=pd.DataFrame([{
                "Saved drafts":len(df),
                "Target":target_sets,
                "Completion":len(df)/float(target_sets),
                "Avg overall grade":df.Overall_grade.mean(),
                "Overall grade std":df.Overall_grade.std(),
                "P10 overall grade":df.Overall_grade.quantile(.10),
                "P50 overall grade":df.Overall_grade.quantile(.50),
                "P90 overall grade":df.Overall_grade.quantile(.90),
                "R2 activation":df.R2_trigger.mean(),
                "R3 activation":df.R3_trigger.mean(),
                "Avg QB count":df.QB_count.mean(),
                "Avg RB count":df.RB_count.mean(),
                "Avg WR count":df.WR_count.mean(),
                "Avg TE count":df.TE_count.mean(),
            }])
            st.markdown("#### Global robustness summary")
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            room=df.groupby("Room",as_index=False).agg(
                Drafts=("Overall_grade","count"),
                Avg_grade=("Overall_grade","mean"),
                Grade_std=("Overall_grade","std"),
                P10_grade=("Overall_grade",lambda x:x.quantile(.10)),
                R2_activation=("R2_trigger","mean"),
                R3_activation=("R3_trigger","mean"),
                R1_3_market=("R1_3_market","mean"),
                R4_6_market=("R4_6_market","mean"),
                R7_10_market=("R7_10_market","mean"),
                R11_plus_market=("R11_plus_market","mean"),
            )
            slot=df.groupby("Draft_slot",as_index=False).agg(
                Drafts=("Overall_grade","count"),
                Avg_grade=("Overall_grade","mean"),
                Grade_std=("Overall_grade","std"),
                P10_grade=("Overall_grade",lambda x:x.quantile(.10)),
            ...
            st.markdown("#### Robustness by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Robustness by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            cutoff=df.Overall_grade.quantile(.10)
            tail=df[df.Overall_grade<=cutoff]
            tail_room=tail.groupby("Room",as_index=False).agg(
                Tail_drafts=("Overall_grade","count"),
                Avg_tail_grade=("Overall_grade","mean"))
            tail_slot=tail.groupby("Draft_slot",as_index=False).agg(
                Tail_drafts=("Overall_grade","count"),
                Avg_tail_grade=("Overall_grade","mean"))
            st.markdown("#### Worst-decile concentration by room")
            st.dataframe(tail_room.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Worst-decile concentration by slot")
            st.dataframe(tail_slot.round(4),use_container_width=True,hide_index=True)

            room_spread=room.Avg_grade.max()-room.Avg_grade.min()
            slot_spread=slot.Avg_grade.max()-slot.Avg_grade.min()

            gate=pd.DataFrame([{
                "Drafts complete":len(df),
                "Room mean spread":room_spread,
                "Slot mean spread":slot_spread,
                "Worst room mean":room.Avg_grade.min(),
                "Worst slot mean":slot.Avg_grade.min(),
                "R2 activation":df.R2_trigger.mean(),
                "R3 activation":df.R3_trigger.mean(),
                "All 5 rooms represented":df.Room.nunique()==5,
                "All 12 slots represented":df.Draft_slot.nunique()==12,
            }])
            st.markdown("#### Certification breadth gate")
            st.dataframe(gate.round(4),use_container_width=True,hide_index=True)

            if len(df)>=int(target_sets):
                robust=(
                    df.Room.nunique()==5
                    and df.Draft_slot.nunique()==12
                    and room_spread<=2.0
                    and slot_spread<=2.0
                    and df.Overall_grade.std()<=6.0
                    and df.R2_trigger.mean()>=.10
                    and df.R3_trigger.mean()>=.10
                )
                if robust:
                    st.success(
                        "V8.11 ROBUSTNESS CERTIFICATION PASSED. V8.05 is stable across all rooms and slots on the selected out-of-sample target."
                    )
                else:
                    st.warning(
                        "V8.11 did not fully clear the frozen robustness gate. Keep V8.05 unchanged and inspect the stability issue before any new rule development."
                    )
            else:
                st.info(f"Progress saved: {len(df):,}/{target_sets:,}. Run the next batch to continue.")

            st.download_button(
                "⬇️ Download current V8.11 robustness certification CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_11_frozen_champion_robustness_certification.csv",
                "text/csv",key="v811_download"
            )
...
with tabs[3]:
    ...
    x=board[~board.player.isin(set(state["taken"]))].copy()
    x=_apply_player_pool_authority(x).sort_values("waiver_score",ascending=False)
