...
st.title("🏈 Fantasy Edge V7.35 — Pick 7 8/35 WR Intercept Champion Validation")
st.caption("V7.35 • V7.24 9× champion retained • Pick 7 8/35 champion validation")
st.caption("Build ID: V7.35-P7W835CV-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v735_live_slot")
    ...
    if "v735_target_queue" not in st.session_state:
        st.session_state.v735_target_queue={}
    queue=st.session_state.v735_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v735_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.35 keeps the V7.24 9× champion live/default while the Pick 7-only 8/35 WR intercept undergoes matched champion validation.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.35 robustness report")
        ...
        st.markdown("### 🧪 V7.35 champion baseline validation")
        st.info("V7.35 does not change live draft behavior. The candidate can fire only at Pick 7 in Rounds 4-6; Pick 12 and all other slots remain untouched.")
        ...
        if "v735_independent_results" not in st.session_state:
            st.session_state.v735_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v735_independent_results"),pd.DataFrame) else "ready to run"))
        if "v735_independent_summary" not in st.session_state:
            st.session_state.v735_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v735_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v735_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v735_run")
        clear_iv=clear_col.button("Clear independent results",key="v735_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v735_independent_results=iv
                st.session_state.v735_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v735_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v735_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.35 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.35 validates only the Pick 7 8/35 WR intercept identified by V7.34.")
            ...
            st.download_button(
                "⬇️ Download V7.35 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_35_champion_baseline.csv",
                "text/csv",
                key="v735_baseline_download"
            )
        ...
        st.markdown("### 🏆 V7.35 — Pick 7 8/35 WR Intercept Champion Validation")
        st.caption(
            "Final matched validation of the V7.34 screening winner. The V7.24 9× champion remains live/default. "
            "The candidate changes only Pick 7 in Rounds 4-6: RB/TE → WR conflict intercept at 8-point evaluation deficit / 35% survival."
        )

        if "v735_validation_results" not in st.session_state:
            st.session_state.v735_validation_results=None
        if "v735_validation_summary" not in st.session_state:
            st.session_state.v735_validation_summary=None

        val_sims=st.number_input(
            "Validation simulations per slot × room",
            min_value=5,max_value=25,value=10,step=5,key="v735_val_sims",
            help="Default 10 = 200 matched cases per model across slots 1/3/7/12 and five room profiles."
        )
        va,vb=st.columns(2)
        run_val=va.button("Run Pick 7 8/35 champion validation",type="primary",key="v735_val_run")
        clear_val=vb.button("Clear Pick 7 validation",key="v735_val_clear")
        ...
        if run_val:
            configs=[
                ("V7.24 CHAMPION • no Pick 7 intercept",False),
                ("V7.35 CANDIDATE • Pick 7 WR 8/35",True),
            ]
            val_slots=list(dict.fromkeys([x for x in [1,3,7,min(12,int(teams))] if 1<=x<=int(teams)]))
            val_rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]
            fast_champ=_fast_benchmark_pool(board,int(teams))

            rows=[]
            total=len(val_slots)*len(val_rooms)*int(val_sims)*len(configs)
            done=0
            prog=st.progress(0)
            status=st.empty()
            ...
            for ds in val_slots:
                for room_name in val_rooms:
                    for j in range(int(val_sims)):
                        seed=935000+int(ds)*10000+j*43+sum(ord(c) for c in room_name)
                        ...
                        for name,enabled in configs:
                            fe=simulate_mock_fast(
                                ...
                                late_wr_enabled=enabled,
                                late_wr_eval_deficit=8.0,
                                late_wr_survival=.35,
                                late_wr_slots=(7,)
                            )
                            fc=independent_draft_grade_components(fe,int(teams))
                            ...
                            row={
                                "Config":name,
                                "Pick7_intercept_enabled":enabled,
                                "Eval_deficit_cap":8.0,
                                "WR_survival_cap":.35,
                                "Eligible_slot":"7 only",
                                "Draft_slot":int(ds),
                                "Room":room_name,
                                "Simulation":j+1,
                                "FE_grade":fc["overall"],
                                "Consensus_grade":cc["overall"],
                                "Difference":fc["overall"]-cc["overall"],
                                "FE_win":fc["overall"]>cc["overall"],
                                "Market_value_difference":fc["market_value"]-cc["market_value"],
                                "Roster_balance_difference":fc["roster_balance"]-cc["roster_balance"],
                                "Starter_completion_difference":fc["starter_completion"]-cc["starter_completion"],
                                "Availability_difference":fc["availability"]-cc["availability"],
                                "FE_avg_reach":fc["avg_reach"],
                                "FE_avg_faller":fc["avg_faller"],
                                "R4_6_QB_count":int((r46.position=="QB").sum()),
                                "R4_6_RB_count":int((r46.position=="RB").sum()),
                                "R4_6_WR_count":int((r46.position=="WR").sum()),
                                "R4_6_TE_count":int((r46.position=="TE").sum()),
                            }
                            for phase,key in [
                                ("R1-3","R1_3"),("R4-6","R4_6"),
                                ("R7-10","R7_10"),("R11+","R11_plus")
                            ]:
                            ...
                            if done==1 or done%40==0 or done==total:
                                status.caption(f"Pick 7 champion validation: {done}/{total}")

            val=pd.DataFrame(rows)
            st.session_state.v735_validation_results=val
            st.session_state.v735_validation_summary={
                "seconds":time.perf_counter()-started,
                "sims":int(val_sims),
                "variant_sims":len(val)
            }
            status.success(f"Pick 7 champion validation complete: {len(val)} model simulations.")
            prog.empty()

        val_saved=st.session_state.get("v735_validation_results")
        if isinstance(val_saved,pd.DataFrame) and len(val_saved):
            meta=st.session_state.get("v735_validation_summary") or {}

            summary=val_saved.groupby(["Config","Pick7_intercept_enabled"],as_index=False).agg(
                Overall_gap=("Difference","mean"),
                FE_grade=("FE_grade","mean"),
                Win_rate=("FE_win","mean"),
                Market_gap=("Market_value_difference","mean"),
                Roster_balance=("Roster_balance_difference","mean"),
                Starter_gap=("Starter_completion_difference","mean"),
                Availability_gap=("Availability_difference","mean"),
                Avg_reach=("FE_avg_reach","mean"),
                Avg_faller=("FE_avg_faller","mean"),
                R1_3_gap=("R1_3_market_gap","mean"),
                R4_6_gap=("R4_6_market_gap","mean"),
                R7_10_gap=("R7_10_market_gap","mean"),
                R11_plus_gap=("R11_plus_market_gap","mean"),
                R4_6_QB=("R4_6_QB_count","mean"),
                R4_6_RB=("R4_6_RB_count","mean"),
                R4_6_WR=("R4_6_WR_count","mean"),
                R4_6_TE=("R4_6_TE_count","mean"),
                Tests=("Simulation","count")
            )

            champ=summary[summary.Config.eq("V7.24 CHAMPION • no Pick 7 intercept")].iloc[0]
            cand=summary[summary.Config.eq("V7.35 CANDIDATE • Pick 7 WR 8/35")].iloc[0]

            compare=pd.DataFrame([
                {"Metric":"Overall gap","Champion":champ.Overall_gap,"Candidate":cand.Overall_gap,
                 "Candidate change":cand.Overall_gap-champ.Overall_gap},
                {"Metric":"R1-3 gap","Champion":champ.R1_3_gap,"Candidate":cand.R1_3_gap,
                 "Candidate change":cand.R1_3_gap-champ.R1_3_gap},
                {"Metric":"R4-6 gap","Champion":champ.R4_6_gap,"Candidate":cand.R4_6_gap,
                 "Candidate change":cand.R4_6_gap-champ.R4_6_gap},
                {"Metric":"R7-10 gap","Champion":champ.R7_10_gap,"Candidate":cand.R7_10_gap,
                 "Candidate change":cand.R7_10_gap-champ.R7_10_gap},
                {"Metric":"R11+ gap","Champion":champ.R11_plus_gap,"Candidate":cand.R11_plus_gap,
                 "Candidate change":cand.R11_plus_gap-champ.R11_plus_gap},
                {"Metric":"Roster balance","Champion":champ.Roster_balance,"Candidate":cand.Roster_balance,
                 "Candidate change":cand.Roster_balance-champ.Roster_balance},
            ])
            st.dataframe(compare.round(3),use_container_width=True,hide_index=True)

            slot=val_saved.groupby(["Config","Draft_slot"]).agg(
                Overall_gap=("Difference","mean"),
                R1_3_gap=("R1_3_market_gap","mean"),
                R4_6_gap=("R4_6_market_gap","mean"),
                R7_10_gap=("R7_10_market_gap","mean"),
            ...
            champ7=slot[(slot.Config=="V7.24 CHAMPION • no Pick 7 intercept") & (slot.Draft_slot==7)].iloc[0]
            cand7=slot[(slot.Config=="V7.35 CANDIDATE • Pick 7 WR 8/35") & (slot.Draft_slot==7)].iloc[0]

            piv=val_saved.pivot_table(
                index=["Draft_slot","Room","Simulation"],
                columns="Config",
                values=["Difference","R4_6_market_gap","R1_3_market_gap","R7_10_market_gap"],
                aggfunc="first"
            )
            overall_delta=(
                piv[("Difference","V7.35 CANDIDATE • Pick 7 WR 8/35")]
                -piv[("Difference","V7.24 CHAMPION • no Pick 7 intercept")]
            )
            r46_delta=(
                piv[("R4_6_market_gap","V7.35 CANDIDATE • Pick 7 WR 8/35")]
                -piv[("R4_6_market_gap","V7.24 CHAMPION • no Pick 7 intercept")]
            )
            p7mask=piv.index.get_level_values("Draft_slot")==7
            p7_overall=overall_delta[p7mask]
            p7_r46=r46_delta[p7mask]

            c1,c2,c3,c4=st.columns(4)
            c1.metric("Pick 7 R4-6 improvement",f"{cand7.R4_6_gap-champ7.R4_6_gap:+.3f}")
            c2.metric("Pick 7 overall improvement",f"{cand7.Overall_gap-champ7.Overall_gap:+.3f}")
            c3.metric("Pick 7 paired overall wins",f"{(p7_overall>0).mean():.1%}")
            c4.metric("Pick 7 paired R4-6 improves",f"{(p7_r46>0).mean():.1%}")

            # Safety check: candidate should be mathematically identical outside Pick 7.
            outside=slot[slot.Draft_slot.isin([1,3,12])].pivot(
                index="Draft_slot",columns="Config",
                values=["Overall_gap","R1_3_gap","R4_6_gap","R7_10_gap","R11_plus_gap"]
            )
            max_outside_delta=0.0
            for metric in ["Overall_gap","R1_3_gap","R4_6_gap","R7_10_gap","R11_plus_gap"]:
                d=(
                    outside[(metric,"V7.35 CANDIDATE • Pick 7 WR 8/35")]
                    -outside[(metric,"V7.24 CHAMPION • no Pick 7 intercept")]
                ).abs().max()
                max_outside_delta=max(max_outside_delta,float(d))

            promote=(
                cand7.R4_6_gap>champ7.R4_6_gap
                and cand7.Overall_gap>=champ7.Overall_gap
                and (cand7.R1_3_gap-champ7.R1_3_gap)>-0.20
                and (cand7.R7_10_gap-champ7.R7_10_gap)>-0.30
                and (p7_r46>0).mean()>=.50
                and max_outside_delta<1e-9
            )

            st.caption(f"Maximum measured change outside Pick 7: {max_outside_delta:.6f}")

            if promote:
                st.success(
                    "PROMOTION GATE PASSED: Pick 7 8/35 improves the target R4-6 problem, preserves Pick 7 overall/adjacent phases, "
                    "and leaves Picks 1/3/12 unchanged. The next build can promote only this Pick 7 intercept."
                )
            else:
                st.warning(
                    "PROMOTION GATE NOT PASSED. Keep the untouched V7.24 champion and inspect the Pick 7 room-level results before changing live logic."
                )

            st.success(
                f"Saved V7.35 validation: {len(val_saved)} model simulations"
                + (f" • {meta.get('seconds',0):.1f}s" if meta else "")
            )
            st.download_button(
                "⬇️ Download V7.35 Pick 7 8/35 champion validation CSV",
                val_saved.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_35_pick7_835_champion_validation.csv",
                "text/csv",key="v735_val_download"
            )
