...
st.title("🏈 Fantasy Edge V7.61 — R7–10 Decision-Level Diagnostic")
st.caption("V7.61 • V7.60 frozen champion • R7–10 diagnostic only")
st.caption("Build ID: V7.61-R7-10-DIAGNOSTIC-20260816")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.61 leaves the certified V7.60 champion completely frozen and audits Rounds 7–10 at the individual-decision level.")
        ...
        st.markdown("### 🔬 V7.61 — R7–10 Decision-Level Diagnostic")
        st.caption(
            "Diagnostic only. V7.60 remains the frozen champion. This audit does not alter recommendations, weights, market transforms, "
            "VORP, roster utility, Pick-12 logic, or the certified R11+ architecture. It measures exactly what V7.60 is buying or paying for in Rounds 7–10."
        )

        if "v761_results" not in st.session_state:
            st.session_state.v761_results=None

        sims=st.number_input(
            "R7–10 diagnostic simulations per slot × room",
            min_value=10,max_value=50,value=25,step=5,key="v761_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 untouched V7.60 champion drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run R7–10 decision audit",type="primary",key="v761_run")
        clear=b.button("Clear V7.61 audit",key="v761_clear")
        if clear:
            st.session_state.v761_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=961000+int(ds)*100000+j*167+sum(ord(c) for c in room_name)
                        d=simulate_mock_fast(pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name)
                        mid=d[
                            pd.to_numeric(d.mock_round,errors="coerce").between(7,10,inclusive="both")
                        ].copy()

                        for _,r in mid.iterrows():
                            market=pd.to_numeric(pd.Series([r.get("market_pick",r.get("consensus_rank",np.nan))]),errors="coerce").iloc[0]
                            pick=pd.to_numeric(pd.Series([r.get("mock_pick",np.nan)]),errors="coerce").iloc[0]
                            model=pd.to_numeric(pd.Series([r.get("model_rank",np.nan)]),errors="coerce").iloc[0]
                            proj=pd.to_numeric(pd.Series([r.get("projection",np.nan)]),errors="coerce").iloc[0]
                            ...
                            rows.append({
                                ...
                                "Round":int(r.mock_round),"Mock_pick":pick,
                                "Player":str(r.player),"Position":str(r.position),
                                "Market_pick":market,"Market_delta":delta,
                                "Model_rank":model,"Projection":proj,"VORP":vorp,"Draft_score":score,
                                "Reach_gt10":bool(pd.notna(delta) and delta>10),
                                "Reach_gt20":bool(pd.notna(delta) and delta>20),
                                "Faller_gt10":bool(pd.notna(delta) and delta<-10),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%100==0 or done==total:
                            status.caption(f"V7.61 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v761_results=df
            status.success(f"V7.61 complete: {len(df)} R7–10 decisions audited.")
            prog.empty()

        df=st.session_state.get("v761_results")
        if isinstance(df,pd.DataFrame) and len(df):
            summary=pd.DataFrame([{
                "R7-10 decisions":len(df),
                "Average market delta":pd.to_numeric(df.Market_delta,errors="coerce").mean(),
                "Median market delta":pd.to_numeric(df.Market_delta,errors="coerce").median(),
                "Reach >10 rate":df.Reach_gt10.mean(),
                "Reach >20 rate":df.Reach_gt20.mean(),
                "Faller >10 rate":df.Faller_gt10.mean(),
                "Average model rank":pd.to_numeric(df.Model_rank,errors="coerce").mean(),
                "Average projection":pd.to_numeric(df.Projection,errors="coerce").mean(),
                "Average VORP":pd.to_numeric(df.VORP,errors="coerce").mean(),
                "Average draft score":pd.to_numeric(df.Draft_score,errors="coerce").mean(),
            }])
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            rnd=df.groupby("Round",as_index=False).agg(
                Decisions=("Player","count"),
                Market_delta=("Market_delta","mean"),
                Reach10=("Reach_gt10","mean"),
                Reach20=("Reach_gt20","mean"),
                Faller10=("Faller_gt10","mean"),
                Projection=("Projection","mean"),
                VORP=("VORP","mean"),
                Draft_score=("Draft_score","mean")
            )
            st.markdown("#### R7–10 by round")
            st.dataframe(rnd.round(4),use_container_width=True,hide_index=True)

            pos=df.groupby("Position",as_index=False).agg(
                Decisions=("Player","count"),
                Market_delta=("Market_delta","mean"),
                Reach10=("Reach_gt10","mean"),
                Reach20=("Reach_gt20","mean"),
            ...
            st.markdown("#### R7–10 by draft slot")
            ...
            st.markdown("#### R7–10 by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            players=df.groupby(["Player","Position"],as_index=False).agg(
                Selections=("Player","count"),
                Market_delta=("Market_delta","mean"),
                Model_rank=("Model_rank","mean"),
                Projection=("Projection","mean"),
                VORP=("VORP","mean"),
                Draft_score=("Draft_score","mean")
            ).sort_values(["Selections","Market_delta"],ascending=[False,False])
            st.markdown("#### Most common R7–10 selections")
            st.dataframe(players.head(30).round(3),use_container_width=True,hide_index=True)

            reaches=df[df.Reach_gt10].sort_values("Market_delta",ascending=False)
            st.markdown("#### Largest R7–10 reaches")
            st.dataframe(
                reaches[["Round","Draft_slot","Room","Player","Position","Mock_pick","Market_pick","Market_delta","Model_rank","Projection","VORP","Draft_score"]].head(50).round(3),
                use_container_width=True,hide_index=True
            )

            st.info(
                "V7.61 is measurement-only. Use this CSV to decide whether the +R7–10 market delta is broad healthy value capture, "
                "position-specific behavior, or a small set of repeated reach decisions worth counterfactual testing in V7.62."
            )

            st.download_button(
                "⬇️ Download V7.61 R7–10 decision audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_61_r7_10_decision_level_audit.csv",
                "text/csv",key="v761_download"
            )
