...
st.title("🏈 Fantasy Edge V7.74 — Final Independent Round-5 Confirmation")
st.caption("V7.74 • V7.67 frozen champion • final Round-5 confirmation")
st.caption("Build ID: V7.74-R5-FINAL-CONFIRMATION-20260817")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.74 keeps V7.67 frozen/live and performs the final independent 2,000-set confirmation of the exact Round-5 dominance rule against precommitted decision gates.")
        ...
        st.markdown("### 🔐 V7.74 — Final Independent Round-5 Confirmation")
        st.caption(
            "Final untouched confirmation of the exact frozen V7.71–V7.73 Round-5 dominance rule. "
            "V7.67 remains the live champion. No rule tuning is permitted. The decision gates below are precommitted before this run: "
            "positive overall and R4–6; non-negative aggregate R7–10 and R11+; ≥70% conditional whole-draft wins; "
            "≥95% conditional R4–6 wins; at least 4/5 rooms positive; and every active draft slot non-negative overall."
        )

        if "v774_results" not in st.session_state:
            st.session_state.v774_results=None

        sims=st.number_input(
            "V7.74 confirmation simulations per slot × room",
            min_value=50,max_value=150,value=100,step=10,key="v774_sims",
            help="Default: 4 slots × 5 rooms × 100 = 2,000 untouched matched sets. Trigger-first execution keeps runtime low."
        )
        a,b=st.columns(2)
        run=a.button("Run final V7.74 confirmation",type="primary",key="v774_run")
        clear=b.button("Clear V7.74 confirmation",key="v774_clear")
        if clear:
            st.session_state.v774_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=974000+int(ds)*100000+j*239+sum(ord(c) for c in room_name)
                        ...
                        rows.append({
                            ...
                            "Trigger_detected":trigger,"Candidate_rerun":trigger,
                            "Behavior_changed":changed>0,"Changed_picks":changed,
                            "Champion_round5_player":champ5,"Champion_round5_position":champ5_pos,
                            "Detected_best_alt":detected_alt,
                            "Candidate_round5_player":cand5,"Candidate_round5_position":cand5_pos,
                        ...
                        if done==1 or done%250==0 or done==total:
                            status.caption(f"V7.74: {done}/{total} matched sets • candidate reruns {reruns}")

            df=pd.DataFrame(rows)
            st.session_state.v774_results=df
            status.success(
                f"V7.74 complete: {len(df)} matched sets using "
                f"{len(df)+int(df.Candidate_rerun.sum())} full simulations instead of {len(df)*2}."
            )
            prog.empty()

        df=st.session_state.get("v774_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            active_slots=slot[slot.Activation>0].copy()
            nonnegative_active_slots=int((active_slots.Overall_change>=0).sum())

            gates={
                "Overall > 0": overall>0,
                "R4-6 > 0": r46>0,
                "R1-3 protected": r13>=0,
                "R7-10 >= 0": r710>=0,
                "R11+ >= 0": r11>=0,
                "Conditional overall win >= 70%": pd.notna(cond) and cond>=.70,
                "Conditional R4-6 win >= 95%": pd.notna(cond46) and cond46>=.95,
                "At least 4/5 rooms positive": positive_rooms>=4,
                "Every active slot non-negative": nonnegative_active_slots==len(active_slots),
            }
            certified=all(gates.values())

            summary=pd.DataFrame([{
                "Matched sets":len(df),"Triggered sets":len(trig),
                "Full simulations executed":full_sims,"Simulation-work reduction":saved,
                "Overall change":overall,"R1-3 change":r13,"R4-6 change":r46,
                "R7-10 change":r710,"R11+ change":r11,"Activation":activation,
                "Conditional overall win":cond,"Conditional non-loss":nonloss,
                "Conditional R4-6 win":cond46,"Positive rooms":positive_rooms,
                "Non-negative active slots":nonnegative_active_slots,"Active slots":len(active_slots),
            }])
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Final confirmation by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Final confirmation by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            gate_df=pd.DataFrame([{"Gate":k,"Passed":v} for k,v in gates.items()])
            st.markdown("#### Precommitted V7.74 decision gates")
            ...
            st.markdown("#### Frozen Round-5 paths")
            st.dataframe(paths.head(30).round(4),use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.74 FINAL CONFIRMATION PASSED. The exact frozen Round-5 dominance rule clears every precommitted gate and is eligible for production promotion."
                )
            else:
                st.error(
                    "V7.74 FINAL CONFIRMATION FAILED. Per the precommitted decision rule, retire the Round-5 challenger and retain V7.67 as champion. Do not retune against this sample."
                )

            st.download_button(
                "⬇️ Download V7.74 final Round-5 confirmation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_74_final_round5_dominance_confirmation.csv",
                "text/csv",key="v774_download"
            )
