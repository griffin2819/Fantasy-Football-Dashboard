...
st.title("🏈 Fantasy Edge V7.62 — Round-7 Reach Counterfactual Audit")
st.caption("V7.62 • V7.60 frozen champion • Round-7 counterfactual only")
st.caption("Build ID: V7.62-R7-REACH-CF-20260816")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            ...
            st.caption("V7.62 leaves V7.60 completely frozen and audits only Round-7 >10-pick reaches.")
        ...
        st.markdown("### 🧪 V7.62 — Round-7 Reach Counterfactual Audit")
        st.caption("Diagnostic only. V7.60 remains frozen. This targets only Round-7 champion selections reaching >10 picks versus effective market; Rounds 8–10 and R11+ are untouched.")
        if "v762_results" not in st.session_state: st.session_state.v762_results=None
        sims=st.number_input("V7.62 simulations per slot × room",10,50,25,5,key="v762_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 fresh champion drafts.")
        a,b=st.columns(2)
        run=a.button("Run Round-7 reach counterfactual",type="primary",key="v762_run")
        clear=b.button("Clear V7.62 audit",key="v762_clear")
        if clear:
            st.session_state.v762_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=962000+int(ds)*100000+j*173+sum(ord(c) for c in room_name)
                        champ=simulate_mock_fast(pool,int(teams),int(ds),15,state["roster_slots"],6.0,True,seed,room_name)
                        r7=champ[pd.to_numeric(champ.mock_round,errors="coerce").eq(7)]
                        if len(r7):
                            chosen=r7.iloc[0]
                            market=pd.to_numeric(pd.Series([chosen.get("market_pick",np.nan)]),errors="coerce").iloc[0]
                            pick=pd.to_numeric(pd.Series([chosen.get("mock_pick",np.nan)]),errors="coerce").iloc[0]
                            reach=market-pick if pd.notna(market) and pd.notna(pick) else np.nan
                            if pd.notna(reach) and reach>10:
                                prior=champ[pd.to_numeric(champ.mock_pick,errors="coerce") < pick]
                                drafted=set(prior.player.astype(str))
                                avail=p[~p.player.astype(str).isin(drafted)].copy()
                                avail["market_pick_cf"]=_market_pick_series(avail,int(teams))
                                legal=avail[pd.to_numeric(avail.market_pick_cf,errors="coerce") <= pick+10].copy()
                                legal["draft_score_cf"]=pd.to_numeric(legal.get("draft_score"),errors="coerce")
                                legal=legal.sort_values(["draft_score_cf","market_pick_cf"],ascending=[False,True])
                                if len(legal):
                                    alt=legal.iloc[0]
                                    def num(row,key):
                                    ...
                                    rows.append({
                                        ...
                                        "Round7_pick":pick,"Champion_player":str(chosen.player),"Champion_position":str(chosen.position),
                                        "Champion_market_pick":market,"Champion_reach":reach,
                                        "Champion_model_rank":num(chosen,"model_rank"),"Champion_projection":num(chosen,"projection"),
                                        "Champion_vorp":num(chosen,"vorp"),"Champion_draft_score":num(chosen,"draft_score"),
                                        "Alternative_player":str(alt.player),"Alternative_position":str(alt.position),
                                        "Alternative_market_pick":num(alt,"market_pick_cf"),
                                        "Alternative_market_delta":num(alt,"market_pick_cf")-pick,
                                        "Alternative_model_rank":num(alt,"model_rank"),"Alternative_projection":num(alt,"projection"),
                                        "Alternative_vorp":num(alt,"vorp"),"Alternative_draft_score":num(alt,"draft_score"),
                                        "Projection_change_alt_minus_champ":num(alt,"projection")-num(chosen,"projection"),
                                        "VORP_change_alt_minus_champ":num(alt,"vorp")-num(chosen,"vorp"),
                                        "Draft_score_change_alt_minus_champ":num(alt,"draft_score")-num(chosen,"draft_score"),
                                        "Market_improvement":market-num(alt,"market_pick_cf")
                                    })
                        done+=1; prog.progress(done/total)
                        if done==1 or done%100==0 or done==total: status.caption(f"V7.62 audit: {done}/{total}")
            df=pd.DataFrame(rows)
            st.session_state.v762_results=df
            status.success(f"V7.62 complete: {len(df)} triggered Round-7 reaches audited.")
            prog.empty()

        df=st.session_state.get("v762_results")
        if isinstance(df,pd.DataFrame) and len(df):
            st.dataframe(pd.DataFrame([{
                "Triggered R7 reaches":len(df),"Average champion reach":df.Champion_reach.mean(),
                "Average market improvement":df.Market_improvement.mean(),
                "Alt projection change":df.Projection_change_alt_minus_champ.mean(),
                "Alt VORP change":df.VORP_change_alt_minus_champ.mean(),
                "Alt draft-score change":df.Draft_score_change_alt_minus_champ.mean()
            }]).round(4),use_container_width=True,hide_index=True)
            players=df.groupby(["Champion_player","Champion_position"],as_index=False).agg(
                Triggers=("Champion_player","count"),Champion_reach=("Champion_reach","mean"),
                Market_improvement=("Market_improvement","mean"),Projection_change=("Projection_change_alt_minus_champ","mean"),
                VORP_change=("VORP_change_alt_minus_champ","mean"),Draft_score_change=("Draft_score_change_alt_minus_champ","mean")
            ).sort_values("Triggers",ascending=False)
            st.markdown("#### Round-7 reach drivers")
            st.dataframe(players.head(25).round(3),use_container_width=True,hide_index=True)
            st.download_button("⬇️ Download V7.62 Round-7 reach counterfactual CSV",
                df.to_csv(index=False).encode("utf-8"),"fantasy_edge_v7_62_round7_reach_counterfactual_audit.csv",
                "text/csv",key="v762_download")
