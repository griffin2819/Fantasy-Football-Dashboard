...
st.title("🏈 Fantasy Edge V8.06.2 — Frozen Champion Whole-Draft Opportunity Audit")
st.caption("V8.06.2 • V8.05 frozen champion • opportunity audit hotfix")
st.caption("Build ID: V8.06.2-WHOLE-DRAFT-AUDIT-HOTFIX-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        sims=st.number_input(
            "V8.06.2 simulations per slot × room",
        ...
        a,b=st.columns(2)
        run=a.button("Run V8.06.2 opportunity audit",type="primary",key="v806_run")
        clear=b.button("Clear V8.06.2 audit",key="v806_clear")
        ...
        if run:
            ...
            pool_df=pool["pool"].copy()

            # V8.06.2: _fast_benchmark_pool is a dict. The canonical arrays below
            # are already aligned to pool_df row-for-row.
            pool_df["_audit_vorp"]=pool["vorp"]
            pool_df["_audit_score"]=pool["draft_score"]
            pool_df["_audit_market"]=pool["market_pick"]
                ...
                for room_name in rooms:
                    for j in range(int(sims)):
                        ...
                        for idx,(_,r) in enumerate(d.sort_values("mock_round").iterrows()):
                            ...
                            alive=pool_df[~pool_df["player"].astype(str).isin(drafted_names)].copy()
                            alt=alive[alive["player"].astype(str)!=str(r.get("player",""))].copy()

                            best_player=""; best_pos=""
                            best_vorp=best_score=best_market=np.nan
                            if len(alt):
                                alt["_vorp"]=pd.to_numeric(alt["_audit_vorp"],errors="coerce")
                                alt["_score"]=pd.to_numeric(alt["_audit_score"],errors="coerce")
                                alt["_market"]=pd.to_numeric(alt["_audit_market"],errors="coerce")
                                alt=alt.sort_values(
                        ...
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V8.06.2: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v806_results=df
            status.success(f"V8.06.2 complete: {len(df):,} champion decisions audited.")
            prog.empty()
        ...
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            st.download_button(
                ...
                "fantasy_edge_v8_06_2_frozen_champion_whole_draft_opportunity_audit.csv",
