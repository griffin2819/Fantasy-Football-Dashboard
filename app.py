...
def early_round_decision_audit_snapshot(fast, candidate_idx, eval_score, chosen_global,
                                        current_pick, next_pick, randomness=12):
    """Diagnostic-only R1-3 audit snapshot for a Fantasy Edge pick.

    Captures the chosen player plus best available alternatives by offensive position.
    It never changes the draft decision.
    """
    cand=np.asarray(candidate_idx,dtype=int)
    ev=np.asarray(eval_score,dtype=float)
    pos=fast["pos_code"]; market=fast["market_pick"]; model_rank=fast["model_rank"]
    pool=fast["pool"]
    chosen_global=int(chosen_global)

    local=np.flatnonzero(cand==chosen_global)
    chosen_eval=float(ev[int(local[0])]) if len(local) else np.nan
    chosen_market=float(market[chosen_global]) if np.isfinite(market[chosen_global]) else np.nan
    chosen_model=float(model_rank[chosen_global]) if np.isfinite(model_rank[chosen_global]) else np.nan
    chosen_pos=str(pool.iloc[chosen_global].position)
    chosen_player=str(pool.iloc[chosen_global].player)
    chosen_reach=max(chosen_market-float(current_pick),0) if np.isfinite(chosen_market) else np.nan
    chosen_fall=max(float(current_pick)-chosen_market,0) if np.isfinite(chosen_market) else np.nan
    chosen_surv=survival_probability(chosen_market,next_pick,randomness) if np.isfinite(chosen_market) else np.nan

    snap={
        "ER_Audit":1,
        "ER_Chosen_player":chosen_player,
        "ER_Chosen_position":chosen_pos,
        "ER_Chosen_eval":chosen_eval,
        "ER_Chosen_market_pick":chosen_market,
        "ER_Chosen_model_rank":chosen_model,
        "ER_Chosen_reach":chosen_reach,
        "ER_Chosen_faller":chosen_fall,
        "ER_Chosen_survival_est":chosen_surv,
        "ER_Best_alt_player":"",
        "ER_Best_alt_position":"",
        "ER_Best_alt_eval":np.nan,
        "ER_Best_alt_market_pick":np.nan,
        "ER_Best_alt_model_rank":np.nan,
        "ER_Best_alt_survival_est":np.nan,
        "ER_Best_alt_survived_actual":np.nan,
        "ER_Eval_gap_vs_best_alt":np.nan,
        "ER_Market_gap_vs_best_alt":np.nan,
        "ER_Best_alt_idx":None
    }

    # Best alternative at a different position from chosen.
    altloc=np.flatnonzero(pos[cand] != pos[chosen_global])
    if len(altloc):
        j=int(altloc[np.argmax(ev[altloc])]); ag=int(cand[j])
        amp=float(market[ag]) if np.isfinite(market[ag]) else np.nan
        amr=float(model_rank[ag]) if np.isfinite(model_rank[ag]) else np.nan
        asurv=survival_probability(amp,next_pick,randomness) if np.isfinite(amp) else np.nan
        snap.update({
            "ER_Best_alt_player":str(pool.iloc[ag].player),
            "ER_Best_alt_position":str(pool.iloc[ag].position),
            "ER_Best_alt_eval":float(ev[j]),
            "ER_Best_alt_market_pick":amp,
            "ER_Best_alt_model_rank":amr,
            "ER_Best_alt_survival_est":asurv,
            "ER_Eval_gap_vs_best_alt":chosen_eval-float(ev[j]),
            "ER_Market_gap_vs_best_alt":(
                chosen_market-amp if np.isfinite(chosen_market) and np.isfinite(amp) else np.nan
            ),
            "ER_Best_alt_idx":ag
        })

    # Best available player by major offensive position.
    code_name={0:"QB",1:"RB",2:"WR",3:"TE"}
    for code,name in code_name.items():
        loc=np.flatnonzero(pos[cand]==code)
        if len(loc):
            j=int(loc[np.argmax(ev[loc])]); gi=int(cand[j])
            mp=float(market[gi]) if np.isfinite(market[gi]) else np.nan
            mr=float(model_rank[gi]) if np.isfinite(model_rank[gi]) else np.nan
            sv=survival_probability(mp,next_pick,randomness) if np.isfinite(mp) else np.nan
            snap[f"ER_Best_{name}_player"]=str(pool.iloc[gi].player)
            snap[f"ER_Best_{name}_eval"]=float(ev[j])
            snap[f"ER_Best_{name}_market_pick"]=mp
            snap[f"ER_Best_{name}_model_rank"]=mr
            snap[f"ER_Best_{name}_survival_est"]=sv
        else:
            snap[f"ER_Best_{name}_player"]=""
            snap[f"ER_Best_{name}_eval"]=np.nan
            snap[f"ER_Best_{name}_market_pick"]=np.nan
            snap[f"ER_Best_{name}_model_rank"]=np.nan
            snap[f"ER_Best_{name}_survival_est"]=np.nan
    return snap


def early_round_audit_summary(draft):
    """Aggregate one draft's R1-3 audit rows into compact CSV diagnostics."""
    if draft is None or len(draft)==0 or "ER_Audit" not in draft.columns:
        return {}
    er=draft[pd.to_numeric(draft.get("ER_Audit"),errors="coerce").fillna(0).eq(1)].copy()
    if er.empty:
        return {}
    out={
        "ER_Audit_picks":int(len(er)),
        "ER_avg_reach":float(pd.to_numeric(er.ER_Chosen_reach,errors="coerce").mean()),
        "ER_avg_faller":float(pd.to_numeric(er.ER_Chosen_faller,errors="coerce").mean()),
        "ER_avg_eval_gap_vs_alt":float(pd.to_numeric(er.ER_Eval_gap_vs_best_alt,errors="coerce").mean()),
        "ER_avg_market_gap_vs_alt":float(pd.to_numeric(er.ER_Market_gap_vs_best_alt,errors="coerce").mean()),
        "ER_alt_survival_est":float(pd.to_numeric(er.ER_Best_alt_survival_est,errors="coerce").mean()),
        "ER_alt_survival_actual":float(pd.to_numeric(er.ER_Best_alt_survived_actual,errors="coerce").mean()),
    }
    for p in ["QB","RB","WR","TE"]:
        g=er[er.ER_Chosen_position.eq(p)]
        out[f"ER_{p}_count"]=int(len(g))
        out[f"ER_{p}_avg_reach"]=float(pd.to_numeric(g.ER_Chosen_reach,errors="coerce").mean()) if len(g) else np.nan
        out[f"ER_{p}_avg_eval_gap"]=float(pd.to_numeric(g.ER_Eval_gap_vs_best_alt,errors="coerce").mean()) if len(g) else np.nan
        out[f"ER_{p}_alt_survival_actual"]=float(pd.to_numeric(g.ER_Best_alt_survived_actual,errors="coerce").mean()) if len(g) else np.nan

    alt_counts=er.ER_Best_alt_position.value_counts(normalize=True)
    for p in ["QB","RB","WR","TE"]:
        out[f"ER_Alt_{p}_share"]=float(alt_counts.get(p,0))
    return out
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None, room_profile="Consensus"):
    ...
    user_diag=[]
    pending_alt_idx=None; pending_alt_diag=None
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            if pending_alt_idx is not None and pending_alt_diag is not None:
                user_diag[pending_alt_diag]["ER_Best_alt_survived_actual"]=1.0 if alive[int(pending_alt_idx)] else 0.0
                pending_alt_idx=None; pending_alt_diag=None
            ...
            if model_user:
                ...
                diag={}
                if round_no<=3:
                    diag=early_round_decision_audit_snapshot(
                        fast,sidx,eval_score,chosen,overall,nxt,randomness
                    )
            else:
            ...
            if model_user and round_no<=3 and diag.get("ER_Best_alt_idx") is not None:
                pending_alt_idx=int(diag["ER_Best_alt_idx"])
                pending_alt_diag=len(user_diag)-1
            counts[pos[chosen]]+=1
        else:
...
st.title("🏈 Fantasy Edge V7.20 — Pick 7 Early-Round Decision Audit")
st.caption("V7.20 • V7.16 draft engine frozen • Pick 7 R1-3 decision audit • diagnostic only")
st.caption("Build ID: V7.20-P7ERDA-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v720_live_slot")
    ...
    if "v720_target_queue" not in st.session_state:
        st.session_state.v720_target_queue={}
    queue=st.session_state.v720_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v720_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.20 is diagnostic only. V7.16 draft logic is frozen while R1-3 decisions from Pick 7 are audited against Picks 1, 3 and 12.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.20 robustness report")
            ...
            st.download_button("⬇️ Download V7.20 diagnostic CSV",csv,"fantasy_edge_v7_20_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.20 Pick 7 audit diagnostics")
        ...
        if "v720_independent_results" not in st.session_state:
            st.session_state.v720_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v720_independent_results"),pd.DataFrame) else "ready to run"))
        if "v720_independent_summary" not in st.session_state:
            st.session_state.v720_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v720_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v720_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v720_run")
        clear_iv=clear_col.button("Clear independent results",key="v720_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                for ds in iv_slots:
                    for room_name in iv_rooms:
                        for j in range(int(iv_sims)):
                            ...
                            row.update(early_round_audit_summary(fe))
                ...
                st.session_state.v720_independent_results=iv
                st.session_state.v720_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v720_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v720_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🔍 V7.20 Pick 7 Early-Round Decision Audit")
            st.caption("Diagnostic only. V7.16 draft choices are frozen. Compare Pick 7's R1-3 decisions with Picks 1, 3 and 12.")

            er_summary=[]
            for ds,g in iv_saved.groupby("Draft_slot"):
                er_summary.append({
                    "Draft slot":int(ds),
                    "R1-3 audit picks":g.ER_Audit_picks.mean(),
                    "Avg reach":g.ER_avg_reach.mean(),
                    "Avg faller":g.ER_avg_faller.mean(),
                    "Eval edge vs best alt":g.ER_avg_eval_gap_vs_alt.mean(),
                    "Chosen-vs-alt market gap":g.ER_avg_market_gap_vs_alt.mean(),
                    "Alt survival estimate":g.ER_alt_survival_est.mean(),
                    "Alt actually survives":g.ER_alt_survival_actual.mean(),
                    "QB picks":g.ER_QB_count.mean(),
                    "RB picks":g.ER_RB_count.mean(),
                    "WR picks":g.ER_WR_count.mean(),
                    "TE picks":g.ER_TE_count.mean(),
                    "R1-3 market gap":g.R1_3_Avg_market_delta_difference.mean()
                })
            er_slot_df=pd.DataFrame(er_summary).sort_values("Draft slot")
            st.dataframe(er_slot_df.round(2),use_container_width=True,hide_index=True)

            p7=er_slot_df[er_slot_df["Draft slot"].eq(7)]
            others=er_slot_df[~er_slot_df["Draft slot"].eq(7)]
            if len(p7) and len(others):
                p7=p7.iloc[0]
                o=others.mean(numeric_only=True)
                a1,a2,a3,a4=st.columns(4)
                a1.metric("Pick 7 R1-3 reach",f"{p7['Avg reach']:.2f}",f"{p7['Avg reach']-o['Avg reach']:+.2f} vs other slots")
                a2.metric("Pick 7 eval edge",f"{p7['Eval edge vs best alt']:+.2f}",f"{p7['Eval edge vs best alt']-o['Eval edge vs best alt']:+.2f}")
                a3.metric("Pick 7 alt survives",f"{p7['Alt actually survives']:.0%}",f"{p7['Alt actually survives']-o['Alt actually survives']:+.0%}")
                a4.metric("Pick 7 R1-3 market gap",f"{p7['R1-3 market gap']:+.2f}",f"{p7['R1-3 market gap']-o['R1-3 market gap']:+.2f}")

            st.markdown("#### Position-specific early decisions by slot")
            pos_rows=[]
            for ds,g in iv_saved.groupby("Draft_slot"):
                for p in ["QB","RB","WR","TE"]:
                    pos_rows.append({
                        "Draft slot":int(ds),
                        "Position":p,
                        "Picks/draft":g[f"ER_{p}_count"].mean(),
                        "Avg reach":g[f"ER_{p}_avg_reach"].mean(),
                        "Eval edge vs best alt":g[f"ER_{p}_avg_eval_gap"].mean(),
                        "Alt actually survives":g[f"ER_{p}_alt_survival_actual"].mean()
                    })
            st.dataframe(pd.DataFrame(pos_rows).round(2),use_container_width=True,hide_index=True)

            st.markdown("#### What positions Fantasy Edge is passing on")
            alt_mix=[]
            for ds,g in iv_saved.groupby("Draft_slot"):
                alt_mix.append({
                    "Draft slot":int(ds),
                    "Passed QB":g.ER_Alt_QB_share.mean(),
                    "Passed RB":g.ER_Alt_RB_share.mean(),
                    "Passed WR":g.ER_Alt_WR_share.mean(),
                    "Passed TE":g.ER_Alt_TE_share.mean()
                })
            st.dataframe(pd.DataFrame(alt_mix).round(2),use_container_width=True,hide_index=True)

            # Automatic diagnosis for Pick 7.
            if len(p7) and len(others):
                diagnoses=[]
                if p7["Avg reach"] > o["Avg reach"] + 1.0:
                    diagnoses.append("higher reach/execution timing")
                if p7["Eval edge vs best alt"] < o["Eval edge vs best alt"] - 2.0:
                    diagnoses.append("weaker model separation from alternatives")
                if p7["Alt actually survives"] < o["Alt actually survives"] - .10:
                    diagnoses.append("scarcer alternatives disappearing before the next pick")
                if p7["WR picks"] < o["WR picks"] - .20:
                    diagnoses.append("lower early WR allocation")
                if p7["QB picks"] > o["QB picks"] + .20:
                    diagnoses.append("higher early QB allocation")
                if diagnoses:
                    st.warning("Pick 7 audit flags: " + "; ".join(diagnoses) + ".")
                else:
                    st.info("Pick 7 does not show one dominant structural difference in these diagnostics; the weakness may come from player-specific ranking interactions rather than a broad positional rule.")
            ...
            st.download_button(
                "⬇️ Download V7.20 Pick 7 audit diagnostics CSV",
                iv_csv,
                "fantasy_edge_v7_20_pick7_early_round_audit.csv",
                "text/csv",
                key="v720_download"
            )
