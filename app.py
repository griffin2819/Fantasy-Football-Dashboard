...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Waiver Instant Path", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
if _refresh_live:
    # Explicit refresh is the ONLY event allowed to invalidate provider caches.
    st.cache_data.clear()
    # V8.09: the weekly DataFrame no longer belongs inside league JSON/cloud state.
    # Remove any legacy embedded blob before later autosaves can serialize/upload it.
...
if _refresh_live:
    ...
    with st.spinner("Refreshing weekly data once…"):
        ...
        # 1) Sleeper directory is independent and is the primary current team/status truth.
        try:
            players=sleeper_players()
            if not isinstance(players,dict) or not players:
        ...
        # 2) Historical usage is optional for identity/injury/team truth.
        _v802_progress.info("Refresh step 2/6: historical stats")
        try:
            hist=nfl_history()
            if not isinstance(hist,pd.DataFrame) or hist.empty:
                raise RuntimeError("NFL history returned no usable rows")
            _v793_sources["history"]={"ok":True,"rows":int(len(hist))}
        except Exception as _e:
            hist=pd.DataFrame()
            _v793_sources["history"]={"ok":False,"error":f"{type(_e).__name__}: {_e}"}

        # 3) Market rankings are optional too.
        _v802_progress.info("Refresh step 3/6: bounded market download")
        try:
            market=market_rankings(ppr)
            if not isinstance(market,pd.DataFrame) or market.empty:
                raise RuntimeError("Consensus rankings returned no usable rows")
            _v793_sources["market"]={"ok":True,"rows":int(len(market))}
        except Exception as _e:
            market=pd.DataFrame()
            _v793_sources["market"]={"ok":False,"error":f"{type(_e).__name__}: {_e}"}

        _v802_progress.info("Refresh step 4/6: rebuilding the base model")
        # Rebuild the base model only when the required inputs are actually present.
        # Otherwise retain the certified snapshot and still keep any successful live
        # identity/status truth applied above.
        if isinstance(players,dict) and players and isinstance(hist,pd.DataFrame) and len(hist) and isinstance(market,pd.DataFrame) and len(market):
            try:
                _rebuilt=make_board(players,hist,market,ppr,pass_td,idp,teams,state["roster_slots"])
                _v802_progress.info("Refresh step 4/6: merging model and player identities")
                _rebuilt["market_pick"]=_market_pick_series(_rebuilt,teams)
                _rebuilt=attach_v9_production(_rebuilt)
                _rebuilt=_v938_unified_player_universe(_rebuilt)
                board=_rebuilt
                # Re-assert live identity immediately because model rebuilds should never
                # be allowed to replace a real NFL team with the generic "NFL" marker.
                board=_v792_apply_raw_sleeper_truth(board,players)
                _v793_sources["base_model"]={"ok":True,"rows":int(len(board))}
            except Exception as _e:
                _v793_sources["base_model"]={"ok":False,"error":f"{type(_e).__name__}: {_e}"}
        else:
            _v793_sources["base_model"]={"ok":False,"error":"Retained prior certified board because one or more model inputs were unavailable"}

        # Injury overlay can fetch team reports; show this separately from model work.
        _v802_progress.info("Refresh step 5/6: reconciling injury reports")
        ...
        _v802_progress.info("Refresh step 6/6: loading projections and matchup data")
...
try:
    if _refresh_live:
        _v802_progress.info("Refresh step 6/6: syncing the active roster directory")
...
if _refresh_live or not _v778_snapshot_loaded:
    if _refresh_live:
        _v802_progress.info("Refresh step 6/6: applying published projections")
...
if _refresh_live:
    ...
    try:
        ...
        pass
    _v802_progress.info("Refresh step 6/6: matching opponents and defenses")
    board,_v678_live_week=_v678_enrich_board_once(board)
    _v802_progress.info("Refresh step 6/6: matching observed player usage")
    ...
    try:
        _v802_progress.info("Refresh step 6/6: matching weekly projections")
...
if False and _refresh_live:
    ...
    _v802_progress.info("Refresh step 6/6: legacy duplicate save skipped")
    try:
...
try:
    ...
    if _refresh_live or not _v779_canonical_ready:
        if _refresh_live:
            _v802_progress.info("Refresh step 6/6: finalizing fantasy-relevant decision rows")
...
if _refresh_live:
    try:
        _v802_progress.info("Refresh step 6/6: writing fast local snapshot")
        ...
        if _v779_saved:
            ...
            _v778_snapshot_loaded=True
            _v802_progress.info("Refresh step 6/6: saving refresh metadata")
            _v809_persist_refresh_state_fast(state)
