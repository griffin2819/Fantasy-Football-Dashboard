...
st.title("🏈 Fantasy Edge V7.96 — Round-4 True-Board Failure Mechanism Audit")
st.caption("V7.96 • V7.93 frozen champion • Round-4 mechanism audit")
st.caption("Build ID: V7.96-R4-TRUEBOARD-MECHANISM-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V7.96 — Round-4 True-Board Failure Mechanism Audit")
        st.caption(
            "Measurement only. V7.93 remains the frozen champion. "
            "V7.96 captures the exact Round-4 true board across all 12 slots and all five room environments, "
            "including the champion selection, post-R1–3 roster state, and the top five genuinely available alternatives."
        )

        if "v796_results" not in st.session_state:
            st.session_state.v796_results=None

        sims=st.number_input(
            "V7.96 simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v796_sims",
            help="Default: 12 slots × 5 rooms × 10 = 600 fresh frozen-champion drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.96 Round-4 audit",type="primary",key="v796_run")
        clear=b.button("Clear V7.96 audit",key="v796_clear")
        if clear:
            st.session_state.v796_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=996000+int(ds)*100000+j*373+sum(ord(c) for c in room_name)
                        ...
                        if len(r4):
                            r=r4.iloc[0]
                            audit=int(pd.to_numeric(pd.Series([r.get("R796_Audit",0)]),errors="coerce").fillna(0).iloc[0])
                            if audit==1:
                                ...
                                row={
                                    ...
                                    "DL_count_pre":r.get("R796_DL_count_pre",0),
                                    "DB_count_pre":r.get("R796_DB_count_pre",0),
                                }
                                for k in range(1,6):
                        ...
                        if done%100==0 or done==total:
                            status.caption(f"V7.96: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v796_results=df
            status.success(f"V7.96 complete: {len(df)} Round-4 true-board decisions audited.")
            prog.empty()

        df=st.session_state.get("v796_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in ["Champion_VORP","Champion_draft_score","Alt1_vorp","Alt1_draft_score","Alt1_market_distance"]:
            ...
            for band in [5,10,15,20]:
                df[f"Sig_le_{band}"]=df.Alt1_dominates & (df.Alt1_market_distance<=band)
            ...
            st.markdown("#### Round-4 true-board mechanism by room")
            ...
            st.markdown("#### Round-4 mechanism by draft slot")
            ...
            st.markdown("#### Most repeated Round-4 fallback paths")
            ...
            st.markdown("#### Post-R1–3 roster-state signatures")
            st.dataframe(roster.head(40).round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.96 is measurement-only. Do not create a Round-4 challenger unless a generic true-board failure signature is repeated across rooms/slots "
                "and remains separate from the already-protected R2, R7, and late-round architecture."
            )

            st.download_button(
                "⬇️ Download V7.96 Round-4 failure mechanism audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_96_round4_true_board_failure_mechanism_audit.csv",
                "text/csv",key="v796_download"
            )
