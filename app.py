...
st.title("🏈 Fantasy Edge V7.26 — R4-6 Positional Utility Decomposition")
st.caption("V7.26 • V7.24 champion frozen • R4-6 evaluation-component decomposition")
st.caption("Build ID: V7.26-MRPUD-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v726_live_slot")
    ...
    if "v726_target_queue" not in st.session_state:
        st.session_state.v726_target_queue={}
    queue=st.session_state.v726_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v726_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.26 is diagnostic only. The V7.24 champion is frozen while the R4-6 evaluation advantage is decomposed into player score, roster marginal utility, and model-vs-market edge.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.26 robustness report")
        ...
        st.markdown("### 🧪 V7.26 R4-6 utility decomposition")
        st.info("V7.26 keeps the V7.24 champion frozen. The independent test now decomposes R4-6 RB/TE-over-WR/QB decisions without changing selections.")
        ...
        if "v726_independent_results" not in st.session_state:
            st.session_state.v726_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v726_independent_results"),pd.DataFrame) else "ready to run"))
        if "v726_independent_summary" not in st.session_state:
            st.session_state.v726_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v726_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v726_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v726_run")
        clear_iv=clear_col.button("Clear independent results",key="v726_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v726_independent_results=iv
                st.session_state.v726_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v726_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v726_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧩 V7.26 R4-6 Positional Utility Decomposition")
            st.caption("Diagnostic only. V7.24 champion logic is frozen. This decomposes the FE advantage that causes RB/TE selections to beat available WR/QB alternatives.")

            u1,u2,u3,u4=st.columns(4)
            u1.metric("R4-6 positional conflict",f"{iv_saved.MR_positional_conflict_rate.mean():.0%}")
            u2.metric("Conflict eval gap",f"{iv_saved.MR_conflict_eval_gap.mean():+.2f}")
            u3.metric("Conflict market gap",f"{iv_saved.MR_conflict_market_gap.mean():+.2f} picks")
            u4.metric("Conflict alternative mix",f"{iv_saved.MR_conflict_alt_WR_share.mean():.0%} WR / {iv_saved.MR_conflict_alt_QB_share.mean():.0%} QB")

            comp=pd.DataFrame([
                {"Component":"Player model / draft score",
                 "Chosen RB/TE advantage":iv_saved.MR_conflict_draftscore_gap.mean(),
                 "All R4-6 chosen vs market-best":iv_saved.MR_avg_component_draftscore_gap.mean()},
                {"Component":"Roster marginal utility",
                 "Chosen RB/TE advantage":iv_saved.MR_conflict_rosterdelta_gap.mean(),
                 "All R4-6 chosen vs market-best":iv_saved.MR_avg_component_rosterdelta_gap.mean()},
                {"Component":"Model-vs-market edge",
                 "Chosen RB/TE advantage":iv_saved.MR_conflict_edge_gap.mean(),
                 "All R4-6 chosen vs market-best":iv_saved.MR_avg_component_edge_gap.mean()},
            ])
            st.markdown("#### Exact evaluation decomposition")
            st.dataframe(comp.round(2),use_container_width=True,hide_index=True)

            drivers=pd.DataFrame([
                {"Primary driver":"Player model / draft score",
                 "Share":iv_saved.MR_driver_player_model_draft_score.mean()},
                {"Primary driver":"Roster marginal utility",
                 "Share":iv_saved.MR_driver_roster_marginal_utility.mean()},
                {"Primary driver":"Model-vs-market edge",
                 "Share":iv_saved.MR_driver_model_vs_market_edge.mean()},
            ])
            drivers["Share"]=drivers.Share.map(lambda x:f"{x:.0%}")
            st.markdown("#### Which component most often creates FE's advantage?")
            ...
            for p in ["QB","RB","WR","TE"]:
                pos_rows.append({
                    "Position":p,
                    "FE selected share":iv_saved[f"MR_Selected_{p}_share"].mean(),
                    "Market-best share":iv_saved[f"MR_MarketBest_{p}_share"].mean(),
                    "Passed-faller share":iv_saved[f"MR_PassedFaller_{p}_share"].mean(),
                })
            st.markdown("#### Position allocation context")
            ...
            for ds,g in iv_saved.groupby("Draft_slot"):
                slot_rows.append({
                    ...
                    "Roster-utility gap":g.MR_conflict_rosterdelta_gap.mean(),
                    "Market-edge gap":g.MR_conflict_edge_gap.mean(),
                })
            st.markdown("#### Decomposition by draft slot")
            st.dataframe(pd.DataFrame(slot_rows).round(2),use_container_width=True,hide_index=True)

            vals={
                "player model / draft score":float(iv_saved.MR_conflict_draftscore_gap.mean()),
                "roster marginal utility":float(iv_saved.MR_conflict_rosterdelta_gap.mean()),
                "model-vs-market edge":float(iv_saved.MR_conflict_edge_gap.mean())
            }
            dominant=max(vals,key=vals.get)
            domval=vals[dominant]
            total=sum(v for v in vals.values() if pd.notna(v))
            share=(domval/total) if total>0 else np.nan
            if pd.notna(share) and share>=.50:
                st.warning(
                    f"Primary R4-6 positional driver: {dominant} contributes about {share:.0%} of the "
                    f"RB/TE-over-WR/QB evaluation advantage ({domval:+.2f} points). "
                    "V7.27 should target this component only."
                )
            else:
                st.info(
                    "The R4-6 positional distortion is shared across multiple evaluation components. "
                    "Avoid a single-component correction until the slot table confirms a stable pattern."
                )
            ...
            st.download_button(
                "⬇️ Download V7.26 R4-6 utility decomposition CSV",
                iv_csv,
                "fantasy_edge_v7_26_mid_round_utility_decomposition.csv",
                "text/csv",
                key="v726_download"
            )
