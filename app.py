...
st.title("🏈 Fantasy Edge V7.58 — Frozen IDP Market-Fix Promotion Validation")
st.caption("V7.58 • V7.50 champion frozen • frozen IDP market-fix promotion validation")
st.caption("Build ID: V7.58-IDPMARKET-PROMOVAL-20260816")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v758_live_slot")
    ...
    if "v758_target_queue" not in st.session_state:
        st.session_state.v758_target_queue={}
    queue=st.session_state.v758_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v758_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.58 keeps V7.50 as the live/default champion with the 55.97 rule unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.58 robustness report")
        ...
        st.markdown("### 🧪 V7.58 champion baseline validation")
        st.info("V7.58 is promotion validation only. The frozen IDP market-pick fix remains off in the live/default champion until certification passes.")
        ...
        if "v758_independent_results" not in st.session_state:
            st.session_state.v758_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v758_independent_results"),pd.DataFrame) else "ready to run"))
        if "v758_independent_summary" not in st.session_state:
            st.session_state.v758_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v758_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v758_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v758_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v758_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v758_independent_results=iv
                st.session_state.v758_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v758_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v758_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.58 Champion Baseline Check")
            st.caption("V7.58 keeps V7.50 fully frozen and performs large fresh-seed validation of the exact V7.57 IDP market-pick integrity correction.")
            ...
            st.download_button(
                "⬇️ Download V7.58 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_58_champion_baseline.csv",
                "text/csv",
                key="v758_baseline_download"
            )
        ...
        st.markdown("### 🏁 V7.58 — Frozen IDP Market-Fix Promotion Validation")
        st.caption(
            "Large independent validation of the exact V7.57 market-integrity fix. "
            "V7.50 remains live/frozen. The challenger is unchanged: IDP effective market pick is capped at raw consensus + eight league rounds. "
            "No cap tuning, player exclusions, room-specific logic, or valuation changes are introduced."
        )

        if "v758_results" not in st.session_state:
            st.session_state.v758_results=None

        sims=st.number_input(
            "Promotion-validation simulations per slot × room",
            min_value=50,max_value=150,value=100,step=10,key="v758_sims",
            help="Default: 4 slots × 5 rooms × 100 simulations = 2,000 fresh matched champion-vs-finalist sets."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.58 promotion validation",type="primary",key="v758_run")
        clear=b.button("Clear V7.58 validation",key="v758_clear")
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=958000+int(ds)*100000+j*151+sum(ord(c) for c in room_name)
                        ...
                        rows.append({
                            ...
                            "Conditional_overall_nonloss":(overall>=0) if changed else np.nan,
                        ...
                        if done==1 or done%250==0 or done==total:
                            status.caption(f"V7.58 validation: {done}/{total} fresh matched sets")

            df=pd.DataFrame(rows)
            st.session_state.v758_results=df
            status.success(f"V7.58 complete: {len(df)} fresh matched sets.")
            prog.empty()

        df=st.session_state.get("v758_results")
        if isinstance(df,pd.DataFrame) and len(df):
            ...
            nonloss=changed.Conditional_overall_nonloss.mean() if len(changed) else np.nan
            ...
            slot=df.groupby("Draft_slot",as_index=False).agg(
                ...
                R1_3_change=("R1_3_change","mean"),
                R4_6_change=("R4_6_change","mean"),
                R7_10_change=("R7_10_change","mean"),
            ...
            st.markdown("#### Validation by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Validation by room profile")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            # Player concentration check for the candidate's final pick.
            p15=df.groupby("Candidate_round15_player",as_index=False).agg(
                Decisions=("Round15_changed","count"),
                Changed=("Round15_changed","sum")
            ).sort_values("Decisions",ascending=False)
            top_share=(p15.Decisions.max()/p15.Decisions.sum()) if len(p15) else np.nan

            st.markdown("#### Candidate Round-15 concentration")
            st.dataframe(p15.head(20),use_container_width=True,hide_index=True)

            positive_slots=int((slot.Overall_change>0).sum())
            positive_rooms=int((room.Overall_change>0).sum())
            positive_r11_slots=int((slot.R11_plus_change>0).sum())
            positive_r11_rooms=int((room.R11_plus_change>0).sum())
            ...
            certified=(
                ...
                and pd.notna(nonloss) and nonloss>=.85
                and pd.notna(cond11) and cond11>=.75
                and positive_slots==len(slot)
                and positive_rooms==len(room)
                and positive_r11_slots==len(slot)
                and positive_r11_rooms==len(room)
                and worst_slot>0
                and worst_room>0
                and (pd.isna(top_share) or top_share<.50)
            )
            ...
            gate=pd.DataFrame([{
                ...
                "Positive slots":positive_slots,
                "Positive rooms":positive_rooms,
                "Positive R11+ slots":positive_r11_slots,
                "Positive R11+ rooms":positive_r11_rooms,
                "Worst slot overall":worst_slot,
                "Worst room overall":worst_room,
                "Top Round-15 player share":top_share,
            }])

            st.markdown("#### V7.58 promotion certification")
            st.dataframe(gate.round(4),use_container_width=True,hide_index=True)

            if certified:
                st.success(
                    "V7.58 CERTIFIED. The exact V7.57 IDP market-integrity transformation clears large-sample promotion validation and is eligible to become the new champion."
                )
            else:
                st.warning(
                    "V7.58 does not clear the full promotion gate. Keep V7.50 frozen and inspect the failed dimension rather than retuning the cap."
                )

            st.download_button(
                "⬇️ Download V7.58 promotion validation CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_58_idp_market_fix_promotion_validation.csv",
                "text/csv",key="v758_download"
            )
