...
st.title("🏈 Fantasy Edge V7.60 — Frozen Champion Baseline")
st.caption("V7.60 • frozen certified champion baseline")
st.caption("Build ID: V7.60-FROZEN-CHAMPION-20260816")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v760_live_slot")
    ...
    if "v760_target_queue" not in st.session_state:
        st.session_state.v760_target_queue={}
    queue=st.session_state.v760_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v760_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.60 is the frozen live/default champion baseline; the 55.97 Pick-12 rule remains unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.60 robustness report")
        ...
        st.markdown("### 🧪 V7.60 champion baseline validation")
        st.info("V7.60 contains only the certified production market transform; retired V7.50 comparison and V7.57–V7.59 promotion machinery are removed.")
        ...
        if "v760_independent_results" not in st.session_state:
            st.session_state.v760_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v760_independent_results"),pd.DataFrame) else "ready to run"))
        if "v760_independent_summary" not in st.session_state:
            st.session_state.v760_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v760_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v760_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v760_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v760_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v760_independent_results=iv
                st.session_state.v760_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v760_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v760_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.60 Champion Baseline Check")
            st.caption("V7.60 freezes the certified V7.59 champion architecture as a clean production baseline with all retired challenger paths removed.")
            ...
            st.download_button(
                "⬇️ Download V7.60 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_60_champion_baseline.csv",
                "text/csv",
                key="v760_baseline_download"
            )
        ...
        st.markdown("### 🧊 V7.60 — Frozen Champion Baseline")
        st.caption(
            "Clean production baseline for the certified V7.59 champion architecture. "
            "The retired V7.50 comparison path and V7.57–V7.59 challenger/promotion machinery have been removed. "
            "This test checks deterministic stability and baseline behavior only; it does not compare against a challenger."
        )

        if "v760_results" not in st.session_state:
            st.session_state.v760_results=None

        sims=st.number_input(
            "Frozen-baseline simulations per slot × room",
            min_value=10,max_value=50,value=25,step=5,key="v760_sims",
            help="Default: 4 slots × 5 rooms × 25 = 500 clean V7.60 champion drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.60 frozen baseline",type="primary",key="v760_run")
        clear=b.button("Clear V7.60 baseline",key="v760_clear")
        if clear:
            st.session_state.v760_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=960000+int(ds)*100000+j*163+sum(ord(c) for c in room_name)

                        d1=simulate_mock_fast(
                            pool,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name
                        )
                        # Exact-seed repeat validates deterministic production stability.
                        d2=simulate_mock_fast(
                            pool,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name
                        )

                        comp=independent_draft_grade_components(d1,int(teams))
                        phase=independent_phase_metrics(d1).set_index("Phase")

                        p1=d1.set_index("mock_pick")["player"].astype(str).to_dict()
                        p2=d2.set_index("mock_pick")["player"].astype(str).to_dict()
                        deterministic=all(p1.get(k)==p2.get(k) for k in set(p1)|set(p2))

                        late=d1[pd.to_numeric(d1.mock_round,errors="coerce").fillna(0)>=11].copy()
                        if len(late):
                            mp=pd.to_numeric(late.get("market_pick",late.get("consensus_rank")),errors="coerce")
                            pk=pd.to_numeric(late.mock_pick,errors="coerce")
                            late_reach=float(np.maximum(mp-pk,0).mean())
                        else:
                            late_reach=np.nan

                        r15=d1[pd.to_numeric(d1.mock_round,errors="coerce").eq(15)]
                        r15_player=str(r15.iloc[0].player) if len(r15) else ""
                        r15_pos=str(r15.iloc[0].position) if len(r15) else ""

                        rows.append({
                            "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                            "Deterministic_repeat":deterministic,
                            "Overall_grade":comp["overall"],
                            "R1_3_market_delta":float(phase.loc["R1-3","Avg_market_delta"]),
                            "R4_6_market_delta":float(phase.loc["R4-6","Avg_market_delta"]),
                            "R7_10_market_delta":float(phase.loc["R7-10","Avg_market_delta"]),
                            "R11_plus_market_delta":float(phase.loc["R11+","Avg_market_delta"]),
                            "Late_reach":late_reach,
                            "Round15_player":r15_player,
                            "Round15_position":r15_pos,
                        })
                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%100==0 or done==total:
                            status.caption(f"V7.60 frozen baseline: {done}/{total}")

            df=pd.DataFrame(rows)
            st.session_state.v760_results=df
            status.success(f"V7.60 baseline complete: {len(df)} champion drafts.")
            prog.empty()

        df=st.session_state.get("v760_results")
        if isinstance(df,pd.DataFrame) and len(df):
            deterministic=float(df.Deterministic_repeat.mean())
            summary=pd.DataFrame([{
                "Champion drafts":len(df),
                "Deterministic repeat rate":deterministic,
                "Average overall grade":pd.to_numeric(df.Overall_grade,errors="coerce").mean(),
                "R1-3 market delta":pd.to_numeric(df.R1_3_market_delta,errors="coerce").mean(),
                "R4-6 market delta":pd.to_numeric(df.R4_6_market_delta,errors="coerce").mean(),
                "R7-10 market delta":pd.to_numeric(df.R7_10_market_delta,errors="coerce").mean(),
                "R11+ market delta":pd.to_numeric(df.R11_plus_market_delta,errors="coerce").mean(),
                "Average late reach":pd.to_numeric(df.Late_reach,errors="coerce").mean(),
            }])
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            slot=df.groupby("Draft_slot",as_index=False).agg(
                Overall_grade=("Overall_grade","mean"),
                R11_plus_market_delta=("R11_plus_market_delta","mean"),
                Late_reach=("Late_reach","mean"),
                Deterministic=("Deterministic_repeat","mean")
            )
            ...
            st.markdown("#### Frozen baseline by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)
            st.markdown("#### Frozen baseline by room")
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)

            r15=df.groupby(["Round15_player","Round15_position"],as_index=False).size()
            r15=r15.rename(columns={"size":"Selections"}).sort_values("Selections",ascending=False)
            top_share=float(r15.Selections.max()/r15.Selections.sum()) if len(r15) else np.nan
            st.markdown("#### Round-15 baseline distribution")
            st.dataframe(r15.head(25),use_container_width=True,hide_index=True)

            frozen_ok=(deterministic==1.0 and (pd.isna(top_share) or top_share<.50))
            if frozen_ok:
                st.success(
                    "V7.60 FROZEN BASELINE PASSED. The promoted champion production path is deterministic and the Round-15 pool remains diversified. "
                    "Use V7.60 as the clean champion baseline for future challengers."
                )
            else:
                st.error(
                    "V7.60 frozen-baseline check failed. Do not start a new challenger until deterministic stability or Round-15 concentration is diagnosed."
                )

            st.download_button(
                "⬇️ Download V7.60 frozen champion baseline CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_60_frozen_champion_baseline.csv",
                "text/csv",key="v760_download"
            )
