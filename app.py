...
st.title("🏈 Fantasy Edge V7.14 — Round-by-Round Value Attribution")
st.caption("V7.14 • V7.13 draft engine frozen • round-by-round market-value attribution • diagnostic only")
st.caption("Build ID: V7.14-RVA-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v714_live_slot")
    ...
    if "v714_target_queue" not in st.session_state:
        st.session_state.v714_target_queue={}
    queue=st.session_state.v714_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v714_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.13 draft logic is frozen: Faller Intercept thresholds remain R1-3 14+ • R4-6 11+ • R7-10 8+ • R11+ 6+. V7.14 only measures which phase is still losing market value.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.14 robustness report")
            ...
            st.download_button("⬇️ Download V7.14 diagnostic CSV",csv,"fantasy_edge_v7_14_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.14 independent validation")
        ...
        if "v714_independent_results" not in st.session_state:
            st.session_state.v714_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v714_independent_results"),pd.DataFrame) else "ready to run"))
        if "v714_independent_summary" not in st.session_state:
            st.session_state.v714_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v714_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v714_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v714_run")
        clear_iv=clear_col.button("Clear independent results",key="v714_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v714_independent_results=iv
                st.session_state.v714_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v714_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v714_independent_summary") or {}
            st.success(
            ...
            st.info(
                f"Largest remaining market-value drag is {worst_phase['Phase']} "
                f"({worst_phase['Market gap']:+.2f} vs consensus). "
                "V7.14 makes no model changes; use this phase to decide the next targeted release."
            )
            ...
            st.download_button(
                "⬇️ Download V7.14 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_14_round_value_attribution.csv",
                "text/csv",
                key="v714_download"
            )
