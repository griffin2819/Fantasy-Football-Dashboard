...
st.set_page_config(page_title="Fantasy Edge v9.46.2 — Exact League + Kicker Requirement", page_icon="🏈", layout="wide")
...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    # v9.46.2 vectorized same-position quality dominance.
    # Avoid O(n²) pairwise loops on every Streamlit rerun.
    ...
    _off_pos=x["position"].isin(["QB","RB","WR","TE"])
    if _off_pos.any():
        _tmp=pd.DataFrame({
            "position":x.loc[_off_pos,"position"],
            "projection":_proj_all.loc[_off_pos],
            "vorp":_vorp_all.loc[_off_pos],
        },index=x.index[_off_pos])

        # Position leaders provide a conservative dominance screen.
        # Candidate is dominated only if one same-position leader is no worse in both
        # projection and VORP and materially better in at least one.
        _grp=_tmp.groupby("position",sort=False)
        _max_proj=_grp["projection"].transform("max")
        _max_vorp=_grp["vorp"].transform("max")
        _dom=(
            _tmp["projection"].notna() & _tmp["vorp"].notna() &
            (_max_proj>=_tmp["projection"]) & (_max_vorp>=_tmp["vorp"]) &
            (((_max_proj-_tmp["projection"])>=0.35) | ((_max_vorp-_tmp["vorp"])>=0.35))
        )
        _gap=((_max_proj-_tmp["projection"]).clip(lower=0)*1.25 +
              (_max_vorp-_tmp["vorp"]).clip(lower=0)*2.0)
        _idx=_tmp.index[_dom]
        x.loc[_idx,"same_pos_dominated"]=True
        x.loc[_idx,"same_pos_quality_gap"]=_gap.loc[_idx]
        x.loc[_idx,"evaluation_score"]-=np.minimum(30.0,12.0+_gap.loc[_idx])
...
st.caption("Build ID: FE-V9.46.2-PERFORMANCE-VECTORIZED-20260821")
