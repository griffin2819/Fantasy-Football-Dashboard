...
st.set_page_config(page_title="Fantasy Edge v9.44 — Exact League + Kicker Requirement", page_icon="🏈", layout="wide")
...
with tabs[2]:
    ...
    st.success("🔒 v9.44 SAME Draft/Mock • TE + K required • 🧠 honest IDP-relative metrics • 🐍 12-team, 17-round snake")

    m=state["mock"]
    c1,c2,c3=st.columns(3)
    slot=c1.number_input("Your draft slot",1,12,min(int(m.get("draft_slot",1)),12))
    rounds=exact_roster_rounds(state["roster_slots"])
    c2.number_input("Rounds",rounds,rounds,rounds,disabled=True,help="Locked to the exact 17-player roster.")
    ...
    st.caption("V9.08 remains the frozen certified core; V9.44 applies exact 1QB/2RB/2WR/1TE/2FLEX/1K/1DL/1DB/6-bench construction (17 rounds), availability, injury, and impact-IDP overlays without changing the frozen core.")
            ...
            if owner==int(slot):
                ...
                if not pick_options:
                    ...
                    if st.button("Draft this player"):
                        ...
                        _commit,_=_v939_shared_candidate_rank(
