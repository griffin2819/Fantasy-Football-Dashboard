...
st.set_page_config(page_title="Fantasy Edge V6.91 — Weekly Command Center", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V6.91-WEEKLY-COMMAND-CENTER"
...
def _owner_key(name):
    ...
    s=re.sub(r"[^a-z0-9]","",s)
    aliases={"kenwalker":"kennethwalker","kennethwalkeriii":"kennethwalker"}
...
def _is_free_agent(s,name):
    return _player_owner(s,name)=="FA"
...
def _v681_sleeper_injury_watch():
    ...
    try:
        ...
        st.session_state["_v681_sleeper_injury_error"]=str(e)
    return out
...
def _v680_espn_injury_watch_feed():
    ...
    try:
        ...
        st.session_state["_v680_injury_watch_error"]=str(e)
    return out
...
def _v679_espn_roster_status():
    ...
    try:
        ...
        st.session_state["_v679_status_error"]=str(e)
    return out
...
def _v677_week_matchups(season, week):
    ...
    try:
        ...
        st.session_state["_v677_schedule_error"]=str(e)
        return {}
...
def _v683_weekly_usage_table(season):
    ...
    try:
        ...
        st.session_state["_v683_usage_error"]=str(e)
        return pd.DataFrame()
...
def _v684_week_actuals(season,week,idp_cfg_json="{}"):
    ...
    try:
        ...
        st.session_state["_v684_actuals_error"]=str(e)
        return pd.DataFrame()
...
def _v690_trade_value(row):
    return _v682_ros_value(row)+_v676_player_value(row)*.55
...
tabs=st.tabs(["⚙️ League Setup","🎯 Draft Mode","🧪 Mock Draft Lab","🧲 Add / Drop","🛡️ IDP Board","📈 Breakout / Regression","🏁 Start / Sit","🔎 Player Lab","📊 Accuracy Lab","🧠 IDP Intelligence","📡 Consensus Watch","🚨 Opportunity Detector","🕵️ League Intel","🔁 Trade Analyzer","🏠 Command Center"])
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        if _moves_df.empty:
            ...
            _rank={"🔥 PRIORITY CLAIM":4,"⬆️ ADD":3,"🧳 STASH":2,"👀 WATCH":1,"⛔ PASS":0}
            _moves_df["_rank"]=_moves_df["Action"].map(_rank).fillna(0)
            _moves_df=_moves_df.sort_values(["_rank","Net ROS","Net Week","Net Roster"],ascending=False)
            _best=_moves_df.groupby("Add",as_index=False,sort=False).head(1).copy()

            if _min_move=="WATCH or better":
                _best=_best[_best["_rank"]>=2]
            elif _min_move=="ADD or better":
                _best=_best[_best["_rank"]>=3]
            elif _min_move=="PRIORITY only":
                _best=_best[_best["_rank"]>=4]

            st.markdown("### Best moves for your roster")
            st.dataframe(_best[["Add","Pos","Action","Net Week","Net ROS","Add Role","Add Recent PPG","Add Status","Add Practice","Add Injury","Add Matchup","Drop","Drop Pos","Drop Role","Drop Recent PPG","Drop Protection","Drop Status","Drop Practice","Add PPG","Drop PPG","Add ROS","Drop ROS","Add VORP","Drop VORP","Why"]].head(40),
                         use_container_width=True,hide_index=True)
        ...
        st.caption("V6.83 adds role/opportunity trend and recent production to the transaction engine; it still does not execute roster moves.")
...
with tabs[10]:
    st.subheader("📡 Consensus Watch")
    st.caption("Flags disagreement between core Fantasy Edge projection and its live role/performance-adjusted decision layer.")
    x=board.copy(); c=x.apply(_v687_consensus_proxy,axis=1,result_type="expand"); c.columns=["Live Proxy","Difference","Flag"]
    x=pd.concat([x.reset_index(drop=True),c.reset_index(drop=True)],axis=1); x["Abs Diff"]=x["Difference"].abs()
    x=x.sort_values("Abs Diff",ascending=False)
    st.dataframe(x[["player","position","team","projection","Live Proxy","Difference","_recent_ppg3","_role_trend","_matchup_grade","Flag"]].head(100).rename(columns={"player":"Player","position":"Pos","team":"Team","projection":"Fantasy Edge","_recent_ppg3":"Recent PPG","_role_trend":"Role","_matchup_grade":"Matchup"}),use_container_width=True,hide_index=True)
    st.caption("This is an internal consensus proxy, not yet a multi-provider market projection feed.")
...
with tabs[12]:
    st.subheader("🕵️ League Intelligence")
    st.caption("Analyzes all 12 rosters for relative strength, weakness and likely positional need.")
    rows=[]
    for owner in [str(i) for i in range(1,13)]:
        d=_v689_team_strength(board,state,owner); d["Team"]=state.get("team_names",{}).get(owner,f"Team {owner}"); rows.append(d)
    df=pd.DataFrame(rows).sort_values("score",ascending=False)
    st.dataframe(df[["Team","score","strength","weakness","roster"]].rename(columns={"score":"Lineup Score","strength":"Strength","weakness":"Need","roster":"Rostered"}),use_container_width=True,hide_index=True)
    st.caption("Need can help identify trade partners and waiver competition.")

# ---------------- V6.90 Trade Analyzer ----------------
with tabs[13]:
    st.subheader("🔁 Trade Analyzer")
    st.caption("Evaluates trades by their impact on your optimized roster plus asset value.")
    my=board[board["player"].map(lambda n:_player_owner(state,n)=="1")].copy(); other=board[board["player"].map(lambda n:_player_owner(state,n) not in ("1","FA"))].copy()
    if my.empty or other.empty: st.info("Need your roster plus opponent rosters to evaluate trades.")
    else:
        give=st.multiselect("You give",sorted(my["player"].tolist()),max_selections=3,key="v690_give")
        get=st.multiselect("You receive",sorted(other["player"].tolist()),max_selections=3,key="v690_get")
        if give and get:
            ev=_v690_trade_eval(my[my["player"].isin(give)].copy(),other[other["player"].isin(get)].copy(),my)
            if ev:
                a,b,c=st.columns(3); a.metric("Roster gain",f"{ev['Roster Gain']:+.2f}"); b.metric("Asset value",f"{ev['Net Asset Value']:+.2f}"); c.metric("Verdict",ev["Verdict"])
...
with tabs[14]:
    ...
    if my.empty: st.info("Set your roster under League Setup to populate the Command Center.")
        ...
        actions=[]
        if top is not None: actions.append({"Priority":"Waiver","Action":f"Top add: {top.get('player','')}","Why":f"{_v683_role_summary(top)} • ROS {_v682_ros_value(top):.1f}"})
        for _,r in injured.head(3).iterrows(): actions.append({"Priority":"Injury","Action":f"{r['player']}: {_v682_injury_action(r)}","Why":f"{_v677_normalize_injury_status(r.get('injury',''))} • {_v682_injury_risk(r)}"})
        opp=board[board.apply(lambda r:bool(_v688_opportunity_flag(r)),axis=1)].copy()
        if len(opp):
            r=opp.iloc[0]; actions.append({"Priority":"Opportunity","Action":f"Watch {r['player']}","Why":_v688_opportunity_flag(r)+" • "+_v683_role_summary(r)})
        st.markdown("### Action items")
        if actions: st.dataframe(pd.DataFrame(actions),use_container_width=True,hide_index=True)
        else: st.success("No urgent roster actions detected.")
