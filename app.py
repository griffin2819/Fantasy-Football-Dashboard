...
st.set_page_config(page_title="Fantasy Edge V7.03 — Complete Waiver Decision System", page_icon="🏈", layout="wide")
APP_DIR = Path(__file__).resolve().parent
V651_BUILD_ID = "V7.03-COMPLETE-WAIVER-DECISION-SYSTEM"
...
def _cloud_load():
    url,_=_cloud_config()
    endpoint=f"{url}/rest/v1/fantasy_edge_state?league_key=eq.{CLOUD_LEAGUE_KEY}&select=state"
...
def save_state(s):
    """Save locally first, then sync the same league state to Supabase."""
    STATE.write_text(json.dumps(s,indent=2))
    try:
        _cloud_save(s)
        # Verify the cloud now returns the same team-name/ownership payload.
        chk=_cloud_load()
        if not isinstance(chk,dict):
            raise RuntimeError("Cloud verification returned no state.")
        st.session_state["_v657_cloud_ok"]=True
        st.session_state["_v657_cloud_error"]=""
    except Exception as e:
        st.session_state["_v657_cloud_ok"]=False
        st.session_state["_v657_cloud_error"]=repr(e)
...
def _v637_consensus_directory_ranks():
    ...
    try:
        ...
        st.session_state["v637_rank_error"]=str(e)
...
def _v680_injury_watch_feed():
    ...
    for key,rec in (espn or {}).items():
        ...
        for fld in ["practice","injury","updated","team"]:
...
_V679_VERIFIED_RESERVE = {
    _owner_key("Tank Dell"): {"status":"IR","source":"Official team reserve list"},
...
def apply_v92_injury_overlay(board,state):
    ...
    for i,row in b.iterrows():
        ...
        watch=_v680_watch.get(key,{}) if isinstance(_v680_watch,dict) else {}
...
state=load_state()
# Cloud is authoritative when reachable. First run seeds cloud from the packaged/local state.
try:
    _remote=_cloud_load()
    ...
    st.session_state["_v657_cloud_ok"]=False
    st.session_state["_v657_cloud_error"]=str(_cloud_e)
...
@st.cache_data(ttl=1800, show_spinner=False)
def _v678_build_matchup_lookup(board_df, season, week):
    """Return one compact player->matchup context lookup for all tabs."""
...
def _v678_enrich_board_once(board_df):
    ...
    lookup=_v678_build_matchup_lookup(
    ...
    if len(lookup):
        ...
        out["_opponent"]="BYE / TBD"
        out["_matchup_grade"]="—"
        out["_matchup_adjustment"]=0.0
        out["_matchup_source"]="No matchup"

    out["_opponent"]=out["_opponent"].fillna("BYE / TBD")
...
def _v677_week_matchups(season, week):
    """NFL opponent map for a specific week via ESPN's public scoreboard feed."""
    out={}
    try:
        url=(
...
def _v677_matchup_context(row, matchup_map, allowed_table):
    ...
    if not opp:
        return {"opponent":"BYE / TBD","site":"","grade":"—","index":0.0,"adjustment":0.0,"source":"No matchup"}
    idx=0.0
...
def _v676_player_value(row):
    ...
    return max(0.0,base * _v676_injury_multiplier(row.get("injury",""),row.get("practice_status","")))
...
def _v683_enrich_role(board_df,season):
    ...
    defaults={
        ...
        "_usage_source":"Unavailable","_snaps_last":0.0,"_snap_pct_last":0.0,
...
def _v683_data_health(board_df,season,week):
    ...
    role=int((pd.to_numeric(board_df.get("_usage_games",pd.Series(0,index=board_df.index)),errors="coerce").fillna(0)>0).sum())
    return {
...
def _v682_drop_protection(row):
    ...
    if status in ("IR","PUP","NFI") and (vorp>0 or rank<=180 or ros>=max(5.0,proj)):
...
def _v684_build_snapshot(board_df,state,season,week,waiver_df=None):
    ...
    for _,sr in starters.iterrows():
        ...
        conf=_v682_start_confidence(src,edge)
        calls.append({
...
def _v703_need_adjustment(position,roster_df,state):
    """Small roster-construction adjustment; never overrides transaction value."""
    ...
    if have<required: return 6.0,"Critical need"
    if pos in ("DL","DB"):
        if have>=required+1: return -8.0,"IDP depth already covered"
        if have>=required: return -3.0,"IDP starter already covered"
    if pos in ("QB","TE","K") and required and have>=required+1:
        return -5.0,"Position depth already covered"
    if pos in ("RB","WR"):
        flex=int(slots.get("FLEX",0) or 0)
        target=required+max(1,(flex+1)//2)+1
        if have<required+1: return 4.0,"Roster need"
        if have>=target+2: return -4.0,"Deep position already"
    return 0.0,"Balanced"
...
def _v703_evidence_quality(row):
    ...
    if _v682_num(row,"_usage_games",0)>0: evidence+=1
    return evidence
...
def _v703_why_not(label,net_week,net_ros,need_label,evidence):
    ...
    if evidence<=1: reasons.append("limited live-data support")
...
def _v697_waiver_priority_score(add_row,drop_row,state,net_week=None,net_ros=None,net_roster=None,need_adjustment=0.0):
    """Calibrated claim urgency based on the simulated roster transaction."""
    ...
    injury_bonus=1.0 if _v677_normalize_injury_status(drop_row.get("injury","")) in ("OUT","IR","PUP","NFI","DOUBTFUL") else 0.0
    position=str(add_row.get("position","") or "").upper()
    score=(42.0+max(-14.0,min(18.0,net_week*6.0))+max(-12.0,min(16.0,net_ros*4.0))
           +max(-8.0,min(8.0,net_roster*2.0))+max(-4.0,min(5.0,role*5.0))
           +max(0.0,min(4.0,scarcity*4.0))+injury_bonus*3.0
           +max(-8.0,min(6.0,float(need_adjustment))))
    if position in ("DL","DB") and net_week<0.75 and net_ros<2.0: score-=6.0
    ...
    if position in ("DL","DB"): priority_gain=priority_gain and (net_week>=0.75 or net_ros>=2.5)
    evidence=_v703_evidence_quality(add_row)
    if evidence<=1: score=min(score,84.0)
    if score>=88 and priority_gain and evidence>=2: label="🔥 PRIORITY CLAIM"
    elif score>=72 and meaningful: label="⬆️ ADD NOW"
    elif score>=62 and net_week<0.75 and net_ros>=0.40: label="🧳 STASH"
    elif score>=58 and net_week>=0.35: label="🎯 STREAMER"
    elif score>=48: label="👀 WATCH"
    else: label="⛔ PASS"
...
def _v694_trade_partner_suggestions(board_df,state,max_rows=20):
    ...
    my_need=_v694_position_need(my)
    suggestions=[]

    # User offer pool: non-core depth first, but allow strong assets for meaningful upgrades.
    my_pool=my.copy()
    ...
    for owner in [str(i) for i in range(2,13)]:
        ...
        if opp.empty:
            continue
        opp_need=_v694_position_need(opp)
        team_name=state.get("team_names",{}).get(owner,f"Team {owner}")

        targets=opp[opp["position"].eq(my_need)].copy()
        if targets.empty:
            # If exact need unavailable, allow best opponent assets.
            targets=opp.copy()
        targets["_tv"]=targets.apply(_v690_trade_value,axis=1)
        targets=targets.sort_values("_tv",ascending=False).head(5)

        offers=my_pool[my_pool["position"].eq(opp_need)].copy()
        if offers.empty:
            offers=my_pool.copy()
        offers=offers.sort_values("_tv",ascending=False).head(7)

        opp_before=_v676_roster_score(opp)
        my_before=_v676_roster_score(my)

        for _,target in targets.iterrows():
            for _,offer in offers.iterrows():
                # Avoid recommending absurd value gaps.
                tv_gap=float(target["_tv"])-float(offer["_tv"])
                if abs(tv_gap)>8.0:
                    continue

                my_after=my[~my["player"].eq(offer["player"])].copy()
                my_after=pd.concat([my_after,pd.DataFrame([target])],ignore_index=True)
                opp_after=opp[~opp["player"].eq(target["player"])].copy()
                opp_after=pd.concat([opp_after,pd.DataFrame([offer])],ignore_index=True)

                my_gain=_v676_roster_score(my_after)-my_before
                opp_gain=_v676_roster_score(opp_after)-opp_before

                # Realistic = helps user and is not clearly harmful to partner.
                if my_gain<=0.10 or opp_gain<-0.75:
                    continue

                realism=100.0-abs(tv_gap)*6.0-max(0,-opp_gain)*18.0
                realism=max(1.0,min(99.0,realism))
                suggestions.append({
                    "Partner":team_name,
                    "Their Need":opp_need,
                    "Your Need":my_need,
                    "You Give":offer["player"],
                    "You Get":target["player"],
                    "Your Gain":round(my_gain,2),
                    "Partner Gain":round(opp_gain,2),
                    "Asset Gap":round(-tv_gap,2),
                    "Realism":round(realism,0),
                    "Why":f"You address {my_need}; {team_name} addresses {opp_need}"
                })

    if not suggestions:
        return pd.DataFrame()
    out=pd.DataFrame(suggestions)
    out["_rank"]=out["Your Gain"]*2.0+out["Partner Gain"]+out["Realism"]/25.0
    return out.sort_values(["_rank","Realism"],ascending=False).drop(columns=["_rank"]).head(max_rows)
...
with tabs[0]:
    ...
    if st.session_state.get("_v657_cloud_ok",False):
        st.success("☁️ Cloud sync connected and verified.")
    else:
        st.warning("☁️ Cloud sync is not verified yet. Local backup is still active.")
        if st.session_state.get("_v657_cloud_error"):
            with st.expander("Cloud sync error details"):
...
with tabs[3]:
    ...
    if _my_roster.empty:
        ...
        _stale=[]
        if int(_v683_health.get("injury_coverage",0) or 0)<70: _stale.append("injury")
        if int(_v683_health.get("matchup_coverage",0) or 0)<50: _stale.append("matchup")
        if int(_v683_health.get("role_coverage",0) or 0)<30: _stale.append("usage/role")
        if _stale:
            st.warning("Limited live-data coverage: "+", ".join(_stale)+". Players with weak evidence cannot receive a Priority Claim.")
        ...
        _fa_eval["_candidate_value"]=_fa_eval.apply(_v676_player_value,axis=1)
        _fa_eval=_fa_eval.sort_values(["_candidate_value","vorp"],ascending=False).head(35)
        ...
        if not _allow_protected:
            _eligible_drops=_drops_all[_drops_all["_drop_protection"].eq("")].copy()
            if _eligible_drops.empty:
                _eligible_drops=_drops_all.copy()
        else:
            _eligible_drops=_drops_all.copy()

        # V6.78 performance pruning:
        # evaluate the weakest bench/depth assets plus same-position alternatives.
        _weak_overall=_eligible_drops.sort_values(["_drop_value","_ros_value"]).head(7)

        for _,add in _fa_eval.iterrows():
            _need_adj,_need_label=_v703_need_adjustment(add["position"],_my_roster,state)
            _evidence=_v703_evidence_quality(add)
            _same_pos=_eligible_drops[_eligible_drops["position"].eq(add["position"])].sort_values(["_drop_value","_ros_value"]).head(4)
            ...
            for _,drop in _drop_pool.iterrows():
                ...
                add_ros=_v682_ros_value(add)
                drop_ros=_v682_ros_value(drop)
                net_ros=add_ros-drop_ros
                action=_v682_move_action(net_lineup,net_ros,add)
                _moves.append({
                    ...
                    "Add PPG":round(float(pd.to_numeric(pd.Series([add.get("projection",0)]),errors="coerce").fillna(0).iloc[0]),2),
                    ...
                    "Action":action,
                    "Need Adj":_need_adj,"Need Fit":_need_label,"Evidence":_evidence,
        ...
        if _moves_df.empty:
            st.info("No legal add/drop combinations were found.")
        else:
            _ps=[]; _pl=[]
            ...
            _call_rank={"🔥 PRIORITY CLAIM":5,"⬆️ ADD NOW":4,"🧳 STASH":3,"🎯 STREAMER":2,"👀 WATCH":1,"⛔ PASS":0}
            ...
            st.dataframe(_best[["Add","Pos","Priority Score","Waiver Call","FAAB","Claim Advice","Move Type","Need Fit","Evidence","Why Not Priority","Net Week","Net ROS","Add Role","Add Recent PPG","Add Status","Add Practice","Add Injury","Add Matchup","Drop","Drop Pos","Drop Role","Drop Recent PPG","Drop Protection","Drop Status","Drop Practice","Add PPG","Drop PPG","Add ROS","Drop ROS","Add VORP","Drop VORP","Why"]].head(40),
                         use_container_width=True,hide_index=True)

            st.markdown("### 🔒 Drop protection")
            _protect_show=_drops_all[_drops_all["_drop_protection"].ne("")][["player","position","team","injury","_drop_protection","_ros_value"]].copy()
            if len(_protect_show):
                ...
                st.dataframe(
                    _protect_show.rename(columns={
                        ...
                        "_drop_protection":"Protection","_ros_value":"ROS Value"
                    }),
                    use_container_width=True,hide_index=True
                )
            else:
            ...
            with st.expander("How to read the move scores"):
                st.write("**Net Week** is the change to your optimized starting lineup this week. **Net ROS** compares long-term Fantasy Edge value of the add versus the drop. Protected players are excluded by default so an IR/PUP stash or core asset is not recommended as a short-term drop.")
...
with tabs[6]:
    st.subheader("🏁 Weekly Start / Sit + Lineup Optimizer")
    st.caption("Weekly Decision Engine: projection + matchup + VORP context + injury/practice risk + the strength of your actual bench alternatives.")
    ...
    st.caption(f"Live NFL context: {_v677_season} Week {_v677_week}. Matchup grades compare fantasy points allowed by position; current-season data takes over after a usable sample is available.")
    ...
    if _my.empty:
        ...
        a,b,c=st.columns(3)
        a.metric("Optimized starters",len(_starters))
        a.metric("Projected lineup",f"{_lineup_score:.1f} PPG")
        _major_flags=int(_my.apply(lambda r:_v676_injury_multiplier(r.get("injury",""),r.get("practice_status",""))<0.5,axis=1).sum())
        _watch_flags=int(_my.apply(lambda r:_v682_injury_risk(r)!="🟢 LOW",axis=1).sum())
        b.metric("Roster players",len(_my))
        b.metric("Injury / practice watch",_watch_flags,help=f"{_major_flags} are major availability flags")
        _bench_value=float(_bench["_start_value"].head(6).sum()) if len(_bench) else 0.0
        c.metric("Bench depth",f"{_bench_value:.1f}")
        c.metric("Bench spots",len(_bench))

        st.markdown("### ✅ Recommended starters")
        if len(_starters):
            _show=_starters.copy()
            _edge_map={}
            _confidence=[]
            _decision=[]
            _floor=[]
            _ceiling=[]
            _why=[]
            for idx,sr in _show.iterrows():
                elig=_v682_eligible_bench(sr,_bench)
                best_alt=float(elig["_start_value"].max()) if elig is not None and len(elig) else 0.0
                edge=float(sr["Adjusted PPG"])-best_alt
                _edge_map[idx]=edge

                # Locate full source row to preserve injury/practice/matchup fields.
                srcrow=_my[_my["player"].eq(sr["Player"])]
                srcrow=srcrow.iloc[0] if len(srcrow) else pd.Series(dtype=object)
                conf=_v682_start_confidence(srcrow,edge)
                fl,ce=_v682_weekly_band(srcrow)
                decision=_v682_decision(conf,edge,srcrow.get("injury",""))

                reasons=[]
                if edge>=1.5: reasons.append(f"+{edge:.1f} over best alternative")
                elif edge>=0: reasons.append(f"+{edge:.1f} lineup edge")
                else: reasons.append(f"{edge:.1f} vs alternative")
                mg=str(srcrow.get("_matchup_grade","") or "")
                if mg and mg!="—": reasons.append(f"{mg.lower()} matchup")
                risk=_v682_injury_risk(srcrow)
                if risk!="🟢 LOW": reasons.append(risk.replace("🔴 ","").replace("🟠 ","").replace("🟡 ",""))

                _confidence.append(conf)
                _decision.append(decision)
                _floor.append(round(fl,2))
                _ceiling.append(round(ce,2))
                _why.append(" • ".join(reasons))

            _show["Decision"]=_decision
            _show["Confidence"]=[f"{x}%" for x in _confidence]
            _show["Edge vs Bench"]=[round(_edge_map[i],2) for i in _show.index]
            _show["Floor"]=_floor
            _show["Ceiling"]=_ceiling
            _show["Why"]=_why
            _show["Projected PPG"]=_show["Projected PPG"].round(2)
            _show["Adjusted PPG"]=_show["Adjusted PPG"].round(2)
            _show["VORP"]=_show["VORP"].round(2)
            _show["Matchup Adj"]=_show["Matchup Adj"].round(2)
            _show["Role"]=[
                _v683_role_summary(_my[_my["player"].eq(p)].iloc[0]) if len(_my[_my["player"].eq(p)]) else "No recent role sample"
                for p in _show["Player"]
            ]
            _show["Recent PPG"]=[
                round(_v682_num(_my[_my["player"].eq(p)].iloc[0],"_recent_ppg3",0),2) if len(_my[_my["player"].eq(p)]) else 0.0
                for p in _show["Player"]
            ]
            st.dataframe(_show[["Slot","Player","Pos","Decision","Confidence","Edge vs Bench","Role","Recent PPG","Status","Practice","Injury","Opponent","Matchup","Projected PPG","Adjusted PPG","Floor","Ceiling","Why"]],
                         use_container_width=True,hide_index=True)

            _risk_starters=[]
            for _,sr in _show.iterrows():
                srcrow=_my[_my["player"].eq(sr["Player"])]
                if len(srcrow):
                    srcrow=srcrow.iloc[0]
                    risk=_v682_injury_risk(srcrow)
                    if risk!="🟢 LOW":
                        elig=_v682_eligible_bench(sr,_bench)
                        backup=elig.sort_values("_start_value",ascending=False).iloc[0]["player"] if elig is not None and len(elig) else "No direct roster replacement"
                        _risk_starters.append({
                            "Starter":sr["Player"],"Risk":risk,"Status":_v677_status_icon(srcrow.get("injury","")),
                            "Practice":_v680_practice_risk(srcrow.get("practice_status","")),
                            "Insurance":backup
                        })
            if _risk_starters:
                st.markdown("### 🚨 Injury insurance plan")
                st.dataframe(pd.DataFrame(_risk_starters),use_container_width=True,hide_index=True)

        st.markdown("### 🪑 Bench / Sit")
        if len(_bench):
            _b=_bench.copy()
            _b["Projected PPG"]=pd.to_numeric(_b["projection"],errors="coerce").round(2)
            _b["Adjusted PPG"]=_b["_start_value"].round(2)
            _b["VORP"]=pd.to_numeric(_b["vorp"],errors="coerce").round(2)
            ...
            _b["Injury"]=_b.get("injury_detail",pd.Series("",index=_b.index)).replace("","—")
            _b["Opponent"]=_b.get("_opponent",pd.Series("—",index=_b.index))
            _b["Matchup"]=_b.get("_matchup_grade",pd.Series("—",index=_b.index))
            _b["Matchup Adj"]=pd.to_numeric(_b.get("_matchup_adjustment",0),errors="coerce").fillna(0).round(2)
            _b["Reason"]=_b.apply(
                lambda r:"Unavailable — do not start"
                if _v676_injury_multiplier(r.get("injury",""),r.get("practice_status",""))==0
                else "Injury/status discount"
                if _v676_injury_multiplier(r.get("injury",""),r.get("practice_status",""))<0.95
                else "Lower optimized lineup value",
                axis=1
            )
            _bands=_b.apply(lambda r:_v682_weekly_band(r),axis=1)
            _b["Floor"]=[round(x[0],2) for x in _bands]
            _b["Ceiling"]=[round(x[1],2) for x in _bands]
            _b["ROS Value"]=_b.apply(_v682_ros_value,axis=1).round(2)
            _b["Risk"]=_b.apply(_v682_injury_risk,axis=1)
            _b["Drop Protection"]=_b.apply(_v682_drop_protection,axis=1)
            _b["Role"]=_b.apply(_v683_role_summary,axis=1)
            _b["Recent PPG"]=pd.to_numeric(_b.get("_recent_ppg3",0),errors="coerce").fillna(0).round(2)
            st.dataframe(_b[["player","position","team","Role","Recent PPG","Status","Practice","Injury","Risk","Opponent","Matchup","Projected PPG","Adjusted PPG","Floor","Ceiling","ROS Value","Drop Protection","Reason"]]
                         .rename(columns={"player":"Player","position":"Pos","team":"Team"}),
                         use_container_width=True,hide_index=True)

        st.markdown("### 🔄 Closest start/sit decisions")
        if len(_bench) and len(_starters):
            _edges=[]
            for _,sr in _starters.iterrows():
                ...
                if elig is None or elig.empty:
                    continue
                br=elig.sort_values("_start_value",ascending=False).iloc[0]
                gap=float(sr["Adjusted PPG"])-float(br["_start_value"])
                srcrow=_my[_my["player"].eq(sr["Player"])]
                srcrow=srcrow.iloc[0] if len(srcrow) else pd.Series(dtype=object)
                conf=_v682_start_confidence(srcrow,gap)
                _edges.append({
                    "Slot":sr["Slot"],"Start":sr["Player"],"Best Alternative":br["player"],
                    "Edge":round(gap,2),"Confidence":f"{conf}%",
                    "Call":_v682_decision(conf,gap,srcrow.get("injury","")),
                    "Starter Matchup":sr.get("Matchup","—"),
                    "Alt Matchup":br.get("_matchup_grade","—")
                })
            if _edges:
                st.dataframe(pd.DataFrame(_edges).sort_values("Edge").head(12),use_container_width=True,hide_index=True)

        st.caption("V6.83 adds current-season role/opportunity trend and recent production to the Weekly Decision Engine while leaving core projections, VORP and Progression/Regression unchanged.")
...
with tabs[13]:
    st.subheader("🔁 V6.94 Automatic Trade Partner Finder")
    st.caption(
        "Finds mutually plausible 1-for-1 trade ideas from your actual league rosters, then keeps the manual analyzer below."
    )
    ...
    if my.empty or other.empty:
        ...
        st.markdown("### Best automatic matches")
        if _ideas.empty:
            st.info("No mutually plausible 1-for-1 trades passed the current realism filters.")
        else:
            st.dataframe(_ideas,use_container_width=True,hide_index=True)
            st.caption(
                "Realism rewards balanced asset value and deals that do not clearly hurt the other manager's optimized roster."
            )
        ...
        if give and get:
            ev=_v690_trade_eval(
                my[my["player"].isin(give)].copy(),
                other[other["player"].isin(get)].copy(),
                my
            )
            if ev:
                a,b,c=st.columns(3)
                a.metric("Roster gain",f"{ev['Roster Gain']:+.2f}")
                b.metric("Asset value",f"{ev['Net Asset Value']:+.2f}")
                c.metric("Verdict",ev["Verdict"])
...
with tabs[14]:
    ...
    if my.empty: st.info("Set your roster under League Setup to populate the Command Center.")
        ...
        st.markdown("### V6.98 Priority Action Queue")
        try:
        ...
        _queue=_v698_action_queue(board,state,state.get("recommendation_history",[]),_aq_cons,_aq_trades,5)
        if _queue.empty: st.success("No urgent roster actions detected.")
        else:
            st.dataframe(_queue,use_container_width=True,hide_index=True)
            st.caption("Priority 1 is the most urgent modeled action for the current week.")
