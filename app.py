...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Command Center UI + Actionability", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
def _v756_stale_flags(board_df,now=None):
    """Timestamp watchdog for critical player records. Unknown timestamps are reported, not guessed stale."""
    now=now or datetime.now(timezone.utc)
    rows=[]
    for _,r in board_df.iterrows():
        status=str(r.get("fe_status",r.get("injury_effective",r.get("injury","ACTIVE"))) or "ACTIVE")
        updated=_v756_parse_dt(r.get("injury_updated",None))
        age=None if updated is None else (now-updated).total_seconds()/3600.0
        critical=status in ("QUESTIONABLE","DOUBTFUL","OUT","IR","PUP","NFI")
        if critical and updated is None:
            rows.append({"Player":r.get("player",""),"Source":"injury","Age h":None,"Status":"TIMESTAMP UNKNOWN"})
        elif critical and age is not None and age>72:
            rows.append({"Player":r.get("player",""),"Source":"injury","Age h":round(age,1),"Status":"STALE"})
    return pd.DataFrame(rows)
...
def _v756_certify(board_df,state_snapshot):
    ...
    add("Source freshness","PASS" if stale.empty else "WARN",f"{len(stale)} critical injury record(s) stale/unknown timestamp" if len(stale) else "Critical injury timestamps current")
...
with tabs[14]:
    ...
    if my.empty:
        ...
        for _,r in my.iterrows():
            ...
            if _status not in _watch_status: continue
