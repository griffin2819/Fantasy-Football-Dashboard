...
with tabs[1]:
    ...
    with st.expander("🚑 Live Injury Center", expanded=False):
        ...
        if len(flagged):
            flagged["Penalty"]=flagged["injury_penalty"].map(lambda v:"DO NOT DRAFT" if v>=900 else f"-{v:.0f}")
            flagged=flagged.sort_values(["injury_penalty","player"],ascending=[False,True])
            st.dataframe(
                flagged[["player","position","team","injury_effective","injury_source","Penalty"]]
