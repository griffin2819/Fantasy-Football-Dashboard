...
st.title("🏈 Fantasy Edge V8.06.4 — Exact True-Board Opportunity Audit")
st.caption("V8.06.4 • V8.05 frozen champion • exact true-board discovery")
st.caption("Build ID: V8.06.4-EXACT-TRUE-BOARD-AUDIT-FIX-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V8.06.4 — Exact True-Board Whole-Draft Opportunity Audit")
        st.caption(
            "Measurement only. V8.05 remains frozen champion. This version reads the simulator's exact alive+eligible board at each user pick, "
            "so players already selected by opponents cannot reappear as false alternatives."
        )

        if "v8063_results" not in st.session_state:
            st.session_state.v8063_results=None

        sims=st.number_input("V8.06.4 simulations per slot × room",5,30,20,5,key="v8063_sims",
            help="Default: 1,200 fresh V8.05 champion drafts.")
        a,b=st.columns(2)
        run=a.button("Run V8.06.4 exact-board audit",type="primary",key="v8063_run")
        clear=b.button("Clear V8.06.4 audit",key="v8063_clear")
        if clear:
            st.session_state.v8063_results=None
            st.rerun()
        ...
        if run:
            ...
            rows=[]; total=12*5*int(sims); done=0
            prog=st.progress(0); status=st.empty()

            for ds in range(1,int(teams)+1):
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8063000+ds*100000+j*433+sum(ord(c) for c in room_name)
                        d=simulate_mock_fast(
                            ...
                            r3_dominance_override_enabled=False,r11_13_offense_dominance_override_enabled=False,
                            late_sequence_variant="champion",r2_generic_adaptation_enabled=True,
                            r4_generic_adaptation_enabled=False,r3_causal_probe_enabled=False,
                            r3_qb_opportunity_cost_enabled=True)
                        ...
                        for _,r in d.iterrows():
                            ...
                            row={
                                ...
                                "Round":rnd,"Pick":r.mock_pick,
                                "Champion_player":str(r.player),"Champion_position":str(r.position),
                                "Champion_VORP":cv,"Champion_draft_score":cs,
                                "Champion_market_pick":r.get("market_pick",np.nan),
                                "Actual_available_count":r.get("R8063_Actual_available_count",np.nan),
                                "Best_alt_player":r.get("R8063_Alt1_player",""),
                                "Best_alt_position":r.get("R8063_Alt1_position",""),
                                "Best_alt_VORP":av,"Best_alt_draft_score":asc,
                                "Best_alt_market_pick":r.get("R8063_Alt1_market_pick",np.nan),
                            ...
                            for k in range(1,6):
                                for fld in ["player","position","market_pick","market_distance","model_rank","projection","vorp","draft_score"]:
                                    row[f"Alt{k}_{fld}"]=r.get(f"R8063_Alt{k}_{fld}","" if fld in ["player","position"] else np.nan)
                            rows.append(row)

                        done+=1
                        prog.progress(done/total)
                        if done%200==0 or done==total:
                            status.caption(f"V8.06.4: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v8063_results=df
            status.success(f"V8.06.4 complete: {len(df):,} exact-board decisions audited.")
            prog.empty()

        df=st.session_state.get("v8063_results")
        if isinstance(df,pd.DataFrame) and len(df):
            by_round=df.groupby("Round",as_index=False).agg(
                Decisions=("Champion_player","count"),
                Avg_available=("Actual_available_count","mean"),
            ...
            room=df.groupby(["Round","Room"],as_index=False).agg(Le10_rate=("Alt_dominates_le10","mean"))
            slot=df.groupby(["Round","Draft_slot"],as_index=False).agg(Le10_rate=("Alt_dominates_le10","mean"))
            rb=room.groupby("Round",as_index=False).agg(Rooms_with_signal=("Le10_rate",lambda x:int((x>0).sum())))
            sb=slot.groupby("Round",as_index=False).agg(Slots_with_signal=("Le10_rate",lambda x:int((x>0).sum())))
            by_round=by_round.merge(rb,on="Round").merge(sb,on="Round")
            by_round["Opportunity_score"]=(
                by_round.Le10_rate*by_round.Avg_VORP_gap.clip(lower=0)*
                (by_round.Avg_score_gap.clip(lower=0)/10.0)*
                (by_round.Rooms_with_signal/5.0)*(by_round.Slots_with_signal/12.0))
            ranked=by_round.sort_values("Opportunity_score",ascending=False)

            st.markdown("#### Corrected exact-board opportunity map")
            st.dataframe(ranked.round(4),use_container_width=True,hide_index=True)

            cand=ranked[~ranked.Protected]
            if len(cand):
                t=cand.iloc[0]
                st.warning(
                    f"Highest unprotected exact-board target: Round {int(t.Round)} "
                    f"(opportunity {float(t.Opportunity_score):.4f}, ≤10 signal {float(t.Le10_rate):.1%}, "
                    f"rooms {int(t.Rooms_with_signal)}/5, slots {int(t.Slots_with_signal)}/12). "
                    "Measure that mechanism before creating a challenger."
                )

            paths=df[(~df.Protected_round)&df.Alt_dominates_le10].groupby(
            ...
            st.markdown("#### Repeated unprotected exact-board mechanisms")
            st.dataframe(paths.head(60).round(4),use_container_width=True,hide_index=True)

            st.info("This audit uses the simulator's actual alive+eligible board. Certified R2, R3, R7 and R11–15 remain protected.")
            st.download_button(
                "⬇️ Download V8.06.3 exact-board audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_06_4_exact_true_board_opportunity_audit.csv",
                "text/csv",key="v8063_download")
