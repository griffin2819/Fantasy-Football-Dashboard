...
def _v694_trade_partner_suggestions(board_df,state,max_rows=20):
    """V7.33 Trade Recommendation Classification.
...
with tabs[13]:
    st.subheader("🔁 V7.33 Trade Recommendation Classification")
    st.caption(
        "Only recommends trades that improve your optimized roster. V7.33 keeps the fast shortlist and adds recommendation tiers so major upgrades, aggressive consolidations, and marginal swaps are no longer presented as equivalent."
    )
    ...
    if my.empty or other.empty:
        ...
        if _ideas.empty:
            ...
            _show_cols=[c for c in ["Recommendation","Trade Fit","Grade","Partner","Type","You Give","You Get","Lineup Gain","Your Gain","ROS Change","Depth Change","Partner Gain"] if c in _ideas.columns]
            ...
            with st.expander("Why these trades rank here", expanded=False):
                st.dataframe(
                    _ideas[[c for c in ["Recommendation","Partner","You Give","You Get","Your Need","Their Need","Asset Gap","ROS Change","Depth Change","Realism","Trade-off","Why"] if c in _ideas.columns]].head(10),
                    use_container_width=True,hide_index=True
                )
            st.caption(
                "A trade cannot appear here with negative Your Gain. V7.33 now separates STRONG SEND, AGGRESSIVE UPGRADE, CONSIDER, and MARGINAL ideas so a +0.9 PPG swap is not presented like a +6 PPG consolidation trade."
            )
