...
st.title("🏈 Fantasy Edge V7.9 — Evaluation + Execution")
st.caption("V7.9 • separate player evaluation + pick execution • persistent Target Queue • independent validator retained")
st.caption("Build ID: V7.9-EEX-20260813")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v79_live_slot")
    ...
    if "v79_target_queue" not in st.session_state:
        st.session_state.v79_target_queue={}
    queue=st.session_state.v79_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v79_target_queue=queue
    ...
    st.dataframe(show[["player","position","team","Model target #","Action","Market pick","Survives next","projection","vorp","Profile","Confidence","injury"]].head(120),use_container_width=True,hide_index=True)
    ...
    if mode=="Interactive mock":
        ...
        if overall<=int(teams)*int(rounds):
            ...
            if owner==int(slot):
                ...
                mt=model_top.iloc[0]; dn=now_top.iloc[0]
                ...
                if nxt:
                    st.caption(f"Current pick #{overall} • next pick #{nxt}. Model target and pick execution are intentionally separate.")
                top=now_top.copy()
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.9 robustness report")
            ...
            st.download_button("⬇️ Download V7.9 diagnostic CSV",csv,"fantasy_edge_v7_9_diagnostics.csv","text/csv")


        st.markdown("### 🧪 V7.9 independent validation")
        ...
        if "v79_independent_results" not in st.session_state:
            st.session_state.v79_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v79_independent_results"),pd.DataFrame) else "ready to run"))
        if "v79_independent_summary" not in st.session_state:
            st.session_state.v79_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v79_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v79_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v79_run")
        clear_iv=clear_col.button("Clear independent results",key="v79_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v79_independent_results=iv
                st.session_state.v79_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v79_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v79_independent_summary") or {}
            st.success(
            ...
            st.download_button(
                "⬇️ Download V7.9 independent validation CSV",
                iv_csv,
                "fantasy_edge_v7_9_independent_validation.csv",
                "text/csv",
                key="v79_download"
            )
