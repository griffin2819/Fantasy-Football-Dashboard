    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                _te_util_mult=(float(p12_te_starter_vorp_mult) if bool(p12_te_utility_dedup_enabled) and int(slot)==12 and 4<=round_no<=6 else (.30 if int(slot)==12 and 4<=round_no<=6 else .90))
...
st.title("🏈 Fantasy Edge V7.46 — Promoted TE .30 + Champion Certification")
st.caption("V7.46 • Pick-12 R4-6 TE utility .30 promoted • champion certification")
st.caption("Build ID: V7.46-P12TE30-CERT-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v746_live_slot")
    ...
    if "v746_target_queue" not in st.session_state:
        st.session_state.v746_target_queue={}
    queue=st.session_state.v746_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v746_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.46 uses .30 as the live/default Pick-12 R4-6 TE-starter utility multiplier; certification retains an explicit .90 control.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.46 robustness report")
        ...
        st.markdown("### 🧪 V7.46 champion baseline validation")
        st.info("V7.46 adds no new tuning beyond the validated .30 promotion and fixes the paired gate by conditioning win rates on actual behavior changes.")
        ...
        if "v746_independent_results" not in st.session_state:
            st.session_state.v746_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v746_independent_results"),pd.DataFrame) else "ready to run"))
        if "v746_independent_summary" not in st.session_state:
            st.session_state.v746_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v746_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v746_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v746_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v746_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v746_independent_results=iv
                st.session_state.v746_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v746_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v746_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.46 Champion Baseline Check")
            st.caption("V7.46 promotes the validated .30 Pick-12 R4-6 TE-starter utility multiplier and runs final fresh-seed certification against the prior .90 setting.")
            ...
            st.download_button(
                "⬇️ Download V7.46 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_46_champion_baseline.csv",
                "text/csv",
                key="v746_baseline_download"
            )
        ...
        st.markdown("### 🏆 V7.46 — Promoted TE .30 + Champion Certification")
        st.caption("V7.46 promotes .30 only for Pick-12 R4-6 TE-starter VORP inside roster utility. Fresh-seed certification compares it directly with the old .90 champion and calculates paired win rates only when behavior actually changes.")
        if "v746_results" not in st.session_state: st.session_state.v746_results=None
        sims=st.number_input("Certification simulations per room",min_value=30,max_value=100,value=75,step=5,key="v746_sims")
        a,b=st.columns(2); run=a.button("Run V7.46 champion certification",type="primary",key="v746_run"); clear=b.button("Clear certification",key="v746_clear")
        if clear: st.session_state.v746_results=None; st.rerun()
        if run:
            rooms=["Consensus","ADP-heavy","Chaotic","Positional runs","Sharp/value"]; fast_champ=_fast_benchmark_pool(board,int(teams)); rows=[]
            total=len(rooms)*int(sims); done=0; prog=st.progress(0); status=st.empty()
            for room_name in rooms:
                for j in range(int(sims)):
                    seed=946000+j*97+sum(ord(c) for c in room_name)
                    oldc=simulate_mock_fast(fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name,p12_te_utility_dedup_enabled=True,p12_te_starter_vorp_mult=.90)
                    newc=simulate_mock_fast(fast_champ,int(teams),12,15,state["roster_slots"],6.0,True,seed,room_name)
                    oc=independent_draft_grade_components(oldc,int(teams)); nc=independent_draft_grade_components(newc,int(teams))
                    op=independent_phase_metrics(oldc).set_index("Phase"); np_=independent_phase_metrics(newc).set_index("Phase")
                    old_players=oldc.set_index("mock_pick")["player"].to_dict()
                    changed=sum(old_players.get(rr.mock_pick,None)!=rr.player for _,rr in newc.iterrows())
                    overall=nc["overall"]-oc["overall"]; r46=float(np_.loc["R4-6","Avg_market_delta"])-float(op.loc["R4-6","Avg_market_delta"])
                    rows.append({"Room":room_name,"Simulation":j+1,"Seed":seed,"V746_vs_old":overall,"Behavior_changed":changed>0,"Changed_picks":changed,
                        "Conditional_overall_win":(overall>0) if changed else np.nan,"Conditional_overall_nonloss":(overall>=0) if changed else np.nan,
                        "R1_3_change":float(np_.loc["R1-3","Avg_market_delta"])-float(op.loc["R1-3","Avg_market_delta"]),"R4_6_change":r46,
                    ...
                    if done==1 or done%75==0 or done==total: status.caption(f"Certification: {done}/{total} matched sets")
            df=pd.DataFrame(rows); st.session_state.v746_results=df; status.success(f"Certification complete: {len(df)} fresh matched sets."); prog.empty()

        df=st.session_state.get("v746_results")
        if isinstance(df,pd.DataFrame) and len(df):
            changed=df[df.Behavior_changed]; overall=df.V746_vs_old.mean(); r46=df.R4_6_change.mean(); r710=df.R7_10_change.mean(); r11=df.R11_plus_change.mean()
            behavior=df.Behavior_changed.mean(); cond=changed.Conditional_overall_win.mean() if len(changed) else np.nan; cond46=changed.Conditional_R4_6_win.mean() if len(changed) else np.nan
            c1,c2,c3,c4=st.columns(4); c1.metric("Overall vs .90",f"{overall:+.3f}"); c2.metric("R4-6 vs .90",f"{r46:+.3f}"); c3.metric("Behavior changed",f"{behavior:.1%}"); c4.metric("Conditional win",f"{cond:.1%}" if pd.notna(cond) else "n/a")
            room=df.groupby("Room",as_index=False).agg(Overall_change=("V746_vs_old","mean"),R4_6_change=("R4_6_change","mean"),R7_10_change=("R7_10_change","mean"),R11_plus_change=("R11_plus_change","mean"),Behavior_change=("Behavior_changed","mean"))
            st.dataframe(room.round(4),use_container_width=True,hide_index=True)
            positive=int((room.Overall_change>0).sum()); positive46=int((room.R4_6_change>0).sum()); worst=float(room.Overall_change.min())
            certified=(overall>0 and r46>0 and r710>-.20 and r11>-.25 and behavior>=.10 and pd.notna(cond) and cond>=.55 and pd.notna(cond46) and cond46>=.55 and positive>=4 and positive46>=4 and worst>-.15)
            gate=pd.DataFrame([{"Certified":certified,"Overall change":overall,"R4-6 change":r46,"R7-10 change":r710,"R11+ change":r11,"Behavior change":behavior,"Conditional overall wins":cond,"Conditional R4-6 wins":cond46,"Positive overall rooms":positive,"Positive R4-6 rooms":positive46,"Worst room":worst}])
            st.markdown("#### Champion certification gate"); st.dataframe(gate.round(4),use_container_width=True,hide_index=True)
            if certified: st.success("V7.46 CERTIFIED — .30 replaces the prior .90 Pick-12 R4-6 TE utility setting as the frozen champion.")
            else: st.warning("V7.46 did not clear final certification. Revert to the prior .90 champion rather than retuning around this sample.")
            st.download_button("⬇️ Download V7.46 certification CSV",df.to_csv(index=False).encode("utf-8"),"fantasy_edge_v7_46_champion_certification.csv","text/csv",key="v746_download")
