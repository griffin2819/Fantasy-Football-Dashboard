        ...
        if owner==int(slot):
            ...
            if model_user:
                ...
                if round_no==2:
                    ...
                    _pre_counts={p:roster_pos.count(p) for p in set(roster_pos)}
                    mr_diag.update({
                        "R790_Audit":1,
...
st.title("🏈 Fantasy Edge V7.90 — Round-2 Generic Adaptation Signature Audit")
st.caption("V7.90 • V7.67 frozen champion • Round-2 adaptation diagnostic")
st.caption("Build ID: V7.90-R2-GENERIC-ADAPTATION-AUDIT-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V7.90 — Round-2 Generic Adaptation Signature Audit")
        st.caption(
            "Measurement only. V7.67 remains frozen champion. "
            "V7.90 studies Round 2 across Consensus, ADP-heavy, and Sharp/value rooms to identify a room-agnostic board-state signature "
            "for when the champion's fallback choice is simultaneously behind an available alternative on both frozen draft score and VORP."
        )

        if "v790_results" not in st.session_state:
            st.session_state.v790_results=None

        sims=st.number_input(
            "V7.90 simulations per slot × room",
            min_value=10,max_value=50,value=20,step=5,key="v790_sims",
            help="Default: all 12 draft slots × 3 focus rooms × 20 = 720 fresh champion drafts. One simulation per seed."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-2 adaptation audit",type="primary",key="v790_run")
        clear=b.button("Clear V7.90 audit",key="v790_clear")
        if clear:
            st.session_state.v790_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=990000+int(ds)*100000+j*337+sum(ord(c) for c in room_name)
                        ...
                        if len(r2):
                            r=r2.iloc[0]
                            audit=int(pd.to_numeric(pd.Series([r.get("R790_Audit",0)]),errors="coerce").fillna(0).iloc[0])
                            if audit==1:
                                ...
                                champ_pick=pd.to_numeric(pd.Series([r.get("mock_pick",np.nan)]),errors="coerce").iloc[0]
                                champ_reach=(champ_market-champ_pick) if pd.notna(champ_market) and pd.notna(champ_pick) else np.nan

                                row={
                                    "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                    "Round2_pick":champ_pick,
                                    "Champion_player":str(r.get("player","")),
                                    "Champion_position":str(r.get("position","")),
                                    "Champion_market_pick":champ_market,
                                    "Champion_reach":champ_reach,
                                    "Champion_VORP":champ_vorp,
                                    "Champion_draft_score":champ_score,
                                    "Actual_available_count":r.get("R790_Actual_available_count",np.nan),
                                    "QB_count_pre":r.get("R790_QB_count_pre",0),
                                    "RB_count_pre":r.get("R790_RB_count_pre",0),
                                    "WR_count_pre":r.get("R790_WR_count_pre",0),
                                    "TE_count_pre":r.get("R790_TE_count_pre",0),
                                    "DL_count_pre":r.get("R790_DL_count_pre",0),
                                    "DB_count_pre":r.get("R790_DB_count_pre",0),
                                }
                        ...
                        if done==1 or done%120==0 or done==total:
                            status.caption(f"V7.90 audit: {done}/{total} champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v790_results=df
            status.success(f"V7.90 complete: {len(df)} Round-2 board states audited.")
            prog.empty()

        df=st.session_state.get("v790_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in ["Champion_VORP","Champion_draft_score","Champion_reach",
                      "Alt1_vorp","Alt1_draft_score","Alt1_market_distance"]:
                df[c]=pd.to_numeric(df[c],errors="coerce")

            df["Alt1_VORP_change"]=df.Alt1_vorp-df.Champion_VORP
            df["Alt1_score_change"]=df.Alt1_draft_score-df.Champion_draft_score
            df["Alt1_dominates"]=(df.Alt1_VORP_change>0)&(df.Alt1_score_change>0)

            # Market-band signatures: can a generic rule identify the event without knowing room label?
            for band in [0,5,10,15,20]:
                name=f"Sig_market_le_{band}"
                df[name]=df.Alt1_dominates & (df.Alt1_market_distance<=band)

            room_summary=df.groupby("Room",as_index=False).agg(
                Decisions=("Champion_player","count"),
                Avg_champion_reach=("Champion_reach","mean"),
                Avg_alt1_VORP_change=("Alt1_VORP_change","mean"),
                Avg_alt1_score_change=("Alt1_score_change","mean"),
                Dominance_rate=("Alt1_dominates","mean"),
                Sig_0=("Sig_market_le_0","mean"),
                Sig_5=("Sig_market_le_5","mean"),
                Sig_10=("Sig_market_le_10","mean"),
                Sig_15=("Sig_market_le_15","mean"),
                Sig_20=("Sig_market_le_20","mean"),
            )
            st.markdown("#### Generic Round-2 signature by room")
            st.dataframe(room_summary.round(4),use_container_width=True,hide_index=True)

            slot=df.groupby(["Draft_slot","Room"],as_index=False).agg(
                Decisions=("Champion_player","count"),
                Avg_alt1_VORP_change=("Alt1_VORP_change","mean"),
                Avg_alt1_score_change=("Alt1_score_change","mean"),
                Dominance_rate=("Alt1_dominates","mean"),
            ...
            st.markdown("#### Draft slot × room signature")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            # Roster-state split after R1.
            roster=df.groupby(
                ["Room","Champion_position","QB_count_pre","RB_count_pre","WR_count_pre","TE_count_pre"],
                as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Alt1_dominates","mean"),
                Avg_VORP_change=("Alt1_VORP_change","mean"),
                Avg_score_change=("Alt1_score_change","mean"),
                Sig_10=("Sig_market_le_10","mean"),
            ).sort_values("Decisions",ascending=False)
            st.markdown("#### Post-R1 roster-state signatures")
            st.dataframe(roster.head(40).round(4),use_container_width=True,hide_index=True)

            paths=df.groupby(
                ["Room","Champion_player","Champion_position","Alt1_player","Alt1_position"],
                as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Avg_market_distance=("Alt1_market_distance","mean"),
                Avg_VORP_change=("Alt1_VORP_change","mean"),
                Avg_score_change=("Alt1_score_change","mean"),
                Dominance_rate=("Alt1_dominates","mean"),
            ).sort_values(["Decisions","Dominance_rate"],ascending=[False,False])
            st.markdown("#### Most repeated Round-2 fallback paths")
            st.dataframe(paths.head(50).round(4),use_container_width=True,hide_index=True)

            # Top-five depth: how many of the available alternatives dominate?
            for k in range(1,6):
                df[f"Alt{k}_vorp"]=pd.to_numeric(df[f"Alt{k}_vorp"],errors="coerce")
                df[f"Alt{k}_draft_score"]=pd.to_numeric(df[f"Alt{k}_draft_score"],errors="coerce")
                df[f"Alt{k}_dominates"]=(
                    (df[f"Alt{k}_vorp"]>df.Champion_VORP) &
                    (df[f"Alt{k}_draft_score"]>df.Champion_draft_score)
                )
            df["Dominating_alternatives_top5"]=sum(df[f"Alt{k}_dominates"].astype(int) for k in range(1,6))
            depth=df.groupby("Room",as_index=False).agg(
                Avg_dominating_alts_top5=("Dominating_alternatives_top5","mean"),
                Any_dominating_alt_top5=("Dominating_alternatives_top5",lambda x:(x>0).mean()),
                Two_plus_dominating_alts=("Dominating_alternatives_top5",lambda x:(x>=2).mean()),
            )
            st.markdown("#### Dominance depth across top five alternatives")
            st.dataframe(depth.round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.90 is measurement-only. The desired outcome is a room-agnostic signature: a condition based on true-board score/VORP, market distance, "
                "and post-R1 roster state that occurs disproportionately in Sharp/value rooms but remains sensible in every environment. "
                "Do not promote a room-name-specific rule."
            )

            st.download_button(
                "⬇️ Download V7.90 Round-2 adaptation signature audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_90_round2_generic_adaptation_signature_audit.csv",
                "text/csv",key="v790_download"
            )
