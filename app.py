...
st.title("🏈 Fantasy Edge V7.56 — Round 15 Market-Rank Integrity Audit")
st.caption("V7.56 • V7.50 champion frozen • Round-15 market-rank integrity diagnostic")
st.caption("Build ID: V7.56-R15MARKETINTEGRITY-20260816")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v756_live_slot")
    ...
    if "v756_target_queue" not in st.session_state:
        st.session_state.v756_target_queue={}
    queue=st.session_state.v756_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v756_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.56 keeps V7.50 as the live/default champion with the 55.97 rule unchanged.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.56 robustness report")
        ...
        st.markdown("### 🧪 V7.56 champion baseline validation")
        st.info("V7.56 is diagnostic only. No live weights, market handling, survival curves, valuation logic, or thresholds are changed.")
        ...
        if "v756_independent_results" not in st.session_state:
            st.session_state.v756_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v756_independent_results"),pd.DataFrame) else "ready to run"))
        if "v756_independent_summary" not in st.session_state:
            st.session_state.v756_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v756_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v756_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v756_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v756_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v756_independent_results=iv
                st.session_state.v756_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v756_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v756_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.56 Champion Baseline Check")
            st.caption("V7.56 keeps the V7.50 champion fully frozen and audits the exact Round-15 player pairs behind the V7.55 counterfactual to test market-rank integrity.")
            ...
            st.download_button(
                "⬇️ Download V7.56 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_56_champion_baseline.csv",
                "text/csv",
                key="v756_baseline_download"
            )
        ...
        st.markdown("### 🧪 V7.56 — Round 15 Market-Rank Integrity Audit")
        st.caption(
            "Diagnostic only. V7.50 remains frozen champion. V7.56 inspects only the Round-15 decisions that triggered the V7.55 counterfactual "
            "and compares the normal V7.50 player with the forced faller to determine whether the giant market-price swing is legitimate or caused by missing/fallback/sentinel rank handling."
        )

        if "v756_results" not in st.session_state:
            st.session_state.v756_results=None

        sims=st.number_input(
            "Round-15 integrity simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v756_sims",
            help="Default: slots 1/3/7/12 × 5 rooms × 10 simulations = 200 base drafts with Round-15 counterfactual reruns."
        )
        a,b=st.columns(2)
        run=a.button("Run Round-15 integrity audit",type="primary",key="v756_run")
        clear=b.button("Clear V7.56 audit",key="v756_clear")
        ...
        if run:
            ...
            slots_to_test=[1,3,7,min(12,int(teams))]
            slots_to_test=list(dict.fromkeys([x for x in slots_to_test if 1<=x<=int(teams)]))
            fast_champ=_fast_benchmark_pool(board,int(teams))
            pool=fast_champ["pool"]
            rows=[]; total=len(slots_to_test)*len(rooms)*int(sims); done=0
            prog=st.progress(0); status=st.empty()

            # Fast lookup by exact player name for static board fields.
            name_to_idx={}
            for ii,nm in enumerate(pool.player.astype(str).tolist()):
                if nm not in name_to_idx:
                    name_to_idx[nm]=ii

            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=956000+int(ds)*10000+j*139+sum(ord(c) for c in room_name)

                        base_draft=simulate_mock_fast(
                            fast_champ,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name,
                            late_empirical_survival_enabled=False,
                            late_force_faller_round=0
                        )
                        cf=simulate_mock_fast(
                            fast_champ,int(teams),int(ds),15,state["roster_slots"],
                            6.0,True,seed,room_name,
                            late_empirical_survival_enabled=False,
                            late_force_faller_round=15
                        )

                        b15=base_draft[pd.to_numeric(base_draft.mock_round,errors="coerce").eq(15)]
                        c15=cf[pd.to_numeric(cf.mock_round,errors="coerce").eq(15)]

                        if len(b15) and len(c15):
                            br=b15.iloc[0]
                            cr_=c15.iloc[0]
                            normal=str(br.player)
                            forced=str(cr_.player)
                            changed=normal!=forced

                            if changed:
                                ni=name_to_idx.get(normal,None)
                                fi=name_to_idx.get(forced,None)
                                ...
                                n_cons=_v(fast_champ["consensus"],ni)
                                f_cons=_v(fast_champ["consensus"],fi)
                                n_market=_v(fast_champ["market_pick"],ni)
                                f_market=_v(fast_champ["market_pick"],fi)
                                n_model=_v(fast_champ["model_rank"],ni)
                                f_model=_v(fast_champ["model_rank"],fi)
                                n_proj=_v(fast_champ["projection"],ni)
                                f_proj=_v(fast_champ["projection"],fi)
                                n_vorp=_v(fast_champ["vorp"],ni)
                                f_vorp=_v(fast_champ["vorp"],fi)
                                n_ds=_v(fast_champ["draft_score"],ni)
                                f_ds=_v(fast_champ["draft_score"],fi)
                                n_repl=_v(fast_champ["replacement_ppg"],ni)
                                f_repl=_v(fast_champ["replacement_ppg"],fi)

                                n_role=_fast_roster_role_snapshot(fast_champ,[
                                    name_to_idx.get(str(x),-1) for x in base_draft[
                                        pd.to_numeric(base_draft.mock_round,errors="coerce")<15
                                    ].player.astype(str).tolist()
                                    if name_to_idx.get(str(x),None) is not None
                                ],ni,state["roster_slots"]) if ni is not None else {}
                                f_role=_fast_roster_role_snapshot(fast_champ,[
                                    name_to_idx.get(str(x),-1) for x in base_draft[
                                        pd.to_numeric(base_draft.mock_round,errors="coerce")<15
                                    ].player.astype(str).tolist()
                                    if name_to_idx.get(str(x),None) is not None
                                ],fi,state["roster_slots"]) if fi is not None else {}

                                bc=independent_draft_grade_components(base_draft,int(teams))
                                cc=independent_draft_grade_components(cf,int(teams))
                                bp=independent_phase_metrics(base_draft).set_index("Phase")
                                cp=independent_phase_metrics(cf).set_index("Phase")

                                normal_cons_missing=not np.isfinite(n_cons)
                                forced_cons_missing=not np.isfinite(f_cons)
                                normal_market_missing=not np.isfinite(n_market)
                                forced_market_missing=not np.isfinite(f_market)
                                normal_market_extreme=np.isfinite(n_market) and n_market>250
                                forced_market_extreme=np.isfinite(f_market) and f_market>250
                                normal_model_extreme=np.isfinite(n_model) and n_model>250
                                forced_model_extreme=np.isfinite(f_model) and f_model>250

                                rows.append({
                                    "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                    "Round":15,
                                    "Normal_player":normal,
                                    "Normal_position":br.position,
                                    "Forced_faller":forced,
                                    "Forced_position":cr_.position,

                                    "Normal_consensus_rank":n_cons,
                                    "Forced_consensus_rank":f_cons,
                                    "Normal_market_pick":n_market,
                                    "Forced_market_pick":f_market,
                                    "Normal_model_rank":n_model,
                                    "Forced_model_rank":f_model,

                                    "Normal_projection":n_proj,
                                    "Forced_projection":f_proj,
                                    "Projection_gap_normal_minus_faller":n_proj-f_proj if pd.notna(n_proj) and pd.notna(f_proj) else np.nan,
                                    "Normal_vorp":n_vorp,
                                    "Forced_vorp":f_vorp,
                                    "VORP_gap_normal_minus_faller":n_vorp-f_vorp if pd.notna(n_vorp) and pd.notna(f_vorp) else np.nan,
                                    "Normal_draft_score":n_ds,
                                    "Forced_draft_score":f_ds,
                                    "Draftscore_gap_normal_minus_faller":n_ds-f_ds if pd.notna(n_ds) and pd.notna(f_ds) else np.nan,
                                    "Normal_replacement_ppg":n_repl,
                                    "Forced_replacement_ppg":f_repl,

                                    "Normal_roster_role":n_role.get("role",""),
                                    "Forced_roster_role":f_role.get("role",""),
                                    "Normal_roster_utility_delta":n_role.get("utility_delta",np.nan),
                                    "Forced_roster_utility_delta":f_role.get("utility_delta",np.nan),
                                    "Roster_utility_gap_normal_minus_faller":(
                                        n_role.get("utility_delta",np.nan)-f_role.get("utility_delta",np.nan)
                                    ) if pd.notna(n_role.get("utility_delta",np.nan)) and pd.notna(f_role.get("utility_delta",np.nan)) else np.nan,

                                    "Normal_consensus_missing":normal_cons_missing,
                                    "Forced_consensus_missing":forced_cons_missing,
                                    "Normal_market_missing":normal_market_missing,
                                    "Forced_market_missing":forced_market_missing,
                                    "Normal_market_extreme_gt250":normal_market_extreme,
                                    "Forced_market_extreme_gt250":forced_market_extreme,
                                    "Normal_model_rank_extreme_gt250":normal_model_extreme,
                                    "Forced_model_rank_extreme_gt250":forced_model_extreme,

                                    "Market_price_improvement":n_market-f_market if pd.notna(n_market) and pd.notna(f_market) else np.nan,
                                    "Overall_change":cc["overall"]-bc["overall"],
                                    "R11_plus_change":float(cp.loc["R11+","Avg_market_delta"])-float(bp.loc["R11+","Avg_market_delta"]),
                                    "Counterfactual_win":cc["overall"]>bc["overall"],
                                })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%40==0 or done==total:
                            status.caption(f"V7.56 integrity audit: {done}/{total} base drafts")

            df=pd.DataFrame(rows)
            st.session_state.v756_results=df
            status.success(f"V7.56 complete: {len(df)} triggered Round-15 decisions inspected.")
            prog.empty()

        df=st.session_state.get("v756_results")
        if isinstance(df,pd.DataFrame) and len(df):
            st.markdown("#### Round-15 integrity summary")
            summary=pd.DataFrame([{
                "Triggered decisions":len(df),
                "Counterfactual win rate":df.Counterfactual_win.mean(),
                "Overall change":pd.to_numeric(df.Overall_change,errors="coerce").mean(),
                "R11+ change":pd.to_numeric(df.R11_plus_change,errors="coerce").mean(),
                "Market-price improvement":pd.to_numeric(df.Market_price_improvement,errors="coerce").mean(),
                "Normal consensus missing":df.Normal_consensus_missing.mean(),
                "Normal market missing":df.Normal_market_missing.mean(),
                "Normal market >250":df.Normal_market_extreme_gt250.mean(),
                "Normal model rank >250":df.Normal_model_rank_extreme_gt250.mean(),
                "Forced market >250":df.Forced_market_extreme_gt250.mean(),
            }])
            st.dataframe(summary.round(4),use_container_width=True,hide_index=True)

            st.markdown("#### Normal V7.50 player vs forced faller")
            compare=pd.DataFrame([
                {"Metric":"Market pick","Normal":pd.to_numeric(df.Normal_market_pick,errors="coerce").mean(),"Faller":pd.to_numeric(df.Forced_market_pick,errors="coerce").mean()},
                {"Metric":"Model rank","Normal":pd.to_numeric(df.Normal_model_rank,errors="coerce").mean(),"Faller":pd.to_numeric(df.Forced_model_rank,errors="coerce").mean()},
                {"Metric":"Projection","Normal":pd.to_numeric(df.Normal_projection,errors="coerce").mean(),"Faller":pd.to_numeric(df.Forced_projection,errors="coerce").mean()},
                {"Metric":"VORP","Normal":pd.to_numeric(df.Normal_vorp,errors="coerce").mean(),"Faller":pd.to_numeric(df.Forced_vorp,errors="coerce").mean()},
                {"Metric":"Draft score","Normal":pd.to_numeric(df.Normal_draft_score,errors="coerce").mean(),"Faller":pd.to_numeric(df.Forced_draft_score,errors="coerce").mean()},
                {"Metric":"Roster utility delta","Normal":pd.to_numeric(df.Normal_roster_utility_delta,errors="coerce").mean(),"Faller":pd.to_numeric(df.Forced_roster_utility_delta,errors="coerce").mean()},
                {"Metric":"Replacement PPG","Normal":pd.to_numeric(df.Normal_replacement_ppg,errors="coerce").mean(),"Faller":pd.to_numeric(df.Forced_replacement_ppg,errors="coerce").mean()},
            ])
            compare["Normal−Faller"]=compare["Normal"]-compare["Faller"]
            st.dataframe(compare.round(3),use_container_width=True,hide_index=True)

            st.markdown("#### Position paths")
            paths=df.groupby(["Normal_position","Forced_position"],as_index=False).agg(
                Decisions=("Counterfactual_win","count"),
                Win_rate=("Counterfactual_win","mean"),
                Overall_change=("Overall_change","mean"),
                Market_improvement=("Market_price_improvement","mean"),
                Draftscore_gap=("Draftscore_gap_normal_minus_faller","mean"),
                Rosterutility_gap=("Roster_utility_gap_normal_minus_faller","mean")
            )
            st.dataframe(paths.sort_values("Decisions",ascending=False).round(3),use_container_width=True,hide_index=True)

            st.markdown("#### Most common normal selections")
            players=df.groupby(["Normal_player","Normal_position"],as_index=False).agg(
                Decisions=("Counterfactual_win","count"),
                Avg_market_pick=("Normal_market_pick","mean"),
                Avg_model_rank=("Normal_model_rank","mean"),
                Avg_projection=("Normal_projection","mean"),
                Avg_vorp=("Normal_vorp","mean"),
                Avg_draft_score=("Normal_draft_score","mean"),
                Avg_roster_utility=("Normal_roster_utility_delta","mean"),
                Counterfactual_win_rate=("Counterfactual_win","mean")
            )
            st.dataframe(players.sort_values("Decisions",ascending=False).head(25).round(3),use_container_width=True,hide_index=True)

            bad_rank=(
                df.Normal_consensus_missing
                | df.Normal_market_missing
                | df.Normal_market_extreme_gt250
                | df.Normal_model_rank_extreme_gt250
            )
            bad_rate=float(bad_rank.mean())
            score_gap=pd.to_numeric(df.Draftscore_gap_normal_minus_faller,errors="coerce").mean()
            util_gap=pd.to_numeric(df.Roster_utility_gap_normal_minus_faller,errors="coerce").mean()
            market_gain=pd.to_numeric(df.Market_price_improvement,errors="coerce").mean()

            if bad_rate>=.50:
                st.warning(
                    f"Structural data issue confirmed: {bad_rate:.0%} of triggered Round-15 normal selections have missing/extreme market or model rank handling. "
                    "V7.57 should fix Round-15 market-rank eligibility/fallback before testing any new execution rule."
                )
            elif market_gain>=100 and score_gap>0 and util_gap>0:
                st.warning(
                    f"No dominant missing-rank defect was found, but V7.50 is paying an average {market_gain:.0f}-pick market premium because both draft score "
                    f"({score_gap:+.2f}) and roster utility ({util_gap:+.2f}) favor the normal player. V7.57 should audit Round-15 roster utility / terminal-pick valuation specifically."
                )
            else:
                st.info(
                    "The Round-15 effect is not explained by a single rank-integrity defect. Use the player and position-path tables to isolate the terminal-pick valuation mechanism."
                )

            st.download_button(
                "⬇️ Download V7.56 Round-15 integrity audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_56_round15_market_rank_integrity_audit.csv",
                "text/csv",key="v756_download"
            )
        else:
            st.info("Run V7.56 to inspect the triggered Round-15 normal-player versus forced-faller pairs.")
