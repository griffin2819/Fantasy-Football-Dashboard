...
st.title("🏈 Fantasy Edge V7.50 — Frozen 55.97 Champion")
st.caption("V7.50 • frozen 55.97 Pick-12 TE rule promoted • new champion")
st.caption("Build ID: V7.50-CHAMPION-GAP5597-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v750_live_slot")
    ...
    if "v750_target_queue" not in st.session_state:
        st.session_state.v750_target_queue={}
    queue=st.session_state.v750_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v750_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.50 is the new live/default champion. The 55.97 threshold is frozen exactly; all non-triggered decisions continue to use .90.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.50 robustness report")
        ...
        st.markdown("### 🧪 V7.50 champion baseline validation")
        st.info("V7.50 introduces no new tuning beyond the previously frozen and validated 55.97 rule. No room-specific conditions or threshold changes were added.")
        ...
        if "v750_independent_results" not in st.session_state:
            st.session_state.v750_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v750_independent_results"),pd.DataFrame) else "ready to run"))
        if "v750_independent_summary" not in st.session_state:
            st.session_state.v750_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v750_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v750_base_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v750_base_run")
        clear_iv=clear_col.button("Clear independent results",key="v750_base_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v750_independent_results=iv
                st.session_state.v750_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v750_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v750_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.50 Champion Baseline Check")
            st.caption("V7.50 promotes the exact frozen 55.97 conditional Pick-12 R4-6 TE utility rule after successful untouched-seed validation and large-sample confirmation.")
            ...
            st.download_button(
                "⬇️ Download V7.50 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_50_champion_baseline.csv",
                "text/csv",
                key="v750_baseline_download"
            )
