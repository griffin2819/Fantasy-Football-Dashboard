...
def draft_eligibility(avail, roster, round_no, rounds, slots):
    ...
    # V7.2: flexible bench construction; only cap extreme RB hoarding.
    if rb>=6: a=a[~a.position.eq("RB")]
...
def _fast_need_by_position(counts, slots):
    ...
    for code,pos in enumerate(["QB","RB","WR","TE","DL","DB"]):
        ...
        if pos=="WR": out[code]=8.0 if flex_need>0 else 6.0 if have==2 else 4.0 if have==3 else 0.5 if have==4 else -5.0
        elif pos=="RB": out[code]=5.0 if (flex_need>0 and have<3) else 3.0 if have==2 else 0.5 if have==3 else -3.5 if have==4 else -10.0
        elif pos=="TE": out[code]=-10.0 if have>=1 else 0.0
        elif pos=="QB": out[code]=-14.0 if have>=1 else 0.0
...
def simulate_mock_fast(fast, teams, slot, rounds, slots, randomness=12, model_user=True, seed=None):
    ...
    for overall in range(1,total+1):
        ...
        if owner==int(slot):
            ...
            if counts[wr]<3: construction[wr]+=3.0
            elif counts[wr]==3: construction[wr]+=1.5
            if counts[rb]>=5: construction[rb]-=3.0
...
st.title("🏈 Fantasy Edge V7.2 — Roster Flexibility")
st.caption("V7.2 • Roster Flexibility Validation • frozen projection/regression + grading • Yahoo + IDP")
st.caption("Build ID: V7.2-RF-20260813")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.2 validation report")
            st.caption("V7.2 keeps the V7.1 identity fixes but removes the fixed roster recipe so QB2, TE2, RB5 and WR5 compete on marginal value. Projection/regression and grading remain frozen.")
            ...
            st.download_button("⬇️ Download V7.2 diagnostic CSV",csv,"fantasy_edge_v7_2_diagnostics.csv","text/csv")
