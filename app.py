...
st.set_page_config(page_title=f"Fantasy Edge V{APP_VERSION} — Startup Bypass", page_icon="🏈", layout="wide")
st.markdown("""
<style>
...
st.caption("⚡ Fast local startup • cloud checks deferred • page switches rerun only the selected page")
...
if _refresh_live:
    _v802_progress.info("Refresh step 6/6: saving the refreshed player board")
    try:
...
try:
    ...
    if _refresh_live or not _v779_canonical_ready:
        _v755_ps=board.apply(_v755_player_state_row,axis=1)
        for _c in _v755_ps.columns: board[_c]=_v755_ps[_c]
        _v755_usage=board.apply(_v755_usage_signal,axis=1)
        board["fe_opportunity_score"]=[x[0] for x in _v755_usage]
        board["fe_opportunity_trend"]=[x[1] for x in _v755_usage]
...
if _refresh_live:
    try:
        _v779_blob=_v778_snapshot_encode(board)
        if _v779_blob:
            state["_weekly_board_snapshot_gz"]=_v779_blob
            ...
            _v778_snapshot_blob=_v779_blob
            _v778_snapshot_board=board.copy()
