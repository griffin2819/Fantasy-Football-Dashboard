...
st.title("🏈 Fantasy Edge V7.30 — VORP Weight Calibration")
st.caption("V7.30 • V7.24 champion retained • matched VORP-weight calibration")
st.caption("Build ID: V7.30-VWC-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v730_live_slot")
    ...
    if "v730_target_queue" not in st.session_state:
        st.session_state.v730_target_queue={}
    queue=st.session_state.v730_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v730_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.30 keeps the V7.24 champion as the live/default model. The matched calibration holds replacement fixed and tests VORP 9×, 7×, 6×, 5× and 4× on identical seeds.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.30 robustness report")
        ...
        st.markdown("### 🧪 V7.30 champion baseline validation")
        st.info("V7.30 keeps the V7.24 champion unchanged in the standard test. Alternative VORP weights run only in the matched calibration matrix.")
        ...
        if "v730_independent_results" not in st.session_state:
            st.session_state.v730_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v730_independent_results"),pd.DataFrame) else "ready to run"))
        if "v730_independent_summary" not in st.session_state:
            st.session_state.v730_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v730_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v730_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v730_run")
        clear_iv=clear_col.button("Clear independent results",key="v730_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v730_independent_results=iv
                st.session_state.v730_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v730_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v730_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.30 Champion Baseline Check")
            st.caption("The standard independent test still uses the unchanged V7.24 champion. Run the VORP-only matched calibration below to compare 9×, 7×, 6×, 5× and 4×.")
            ...
            st.download_button(
                "⬇️ Download V7.30 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_30_champion_baseline.csv",
                "text/csv",
                key="v730_baseline_download"
            )
        ...
        st.markdown("### ⚖️ V7.30 VORP Weight Calibration")
        st.caption(
            "The live/default model remains the V7.24 champion. This matrix holds the current 38/55/7 replacement baseline fixed "
            "and tests only the VORP multiplier on identical seeds."
        )

        if "v730_calibration_results" not in st.session_state:
            st.session_state.v730_calibration_results=None
        if "v730_calibration_summary" not in st.session_state:
            st.session_state.v730_calibration_summary=None

        cal_sims=st.number_input(
            "Calibration simulations per slot × room",
            min_value=2,max_value=10,value=3,step=1,key="v730_cal_sims",
            help="Use 3 for screening. If a new VORP weight wins, rerun at 10 before promotion."
        )
        ca,cb=st.columns(2)
        run_cal=ca.button("Run VORP weight calibration",type="primary",key="v730_cal_run")
        clear_cal=cb.button("Clear VORP calibration",key="v730_cal_clear")
            ...
            for ds in cal_slots:
                for room_name in cal_rooms:
                    for j in range(int(cal_sims)):
                        seed=930000+int(ds)*10000+j*31+sum(ord(c) for c in room_name)
                        ...
                        for name,weight in configs:
                            ...
                            r13=fe[pd.to_numeric(fe.mock_round,errors="coerce").fillna(99).between(1,3)]
                            ...
                            row={
                                ...
                                "R1_3_QB_count":int((r13.position=="QB").sum()),
                                "R1_3_RB_count":int((r13.position=="RB").sum()),
                                "R1_3_WR_count":int((r13.position=="WR").sum()),
                                "R1_3_TE_count":int((r13.position=="TE").sum()),
                                ...
                                "R7_10_RB_count":int((r710.position=="RB").sum()),
                                "R7_10_WR_count":int((r710.position=="WR").sum()),
                            }
                            ...
                            for phase,key in [
                                ...
                                row[f"{key}_market_gap"]=(
                                    float(fep.loc[phase,"Avg_market_delta"])
                                    -float(cop.loc[phase,"Avg_market_delta"])
                                )
                            ...
                            if done_cal==1 or done_cal%25==0 or done_cal==total_cal:
                                status_cal.caption(f"VORP calibration: {done_cal}/{total_cal}")

            cal=pd.DataFrame(rows_cal)
            st.session_state.v730_calibration_results=cal
            st.session_state.v730_calibration_summary={
                "seconds":time.perf_counter()-started,
                "sims":int(cal_sims),
                "variant_sims":len(cal)
            }
            status_cal.success(f"VORP calibration complete: {len(cal)} variant simulations.")
            prog_cal.empty()

        cal_saved=st.session_state.get("v730_calibration_results")
        if isinstance(cal_saved,pd.DataFrame) and len(cal_saved):
            meta_cal=st.session_state.get("v730_calibration_summary") or {}

            summary=cal_saved.groupby(["Config","VORP_weight"],as_index=False).agg(
                ...
                R1_3_QB=("R1_3_QB_count","mean"),
                R1_3_RB=("R1_3_RB_count","mean"),
                R1_3_WR=("R1_3_WR_count","mean"),
                R1_3_TE=("R1_3_TE_count","mean"),
            ...
            control=summary[summary.Config.eq("VORP 9× CURRENT")].iloc[0]
            ...
            summary["Calibration_score"]=(
                summary.Overall_gap
                +.25*(summary.R4_6_gap-control.R4_6_gap)
                -.20*summary.Non_mid_leakage
            )

            summary=summary.sort_values(
                ["Calibration_score","Overall_gap","R4_6_gap"],ascending=False
            ).reset_index(drop=True)
            summary["Rank"]=np.arange(1,len(summary)+1)

            disp=summary[[
                "Rank","Config","Overall_gap","FE_grade","Win_rate","Market_gap",
                "R1_3_gap","R4_6_gap","R7_10_gap","R11_plus_gap",
                "R1_3_QB","R1_3_RB","R1_3_WR","R1_3_TE",
                "R4_6_QB","R4_6_RB","R4_6_WR","R4_6_TE",
                "Avg_reach","Avg_faller","Roster_balance","Non_mid_leakage"
            ]].copy()
            ...
            c2.metric("Leader overall gap",f"{winner.Overall_gap:+.3f}",
                      f"{winner.Overall_gap-control.Overall_gap:+.3f} vs 9×")
            c3.metric("Leader R4-6 gap",f"{winner.R4_6_gap:+.3f}",
                      f"{winner.R4_6_gap-control.R4_6_gap:+.3f}")
            c4.metric("Leader R4-6 WRs",f"{winner.R4_6_WR:.2f}/draft",
                      f"{winner.R4_6_WR-control.R4_6_WR:+.2f}")
            ...
            st.markdown("#### VORP calibration by draft slot")
            st.dataframe(slot_cfg.round(3),use_container_width=True,hide_index=True)

            st.success(
                f"Saved VORP calibration: {len(cal_saved)} variant simulations"
                + (f" • {meta_cal.get('seconds',0):.1f}s" if meta_cal else "")
            )
            if int(meta_cal.get("sims",0))<10:
                st.info(
                    f"Screening leader: {winner.Config}. If it beats 9× on overall gap and R4-6 "
                    "without meaningful R1-3/R7+ leakage, rerun at 10 simulations per slot × room."
                )
            else:
                if winner.Config=="VORP 9× CURRENT":
                    st.success("Full calibration keeps VORP at 9×.")
                else:
                    st.success(
                        f"Full matched calibration leader: {winner.Config}. "
                        "Use a clean champion-validation build before promotion."
                    )

            st.download_button(
                "⬇️ Download V7.30 VORP weight calibration CSV",
                cal_saved.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_30_vorp_weight_calibration.csv",
                "text/csv",
                key="v730_cal_download"
            )
