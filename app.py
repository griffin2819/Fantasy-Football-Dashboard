...
def prepare_user_draft_candidates(avail, roster, round_no, current_pick, slot, teams, slots, randomness):
    ...
    if counts.get("WR",0)<3:
        x.loc[x.position.eq("WR"),"_construction_bonus"]+=5.0
    elif counts.get("WR",0)==3:
        x.loc[x.position.eq("WR"),"_construction_bonus"]+=4.0
    elif counts.get("WR",0)==4:
        x.loc[x.position.eq("WR"),"_construction_bonus"]+=1.0
    if counts.get("RB",0)>=4:
        x.loc[x.position.eq("RB"),"_construction_bonus"]-=4.0
...
def make_board(players, hist, market, ppr, pass_td, idp, teams, slots):
    ...
    _km=b.position.eq("K")
    if _km.any():
        _kmarket=pd.to_numeric(b.loc[_km,"market_pick"],errors="coerce")
