...
def mid_round_pipeline_audit_snapshot(fast, candidate_idx, eval_score, chosen_global,
...
st.title("🏈 Fantasy Edge V7.25 — R4-6 Decision Pipeline Audit")
st.caption("V7.25 • V7.24 10/52 champion frozen • R4-6 decision audit")
st.caption("Build ID: V7.25-MRPA-20260814")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v725_live_slot")
    ...
    if "v725_target_queue" not in st.session_state:
        st.session_state.v725_target_queue={}
    queue=st.session_state.v725_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v725_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.25 is diagnostic only. The V7.24 10/52 champion is frozen while R4-6 selections are audited for position allocation, reaching, passed fallers, and ranking disagreement.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.25 robustness report")
        ...
        st.markdown("### 🧪 V7.25 R4-6 pipeline validation")
        st.info("V7.25 keeps the V7.24 champion frozen. The independent test now records detailed R4-6 decision diagnostics without changing selections.")
        ...
        if "v725_independent_results" not in st.session_state:
            st.session_state.v725_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v725_independent_results"),pd.DataFrame) else "ready to run"))
        if "v725_independent_summary" not in st.session_state:
            st.session_state.v725_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v725_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v725_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v725_run")
        clear_iv=clear_col.button("Clear independent results",key="v725_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v725_independent_results=iv
                st.session_state.v725_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v725_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v725_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🔎 V7.25 R4-6 Decision Pipeline Audit")
            st.caption("Diagnostic only. V7.24 10/52 champion logic is frozen. This identifies whether the mid-round deficit comes from position allocation, reaches, passed fallers, or player-ranking disagreement.")

            m1,m2,m3,m4=st.columns(4)
            m1.metric("R4-6 selected reach",f"{iv_saved.MR_avg_selected_reach.mean():.2f}")
            m2.metric("Market gap vs best available",f"{iv_saved.MR_avg_market_gap_vs_market_best.mean():+.2f} picks")
            m3.metric("FE eval edge vs market-best",f"{iv_saved.MR_avg_eval_edge_vs_market_best.mean():+.2f}")
            m4.metric("Passed faller size",f"{iv_saved.MR_passed_faller_amount.mean():.2f} picks")

            m5,m6=st.columns(2)
            m5.metric("Market-best alternative survives",f"{iv_saved.MR_market_best_survival_actual.mean():.0%}")
            m6.metric("Passed faller survives",f"{iv_saved.MR_passed_faller_survival_actual.mean():.0%}")
            ...
            for ds,g in iv_saved.groupby("Draft_slot"):
                slot_rows.append({
                    ...
                    "Market gap vs best":g.MR_avg_market_gap_vs_market_best.mean(),
                    "Eval edge vs market-best":g.MR_avg_eval_edge_vs_market_best.mean(),
                    "Passed faller":g.MR_passed_faller_amount.mean(),
                    "Passed faller survives":g.MR_passed_faller_survival_actual.mean(),
                })
            st.markdown("#### R4-6 behavior by draft slot")
            st.dataframe(pd.DataFrame(slot_rows).round(2),use_container_width=True,hide_index=True)

            # Automatic diagnosis.
            market_gap=float(iv_saved.MR_avg_market_gap_vs_market_best.mean())
            eval_edge=float(iv_saved.MR_avg_eval_edge_vs_market_best.mean())
            faller=float(iv_saved.MR_passed_faller_amount.mean())
            fall_surv=float(iv_saved.MR_passed_faller_survival_actual.mean())
            sel_reach=float(iv_saved.MR_avg_selected_reach.mean())
            flags=[]
            if sel_reach>4.0: flags.append("excess mid-round reaching")
            if market_gap>5.0 and eval_edge>5.0: flags.append("player-ranking disagreement with market")
            if faller>6.0 and fall_surv<.60: flags.append("valuable fallers being passed and disappearing")
            # Largest positional mismatch.
            diffs={}
            for p in ["QB","RB","WR","TE"]:
                diffs[p]=float(iv_saved[f"MR_Selected_{p}_share"].mean()-iv_saved[f"MR_MarketBest_{p}_share"].mean())
            over=max(diffs,key=diffs.get); under=min(diffs,key=diffs.get)
            if diffs[over]>.15 or diffs[under]<-.15:
                flags.append(f"position allocation mismatch ({over} over-selected / {under} under-selected)")
            if flags:
                st.warning("R4-6 audit flags: "+"; ".join(flags)+".")
            else:
                st.info("No single broad R4-6 mechanism dominates; inspect slot and position tables before changing the champion.")
            ...
            st.download_button(
                "⬇️ Download V7.25 R4-6 pipeline audit CSV",
                iv_csv,
                "fantasy_edge_v7_25_mid_round_pipeline_audit.csv",
                "text/csv",
                key="v725_download"
            )
