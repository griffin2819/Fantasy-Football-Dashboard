...
st.title("🏈 Fantasy Edge V7.99 — Round-3 Downstream Mechanism Audit")
st.caption("V7.99 • V7.93 frozen champion • Round-3 downstream mechanism")
st.caption("Build ID: V7.99-R3-DOWNSTREAM-MECHANISM-20260818")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V7.99 — Round-3 Downstream Mechanism Audit")
        st.caption(
            "Measurement only. V7.93 remains frozen champion. "
            "After V7.98 showed that local score+VORP superiority can still damage the full draft, "
            "V7.99 studies Round 3 together with downstream R4–6 construction rather than building another immediate override."
        )

        if "v799_results" not in st.session_state:
            st.session_state.v799_results=None

        sims=st.number_input(
            "V7.99 simulations per slot × room",
            min_value=5,max_value=20,value=10,step=5,key="v799_sims",
            help="Default: 12 slots × 5 rooms × 10 = 600 fresh V7.93 champion drafts."
        )
        a,b=st.columns(2)
        run=a.button("Run V7.99 Round-3 downstream audit",type="primary",key="v799_run")
        clear=b.button("Clear V7.99 audit",key="v799_clear")
        if clear:
            st.session_state.v799_results=None
            st.rerun()
            ...
            for ds in slots_to_test:
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=999000+int(ds)*100000+j*389+sum(ord(c) for c in room_name)
                        ...
                        if len(r3):
                            r=r3.iloc[0]
                            audit=int(pd.to_numeric(pd.Series([r.get("R799_Audit",0)]),errors="coerce").fillna(0).iloc[0])
                            if audit==1:
                                ...
                                # Actual downstream champion construction through R6.
                                ...
                                r46_players=" | ".join(r46.sort_values("mock_round")["player"].astype(str).tolist())
                                ...
                                r46_market=(
                                    pd.to_numeric(r46.get("market_pick",pd.Series(dtype=float)),errors="coerce")
                                    - pd.to_numeric(r46.get("mock_pick",pd.Series(dtype=float)),errors="coerce")
                                ).mean()

                                row={
                                    "Draft_slot":int(ds),"Room":room_name,"Simulation":j+1,"Seed":seed,
                                    "Round3_pick":champ_pick,
                                    "Champion_player":str(r.get("player","")),
                                    "Champion_position":str(r.get("position","")),
                                    "Champion_market_pick":champ_market,
                                    "Champion_market_delta":champ_market-champ_pick if pd.notna(champ_market) and pd.notna(champ_pick) else np.nan,
                                    ...
                                    "R4_6_players":r46_players,
                                    "R4_6_avg_VORP":r46_vorp,
                                    "R4_6_avg_draft_score":r46_score,
                                    "R4_6_avg_market_delta":r46_market,
                                }
                        ...
                        if done%100==0 or done==total:
                            status.caption(f"V7.99: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)
            st.session_state.v799_results=df
            status.success(f"V7.99 complete: {len(df)} Round-3 downstream states audited.")
            prog.empty()

        df=st.session_state.get("v799_results")
        if isinstance(df,pd.DataFrame) and len(df):
            for c in [
                "Champion_VORP","Champion_draft_score",
                "Alt1_vorp","Alt1_draft_score","Alt1_market_distance",
                "R4_6_avg_VORP","R4_6_avg_draft_score","R4_6_avg_market_delta"
            ]:
            ...
            df["Alt1_le10"]=df.Alt1_dominates & (df.Alt1_market_distance<=10)

            room=df.groupby("Room",as_index=False).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Alt1_dominates","mean"),
                Le10_rate=("Alt1_le10","mean"),
                Avg_alt_VORP_change=("Alt1_VORP_change","mean"),
                Avg_alt_score_change=("Alt1_score_change","mean"),
                Avg_R4_6_VORP=("R4_6_avg_VORP","mean"),
                Avg_R4_6_score=("R4_6_avg_draft_score","mean"),
                Avg_R4_6_market_delta=("R4_6_avg_market_delta","mean"),
            )
            st.markdown("#### Round-3 mechanism by room")
            ...
            st.markdown("#### Round-3 mechanism by draft slot")
            st.dataframe(slot.round(4),use_container_width=True,hide_index=True)

            seq=df.groupby(
                ["Champion_position","R4_6_position_sequence"],as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Alt1_dominates","mean"),
                Le10_rate=("Alt1_le10","mean"),
            ...
            st.markdown("#### R3 choice → R4–6 sequence")
            st.dataframe(seq.head(50).round(4),use_container_width=True,hide_index=True)

            roster=df.groupby(
                ["QB_count_pre","RB_count_pre","WR_count_pre","TE_count_pre","Champion_position"],
                as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Alt1_dominates","mean"),
            ...
            st.markdown("#### Post-R1–2 roster-state → downstream profile")
            st.dataframe(roster.head(50).round(4),use_container_width=True,hide_index=True)

            paths=df.groupby(
                ["Champion_player","Champion_position","Alt1_player","Alt1_position"],as_index=False
            ).agg(
                Decisions=("Champion_player","count"),
                Dominance_rate=("Alt1_dominates","mean"),
                Le10_rate=("Alt1_le10","mean"),
            ...
            st.markdown("#### Most repeated Round-3 paths with downstream context")
            st.dataframe(paths.head(50).round(4),use_container_width=True,hide_index=True)

            st.info(
                "V7.99 is measurement-only. The goal is to identify whether any Round-3 local weakness also corresponds to a repeatable downstream R4–6 construction weakness. "
                "Do not build another score+VORP override unless both the local and downstream mechanism agree."
            )

            st.download_button(
                "⬇️ Download V7.99 Round-3 downstream mechanism audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_99_round3_downstream_mechanism_audit.csv",
                "text/csv",key="v799_download"
            )
