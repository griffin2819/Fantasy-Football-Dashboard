...
st.title("🏈 Fantasy Edge V7.37 — Pick 7 8/35 Pooled Robustness Validation")
st.caption("V7.37 • V7.24 champion retained • Pick 7 8/35 pooled robustness validation")
st.caption("Build ID: V7.37-P7835PRV-20260815")
...
with tabs[1]:
    ...
    live_slot=st.number_input("Your Yahoo draft slot (for pick-timing estimates)",1,int(teams),min(int(state.get("mock",{}).get("draft_slot",1)),int(teams)),key="v737_live_slot")
    ...
    if "v737_target_queue" not in st.session_state:
        st.session_state.v737_target_queue={}
    queue=st.session_state.v737_target_queue
    ...
    for name in list(queue):
        ...
        if len(rr):
            ...
            if action!="WAIT / TARGET NEXT PICK":
                queue.pop(name,None)
    st.session_state.v737_target_queue=queue
...
with tabs[2]:
    ...
    st.caption("V7.37 keeps the V7.24 9× champion live/default. The Pick-7 8/35 intercept runs only inside the pooled validation.")
    ...
    if mode=="Interactive mock":
        ...
        if st.button("▶️ Run benchmark"):
            ...
            st.markdown("### 🔬 V7.37 robustness report")
        ...
        st.markdown("### 🧪 V7.37 champion baseline validation")
        st.info("V7.37 is validation only. It makes no live model change and decides whether the Pick-7 8/35 intercept is robust enough to promote or should be retired.")
        ...
        if "v737_independent_results" not in st.session_state:
            st.session_state.v737_independent_results=None
        st.caption("Independent test status: " + ("results saved" if isinstance(st.session_state.get("v737_independent_results"),pd.DataFrame) else "ready to run"))
        if "v737_independent_summary" not in st.session_state:
            st.session_state.v737_independent_summary=None

        iv_slots=st.multiselect("Draft slots",[1,3,5,7,9,12],default=[1,3,7,12],key="v737_slots")
        ...
        iv_sims=st.number_input("Simulations per slot × room",5,50,10,5,key="v737_sims")

        run_col,clear_col=st.columns(2)
        run_iv=run_col.button("Run independent stress test",type="primary",key="v737_run")
        clear_iv=clear_col.button("Clear independent results",key="v737_clear")
        ...
        if run_iv:
            if not iv_slots or not iv_rooms:
                ...
                st.session_state.v737_independent_results=iv
                st.session_state.v737_independent_summary={
                    "tests":len(iv),
        ...
        iv_saved=st.session_state.get("v737_independent_results")
        if isinstance(iv_saved,pd.DataFrame) and len(iv_saved):
            meta=st.session_state.get("v737_independent_summary") or {}
            st.success(
            ...
            st.markdown("### 🧪 V7.37 Champion Baseline Check")
            st.caption("The standard independent test uses the unchanged V7.24 9× champion. V7.37 runs a large pooled robustness validation of the Pick-7 8/35 intercept.")
            ...
            st.download_button(
                "⬇️ Download V7.37 champion baseline CSV",
                iv_csv,
                "fantasy_edge_v7_37_champion_baseline.csv",
                "text/csv",
                key="v737_baseline_download"
            )
        ...
        st.markdown("### 🧱 V7.37 — Pick 7 8/35 Pooled Robustness Validation")
        st.caption(
            "Large final robustness test. The V7.24 9× champion stays live/default. "
            "The only candidate change is the Pick-7, R4-6 WR 8/35 intercept."
        )

        if "v737_validation_results" not in st.session_state:
            st.session_state.v737_validation_results=None
        if "v737_validation_meta" not in st.session_state:
            st.session_state.v737_validation_meta=None

        val_sims=st.number_input(
            "Robustness simulations per room",
            min_value=10,max_value=40,value=25,step=5,key="v737_val_sims",
            help="Default 25 × five room profiles = 125 matched Pick-7 draft pairs / 250 model drafts."
        )
        va,vb=st.columns(2)
        run_val=va.button("Run pooled Pick 7 robustness validation",type="primary",key="v737_val_run")
        clear_val=vb.button("Clear pooled validation",key="v737_val_clear")
        ...
        if run_val:
            ...
            rows=[]
            total=len(rooms)*int(val_sims)*2
            done=0
            prog=st.progress(0)
            status=st.empty()
            ...
            for room_name in rooms:
                for j in range(int(val_sims)):
                    seed=937000+j*53+sum(ord(c) for c in room_name)

                    co=simulate_mock_fast(
                        fast_champ,int(teams),7,15,state["roster_slots"],
                    ...
                    for config,enabled in [
                        ("V7.24 CHAMPION",False),
                        ("V7.37 PICK7 8/35",True),
                    ]:
                        fe=simulate_mock_fast(
                            fast_champ,int(teams),7,15,state["roster_slots"],
                        ...
                        r710=fe[pd.to_numeric(fe.mock_round,errors="coerce").fillna(99).between(7,10)]
                        ...
                        row={
                            ...
                            "Starter_completion_difference":fc["starter_completion"]-cc["starter_completion"],
                            "Availability_difference":fc["availability"]-cc["availability"],
                            "Avg_reach":fc["avg_reach"],
                            "Avg_faller":fc["avg_faller"],
                            "R4_6_QB":int((r46.position=="QB").sum()),
                            "R4_6_RB":int((r46.position=="RB").sum()),
                            "R4_6_WR":int((r46.position=="WR").sum()),
                            "R4_6_TE":int((r46.position=="TE").sum()),
                            "R7_10_QB":int((r710.position=="QB").sum()),
                            "R7_10_RB":int((r710.position=="RB").sum()),
                            "R7_10_WR":int((r710.position=="WR").sum()),
                            "R7_10_TE":int((r710.position=="TE").sum()),
                        }
                        for phase,key in [
                            ("R1-3","R1_3"),("R4-6","R4_6"),
                        ...
                        if done==1 or done%25==0 or done==total:
                            status.caption(f"Pooled validation: {done}/{total}")

            val=pd.DataFrame(rows)
            st.session_state.v737_validation_results=val
            st.session_state.v737_validation_meta={
                "seconds":time.perf_counter()-started,
                "sims":int(val_sims),
                "model_drafts":len(val),
                "matched_pairs":len(val)//2
            }
            status.success(f"Pooled robustness validation complete: {len(val)//2} matched pairs.")
            prog.empty()

        val=st.session_state.get("v737_validation_results")
        meta=st.session_state.get("v737_validation_meta") or {}

        if isinstance(val,pd.DataFrame) and len(val):
            champ="V7.24 CHAMPION"
            cand="V7.37 PICK7 8/35"

            summary=val.groupby("Config",as_index=False).agg(
                Overall_gap=("Difference","mean"),
                FE_grade=("Overall_grade","mean"),
                ...
                Avg_reach=("Avg_reach","mean"),
                Avg_faller=("Avg_faller","mean"),
                ...
                R4_6_QB=("R4_6_QB","mean"),
                R4_6_RB=("R4_6_RB","mean"),
                R4_6_WR=("R4_6_WR","mean"),
                R4_6_TE=("R4_6_TE","mean"),
                R7_10_QB=("R7_10_QB","mean"),
                R7_10_RB=("R7_10_RB","mean"),
                R7_10_WR=("R7_10_WR","mean"),
                R7_10_TE=("R7_10_TE","mean"),
                Tests=("Simulation","count")
            )

            a=summary[summary.Config.eq(champ)].iloc[0]
            b=summary[summary.Config.eq(cand)].iloc[0]

            compare=pd.DataFrame([
                {"Metric":"Overall gap","Champion":a.Overall_gap,"8/35 candidate":b.Overall_gap,"Change":b.Overall_gap-a.Overall_gap},
                {"Metric":"R1-3 gap","Champion":a.R1_3_gap,"8/35 candidate":b.R1_3_gap,"Change":b.R1_3_gap-a.R1_3_gap},
                {"Metric":"R4-6 gap","Champion":a.R4_6_gap,"8/35 candidate":b.R4_6_gap,"Change":b.R4_6_gap-a.R4_6_gap},
                {"Metric":"R7-10 gap","Champion":a.R7_10_gap,"8/35 candidate":b.R7_10_gap,"Change":b.R7_10_gap-a.R7_10_gap},
                {"Metric":"R11+ gap","Champion":a.R11_plus_gap,"8/35 candidate":b.R11_plus_gap,"Change":b.R11_plus_gap-a.R11_plus_gap},
                {"Metric":"Roster balance","Champion":a.Roster_balance,"8/35 candidate":b.Roster_balance,"Change":b.Roster_balance-a.Roster_balance},
            ])
            st.dataframe(compare.round(3),use_container_width=True,hide_index=True)

            piv=val.pivot_table(
                index=["Room","Simulation","Seed"],columns="Config",
                values=["Difference","R4_6_market_gap","R7_10_market_gap","R11_plus_market_gap"],
                aggfunc="first"
            )
            overall_delta=piv[("Difference",cand)]-piv[("Difference",champ)]
            r46_delta=piv[("R4_6_market_gap",cand)]-piv[("R4_6_market_gap",champ)]
            r710_delta=piv[("R7_10_market_gap",cand)]-piv[("R7_10_market_gap",champ)]
            r11_delta=piv[("R11_plus_market_gap",cand)]-piv[("R11_plus_market_gap",champ)]

            c1,c2,c3,c4=st.columns(4)
            c1.metric("Paired overall wins",f"{(overall_delta>0).mean():.1%}")
            c2.metric("Paired R4-6 improves",f"{(r46_delta>0).mean():.1%}")
            c3.metric("Paired R7-10 improves",f"{(r710_delta>0).mean():.1%}")
            c4.metric("Paired equal/better overall",f"{(overall_delta>=0).mean():.1%}")

            room_rows=[]
            for room_name,g in val.groupby("Room"):
                p=g.pivot(index=["Simulation","Seed"],columns="Config",
                          values=["Difference","R4_6_market_gap","R7_10_market_gap","R11_plus_market_gap"])
                od=p[("Difference",cand)]-p[("Difference",champ)]
                md=p[("R4_6_market_gap",cand)]-p[("R4_6_market_gap",champ)]
                rd=p[("R7_10_market_gap",cand)]-p[("R7_10_market_gap",champ)]
                ld=p[("R11_plus_market_gap",cand)]-p[("R11_plus_market_gap",champ)]
                room_rows.append({
                    "Room":room_name,
                    "Overall change":od.mean(),
                    "Overall win rate":(od>0).mean(),
                    "R4-6 change":md.mean(),
                    "R4-6 improve rate":(md>0).mean(),
                    "R7-10 change":rd.mean(),
                    "R11+ change":ld.mean(),
                })
            room_df=pd.DataFrame(room_rows)
            st.markdown("#### Robustness by room profile")
            st.dataframe(room_df.round(3),use_container_width=True,hide_index=True)

            # Promotion gate: require pooled improvement, no meaningful downstream regression,
            # and broad room support rather than one-room dependence.
            positive_rooms=int((room_df["Overall change"]>0).sum())
            nonnegative_mid_rooms=int((room_df["R4-6 change"]>=0).sum())
            worst_room=float(room_df["Overall change"].min())

            promote=(
                b.Overall_gap>a.Overall_gap
                and b.R4_6_gap>a.R4_6_gap
                and (b.R7_10_gap-a.R7_10_gap)>-0.30
                and (b.R11_plus_gap-a.R11_plus_gap)>-0.30
                and (overall_delta>0).mean()>=.50
                and (r46_delta>0).mean()>=.50
                and positive_rooms>=4
                and nonnegative_mid_rooms>=4
                and worst_room>-0.20
            )

            st.caption(
                f"Promotion checks: positive overall rooms {positive_rooms}/5 • non-negative R4-6 rooms {nonnegative_mid_rooms}/5 • "
                f"worst room overall change {worst_room:+.3f}"
            )

            if promote:
                st.success(
                    "PROMOTION GATE PASSED: Pick-7 8/35 is positive across the pooled sample, improves the target R4-6 phase, "
                    "avoids meaningful downstream regression, and is not dependent on a single room profile. V7.38 can promote it."
                )
            else:
                st.warning(
                    "PROMOTION GATE NOT PASSED. Keep the V7.24 champion and retire the Pick-7 8/35 intercept rather than tuning further around noise."
                )

            st.success(
                f"Saved V7.37 pooled validation: {meta.get('matched_pairs',len(val)//2)} matched pairs"
                + (f" • {meta.get('seconds',0):.1f}s" if meta else "")
            )
            st.download_button(
                "⬇️ Download V7.37 pooled Pick 7 validation CSV",
                val.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v7_37_pick7_835_pooled_validation.csv",
                "text/csv",key="v737_download"
            )
