...
def v939_shared_candidate_rank(avail, roster, round_no, current_pick, next_pick, slots, randomness=6):
    ...
    x.loc[~x["post_pick_feasible"],"execution_score"]=-1e12
...
def _sim_user_pick_same_engine(avail,roster,round_no,current_pick,next_pick,slots,randomness=6):
    """Use the exact Live/Mock shared candidate engine for automated user picks."""
    ranked,_=v939_shared_candidate_rank(
        avail.copy(),roster.copy(),int(round_no),int(current_pick),int(next_pick),
        slots,int(randomness)
    )
    if ranked is None or len(ranked)==0:
        raise RuntimeError("Production candidate engine returned no candidates.")
...
def simulate_current_fantasy_edge_once(board, teams, slot, rounds, slots, randomness, seed):
    ...
    for overall in range(1,int(teams)*int(rounds)+1):
        ...
        if int(owner)==int(slot):
            ...
            _sim_next=next_user_pick(int(overall),int(slot),int(teams),int(rounds))
            choice=_sim_user_pick_same_engine(
                avail.reset_index(drop=True),roster,int(rnd),int(overall),int(_sim_next),
                slots,int(randomness)
            )
    ...
    return {
        ...
        "certified_17_player_roster":bool(len(roster)==17),
    }
        ...
        if len(sim_df):
            ...
            if len(sim_df)>=target_sims:
                _cert={
                    ...
                    "17 selections per roster": bool((pd.to_numeric(sim_df["roster_size"],errors="coerce").fillna(0)==17).all()),
