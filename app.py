...
st.title("🏈 Fantasy Edge V8.10 — Identity-Suppressed Residual Opportunity Audit")
st.caption("V8.10 • V8.05 frozen champion • residual opportunity discovery")
st.caption("Build ID: V8.10-IDENTITY-SUPPRESSED-RESIDUAL-AUDIT-20260819")
...
with tabs[2]:
    ...
    if mode=="Interactive mock":
        ...
        st.markdown("### 🔬 V8.10 — Residual Opportunity Audit with Identity Suppression")
        st.caption(
            "Measurement only. V8.05 remains the frozen production champion. "
            "V8.10 reruns the exact true-board whole-draft audit, but automatically suppresses dominant alternative-player identities "
            "before ranking remaining opportunities. Certified R2, R3 and R7 plus protected R11–15 remain excluded."
        )

        if "v810_results" not in st.session_state:
            st.session_state.v810_results=None

        sims=st.number_input(
            "V8.10 simulations per slot × room",
            min_value=10,max_value=30,value=20,step=5,key="v810_sims",
            help="Default: 12 slots × 5 rooms × 20 = 1,200 fresh V8.05 champion drafts."
        )

        identity_threshold=st.selectbox(
            "Dominant identity suppression threshold",
            options=[0.20,0.25,0.30],
            index=0,
            format_func=lambda x:f"{x:.0%}",
            key="v810_identity_threshold",
            help="Pre-registered default: suppress any alternative player representing at least 20% of a round's exact-board ≤10 signal."
        )

        a,b=st.columns(2)
        run=a.button("Run V8.10 residual audit",type="primary",key="v810_run")
        clear=b.button("Clear V8.10 audit",key="v810_clear")
        ...
        if run:
            ...
            rows=[]; total=int(teams)*len(rooms)*int(sims); done=0
            prog=st.progress(0); status=st.empty()

            for ds in range(1,int(teams)+1):
                for room_name in rooms:
                    for j in range(int(sims)):
                        seed=8100000+ds*100000+j*457+sum(ord(c) for c in room_name)

                        d=simulate_mock_fast(
                            pool,int(teams),ds,15,state["roster_slots"],6.0,True,seed,room_name,
                            r5_dominance_override_enabled=False,
                            r2_dominance_override_enabled=False,
                            r3_dominance_override_enabled=False,
                            r11_13_offense_dominance_override_enabled=False,
                        ...
                        for _,r in d.iterrows():
                            if int(pd.to_numeric(pd.Series([r.get("R8063_Audit",0)]),errors="coerce").fillna(0).iloc[0])!=1:
                                continue

                            rnd=int(r.mock_round)
                            cv=pd.to_numeric(pd.Series([r.get("vorp",np.nan)]),errors="coerce").iloc[0]
                            cs=pd.to_numeric(pd.Series([r.get("draft_score",np.nan)]),errors="coerce").iloc[0]
                            av=pd.to_numeric(pd.Series([r.get("R8063_Alt1_vorp",np.nan)]),errors="coerce").iloc[0]
                            asc=pd.to_numeric(pd.Series([r.get("R8063_Alt1_draft_score",np.nan)]),errors="coerce").iloc[0]
                            amd=pd.to_numeric(pd.Series([r.get("R8063_Alt1_market_distance",np.nan)]),errors="coerce").iloc[0]

                            vg=av-cv if pd.notna(av) and pd.notna(cv) else np.nan
                            sg=asc-cs if pd.notna(asc) and pd.notna(cs) else np.nan
                            dom=bool(pd.notna(vg) and pd.notna(sg) and vg>0 and sg>0)
                            le10=bool(dom and pd.notna(amd) and amd<=10)
                            protected=(rnd in [2,3,7]) or rnd>=11

                            rows.append({
                                "Draft_slot":ds,"Room":room_name,"Simulation":j+1,"Seed":seed,
                                "Round":rnd,"Pick":r.get("mock_pick",np.nan),
                                "Champion_player":str(r.get("player","")),
                                "Champion_position":str(r.get("position","")),
                                "Best_alt_player":str(r.get("R8063_Alt1_player","")),
                                "Best_alt_position":str(r.get("R8063_Alt1_position","")),
                                "VORP_gap":vg,"Draft_score_gap":sg,
                                "Alt_market_distance":amd,
                                "Alt_dominates":dom,"Alt_dominates_le10":le10,
                                "Protected_round":protected,
                                "QB_count_pre":r.get("R8063_QB_count_pre",0),
                                "RB_count_pre":r.get("R8063_RB_count_pre",0),
                                "WR_count_pre":r.get("R8063_WR_count_pre",0),
                                "TE_count_pre":r.get("R8063_TE_count_pre",0),
                                "DL_count_pre":r.get("R8063_DL_count_pre",0),
                                "DB_count_pre":r.get("R8063_DB_count_pre",0),
                            })

                        done+=1
                        prog.progress(done/total)
                        if done==1 or done%200==0 or done==total:
                            status.caption(f"V8.10: {done}/{total} frozen-champion drafts")

            df=pd.DataFrame(rows)

            # Learn dominant alternative identities separately within each round.
            df["Identity_suppressed"]=False
            df["Dominant_identity_share"]=0.0
            df["Dominant_identity_name"]=""

            signal=df[df.Alt_dominates_le10].copy()
            for rnd,g in signal.groupby("Round"):
                vc=g.Best_alt_player.value_counts()
                if len(vc)==0:
                    continue
                shares=vc/len(g)
                dominant=set(shares[shares>=float(identity_threshold)].index.astype(str).tolist())
                if dominant:
                    mask=(df.Round==rnd)&df.Best_alt_player.astype(str).isin(dominant)&df.Alt_dominates_le10
                    df.loc[mask,"Identity_suppressed"]=True
                    for name in dominant:
                        share=float(shares.loc[name])
                        nm=(df.Round==rnd)&(df.Best_alt_player.astype(str)==name)&df.Alt_dominates_le10
                        df.loc[nm,"Dominant_identity_share"]=share
                    df.loc[mask,"Dominant_identity_name"]=df.loc[mask,"Best_alt_player"].astype(str)

            st.session_state.v810_results=df
            status.success(f"V8.10 complete: {len(df):,} exact-board decisions audited.")
            prog.empty()

        df=st.session_state.get("v810_results")
        if isinstance(df,pd.DataFrame) and len(df):
            residual=df[
                df.Alt_dominates_le10 &
                (~df.Identity_suppressed) &
                (~df.Protected_round)
            ].copy()

            raw=df[df.Alt_dominates_le10 & (~df.Protected_round)].copy()

            raw_round=raw.groupby("Round",as_index=False).agg(
                Raw_signals=("Champion_player","count"),
                Raw_rooms=("Room","nunique"),
                Raw_slots=("Draft_slot","nunique")
            ) if len(raw) else pd.DataFrame(columns=["Round","Raw_signals","Raw_rooms","Raw_slots"])

            residual_round=residual.groupby("Round",as_index=False).agg(
                Residual_signals=("Champion_player","count"),
                Avg_VORP_gap=("VORP_gap","mean"),
                Avg_score_gap=("Draft_score_gap","mean"),
                Avg_market_distance=("Alt_market_distance","mean"),
                Rooms_with_signal=("Room","nunique"),
                Slots_with_signal=("Draft_slot","nunique"),
                Unique_alt_players=("Best_alt_player","nunique"),
                Unique_champion_players=("Champion_player","nunique"),
            ) if len(residual) else pd.DataFrame(
                columns=["Round","Residual_signals","Avg_VORP_gap","Avg_score_gap","Avg_market_distance",
                         "Rooms_with_signal","Slots_with_signal","Unique_alt_players","Unique_champion_players"]
            )

            ranked=raw_round.merge(residual_round,on="Round",how="left")
            for c in ["Residual_signals","Rooms_with_signal","Slots_with_signal","Unique_alt_players","Unique_champion_players"]:
                if c in ranked.columns:
                    ranked[c]=ranked[c].fillna(0)
            ranked["Residual_share"]=ranked.Residual_signals/ranked.Raw_signals.replace(0,np.nan)
            ranked["Breadth_factor"]=(
                (ranked.Rooms_with_signal/5.0) *
                (ranked.Slots_with_signal/12.0)
            )
            ranked["Player_breadth_factor"]=np.minimum(ranked.Unique_alt_players/4.0,1.0)
            ranked["Residual_opportunity_score"]=(
                ranked.Residual_signals/1200.0 *
                ranked.Avg_VORP_gap.fillna(0).clip(lower=0) *
                (ranked.Avg_score_gap.fillna(0).clip(lower=0)/10.0) *
                ranked.Breadth_factor *
                ranked.Player_breadth_factor
            )
            ranked=ranked.sort_values("Residual_opportunity_score",ascending=False)

            st.markdown("#### Identity-suppressed residual opportunity ranking")
            st.dataframe(ranked.round(4),use_container_width=True,hide_index=True)

            suppress=df[df.Identity_suppressed & df.Alt_dominates_le10].groupby(
                ["Round","Best_alt_player"],as_index=False
            ).agg(
                Suppressed_signals=("Champion_player","count"),
                Identity_share=("Dominant_identity_share","max"),
                Rooms=("Room","nunique"),
                Slots=("Draft_slot","nunique")
            ).sort_values(["Round","Suppressed_signals"],ascending=[True,False])

            st.markdown("#### Suppressed dominant player identities")
            st.dataframe(suppress.round(4),use_container_width=True,hide_index=True)

            if len(residual):
                mechanisms=residual.groupby(
                    ["Round","Champion_position","Best_alt_position"],as_index=False
                ).agg(
                    Signals=("Champion_player","count"),
                    Unique_alt_players=("Best_alt_player","nunique"),
                    Rooms=("Room","nunique"),
                    Slots=("Draft_slot","nunique"),
                    Avg_VORP_gap=("VORP_gap","mean"),
                    Avg_score_gap=("Draft_score_gap","mean"),
                    Avg_market_distance=("Alt_market_distance","mean")
                ).sort_values(["Signals","Unique_alt_players"],ascending=[False,False])
            else:
                mechanisms=pd.DataFrame()

            st.markdown("#### Residual generic mechanisms")
            st.dataframe(mechanisms.head(60).round(4),use_container_width=True,hide_index=True)

            viable=ranked[
                (ranked.Residual_signals>=20) &
                (ranked.Rooms_with_signal>=4) &
                (ranked.Slots_with_signal>=6) &
                (ranked.Unique_alt_players>=3)
            ]

            if len(viable):
                t=viable.iloc[0]
                st.warning(
                    f"Highest broad residual target after identity suppression: Round {int(t.Round)} "
                    f"with {int(t.Residual_signals)} residual signals, "
                    f"{int(t.Unique_alt_players)} alternative players, "
                    f"{int(t.Rooms_with_signal)}/5 rooms, and {int(t.Slots_with_signal)}/12 slots. "
                    "This earns a dedicated mechanism audit before any challenger."
                )
            else:
                st.success(
                    "No residual opportunity clears the generic breadth screen after dominant player identities are suppressed. "
                    "V8.05 may be near the point where additional local override rules are more likely to overfit than improve the engine."
                )

            st.info(
                "V8.10 is measurement-only. Identity suppression is used only for discovery ranking and does not alter production drafting behavior."
            )

            st.download_button(
                "⬇️ Download V8.10 identity-suppressed residual audit CSV",
                df.to_csv(index=False).encode("utf-8"),
                "fantasy_edge_v8_10_identity_suppressed_residual_opportunity_audit.csv",
                "text/csv",key="v810_download"
            )
